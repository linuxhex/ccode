...
/// v1.18: Added `shell_version`: the ccode-shell agent binary version, distinct
///        from `client_version` (the UI client's version). They coincide for the
///        TUI but differ for embedding clients like ccode-desktop.
...
/// OS-level sandbox state for a trace turn (local `ccode-sandbox`, not cloud sandbox).
...
pub struct PromptMetadata {
    ...
    /// The agent type / harness name for this session (e.g. "ccode-build", "codex").
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_type: Option<String>,
    /// Version of the ccode-shell agent binary that handled this turn
    /// (`ccode_version::VERSION`). Self-reported by the agent, so it reflects
    ...
    /// like ccode-desktop the bundled shell differs from the app version.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn cwd_round_trips() {
        let mut meta: PromptMetadata = serde_json::from_str(minimal_json()).unwrap();
        meta.cwd = Some("/root/code/ccode".into());
        let json = serde_json::to_string(&meta).unwrap();
        let deserialized: PromptMetadata = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.cwd.as_deref(), Some("/root/code/ccode"));
    }
    #[test]
