//! Pure data types for the ccode sampling / chat-completion API layer.
//!
...
//! ccode agent stack.  It intentionally contains **no I/O** (no HTTP clients,
//! no file system access) so it can be depended on by downstream crates
//! (e.g., `ccode-chat`) without pulling in the full `ccode-shell`.
