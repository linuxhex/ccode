...
// Re-export ToolDefinition and FunctionTool from ccode-tools.
...
pub use ccode_tools::types::definition::{FunctionTool, ToolDefinition};
...
pub struct Usage {
    ...
    /// ccode extension: request price in USD ticks (1 USD = 1e10 ticks).
...
/// custom header fields for ccode request tracking, similar to
...
pub struct CreateResponseWrapper {
    ...
    /// ccode-specific tool definitions that can't be expressed via
...
/// Wrapper around `MessagesRequest` that adds custom header fields for ccode
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "ccode" }).as_object().unwrap()))
                .is_none()
        );
    }
