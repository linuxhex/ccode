...
//!
//! TODO: Move from ccode-shell/src/sampling/error.rs
...
/// Flat error from the Ccode proxy/gateway: `{"code": "...", "error": "..."}`.
#[derive(Debug, Deserialize)]
...
pub fn status_user_message(status: StatusCode) -> String {
    match status.as_u16() {
        code @ 502..=504 => {
            format!("Ccode is temporarily unavailable. Please try again in a moment. (HTTP {code}).")
        }
        // Cloudflare edge codes (origin down / connect fail / timeout / …).
        code @ 520..=524 => {
            format!(
                "Connection to Ccode timed out or was interrupted. Please try again. (HTTP {code})."
            )
        }
...
<html lang="en-US">
<head><title>ccode.dev | 524: A timeout occurred</title></head>
