//! Headless markdown analysis sharing Ccode Build's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `ccode-markdown` so analysis matches what Ccode
//! Build actually renders 1:1.
//!
//! After parsing, Ccode applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Ccode Build uses to render markdown.
///
...
/// Offset event stream from Ccode's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Ccode Build's options; count elements and flag structural issues.
