//! Shared subprocess lifecycle ownership for ccode-build test harnesses.
//!
...
pub struct TestProcessTree {
    pid: u32,
    label: String,
    group: Option<ccode_tty::ProcessGroup>,
...
impl TestProcessTree {
    ...
    pub fn try_attach(pid: u32, label: impl Into<String>) -> io::Result<Self> {
        ...
        let mut group = ccode_tty::ProcessGroup::new()?;
    ...
    pub fn attach(pid: u32, label: impl Into<String>) -> Self {
        let label = label.into();
        let (group, attachment_error) = match ccode_tty::ProcessGroup::new() {
    ...
    fn from_group(pid: u32, label: String, group: ccode_tty::ProcessGroup) -> Self {
        Self {
            pid,
            label,
            group: Some(group),
