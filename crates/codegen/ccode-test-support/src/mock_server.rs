...
mod tests {
    ...
    async fn post_foreground(
        ...
        reqwest::Client::new()
            ...
            .header("x-ccode-req-id", request_id)
    ...
    fn explicit_request_headers_override_tool_count_heuristic() {
        ...
        headers.insert("x-ccode-req-id", "title-request".parse().unwrap());
        assert!(
            !overrides
        ...
        headers.insert("x-ccode-turn-idx", "1".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
        ...
        headers.insert("x-ccode-req-id", "".parse().unwrap());
        headers.insert("x-ccode-turn-idx", "".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
    ...
    async fn cancelling_replay_waits_for_primary_before_satisfaction() {
        ...
        let replay = tokio::spawn({
            ...
            async move {
                reqwest::Client::new()
                    .post(url)
                    .header("x-ccode-req-id", "cancel-replay")
                    .header("x-ccode-turn-idx", "1")
                    .json(&body)
                    .send()
                    .await
