//! Leader-mode (`ccode agent --leader stdio`) test harness.
//!
...
use ccode_acp::LineBufferedRead;

use crate::env::ccode_binary;
...
/// (version-skew) test. Falls back to [`ccode_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "CCODE_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`ccode_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "CCODE_BINARY_CLIENT";
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        if &*args.method == "ccode.dev/leader_reconnected" {
            self.capture
                .reconnected_count
                .fetch_add(1, Ordering::SeqCst);
        }
...
/// A `ccode agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderFixture {
    ...
    pub async fn start(
        ...
        Self::start_with_binary(&ccode_binary(), server, cwd, sandbox).await
    }
    ...
    async fn start_with_binary_timeout(
        ...
        let socket = sandbox.ccode_home().join("leader.sock");
        let lock = sandbox.ccode_home().join("leader.lock");
        ...
        cmd.envs(ccode_tty::pager_env())
            .env("CCODE_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("CCODE_CCODE_API_BASE_URL", server.url())
            .env("CCODE_MODELS_BASE_URL", server.url())
            .env("CCODE_FEEDBACK_BASE_URL", server.url())
            .env("CCODE_TRACE_UPLOAD_URL", server.url())
            .env("CCODE_API_KEY", "test-key-for-ci")
            .env("CCODE_LEADER_SOCKET", &socket)
            .env("RUST_LOG", "ccode_shell=debug");
        let log_path = sandbox.ccode_home().join("leader.log");
        ...
        let pid = child.id();
        let tree = match TestProcessTree::try_attach(pid, "persistent ccode test leader") {
            Ok(tree) => tree,
            Err(error) => {
                let _ = child.kill();
...
impl LeaderStdioClient {
    async fn spawn_with_binary_and_socket(
        ...
        let mut process = TestProcess::spawn(
            ...
            TestProcessConfig::new()
                .label("ccode leader stdio client")
                ...
                .env("CCODE_TRACE_UPLOAD_URL", server.url())
                .env("CCODE_API_KEY", "test-key-for-ci")
                .env("CCODE_LEADER_SOCKET", leader_socket)
                .env("RUST_LOG", "ccode_shell=debug"),
        )
        .map_err(|error| {
            io::Error::new(
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            .auth_methods
            .iter()
            .find(|method| &*method.id().0 == "ccode.api_key")
            .expect("ccode.api_key auth method");
        self.conn
            .authenticate(
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".ccode").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".ccode").join("leader.log")).unwrap_or_default()
}
