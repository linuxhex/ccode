//! ACP stdio clients for testing ccode sessions end-to-end: the typed
//! [`CcodeStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
/// Spawn `ccode agent stdio` with the sandbox's canonical hermetic environment.
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init_resp
            ...
            .unwrap_or_else(|| {
                ...
                panic!(
                    "expected auth method 'ccode.api_key' but got: {ids:?}\n\
...
/// Drives `ccode agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`CcodeStdioClient`] (`ClientSideConnection`,
