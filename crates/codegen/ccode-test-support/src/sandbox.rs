//! Hermetic filesystem and child-environment owner for ccode integration tests.
...
/// ccode network kill switches, and explicit overrides.
...
impl TestSandbox {
    ...
    /// Explicit ccode state root.
    pub fn ccode_home(&self) -> &Path {
        &self.ccode_home
    }
    ...
    pub fn git_command(&self) -> Command {
        ...
        cmd.stdin(Stdio::null()).envs(ccode_tty::pager_env());
        for &(key, value) in &ccode_tty::GIT_AUTH_SUPPRESSION_ENVS {
            cmd.env(key, value);
        }
    ...
    pub fn diagnostic_summary(&self) -> String {
        let mut summary = format!(
            "root={} home={} ccode_home={} workspace={} temp={}",
            self.root().display(),
            self.home.display(),
            self.ccode_home.display(),
...
impl TestSandboxBuilder {
    /// Wire ccode API, models, feedback, trace, conversation, and web traffic to
    ...
    pub fn build(self) -> TestSandbox {
        ...
        for path in [&home, &ccode_home, &workspace, &temp] {
        ...
        let mut env = baseline_env(&home, &ccode_home, &temp, &parent_cwd);
...
impl TestSandbox {
    fn init_git_workspace(&self) {
        ...
        run_git(self, &["config", "user.name", "Ccode Test"]);
...
fn apply_mock_url(env: &mut BTreeMap<OsString, OsString>, url: String) {
    ...
    env.insert("CCODE_API_KEY".into(), TEST_API_KEY.into());
}
...
fn baseline_env(
    ...
    baseline_env_from_parent(home, ccode_home, temp, parent_cwd, &parent_env)
}
...
fn baseline_env_from_parent(
    ...
    for (key, value) in [
        ("CCODE_TELEMETRY_ENABLED", "false"),
        ("CCODE_TELEMETRY_TRACE_UPLOAD", "false"),
        ("CCODE_FEEDBACK_ENABLED", "false"),
        ("CCODE_TRACE_UPLOAD", "false"),
        ("CCODE_INSTRUMENTATION", "disabled"),
        ("OTEL_SDK_DISABLED", "true"),
        ("DISABLE_TELEMETRY", "1"),
        ("DISABLE_FEEDBACK_COMMAND", "1"),
        ("CCODE_DISABLE_AUTOUPDATER", "1"),
        ("CCODE_PROMPT_SUGGESTIONS", "false"),
    ...
    env.insert(
        ...
        ccode_home.join("gitconfig").into_os_string(),
    );
    env
}
...
fn diagnostic_value_is_sensitive(key: &OsStr) -> bool {
    ...
    is_secret_key(&key)
        ...
        || matches!(
            ...
            "HOME" | "USERPROFILE" | "CCODE_HOME" | "TMPDIR" | "TMP" | "TEMP" | "GIT_CONFIG_GLOBAL"
        )
}
...
mod tests {
    ...
    fn owns_distinct_isolated_paths() {
        ...
        assert_eq!(sandbox.ccode_home(), sandbox.home().join(".ccode"));
    }
    ...
    fn git_command_uses_sandbox_state_without_process_global_mutation() {
        ...
        let sandbox = TestSandbox {
            ...
            ccode_home: root.path().join("home/.ccode"),
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "CCODE_HOME"),
            Some(sandbox.ccode_home().into())
        );
        assert_eq!(
        ...
        assert_eq!(
            env_value(&sandbox, "CCODE_API_KEY").as_deref(),
            Some(OsStr::new(TEST_API_KEY))
        );
        assert_eq!(
            env_value(&sandbox, "CCODE_DISABLE_AUTOUPDATER").as_deref(),
            Some(OsStr::new("1"))
        );
        assert_eq!(
            env_value(&sandbox, "CCODE_TELEMETRY_TRACE_UPLOAD").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "CCODE_LEADER_SOCKET"), None);
        assert_eq!(env_value(&sandbox, "CCODE_DISABLE_WEB_FETCH"), None);
        assert_eq!(env_value(&sandbox, "CCODE_WEB_FETCH"), None);
    }
    ...
    fn command_application_clears_ambient_env_and_command_override_wins() {
        ...
        cmd.env("AMBIENT_SECRET", "must-disappear")
            .env("CCODE_PROMPT_SUGGESTIONS", "ambient");
        sandbox.apply_to_std_command(&mut cmd);
        cmd.env("CCODE_PROMPT_SUGGESTIONS", "command");
        ...
        assert_eq!(
            env.get(OsStr::new("CCODE_PROMPT_SUGGESTIONS"))
                .map(OsString::as_os_str),
    ...
    fn explicit_overrides_win_and_can_remove_baseline_entries() {
        ...
        sandbox
            ...
            .set_env("CCODE_PROMPT_SUGGESTIONS", "true")
            .set_env("NO_PROXY", "override.invalid")
            .remove_env("CCODE_DISABLE_AUTOUPDATER");
        assert_eq!(
            env_value(&sandbox, "TERM_PROGRAM").as_deref(),
            Some(OsStr::new("vscode"))
        );
        assert_eq!(
            env_value(&sandbox, "CCODE_PROMPT_SUGGESTIONS").as_deref(),
            Some(OsStr::new("true"))
        );
        assert_eq!(
            env_value(&sandbox, "NO_PROXY").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "CCODE_DISABLE_AUTOUPDATER"), None);
    }
    ...
    fn diagnostics_fail_closed_for_credential_keys() {
        ...
        for (key, value) in [
            ...
            ("CCODE_DEPLOYMENT_KEY", "deployment-key-do-not-print"),
            ("CCODE_EXTRA_AUTH_KEY", "alpha-test-key-do-not-print"),
