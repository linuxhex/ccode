//! Headless mode (`ccode -p`) test runner.
//!
//! Runs the ccode binary as a subprocess with the mock server, captures output.
...
/// Timeout for one headless ccode invocation: 60 seconds, multiplied by
/// [`crate::scaled`]'s `CCODE_TEST_TIMEOUT_SCALE`.
...
/// Run `ccode` with the given args against the mock server, bounded by the
...
mod tests {
    ...
    async fn custom_headless_env_is_explicit_and_wins_after_sandbox_baseline() {
        ...
        cmd.args([
            "-c",
            "printf '%s|%s|%s|%s' \"${AMBIENT_ONLY-unset}\" \"$CCODE_PROMPT_SUGGESTIONS\" \"$FEATURE_TEST_VAR\" \"$HOME\"",
        ])
        .env("AMBIENT_ONLY", "discarded")
        .env("CCODE_PROMPT_SUGGESTIONS", "command-level-discarded");

        let result = run_headless_in_sandbox_borrowed_with_env(
            cmd,
            &sandbox,
            &[
                ("CCODE_PROMPT_SUGGESTIONS", "explicit-override"),
