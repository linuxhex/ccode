...
use crate::db::{ListFilter, WorktreeDb, WorktreeKind, now_epoch_secs, resolve_ccode_home};
...
pub const ENV_AUTO_GC: &str = "CCODE_WORKTREE_AUTO_GC";
/// `1` / `true` / `on` forces age-count without delete.
pub const ENV_AUTO_GC_DRY_RUN: &str = "CCODE_WORKTREE_AUTO_GC_DRY_RUN";
/// Default max age in seconds (overrides TOML/remote when set and parseable).
pub const ENV_AUTO_GC_MAX_AGE: &str = "CCODE_WORKTREE_AUTO_GC_MAX_AGE";
/// `1` / `true` / `on` enables optional discovery rebuild + stale git prune.
pub const ENV_AUTO_GC_REBUILD: &str = "CCODE_WORKTREE_AUTO_GC_REBUILD";
...
/// Parse `CCODE_WORKTREE_AUTO_GC_MAX_AGE` as seconds; invalid/absent → None.
...
fn maybe_run_rebuild(
    ...
    let home = match resolve_ccode_home() {
        Ok(h) => h,
        Err(e) => {
            tracing::warn!(error = %e, "auto worktree rebuild skipped: ccode home unresolved");
            return (None, false);
        }
    };
...
mod tests {
    ...
    #[test]
    fn include_rebuild_true_registers_untracked_under_ccode_home() {
        let _g = env_guard();
        clear_auto_gc_env();
        let fx = crate::db::CcodeHomeFixture::new();
        let db = WorktreeDb::open(&fx.home).unwrap();
        ...
        assert!(
            ...
            "untracked dir under ccode_home/worktrees must be registered"
        );
    }
