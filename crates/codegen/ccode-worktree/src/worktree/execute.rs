...
/// Record the source repo root in `<worktree>/.git/ccode-worktree-source`.
///
...
fn record_main_repo_marker(source: &Path, worktree: &Path) {
    ...
    let marker = git_dir.join("ccode-worktree-source");
    if marker.exists() {
        return;
    }
...
mod tests {
    ...
    fn test_marker_written_for_standalone_git_dir() {
        ...
        ccode_test_utils::git::init_git_repo(&source);
        ...
        let marker = dest.join(".git/ccode-worktree-source");
    ...
    fn test_marker_skipped_for_linked_worktree() {
        ...
        assert!(!dest.join(".git/ccode-worktree-source").exists());
    }
