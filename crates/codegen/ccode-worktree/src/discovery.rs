...
pub fn discover_worktrees(ccode_home: &Path) -> DiscoveryReport {
    let mut report = DiscoveryReport::default();
    scan_two_level_dir(
        &ccode_home.join("worktrees"),
        WorktreeKind::Session,
        &mut report,
    );
    scan_two_level_dir(
        &ccode_home.join("worktree_pool"),
        WorktreeKind::Pool,
        &mut report,
    );
    report
...
fn managed_worktree_roots(ccode_home: &Path) -> [PathBuf; 2] {
    [ccode_home.join("worktrees"), ccode_home.join("worktree_pool")]
...
pub fn path_under_managed_worktree_roots(path: &Path, ccode_home: &Path) -> bool {
    path_under_roots(path, &managed_worktree_roots(ccode_home))
}
...
pub fn rebuild_worktree_db(
    ...
    let discovery = discover_worktrees(ccode_home);
    let mut report = RebuildReport {
        discovered: discovery.found.len() as u64,
        ..Default::default()
    };
    let now = now_epoch_secs();
    let roots = managed_worktree_roots(ccode_home);
    ...
    for wt in discovery.found {
        ...
        if !path_under_roots(&path, &roots) {
            tracing::warn!(
                path = %path.display(),
                "rebuild skipped path outside ccode worktrees/worktree_pool"
            );
            continue;
        }
...
mod tests {
    ...
    fn discover_session_worktrees() {
        ...
        let wt = ccode_home.join("worktrees/myrepo/worktree-abc123");
        make_fake_linked_worktree(&wt, "/repo/.git/worktrees/abc123");

        let report = discover_worktrees(ccode_home);
    ...
    fn discover_pool_worktrees() {
        ...
        let wt = ccode_home.join("worktree_pool/inst-1/pool-a");
    ...
    fn skips_dot_prefixed_and_markers() {
        ...
        let base = ccode_home.join("worktrees/myrepo");
    ...
    fn rebuild_registers_and_skips_duplicates() {
        ...
        let wt = ccode_home.join("worktrees/repo/worktree-sess1");
        make_fake_standalone_worktree(&wt);

        let db = crate::db::WorktreeDb::open_in_memory().unwrap();

        let r1 = rebuild_worktree_db(&db, ccode_home).unwrap();
        assert_eq!(r1.discovered, 1);
        assert_eq!(r1.registered, 1);
        assert_eq!(r1.already_tracked, 0);

        let r2 = rebuild_worktree_db(&db, ccode_home).unwrap();
    ...
    fn rebuild_keeps_same_basename_worktrees_in_different_repos() {
        ...
        let wt_a = ccode_home.join("worktrees/repo-a/wt-abc");
        let wt_b = ccode_home.join("worktrees/repo-b/wt-abc");
        make_fake_standalone_worktree(&wt_a);
        make_fake_standalone_worktree(&wt_b);

        let db = crate::db::WorktreeDb::open_in_memory().unwrap();
        let report = rebuild_worktree_db(&db, ccode_home).unwrap();
        ...
        let report2 = rebuild_worktree_db(&db, ccode_home).unwrap();
    ...
    fn rebuild_sets_last_accessed_at() {
        let tmp = tempfile::TempDir::new().unwrap();
        let ccode_home = tmp.path();
        let wt = ccode_home.join("worktrees/repo/sess");
        make_fake_standalone_worktree(&wt);
        let db = crate::db::WorktreeDb::open_in_memory().unwrap();
        rebuild_worktree_db(&db, ccode_home).unwrap();
    ...
    fn rebuild_skips_symlink_escape_outside_managed_roots() {
        let tmp = tempfile::TempDir::new().unwrap();
        let ccode_home = tmp.path().join("ccode");
        let outside = tmp.path().join("outside-real");
        make_fake_standalone_worktree(&outside);
        let link_parent = ccode_home.join("worktrees/repo");
