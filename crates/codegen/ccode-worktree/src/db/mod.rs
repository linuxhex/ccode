...
impl WorktreeDb {
    /// Open (or create) the DB at `ccode_home/worktrees.db`.
    pub fn open(ccode_home: &Path) -> Result<Self> {
        Self::open_at(&ccode_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.ccode/worktrees.db`.
    ///
    /// Discovers ccode home via `$CCODE_HOME`, falling back to the canonicalized
    /// `$HOME/.ccode` (matching `ccode_config::ccode_home`).
...
pub fn resolve_ccode_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("CCODE_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $CCODE_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical .ccode
    // tree as trust/hooks even when it is symlinked. The dunce canonicalization
    // must stay in sync with ccode_config::default_ccode_home();
    // home resolution deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".ccode"))
}

/// Serializes tests that mutate the process-global `CCODE_HOME` env var so they
...
#[cfg(test)]
static CCODE_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`CCODE_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `CCODE_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `CCODE_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct CcodeHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated ccode home; pass to `WorktreeDb::open` to read the same DB
...
#[cfg(test)]
impl CcodeHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = CCODE_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("ccode-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via CCODE_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until CCODE_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("CCODE_HOME");
        unsafe { std::env::set_var("CCODE_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for CcodeHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("CCODE_HOME", p),
                None => std::env::remove_var("CCODE_HOME"),
            }
        }
    }
}
