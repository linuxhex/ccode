...
use ccode_pager::app::app_view::{AuthState, TrustState};
    ...
    match hint {
        MinimalAuthHint::SigningIn { url, code } => {
            y = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Sign in to Ccode", bold)),
            );
...
mod tests {
    ...
    fn device_user_code_parses_verification_url() {
        assert_eq!(
            device_user_code("https://accounts.ccode.dev/oauth2/device?user_code=ABCD-EFGH"),
            Some("ABCD-EFGH")
        );
        assert_eq!(
            device_user_code("https://accounts.ccode.dev/oauth2/device"),
            None
        );
    ...
    fn auth_hint_maps_auth_state() {
        use ccode_pager::app::app_view::AuthMode;
        ...
        let st = AuthState::Authenticating {
            request_seq: 1,
            handle: None,
            auth_url: Some("https://accounts.ccode.dev/device?user_code=ABCD-EFGH".into()),
        ...
        match minimal_auth_hint(&st, &trust_done, true, false) {
            MinimalAuthHint::SigningIn { url, code } => {
                assert_eq!(
                    url.as_deref(),
                    Some("https://accounts.ccode.dev/device?user_code=ABCD-EFGH")
                );
    ...
    fn render_auth_shows_url_and_code() {
        ...
        let hint = MinimalAuthHint::SigningIn {
            url: Some("https://accounts.ccode.dev/device?user_code=ABCD-EFGH".into()),
            code: Some("ABCD-EFGH".into()),
        };
        render_auth(&mut buf, area, &theme, &hint);
        let text = buffer_text(&buf, area);
        assert!(text.contains("Sign in to Ccode"), "header: {text:?}");
        assert!(text.contains("accounts.ccode.dev/device"), "url: {text:?}");
