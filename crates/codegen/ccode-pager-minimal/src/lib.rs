//! Minimal (scrollback-native) render mode — `ccode --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `ccode_ratatui::Terminal::insert_before`,
...
//! # Wiring
//!
//! `ccode-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`ccode_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`ccode-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `ccode-pager`.
///
...
/// subsequent calls are ignored (see [`ccode_pager::minimal_hook`]).
pub fn install() {
    ccode_pager::minimal_hook::install(ccode_pager::minimal_hook::MinimalHooks { draw });
}
