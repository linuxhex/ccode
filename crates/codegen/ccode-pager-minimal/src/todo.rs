...
use ccode_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (ccode_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    ccode_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use ccode_shell::tools::{TodoItem, TodoPriority};
