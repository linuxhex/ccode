...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"CcodeBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"CcodeBuild:run_terminal_cmd"`).
...
/// between namespaces (e.g. `CcodeBuild:run_terminal_cmd` vs.
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("CcodeBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("CcodeBuild:read_file", "legacy-0.4.10"),
            ("CcodeBuild:search_replace", "legacy-0.4.10"),
            ("CcodeBuild:get_task_output", "legacy-0.4.10"),
            ("CcodeBuild:kill_task", "legacy-0.4.10"),
            ("CcodeBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("CcodeBuild:run_terminal_cmd", "current"),
            ("CcodeBuild:read_file", "current"),
            ("CcodeBuild:search_replace", "current"),
            ("CcodeBuild:list_dir", "current"),
            ("CcodeBuild:grep", "current"),
            ("CcodeBuild:kill_task", "current"),
            ("CcodeBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "CcodeBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "CcodeBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "CcodeBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "CcodeBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }
    ...
    fn override_on_unmanaged_tool_errors() {
        let err =
            resolve_version("current", "CcodeBuild:web_search", Some("legacy-0.4.10")).unwrap_err();
        assert!(err.contains("unmanaged tool"));
    }

    #[test]
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "CcodeBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "CcodeBuild:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // CcodeBuildConcise tools should NOT be version-managed.
        assert!(!is_version_managed("CcodeBuildConcise:run_terminal_cmd"));
        let v = resolve_version("current", "CcodeBuildConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("CcodeBuild:run_terminal_cmd"));
        assert!(is_version_managed("CcodeBuild:read_file"));
        assert!(is_version_managed("CcodeBuild:search_replace"));
        assert!(is_version_managed("CcodeBuild:list_dir"));
        assert!(is_version_managed("CcodeBuild:grep"));
        assert!(is_version_managed("CcodeBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("CcodeBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "CcodeBuild:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "CcodeBuild:run_terminal_cmd", None)
                .unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("CcodeBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("CcodeBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "CcodeBuild:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err = resolve_version_with_warnings("current", "CcodeBuild:grep", Some("legacy-0.4.10"))
            .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
