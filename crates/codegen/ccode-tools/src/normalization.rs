...
/// Resolve `wire_name` in `toolset` and merge the canonical `ccode.dev/tool` object
...
/// the `ccode.dev/tool` `_meta` object. Equivalent tools across toolsets emit the
...
mod tests {
    ...
    fn canonical_omits_absent_options_not_null() {
        let ccode = parse(serde_json::json!({"variant":"ReadFile","target_file":"/a"}));
        let g = canonical_input(&ccode).unwrap();
