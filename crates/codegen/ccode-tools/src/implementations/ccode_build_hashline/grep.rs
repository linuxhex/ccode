...
use crate::implementations::ccode_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl ccode_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = ccode_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(ccode_tool_runtime::Cwd(cwd));
        let mut result = ccode_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                ccode_tool_runtime::ToolError::execution(
                    ccode_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(ccode_tool_runtime::Tool::id(&tool).as_str(), "hashline_grep");
