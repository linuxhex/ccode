...
impl ccode_tool_runtime::Tool for ReadTool {
    type Args = ReadInput;
    type Output = ReadFileOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("read").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        if let Ok(meta) =
            crate::implementations::ccode_build::read_file::bytes_to_metadata(&file_bytes)
            && meta.is_image()
        {
...
mod tests {
    ...
    async fn runtime_footer_tracks_renamed_offset() {
        ...
        let result = ccode_tool_runtime::Tool::run(&ReadTool, ctx, input)
            .await
            .unwrap();
        match result {
