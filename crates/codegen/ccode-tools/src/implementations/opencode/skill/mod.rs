...
${%- endif %}
${%- if skill.effort %}
    <effort>${{ skill.effort }}</effort>
${%- endif %}
  </skill>
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.ccode/skills/ or .ccode/skills/)
${%- endif %}
</available_skills>"#;
...
impl ccode_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, ccode_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(ccode_tool_runtime::Tool::id(&tool).as_str(), "skill");
