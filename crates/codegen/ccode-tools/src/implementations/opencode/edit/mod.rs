...
use crate::implementations::ccode_build::search_replace::CONTEXT_LINES;
use crate::implementations::ccode_build::search_replace::helpers::{
...
// the snake_case `params.edit.old_string` keys of the ccode_build twin resolve
...
impl ccode_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: EditInput,
    ) -> Result<SearchReplaceOutput, ccode_tool_runtime::ToolError> {
...
async fn ensure_parent_dirs(path: &std::path::Path) -> Result<(), ccode_tool_runtime::ToolError> {
    if let Some(parent) = path.parent()
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            ccode_tool_runtime::ToolError::execution(
                ccode_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
// are imported from `ccode_build::search_replace::helpers` — shared across
// both the ccode_build and opencode edit tools.
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(ccode_tool_runtime::Tool::id(&tool).as_str(), "edit");
