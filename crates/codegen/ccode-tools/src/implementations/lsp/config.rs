//! LSP server configuration from `.ccode/lsp.json`.
...
/// This is the canonical merge function — both session startup and `ccode inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::ccode_home::ccode_home().join("lsp.json");
    let project_path = cwd.join(".ccode").join("lsp.json");
...
/// Load LSP server configs from `~/.ccode/lsp.json` and `<cwd>/.ccode/lsp.json`.
