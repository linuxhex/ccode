...
///
/// `skills` is the standard layout (`.ccode/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Ccode Build sessions.
...
/// name (e.g. `~/.ccode/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Ccode Build.
...
/// `cwd` (exclusive). At each directory, checks for `.ccode/skills/`,
...
/// visits known config roots (`.ccode`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.ccode` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".ccode", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.ccode is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.ccode/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.ccode/ → kept (user content).
        let ccode_shell = tmp.path().join(".ccode").join("skills").join("shell");
        std::fs::create_dir_all(&ccode_shell).unwrap();
        std::fs::write(
            ccode_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (ccode_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.ccode/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .ccode skill in an intermediate dir.
        ...
        .unwrap();
        let ccode_skill = sub.join(".ccode").join("skills").join("ccode-dyn");
        std::fs::create_dir_all(&ccode_skill).unwrap();
        std::fs::write(ccode_skill.join("SKILL.md"), "---\nname: ccode-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"ccode-dyn"),
            "ccode-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only ccode-dyn discovered.
        ...
        assert!(
            names_off.contains(&"ccode-dyn"),
            "ccode-dyn missing: {names_off:?}"
        );
        assert!(
