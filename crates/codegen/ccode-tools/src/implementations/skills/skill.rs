...
pub struct SkillOutput {
    ...
    /// Model override requested by this skill. When set, the caller should
    /// switch the session model before processing the skill content.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub model: Option<String>,
    /// Reasoning effort override requested by this skill.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub effort: Option<String>,
}

// Old `SkillToolImpl` + `impl Tool` deleted.
// New implementation is in `ccode_build/skill/`.
...
///
/// When the skill specifies a `model` or `effort` override, an advisory
/// note is appended so the model is aware of the recommended configuration.
///
/// ```text
...
pub fn build_skill_message(skill: &SkillInfo, content: &str) -> String {
    ...
    // Append model/effort advisory so the model and any integration layer
    // know that this skill recommends a specific model configuration.
    if skill.model.is_some() || skill.effort.is_some() {
        message.push_str("\n\n<!-- skill-config");
        if let Some(ref model) = skill.model {
            message.push_str(&format!(" model=\"{}\"", model));
        }
        if let Some(ref effort) = skill.effort {
            message.push_str(&format!(" effort=\"{}\"", effort));
        }
        message.push_str(" -->");
    }
    message
...
///
/// Supported variables (Ccode-native names + compat aliases):
///
...
/// | `${CCODE_PLUGIN_ROOT}` | `${CLAUDE_PLUGIN_ROOT}` | Plugin root dir (plugin-backed skills) |
/// | `${CCODE_PLUGIN_DATA}` | `${CLAUDE_PLUGIN_DATA}` | Plugin data dir (plugin-backed skills) |
///
...
/// `load_skill_content` in `ccode_build/skill/mod.rs` is a duplicate
/// of this.
...
It has multiple lines."#;
    ...
    fn test_build_skill_message_exact_format() {
        let skill = SkillInfo {
            ...
            author: None,
            argument_hint: None,
            path: "/home/user/.ccode/skills/commit/SKILL.md".to_string(),
            scope: SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
        ...
        let expected = "\
<skill name=\"commit\" description=\"Create a git commit\" path=\"/home/user/.ccode/skills/commit/SKILL.md\">
# Git Commit Skill
...
<skill name=\"empty\" description=\"An empty skill\" path=\"/skills/empty/SKILL.md\">
    ...
    fn test_build_skill_message_multiline_content() {
        let skill = SkillInfo {
            ...
            path: "/repo/.ccode/skills/review/SKILL.md".to_string(),
            ..SkillInfo::default()
        };
        ...
        let expected = "\
<skill name=\"review\" description=\"Review code\" path=\"/repo/.ccode/skills/review/SKILL.md\">
# Code Review
...
- No warnings
    ...
    #[test]
    fn test_build_skill_message_with_model_and_effort() {
        let skill = SkillInfo {
            name: "deep-analysis".to_string(),
            description: "Deep code analysis".to_string(),
            path: "/skills/deep-analysis/SKILL.md".to_string(),
            model: Some("ccode-3".to_string()),
            effort: Some("high".to_string()),
            ..SkillInfo::default()
        };
        let content = "Analyze deeply.";
        let message = build_skill_message(&skill, content);
        assert!(message.contains("<!-- skill-config model=\"ccode-3\" effort=\"high\" -->"));
    }

    #[test]
    fn test_build_skill_message_with_model_only() {
        let skill = SkillInfo {
            name: "fast-review".to_string(),
            description: "Quick review".to_string(),
            path: "/skills/fast-review/SKILL.md".to_string(),
            model: Some("ccode-mini".to_string()),
            ..SkillInfo::default()
        };
        let content = "Review quickly.";
        let message = build_skill_message(&skill, content);
        assert!(message.contains("<!-- skill-config model=\"ccode-mini\" -->"));
        assert!(!message.contains("effort="));
    }
    ...
    fn test_substitutions_skill_dir() {
        ...
        apply_substitutions(
            ...
            None,
            &SubstitutionContext {
                skill_dir: Some("/home/user/.ccode/skills/deploy"),
                ..Default::default()
            },
        );
        assert_eq!(
            content,
            "Config at /home/user/.ccode/skills/deploy/config.json"
        );
    }
    ...
    #[test]
    fn test_ccode_plugin_aliases_substitution() {
        let mut content = "Root ${CCODE_PLUGIN_ROOT}, data ${CCODE_PLUGIN_DATA}".to_string();
        apply_substitutions(
            &mut content,
            None,
            &SubstitutionContext {
...
Review code.
    ...
    fn test_build_skill_information_single() {
        ...
        let refs = vec![SkillRef {
            name: "commit",
            path: "/home/user/.ccode/skills/commit/SKILL.md",
        }];
        ...
        assert!(
            result.contains(
                "<skill name=\"commit\" path=\"/home/user/.ccode/skills/commit/SKILL.md\"/>"
            )
        );
    ...
    fn test_build_skill_information_multi() {
        ...
        let refs = vec![
            SkillRef {
                name: "review",
                path: "/project/.ccode/skills/review/SKILL.md",
            },
            SkillRef {
                name: "lint",
                path: "/project/.ccode/skills/lint/SKILL.md",
            },
        ];
        let result = build_skill_information(&blocks, &refs);
        assert!(result.starts_with("<skill_information>\n"));
