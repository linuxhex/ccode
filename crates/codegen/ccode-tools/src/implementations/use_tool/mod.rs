...
crate::register_resource!("ccode_build", "UseTool", UseToolParams);
...
async fn dispatch_local_mcp(
    ...
    let tool_id = ccode_tool_protocol::ToolId::new(tool_name).map_err(|_| {
        ccode_tool_runtime::ToolError::invalid_arguments(format!("invalid tool name: '{tool_name}'"))
    })?;
...
fn is_local_tool_id_rejection(err: &ccode_tool_runtime::ToolError, tool_name: &str) -> bool {
    err.kind == ccode_tool_runtime::ToolErrorKind::InvalidArguments
    ...
    if let Some(source) = gateway_source {
            ...
            && let Some(dispatch) = dispatch.clone()
        {
            match dispatch_local_mcp(dispatch, tool_name, tool_input.clone(), ctx.clone()).await {
                ...
                Err(err)
                    if err.kind != ccode_tool_runtime::ToolErrorKind::NotFound
...
impl ccode_tool_runtime::Tool for UseTool {
    type Args = UseToolInput;
    type Output = ToolOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("use_tool").expect("valid tool id")
    }
...
mod tests {
    ...
    impl ccode_tool_runtime::ToolDispatch for MockToolDispatch {
        async fn call(
            &self,
            ...
            ccode_tool_runtime::terminal_only(Ok(ccode_tool_runtime::TypedToolOutput::from_value(
                tool_id, value,
            )))
        }
    }
    ...
    #[async_trait::async_trait]
    impl ccode_tool_runtime::ToolDispatch for CapturingDispatch {
        async fn call(
            &self,
            tool_id: ccode_tool_protocol::ToolId,
            ...
            if !matches!(
                ...
                return ccode_tool_runtime::terminal_only(Err(
                    ccode_tool_runtime::ToolError::not_found(tool_id, "Tool not found"),
                ));
            }
    ...
    fn ctx_capturing() -> (ccode_tool_runtime::ToolCallContext, SharedArgs) {
    ...
    #[async_trait::async_trait]
    impl ccode_tool_runtime::ToolDispatch for NotFoundDispatch {
        async fn call(
            &self,
            tool_id: ccode_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            ccode_tool_runtime::terminal_only(Err(ccode_tool_runtime::ToolError::not_found(
                tool_id,
                "Tool not found",
            )))
        }
    ...
    #[async_trait::async_trait]
    impl ccode_tool_runtime::ToolDispatch for InvalidArgumentsDispatch {
        async fn call(
            &self,
            _tool_id: ccode_tool_protocol::ToolId,
            _args: serde_json::Value,
            _ctx: ccode_tool_runtime::ToolCallContext,
        ) -> ccode_tool_runtime::ToolStream<ccode_tool_runtime::TypedToolOutput> {
            ccode_tool_runtime::terminal_only(Err(ccode_tool_runtime::ToolError::invalid_arguments(
    ...
    #[async_trait::async_trait]
    impl ccode_tool_runtime::ToolDispatch for ErrorToolDispatch {
        async fn call(
            &self,
            _tool_id: ccode_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            let tid = ccode_tool_protocol::ToolId::new(&self.error)
    ...
    fn new_ctx() -> ccode_tool_runtime::ToolCallContext {
        let call_id = ccode_tool_protocol::ToolCallId::new_v7();
        ccode_tool_runtime::ToolCallContext::new(call_id)
    }

    fn ctx_with_dispatch(
        dispatch: impl ccode_tool_runtime::ToolDispatch + 'static,
    ...
    async fn rejects_builtin_tool_names() {
        ...
        assert_eq!(err.kind, ccode_tool_runtime::ToolErrorKind::InvalidArguments);
    ...
    fn classify_python_repr_single_line_is_long_line_text() {
        ...
        let row = "{'id': 0, 'name': 'user0', 'email': 'u0@ccode.dev', 'age': 20}";
    ...
    fn classify_csv_stays_plain_text() {
        ...
        for i in 0..50 {
            csv.push_str(&format!("{i},user{i},u{i}@ccode.dev,{}\n", 20 + i));
        }
    ...
    fn ctx_capturing_with_resources(
        resources: crate::types::resources::SharedResources,
    ) -> (ccode_tool_runtime::ToolCallContext, SharedArgs) {
        let args: SharedArgs = Arc::new(std::sync::Mutex::new(None));
        let mut ctx = new_ctx();
        ctx.extensions
