...
mod tests {
    ...
    use crate::implementations::ccode_build::read_file::{
        ReadFileInput, ReadFileParams, ReadFileTool as CcodeReadFileTool,
    };
    ...
    fn resources_with_ccode_rules_on_read(root: &Path) -> SharedResources {
        let mut resources = Resources::new();
        resources.insert(Cwd(root.to_path_buf()));
        resources.insert(FileSystem(Arc::new(LocalFs)));
        resources.insert(NotificationHandle(ToolNotificationHandle::noop()));
    ...
    #[tokio::test]
    async fn ccode_read_file_output_includes_matching_rule_once() {
        let temp = TempDir::new().unwrap();
        let root = temp.path();
        ...
        let shared = resources_with_ccode_rules_on_read(root);
        ...
        let first = ccode_tool_runtime::Tool::run(&CcodeReadFileTool, test_ctx(shared), input)
            .await
            .unwrap();
