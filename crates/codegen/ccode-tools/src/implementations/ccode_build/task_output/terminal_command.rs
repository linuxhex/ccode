...
use ccode_tool_types::{TaskOutputOutput, TaskOutputToolInput};
...
impl ccode_tool_runtime::Tool for GetTerminalCommandOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("get_terminal_command_output").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        ccode_tool_runtime::Tool::run(&TaskOutputTool, ctx, input).await
    }
}
...
mod tests {
    use super::*;
    use crate::implementations::ccode_build::task_output::test_helpers::{
