...
use crate::implementations::ccode_build::task_output::{
...
use ccode_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            ccode_tool_types::build_wait_tasks_description(&ccode_tool_types::WaitTasksToolNaming {
...
/// [`ccode_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl ccode_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout =
            crate::implementations::ccode_build::task_output::capped_wait_timeout(input.timeout_ms);
