//! `image_gen` tool — generates images via the ccode Imagine API and saves
...
const CCODE_IMAGINE_MODEL: &str = "ccode-imagine-image-quality";
// Some Imagine models (e.g. `ccode-imagine-image`, selectable via `model_override`)
...
/// Premium upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a Premium feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to Premium: https://ccode.dev/premium?referrer=ccode-build. Do not retry this tool.";

/// HTTP client for ccode Imagine API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct ImageGenClient {
    ...
    /// [`CCODE_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
    edit_model: String,
    ...
    /// HTTP call and return the Premium upsell prose instead. See
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| CCODE_IMAGINE_MODEL.to_owned());
        let edit_model = edit_model_override
            .clone()
            .filter(|m| !m.trim().is_empty())
            .unwrap_or_else(|| super::image_edit::CCODE_IMAGINE_EDIT_MODEL.to_owned());
    ...
    /// with the Premium upsell instead of issuing a doomed request.
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// ([`CCODE_IMAGINE_MODEL`]). Driven by the remote
        ...
        /// Premium upsell prose instead of a doomed request. Set by the
...
impl ImageGenConfig {
    ...
    /// default quality model ([`CCODE_IMAGINE_MODEL`]) is used.
...
impl ccode_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(ccode_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("ccode-imagine-image".into()),
        ...
        assert_eq!(cfg.model_override(), Some("ccode-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("ccode-imagine-image")), None)
                .unwrap()
                .model,
            "ccode-imagine-image"
        );
    ...
    fn client_selects_edit_model_from_override() {
        ...
        assert_eq!(
            ...
            super::super::image_edit::CCODE_IMAGINE_EDIT_MODEL
        );
        assert_eq!(
        ...
        let client = ImageGenClient::new(&mk(Some("ccode-imagine-image-v2")), None).unwrap();
        assert_eq!(client.edit_model(), "ccode-imagine-image-v2");
        assert_eq!(client.model, CCODE_IMAGINE_MODEL);
    }
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the Premium upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("Premium"), "got: {}", t.text);
                assert!(t.text.contains("premium?referrer=ccode-build"));
            }
