...
//! `LspTool` (Tool trait impl) under the `CcodeBuild` namespace.
...
impl ccode_tool_runtime::ToolOutput for LspToolOutput {}
...
impl ccode_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, ccode_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    ccode_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.ccode/lsp.json or <cwd>/.ccode/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
