...
use regex::Regex;
use ccode_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<ccode_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| ccode_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(ccode_tool_protocol::ToolScope::Write),
        streaming: Some(ccode_tool_protocol::StreamingSpec {
...
pub struct BashParams {
    ...
    /// ccode-build opts up to 10h via ccode-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "ccode_build.Bash";
...
impl ccode_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<ccode_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => ccode_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production ccode-build sets it to 10h in ccode-shell's
...
/// [`ccode_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `CCODE_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("CCODE_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in ccode_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the ccode_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`CCODE_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `CCODE_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = ccode_config::shell::chain_separator();
...
impl ccode_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: ccode_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> ccode_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield ccode_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield ccode_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield ccode_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, ccode_tool_runtime::ToolError> {
        ...
        let ampersand = ccode_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(ccode_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background && !background_enabled {
            return Err(ccode_tool_runtime::ToolError::invalid_arguments(
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing ccode_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (ccode_build backend) and can surface an educational
...
mod tests {
    ...
    fn read_chunk_progress(p: &ccode_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            ccode_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let ccode_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = ccode_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, ccode_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                ccode_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn foreground_command_success() {
        ...
        let result = ccode_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = ccode_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn errors_when_terminal_not_in_resources() {
        ...
        let result =
            ccode_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), make_input("ls"))
                .await;
        assert!(result.is_err());
        assert!(
            result
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = ccode_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(ccode_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // ccode-shell opts up to 10h via max_timeout_secs. Background unaffected.
    ...
    mod versioned_background_check {
        ...
        ) -> ccode_tool_runtime::ToolCallContext {
            let mut ctx =
                ccode_tool_runtime::ToolCallContext::new(ccode_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(ccode_tool_runtime::BehaviorVersion(
