//! `image_edit` tool — edits or transforms images via the ccode Imagine
...
use crate::implementations::ccode_build::image_gen::{ImageGenClient, ImageGenResponse};
...
pub(crate) const CCODE_IMAGINE_EDIT_MODEL: &str = "ccode-imagine-image-quality";
...
fn compress_reference(
    raw_bytes: Vec<u8>,
) -> Result<(Vec<u8>, &'static str), ccode_tool_runtime::ToolError> {
    ...
    let img = ImageReader::new(Cursor::new(&raw_bytes))
        ...
        .ok_or_else(|| {
            ccode_tool_runtime::ToolError::invalid_arguments("failed to decode image reference")
        })?;
...
async fn resolve_to_data_url(value: &str) -> Result<String, ccode_tool_runtime::ToolError> {
...
impl ccode_tool_runtime::Tool for ImageEditTool {
    type Args = ImageEditInput;
    type Output = ToolOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("image_edit").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(ccode_tool_runtime::Tool::id(&tool).as_str(), "image_edit");
