...
pub use ccode_tools_api::slash_commands::WORKFLOW_TOOL_NAME;
...
pub struct WorkflowToolInput {
    ...
    #[serde(default)]
    #[schemars(
        description = "Name of a registered workflow (built-in, or discovered from the project `.ccode/workflows/` or user `~/.ccode/workflows/`). Exactly one of `name`, `script`, or `script_path` must be set."
    )]
...
impl ccode_tool_runtime::ToolOutput for WorkflowToolOutput {}
...
A started run gets a session-unique display name (e.g. `review-changes`, `review-changes-2`) — the handle to show the user and use with `/workflow pause|resume|stop <name>`; keep run IDs internal. Each launch persists an editable `script_path`; edit it and launch as a new run to iterate. Use `resume_from_run_id` only for a same-process paused run (process restarts are terminal); a budget-limited run resumes only with a higher `agent_budget`. Save reusable scripts to `.ccode/workflows/<name>.rhai`."##
    }
...
impl ccode_tool_runtime::Tool for WorkflowTool {
    type Args = WorkflowToolInput;
    type Output = WorkflowToolOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new(WORKFLOW_TOOL_NAME).expect("valid tool id")
    }
    ...
    async fn run(
    ...
    ) -> Result<WorkflowToolOutput, ccode_tool_runtime::ToolError> {
        ...
        match ack_rx.await {
            ...
            Ok(WorkflowLaunchAck::Rejected { code, detail }) => {
                Err(ccode_tool_runtime::ToolError::custom(code, detail))
            }
            Err(_) => Err(ccode_tool_runtime::ToolError::custom(
