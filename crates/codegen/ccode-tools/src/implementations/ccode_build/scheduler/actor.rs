...
mod tests {
    ...
    fn make_test_actor_with_subagents_at(
        ...
        resources.insert(
            crate::implementations::ccode_build::task::types::SessionIdResource(
    ...
    ) -> crate::implementations::ccode_build::task::types::SubagentSnapshot {
        crate::implementations::ccode_build::task::types::SubagentSnapshot {
    ...
    async fn failed_spawn_resets_chain_state() {
        ...
        let _ = request.result_tx.send(
            crate::implementations::ccode_build::task::types::SubagentResult {
                success: false,
    ...
    async fn durable_delete_retries_persistence_and_reuses_version() {
        ...
        .unwrap();
        assert_eq!(persisted["state"]["ccode_build.WebCitation"]["counter"], 7);
        assert_eq!(
            persisted["state"]["ccode_build.Scheduler"]["tasks"],
            serde_json::json!([])
        );
        cancel.cancel();
    }
    ...
    async fn durable_expiry_persists_before_ack_and_commits_version() {
...

        {
            ...
            assert_eq!(
                snapshot["state"]["ccode_build.Scheduler"]["tasks"],
                serde_json::json!([])
            );
            assert!(notifications.try_recv().is_err());
