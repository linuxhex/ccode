...
//!
//! - **`ccode-tools`** — tool blocks on a oneshot, formats the result.
//! - **`ccode-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`ccode-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `ccode-tools`.
...
register_resource!("ccode_build", "UserQuestionSender", UserQuestionSender);
