...
use ccode_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"ccode-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`ccode_tool_types::is_not_sentinel`] for existing call sites.
pub use ccode_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `ccode-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "CcodeBuild:run_terminal_cmd" | "CcodeBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("ccode_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-ccode-build
...
pub type MonitorEventBuffer = ccode_interjection::EventQueue<MonitorEventNotification>;

register_resource!("ccode_build", "MonitorEventBuffer", MonitorEventBuffer);
...
register_resource!("ccode_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("ccode_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("ccode_build", "SessionIdResource", SessionIdResource);
...
///
/// Set by ccode-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by ccode-shell at turn start.
...
register_resource!("ccode_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("CcodeBuild:run_terminal_cmd", ToolKind::Execute),
                tc("CcodeBuild:read_file", ToolKind::Read),
                tc("CcodeBuild:list_dir", ToolKind::List),
                tc("CcodeBuild:grep", ToolKind::Search),
                tc("CcodeBuild:kill_task", ToolKind::KillTaskAction),
                tc("CcodeBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("CcodeBuild:kill_task", ToolKind::KillTaskAction),
                tc("CcodeBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("CcodeBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("CcodeBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
