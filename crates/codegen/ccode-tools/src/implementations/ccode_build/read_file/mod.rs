...
mod versions;
use crate::types::schema::CcodeIntegerSchema;
...
crate::register_resource!("ccode_build", "ReadFile", ReadFileParams);
...
/// `PlainText` / `Append`.
static READ_FILE_CAPABILITIES: LazyLock<ccode_tool_protocol::ToolCapabilities> =
...
async fn handle_pptx(
    file_bytes: Vec<u8>,
    path: &std::path::Path,
) -> Result<ReadFileOutput, ccode_tool_runtime::ToolError> {
...
/// an empty window (harness-aligned; not a Ccode-line clamp).
...
impl ccode_tool_runtime::Tool for ReadFileTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;
    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("read_file").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    async fn execute(
        ...
        input: ReadFileInput,
    ) -> ccode_tool_runtime::ToolStream<ReadFileOutput> {
        ...
        Box::pin(async_stream::stream! {
            ...
            match ReadFileTool::read_with_streamability(&ctx, input).await {
                Ok((output, streamable)) => {
                        ...
                        && !fc.content.is_empty()
                    {
                        ...
                        while window_start < content.len() {
                            ...
                            if let Some(p) = ccode_tool_runtime::stream_chunk(
                                spec,
                                &content[..window_end],
                                window_end as u64,
                                &mut last_total,
                ...
                Err(e) => yield ccode_tool_runtime::ToolStreamItem::Terminal(Err(e)),
            }
        })
    }
...
impl ReadFileTool {
    ...
    async fn read_with_streamability(
        ctx: &ccode_tool_runtime::ToolCallContext,
        input: ReadFileInput,
    ) -> Result<(ReadFileOutput, bool), ccode_tool_runtime::ToolError> {
        let resources = crate::types::tool_metadata::shared_resources(ctx)?;
        let cwd_override = ctx
            .extensions
            .get::<ccode_tool_runtime::Cwd>()
            .map(|c| c.0.clone());
...
mod tests {
    ...
    async fn read_file_basic() {
        ...
        let result = ccode_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
            .await
            .unwrap();
        match result {
    ...
    async fn legacy_read_file_not_found_returns_exact_historical_message() {
        ...
        let result = ccode_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        let expected = format!(
            "Failed to read file: {}",
    ...
    async fn current_read_file_not_found_returns_structured_not_found() {
        ...
        let result = ccode_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap();
        match result {
            ReadFileOutput::FileNotFound(msg) => {
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".ccode/skills/commit");
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".ccode/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn md_in_skills_dir_ignores_model_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".ccode/skills/my-skill");
        std::fs::create_dir_all(&skill_dir).unwrap();
        let content = (1..=1200)
            .map(|n| format!("line{n}"))
            .collect::<Vec<_>>()
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".ccode/skills/my-skill/reference.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn run_read_file_on(filename: &str, content: &[u8]) -> ReadFileOutput {
        ...
        ccode_tool_runtime::Tool::run(&ReadFileTool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap()
    }
    #[tokio::test]
    ...
    async fn execute_collect(
        ...
        let mut stream = ccode_tool_runtime::Tool::execute(&ReadFileTool, ctx, input).await;
    ...
    /// phantom field only (start past any `split_inclusive` line), so Ccode
