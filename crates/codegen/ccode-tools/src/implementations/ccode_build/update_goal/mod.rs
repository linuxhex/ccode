...
pub use ccode_tools_api::slash_commands::UPDATE_GOAL_TOOL_NAME;
...
impl ccode_tool_runtime::ToolOutput for UpdateGoalOutput {}
...
impl ccode_tool_runtime::Tool for UpdateGoalTool {
    type Args = UpdateGoalInput;
    type Output = UpdateGoalOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new(UPDATE_GOAL_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<UpdateGoalOutput, ccode_tool_runtime::ToolError> {
...
pub fn render_ack_into_output(
    ...
    match ack {
        ...
        UpdateGoalAck::Rejected { reason, detail } => Err(ccode_tool_runtime::ToolError::custom(
            reason.error_code(),
            detail,
        )),
    }
