...
impl ccode_tool_runtime::Tool for MonitorTool {
    type Args = MonitorInput;
    type Output = MonitorOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new("monitor").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: MonitorInput,
    ) -> Result<MonitorOutput, ccode_tool_runtime::ToolError> {
