...
//! the ccode Video Generation API and save them to the local filesystem so the
...
const CCODE_VIDEO_BASE_MODEL: &str = "ccode-imagine-video";
const CCODE_VIDEO_QUALITY_MODEL: &str = "ccode-imagine-video-1.5-preview";
...
const DEFAULT_ZDR_VIDEO_KEY_PREFIX: &str = "ccode-videos/";
...
pub use ccode_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> ccode_fs::s3::S3StaticCredentials {
        ccode_fs::s3::S3StaticCredentials {
...
/// HTTP client for ccode Video Generation API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct VideoGenClient {
    ...
    /// and return the Premium upsell prose. See [`VideoGenClient::is_tier_restricted`].
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, ccode_tool_runtime::ToolError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|e| {
                ccode_tool_runtime::ToolError::invalid_arguments(format!(
        ...
        extra_headers.into_iter().try_for_each(|(key, value)| {
            ...
            Ok::<(), ccode_tool_runtime::ToolError>(())
        })?;
    ...
    /// Premium upsell instead of issuing a doomed request.
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, ccode_tool_runtime::ToolError> {
        ...
        if !status.is_success() {
            ...
            return Err(ccode_tool_runtime::ToolError::new(
                ccode_tool_runtime::ToolErrorKind::Custom,
                ...
                "done" => {
                    ...
                    return match presigned {
                        ...
                        None if video_url.is_empty() => {
                            Err(ccode_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn download_video(&self, url: &str) -> Result<Vec<u8>, ccode_tool_runtime::ToolError> {
        let response = self.download_http.get(url).send().await.map_err(|e| {
            ccode_tool_runtime::ToolError::invalid_arguments(format!("Failed to download video: {e}"))
        })?;
    ...
    async fn finish_zdr_video(
        ...
        let config = self.zdr_video_output_s3.as_ref().ok_or_else(|| {
            ccode_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, ccode_tool_runtime::ToolError> {
        ...
        let upload_url = ccode_fs::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, ccode_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = ccode_fs::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
pub enum VideoGenConfig {
    ...
    Enabled {
        ...
        /// at call time with the Premium upsell prose. Set by the host from
...
/// the deliberate `/imagine-video` slash command shows the Premium upsell
/// modal instead.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Video generation is a Premium feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to Premium: https://ccode.dev/premium?referrer=ccode-build. Do not retry this tool.";
...
async fn resolve_image_reference(value: &str) -> Result<String, ccode_tool_runtime::ToolError> {
    ...
    if value.starts_with("data:image/") {
        let comma = value.find(',').ok_or_else(|| {
            ccode_tool_runtime::ToolError::invalid_arguments("malformed data URL in image reference")
        })?;
    ...
    let (_w, _h, mime) =
        crate::util::image_validate::validate_image_bytes(&raw_bytes).map_err(|e| {
            ccode_tool_runtime::ToolError::invalid_arguments(format!("invalid image reference: {e}"))
        })?;
...
fn validate_one_of(
    ...
    Err(ccode_tool_runtime::ToolError::invalid_arguments(format!(
        "`{field}` must be one of: {}. Got {value}.",
        allowed.join(", ")
    )))
}

fn validate_imagine_duration(duration: Option<u32>) -> Result<(), ccode_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), ccode_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, ccode_tool_runtime::ToolError> {
    let absolute_path = client
        .writer
        .save(session_folder, video_bytes, None)
        .await
        .map_err(|e| ccode_tool_runtime::ToolError::invalid_arguments(e.to_string()))?;
...
) -> Result<MediaGenOutput, ccode_tool_runtime::ToolError> {
    match outcome {
...
impl ccode_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
impl ccode_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> ccode_tool_protocol::ToolId {
        ccode_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    fn image_and_reference_payload_fields_are_serialized() {
        let payload = GenerateVideoPayload {
            model: CCODE_VIDEO_QUALITY_MODEL,
            prompt: "animate",
    ...
    async fn image_to_video_rejects_bad_duration() {
        ...
        let err = ccode_tool_runtime::Tool::run(
            &tool,
