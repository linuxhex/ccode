//! ToolBridge: adapter that wraps `ccode-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the ccode_build agent and `None` for
    ...
    pub async fn kill_background_task(
        &self,
        task_id: &str,
    ) -> Result<KillOutcome, ccode_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn list_scheduled_tasks(
        &self,
    ) -> Vec<crate::implementations::ccode_build::scheduler::types::ScheduledTask> {
        use crate::implementations::ccode_build::scheduler::types::{
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, ccode_tool_runtime::ToolError> {
        use crate::implementations::ccode_build::scheduler::types::{
            SchedulerCommand, SchedulerHandle,
        };
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    ccode_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        sender
            ...
            .map_err(|_| {
                ccode_tool_runtime::ToolError::custom("process_manager", "Scheduler actor stopped")
            })?;
        reply_rx
            .await
            .map_err(|_| {
            ...
            .map_err(crate::implementations::ccode_build::scheduler::types::scheduler_tool_error)
    }
...
mod tests {
    ...
    impl ccode_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> ccode_tool_protocol::ToolId {
            ccode_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::ccode_tool_runtime::ListToolsContext,
        ) -> ccode_tool_types::ToolDescription {
            ccode_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + ccode_build's snake_case in one registry
