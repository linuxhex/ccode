...
//!
//! `MemoryBackend` is defined in `ccode-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `ccode-tools`.
/// Mirrors `ccode_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "ccode_memory";
