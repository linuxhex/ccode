...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.ccode/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.ccode/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.ccode/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.ccode/ or ~/.claude/.
    User { path: PathBuf },
