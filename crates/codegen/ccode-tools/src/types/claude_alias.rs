//! Canonical external-settings tool name ↔ Ccode tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`ccode-hooks`) needs the
//! Ccode tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`ccode-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `ccode` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Ccode, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Ccode [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Ccode tool names this Claude tool maps to (empty when there is no direct
    /// Ccode tool — the entry then only contributes a `kind`).
    ccode: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, ccode: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        ccode,
...
const fn match_only(claude: &'static str, ccode: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        ccode,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (ccode-build reads SKILL.md)
...
/// The Ccode [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Ccode tool names a Claude matcher term fires on.
pub fn ccode_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.ccode)
...
/// The Claude names that map to `ccode_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(ccode_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.ccode.contains(&ccode_name))
        .map(|t| t.claude)
}

/// Every distinct Ccode name the table references, for the `ccode-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn ccode_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.ccode.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Ccode name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.ccode.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
