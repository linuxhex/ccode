...
//!
//! `ToolSearchIndex` is defined in `ccode-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `ccode-shell`
...
/// in `ccode-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `ccode-shell` after MCP initialization.
#[derive(Clone)]
