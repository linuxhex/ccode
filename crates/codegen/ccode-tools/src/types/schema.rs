...
///
/// Use with `#[schemars(with = "CcodeIntegerSchema")]` on `Option<usize>` fields.
pub struct CcodeIntegerSchema;
impl schemars::JsonSchema for CcodeIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "ccode_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `ccode-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use ccode_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
