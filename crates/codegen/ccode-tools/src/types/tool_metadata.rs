//! `ToolMetadata` — ccode-tools-specific metadata for tools.
//!
//! Each tool implements two traits:
//! 1. `ccode_tool_runtime::Tool` — typed Args/Output, `run()` with actual logic
...
//! ## Context helpers
//!
//! Tools access session state through `ccode_tool_runtime::ToolCallContext`
...
/// Ccode-tools-specific metadata trait.
///
/// Each tool struct implements this alongside `ccode_tool_runtime::Tool`.
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Namespace grouping (CcodeBuild, Cursor, OpenCode, ...).
...
) -> Result<SharedResources, ccode_tool_runtime::ToolError> {
    ctx.extensions
        .get::<SharedResources>()
        .map(|arc| (*arc).clone())
        .ok_or_else(|| {
...
pub async fn resolve_cwd(
...
) -> Result<PathBuf, ccode_tool_runtime::ToolError> {
    if let Some(cwd) = ctx.extensions.get::<ccode_tool_runtime::Cwd>() {
    ...
    res.get::<crate::types::resources::Cwd>()
        .map(|c| c.0.clone())
        .ok_or_else(|| {
            ccode_tool_runtime::ToolError::custom("missing_cwd", "Cwd not available in Resources")
        })
}
...
pub fn test_ctx(resources: SharedResources) -> ccode_tool_runtime::ToolCallContext {
    ...
    ctx.extensions
        .insert(ccode_tool_runtime::WorkspaceViewerContext {
...
pub fn test_ctx_with_call_id(
    ...
    let id = ccode_tool_protocol::ToolCallId::new(call_id)
        .unwrap_or_else(|_| ccode_tool_protocol::ToolCallId::new_v7());
    let mut ctx = ccode_tool_runtime::ToolCallContext::new(id);
    ctx.extensions.insert(resources);
    ctx.extensions
        .insert(ccode_tool_runtime::WorkspaceViewerContext {
            stream_tool_progress: true,
...
pub fn behavior_version(ctx: &ccode_tool_runtime::ToolCallContext) -> Option<String> {
    ctx.extensions
