...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (ccode build harness) ─────────────
...
///
/// `"/path/.ccode/skills/my-skill/SKILL.md"` -> `"/path/.ccode/skills/"`
...
/// Used by the budgeted (ccode build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Ccode build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the ccode
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // ccode-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: ccode build harness (budgeted XML) ───────────
