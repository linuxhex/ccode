...
//!
//! The tool runtime contract (`Tool` trait) lives in `ccode_tool_runtime`.
...
///
/// Serializes to snake_case (`ccode_build`, `mcp`, …) for the
...
/// (e.g. `"CcodeBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "CcodeBuild")]
    CcodeBuild,
    #[serde(alias = "CcodeBuildConcise")]
    CcodeBuildConcise,
    #[serde(alias = "CcodeBuildHashline")]
    CcodeBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
