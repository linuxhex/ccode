...
//!
//! Historically the agent hard-coded the dir lists `[".ccode", ".agents",
...
impl CompatCell {
    ...
    /// Whether Ccode currently implements this compatibility surface.
    ///
...
pub const COMPAT_CELLS: [CompatCell; 18] = [
    CompatCell::new(
        ...
        "CCODE_CURSOR_SKILLS_ENABLED",
        Some(CompatRemoteKey::CursorSkills),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Rules,
        "CCODE_CURSOR_RULES_ENABLED",
        Some(CompatRemoteKey::CursorRules),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Agents,
        "CCODE_CURSOR_AGENTS_ENABLED",
        Some(CompatRemoteKey::CursorAgents),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Mcps,
        "CCODE_CURSOR_MCPS_ENABLED",
        Some(CompatRemoteKey::CursorMcps),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Hooks,
        "CCODE_CURSOR_HOOKS_ENABLED",
        Some(CompatRemoteKey::CursorHooks),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Sessions,
        "CCODE_CURSOR_SESSIONS_ENABLED",
        Some(CompatRemoteKey::CursorSessions),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Skills,
        "CCODE_CLAUDE_SKILLS_ENABLED",
        Some(CompatRemoteKey::ClaudeSkills),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Rules,
        "CCODE_CLAUDE_RULES_ENABLED",
        Some(CompatRemoteKey::ClaudeRules),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Agents,
        "CCODE_CLAUDE_AGENTS_ENABLED",
        Some(CompatRemoteKey::ClaudeAgents),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Mcps,
        "CCODE_CLAUDE_MCPS_ENABLED",
        Some(CompatRemoteKey::ClaudeMcps),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Hooks,
        "CCODE_CLAUDE_HOOKS_ENABLED",
        Some(CompatRemoteKey::ClaudeHooks),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Sessions,
        "CCODE_CLAUDE_SESSIONS_ENABLED",
        Some(CompatRemoteKey::ClaudeSessions),
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Skills,
        "CCODE_CODEX_SKILLS_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Rules,
        "CCODE_CODEX_RULES_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Agents,
        "CCODE_CODEX_AGENTS_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Mcps,
        "CCODE_CODEX_MCPS_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Hooks,
        "CCODE_CODEX_HOOKS_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Sessions,
        "CCODE_CODEX_SESSIONS_ENABLED",
...
impl CompatConfig {
    ...
    /// priority order. `.ccode` and `.agents` are always included; `.claude`
    /// and `.cursor` are gated on their respective `skills` cell.
    ///
    /// Replaces the hard-coded `[".ccode", ".agents", ".claude", ".cursor"]`
    ...
    /// Subdirectories scanned for `*.md` rules files. `.ccode/rules` is always
...
mod tests {
    ...
    fn skill_config_dirs_all_on_matches_legacy_constant() {
        // Historical constant was `[".ccode", ".agents", ".claude", ".cursor"]`.
        assert_eq!(
            CompatConfig::default().skill_config_dirs(),
            vec![".ccode", ".agents", ".claude", ".cursor"]
        );
    }
    ...
    fn skill_config_dirs_gates_each_vendor() {
        ...
        assert_eq!(c.skill_config_dirs(), vec![".ccode", ".agents", ".claude"]);

        c.claude.skills = false;
        assert_eq!(c.skill_config_dirs(), vec![".ccode", ".agents"]);

        // Only the `cursor` cell on (`claude` off): `cursor` still appended last.
        let mut c2 = CompatConfig::default();
        c2.claude.skills = false;
        assert_eq!(c2.skill_config_dirs(), vec![".ccode", ".agents", ".cursor"]);
    }

    #[test]
    fn rules_dirs_all_on_matches_legacy_constant() {
        // Historical `RULES_DIRS` was `[".ccode/rules", ".claude/rules", ".cursor/rules"]`.
        assert_eq!(
            CompatConfig::default().rules_dirs(),
            vec![".ccode/rules", ".claude/rules", ".cursor/rules"]
        );
    }
    ...
    fn rules_dirs_gates_each_vendor() {
        ...
        assert_eq!(c.rules_dirs(), vec![".ccode/rules", ".claude/rules"]);
        c.claude.rules = false;
        assert_eq!(c.rules_dirs(), vec![".ccode/rules"]);
    }
