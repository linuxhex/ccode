...
//! `ToolInput`, proto, or runtime). A future `ccode-tool-taxonomy` leaf crate
...
/// object (mirroring `ccode.dev/mcp_tool`). Consumers deserialize it into
/// [`CanonicalToolMeta`].
pub const TOOL_META_KEY: &str = "ccode.dev/tool";
...
impl ToolKind {
    ...
    /// `ccode.dev/tool`. Exhaustive, so a new `ToolKind` must add a label to compile.
...
///   should read `namespace` loosely (as a string) and, on any `ccode.dev/tool`
...
impl CanonicalToolMeta {
    ...
    /// (`bash_mode`, `backend`, `ccode.dev/mcp_tool`, …).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::CcodeBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::CcodeBuild => ("ccode_build", "CcodeBuild"),
                ToolNamespace::CcodeBuildConcise => ("ccode_build_concise", "CcodeBuildConcise"),
                ToolNamespace::CcodeBuildHashline => ("ccode_build_hashline", "CcodeBuildHashline"),
