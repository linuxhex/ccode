...
//! after every tool call to check for SKILL.md files in `.ccode/skills/`,
...
/// Directories that contain skill definitions (`.ccode/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".ccode", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.ccode/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.ccode/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.ccode/) are outside the git root so the walk breaks early.
