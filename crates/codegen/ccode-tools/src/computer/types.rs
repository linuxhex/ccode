...
///
/// Serialized over the wire in the `ccode.dev/task/kill` ext response
/// (`ccode-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (ccode-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in ccode-tools, spawns processes)
/// - `AcpTerminalBackend` (in ccode-shell, calls ACP protocol)
#[async_trait::async_trait]
