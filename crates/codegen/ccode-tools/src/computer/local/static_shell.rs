...
pub use ccode_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__CCODE_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    ccode_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = ccode_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias ccode_alias_probe='echo ALIAS_OK'\nccode_fn_probe() { echo FN_OK; }\n",
            "ccode_alias_probe && ccode_fn_probe",
        )
        .await;
