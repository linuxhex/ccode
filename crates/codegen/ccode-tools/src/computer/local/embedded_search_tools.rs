...
//! config helper `ccode-shell::util::config::resolve_search_tools_enabled`
//! (requirements > env `CCODE_TOOLS_FIND_BFS` / `CCODE_TOOLS_GREP_UGREP` (+
//! `CCODE_FIND_BFS` / `CCODE_GREP_UGREP` aliases, `DISABLE_EMBEDDED_SEARCH_TOOLS`
...
//! (release builds, self-extracted to `~/.ccode/vendor/<name>-<ver>-<target>`) →
//! `~/.ccode/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
//! function (which tags itself with a `__ccode_shadow_{name}` marker) or a
...
// Binaries embedded by build.rs when `CCODE_TOOLS_BUNDLE_{BFS,UGREP}_PATH` is set
// (release pipeline). Self-extracted to `~/.ccode/vendor` on first use, mirroring
// the ripgrep bundling in `ccode_build::grep::ripgrep`.
#[cfg(bundle_bfs)]
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("CCODE_TOOLS_BFS_VER"),
    "-",
    env!("CCODE_TOOLS_BFS_TARGET"),
    ".bin"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("CCODE_TOOLS_UGREP_VER"),
    "-",
    env!("CCODE_TOOLS_UGREP_TARGET"),
    ".bin"
));
...
/// OS binary again. Gated on the `__ccode_shadow_{name}` marker that
...
fn restore_command(name: &str) -> String {
    format!(
        "if [ -n \"${{__ccode_shadow_{name}-}}\" ]; then \
           unalias {name} 2>/dev/null || true; \
           unset -f {name} 2>/dev/null || true; \
           unset __ccode_shadow_{name} 2>/dev/null || true; \
         fi"
    )
}
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "CCODE_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "CCODE_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}

/// Write embedded `bytes` to `~/.ccode/vendor/<versioned_name>` (chmod 755) on
...
fn extract_bundled(versioned_name: &str, bytes: &[u8]) -> std::io::Result<PathBuf> {
    ...
    let dir = crate::util::ccode_home().join("vendor");
...
fn resolve_tool(bin_name: &str, env_override: &str, bundled: Option<PathBuf>) -> Option<PathBuf> {
    resolve_tool_from(
        ...
        bundled,
        crate::util::ccode_home().join("vendor").join(bin_name),
        bin_name,
    )
}

/// Resolution order: explicit env path → bundled (self-extracted) →
/// `~/.ccode/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
/// `__ccode_shadow_{name}=1` marks this as a harness shadow so `restore_command`
...
fn shell_function(
    ...
    // `local __ccode_bin` is re-resolved every call. The host hint is trusted
    ...
    // binary. `|| __ccode_bin=''` keeps the lookup `set -e`-safe (a failed
    ...
    format!(
        "unalias {name} 2>/dev/null || true; \
         {name}() {{ \
           local __ccode_bin={qpref}; \
           [ -x \"$__ccode_bin\" ] || __ccode_bin=$(command -v {bin_name} 2>/dev/null) || __ccode_bin=''; \
           if [ -z \"$__ccode_bin\" ]; then command {name} \"$@\"; return; fi; \
           if [[ -z ${{ZSH_VERSION-}} ]] && (( BASH_SUBSHELL > 0 )); then \
             exec -a {name} \"$__ccode_bin\" {prepend}\"$@\"; \
           else \
             (exec -a {name} \"$__ccode_bin\" {prepend}\"$@\"); \
           fi; \
         }}; \
         __ccode_shadow_{name}=1"
    )
}
...
mod tests {
    ...
    fn shell_function_shape() {
        ...
        // Preferred path is the fast-path hint; the shadow execs `$__ccode_bin`.
        assert!(fn_body.contains("local __ccode_bin=/tmp/bfs"));
        assert!(fn_body.contains("exec -a find \"$__ccode_bin\" \"$@\""));
        // Hint is trusted only when executable (`[ -x ]`, not `[ -f ]`), so a
        // non-exec hint falls through instead of hard-failing exec.
        assert!(fn_body.contains("[ -x \"$__ccode_bin\" ]"));
        assert!(!fn_body.contains("[ -f \"$__ccode_bin\" ]"));
        ...
        assert!(fn_body.contains("__ccode_shadow_find=1"));
    ...
    fn shell_function_unresolved_uses_empty_hint() {
        ...
        assert!(fn_body.contains("local __ccode_bin=''"));
        assert!(fn_body.contains("command -v bfs"));
        assert!(fn_body.contains("command find \"$@\""));
    }
    ...
    fn grep_prepends_ugrep_defaults() {
        ...
        assert!(fn_body.contains("local __ccode_bin=/tmp/ugrep"));
        assert!(fn_body.contains("\"$__ccode_bin\" -G --ignore-files --hidden -I"));
    ...
    fn bash_safe_quote_escapes_metacharacters() {
        ...
        assert!(body.contains("local __ccode_bin='/tmp/evil$(id)'"), "{body}");
    ...
    fn restore_command_is_marker_gated() {
        ...
        assert!(r.contains("if [ -n \"${__ccode_shadow_find-}\" ]"));
        assert!(r.contains("unalias find"));
        assert!(r.contains("unset -f find"));
        assert!(r.contains("unset __ccode_shadow_find"));
    }
    ...
    fn build_injection_off_emits_marker_gated_restore_not_function() {
        ...
        assert!(inject.contains("if [ -n \"${__ccode_shadow_find-}\" ]"));
        assert!(inject.contains("if [ -n \"${__ccode_shadow_grep-}\" ]"));
    ...
    fn build_injection_on_shadows_resolved_tools() {
        ...
        assert!(inject.contains("__ccode_shadow_find=1"));
    }
    ...
    fn resolve_tool_precedence() {
        ...
        let dir = std::env::temp_dir().join(format!("ccode-resolve-{}-{:?}", std::process::id(), {
    ...
    /// `CCODE_TOOLS_BUNDLE_{BFS,UGREP}_PATH` at build time). Verifies the embedded
    /// bytes self-extract under `~/.ccode/vendor` and the extracted `bfs` runs.
    #[cfg(all(bundle_bfs, bundle_ugrep))]
    #[test]
    fn bundled_binaries_extract_and_run() {
        let vendor = crate::util::ccode_home().join("vendor");
    ...
    /// mode-0644 `CCODE_TOOLS_*_PATH` / vendor copy) must fall through to the OS
