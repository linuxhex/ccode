...
impl LocalTerminalActor {
    ...
    async fn spawn_static_command(
        ...
        #[cfg(target_os = "linux")]
        if ccode_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| ccode_sandbox::child_net::install_child_network_filter());
            }
        }
                ...
                && process.exit_status.is_none()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::ccode_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
#[cfg(unix)]
const ENV_LOGIN_ENV: &str = "CCODE_LOGIN_ENV";
...
/// active policy filters out and those already set in ccode's own environment.
#[cfg(unix)]
fn layer_login_env_vars(
    ...
    if let Some(login) = login_env {
        for (key, value) in login {
            // `var_os` reads ccode's own process env (not the possibly cleared
            // child env): a login var already present in ccode's environment is
...
/// policy base, login-shell capture, ccode control vars, request env, pager
...
/// name cannot re-enter; ccode's own control vars, login `PATH`, and the marker
...
/// path must inherit ccode's environment untouched (including non-UTF-8 vars).
...
fn apply_child_env(
    ...
    // 2. Login-shell capture (filtered). 3. Ccode control vars. 4. Request env
    ...
    crate::util::apply_ccode_agent_marker(cmd);
}
...
fn spawn_shell_command(
    ...
    let mut cmd = {
        ...
        // The parent (ccode) process retains network for the LLM API.
    ...
    let mut build_cmd = |with_breakaway: bool| {
        ...
        let inv = ccode_config::shell::shell_command_argv(command);
        ...
        // `inv.env` is ccode's trusted shell setup, so it is not filtered.
        ...
        cmd.envs(crate::util::pager_env());
        crate::util::apply_ccode_agent_marker(&mut cmd);
        ...
        //     have JOB_OBJECT_LIMIT_BREAKAWAY_OK set — common when ccode-agent
...
mod tests {
    ...
    fn apply_child_env_layers_in_fixed_order() {
        ...
        let policy = ShellEnvironmentPolicy {
            ...
            set: HashMap::from([("CCODE_TEST_BASE".to_string(), "1".to_string())]),
            ..Default::default()
        };
        let login = HashMap::from([
            ("CCODE_TEST_LOGIN".to_string(), "l".to_string()),
            ("PATH".to_string(), "/login/bin".to_string()),
        ]);
        let request = HashMap::from([
            ("CCODE_TEST_REQ".to_string(), "r".to_string()),
            ("PATH".to_string(), "/req/bin".to_string()),
            ("CCODE_TEST_SECRET".to_string(), "s".to_string()),
        ]);
        ...
        assert_eq!(env.get("CCODE_TEST_BASE").map(String::as_str), Some("1"));
        assert_eq!(env.get("CCODE_TEST_LOGIN").map(String::as_str), Some("l"));
        assert_eq!(env.get("CCODE_TEST_REQ").map(String::as_str), Some("r"));
        // Request env is filtered by the policy.
        assert!(!env.contains_key("CCODE_TEST_SECRET"));
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export CCODE_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $CCODE_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
    ...
    async fn test_persistent_shell_clears_gpg_tty() {
        ...
        req.env
            .insert("GPG_TTY".to_string(), "/ccode-sentinel-tty".to_string());
