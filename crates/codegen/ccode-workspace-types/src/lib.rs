//! Wire types for the `ccode-workspace` API.
//!
...
//! `ccode-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `ccode-workspace-types` (instead of `ccode-workspace` or
/// `ccode-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `ccode_workspace::permission`
