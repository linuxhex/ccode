...
//!
//! `ccode_hooks` pulls in `git2`/`reqwest`/`ccode-tools`, too heavy for
...
/// Wire mirror of `ccode_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `ccode_hooks::config::HookSpec`.
///
...
/// `ccode-hooks`, so a server-side test (`ccode-workspace`'s
...
/// Wire mirror of `ccode_hooks::event::HookEventName`.
///
