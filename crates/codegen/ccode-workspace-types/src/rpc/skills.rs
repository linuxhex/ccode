...
//! `ccode-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.ccode/skills
    Local,
    /// repo_root/.ccode/skills
    Repo,
    /// ~/.ccode/skills
    User,
    /// ~/.ccode/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    // Fixture mirrored field-for-field from the ccode-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.ccode/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.ccode/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.ccode/plugins/infra-plugin",
            "plugin_data": "/root/.ccode/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "ccode-4",
            "effort": "high",
            "user_invocable": true,
