...
//! the shell's `ccode.dev/fs/*` ACP methods.
...
// workspace server (`ccode-workspace`) and the ccode.dev backend
...
// `workspace.fs_*` ops above already serve the shell's `ccode.dev/fs/*`
...
/// Filesystem node kind. Wire values match the shell's `ccode.dev/fs/list`
/// node `type` strings.
...
/// `ccode.dev/fs/list` plus `offset` pagination).
