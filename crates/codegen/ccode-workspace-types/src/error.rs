...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] ccode_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `ccode-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `ccode_tools::ToolError`) happens
