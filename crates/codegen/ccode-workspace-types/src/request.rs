...
//!   here would force every consumer of `ccode-workspace-types`
...
//! (`ccode-workspace`) wraps this in its own `Request<T>` that
//! adds the cancellation token and extensions map.
//!
//! Splitting the envelope this way keeps `ccode-workspace-types`
