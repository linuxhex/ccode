//! Ccode-owned hook write-deny: plan, identity revalidation, and post-reexec checks.
//! Namespace lockdown is in [`crate::child_net`].

use std::path::{Path, PathBuf};

use ccode_config::{
    GlobalHookSource, ensure_ccode_hook_slots, missing_configured_sources,
    resolve_global_hook_sources,
};

#[cfg(target_os = "linux")]
use ccode_config::unique_ancestors_rootward;
#[cfg(unix)]
use ccode_config::validated_hook_json_files_for_sources;
...
impl From<ccode_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: ccode_config::GlobalHookSourceError) -> Self {
...
fn reject_hardlinked_files(sources: &[GlobalHookSource]) -> Result<(), HookWriteDenyError> {
    use std::os::unix::fs::MetadataExt;
    use ccode_config::GlobalHookSourceKind;
    for s in sources {
...
pub fn resolve_hook_write_deny_snapshot() -> Result<Vec<GlobalHookSource>, HookWriteDenyError> {
    ...
    let resolved =
        resolve_global_hook_sources(Some(ccode.as_path()), /* reject_symlinks */ true)?;
...
pub fn prepare_hook_write_deny(
    ...
    let ccode = ccode_home();
    ensure_ccode_hook_slots(ccode.as_path())?;
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use ccode_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
