...
//! OS-level sandboxing for Ccode Build via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use ccode_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__CCODE_INSIDE_BWRAP";
...
impl SandboxManager {
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        if requires_hook_write_deny(&self.profile, workspace) {
            ccode_config::ensure_ccode_hook_slots(paths::ccode_home().as_path())
...
/// Zero-permission placeholder (file or dir) under `ccode_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent ccode
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::ccode_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    fn bwrap_hook_plan_binds_ancestors_then_leaves() {
        ...
        let sources = [ccode_config::GlobalHookSource {
            path: leaf.clone(),
            kind: ccode_config::GlobalHookSourceKind::ConfiguredSource,
        }];
    ...
    /// Create a temp workspace whose `.ccode/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("ccode-{tag}-{}-{nanos}", std::process::id()));
        let ccode = ws.join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(ccode.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("ccode-deny-dir-{}", std::process::id()));
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::ccode_home()
