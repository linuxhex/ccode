...
//! `off`. Custom profiles via `~/.ccode/sandbox.toml` or `.ccode/sandbox.toml`.
...
/// Load sandbox config from `~/.ccode/sandbox.toml` and `.ccode/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.ccode/sandbox.toml
    let global_path = ccode_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.ccode/sandbox.toml (additive only)
    let project_path = workspace.join(".ccode").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&ccode_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".ccode").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.ccode/ that may not exist on first run.
        ...
        if !profile.write_deny.is_empty() {
            ...
            {
                let files =
                    ccode_config::validated_hook_json_files_for_sources(&profile.write_deny)
    ...
    fn resolve(&self, workspace: &Path, config: &SandboxConfig) -> anyhow::Result<SandboxProfile> {
        match self {
            ...
            Self::Strict => {
                ...
                .chain(std::iter::once(ccode_home()))
                .collect();
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.ccode/sandbox.toml or .ccode/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
...
mod tests {
    ...
    /// Hosts with a retargetable `$CCODE_HOME/hooks` symlink (fail-closed under
...
"#;
    ...
    fn enxio_device_file_is_skipped_but_directory_is_not() {
        ...
        let dir = std::env::temp_dir().join(format!("ccode-sbx-test-{}", std::process::id()));
