...
pub(crate) fn ancestors_within_writable_roots(
    ...
    let mut out = Vec::new();
    for anc in ccode_config::existing_ancestor_chain(path) {
...
pub(crate) fn apply_write_deny_paths_to_capability_set(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        tracing::info!(
            count = entries.len(),
            "Applied Seatbelt write-deny for Ccode-owned direct hook sources"
        );
    }
    #[cfg(target_os = "linux")]
    {
...
mod tests {
    ...
    fn ancestors_pin_under_writable_root_not_home() {
        ...
        let ccode = tmp.join("ccode");
        let sessions = ccode.join("sessions");
        let leaf = sessions.join("extra-hooks");
        std::fs::create_dir_all(&leaf).unwrap();
        let ws = tmp.join("ws");
        std::fs::create_dir_all(&ws).unwrap();

        let roots = [ccode.clone(), ws.clone()];
        let pin = ancestors_within_writable_roots(&leaf, &roots);
        assert!(
            pin.iter().any(|p| p == &sessions),
            "must pin sessions under CCODE_HOME: {pin:?}"
        );
        assert!(
            pin.iter().any(|p| p == &ccode),
            "must pin CCODE_HOME grant root: {pin:?}"
        );
        assert!(
