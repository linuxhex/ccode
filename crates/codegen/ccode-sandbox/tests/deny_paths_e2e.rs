//! E2E path-deny and Ccode hook write-deny (subprocess; arm64-tagged).
...
const CCODE_HOME_ENV: &str = "SANDBOX_E2E_CCODE_HOME";
...
fn apply_fixture_env(cmd: &mut Command, home: &Path, ccode_home: &Path, workspace: &Path) {
    cmd.env(WORKSPACE_ENV, workspace.as_os_str())
        .env(HOME_ENV, home.as_os_str())
        .env(CCODE_HOME_ENV, ccode_home.as_os_str())
        .env("HOME", home.as_os_str())
        .env("CCODE_HOME", ccode_home.as_os_str());
}
...
fn run_scenario(
    ...
    apply_fixture_env(&mut cmd, home, ccode_home, workspace);
    let output = cmd
...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = ccode_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> ccode_sandbox::ProfileName {
    ccode_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    // Isolate HOME/CCODE_HOME before any config OnceLock init.
    let home = PathBuf::from(std::env::var(HOME_ENV).expect(HOME_ENV));
    let ccode_home = PathBuf::from(std::env::var(CCODE_HOME_ENV).expect(CCODE_HOME_ENV));
    ...
    match scenario.as_str() {
        ...
        "hook_write_deny_marker_spoof" => subprocess_hook_write_deny_marker_spoof(&ccode_home),
        other => {
...
fn subprocess_profile_and_bwrap_reexec(profile: &ccode_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !ccode_sandbox::is_inside_bwrap() {
            ...
            match ccode_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = ccode_sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_ccode_home: &Path) {
    #[cfg(not(target_os = "linux"))]
    {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        unsafe {
            std::env::set_var("__CCODE_INSIDE_BWRAP", "1");
        }
        match ccode_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
/// Workspace-profile Ccode-owned hook write-deny probes (existing sources + first-run).
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    let home = PathBuf::from(std::env::var(CCODE_HOME_ENV).expect(CCODE_HOME_ENV));

    let profile = ccode_sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
    ...
    if first_run {
        ...
        eprintln!("OK: first-run Ccode hook slots denied");
    } else {
    ...
    // Parent grants remain creatable: Ccode runtime sibling, workspace, temp.
    assert_write_ok(
        "ccode runtime sibling",
...
/// Create isolated HOME + CCODE_HOME fixture dirs for a scenario.
fn fixture_homes(
    tag: &str,
) -> (
    PathBuf,
    ...
    let ccode = unique_temp_dir(&format!("{tag}-ccode"));
    let workspace = unique_temp_dir(&format!("{tag}-ws"));
    // Empty global sandbox config under fixture CCODE_HOME so generic tests do
    // not inherit the developer/runner's ~/.ccode/sandbox.toml.
    fs::write(ccode.join("sandbox.toml"), "").expect("empty global sandbox.toml");
    (
        home.clone(),
        ccode.clone(),
        workspace.clone(),
...
fn run_deny_case(
    ...
    let (home, ccode, tmp, _ch, _cg, _cw) = fixture_homes(tag);
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".ccode")).expect("mkdir .ccode");
    fs::write(
        tmp.join(".ccode").join("sandbox.toml"),
        format!("[profiles.{profile}]\nextends = \"workspace\"\ndeny = [{deny_list}]\n"),
    )
    .expect("write sandbox.toml");

    // Ensure Ccode fixed slots exist so workspace-based custom profiles can
    // resolve hook write-deny without depending on the real user tree.
    fs::create_dir_all(ccode.join("hooks")).expect("mkdir fixture hooks");
    fs::write(ccode.join("hooks-paths"), b"").expect("write fixture hooks-paths");
    ...
    let (status, stderr) = run_scenario(&home, &ccode, &tmp, profile, targets, controls, postlaunch);
    assert!(
        status.success(),
...
fn hardlinked_hooks_paths_refuses_startup() {
    ...
    let (home, ccode, workspace, _ch, _cg, _cw) = fixture_homes("hook-hl");
    fs::create_dir_all(ccode.join("hooks")).unwrap();
    let reg = ccode.join("hooks-paths");
    let alias = ccode.join("hooks-paths-alias");
    fs::write(&reg, b"").unwrap();
    fs::hard_link(&reg, &alias).unwrap();

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &ccode, &workspace, "hook_write_deny");
    assert!(
        !status.success(),
...
/// Workspace profile: Ccode-owned direct hook sources are write-denied but readable;
/// create / overwrite / unlink / rename / mkdir fail; absolute hooks-paths
/// targets are denied; parent rename is blocked; Ccode/CWD/temp siblings stay writable.
#[test]
fn workspace_protects_direct_hook_sources() {
    ...
    let (home, ccode, workspace, _ch, _cg, _cw) = fixture_homes("hook");

    fs::create_dir_all(ccode.join("hooks")).expect("mkdir hooks");
    fs::write(ccode.join("hooks").join("keep.json"), r#"{"keep-me":true}"#)
        .expect("write keep.json");
    let dynamic = ccode.join("sessions").join("extra-hooks");
    ...
    for needle in [
        ...
        "OK: ccode runtime sibling writable",
...
fn hardlinked_hooks_json_refuses_startup() {
    ...
    let (home, ccode, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-hl");
    fs::create_dir_all(ccode.join("hooks")).unwrap();
    fs::write(ccode.join("hooks-paths"), b"").unwrap();
    let active = ccode.join("hooks").join("active.json");
    let alias = ccode.join("hooks").join("active-alias.json");
...
fn symlinked_hooks_json_refuses_startup() {
    ...
    let (home, ccode, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-sym");
    fs::create_dir_all(ccode.join("hooks")).unwrap();
    fs::write(ccode.join("hooks-paths"), b"").unwrap();
    let real = ccode.join("real-active.json");
    let active = ccode.join("hooks").join("active.json");
...
/// First-run: missing fixed slots are created as real Ccode state before apply,
...
fn workspace_protects_direct_hook_sources_first_run() {
    ...
    let (home, ccode, workspace, _ch, _cg, _cw) = fixture_homes("hook-fr");
    // Intentionally leave hooks/ and hooks-paths absent (first-run ensure path).

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &ccode, &workspace, "hook_write_deny_first_run");
    assert!(
        status.success(),
    ...
    for needle in [
        "OK: first-run Ccode hook slots denied",
    ...
    // Post-exit host: Ccode slots exist and are valid; no vendor artifacts.
    assert!(
        ccode.join("hooks").is_dir(),
        "post-exit: hooks dir must exist as a real directory"
    );
    assert!(
        ccode.join("hooks-paths").is_file(),
        "post-exit: hooks-paths must exist as a real file"
    );
    assert_eq!(
        fs::read(ccode.join("hooks-paths")).expect("read hooks-paths"),
        b"",
...
fn hook_write_deny_refuses_marker_spoof() {
    ...
    #[cfg(target_os = "linux")]
    {
        let (home, ccode, workspace, _ch, _cg, _cw) = fixture_homes("hook-spoof");
        fs::create_dir_all(ccode.join("hooks")).unwrap();
        fs::write(ccode.join("hooks").join("x.json"), b"{}").unwrap();
        fs::write(ccode.join("hooks-paths"), b"").unwrap();
        let (status, stderr) =
            run_hook_write_deny_scenario(&home, &ccode, &workspace, "hook_write_deny_marker_spoof");
        assert!(
            status.success(),
