...
//!
//! `ccode-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "CCODE_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `CcodeComConfig`).
///
/// Built by the binaries (`ccode-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `ccode_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-Ccode-Token-Auth` header (typically `"ccode-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`ccode-pager`) -> `client.name`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://cli-chat-proxy.ccode.dev/v1/traces`).
...
///
/// - `client_name`: binary name (e.g. `"ccode-tui"`, `"ccode-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-ccode-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("ccode-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid CCODE_OTEL_FILTER '{}': {}. Using default '{}'.",
...
struct RefreshableSpanExporter {
    ...
    /// Value for `X-Ccode-Token-Auth`. Only sent when `credentials.needs_token_auth_header()`.
...
fn build_export_headers(
    ...
    snapshot: &ccode_auth::CredentialSnapshot,
    ...
    if let Some(value) = token_auth_header {
        headers.insert("X-Ccode-Token-Auth".to_string(), value.to_string());
    }
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-ccode-client-version".to_string(),
...
mod tests {
    ...
    use std::sync::Mutex;
    use ccode_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("ccode-cli"),
