...
//!
//! Extracted from `ccode-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$CCODE_HOME` is resolved through `ccode-config::ccode_home`.
...
/// This function caches the result in `$CCODE_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = ccode_config::ccode_home().join("agent_id");
...
/// Write `$CCODE_HOME/agent_id` as owner-read/write only (Unix 0o600) — it is a
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    ccode_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
...
/// Returns true when workspace marker env vars (`CCODE_ROOT` and `CCODE_USER`) are set.
///
...
pub fn has_workspace_env_markers() -> bool {
    std::env::var("CCODE_ROOT").is_ok() && std::env::var("CCODE_USER").is_ok()
}

/// Opt-in special-user gate for telemetry.
///
/// Enabled only when `CCODE_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("CCODE_TELEMETRY_SPECIAL_USER").as_deref(),
