...
//! file logger that writes to `~/.ccode/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `ccode_hooks` or `ccode_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! CCODE_HOOKS_LOG=1 ccode              # enable, write to ~/.ccode/logs/hooks.log
//! CCODE_HOOKS_LOG=/tmp/h.log ccode     # write to custom path
//! CCODE_HOOKS_LOG=0 ccode              # explicitly disable
//! tail -f ~/.ccode/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "CCODE_HOOKS_LOG";
...
///
/// Writes to `~/.ccode/logs/hooks.log` (or custom path via `CCODE_HOOKS_LOG`).
/// Filters to hooks (`ccode_hooks`) and plugins (`ccode_agent::plugins`) targets.
/// Set `CCODE_HOOKS_LOG=0` to disable, `CCODE_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "ccode_hooks=debug,ccode_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || ccode_home().join("logs").join("hooks.log");
