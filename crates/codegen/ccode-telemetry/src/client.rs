...
//!
//! Extracted from `ccode-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`ccode-shell-` /
/// `ccode-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("Premium Heavy")
/// and JWT-derived keys ("premium_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "Premium Heavy" | "premium_heavy" => "premium_heavy",
        "Premium" | "premium" => "premium",
        "Premium Lite" | "premium_lite" => "premium_lite",
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("Ccode Code"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("ccode-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("ccode-shell-turn"), "turn");
        assert_eq!(
            event_value("ccode-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("ccode-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("ccode-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("Premium"), "premium");
        assert_eq!(normalize_tier("Premium Heavy"), "premium_heavy");
        assert_eq!(normalize_tier("premium_heavy"), "premium_heavy");
        ...
        assert_eq!(normalize_tier("Premium Lite"), "premium_lite");
    ...
    /// (e.g. a future `"ccode-shell-ext-"` origin), an earlier `ALL` entry could
