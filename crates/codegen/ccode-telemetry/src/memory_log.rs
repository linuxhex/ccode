...
//!
//! Provides a dedicated tracing target (`ccode_memory`) with an optional
//! file logger that writes to `~/.ccode/logs/memory.log`.
//!
//! ## When to use
//!
...
//! CCODE_MEMORY_LOG=0 ccode                # disable even when enabled
//! tail -f ~/.ccode/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "ccode_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "CCODE_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.ccode/logs/memory.log`. Filters to `ccode_memory=trace`.
    /// Set `CCODE_MEMORY_LOG=0` to disable, `CCODE_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || ccode_home().join("logs").join("memory.log");
