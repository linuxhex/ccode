...
//! `Authorization`/`X-Ccode-Token-Auth`/`x-userid`;
...
fn build_resource(cfg: &ExternalOtelConfig) -> opentelemetry_sdk::Resource {
    let mut attrs = vec![
        ...
        opentelemetry::KeyValue::new("ccode_code.schema.version", super::schema::SCHEMA_VERSION),
    ];
    ...
    opentelemetry_sdk::Resource::builder_empty()
        // RQ6 (final): `ccode-cli`, a wire commitment.
...
mod tests {
    ...
    /// `X-Ccode-Token-Auth`, `x-userid`, or `x-teamid` unless customer-supplied
    ...
    fn exporter_headers_are_exactly_customer_headers() {
        ...
        for forbidden in ["Authorization", "X-Ccode-Token-Auth", "x-userid", "x-teamid"] {
            assert!(
    ...
    fn exporter_metadata_is_customer_headers_only() {
        ...
        for forbidden in ["x-ccode-token-auth", "x-userid", "x-teamid"] {
