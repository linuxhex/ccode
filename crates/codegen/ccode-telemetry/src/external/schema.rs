...
/// Wire schema version, exported as resource attr `ccode_code.schema.version`.
/// Additive changes (new events/attrs) do not bump it; renames/removals do.
pub const SCHEMA_VERSION: &str = "v1";

/// Meter/logger instrumentation scope name.
pub const SCOPE_NAME: &str = "ai.ccode.ccode_code";
...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "ccode_code.session_start",
            Self::SessionEnd => "ccode_code.session_end",
            Self::UserPrompt => "ccode_code.user_prompt",
            Self::TurnCompleted => "ccode_code.turn_completed",
            Self::ApiRequest => "ccode_code.api_request",
            Self::ApiError => "ccode_code.api_error",
            Self::ToolResult => "ccode_code.tool_result",
            Self::ToolDecision => "ccode_code.tool_decision",
            Self::McpServerConnection => "ccode_code.mcp_server_connection",
            Self::PermissionModeChanged => "ccode_code.permission_mode_changed",
            Self::SkillActivated => "ccode_code.skill_activated",
            Self::PluginLoaded => "ccode_code.plugin_loaded",
            Self::Compaction => "ccode_code.compaction",
            Self::Subagent => "ccode_code.subagent",
            Self::Auth => "ccode_code.auth",
            Self::InternalError => "ccode_code.internal_error",
            Self::ModelSwitched => "ccode_code.model_switched",
            Self::ContextualTip => "ccode_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `ccode_code.session.count`.
    SessionCount,
    /// `ccode_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `ccode_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `ccode_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `ccode_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `ccode_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "ccode_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "ccode_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "ccode_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "ccode_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "ccode_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "ccode_code.error.count";
...
/// Built-in tool names (closed enum surface in `ccode-tools`) that pass
...
fn tool_outcome_label(o: &ccode_fs::events::types::ToolOutcome) -> &'static str {
    use ccode_fs::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `ccode_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `ccode_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `ccode_code.session_end`.
...
/// `PromptSubmitted` → `ccode_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `ccode_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `ccode_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `ccode_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `ccode_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `ccode_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `ccode_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `ccode_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `ccode_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `ccode_code.permission_mode_changed`.
...
/// `ContextualTip` → `ccode_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `ccode_code.permission_mode_changed`.
...
/// `SkillDispatched` → `ccode_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `ccode_code.plugin_loaded`.
...
/// `PluginUsed` → `ccode_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `ccode_code.compaction`.
...
/// `CompactionTriggered` → `ccode_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `ccode_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `ccode_code.subagent` (`phase=completed`).
...
/// `InternalError` → `ccode_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `ccode_code.model_switched`.
