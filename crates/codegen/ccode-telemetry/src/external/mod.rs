...
//!
//! Enterprise customers point the Ccode CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `CCODE_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `CCODE_EXTERNAL_OTEL` unset (or no exporter
...
//!   data-retention opt-outs (user-confirmed): those govern ccode-side
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &ccode_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             CCODE_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
