...
//!
//! Extracted from `ccode-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `ccode-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `ccode-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "ccode-shell-",
            EmitterOrigin::Workspace => "ccode-workspace-",
        }
    }
}
...
/// event-name prefix (`ccode-shell-*` vs `ccode-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "ccode-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "ccode-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("ccode-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("ccode-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "ccode-workspace-turn");
    }
