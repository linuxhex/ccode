//! Tracing layer for `target: "sampling_log"` → `~/.ccode/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `CCODE_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "CCODE_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = ccode_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
