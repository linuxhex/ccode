...
pub enum TraceUploadReason {
    ...
    FeatureOff,
    /// No ccode.dev auth or deployment key.
    NoCredentials,
    /// Direct-to-bucket S3 upload.
    DirectS3,
    /// Proxy mode via ccode.dev auth.
    Proxy,
...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<ccode_fs::UploadMethod>) -> Self {
        match method {
            Some(ccode_fs::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(ccode_fs::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(ccode_fs::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use ccode_fs::UploadMethod;

    use super::TraceUploadReason;

    /// The `ccode-shell-doom_loop_recovery` Mixpanel event's name and
