...
//! over ACP (`ccode.dev/log` notifications); shell receives them in
...
pub const LOG_METHOD: &str = "ccode.dev/log";
...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "ccode-pager")]
    #[serde(rename = "ccode-pager")]
    CcodePager,
    #[strum(serialize = "ccode-desktop")]
    #[serde(rename = "ccode-desktop")]
    CcodeDesktop,
}
...
/// Wire format for the `ccode.dev/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn log_path() -> PathBuf {
    ccode_home().join(LOG_DIR).join(LOG_FILE)
}
...
///
/// Called by the `ccode.dev/log` notification handler. Entries from
