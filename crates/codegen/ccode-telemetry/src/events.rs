...
//!
//! These structs were extracted from `ccode-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `ccode wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
/// A login method was chosen from the picker. `method` is "ccode" or "api_key";
...
/// A login flow completed successfully. `method` is "ccode" or "api_key";
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage = ccode_tokens::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: ccode_fs::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from ccode-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when Ccode Build is launched from a non-project directory).
...
#[derive(Serialize)]
pub struct PremiumUpsellShown {
    pub source: PremiumUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct PremiumUpsellClicked {
    pub source: PremiumUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`PremiumUpsell`]; lets the funnel attribute the click to the
...
pub struct ClipboardCopy {
    ...
    /// An explicit `ccode wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
/// is lifted — i.e. they subscribed (externally on ccode.dev) and came back.
/// This is the actual conversion signal for Premium Heavy subscriptions
/// attributed to Ccode Build: the user saw the gate in Ccode Build, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Ccode Build's upsell surface.
...
/// `ccode-shell-manual_auth` (the `manual_auth` binding gets the `ccode-shell-`
...
telemetry_event!(PremiumUpsellShown, "premium_upsell_shown");
telemetry_event!(PremiumUpsellClicked, "premium_upsell_clicked");
