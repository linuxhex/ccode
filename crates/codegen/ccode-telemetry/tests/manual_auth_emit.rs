...
//! `ccode-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use ccode_telemetry::client;
use ccode_telemetry::config::{TelemetryConfig, TelemetryMode};
use ccode_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn manual_auth_posts_to_events_endpoint_as_ccode_shell_manual_auth() {
    ...
    ccode_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "ccode-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no ccode-shell-manual_auth POST received"
        );
