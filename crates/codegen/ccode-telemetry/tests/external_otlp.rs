...
fn external_stream_end_to_end() {
    ...
    let mut cfg = ccode_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = ccode_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    ccode_telemetry::external::init(Some(cfg));
    assert!(ccode_telemetry::external::is_active());
    ...
    ccode_telemetry::log_event(ccode_telemetry::events::SessionNew {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("ccode-cli")),
        "service.name=ccode-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "ccode_code.session_start",
        "ccode_code.user_prompt",
        "ccode_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "ccode_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "ccode_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "ccode_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "ccode_code.token.usage"));
    assert_eq!(
    ...
    ccode_telemetry::external::shutdown();
    assert!(
