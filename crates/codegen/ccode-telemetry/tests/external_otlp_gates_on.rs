...
use ccode_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
const SECRET_MODEL: &str = "ccode-4-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    let mut cfg = external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "CCODE_EXTERNAL_OTEL" => Some("1".into()),
    ...
    assert!(!ccode_telemetry::is_enabled());

    ccode_telemetry::log_event(ccode_telemetry::events::SessionHarness {
        ...
        permission_mode: ccode_telemetry::enums::PermissionMode::Ask,
    ...
    ccode_telemetry::log_event(ccode_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "ccode-4".into(),
    ...
    ccode_telemetry::log_event(ccode_telemetry::events::ModelResponseReceived {
    ...
    ccode_telemetry::log_event(ccode_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "ccode_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.ccode.ccode_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("ccode-cli"),
        "service.name=ccode-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("ccode_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "ccode_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "ccode_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "ccode_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "ccode_code.session.count");
