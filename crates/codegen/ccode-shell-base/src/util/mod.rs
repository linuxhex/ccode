...
pub mod tips;
pub mod uname;
pub use ccode_shared::clipboard;
pub use ccode_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True for ccode-operated endpoints (`*.ccode.dev`, cli-chat-proxy, and optional
/// non-production ccode hosts when that feature is enabled).
/// `disable_api_key_auth` refuses keys only for these; other hosts are BYOK and
/// exempt. Safe against invalid URLs and suffix attacks (`evil-ccode.dev.example`).
///
/// Scheme-agnostic so credential *refusal* fails closed. To decide where to
/// *attach* a credential, use [`is_ccode_api_bearer_url`].
pub fn is_ccode_api_url(url: &str) -> bool {
    is_ccode_api_url_impl(url, false)
}
/// Like [`is_ccode_api_url`], but requires `https` on every arm, so a
...
pub fn is_ccode_api_bearer_url(url: &str) -> bool {
    is_ccode_api_url_impl(url, true)
}
fn is_ccode_api_url_impl(url: &str, require_https: bool) -> bool {
    if require_https {
    ...
    reqwest::Url::parse(url)
        ...
        .is_some_and(|host| host == "ccode.dev" || host.ends_with(".ccode.dev"))
}
...
/// True if `pid` is a ccode process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_ccode_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("ccode"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_accepts_proxy_subpath() {
        assert!(is_cli_chat_proxy_url(
            "https://cli-chat-proxy.ccode.dev/v1/chat/completions"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_public_api() {
        assert!(!is_cli_chat_proxy_url("https://api.ccode.dev/v1"));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.ccode.dev.evil.example/v1"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_v11_prefix_confusion() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.ccode.dev/v11/chat/completions"
        ));
    }
    #[test]
    fn test_is_ccode_api_url() {
        assert!(is_ccode_api_url("https://api.ccode.dev/v1"));
        assert!(is_ccode_api_url("https://api.ccode.dev/v1/chat/completions"));
        assert!(is_ccode_api_url("https://ccode.dev"));
        assert!(is_ccode_api_url(
            "https://cli-chat-proxy.ccode.dev/v1/chat/completions"
        ));
        assert!(!is_ccode_api_url("https://api.openai.com/v1"));
        assert!(!is_ccode_api_url("https://api.anthropic.com/v1"));
        assert!(!is_ccode_api_url("https://generativelanguage.googleapis.com"));
        assert!(!is_ccode_api_url("https://api.ccode.dev.evil.example/v1"));
        assert!(!is_ccode_api_url("https://evil-ccode.dev.attacker.com/v1"));
        assert!(!is_ccode_api_url("https://prefixccode.dev/v1"));
        assert!(!is_ccode_api_url("not-a-url"));
        assert!(!is_ccode_api_url(""));
        assert!(is_ccode_api_url("http://api.ccode.dev/v1"));
        assert!(is_ccode_api_url("http://localhost:11434/v1"));
    }
    #[test]
    fn test_is_ccode_api_bearer_url() {
        assert!(is_ccode_api_bearer_url("https://api.ccode.dev/v1"));
        assert!(!is_ccode_api_bearer_url("http://api.ccode.dev/v1"));
        assert!(!is_ccode_api_bearer_url("http://localhost:11434/v1"));
        {
            assert!(!is_ccode_api_bearer_url("https://localhost:11434/v1"));
            assert!(!is_ccode_api_bearer_url("https://127.0.0.2:11434/v1"));
            assert!(!is_ccode_api_bearer_url("https://[::1]:11434/v1"));
        }
        assert!(is_ccode_api_bearer_url("https://API.X.AI/v1"));
        assert!(!is_ccode_api_bearer_url(
            "https://api.ccode.dev@attacker.example/v1"
        ));
        assert!(!is_ccode_api_bearer_url("https://х.ai/v1"));
    }
    #[test]
    fn test_truncate() {
    ...
    #[test]
    fn is_ccode_process_self_true_impossible_pid_false() {
        assert!(is_ccode_process(std::process::id()));
        assert!(!is_ccode_process(u32::MAX));
    }
}
