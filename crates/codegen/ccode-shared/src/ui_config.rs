...
use ccode_config_types::DisplayRefreshSettings;
...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `CCODE_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A Ccode STT
    /// catalog code (`en`, `es`, `ja`, … — see ccode STT supported languages) or
...
pub struct ContextualHints {
    ...
    /// SSH wrap session-load tip (recommend `ccode wrap ssh` when the session
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: ccode_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
