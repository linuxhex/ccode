...
//! image bytes.
//!
//! Both the TUI ([`ccode_pager::prompt_images`]) and the server-side
...
//! [`ccode_pager::prompt_images::display_text`]. The shape of this
...
/// position (the two diverge — see `AttachedImages` in `ccode-tools`).
pub const IMAGE_DISPLAY_NUMBER_META_KEY: &str = "ccode.dev/imageDisplayNumber";
...
/// [`AttachedImages`](ccode_tools::types::resources::AttachedImages))
...
///   separator — see [`ccode_pager::prompt_images::display_text`].
...
/// [`ccode_pager::prompt_images::read_image_at_path`], which has no
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    ccode_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    /// `ccode_pager::prompt_images::display_text`. A
    ...
    fn attached_image_references_prefers_file_path_over_data() {
        ...
        let img = agent_client_protocol::ImageContent::new("AAAA", "image/png")
            .uri(Some(
                "file:///Users/me/.ccode/sessions/s/images/image-1.png".into(),
            ))
        ...
        assert_eq!(
            ...
            vec![(
                1,
                "/Users/me/.ccode/sessions/s/images/image-1.png".to_string()
            )]
        );
    }
