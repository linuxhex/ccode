...
//!
//! Extracted from `ccode-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `ccode-chat::compaction_utils::strip_system_tags` which
...
/// `ccode-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use ccode_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(ccode_sampling_types::synthesized_reasoning_item(text))
    }
