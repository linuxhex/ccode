...
use std::path::Path;
use ccode_agent::config::{AgentDefinition, IsolationMode};
use ccode_agent::plugins::PluginRegistry;
use ccode_agent::prompt::context::{PromptAudience, PromptContext};
...
pub fn discover_agent_definition(
    ...
    ccode_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        context.cwd,
        context.plugins,
    )
...
pub fn available_agent_names(context: &DefinitionResolutionContext<'_>) -> Vec<String> {
    let mut available: Vec<String> = ccode_agent::discovery::all_subagents_with_plugins(
        context.cwd,
        context.toggles,
        context.plugins,
    )
...
pub async fn render_subagent_initial_user_message(
    ...
    let agents_md_files = ccode_agent::prompt::agents_md::read_agents_config_with_paths(
