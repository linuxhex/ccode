...
//!
//! This crate defines the wire format for `ccode.dev/hooks/*` and `ccode.dev/plugins/*`
//! ACP extension methods. It is dependency-free (only `serde`) so both
//! `ccode-shell` and `ccode-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `ccode-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `ccode-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// Ccode's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `ccode-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.ccode/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}

/// Response for `ccode.dev/hooks/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `ccode.dev/plugins/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `ccode.dev/mcp/list` as consumed by the pager.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `ccode.dev/hooks/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `ccode.dev/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Shared action response for both `ccode.dev/hooks/action` and `ccode.dev/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.ccode/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.ccode/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.ccode/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserCcode),
            conflict: None,
        };
...
// ---------------------------------------------------------------------------
// Marketplace types (wire format for ccode.dev/marketplace/* ACP endpoints)
// ---------------------------------------------------------------------------

/// Response for `ccode.dev/marketplace/list`.
...
/// Request wrapper for `ccode.dev/marketplace/action`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
