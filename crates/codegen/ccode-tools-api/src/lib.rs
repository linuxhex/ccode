//! Shared API definitions for Ccode tools: protobuf types, config validation,
...
pub mod pb {
    include!(concat!(env!("OUT_DIR"), "/ccode.tools.v1.rs"));
}
...
///
/// Tool ids are colon-separated `Namespace:tool` (e.g. `CcodeBuild:grep`); the
...
mod default_client_name_tests {
    ...
    fn pins_first_colon_derivation() {
        assert_eq!(default_client_name("CcodeBuild:grep"), "grep");
