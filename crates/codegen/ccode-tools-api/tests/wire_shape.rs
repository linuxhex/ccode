...
//! field rename/retype in `ccode-tools.proto` cannot silently break those
//! wire contracts (the producer and consumer live in separate services).

use ccode_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "CcodeBuild:grep".to_owned(),
...
fn finalize_request_minimal() -> ccode_tools_api::FinalizeToolServerConfigRequest {
    ccode_tools_api::FinalizeToolServerConfigRequest {
        tools: vec![],
        truncation: None,
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "CcodeBuild:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "CcodeBuild:read_file");
...
fn finalize_request_callback_fields_default_when_absent() {
    let back: ccode_tools_api::FinalizeToolServerConfigRequest =
