...
//! `ccode-textarea`, and adds [`SystemClipboard`] backed by `arboard`.
//!
...
pub use ccode_textarea::{ClipboardProvider, InternalClipboard};
...
/// Documented in `ccode-pager/docs/internal/22-environment-variables.md`.
pub const CCODE_COPY_FILE_ENV: &str = "CCODE_COPY_FILE";

/// Cached result of the remote-session check (env vars don't change at runtime).
fn is_remote() -> bool {
    static REMOTE: OnceLock<bool> = OnceLock::new();
    *REMOTE.get_or_init(ccode_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(ccode_shared::clipboard::is_containerized_without_display)
}

/// Cached result of the "an upstream OSC 52 sink is capturing our output" check.
///
/// `ccode wrap` runs a command inside a local PTY, scans its output for OSC 52
/// clipboard sequences, and writes their payload to the *real* (local) system
/// clipboard (see `ccode-pager`'s `pty_wrap` module). It advertises this to
/// the wrapped program via an environment variable so the
/// inner `ccode` knows its OSC 52 writes are reliably intercepted and copied,
...
///
/// Two names are accepted: the canonical `CCODE_OSC52_SINK` (inherited by local
/// children) and the `LC_`-prefixed `LC_CCODE_OSC52_SINK`, which the default
/// OpenSSH client/server configs forward (`SendEnv LANG LC_*` /
/// `AcceptEnv LANG LC_*`) so the signal survives the hop into a remote `ccode`.
pub fn osc52_sink_active() -> bool {
    static SINK: OnceLock<bool> = OnceLock::new();
    *SINK.get_or_init(|| {
        std::env::var_os("CCODE_OSC52_SINK").is_some()
            || std::env::var_os("LC_CCODE_OSC52_SINK").is_some()
    })
}

/// Kill switch: never emit OSC 52 clipboard sequences.
///
/// Set `CCODE_CLIPBOARD_NO_OSC52` (any value) before starting Ccode. Presence
/// forces the OSC 52 leg off for the whole process — including Linux "always
/// emit", tmux, SSH, container, and `CCODE_OSC52_SINK` paths. Use this when the
/// host terminal paints OSC 52 payloads as visible garbage (e.g. OpenText
/// Exceed and other non-supporting emulators).
///
/// Same convention as `CCODE_CLIPBOARD_NO_DATA_CONTROL`: env presence enables
/// the kill switch; resolved once and cached for the process lifetime.
pub fn osc52_disabled() -> bool {
    static DISABLED: OnceLock<bool> = OnceLock::new();
    *DISABLED.get_or_init(|| std::env::var_os("CCODE_CLIPBOARD_NO_OSC52").is_some())
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if ccode_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
/// matters for non-tmux contexts — unless `CCODE_CLIPBOARD_NO_OSC52` is set,
...
fn resolve_clipboard_route_with(ctx: &TerminalContext, no_osc52: bool) -> ClipboardRoute {
    ...
    // upstream `ccode wrap` sink is capturing our output and will forward
    // the sequence to the real clipboard.
    // `CCODE_CLIPBOARD_NO_OSC52` wins over every automatic path.
    let osc52 = !no_osc52
        && (cfg!(target_os = "linux")
            || is_tmux
            || is_remote()
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = ccode_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = ccode_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`ccode_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        ccode_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = ccode_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match ccode_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
impl ClipboardFeedback {
    ...
    fn message(self) -> &'static str {
        match self {
            ...
            Self::UnverifiedOscRemote | Self::UnverifiedOscContainer => {
                "Copy sent. If paste fails, use ccode wrap or /minimal."
            }
...
/// Write text and return a toast; emits `ccode-shell-clipboard_copy` when enabled.
...
///
/// Override with [`CCODE_COPY_FILE_ENV`] (supports `~`). Otherwise
/// `~/.ccode/last-copy.txt` (ccode's per-user home — short, stable, and
/// readable in a toast, unlike macOS's `/var/folders/...` temp dir).
///
/// `None` when no ccode home resolves and the env var is unset: rather than
...
pub fn default_copy_fallback_path() -> Option<std::path::PathBuf> {
    if let Ok(raw) = std::env::var(CCODE_COPY_FILE_ENV) {
    ...
    ccode_config::user_ccode_home().map(|ccode_home| ccode_home.join("last-copy.txt"))
}

/// Render a backup-file path for user-facing messages using the codebase-wide
/// abbreviation convention ([`crate::util::abbreviate_path`]): a ccode-home
/// prefix collapses to `~/.ccode` (or `$CCODE_HOME` when overridden), and a
...
/// ccode) is tightened via `set_permissions` since the create-time `mode`
...
/// `CCODE_COPY_FILE`) — the backup file is skipped rather than written to a
/// predictable temp location.
///
/// On Unix a missing parent directory is created `0700` (a custom
/// `CCODE_COPY_FILE` may point at a not-yet-created private directory;
/// `~/.ccode` normally already exists).
pub fn write_copy_fallback(text: &str) -> std::io::Result<std::path::PathBuf> {
    let Some(path) = default_copy_fallback_path() else {
        return Err(std::io::Error::new(
            ...
            "no home directory resolves; set CCODE_COPY_FILE to enable the copy backup file",
        ));
    };
    #[cfg(unix)]
...
/// clipboard over SSH (notably Apple Terminal without `ccode wrap`); a failed
...
fn log_clipboard_copy_event(
    ...
    if !ccode_telemetry::client::is_enabled() {
        return;
    }
    ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    ccode_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !ccode_shared::clipboard::x11_display_env_present() {
        return None;
    }
    ccode_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && ccode_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    ccode_telemetry::session_ctx::log_event(
        ccode_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match ccode_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use ccode_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    ccode_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    ccode_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    ccode_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(ccode_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match ccode_shared::clipboard::get_image() {
        Ok(img) => {
...
mod tests {
    ...
    fn clipboard_route_matrix() {
        ...
        for case in cases {
            // Pure helper with kill switch off so ambient CCODE_CLIPBOARD_NO_OSC52
    ...
    fn clipboard_route_no_osc52_kill_switch_forces_off() {
        // CCODE_CLIPBOARD_NO_OSC52 must win over Linux/tmux/SSH automatic emit.
        for ctx in [
            plain_terminal_ctx(),
            plain_tmux_ctx(),
            byobu_tmux_ctx(),
    ...
    /// the file must be owner-only (`0600`) — including when an older ccode
    ...
    #[test]
    #[serial_test::serial(ccode_copy_file)]
    fn default_copy_fallback_path_respects_ccode_copy_file() {
        let dir = tempfile::tempdir().expect("tempdir");
        let custom = dir.path().join("custom-copy.txt");
        // SAFETY: test-only env mutation; serialized on the ccode_copy_file key.
        unsafe {
            std::env::set_var(CCODE_COPY_FILE_ENV, &custom);
        }
        let resolved = default_copy_fallback_path();
        unsafe {
            std::env::remove_var(CCODE_COPY_FILE_ENV);
        }
    ...
    /// Without `CCODE_COPY_FILE`, the default is `~/.ccode/last-copy.txt`
    /// (ccode home) — short and toast-friendly, unlike macOS's temp dir.
    #[test]
    #[serial_test::serial(ccode_copy_file)]
    fn default_copy_fallback_path_is_ccode_home() {
        unsafe {
            std::env::remove_var(CCODE_COPY_FILE_ENV);
        }
        let path = default_copy_fallback_path();
        // Test envs always resolve a home (or set CCODE_HOME).
        let expected = ccode_config::user_ccode_home()
            .expect("home resolves in tests")
            .join("last-copy.txt");
        assert_eq!(path, Some(expected));
    }

    /// Toast paths collapse the home prefix to `~` (ccode-home paths go
    /// through the shared `abbreviate_path` convention, covered further by
    /// the `CCODE_HOME`-override integration test in `ccode-pager`).
    #[test]
    fn display_copy_path_abbreviates_home() {
        if std::env::var_os("CCODE_HOME").is_none() {
            let home = dirs::home_dir().expect("home resolves in tests");
            assert_eq!(
                display_copy_path(&home.join(".ccode").join("last-copy.txt")),
                "~/.ccode/last-copy.txt"
            );
        }
        ...
        assert_eq!(
            display_copy_path(std::path::Path::new("/tmp/ccode-0/last-copy.txt")),
            "/tmp/ccode-0/last-copy.txt"
        );
        assert_eq!(
    ...
    fn delivery_clipboard_success_carries_backup_file() {
        let path = std::path::PathBuf::from("/tmp/ccode-1/last-copy.txt");
    ...
    fn toast_message_always_names_backup_file() {
        ...
        assert_eq!(
            plain.toast_message(),
            "Copied! — saved to /tmp/ccode-1/last-copy.txt"
        );
        ...
        assert_eq!(
            ...
            "Copy sent — saved to /tmp/ccode-1/last-copy.txt"
        );
        ...
        assert_eq!(
            ...
            "Clipboard unreachable — wrote /tmp/ccode-1/last-copy.txt"
        );
