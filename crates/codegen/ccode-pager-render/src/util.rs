...
pub use ccode_config::ccode_home;

/// Path to `$CCODE_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    ccode_home().join("pager.toml")
}

/// User-facing label for the user ccode directory (``~/.ccode`` or ``$CCODE_HOME``).
///
/// Derived from resolved [`ccode_home()`] vs `ccode_config::default_ccode_home()`,
/// not from whether `CCODE_HOME` is set in the environment.
pub fn display_ccode_home_prefix() -> String {
    display_ccode_home_prefix_for(&ccode_home())
}

fn display_ccode_home_prefix_for(home: &Path) -> String {
    if home == ccode_config::default_ccode_home() {
        "~/.ccode".to_string()
    } else {
        "$CCODE_HOME".to_string()
    }
}

/// User-facing path under [`ccode_home()`], e.g. ``~/.ccode/config.toml``.
pub fn display_user_ccode_path(relative: impl AsRef<Path>) -> String {
    display_user_ccode_path_for(&ccode_home(), relative)
}

fn display_user_ccode_path_for(home: &Path, relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_ccode_home_prefix_for(home);
...
/// Abbreviate an absolute path for display: prefer [`ccode_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let ccode = ccode_home();
    if let Ok(rest) = path_buf.strip_prefix(&ccode) {
        let prefix = display_ccode_home_prefix();
...
/// True when `path` is under user [`ccode_home()`] (not project `{cwd}/.ccode`).
pub fn is_under_user_ccode_home(path: &Path) -> bool {
    path.starts_with(ccode_home())
}
...
mod tests {
    ...
    #[test]
    fn display_ccode_home_prefix_default_install() {
        if std::env::var("CCODE_HOME").is_ok() {
            return;
        }
        assert_eq!(display_ccode_home_prefix(), "~/.ccode");
    }

    #[test]
    fn display_user_ccode_path_joins_relative() {
        let path = display_user_ccode_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".ccode") || path.contains("$CCODE_HOME"));
    }

    #[test]
    fn display_user_ccode_path_for_custom_home_uses_override_label() {
        let custom = std::env::temp_dir().join("ccode-home-display-regression");
        assert_eq!(
            display_user_ccode_path_for(&custom, "config.toml"),
            "$CCODE_HOME/config.toml"
        );
        assert_eq!(
            display_user_ccode_path_for(&custom, "sandbox.toml"),
            "$CCODE_HOME/sandbox.toml"
        );
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_ccode() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.ccode/memory/MEMORY.md");
