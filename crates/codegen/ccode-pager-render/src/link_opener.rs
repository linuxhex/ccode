...
//!
//! Extracted from the `OpenPremiumUrl` dispatch handler so that any
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("CCODE_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "CCODE_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    ccode_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// The returned command is TTY-guarded via [`ccode_tty::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    ccode_tty::detach_std_command(&mut command);
    command
}
...
///
/// Used by the Premium upsell flow to attribute clicks to `referrer=ccode-build`,
/// matching the OAuth consent screen and ccode.dev/cli marketing links regardless of
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/ccode session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://ccode.dev/premium", "referrer", "ccode-build");
        assert_eq!(out, "https://ccode.dev/premium?referrer=ccode-build");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://ccode.dev/premium?referrer=other",
            "referrer",
            "ccode-build",
        );
        assert_eq!(out, "https://ccode.dev/premium?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://ccode.dev/premium?heavy=1",
            "referrer",
            "ccode-build",
        );
        assert_eq!(
            out,
            "https://ccode.dev/premium?heavy=1&referrer=ccode-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`ccode.dev/#premium`); we still want the referrer attached.
        let out = ensure_query_param("https://ccode.dev/#premium", "referrer", "ccode-build");
        assert_eq!(out, "https://ccode.dev/?referrer=ccode-build#premium");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "ccode-build");
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://ccode.dev/premium?referrer=ccode-build";
