...
pub enum TerminalName {
    ...
    JetBrains,
    /// Ccode Desktop (Electron app).
    #[strum(to_string = "Ccode Desktop")]
    CcodeDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` ccode is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches ccode, which
    ...
    pub fn telemetry_snapshot(&self) -> ccode_telemetry::events::TerminalTelemetry {
        ...
        ccode_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: ccode_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> ccode_shared::session::FeedbackTerminalInfo {
        use ccode_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(ccode_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `ccode ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `ccode-pager-pty/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "ccodedesktop" => Some(TerminalName::CcodeDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.ccode/pager.toml` and
