//! Detects whether ccode is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` ccode is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `ccode-pager-pty/src/pty.rs`
/// (test-env hygiene).
