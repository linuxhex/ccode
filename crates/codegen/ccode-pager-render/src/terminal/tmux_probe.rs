...
fn run_tmux_bounded(
    ...
    let group = ccode_tty::ProcessGroup::new()
        .and_then(|mut group| {
...
fn terminate_tmux_tree(group: &ccode_tty::ProcessGroup, child: &mut std::process::Child) {
    terminate_owned_group(group);
    let _ = child.wait();
}

fn terminate_owned_group(group: &ccode_tty::ProcessGroup) {
    ...
    fn command_protocol_uses_exact_argv_and_pager_env() {
        ...
        for (request, args) in cases {
            ...
            let actual: HashMap<OsString, Option<OsString>> = cmd
                ...
                .collect();
            let expected: HashMap<OsString, Option<OsString>> = ccode_tty::pager_env()
                .into_iter()
