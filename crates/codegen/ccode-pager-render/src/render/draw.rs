...
//! [`ccode_ratatui::Terminal`] whose `flush()` returns `bool` indicating
...
pub type PagerTerminal = ccode_ratatui::Terminal<CrosstermBackend<TermWriter>>;
#[derive(Debug)]
...
pub fn spawn_writer_thread() -> (
    ...
    let test_delay = std::env::var("CCODE_TEST_FRAME_WRITE_DELAY_MS")
        .ok()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || -> std::io::Result<()> {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = ccode_tty::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(payload) = rx.recv() {
                ...
                let result = {
                    let _guard = ccode_shared::stderr::stderr_lock();
