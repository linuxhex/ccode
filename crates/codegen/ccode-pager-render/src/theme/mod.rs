...
//! The default theme is CcodeNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! CcodeNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::CcodeNight, ThemeKind::CcodeDay];
    ...
    /// when quantized to 256 or 16 colors. CcodeNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::CcodeNight => false,
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.ccode/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `CCODE_FORCE_COLOR_LEVEL=basic`),
    ...
    /// theme (e.g., CcodeNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // CcodeNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    ccode_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::CcodeNight.is_auto());
        assert!(!ThemeKind::CcodeDay.is_auto());
