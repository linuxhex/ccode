...
//! lookup must be cheaper than re-loading from `~/.ccode/config.toml`.
...
//! `ccode_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::CcodeNight as u8);
...
/// back to `CcodeNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::CcodeNight as u8 => ThemeKind::CcodeNight,
        x if x == ThemeKind::CcodeDay as u8 => ThemeKind::CcodeDay,
...
/// When `None`, `to_theme_kind()` defaults to `CcodeNight` / `CcodeDay`.
...
///
/// On the first call, reads from `~/.ccode/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    ccode_markdown::set_color_level_cap(if locked {
        ccode_markdown::ColorLevel::Basic
    } else {
        ccode_markdown::ColorLevel::TrueColor
    });
    ccode_markdown::set_polarity_safe_syntax(locked);
}
...
///
/// Precedence:
/// 1. Environment variable (`CCODE_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `CcodeNight`
#[must_use]
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::CcodeNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `CcodeNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `ccode_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = ccode_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = ccode_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::CcodeNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::CcodeDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::CcodeNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::CcodeDay);
            assert_eq!(applied, ThemeKind::CcodeNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::CcodeNight);
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!ccode_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                ccode_markdown::polarity_safe_syntax(),
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::CcodeDay), false),
                ThemeKind::CcodeDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::CcodeNight);
        });
    }
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::CcodeDay), true);
            assert_eq!(result, ThemeKind::CcodeDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::CcodeDay);
            assert_eq!(current_kind(), ThemeKind::CcodeDay);
        });
    }
