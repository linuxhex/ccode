...
/// in `config.toml`. When `None`, defaults to `CcodeNight` / `CcodeDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::CcodeDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::CcodeNight),
    }
}
