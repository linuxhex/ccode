...
//! `current_kind()` is a nominal `CcodeNight` (so leftover kind-keyed paths
...
pub use ccode_markdown::Syntect;
...
static SYNTECT_CCODE_NIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_CCODE_DAY: OnceLock<Syntect> = OnceLock::new();
...
/// reports a nominal `CcodeNight`, so this returns the night theme. Token
