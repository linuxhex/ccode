...
// (`~/.ccode/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_ccode_home>/trusted-hook-projects`), or `None` when no user ccode home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(ccode_config::user_ccode_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user ccode home (set $CCODE_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$CCODE_HOME/disabled-hooks`, or `None` when no user ccode
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(ccode_config::user_ccode_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let ccode_dir = dir.join(".ccode");
        std::fs::create_dir_all(&ccode_dir).unwrap();
        ccode_dir.join("trusted-hook-projects")
    }
