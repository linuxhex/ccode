...
pub struct HookSpec {
    ...
    ///    `CCODE_SESSION_ID`, `CCODE_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are
    ///    stripped at load time with a tracing warning.
    /// 2. Plugin-injected vars merged by the plugin adapter
    ///    (`ccode-agent::plugins::hooks_adapter`): `CCODE_PLUGIN_ROOT`,
    ///    `CLAUDE_PLUGIN_ROOT`, `CCODE_PLUGIN_DATA`, `CLAUDE_PLUGIN_DATA`, which
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.ccode/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.ccode/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "CCODE_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "CCODE_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "CCODE_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "CCODE_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "CCODE_HOOKS_PARSE_TEST_MATCHER_VAR";
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "CCODE_HOOK_EVENT": "spoofed",
                                    "CCODE_HOOK_NAME": "spoofed",
                                    "CCODE_SESSION_ID": "spoofed",
                                    "CCODE_WORKSPACE_ROOT": "/etc",
