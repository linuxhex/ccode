...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("CCODE_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = ccode_config::shell::shell_command_argv(&command_str);
    ...
    // plugin) can spoof `CCODE_HOOK_EVENT`, `CCODE_HOOK_NAME`, `CCODE_SESSION_ID`,
    // `CCODE_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR`, which are the
    ...
    let mut child = match cmd
        ...
        .env("CCODE_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("CCODE_HOOK_NAME", &spec.name)
        .env("CCODE_SESSION_ID", ctx.session_id)
        .env("CCODE_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `CCODE_WORKSPACE_ROOT`; native `.ccode` hooks should use
        // `CCODE_WORKSPACE_ROOT`.
...
///   hooks),
/// * the Ccode process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_command_path_variants() {
        ...
        assert_eq!(
            ...
            Some(std::path::PathBuf::from(
                "/project/.ccode/hooks/bin/check.sh"
            ))
        );
        assert_eq!(
    ...
    fn find_unresolved_skips_resolvable_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${CCODE_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${CCODE_SESSION_ID}/${CLAUDE_PLUGIN_ROOT}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.ccode-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".ccode-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.ccode-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.ccode-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.ccode-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
