...
fn load_hooks_from_directory(dir: &Path) -> (Vec<HookSpec>, Vec<HookError>) {
    ...
    // hooks still load. (Sandbox fail-closed listing lives in ccode_config.)
    ...
    for entry in entries {
        ...
        if !ccode_config::is_direct_hook_json_name(name) || !path.is_file() {
            continue;
        }
...
fn is_valid_hook_file(path: &Path) -> bool {
    ...
    ccode_config::is_direct_hook_json_name(name) && path.is_file()
}
...
mod tests {
    ...
    fn load_from_settings_file_no_hooks_key() {
        let dir = tempfile::tempdir().unwrap();
        let settings = dir.path().join("settings.json");
        std::fs::write(&settings, r#"{"theme": "dark", "model": "ccode-3"}"#).unwrap();
