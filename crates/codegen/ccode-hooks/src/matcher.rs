use regex::Regex;
use ccode_tools::types::{claude_names_for, ccode_names_for};
...
///   **exact** match against each name (after external→Ccode alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the Ccode tool `run_terminal_command`).
///
...
/// plus any Ccode tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to Ccode registry in
/// `ccode-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for ccode_name in ccode_names_for(term) {
            push(ccode_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_ccode_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // Ccode name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_ccode_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // Ccode equivalent
        assert!(m.is_match("hashline_edit")); // second Ccode alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn regex_against_claude_alias_matches_ccode_tool() {
        // A regex written against an external alias still matches the Ccode tool
