...
/// execution details (for scrollback enrichment), optional hook rewrite
/// information, and the full permission chain result.
...
/// but the tool call continues as if the hook had allowed it. Ccode
...
///
/// When `rules` is `Some`, the full permission decision chain is evaluated
/// after hook dispatch: pre-filter → hook decision → rule engine → default.
/// The chain result is returned in `PreToolUseResult::chain_result`.
///
/// Hook rewrite information (updatedInput, additionalContext) is extracted
/// from hook stdout via `parse_hook_output` and returned in
/// `PreToolUseResult::rewrite`.
pub async fn dispatch_pre_tool_use(
    registry: &HookRegistry,
    envelope: &HookEventEnvelope,
    ctx: &RunContext<'_>,
    rules: Option<&PermissionRuleSet>,
    auto_mode: bool,
) -> PreToolUseResult {
    let hooks = registry.hooks_for(HookEventName::PreToolUse);
    if hooks.is_empty() && rules.is_none() {
    ...
    let (tool_name, tool_input) = match &envelope.payload {
        crate::event::HookPayload::PreToolUse { tool_name, tool_input, .. } => {
            (tool_name.clone(), tool_input.clone())
        }
    ...
    for spec in hooks {
        ...
        match result {
            HookRunnerResult::Decision(HookDecision::Deny { reason, .. }) => {
                ...
                hook_decision = Some(HookDecision::Deny {
                    reason,
                    hook_name: spec.name.clone(),
                });
                record_dispatch_counts(&span, &run_results);
                // Short-circuit: deny from hook
                break;
            }
            HookRunnerResult::Decision(HookDecision::Allow) => {
                ...
                hook_decision = Some(HookDecision::Allow);
            }
    ...
    // Evaluate the full permission decision chain if rules are provided
    let chain_result = rules.map(|rules| {
        permission_chain::evaluate_permission_chain(
            &tool_name,
            &tool_input,
            rules,
            hook_decision.clone(),
    ...
    // If the chain overrides the hook decision (e.g. pre-filter deny),
    // the chain result takes precedence.
    let final_decision = if let Some(ref chain) = chain_result {
        match &chain.decision {
            crate::permission_rules::PermissionDecision::Deny { reason } => {
                HookDecision::Deny {
                    reason: reason.clone(),
                    hook_name: String::new(),
                }
            }
            _ => hook_decision.unwrap_or(HookDecision::Allow),
        }
    } else {
        hook_decision.unwrap_or(HookDecision::Allow)
    };
...
mod tests {
    ...
    async fn empty_registry_allows() {
        ...
        let result = dispatch_pre_tool_use(&registry, &envelope, &run_ctx(), None, false).await;
    ...
    async fn matcher_filters_by_tool() {
        ...
        let skipped =
            dispatch_pre_tool_use(&registry, &pre_tool_use_envelope("read_file"), &run_ctx(), None, false).await;
