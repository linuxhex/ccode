...
Sample hooks for Ccode. Copy to `~/.ccode/hooks/` to enable globally, or to `<project>/.ccode/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.ccode/hooks/bin
cp examples/hooks/safe-shell.json ~/.ccode/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.ccode/hooks/bin/
chmod +x ~/.ccode/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.ccode/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.ccode/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.ccode/hooks/bin/
chmod +x ~/.ccode/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.ccode/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.ccode/hooks/bin
cp examples/hooks/session-log.json ~/.ccode/hooks/
cp examples/hooks/bin/session-log.sh ~/.ccode/hooks/bin/
chmod +x ~/.ccode/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.ccode/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.ccode/hooks/bin
cp examples/hooks/tool-logger.json ~/.ccode/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.ccode/hooks/bin/
chmod +x ~/.ccode/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.ccode/hooks/bin
cp examples/hooks/stop-verify.json ~/.ccode/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.ccode/hooks/bin/
chmod +x ~/.ccode/hooks/bin/stop-verify.sh
```
...
- **Event names:** `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, `SessionEnd` (see the [user guide](../../ccode-pager/docs/user-guide/10-hooks.md) for the full set)
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Ccode names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.ccode/hooks/`. The hook stops running on the next session.