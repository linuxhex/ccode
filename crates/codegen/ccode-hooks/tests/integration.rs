//! Integration tests for ccode-hooks.
//!
...
use ccode_hooks::event::*;
use ccode_hooks::result::HookDecision;
...
async fn hook_deny_via_exit_code_only() {
    ...
    let pre_result =
        dispatcher::dispatch_pre_tool_use(&registry, &pre_tool_use_envelope("read_file"), &ctx, None, false)
            .await;
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: ccode_hooks::event::StopFailureKind::RateLimit,
...
/// runner-reserved keys (`CCODE_HOOK_EVENT`, `CCODE_HOOK_NAME`,
/// `CCODE_SESSION_ID`, `CCODE_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$CCODE_HOOK_EVENT" > {f}; echo "NAME=$CCODE_HOOK_NAME" >> {f}; echo "SESSION=$CCODE_SESSION_ID" >> {f}; echo "ROOT=$CCODE_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "CCODE_HOOK_EVENT": "spoofed_event",
                                "CCODE_HOOK_NAME": "spoofed_name",
                                "CCODE_SESSION_ID": "spoofed_session",
                                "CCODE_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "CCODE_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed CCODE_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "CCODE_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed CCODE_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "CCODE_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed CCODE_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
