...
//! `ccode_sampling_types::SamplingConfig` so that the sampler crate
...
/// composing chat-state's `ccode_sampling_types::SamplingConfig`
/// with `Credentials` (api key, client version).
///
/// URL-derived request headers (e.g. `X-Ccode-Token-Auth` for the
/// cli-chat-proxy) are
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-ccode-doom-loop-check` header on
