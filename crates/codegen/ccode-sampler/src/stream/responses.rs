...
pub(crate) fn stream_responses_tracked<'a>(
    ...
    async_stream::stream! {
        ...
        let mut items = ccode_sampling_types::response_to_conversation_items(response);
        ccode_sampling_types::inject_streaming_reasoning_fallback(&mut items, reasoning_acc);
...
mod tests {
    ...
    async fn doom_loop_collector_signals_land_on_completed_response() {
        use ccode_sampling_types::doom_loop::{
