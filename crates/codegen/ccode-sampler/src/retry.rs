...
/// Resolve max API retries: `CCODE_MAX_RETRIES` env > model config > default ([`DEFAULT_MAX_RETRIES`]).
pub fn resolve_max_retries(model_max_retries: Option<u32>) -> u32 {
    let env_override = std::env::var("CCODE_MAX_RETRIES").ok();
    ...
    fn zero_retry_budget_never_reuses_a_model_output_cap() {
        for err in [
            ...
            SamplingError::EmptyResponse {
                context: ccode_sampling_types::EmptyResponseContext {
                    reason: ccode_sampling_types::EmptyReason::NoVisibleContent,
                    had_reasoning: false,
                    content_len: 0,
                    tool_call_count: 0,
