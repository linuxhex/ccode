...
//! it" buckets.
//!
//! `ccode-sampler` is intentionally decoupled from `ccode-shell`
...
/// tools live in `ccode-tools`
/// (`crates/codegen/ccode-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `ccode_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `ccode_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `ccode-shell/src/auth/manager.rs` to match, otherwise the
