//! HTTP client for the ccode sampling APIs.
//!
...
use ccode_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use ccode_sampling_types::ApiBackend;

/// Process-level fallback for the `x-ccode-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "ccode-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "ccode-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-ccode-*` headers. Optional fields are skipped when empty/`None`.
struct CcodeRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
impl CcodeRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-ccode-conv-id", self.conv_id)
            .header("x-ccode-req-id", self.req_id)
            .header("x-ccode-model-override", self.model_id)
            .header("x-ccode-session-id", self.session_id)
            .header("x-ccode-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-ccode-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-ccode-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-ccode-user-id", id);
        }
        b
    }
}
...
/// Deserialize a Responses API SSE event, with a fallback for ccode-specific
...
/// when the API emits the ccode-specific `context_details` field.
    ...
    let mut event = match serde_json::from_str::<rs::ResponseStreamEvent>(data) {
        ...
        Err(first_err) => {
            ...
            if let Ok(mut value) = serde_json::from_str::<serde_json::Value>(data) {
                ...
                // (e.g., ccode-specific "x_search"). Instead of maintaining a
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = ccode_sampling_types::reported_cost_ticks(
        value
...
pub(crate) const COST_USD_TICKS_METADATA_KEY: &str = "ccode.cost_usd_ticks";
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-ccode-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-ccode-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<ccode_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-ccode-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-ccode-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-ccode-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-ccode-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-ccode-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    pub async fn chat_completion(
        ...
        let x_ccode_conv_id = &payload.x_ccode_conv_id.clone().unwrap_or_default();
        let x_ccode_req_id = &payload.x_ccode_req_id.clone().unwrap_or_default();
        ...
        let ccode_headers = CcodeRequestHeaders {
            conv_id: x_ccode_conv_id,
            req_id: x_ccode_req_id,
            model_id: &model_id,
            session_id: payload.x_ccode_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_ccode_turn_idx.as_deref(),
            agent_id: payload.x_ccode_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_ccode_deployment_id.as_deref(),
            user_id: payload.x_ccode_user_id.as_deref(),
        };
        let http_request = ccode_headers
    ...
    pub async fn create_response(
        ...
        let x_ccode_conv_id = request.x_ccode_conv_id.as_deref().unwrap_or_default();
        let x_ccode_req_id = request.x_ccode_req_id.as_deref().unwrap_or_default();
        ...
        let ccode_headers = CcodeRequestHeaders {
            ...
            req_id: x_ccode_req_id,
            model_id: &model_id,
            session_id: request.x_ccode_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_ccode_turn_idx.as_deref(),
            agent_id: request.x_ccode_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_ccode_deployment_id.as_deref(),
            user_id: request.x_ccode_user_id.as_deref(),
        };
        ...
        ccode_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = ccode_headers
    ...
    /// gate that adds the opt-in `x-ccode-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        // Inject ccode-specific fields not in async-openai's CreateResponse type.
        if self.defaults.stream_tool_calls {
            request_body["stream_tool_calls"] = serde_json::json!(true);
        }
        // Inject ccode-specific tools (e.g., x_search) that can't be expressed
        ...
        let mut http_request = ccode_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_ccode_conv_id = request.x_ccode_conv_id.clone();
        let x_ccode_req_id = request.x_ccode_req_id.clone();
        let x_ccode_session_id = request.x_ccode_session_id.clone();
        let x_ccode_turn_idx = request.x_ccode_turn_idx.clone();
        let x_ccode_agent_id = request.x_ccode_agent_id.clone();

        // Collect ccode-specific tools that can't be expressed via rs::Tool
        // (e.g., x_search). These are injected as raw JSON after serialization.
        let extra_tools = ccode_sampling_types::extra_tool_entries(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_ccode_conv_id = x_ccode_conv_id;
        wrapper.x_ccode_req_id = x_ccode_req_id;
        wrapper.x_ccode_session_id = x_ccode_session_id;
        wrapper.x_ccode_turn_idx = x_ccode_turn_idx;
        wrapper.x_ccode_agent_id = x_ccode_agent_id;
...
mod tests {
    use super::*;
    use indexmap::IndexMap;
    use ccode_sampling_types::types::ChatRequestMessage;
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            reasoning_effort: None,
            x_ccode_conv_id: None,
            x_ccode_req_id: None,
            x_ccode_session_id: None,
            x_ccode_turn_idx: None,
            x_ccode_agent_id: None,
            x_ccode_deployment_id: None,
            x_ccode_user_id: None,
            trace: None,
        };
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-CCODE-token-auth".to_string(), "ccode-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the ccode-shell agent product.
        assert!(ua.starts_with("my-client ccode-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = ccode_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "ccode-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
