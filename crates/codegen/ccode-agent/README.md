...
`ccode-shell`, another in-process host, or a headless batch runner.
...
in `.ccode/agents/` (project-level) or `~/.ccode/agents/` (user-level).

```rust
use ccode_agent::{AgentDefinition, AgentBuilder};
use ccode_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".ccode/agents/code-reviewer.md")?;
...
```rust
use ccode_agent::discovery;

// Find all .md files in .ccode/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  ccode-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`ccode-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `ccode-agent` depends on it for tool setup.
- **`ccode-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `ccode-agent` (AGENTS.md
...
|---|---|---|
| `ccode-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p ccode-agent

# Test
cargo test -p ccode-agent

# Clippy
cargo clippy -p ccode-agent --fix --allow-dirty
