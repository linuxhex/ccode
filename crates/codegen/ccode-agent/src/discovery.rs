...
//!
//! Searches `.ccode/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.ccode/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.ccode/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".ccode/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.ccode/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.ccode/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), ccode.as_deref())
}
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, ccode_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.ccode/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.ccode/agents/` walking from `cwd` up to repo root
/// 2. `~/.ccode/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.ccode/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user ccode agents, `.claude`
/// compat agents, then bundled. `.ccode` dirs resolve from `ccode_home`
/// (CCODE_HOME-aware) plus the legacy literal `~/.ccode` when CCODE_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    ccode_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.ccode, included only when it differs from ccode_home
    // (i.e. CCODE_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_ccode = home
        .map(|h| h.join(".ccode"))
        .filter(|legacy| ccode_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = ccode_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_ccode {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let ccode = ccode_config::user_ccode_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), ccode.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, ccode_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let ccode = ccode_config::user_ccode_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), ccode.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, ccode_home);
...
///
/// Project-level `.ccode/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let ccode = ccode_config::user_ccode_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), ccode.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, ccode_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, ccode_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Ccode
...
/// Load project agent definitions from every `.ccode/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectCcode,
            PluginScope::User => PluginOrigin::UserCcode,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_ccode_when_ccode_home_differs() {
        let home = Path::new("/home/u");
        let ccode = Path::new("/custom/ccode-home");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(ccode))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&ccode.join("agents")));
        assert!(paths.contains(&home.join(".ccode").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&ccode.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".ccode").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_ccode_home_is_dot_ccode() {
        let home = Path::new("/home/u");
        let ccode = home.join(".ccode");
        let count = user_agent_dirs(Some(home), Some(&ccode))
            .into_iter()
            .filter(|(p, _)| *p == ccode.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.ccode/agents when ccode_home == ~/.ccode"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_ccode_build() {
        let def = by_name("ccode-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "ccode-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".ccode").join("agents");
        let agents_dir_2 = inner_dir.join(".ccode").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".ccode").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".ccode")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".ccode")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".ccode").join("agents");
        let bundled_dir = home.join(".ccode").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".ccode")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".ccode")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".ccode").join("agents");
        let bundled_dir = home.join(".ccode").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "ccode-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "ccode-build.md",
            "ccode-build",
        ...
        let def = by_name_in_cwd("ccode-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "ccode-build");
        assert_eq!(def.description, "Custom ccode-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .ccode/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("ccode-build-orchestrator")
            .expect("from_str must resolve ccode-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::CcodeBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "ccode-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("ccode-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find ccode-build-orchestrator");
        assert_eq!(def.name, "ccode-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.ccode/agents/) agent named "explore" should NOT shadow
