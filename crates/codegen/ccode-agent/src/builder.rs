...
use std::sync::Arc;
use ccode_tools::bridge::ToolBridge;
use ccode_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
/// The Ccode [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-Ccode
/// tool registry in `ccode-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    ccode_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.ccode/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<ccode_tools::implementations::ccode_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn ccode_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: ccode_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: ccode_tools::implementations::ccode_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn ccode_tools::implementations::lsp::LspBackend>>,
    image_gen_config: ccode_tools::implementations::ccode_build::image_gen::ImageGenConfig,
    video_gen_config: ccode_tools::implementations::ccode_build::video_gen::VideoGenConfig,
    ...
    api_key_provider: Option<ccode_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<ccode_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in ccode-shell).
    ...
    preloaded_skills: Option<Vec<ccode_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut ccode_tools::registry::types::ToolServerConfig) {
    use ccode_tools::implementations::ccode_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("CcodeBuild:enter_plan_mode");
    let missing_exit = !existing.contains("CcodeBuild:exit_plan_mode");
    let missing_ask = !existing.contains("CcodeBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&ccode_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&ccode_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&ccode_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut ccode_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(ccode_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<ccode_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn ccode_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: ccode_tools::implementations::ccode_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: ccode_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `CCODE_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: ccode_tools::implementations::ccode_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn ccode_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    /// allowing image generation via the ccode Imagine API with session
    /// credentials. When `Disabled` (default), the tool is not registered.
    pub fn with_image_gen_config(
        mut self,
        config: ccode_tools::implementations::ccode_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    /// allowing video generation via the ccode Video Generation API with
    /// session credentials. When `Disabled` (default), the tool is not
    /// registered.
    pub fn with_video_gen_config(
        mut self,
        config: ccode_tools::implementations::ccode_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: ccode_tools::implementations::ccode_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: ccode_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the CcodeBuild Task description.
    ...
    ///
    /// When disabled, `CcodeBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.ccode/skills, ~/.ccode/skills)
    /// are included — custom paths added via `ccode.dev/skills/add` would be ignored.
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use ccode_tools::implementations::ccode_build;
                tool_config.tools.push((&ccode_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use ccode_tools::implementations::ccode_build;
                tool_config.tools.push((&ccode_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&ccode_tools::implementations::ccode_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&ccode_tools::implementations::ccode_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&ccode_tools::implementations::ccode_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&ccode_tools::implementations::ccode_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&ccode_tools::implementations::ccode_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&ccode_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let ccode_build_ns = ccode_tools::types::tool::ToolNamespace::CcodeBuild.to_string();
            let mem_search_id = format!(
                "{ccode_build_ns}:{}",
                ccode_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{ccode_build_ns}:{}",
                ccode_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                ccode_tools::types::tool::ToolNamespace::CcodeBuild,
            );
        ...
        if task_stripped {
            use ccode_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::CcodeBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::CcodeBuildConcise, "run_terminal_cmd", true)
        ...
        if let ccode_tools::implementations::ccode_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["CcodeBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "CcodeBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    "tools allowlist had unmappable entries; keeping full ccode toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                ccode_tools::types::resources::TruncationCfg(
                    ccode_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(ccode_sampling_types::HostedTool::WebSearch { options: None });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(ccode_sampling_types::HostedTool::XSearch { options: None });
            }
            ccode_sampling_types::apply_tool_overrides(
                &mut hosted_tools,
...
/// CLI naming for the shared [`ccode_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: ccode_tool_types::TaskToolNaming<'static> = ccode_tool_types::TaskToolNaming {
...
/// CLI [`ccode_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: ccode_tool_types::SubagentToolNaming<'static> =
    ccode_tool_types::SubagentToolNaming {
...
/// shared [`ccode_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => ccode_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => ccode_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => ccode_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`ccode_tool_types::SubagentDescriptor`] and defers to
/// [`ccode_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<ccode_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            ccode_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = ccode_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        ccode_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: ccode_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(ccode_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(ccode_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: ccode_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("ccode", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use ccode_tools::types::template_renderer::TemplateRenderer;
        use ccode_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use ccode_tools::computer::local::LocalTerminalBackend;
        use ccode_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "ccode-build / subagents+ask_user",
                profile: AgentDefinition::default_ccode_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ccode-build / subagents / no-ask-user",
                profile: AgentDefinition::default_ccode_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ccode-build / no-subagents / ask_user",
                profile: AgentDefinition::default_ccode_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ccode-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_ccode_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ccode-build-ask-user / subagents",
                profile: AgentDefinition::ccode_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ccode-build-ask-user / no-subagents",
                profile: AgentDefinition::ccode_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ccode-build-plan",
                profile: AgentDefinition::ccode_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ccode-build-plan / no-ask-user",
                profile: AgentDefinition::ccode_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ccode-build-plan-no-subagents",
                profile: AgentDefinition::ccode_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ccode-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::ccode_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use ccode_tools::computer::local::LocalTerminalBackend;
        use ccode_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_ccode_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use ccode_tools::computer::local::LocalTerminalBackend;
        use ccode_tools::implementations::ccode_build::ask_user_question::AskUserQuestionParams;
        use ccode_tools::notification::ToolNotificationHandle;
        use ccode_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_ccode_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "CcodeBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use ccode_tools::computer::local::LocalTerminalBackend;
        use ccode_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_ccode_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("CcodeBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use ccode_tools::computer::local::LocalTerminalBackend;
        use ccode_tools::implementations::ccode_build::bash::BashParams;
        use ccode_tools::notification::ToolNotificationHandle;
        use ccode_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_ccode_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Ccode
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_ccode_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their ccode
    ...
    /// treat `mcp__*` as always-on, so ccode keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        ...
        use ccode_tools::implementations::web_search::WebSearchConfig;
        use ccode_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_ccode_build();
    ...
    /// ccode-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to ccode's `search_tool` (MCP
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<ccode_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use ccode_tools::computer::local::LocalTerminalBackend;
        use ccode_tools::implementations::web_search::WebSearchConfig;
        use ccode_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, ccode_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, ccode_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = ccode_sampling_types::XSearchOptions {
            date_bound: Some(
                ccode_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(ccode_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&ccode_sampling_types::HostedTool::XSearch {
