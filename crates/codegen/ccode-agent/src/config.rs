//! Agent definition types — parsed from `.ccode/agents/*.md` files.
...
use ccode_tools::implementations::codex;
use ccode_tools::implementations::ccode_build;
use ccode_tools::implementations::ccode_build_concise;
use ccode_tools::implementations::memory;
use ccode_tools::implementations::opencode;
use ccode_tools::implementations::search_tool;
use ccode_tools::implementations::use_tool;
use ccode_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard CcodeBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&ccode_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&ccode_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&ccode_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&ccode_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&ccode_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_ccode_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_ccode_build_toolset() -> ToolServerConfig {
    let mut tools = default_ccode_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&ccode_build::EnterPlanModeTool).into());
    tools.push((&ccode_build::ExitPlanModeTool).into());
    tools.push((&ccode_build::AskUserQuestionTool).into());
    tools.push((&ccode_build::WebSearchTool).into());
    tools.push((&ccode_build::ImageGenTool).into());
    tools.push((&ccode_build::ImageToVideoTool).into());
    tools.push((&ccode_build::ReferenceToVideoTool).into());
    tools.push((&ccode_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&ccode_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `ccode-computer` (workspace/sandbox) preset.
fn ccode_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&ccode_build::ReadFileTool).into(),
        (&ccode_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&ccode_build::ListDirTool).into(),
        (&ccode_build::GrepTool).into(),
        (&ccode_build::KillTerminalCommandTool).into(),
        (&ccode_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("ccode-build", workspace_ccode_build_toolset()),
        ("ccode-build-concise", ccode_build_concise_toolset()),
        ("ccode-build-plan", ccode_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("ccode-computer", ccode_computer_toolset()),
    ]
}
...
fn default_ccode_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ccode_build::ReadFileTool).into(),
            ...
            (&ccode_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&ccode_build::SchedulerCreateTool).into(),
            (&ccode_build::SchedulerDeleteTool).into(),
            (&ccode_build::SchedulerListTool).into(),
            (&ccode_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&ccode_build::UpdateGoalTool).into(),
            (&ccode_build::WorkflowTool).into(),
        ],
        behavior_preset: None,
    }
}
fn ccode_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&ccode_build_concise::BashConciseTool).into(),
            (&ccode_build_concise::ReadFileConciseTool).into(),
            (&ccode_build_concise::SearchReplaceConciseTool).into(),
            (&ccode_build::ListDirTool).into(),
            (&ccode_build::GrepTool).into(),
            kill_task_tool_config(),
            (&ccode_build::TodoWriteTool).into(),
...
pub fn ccode_build_hashline_toolset(
    hashline_tools: Vec<ccode_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<ccode_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&ccode_build::WebSearchTool).into(),
        (&ccode_build::SchedulerCreateTool).into(),
        (&ccode_build::SchedulerDeleteTool).into(),
        (&ccode_build::SchedulerListTool).into(),
        (&ccode_build::MonitorTool).into(),
...
/// Ccode Build + plan mode toolset.
///
/// Extends the default `ccode-build` toolset with plan mode tools:
...
fn ccode_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard ccode-build tools
            bash_tool_config(),
            (&ccode_build::ReadFileTool).into(),
            (&ccode_build::SearchReplaceTool).into(),
            (&ccode_build::ListDirTool).into(),
            ...
            (&ccode_build::WorkflowTool).into(),
            // Plan mode tools
            (&ccode_build::EnterPlanModeTool).into(),
            (&ccode_build::ExitPlanModeTool).into(),
            (&ccode_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            // Web tools
            (&ccode_build::WebSearchTool).into(),
            (&ccode_build::WebFetchTool).into(),
            // Imagine
            (&ccode_build::ImageGenTool).into(),
            (&ccode_build::ImageToVideoTool).into(),
            (&ccode_build::ReferenceToVideoTool).into(),
            // Memory
...
/// Ccode Build + plan mode toolset WITHOUT subagent tools.
///
/// Same as `ccode_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn ccode_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard ccode-build tools (minus TaskTool only — KillTaskTool and
...
/// Default Ccode Build toolset + `ask_user_question`.
///
/// Same as `default_ccode_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn ccode_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ccode_build::ReadFileTool).into(),
...
/// - `model: ccode-3-fast` → `Override("ccode-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "ccode-build-orchestrator")]
    CcodeBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::CcodeBuild => AgentDefinition::default_ccode_build(),
            Self::CcodeBuildConcise => AgentDefinition::ccode_build_concise(),
            Self::CcodeBuildPlan => AgentDefinition::ccode_build_plan(),
            Self::CcodeBuildPlanNoSubagents => AgentDefinition::ccode_build_plan_no_subagents(),
            Self::CcodeBuildAskUser => AgentDefinition::ccode_build_ask_user(),
            ...
            Self::CcodeBuildOrchestrator => AgentDefinition::ccode_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .ccode/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_ccode_build_toolset")]
    ...
    pub capability_mode: Option<ccode_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<ccode_sampling_types::ToolOverrides>,
...
pub enum AgentScope {
    /// .ccode/agents/ (project-level, highest priority)
    Project,
    /// ~/.ccode/agents/ (user-level)
    User,
    /// ~/.ccode/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_ccode_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.ccode/agent-memory/<name>/`
    User,
    /// `<project>/.ccode/agent-memory/<name>/`
    Project,
    /// `<project>/.ccode/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: ccode_config::ccode_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".ccode/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".ccode/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `ccode_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), ccode.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".ccode/agents/") || path_str.contains(".ccode\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".ccode/bundled/agents/")
            || path_str.contains(".ccode\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `ccode-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<ccode_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["CcodeBuild:read_file", "CcodeBuildHashline:hashline_read"],
            [
                "CcodeBuild:search_replace",
                "CcodeBuildHashline:hashline_edit",
            ],
            ["CcodeBuild:grep", "CcodeBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_ccode_build_toolset(),
    ...
    pub fn default_ccode_build() -> Self {
        Self::base(
            BuiltinAgentName::CcodeBuild,
            "Ccode Build agent for software engineering tasks.",
        )
    }
    /// Ccode Build Concise agent definition — concise output format for SFT/RL.
    pub fn ccode_build_concise() -> Self {
        Self {
            tool_config: ccode_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::CcodeBuildConcise,
                "Ccode Build agent with concise output format.",
            )
        }
    }
    /// Ccode Build agent with plan mode tools.
    pub fn ccode_build_plan() -> Self {
        Self {
            tool_config: ccode_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::CcodeBuildPlan,
                "Ccode Build agent with plan mode support.",
            )
        }
    }
    /// Ccode Build + plan mode WITHOUT subagent tools.
    pub fn ccode_build_plan_no_subagents() -> Self {
        Self {
            tool_config: ccode_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::CcodeBuildPlanNoSubagents,
                "Ccode Build agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Ccode Build agent with the `ask_user_question` tool.
    pub fn ccode_build_ask_user() -> Self {
        Self {
            tool_config: ccode_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::CcodeBuildAskUser,
                "Ccode Build agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: ccode_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: ccode_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: ccode_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// Ccode Build Orchestrator — GBL model with full CcodeBuild tools
    ...
    pub fn ccode_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::CcodeBuildOrchestrator,
                "CcodeBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_ccode_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("ccode-build").unwrap();
    ...
    fn ccode_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&ccode_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&ccode_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn ccode_computer_preset_is_curated_ccode_build_subset() {
        let gc = toolset_for_preset("ccode-computer").unwrap();
        let gb = toolset_for_preset("ccode-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = ccode_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "ccode-computer tool `{}` must also ship in the ccode-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "ccode-computer should be a curated subset of ccode-build"
        );
    }
    #[test]
    fn ccode_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("ccode-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&ccode_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&ccode_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&ccode_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&ccode_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&ccode_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&ccode_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&ccode_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The ccode-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the ccode-build
    ...
    #[test]
    fn ccode_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("ccode-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "ccode-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn ccode_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("ccode-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&ccode_build::LspTool).id,
            ToolConfig::from(&ccode_build::EnterPlanModeTool).id,
            ToolConfig::from(&ccode_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "ccode-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_ccode_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_ccode_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::CcodeBuildOrchestrator => true,
            BuiltinAgentName::CcodeBuild
            | BuiltinAgentName::CcodeBuildConcise
            | BuiltinAgentName::CcodeBuildPlan
            | BuiltinAgentName::CcodeBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "ccode-build-orchestrator"] {
            assert!(
        ...
        for non_strict in [
            ...
            "ccode-build-plan",
            "ccode-build-concise",
            "ccode-build-ask-user",
            "opencode",
            "browser-use",
            "custom-user-agent",
            "",
            "ccode-build-totally-made-up",
        ] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("ccode-3-fast".to_string()).to_string(),
            "ccode-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("ccode-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.ccode/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.ccode/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_ccode_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have ccode_build tools"
        );
    }
    #[test]
    ...
    fn test_from_json_has_default_toolset_with_task_tool() {
        ...
        let def = AgentDefinition::from_json(&json).unwrap();
        let task_tool_id = "CcodeBuild:task";
        assert!(
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"ccode-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("ccode-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("ccode-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"ccode-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        ...
        assert_eq!(
            def.model,
            ModelOverride::Override("ccode-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!({
            "name": "test",
            "description": "Test",
            "model": "ccode-code-fast-1"
        });
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("ccode-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("ccode-build", BuiltinAgentName::CcodeBuild),
            ("ccode-build-concise", BuiltinAgentName::CcodeBuildConcise),
            ("ccode-build-ask-user", BuiltinAgentName::CcodeBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_ccode_build();
