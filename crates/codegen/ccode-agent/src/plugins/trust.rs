...
//!
//! Plugins from project directories (`.ccode/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.ccode/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.ccode/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.ccode/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_ccode_home() so a project's `.ccode/trusted-plugins` is never
        // read as the user trust store when neither CCODE_HOME nor a home dir resolves.
        let Some(ccode) = ccode_config::user_ccode_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = ccode.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.ccode/trusted-plugins`.
    ...
    /// rewrites `~/.ccode/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.ccode/trusted-plugins`.
