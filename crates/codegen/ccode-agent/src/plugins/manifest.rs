...
///
/// Replaces `${CCODE_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${CCODE_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`ccode_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    ccode_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".ccode-plugin")).unwrap();

        // Write manifest in .ccode-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".ccode-plugin/plugin.json"),
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${CCODE_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${CCODE_PLUGIN_DATA}/cache";
