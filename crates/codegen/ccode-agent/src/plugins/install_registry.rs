...
/// Default install directory name under `~/.ccode/`.
...
/// Lives here (not in ccode-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MarketplaceProvenance {
    ...
    /// Plugin subdirectory within marketplace (e.g., "plugins/ccode-code-review").
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("CCODE_CCODE_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    /// 2. Default: `~/.ccode/installed-plugins/`
    pub fn resolve_install_dir() -> PathBuf {
        if let Some(dir) = Self::read_install_dir_from_config() {
            return dir;
        }

        ccode_config::ccode_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
...
pub enum InstallError {
    ...
    #[error(
        "refusing unpinned remote plugin code for '{plugin}' from {url}: \
         marketplace.require_sha / CCODE_MARKETPLACE_REQUIRE_SHA is enabled and \
