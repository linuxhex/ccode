...
//!
//! Searches from cwd to repo root, plus `~/.ccode/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.ccode/rules/`,
...
//! (`~/.ccode/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.ccode/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.ccode/, git repo root, and session cwd.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, ccode_home, true, HOME_RULES_DIRS);
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                ccode_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `ccode-shell` to structurally detect legacy untagged AGENTS.md
...
/// Open/close `system-reminder` (Ccode) or `system_reminder` (Cursor/IDE), case-insensitive.
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        ...
        for path in [
            ccode_home.join(".ccode/rules/doubled.md"),
    ...
    async fn vendor_home_agents_and_rules_cells_are_independent() {
        let tmp = tempfile::tempdir().unwrap();
        let ccode_home = tmp.path().join("ccode-home");
        let home = tmp.path().join("home");
        let cwd = tmp.path().join("project");
        fs::create_dir_all(&ccode_home).unwrap();
    ...
    #[tokio::test]
    async fn nested_ccode_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".ccode/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".ccode/rules/project.md"), "nested-project-rule").unwrap();
    ...
    #[tokio::test]
    async fn overlapping_ccode_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".ccode/rules/project.md"), "project-ccode-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".ccode/.ccode/rules")).unwrap();
        fs::write(repo.join(".ccode/.ccode/rules/doubled.md"), "doubled").unwrap();
        ...
        .await;
        for expected in ["home-rule", "project-ccode-rule", "project-claude-rule"] {
            assert_eq!(
                configs
                    .iter()
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (ccode_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".ccode/rules/project.md"), "ccode-project-body"),
