...
//! - `Default`: the legacy Ccode Build prefix (built by the shell layer).
...
use ccode_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Ccode Build prefix (`<user_info>` + optional `<git_status>`), built directly by the
...
/// One discovered rule file (AGENTS.md / Claude.md / .ccode/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `ccode_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.ccode/, ~/.claude/).
    ...
    /// (`~/.ccode/projects/<encoded-cwd>/mcps`). Surfaced in
