...
//! primitive consumed cross-crate by `ccode-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.ccode/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.ccode`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `ccode-workspace`, which depends on THIS crate) to keep the dep edge
