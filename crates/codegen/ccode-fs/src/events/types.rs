...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `ccode-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `ccode-shell::session::events`.
    ...
    /// discriminator constants in `ccode-shell::session::events`.
    ...
    /// `ccode-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `ccode-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
pub enum InterjectionSource {
    /// Direct `ccode.dev/interject` while a turn was running (Ctrl+Enter).
    Direct,
...
pub enum RedirectKind {
    /// Mid-turn interjection — Ctrl+O / `ccode.dev/interject`, or "Send now" on a
...
/// Telemetry mirror of `ccode-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `ccode-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `ccode-shell`'s `GoalPauseReason`. The two types
