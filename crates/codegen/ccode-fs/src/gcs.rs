...
use ccode_auth::{AuthCredentialProvider, StaticAuthCredentialProvider};

use crate::storage_client::{Auth401AttributionCallback, StaticCcodeAuth, StorageClient};
...
/// `StaticCcodeAuth` carrying the inline user / deployment keys from
...
fn build_proxy_client_with_fallback(
    ...
    let provider = credentials.unwrap_or_else(|| {
        let mut creds = StaticCcodeAuth::new(Some(user_token.to_owned()));
