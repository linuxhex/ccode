...
use ccode_circuit_breaker::{Disposition, RetryPolicy};
...
impl ResolvedStorageConfig {
    ...
    /// mirrors `CcodeAuthCredentials::apply` precedence (deployment key wins).
...
/// Resolve a `CCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS` override (seconds) into a probe
...
/// `session_id`, no GCS path). Read by `ccode_workspace::recovery`.
...
struct UploadQueueItem {
    ...
    /// Ccode client version string, stamped on the `gcs_queue_upload` tracing span.
...
pub struct UploadQueue {
    ...
    max_queue_bytes: u64,
    /// Ccode client version string stamped on every `gcs_queue_upload` tracing span.
...
impl UploadQueue {
    ...
    pub fn spawn(
        ...
        Self::spawn_with_concurrency(ccode_home, resolver, retry_policy, DEFAULT_MAX_CONCURRENT)
    }
    ...
    pub fn spawn_with_concurrency(
        ...
        let queue_dir = ccode_home.join("upload_queue");
        if let Err(e) = std::fs::create_dir_all(&queue_dir) {
            tracing::warn!(error = %e, "Failed to create upload queue dir");
        }
        if let Some(raw_secs) = std::env::var("CCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS")
            .ok()
            .and_then(|s| s.parse::<u64>().ok())
        {
            match auth_park_probe_override(raw_secs) {
                ...
                None => {
                    tracing::warn!(
                        "Ignoring CCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS={raw_secs}: a zero probe \
        ...
        let max_queue_bytes = std::env::var("CCODE_UPLOAD_QUEUE_MAX_BYTES")
            .ok()
    ...
    /// Set the ccode client version to stamp on every `gcs_queue_upload` span.
...
/// of entries removed.
pub fn cleanup_orphaned_uploads(ccode_home: &Path, max_age: Duration) -> u64 {
    let cleaned = cleanup_queue_dir(&ccode_home.join("upload_queue"), max_age, None);
