...
use ccode_announcements::RemoteAnnouncement;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-ccode-doom-loop-check` header and parse the reported
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `ccode_build_settings`
    ...
    /// OAuth2 provider issuer URL (e.g., "https://auth.ccode.dev"). When present
    ...
    /// default (on). Sits below env `CCODE_FOLDER_TRUST`, user
    ...
    /// remote settings `ccode_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `ccode_build_settings` remote settings key) to enable it. See
    ...
    /// `ccode_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `CCODE_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `ccode_build_settings`).
    ...
    /// remote settings gate for first-run auto-registration of the official ccode
    ...
    /// ghost text), from the `ccode_build_settings` remote settings flag. Sits below
    /// env (`CCODE_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `ccode-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `ccode-imagine-image`) instead of the default quality model
    /// (`ccode-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `CCODE_VOICE_MODE`. Free-tier Premium upsell
    ...
    /// remain coding-data opted-out. From `ccode_build_settings`.
    ...
    /// User's subscription tier from remote settings `ccode_build_access_gate`.
    /// E.g. "free", "premium", "premium", "premium_heavy".
    ...
    /// Whether the user is allowed to use Ccode Build. Set by remote settings
    /// `ccode_build_access_gate` targeting rules. `None` = no server response
    ...
    /// (e.g. "Premium", "X Premium+", "Free", "API Key"). Set by CCP
    ...
    /// Server-controlled via the remote settings `ccode_build_usage_redirect_url`
    ...
    /// `ccode_build_settings`. Per-model override on `ModelInfo`
    /// (`ccode_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`CCODE_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`CCODE_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`CCODE_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`CCODE_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `ccode workspace` CLI command (Computer Hub
    /// workspace exposure), from `ccode_build_settings.workspace_command_enabled`.
...
pub struct ContextualHintsRemote {
    ...
    /// SSH wrap session-load tip (recommend `ccode wrap ssh` for remote sessions).
    #[serde(default)]
...
/// only guaranteed to work with a compatible harness (cursor vs ccode-build).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoalRoleModel {
    /// Model id, e.g. "ccode-4". Resolved against available models at
    /// spawn time; unknown/unauthorized ⇒ fail-open to current model.
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "ccode-build-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-ccode-build toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "ccode-build"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("ccode-build"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "ccode-build-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("ccode-build-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_goal_planner_model_round_trip() {
        let json =
            r#"{"goal_planner_model": {"model": "ccode-4", "agent_type": "general-purpose"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_strategist_model_round_trip() {
        let json = r#"{"goal_strategist_model": {"model": "ccode-4.5", "agent_type": "cursor"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_fully_valid_pool_round_trips() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "ccode-4", "agent_type": "general-purpose"},
            {"model": "ccode-3", "agent_type": "cursor"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_extra_unknown_fields_kept() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "ccode-4", "agent_type": "general-purpose", "reasoning_effort": "high"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_skeptic_models,
    ...
    fn remote_settings_goal_role_models_malformed_pair_does_not_drop_other_fields() {
        let json = r#"{
            ...
            "goal_strategist_model": {"model": "ccode-4.5", "agent_type": "cursor"},
            "default_model": "ccode-4"
        }"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.goal_planner_model, None);
        assert_eq!(
        ...
        assert_eq!(s.default_model.as_deref(), Some("ccode-4"));
    }
    #[test]
    fn remote_settings_goal_role_model_extra_unknown_fields_kept_single_pair() {
        let json = r#"{"goal_planner_model": {"model": "ccode-4", "agent_type": "general-purpose", "future": true}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_planner_model,
            Some(GoalRoleModel {
