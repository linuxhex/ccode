//! Memory-system configuration value types, extracted from ccode-shell
...
//! `ccode-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.ccode/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.ccode/memory/` are scanned:
