//! Shared announcement types, persistence, and formatting for Ccode CLI apps.
//!
//! This crate provides the common logic used by `ccode-shell` and
//! `ccode-pager` for handling announcements (banner notifications).
...
/// Payload for `ccode.dev/announcements/update` ACP notification.
...
/// Read hidden announcement ids from `~/.ccode/announcements.json`.
...
/// Write hidden announcement ids to `~/.ccode/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    ccode_tools::util::ccode_home::ccode_home().join("announcements.json")
}
...
///
/// Precedence: `CCODE_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("CCODE_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid CCODE_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("CCODE_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("CCODE_ANNOUNCEMENTS_OVERRIDE");
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Get Premium","url":"https://ccode.dev/ccode","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Get Premium"));
        assert_eq!(cta.url.as_deref(), Some("https://ccode.dev/ccode"));
