...
# are the single source of truth for the `ccode.dev/announcements/update` payload.
# Pipeline:
...
DESKTOP_DIR="$REPO_ROOT/frontend/apps/ccode-desktop"
...
for f in "${bindings[@]}"; do
  {
    ...
    echo "// Regenerate via generate.sh in the ccode-announcements package."
    cat "$f"
  } >"$DST/$(basename "$f")"
done
