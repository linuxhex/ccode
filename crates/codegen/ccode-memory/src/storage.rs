...
//! `~/.ccode/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use ccode_tools::util::ccode_home::ccode_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `ccode_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.ccode/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.ccode/memory/ccode-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.ccode/memory/`
    global_dir: PathBuf,
    /// `~/.ccode/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.ccode/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| ccode_home().join("memory"));
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        // mounts (SIGBUS); see ccode_journal::JournalMode::open_readonly.
        ccode_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.ccode/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
...
mod tests {
    ...
    fn test_compute_workspace_hash_human_readable() {
        let name = compute_workspace_hash(Path::new("/users/me/work/ccode"));
        assert!(
            name.starts_with("ccode-"),
