...
- **Using Ccode**
  ...
  - [SSH Passthrough](#ssh-passthrough-ccode-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`ccode inspect`)](#introspection)
...
- [Building with Ccode](#building-with-ccode) — headless API, ACP SDK integration
...
curl -fsSL https://ccode.dev/cli/install.sh | bash -s 0.1.42
```
...
On first launch, Ccode opens your browser to authenticate with ccode.dev:

```bash
ccode
```

Credentials are stored in `~/.ccode/auth.json` and persist across sessions. Tokens expire after 7 days; Ccode will prompt you to re-authenticate when needed.
...
For CI/CD, automation, or environments without browser access, use an API key from [console.ccode.dev](https://console.ccode.dev):
...
Authenticate developers via your own Identity Provider (Okta, Azure AD, Auth0) instead of `accounts.ccode.dev`.
...
export CCODE_OIDC_ISSUER="https://acme.okta.com"
export CCODE_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export CCODE_CLI_CHAT_PROXY_BASE_URL="https://ccode-proxy.acme.com/v1"
```

**3. Run `ccode`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.ccode/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Ccode is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     Ccode     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. Ccode runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by Ccode and saved to `~/.ccode/auth.json` as the access token
5. exit 0 → success; exit non-zero → Ccode falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | Ccode (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Ccode reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Ccode to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Ccode when to proactively refresh. Without it, Ccode assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Ccode captures this)
...
```bash
# Or via environment variables
export CCODE_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export CCODE_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export CCODE_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Ccode cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with ccode.dev". In headless mode (`ccode -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=ccode-cli")
...
When Ccode needs to refresh an expired token, it re-runs your binary with `CCODE_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$CCODE_AUTH_EXPIRED" = "1" ]; then
...
`CCODE_AUTH_EXPIRED` is optional — if your binary ignores it, Ccode still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

Ccode supports automatic credential refresh for external auth providers and OIDC. When Ccode detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Ccode handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Ccode re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Ccode re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Ccode silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Ccode refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export CCODE_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `ccode login` before starting — Ccode runs your binary automatically on first launch. You _can_ run `ccode login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Ccode tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
ccode --debug-file /tmp/ccode-auth.log -p "hello"
tail -f /tmp/ccode-auth.log
```
...
`auth_provider_command` above replaces Ccode's *session* auth: it mints the token sent to ccode's backend. If you instead want ccode models on normal ccode login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- `CCODE_AUTH_EXPIRED=1` is set whenever Ccode re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `CCODE_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `CCODE_AUTH_PROVIDER_REFRESH_TOKEN` / `CCODE_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and Ccode re-runs the command on restart to re-mint.
- Ccode runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Interaction with other credentials:** a literal `api_key`/`env_key` on the model wins over its `auth_provider`. Provider-backed models are BYOK: your ccode session token is never sent to their endpoints, and a failing provider command fails the request rather than falling back to the session token.

**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.ccode/config.toml`, managed config, requirements). A project's `.ccode/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits Ccode's environment (so it sees `PATH`, `HOME`, and any other secrets there), but Ccode's own first-party credentials (`CCODE_API_KEY`, `CCODE_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `CCODE_AUTH_PROVIDER_*` handback for the prior credential.

### Using auth.json for API Access

If you've authenticated with `ccode login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the ccode CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.ccode.dev/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.ccode.dev/sign-in".key' ~/.ccode/auth.json)" \
  -H "X-Ccode-Token-Auth: ccode-cli" \
  -H "x-ccode-model-override: ccode-build" \
  -d '{
    "model": "ccode-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.ccode/auth.json` (set by `ccode login`)                                                                                                                              |
| `X-Ccode-Token-Auth: ccode-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-ccode-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `ccode-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `ccode-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `ccode login` to refresh.
...
ccode --cwd ~/projects/my-app

# Start with an initial task
ccode --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
ccode --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
ccode --always-approve --prompt "Format all files"
```
...
Run Ccode non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `ccode-build`)               |
...
ccode -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
ccode -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
ccode -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
ccode -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
ccode -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
ccode -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
ccode -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
ccode -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
ccode -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
ccode --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
ccode -p "What does this project do?"

# Use a specific model
ccode -p "Optimize this function" -m ccode-build

# Get JSON output for parsing
ccode -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
ccode -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
ccode -p "Remember: the secret number is 42" --output-format json
ccode -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
ccode -p "Continue where we left off" -c

# Run in a different directory
ccode -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
ccode -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
ccode -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
ccode -p "Generate a README" > README.md

# Parse JSON output with jq
ccode -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
ccode -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | ccode -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  ccode -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
ccode -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Ccode as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `ccode-build`)                           |
...
```bash
ccode agent headless --ccode-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`ccode ssh`)

Use `ccode ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
ccode ssh -t user@host
ccode ssh -L 8080:localhost:8080 user@host

# With remote command
ccode ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `ccode ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Ccode can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `ccode -p` and parses JSON output.
...
class CcodeChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["ccode", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="ccode-build", stream=False):
...
class CcodeChat {
  ...
  async create(
    ...
    { model = "ccode-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("ccode", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await CcodeACPChat(cwd=".").init()
...
class CcodeACPChat {
  ...
  async init() {
    this.proc = spawn("ccode", ["agent", "stdio"]);
...
// Usage
const client = await new CcodeACPChat(".").init();
...
Ccode implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Ccode reads configuration from `~/.ccode/config.toml`. If the file doesn't exist, Ccode uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "ccode-build"           # model used for new sessions
web_search = "ccode-4.20-multi-agent"   # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: CCODE_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: CCODE_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: CCODE_RESPECT_GITIGNORE)
...
allowed_domains = ["docs.rs", "ccode.dev"]           # override the built-in ~84-domain allowlist
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: CCODE_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: CCODE_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: CCODE_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: CCODE_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: CCODE_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `CCODE_TELEMETRY_BUILD_EVENTS_URL`, `CCODE_TELEMETRY_BUILD_EVENTS_API_KEY`, and `CCODE_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Ccode can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Ccode language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Ccode can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Ccode can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Ccode looks for server definitions in:

- project config: `<repo>/.ccode/lsp.json`
- user config: `~/.ccode/lsp.json`
...
- LSP tools are enabled (`CCODE_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
CCODE_LSP_TOOLS=1 ccode
```
...
If LSP tools are enabled but no usable server config is found, Ccode emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Ccode does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `CCODE_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[models]
default = "company-ccode"

[model.company-ccode]
model = "ccode-build"
base_url = "https://ccode-proxy.acme.com/"
name = "Ccode Build Latest (Proxy)"
...
With this config, `ccode` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Ccode reads these files and appends their contents to the system prompt.

Ccode scans for agent rules in this order:

1. `~/.ccode/` (global rules)
...
Within each directory, Ccode checks for these filenames:
...
Skills are reusable prompt packages that extend Ccode with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Ccode discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.ccode/skills/`           | Local | Highest  |
| `<repo_root>/.ccode/skills/` | Repo  | Medium   |
| `~/.ccode/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.ccode/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Ccode decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Ccode invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Ccode automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Ccode discovers agent definitions from `.ccode/agents/` (project), `~/.ccode/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `CCODE_AGENT` env var
4. Default `ccode-build` agent
...
```bash
ccode --agent-profile ./my-agent.md
# or
export CCODE_AGENT="my-custom-agent"
```
...
```bash
export CCODE_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "ccode-build"              # route explore to a lighter model
```
...
model = "ccode-build"
prompt_file = ".ccode/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".ccode/personas/concise.md"  # or load from file
```

Both are also discovered from `.ccode/roles/*.toml` and `.ccode/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Ccode with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.ccode/plugins/`            | Project |
| `~/.ccode/plugins/`          | User    |
...
Ccode discovers hooks from `.ccode/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → cached `auth_provider` token (terminal: a cache miss resolves to no credential, never the session token) → session token → `CCODE_API_KEY`. See [Per-Model Auth Providers](#per-model-auth-providers).

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Ccode falls back to built-in defaults for known models.
...
[model.ccode-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, Ccode starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `CCODE_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `ccode models` output). You also need a `[model.*]` entry so Ccode knows
...
>
> [model.my-custom-model]              # 2. tell Ccode how to reach it
...
Point Ccode at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `CCODE_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `CCODE_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `CCODE_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export CCODE_MODELS_BASE_URL="https://api.acme.com/v1"
export CCODE_API_KEY="ccode-..."
ccode
```

Ccode fetches the model list from `{CCODE_MODELS_BASE_URL}/models` on startup and sends inference requests to `CCODE_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `CCODE_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.ccode/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Ccode uses API key auth (`Authorization: Bearer`) instead of session auth. `ccode login` is not required — only the API key.

---

## MCP Servers

Extend Ccode's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.ccode/config.toml`:
...
MCP servers can also be configured per-project in `.ccode/config.toml`. Ccode walks from the current directory up to the git repo root, loading `.ccode/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.ccode/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.ccode/config.toml` | This repository   | ↑        |
| `<cwd>/.ccode/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.ccode/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.ccode/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.ccode/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.ccode/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `CCODE_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Ccode remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.ccode/memory/`:
- **Global** (`~/.ccode/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.ccode/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.ccode/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries

Workspace directories are suffixed with a short hash for uniqueness (e.g. `ccode-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
At the end of each session, Ccode saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.ccode/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Ccode searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.ccode/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.ccode/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Ccode emits the `ccode-shell-memory_injection`
...
Ccode can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.ccode/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.ccode/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.ccode/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.ccode/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.ccode/sandbox.toml` (global) or `.ccode/sandbox.toml` (per-project):
...
```bash
ccode --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire ccode process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Ccode logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.ccode/sandbox-events.jsonl`
...
Use `ccode inspect` to see everything Ccode discovers in the current directory:

```bash
ccode inspect          # human-readable output
ccode inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.ccode/skills/`, `~/.ccode/skills/`, plugins, and config paths
...
Ccode automatically discovers configuration from Claude Code directories alongside native `.ccode/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Ccode uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.ccode/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Ccode at runtime.

---

## Built-in Tools

Ccode includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `CCODE_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (SpaceCcode, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `ccode.dev/company`).

---

## Session Persistence

Ccode automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.ccode/sessions/`, organized by URL-encoded working directory:

```
~/.ccode/sessions/<encoded-cwd>/<session-id>/
...
ccode -p "Remember: X=42" -s my-session
ccode -p "What is X?" -s my-session

# Resume existing session (errors if not found)
ccode -p "Continue" -r my-session

# Continue most recent session in current directory
ccode -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
ccode -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.ccode/config.toml` | Configuration file                                  |
| `~/.ccode/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.ccode/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.ccode/memory/`     | Cross-session memory files and index                |
| `~/.ccode/skills/`     | User-scoped skill definitions                       |
| `~/.ccode/plugins/`    | User-scoped plugins                                 |
| `~/.ccode/agents/`     | User-scoped agent definitions                       |
| `.ccode/config.toml`   | Project-scoped config (MCP servers)                 |
| `.ccode/skills/`       | Project-scoped skill definitions                    |
| `.ccode/plugins/`      | Project-scoped plugins                              |
| `.ccode/agents/`       | Project-scoped agent definitions                    |
| `.ccode/hooks/`        | Project-scoped hooks                                |
| `.ccode/lsp.json`      | LSP server configuration                            |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `CCODE_API_KEY`         | API key from [console.ccode.dev](https://console.ccode.dev). Used for custom endpoint auth and API key login      |
| `CCODE_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.ccode.dev/v1`)                          |
| `CCODE_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `CCODE_MODELS_LIST_URL`          | Override the model list URL if it differs from `{CCODE_MODELS_BASE_URL}/models`                                              |
| `CCODE_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `CCODE_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `CCODE_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `CCODE_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `CCODE_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `CCODE_HOME`                     | Override config directory (default: `~/.ccode`)                                                           |
| `CCODE_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `CCODE_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `CCODE_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `CCODE_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `CCODE_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `CCODE_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `CCODE_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `CCODE_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `CCODE_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `CCODE_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.ccode/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `CCODE_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `ccode` commands and flags.
...
ccode completions bash > ~/.local/share/bash-completion/completions/ccode
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Ccode-managed location):

```bash
mkdir -p ~/.ccode/completions/bash
ccode completions bash > ~/.ccode/completions/bash/ccode.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.ccode/completions/bash/ccode.bash" ]] && source "$HOME/.ccode/completions/bash/ccode.bash"
```
...
ccode completions zsh > ~/.zsh/completions/_ccode
```
...
```bash
mkdir -p ~/.ccode/completions/zsh
ccode completions zsh > ~/.ccode/completions/zsh/_ccode
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.ccode/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `ccode` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `ccode --debug` or `CCODE_LOG_FILE` instead:

```bash
# Per-session debug log (~/.ccode/debug/<sessionId>.txt)
ccode --debug

# Log to a custom path
CCODE_LOG_FILE=/tmp/ccode-debug.log ccode

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.ccode/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `CCODE_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
CCODE_LOG_FILE=/tmp/ccode-debug.log RUST_LOG="info,ccode_shell::auth=debug" ccode
```
...
cat ~/.ccode/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.ccode/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.ccode/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
