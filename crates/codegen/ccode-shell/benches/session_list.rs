...
//!
//! Run: `cargo bench -p ccode-shell --bench session_list`
...
use ccode_worktree::{ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
use ccode_shell::session::info::Info;
use ccode_shell::session::persistence::Summary;
use ccode_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use ccode_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = ccode_shell::util::ccode_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = ccode_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/ccode-org/session-list-benchmark.git",
    )
    ...
    let worktree_base = ccode_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(ccode_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:ccode-org/benchmark.git".to_owned()],
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("CCODE_HOME", home.path());
    }
    assert_eq!(ccode_shell::util::ccode_home::ccode_home(), home.path());
