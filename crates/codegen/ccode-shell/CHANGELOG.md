...
- You can now run `ccode doctor fix` commands directly from inside the TUI instead of only from the CLI.

## Bug Fixes

- **`!cmd` commands** now allow up to one hour before timing out.
- **npm package** now installs the native binary under `$CCODE_HOME/bin` (honoring the same override as the Rust CLI).
...
- Ccode now automatically stops a turn that keeps repeating the exact same tool call many times in a row.
...
- When the auto-permission classifier times out or fails, Ccode now shows a normal permission prompt instead of silently denying.
...
- **ccode doctor fix ssh-wrap** can set up `ccode wrap ssh` automatically for Bash, zsh, and fish.
...
- **ccode doctor** checks terminal, tmux, clipboard, and keyboard setup without opening the TUI.

## Bug Fixes

- **Image paste** over ccode wrap now works on headless remotes.
...
- **Added CCODE_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `ccode --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Default model** is now Ccode 4.5 with high/medium/low reasoning effort and improved compaction settings.
...
- **Global rules** from ~/.ccode/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `ccode wrap ssh <host>` for clipboard and terminal restore.
...
- **ccode** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **ccode wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `ccode login`.
...
- **ccode login** now requests Ccode Projects scopes so workspace listing works after consent.
...
- **`ccode plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using ccode -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **ccode inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct ccode --minimal --resume command.
...
- **Ccode no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **ccode setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask Ccode above the footer shortcuts.
...
- **ccode.dev/pr/status** now reports whether an open PR is in the merge queue.
...
- **`ccode worktree ls`** now works as a short alias for `ccode worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.ccode/config.toml`.
...
- **Resumed ccode.dev chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.ccode/worktrees/.
...
- **Terminal command cards** on ccode.dev now correctly settle after foreground bash tasks.
...
- **ccode inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain ccode launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive ccode agent stdio without silent parse drops on session/* calls.
...
- **ccode --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by ccode.dev" link in the Extensions modal is now clickable and underlined.
...
- **ccode --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **ccode update** on Windows no longer fails when a previous .old executable is still running.
...
- **ccode models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `ccode plugin marketplace add`.
...
- **Plan files** now default to .ccode/plan.md to match Ccode conventions.
...
- **--minimal** flag now shows in `ccode --help`.
...
- **Tool result previews** no longer paint opaque panels in `ccode --minimal`.
- **ccode wrap** now correctly handles quoted strings and shell aliases.
...
- **Ccode.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **Chat sessions show the ccode.dev model catalog** in the picker.
...
- **`ccode update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `ccode wrap`.
...
- **ccode agent stdio** now checks for updates in the background like other modes.
...
- **ccode wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Added `ccode wrap`** to run any command with local clipboard support.
...
- **Fixed `ccode agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or ccode-desktop.
...
- You can now pass --json-schema to ccode -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `CCODE_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **OIDC sessions** with CCODE_API_KEY present no longer lose refresh on idle.
...
- **ccode -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in ccode -p and ccode agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `ccode` after running `ccode update`.
- Headless (`ccode -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`ccode plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **ccode sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **ccode quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, ccode sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, CCODE_HUNK_TRACKER, or config.
...
- **Ccode** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`ccode dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`ccode mcp add`** now accepts positional arguments (e.g. `ccode mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`ccode update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.ccode.
...
- **MCP servers menu** and slash commands now work when starting ccode outside a project directory.
...
- **ccode --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `ccode --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `ccode update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`ccode --debug`** now produces per-session log files under ~/.ccode/debug/ even with a leader process.
...
- **`ccode update`** now restarts any older running leader so all clients get the new binary.
...
- **`ccode login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `ccode agent --leader` without forced interactive login or timeouts.
...
- **Official ccode plugin marketplace** now appears automatically in the Marketplace tab on first launch.
- **Image and video generation** now use api.ccode.dev directly for all users.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the CCODE_IMAGE_GEN_HARNESS env var.
...
- **ccode inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via CCODE_CURSOR_MCPS_ENABLED / CCODE_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside Ccode and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the ccode-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality ccode-imagine-image-quality model.
...
- Memory system: /remember command, note modal with raw/enhanced preview, ccode.dev/memory/rewrite ACP extension, Ctrl+F fullscreen toggle for /memory modal.
