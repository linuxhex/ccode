...
use ccode_shell::sampling::ConversationItem;
...
async fn create_test_session(storage: &JsonlStorageAdapter, session_id: &str, cwd: &str) -> Info {
    ...
    let model_id = acp::ModelId::new("ccode-code-fast-1");
    ...
    storage
        .append_update(
            &info,
            &ccode_shell::session::storage::SessionUpdate::Acp(Box::new(notification)),
        )
        .await
        .unwrap();
...
async fn test_fork_session_creates_new_session_with_parent_tracking() {
    ...
    let options = ccode_shell::session::storage::CopySessionOptions {
    ...
    assert_eq!(loaded.summary.current_model_id, acp::ModelId::new("ccode-3"));
    assert_eq!(
    ...
    match &loaded.updates[0] {
        ccode_shell::session::storage::SessionUpdate::Acp(notification) => {
