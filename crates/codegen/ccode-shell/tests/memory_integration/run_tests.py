...
Launches ccode agent stdio in isolated environments ($HOME override)
...
Usage:
    ...
    python3 crates/codegen/ccode-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/ccode-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/ccode-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    CCODE_BINARY=/path/to/ccode-pager python3 run_tests.py --fast
"""
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a ccode agent stdio subprocess."""
...
class IsolatedEnv:
    """Creates a fully isolated ccode environment with custom $HOME.

    The agent subprocess gets a fake $HOME with:
      ~/.ccode/auth.json   (copied from real home)
      ~/.ccode/memory/     (pre-populated by tests)
      ~/.ccode/logs/       (memory.log appears here)

    And a workspace directory used as cwd when spawning the agent.
    """

    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="ccode-memtest-")
        ...
        # Create .ccode dirs in fake home
        self.ccode_home = os.path.join(self.fake_home, ".ccode")
        self.memory_dir = os.path.join(self.ccode_home, "memory")
        self.logs_dir = os.path.join(self.ccode_home, "logs")
        os.makedirs(self.memory_dir)
        os.makedirs(self.logs_dir)

        # Copy auth from real home
        real_auth = os.path.expanduser("~/.ccode/auth.json")
        if os.path.isfile(real_auth):
            shutil.copy2(real_auth, os.path.join(self.ccode_home, "auth.json"))

    def write_config(self, toml_str):
        """Write global ~/.ccode/config.toml (where the agent reads config)."""
        with open(os.path.join(self.ccode_home, "config.toml"), "w") as f:
            f.write(toml_str)
    ...
    def spawn_agent(self, extra_env=None):
        binary = os.environ.get("CCODE_BINARY", "ccode")
        if not shutil.which(binary) and not os.path.isfile(binary):
            print(f"{R}ccode binary not found: {binary}{N}")
            sys.exit(1)
        env = os.environ.copy()
...
def _default_config() -> str:
    embed_model = os.environ.get("CCODE_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"CCODE_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """CCODE_MEMORY=1 env var enables memory even without config."""
    section("13. CCODE_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"CCODE_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with CCODE_MEMORY=1")
        else:
            fail("no index despite CCODE_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.ccode/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("CCODE_BINARY", "ccode")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["CCODE_MEMORY"] = "1"
...
def build_binary():
    """Build ccode-pager with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "ccode-pager")

    # Find rg for CCODE_SHELL_BUNDLE_RG_PATH
    rg = shutil.which("rg")
    if not rg:
    ...
    env = os.environ.copy()
    env["CCODE_SHELL_BUNDLE_RG_PATH"] = rg

    print(f"{B}Building ccode-pager (release + dev)...{N}")
    result = subprocess.run(
        ["cargo", "build", "-p", "ccode-pager", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
...
def main():
    ...
    if "CCODE_BINARY" in os.environ:
        binary = os.environ["CCODE_BINARY"]
        print(f"Binary: {binary} (from $CCODE_BINARY, skipping build)")
    else:
        binary = build_binary()
        os.environ["CCODE_BINARY"] = binary
