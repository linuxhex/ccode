...
//! `CCODE_PERF_SESSION_SRC=/path/to/<session-dir>`.
//!
//! Run:
//!   cargo test -p ccode-shell --test session_load_perf -- --nocapture
//!   cargo test -p ccode-shell --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use ccode_shell::session::info::Info;
use ccode_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use ccode_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
impl GenOpts {
    fn from_env() -> Self {
        ...
        let scale = g("CCODE_PERF_SCALE", 1).max(1);
        Self {
            turns: g("CCODE_PERF_TURNS", 80) * scale,
            acu_per_turn: g("CCODE_PERF_ACU_PER_TURN", 15),
            catalog_commands: g("CCODE_PERF_CATALOG_COMMANDS", 64),
            catalog_desc_len: g("CCODE_PERF_CATALOG_DESC_LEN", 320),
            agent_chunks_per_turn: g("CCODE_PERF_AGENT_CHUNKS_PER_TURN", 8),
            agent_chunk_len: g("CCODE_PERF_AGENT_CHUNK_LEN", 2000),
            rewind_points: g("CCODE_PERF_REWIND_POINTS", 60) * scale,
            files_per_rewind: g("CCODE_PERF_FILES_PER_REWIND", 40),
            file_content_len: g("CCODE_PERF_FILE_CONTENT_LEN", 8000),
        }
    }
}
...
/// `Info` and the session directory path. Uses `CCODE_PERF_SESSION_SRC` if set
...
async fn prepare_session(root: &Path, cwd: &Path, opts: &GenOpts) -> (Info, PathBuf) {
    ...
    if let Ok(src) = std::env::var("CCODE_PERF_SESSION_SRC") {
...
/// `--ignored` (optionally `CCODE_PERF_SESSION_SRC=...`) to get the numbers.
...
fn generate_or_restore_rewind(path: &Path, opts: &GenOpts) {
    if std::env::var("CCODE_PERF_SESSION_SRC").is_ok() {
        return;
    }
...
use ccode_shell::agent::mvp_agent::MvpAgent;
...
async fn full_session_load_e2e() {
    ...
    let server = ccode_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    let cwd = TempDir::new().unwrap();
    let opts = GenOpts::from_env();
    let instr_log = ccode_home.path().join("instr.jsonl");

    // SAFETY: single-threaded current-thread runtime; set before any agent code
    // reads these process-globals (ccode_home()/instrumentation mode are OnceLock).
    unsafe {
        std::env::set_var("CCODE_HOME", ccode_home.path());
        std::env::set_var("CCODE_INSTRUMENTATION", "log");
        std::env::set_var("CCODE_INSTRUMENTATION_LOG", &instr_log);
        std::env::set_var("CCODE_CLI_CHAT_PROXY_BASE_URL", server.url());
        std::env::set_var("CCODE_CCODE_API_BASE_URL", server.url());
        std::env::set_var("CCODE_API_KEY", "test-key-for-ci");
        std::env::set_var("CCODE_TELEMETRY_ENABLED", "false");
        std::env::set_var("CCODE_FEEDBACK_ENABLED", "false");
        std::env::set_var("CCODE_TRACE_UPLOAD", "false");
    }
    ...
    let _ = tracing_subscriber::registry()
        .with(ccode_shell::instrumentation::layer::<Registry>())
        .try_init();

    let (info, dir) = prepare_session(ccode_home.path(), cwd.path(), &opts).await;
    let updates_path = dir.join("updates.jsonl");
    let rewind_path = dir.join("rewind_points.jsonl");
    eprintln!(
    ...
    local
        .run_until(async move {
            ...
            if let Some(method) = init.auth_methods.iter().find(|m| &*m.id().0 == "ccode.api_key") {
                let _ = client_conn
            ...
            let _ = ccode_shell::instrumentation::finalize();
