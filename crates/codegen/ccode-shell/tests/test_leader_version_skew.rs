...
//! - `CCODE_BINARY_LEADER` — the binary that elects the initial leader.
//! - `CCODE_BINARY_CLIENT` — the second client.
//!
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: CCODE_BINARY_LEADER/CCODE_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".ccode").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set CCODE_BINARY_LEADER/CCODE_BINARY_CLIENT and run with --ignored"]
            ...
            common::leader::run_with_cleanup(&fixture, &mut clients, |fixture, clients| {
                Box::pin(async move {
                    ...
                    let control = LeaderClient::connect(
                        sandbox.home().join(".ccode").join("leader.sock"),
                        "ccode-pager-update",
                        ClientMode::Stdio,
            ...
            common::leader::run_with_cleanup(&fixture, &mut clients, |fixture, clients| {
                Box::pin(async move {
                    ...
                    let old_pid = wait_for_live_leader(sandbox.home(), Duration::from_secs(10))
                        .await
                        .expect("no live old leader");
                    let auth_path = sandbox.home().join(".ccode").join("auth.json");
