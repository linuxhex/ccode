...
//!
//! This test spawns a real `ccode agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p ccode-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        let ccode_home = shared_sandbox.ccode_home().to_path_buf();
        let session_dir = locate_session_dir(&ccode_home, session_id.0.as_ref());
