...
use ccode_sampler::RetryPolicy;
use ccode_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
...
use ccode_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// `x-ccode-turn-idx`) carries the opt-in header. Aux side-queries the binary
...
/// ```bash
/// cargo test -p ccode-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = ccode_test_support::git_workdir();
    let sandbox = ccode_test_support::TestSandbox::builder()
        .mock_url(server.url())
        .build();
    ...
    let mut cmd = tokio::process::Command::new(ccode_test_support::ccode_binary());
    ...
    let result = ccode_test_support::run_headless_in_sandbox(cmd, sandbox).await;
    ccode_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
    ...
    let responses_posts: Vec<_> = requests
        ...
        .filter(|e| e.method == "POST" && e.path.contains("/responses"))
        .collect();
    // The session turn carries `x-ccode-turn-idx`; aux side-queries (session
    // title, etc.) do not.
    let (turns, aux): (Vec<_>, Vec<_>) = responses_posts
        .into_iter()
        .partition(|e| e.header("x-ccode-turn-idx").is_some());
    assert!(
        !turns.is_empty(),
    ...
    for turn in turns {
        assert_eq!(
            turn.header("x-ccode-doom-loop-check"),
            Some("true"),
    ...
    for side_query in aux {
        assert_eq!(
            side_query.header("x-ccode-doom-loop-check"),
            None::<&str>,
