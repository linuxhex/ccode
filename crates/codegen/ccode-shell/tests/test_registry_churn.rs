...
//! asserts via `ccode.dev/debug/agent` that every registry count returns
...
async fn read_counts(conn: &acp::ClientSideConnection) -> Counts {
    let resp = ext_method(conn, "ccode.dev/debug/agent", json!({})).await;
    serde_json::from_value(resp["result"]["registries"].clone())
        .unwrap_or_else(|e| panic!("ccode.dev/debug/agent: bad registries payload: {e}\n{resp}"))
}
...
async fn close_session(conn: &acp::ClientSideConnection, session_id: &acp::SessionId) {
    ...
    assert_eq!(
        resp["result"]["success"],
        json!(true),
        "ccode.dev/session/close on {} failed: {resp}",
        session_id.0
    );
}
...
async fn connect_and_auth() -> acp::ClientSideConnection {
    ...
    let method = init
        ...
        .expect("ccode.api_key auth method not advertised");
    tokio::time::timeout(
        RPC_TIMEOUT,
