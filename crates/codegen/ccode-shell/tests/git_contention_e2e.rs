...
//!
//! With the fs-watch machinery on (`ccode.dev/hunkTracker` + `ccode.dev/gitHeadChanged`
...
//!
//! Knobs: CCODE_PERF_GIT_FILES (default 300), CCODE_PERF_GIT_PICKS (default 6).
...
use ccode_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};

const DUPLEX_BUFFER_BYTES: usize = 8 * 1024 * 1024;

use ccode_test_utils::env::env_usize;

/// Run a git command with deterministic author/committer; assert success.
fn git(dir: &Path, args: &[&str]) -> String {
    ccode_test_utils::git::run_git(dir, args)
}

// ── scan counting ─────────────────────────────────────────────────────────

use ccode_hunk::{REFRESH_SCAN_LOG_PREFIX, REFRESH_SKIP_LOG_PREFIX};
use ccode_test_utils::tracing_capture::MessagePrefixCounter;
...
fn install_global_scan_counter() -> ScanCounter {
    // CCODE_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("CCODE_E2E_LOG").ok();
    ScanCounter(
        ccode_test_utils::tracing_capture::install_prefix_counter_global(
...
fn build_repo(files: usize, picks: usize) -> (TempDir, String) {
    ...
    ccode_test_utils::git::write_fanout_tree(wd, files, 100);
    git(wd, &["add", "."]);
    git(wd, &["commit", "-m", "populate tree"]);

    let base = ccode_test_utils::git::make_feature_branch(wd, picks);
    (dir, base)
}
...
async fn run_storm(
    ...
    let files = env_usize("CCODE_PERF_GIT_FILES", 300);
    let picks = env_usize("CCODE_PERF_GIT_PICKS", 6);
    ...
    local
        .run_until(async move {
            ...
            let method = init
                .auth_methods
                .iter()
                .find(|m| &*m.id().0 == "ccode.api_key")
                .unwrap_or_else(|| panic!("{label}: ccode.api_key auth method not advertised"));
            client_conn
                .authenticate(
...
fn git_rebase_refresh_storm_e2e() {
    ...
    let server = mock_rt
        ...
        .expect("mock server");
    let ccode_home = TempDir::new().expect("ccode home");
    ...
    let on = agent_rt.block_on(run_storm(
        &server,
        Some(json!({
            "ccode.dev/hunkTracker": { "mode": "agent_only" },
