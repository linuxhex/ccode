...
//!
//! Runs the built ccode binary headless against the mock inference server with
//! a config-defined BYOK model whose bearer comes from a mock auth binary (a
//! script the test writes to disk). The harness's `CCODE_API_KEY` stands in for
//! the session-tier credential.
//!
//! `#[ignore]` (needs a built binary). The CI lifecycle lanes run it against
//! the release artifact via `CCODE_BINARY`; run locally (auto-builds the pager):
//! ```bash
//! cargo test -p ccode-shell --test test_auth_provider_e2e -- --ignored
//! ```
//!
...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    sandbox.remove_env("CCODE_LEADER_SOCKET");

    let ccode_home = sandbox.ccode_home().to_path_buf();
    std::fs::create_dir_all(&ccode_home).expect("create .ccode home");

    let counter = ccode_home.join("mint-count");
    let helper = ccode_home.join("mock-auth.sh");
    std::fs::write(
        &helper,
        format!(
...
"#,
    ...
    // `test-key-for-ci` is the harness's CCODE_API_KEY; it stands in for the
...
async fn provider_with_args_and_json_output_sends_minted_token() {
    ...
    let seen_args = ccode_home.join("seen-args");
    let helper = ccode_home.join("mock-auth-json.sh");
    std::fs::write(
        &helper,
        format!(
            "#!/bin/sh\nprintf '%s' \"$*\" > {}\n\
