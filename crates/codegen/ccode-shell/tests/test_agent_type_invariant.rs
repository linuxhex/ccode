...
//! - Session resume
//!
//! Each test spawns a real `ccode agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p ccode-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "ccode-build")
/// - `model-b`: no agent_type (→ "ccode-build")
...
/// `ccode-build` harness. The system prompt sent to the LLM should contain
/// the ccode-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_ccode_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Ccode") || sys_prompt.contains("ccode"),
            "default model should use ccode-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        drop(writer);
        let reader =
            CcodeStdioClient::spawn_with_sandbox(&server, workdir.workspace(), shared_sandbox).await;
        ...
        let original_has_ccode =
            original_sys_prompt.contains("Ccode") || original_sys_prompt.contains("ccode");
        let resumed_has_ccode =
            resumed_sys_prompt.contains("Ccode") || resumed_sys_prompt.contains("ccode");
        assert_eq!(
            original_has_ccode,
            resumed_has_ccode,
            "resumed session should use the same harness as the original\n\
             original identity markers: ccode={original_has_ccode}\n\
             resumed identity markers: ccode={resumed_has_ccode}\n\
...
/// default to `"ccode-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_ccode_build() {
    with_local_set(|| async {
            ...
            assert!(
            sys_prompt.contains("Ccode") || sys_prompt.contains("ccode"),
            "model without agent_type should default to ccode-build harness\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
}
/// The `CCODE_AGENT` escape hatch should override the model's agent_type.
/// Setting `CCODE_AGENT=ccode-build` with an alternate-agent model should use
/// ccode-build harness.
#[tokio::test]
#[ignore]
async fn test_ccode_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            assert!(
            result.is_ok(),
            "prompt with CCODE_AGENT override failed: {:?}\nstderr:\n{}",
            result.err(),
            client.stderr()
        );
            let sys_prompt = server
            ...
            assert!(
            sys_prompt.contains("Ccode") || sys_prompt.contains("ccode"),
            "CCODE_AGENT=ccode-build should override catalog model agent_type\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
