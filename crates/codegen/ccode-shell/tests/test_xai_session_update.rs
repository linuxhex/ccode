//! Integration test for `_ccode.dev/session/update` notifications.
//!
//! This test verifies that:
//! 1. ccode session notifications (e.g., diff_review) can be sent via ext_notification
...
use ccode_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as CcodeSessionUpdate,
};
use ccode_shell::session::info::Info as SessionInfo;
use ccode_shell::session::persistence::default_model_id;
use ccode_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that ccode session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_xai_session_notification_storage_roundtrip() {
    ...
    let ccode_notification = SessionNotification {
        session_id: session_id.clone(),
        update: CcodeSessionUpdate::DiffReview {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Ccode(Box::new(ccode_notification.clone())),
        )
        .await
        .unwrap();
...
/// must survive `updates.jsonl` and reload as a replayable `_ccode.dev/session/update`.
#[tokio::test]
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Ccode(Box::new(ccode_notification)))
        .await
        .unwrap();
...
/// Test that totalTokens can be extracted from both ACP and ccode notifications.
#[tokio::test]
async fn test_extract_total_tokens_from_mixed_updates() {
    ...
    // Add ccode notification with totalTokens
    let ccode_notification = SessionNotification {
        session_id: session_id.clone(),
        update: CcodeSessionUpdate::DiffReview { content: vec![] },
