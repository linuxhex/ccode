...
/// All tests share this as CCODE_HOME (the `OnceLock` in ccode-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = ccode_shell::config::load_effective_config().unwrap();
    let effective_cfg = ccode_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    ccode_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = ccode_shell::util::config::load_config_from_toml(&user_toml);
    ...
    assert_eq!(user_cfg.models.default.as_deref(), Some("ccode-3"));
}
