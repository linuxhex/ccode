...
//!
//! Runs the built ccode binary against the mock inference server with a
//! caller-owned `$CCODE_HOME`, then inspects `~/.ccode/debug/`:
...
//!   `~/.ccode/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `CCODE_LOG_SAMPLING`/`CCODE_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `CCODE_DEBUG_LOG=1`) writes a per-session
//!   `~/.ccode/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.ccode/debug/` files).
//! - `CCODE_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p ccode-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$CCODE_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".ccode").join("debug")
}

/// List firehose `*.txt` files under `~/.ccode/debug` (excluding the `latest.txt`
...
/// Build a headless `ccode -p` command with a pinned `$CCODE_HOME` so the firehose
/// lands under `<home>/.ccode/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    sandbox
        ...
        .set_env("CCODE_HOME", home.join(".ccode"));
...
/// assert it carries first-party (`ccode`) content. Panics with the captured
/// stderr tail if it never fills. Shared by the live-agent tests.
async fn read_session_firehose_when_ready(path: &Path, client: &CcodeStdioClient) -> String {
...
/// headless `ccode -p` client is near-silent, so its lazily-opened firehose may
...
async fn debug_flag_enables_firehose_without_crashing() {
    ...
    assert_headless_success(&result, "ccode --debug headless", Some(&server));
...
async fn no_debug_flag_writes_no_debug_dir() {
    ...
    assert_headless_success(&result, "ccode headless (no --debug)", Some(&server));
    assert!(
...
/// A live `agent` session writes `~/.ccode/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        sandbox.set_env("CCODE_DEBUG_LOG", "1");
        let ccode_home = sandbox.ccode_home().to_path_buf();

        let client =
            CcodeStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
        ...
        let session_file = ccode_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.ccode/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = ccode_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `CCODE_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `CCODE_LOG_SAMPLING`/`CCODE_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // Drive `ccode --debug agent stdio`: the master switch (which runs before
        // the agent dispatch) must be what enables the firehose — NOT a direct
        // CCODE_DEBUG_LOG env. The sandbox baseline excludes inherited firehose
        // toggles, so the `--debug` flag is the only thing enabling logging here.
        let client = CcodeStdioClient::spawn_with_sandbox_env_and_args(
            &server,
            workdir.workspace(),
            sandbox,
            &[],
        ...
        // clears CCODE_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.ccode/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // CCODE_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = ccode_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.ccode/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn debug_file_flag_writes_single_file_and_bypasses_routing() {
    ...
    assert_headless_success(&result, "ccode --debug-file", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        explicit.exists(),
...
/// `CCODE_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn ccode_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    sandbox.set_env("CCODE_LOG_FILE", &custom);
    let result = run_headless_in_sandbox(cmd, sandbox).await;

    assert_headless_success(&result, "ccode CCODE_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit CCODE_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "CCODE_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
