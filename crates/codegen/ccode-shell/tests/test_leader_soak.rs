...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p ccode-shell --test test_leader_soak -- --ignored --nocapture
//! ```
...
/// Copied from `ccode-graph/tests/memory_integration.rs`.
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    let sock_path = ccode_home.path().join("leader-soak.sock");
    ...
    local
        .run_until(async {
            ...
            // the same wiring: ccode-pager/src/app/leader_cluster/mod.rs
            ...
            rpc(
                &mut bootstrap,
                r#"{"jsonrpc":"2.0","id":2,"method":"authenticate","params":{"methodId":"ccode.api_key","_meta":{"headless":true}}}"#.to_string(),
                2,
                "authenticate",
            )
            .await;
