...
//! `.ccode`, `.cursor`, and `.claude` vendor dirs, spawns a real `ccode agent
//! stdio` process against the mock inference server (toggling the
//! `CCODE_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!
//! - the Ccode-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p ccode-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_CCODE_SKILL: &str = "ZZ_CCODE_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".ccode", "ccode-skill", MARKER_CCODE_SKILL);
...
async fn run_scenario(env: &[(&str, &str)]) -> String {
    ...
    let client = CcodeStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
...
/// Defaults (all vendors on): ccode + cursor-vendor + claude-vendor skills present; the
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_CCODE_SKILL),
            "ccode-skill must always be present"
        );
        assert!(
...
/// `CCODE_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; ccode stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("CCODE_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_CCODE_SKILL),
            "ccode-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `CCODE_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; ccode stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("CCODE_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_CCODE_SKILL),
            "ccode-skill always present"
        );
...
/// `CCODE_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("CCODE_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `CCODE_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("CCODE_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `CCODE_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("CCODE_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `CCODE_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("CCODE_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("CCODE_CURSOR_SKILLS_ENABLED", "false"),
            ("CCODE_CURSOR_RULES_ENABLED", "false"),
            ("CCODE_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_CCODE_SKILL), "ccode always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("CCODE_CLAUDE_SKILLS_ENABLED", "false"),
            ("CCODE_CLAUDE_RULES_ENABLED", "false"),
            ("CCODE_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only ccode-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary
