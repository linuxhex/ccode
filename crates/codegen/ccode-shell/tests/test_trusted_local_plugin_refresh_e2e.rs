...
//! 4. Smoke-validate session JSON under `$CCODE_HOME/sessions/` after exit.
//!
//! Requires a built `ccode` binary (`CCODE_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p ccode-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p ccode-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use ccode_agent::plugins::SharedPluginRegistryHandle;
use ccode_agent::plugins::discovery::DiscoveryConfig;
use ccode_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _ccode_guard = EnvVarGuard::set("CCODE_HOME", &ccode_home);
    ...
    let mut registry = InstallRegistry::empty(ccode_home.join("installed-plugins"));
    ...
    // ccode_home() caches the first CCODE_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        ccode_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built ccode binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/CCODE_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via ccode_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _ccode_guard = EnvVarGuard::set("CCODE_HOME", &ccode_home);
    ...
    // Smoke: session storage under CCODE_HOME/sessions has JSON artifacts.
