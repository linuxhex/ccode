...
//!
//! Each test spawns a real `ccode agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p ccode-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.ccode/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".ccode").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let ccode_dir = sandbox.ccode_home();
        std::fs::write(
