//! Built-binary end-to-end tests for the ccode (ccode-pager) binary.
//!
//! These tests verify that the built ccode binary works end-to-end against a mock
...
//!
//! The tests exercise:
//! - **Smoke** (`ccode --version`): binary loads without crashing
//! - **ACP stdio** (`ccode agent stdio`): full protocol lifecycle via ClientSideConnection
//!
//! Tests are `#[ignore]`d by default — they require a pre-built binary.
//!
//! Run locally (auto-builds the binary if not already present):
//! ```bash
//! cargo test -p ccode-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
//! In CI, set `CCODE_BINARY` to point at the release artifact:
//! ```bash
//! CCODE_BINARY=./artifacts/ccode-0.1.159-linux-x86_64 \
//!   cargo test -p ccode-shell --test test_built_binary_e2e -- --ignored
//! ```
...
async fn ccode_build_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
        MockModelEntry::with_agent_type("ccode-4.5", "ccode-build")
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("CCODE_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "ccode --version with CCODE_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_session_in_git_repo() {
    ...
    assert_headless_success(&result, "ccode -p in git repo", Some(&server));
...
/// Verify ccode works in a non-git directory (exercises the fallback codepath
...
async fn test_headless_session_in_non_git_dir() {
    ...
    assert_headless_success(&result, "ccode -p in non-git dir", Some(&server));
    assert_no_crashes(&result.stderr);
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    let server = ccode_build_server().await;
    ...
    assert_headless_success(&result, "ccode -p --tools with web tools", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_tools_allowlist_does_not_fail_open_for_disabled_web_fetch() {
    ...
    assert_headless_success(
        &result,
        "ccode -p --tools with disabled web_fetch",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
    let names = inference_tool_names(&server);
...
async fn test_headless_terminal_only_allowlist_is_foreground_only() {
    ...
    assert_headless_success(&result, "ccode -p --tools run_terminal_cmd", Some(&server));
    assert_no_crashes(&result.stderr);
    let request = inference_request(&server);
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free Ccode Build usage limit"),
...
async fn test_headless_streaming_json_output() {
    ...
    assert_headless_success(
        &result,
        "ccode -p --output-format streaming-json",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_json_reports_server_cost() {
    use ccode_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ccode_test_support::scripted::ScriptedResponse::sse(vec![
    ...
    assert_headless_success(&result, "ccode -p (scripted cost)", Some(&server));
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ccode_test_support::scripted::ScriptedResponse::sse(
            ccode_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
async fn headless_json_schema_chat_completions_uses_response_format() {
    ...
    assert_headless_success(
        &result,
        "ccode -p --json-schema (chat_completions)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_responses_uses_text_format() {
    let server = single_model_server("ccode-4.5", "responses").await;
    server.set_response(r#"{"name":"Alice","age":30}"#);
    ...
    assert_headless_success(&result, "ccode -p --json-schema (responses)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_backend_uses_structured_output_tool() {
    ...
    assert_headless_success(&result, "ccode -p --json-schema (messages)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_validates_text_when_tool_not_called() {
    ...
    assert_headless_success(
        &result,
        "ccode -p --json-schema (messages, text)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_retries_on_schema_violation() {
    ...
    assert_headless_success(
        &result,
        "ccode -p --json-schema (messages, retry)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn invalid_json_schema_disables_structured_output_and_surfaces_error() {
    ...
    assert_headless_success(
        &result,
        "ccode -p --json-schema (invalid schema)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
// ============================================================================
// ACP stdio tests (ccode agent stdio)
//
// These test the agent as a server: spawn `ccode agent stdio`, speak the full
...
async fn test_stdio_full_session_lifecycle() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let client = CcodeStdioClient::spawn(&server, workdir.workspace()).await;
...
/// Verify that ccode.dev/session/close frees the session.
...
async fn test_stdio_prompt_then_immediate_load_session() {
    with_local_set(|| async {
        let server = MockInferenceServer::start().await.expect("start mock server");
        let workdir = git_workdir();
        let mut writer = CcodeStdioClient::spawn(&server, workdir.workspace()).await;
        ...
        let reader = CcodeStdioClient::spawn_with_sandbox(
            &server,
            workdir.workspace(),
            shared_sandbox,
        )
...
async fn test_stdio_xcode_escaped_slash_methods_get_responses() {
    ...
    agent
        .send_line(&format!(
            r#"{{"jsonrpc":"2.0","id":"{auth_id}","method":"authenticate","params":{{"methodId":"ccode.api_key","_meta":{{"headless":true}}}}}}"#
        ))
        .await;
    let auth_resp = agent
...
/// Isolated headless run with a custom `~/.ccode/`. Clean env (no leaked
/// host credentials). Write config files into `ccode_dir()` before `run()`.
...
impl ConfigTestHarness {
    ...
    fn ccode_dir(&self) -> std::path::PathBuf {
        self.sandbox.ccode_home().to_path_buf()
    }
    ...
    async fn run(self) -> HeadlessResult {
        let mut cmd = tokio::process::Command::new(ccode_binary());
...
async fn test_headless_managed_config_byok_sends_authorized_requests() {
    let server = MockInferenceServer::start_with_required_auth(
        vec![MockModelEntry::new("ccode-4.5")],
    ...
    std::fs::write(
        h.ccode_dir().join("managed_config.toml"),
        format!(
            r#"
[endpoints]
deployment_key = "test-deployment-key"
ccode_api_base_url = "{url}"

[model."ccode-4.5"]
api_backend = "responses"
base_url = "{url}"
context_window = 500000
env_key = "CCODE_TEST_BYOK_TOKEN"
model = "ccode-4.5"
...
"#,
    ...
    .unwrap();
    h.env("CCODE_TEST_BYOK_TOKEN", "test-byok-secret-token");
...
async fn headless_reasoning_efforts_payload_parses_and_legacy_effort_rides_wire() {
    ...
    assert_headless_success(&result, "ccode -p reasoning_efforts list", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_timeout_exit_kills_pending_background_task() {
    ...
    assert_headless_success(
        &result,
        "ccode -p with pending background task",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_no_wait_exit_kills_background_task() {
    ...
    assert_headless_success(&result, "ccode -p --no-wait-for-background", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_waits_for_short_background_task_and_exits_clean() {
    ...
    assert_headless_success(&result, "ccode -p with short background task", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        marker.exists(),
