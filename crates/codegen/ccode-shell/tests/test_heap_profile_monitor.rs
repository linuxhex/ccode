...
use ccode_shell::auth::{AuthManager, AuthMode, CcodeAuth, CcodeComConfig};
use ccode_shell::heap_profile::{
...
use ccode_test_support::{EnvGuard, MockInferenceServer};
use ccode_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = CcodeComConfig::default().auth_scope();
    let auth = CcodeAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[ccode_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), CcodeComConfig::default()));
