...
//! default since they need the ccode binary (`CCODE_BINARY` or a local debug build):
//! ```bash
//! cargo test -p ccode-shell --test test_stop_hook_e2e -- --ignored
//! ```
...
async fn run_with_stop_hook(respond: &str) -> StopHookRun {
    ...
    let hooks_dir = sandbox.ccode_home().join("hooks");
...
async fn stop_block_loop_ends_at_continuation_cap() {
    ...
    assert_eq!(
        run.invocations(),
        ccode_shell::session::MAX_STOP_HOOK_CONTINUATIONS_PER_TURN,
