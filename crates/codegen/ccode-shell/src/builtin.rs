//! Built-in files extracted to `~/.ccode/` on startup.

const BUILTIN_FILES: &[(&str, &str)] = &[("README.md", include_str!("../README.md"))];

/// Extract built-in metadata files to `~/.ccode/` on startup.
///
/// User skills under `~/.ccode/skills/` are never managed here. Platform skills
/// are delivered separately through the bundled skill cache.
pub fn extract_builtin_files(ccode_home: &std::path::Path) {
    let version = ccode_version::VERSION;
    let marker = ccode_home.join(".metadata_version");
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(ccode_home.join(stale));
    }

    for &(filename, content) in BUILTIN_FILES {
        if let Err(e) = std::fs::write(ccode_home.join(filename), content) {
...
mod tests {
    ...
    fn same_version_does_not_restore_missing_or_delete_legacy_skills() {
        ...
        std::fs::write(home.join(".metadata_version"), ccode_version::VERSION).unwrap();
