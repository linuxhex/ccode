...
use ccode_agent::plugins::discovery::PluginScope;
use ccode_agent::plugins::git_install::{self, UpdateStatus};
use ccode_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use ccode_marketplace::git::{self, SourceCacheLease};
use ccode_marketplace::{
...
/// When `keep_data` is true, `~/.ccode/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &ccode_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `ccode plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `ccode plugin install ./{name}`, or add a \
                     source with `ccode plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `ccode plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `ccode plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub fn marketplace_require_sha() -> bool {
    ccode_config::load_effective_config_disk_only()
        .map(|c| ccode_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| ccode_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &ccode_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.ccode/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user ccode via user_ccode_home() (None when no home resolves) and
    // home separately, so removal still runs from $CCODE_HOME when no home dir
    // exists, and never touches a cwd-relative .ccode.
    let home = dirs::home_dir();
    let ccode = ccode_config::user_ccode_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref ccode) = ccode {
        settings_candidates.push(ccode.join("settings.local.json"));
        settings_candidates.push(ccode.join("settings.json"));
    }
    ...
    if let Some(ref ccode) = ccode {
        known_candidates.push(ccode.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            registered_source_label(&git_source(
                "ccode Official",
                "https://github.com/ccode-org/plugin-marketplace.git"
            )),
            "ccode Official (ccode-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "ccode Official (pin: sentry@ccode-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "ccode Official (ccode-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - ccode Official (ccode-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "ccode-org/plugin-marketplace".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("ccode plugin install ./sentry"), "{msg}");
        assert!(msg.contains("ccode plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec!["ccode Official (pin: sentry@ccode-org/plugin-marketplace)".into()],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - ccode Official (pin: sentry@ccode-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("ccode plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/ccode-org/plugin-marketplace.git";

    #[test]
    fn plan_install_qualifier_unknown_lists_registered_labels() {
        let sources = [
            git_source("ccode Official", OFFICIAL_URL),
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "ccode Official (ccode-org/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_ambiguous_lists_source_names() {
        let sources = [
            ...
            git_source("Mirror B", "git@github.com:ccode-org/plugin-marketplace.git"),
        ];
        let err = plan_install(
            &sources,
            "sentry",
            Some("ccode-org/plugin-marketplace"),
            |_| Ok(Vec::new()),
        )
        .expect_err("two sources share the owner/repo");
        match err {
            MarketplaceInstallError::AmbiguousQualifier { qualifier, sources } => {
                assert_eq!(qualifier, "ccode-org/plugin-marketplace");
                assert_eq!(
                    sources,
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("ccode Official", OFFICIAL_URL)];
        let err = plan_install(
            &sources,
            "sentry",
            Some("ccode-org/plugin-marketplace"),
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "ccode Official");
            }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> ccode_workspace::permission::resolution::MarketplaceAllowlist {
        ccode_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = ccode_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "ccode-org/plugin-marketplace")
            .expect("qualifier should match the official source");
        assert_eq!(name, "ccode Official");
    }
