...
use url::Url;
use ccode_fs::queue::{EnqueueOutcome, TraceExportSource, UploadQueue, UploadRetryPolicy};
use ccode_workspace::permission::PermissionEvent;
...
pub(crate) async fn upload_tool_definitions(
    ...
    let ok = ccode_fs::gcs::upload_bytes(
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        ccode_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        ccode_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = ccode_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: ccode_sandbox::is_active(),
    })
}
...
fn classify_workspace(cwd: &str) -> String {
    ...
    if path.ancestors().any(|p| p.join(".git").exists()) {
        "git".to_owned()
    } else if ccode_fs::workspace_classifier::is_project_dir(path) {
        "project".to_owned()
    } else {
...
pub(crate) async fn upload_subagent_metadata(
    ...
    if let Err(e) =
        ccode_fs::gcs::upload_bytes(&config, &gcs_path, &json, "application/json").await
    {
        tracing::warn!(
            session_id = %metadata.child_session_id,
            gcs_path = %gcs_path,
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&ccode_agent::plugins::PluginRegistry>,
) {
...
pub(crate) async fn upload_artifact_to_gcs(
    ...
    match upload_bytes(&config, gcs_path, content, content_type).await {
        ...
        Err(e) => {
            let status_code = e
                .downcast_ref::<ccode_fs::storage_client::HttpUploadError>()
                .map(|e| e.status_code);
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        ccode_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(ccode_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) async fn upload_turn_messages(
    ctx: &PromptTraceContext,
    _capture: ccode_chat::TurnCapture,
    _wait: UploadWait,
) -> bool {
    super::manifest::skip_artifact(
        &ctx.artifact_tracker,
...
pub(crate) fn build_chat_history_session_state(
    messages: &[ccode_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_attribution(
        &self,
    ) -> Option<Arc<dyn ccode_fs::storage_client::Auth401AttributionCallback>> {
        ccode_fs::gcs::StorageConfig::proxy_attribution(&self.with_auth())
    }
    fn proxy_credentials(&self) -> Option<Arc<dyn ccode_auth::AuthCredentialProvider>> {
        ccode_fs::gcs::StorageConfig::proxy_credentials(&self.with_auth())
    }
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        ccode_fs::gcs::StorageConfig::proxy_http_client(&self.with_auth())
    }
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    ccode_workspace::recovery::run_startup_recovery(&ccode_home, &queue).await;
                tracing::info!(?report, "startup spill recovery complete");
                queue.cleanup_orphans(ccode_fs::queue::DEFAULT_MAX_AGE);
            }
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    ccode_workspace::recovery::purge_spilled_items(&ccode_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) fn spawn_purge_stale_upload_scratch() {
    {
        ...
        let dir = crate::util::ccode_home::ccode_home()
            .join("upload_queue")
            .join("scratch");
...
pub(crate) fn spawn_upload_queue(
    ...
    let queue = UploadQueue::spawn(ccode_home, resolver, UploadRetryPolicy::default());
...
mod tests {
    ...
    fn dynamic_resolver_refreshes_proxy_token() {
        ...
        let ccode_com_config = CcodeComConfig::default();
        let scope = ccode_com_config.auth_scope();
        let initial_auth = CcodeAuth {
    ...
    async fn resolve_async_falls_back_when_no_refresher() {
        ...
        let auth_json = serde_json::to_string_pretty(&store).unwrap();
        std::fs::write(dir.path().join("auth.json"), &auth_json).unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), ccode_com_config));
        let resolver = DynamicResolver {
            auth_manager,
            base_config: TraceExportConfig {
                bucket_url: None,
    ...
    async fn resolve_async_drives_refresh_chain_when_token_expired() {
        ...
        impl crate::auth::refresh::TokenRefresher for FreshRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(crate::auth::CcodeAuth {
    ...
    async fn proactive_refresh_makes_trace_resolve_a_cache_hit() {
        ...
        let ccode_com_config = CcodeComConfig::default();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), ccode_com_config));
        auth_manager.hot_swap(CcodeAuth {
            key: "expired-oidc".into(),
    ...
    fn dynamic_resolver_supplies_proxy_credentials_and_attribution() {
        use crate::session::repo_changes::UploadMethod;
        use ccode_fs::queue::TraceExportSource;
        let dir = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
    ...
    async fn dynamic_resolver_auth_recovery_wakes_on_already_rotated_token() {
        ...
        auth_manager.hot_swap(crate::auth::CcodeAuth {
    ...
    fn home_project_dir() -> Option<tempfile::TempDir> {
        ...
        if !ccode_fs::workspace_classifier::is_project_dir(&home.join("probe")) {
            return None;
        }
