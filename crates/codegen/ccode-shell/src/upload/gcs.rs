...
//! `StorageClient` constructed inside `ccode_fs::gcs::*` helpers.
//!
...
/// shell-side adapter that lets `ccode_fs::gcs::*` helpers wire
...
/// `CCODE_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("CCODE_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match ccode_fs::gcs::upload_bytes(
