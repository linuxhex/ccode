...
//! connection to the ccode.dev relay server with automatic reconnection.
...
/// Config for the ccode.dev WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a ccode.dev first-party session
    /// (`is_ccode_auth`: ccode.dev-issuer OIDC or external credential) with a
    /// non-empty bearer. BYOK/ApiKey, non-ccode.dev issuers (enterprise OIDC,
    ...
    pub(crate) fn for_session(
        ...
        if !session.is_ccode_auth() || session.key.is_empty() {
            return None;
        }
        let _ = alpha_test_key;
        Some(Self {
            ws_url: ctx.ccode_ws_url.clone(),
            ws_origin: ctx.ccode_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to ccode.dev with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `ccode login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Ccode WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(ccode_version::VERSION),
    );
    req.headers_mut().insert(
...
mod tests {
    ...
    /// Helper to create a test CcodeAuth with the given key.
    fn test_auth(key: &str) -> CcodeAuth {
        CcodeAuth {
            key: key.to_string(),
    ...
    #[test]
    fn for_session_builds_only_for_ccode_issuer() {
        use crate::auth::CCODE_OAUTH2_ISSUER;
        let cfg = CcodeComConfig::default();
        let builds = |a: &CcodeAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let ccode_auth = CcodeAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(CCODE_OAUTH2_ISSUER.to_string()),
            ..test_auth("ccode-bearer")
        };
        assert!(ccode_auth.is_ccode_auth(), "precondition: is_ccode_auth");
        assert!(builds(&ccode_auth));
        let external_ccode = CcodeAuth {
            auth_mode: AuthMode::External,
            oidc_issuer: Some(CCODE_OAUTH2_ISSUER.to_string()),
            ..test_auth("ext-bearer")
        };
        assert!(external_ccode.is_ccode_auth(), "precondition: is_ccode_auth");
        assert!(builds(&external_ccode));
        assert!(!builds(&CcodeAuth {
            key: String::new(),
            ..ccode_auth.clone()
        }));
        assert!(!builds(&CcodeAuth {
    ...
    /// Helper: write a CcodeAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &CcodeAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        impl TokenRefresher for CountingRefresher {
            async fn refresh(
                ...
                RefreshOutcome::Success(Box::new(CcodeAuth {
        ...
        let mut config = RelayConfig::for_session(&expired_session, &cfg, None, Some(am.clone()))
            .expect("ccode.dev OIDC session is relay-eligible");
        let cancel = CancellationToken::new();
