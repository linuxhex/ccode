//! ccode.dev chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching ccode-web's refetch cadence.
...
pub const CCODE_CHAT_MODE_ENV: &str = "CCODE_CHAT_MODE";
...
impl ChatModesManager {
    ...
    /// The active ccode.dev identity, or `None` when unauthenticated. Modes are
...
/// Maps ccode.dev modes → `SessionModelState`: keeps only `available` modes,
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "ccode-4.5");
    }
}
