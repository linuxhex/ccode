...
/// Router for ccode.dev/session/* and ccode.dev/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "ccode.dev/session/info" => handle_session_info(agent, args).await,
        "ccode.dev/session/close" => handle_session_close(agent, args).await,
        "ccode.dev/session/list" => handle_session_list(agent, args).await,
        "ccode.dev/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("ccode.dev/session_summaries/") => {
...
/// `ccode.dev/sessions/list` — the FleetView roster. Returns every
...
/// `ccode.dev/sessions/changed` broadcast.
...
async fn handle_session_close(
    ...
    if existed {
        ...
        tracing::info!(session_id = %req.session_id, "session closed via ccode.dev/session/close");
    } else {
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "ccode.dev/session_summaries/session_list" => {
        ...
        "ccode.dev/session_summaries/workspace_list" => {
            tracing::debug!("ccode/session_summaries/workspace_list is working");
        ...
        "ccode.dev/session_summaries/workspace_list_recent" => {
