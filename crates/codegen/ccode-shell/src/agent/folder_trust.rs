...
//! `.ccode/config.toml` including `[permission]` / `[mcp_servers]` /
//! `[plugins].paths`, `~/.claude.json` `projects.<cwd>`, project `.ccode/lsp.json`).
...
//! interactive prompt, and the durable [`ccode_workspace::trust::TrustStore`]
//! reads/writes — lives in `ccode-workspace` (client-side); this module keeps
...
//! on [`ccode_workspace::folder_trust::decide`]; the consume-side nuance is
...
use ccode_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};

// Decision-side (scan/decide/prompt/store) relocated to `ccode-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `ccode_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use ccode_workspace::folder_trust::grant_folder_trust;
use ccode_workspace::folder_trust::{
...
// NOTE: this folder-trust store (`~/.ccode/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`ccode_agent::plugins::TrustStore` at `~/.ccode/trusted-plugins`, plus the
...
/// [`ccode_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = ccode_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// `ccode.dev/folderTrust.interactive`); the TTY-based [`decide_inputs`] default is
...
/// ([`ccode_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.ccode/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    ccode_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: ccode_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != ccode_agent::config::AgentScope::Project || trusted()
}
...
/// whose cwd differs from the launch dir, `ccode mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `ccode --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `ccode mcp doctor`). It MUST
...
///
/// Sources: project `.ccode/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.ccode/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.ccode/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            ccode_tools::implementations::lsp::config::LspServerConfig,
            ccode_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, ccode_tools::implementations::lsp::config::LspServerConfig>
{
    ccode_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use ccode_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(ccode_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; CCODE_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because CCODE_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. CCODE_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because CCODE_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. CCODE_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because CCODE_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; CCODE_HOME-isolated store; CCODE_FOLDER_TRUST unset so the
        ...
        std::fs::create_dir_all(home.path().join(".ccode").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. CCODE_HOME-isolated so the trust store is empty;
        // CCODE_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = ccode_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. CCODE_HOME-isolated so the trust store is empty;
        // CCODE_FOLDER_TRUST unset so the default-on feature flag applies.
        use ccode_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). CCODE_HOME-isolated so the trust store is empty.
        use ccode_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. CCODE_HOME-isolated (empty store).
        use ccode_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".ccode").join("agents");
        ...
        .unwrap();
        let def = ccode_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.ccode/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). CCODE_HOME-isolated (empty
        // store); CCODE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + CCODE_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. CCODE_HOME-isolated so the seeded store is the temp one;
        // CCODE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // CCODE_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(ccode_version::TEST_VERSION_ENV);
        if option_env!("CCODE_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.ccode/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. CCODE_HOME-isolated (empty store);
        // CCODE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_denies_untrusted_permission_only_repo() {
        // Bridge: a clone whose ONLY repo-local config is `.ccode/config.toml`
        ...
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(
            ccode.join("config.toml"),
            "[permission]\nallow = [\"Bash(*)\"]\n",
        )
        .unwrap();
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // CCODE_HOME-isolated (empty store); CCODE_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".ccode").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // CCODE_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); CCODE_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use ccode_agent::plugins::discovery::DiscoveryConfig;
        use ccode_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".ccode").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // CCODE_HOME-isolated so the grant writes to a temp store; CCODE_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".ccode").join("hooks");
        ...
        let has_project_hook = |reg: &ccode_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = ccode_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &ccode_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use ccode_tools::implementations::lsp::config::LspServerConfig;
        use ccode_tools::types::config_source::ConfigSource;
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.ccode/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.ccode/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use ccode_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use ccode_tools::types::config_source::ConfigSource;

        // A `<cwd>/.ccode/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.ccode/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(ccode.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.ccode/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(ccode.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.ccode/config.toml [mcp_servers]`).
    ...
    /// `.ccode/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `ccode --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(ccode_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("CCODE_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no CCODE_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(ccode_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(ccode_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("CCODE_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // CCODE_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("CCODE_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".ccode").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".ccode").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with CCODE_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. CCODE_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(ccode_version::TEST_VERSION_ENV);
        if option_env!("CCODE_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. CCODE_HOME-isolated so the store starts
        // empty; `#[serial]` because CCODE_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // CCODE_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
        // opt-out would otherwise false-fail the Prompt assertion.
        let _sim = EnvGuard::set(ccode_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; CCODE_HOME-isolated and
        // CCODE_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CCODE_HOME", home.path());
        let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".ccode").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".ccode").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.ccode/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode")).unwrap();
        std::fs::write(tmp.path().join(".ccode").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
