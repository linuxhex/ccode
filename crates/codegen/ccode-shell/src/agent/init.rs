...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    if cfg.remote_settings.is_none()
        && let Some(handle) =
            crate::agent::models::start_early_prefetch(Some(cfg.ccode_com_config.clone()))
    {
        match handle.join() {
            Ok(result) => {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (ccode -p, subagents).
    ...
    // Writeback talks to the code backend; requires ccode.dev auth.
    if cfg.storage_mode == StorageMode::Writeback
        && !auth_manager.current().is_some_and(|a| a.is_ccode_auth())
    {
        tracing::info!("Writeback is disabled: requires auth with ccode.dev");
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_builtin_files(&ccode_home);

        crate::extensions::marketplace::purge_default_skills_installs(&ccode_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&ccode_home);
        }
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let ccode_auth = auth_manager.current().filter(|a| a.is_ccode_auth());
    let user_id = ccode_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = ccode_auth.as_ref().and_then(|a| a.team_id.clone());
