...
use ccode_subagent::resolve_effective_overrides;
...
fn canonical_total_tokens_does_not_double_count_reasoning() {
    let totals = ccode_chat::UsageTotals {
        input_tokens: 100,
        output_tokens: 40,
        reasoning_tokens: 25,
        ..Default::default()
...
async fn usage_ack_precedes_terminal_presentation() {
    ...
    assert!(matches!(
            ...
            Ok(SessionCommand::CcodeSessionNotification {
...
async fn subagent_inherits_session_cli_overrides() {
    use ccode_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    use ccode_agent::config::PermissionMode;
    const PIN: &str = ccode_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
async fn emit_subagent_notification_stamps_one_event_id_on_both_paths() {
    ...
    let persisted_id = match cmd_rx.try_recv().expect("persist hop must fire") {
        SessionCommand::CcodeSessionNotification { notification } => {
            notification
                .meta
                .as_ref()
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
            resume_worktree_action(true, Some("refs/ccode/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
            resume_worktree_action(false, Some("refs/ccode/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn ccode_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
    let mut ctx = ctx_with_toggle(HashMap::new());
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let reservations = ccode_tools::reminders::task_completion::TaskCompletionReservations::default();
...
fn explicit_override_takes_precedence_over_role() {
    let overrides = SubagentRuntimeOverrides {
        ...
        capability_mode: Some(ccode_tool_types::SubagentCapabilityMode::All),
        ..Default::default()
    };
    let role = ccode_subagent::config::SubagentRole {
    ...
    assert_eq!(
            ...
            Some(ccode_tool_types::SubagentCapabilityMode::ReadOnly)
        );
}
#[test]
...
fn partial_override_fills_from_role() {
    ...
    assert_eq!(
            resolved.capability_mode,
            Some(ccode_tool_types::SubagentCapabilityMode::Execute)
        );
}
#[test]
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            ccode_subagent::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = ccode_subagent::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = ccode_subagent::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use ccode_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
            ConversationItem::system("parent system"),
...
async fn bootstrap_fork_live_parent_chat_state_is_forked_with_marker() {
    ...
    let req = bootstrap_test_request(true);
    let mut ctx = ctx_with_toggle(HashMap::new());
    let chat = spawn_test_parent_chat_state("ccode-4.5");
...
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("CcodeBuild:read_file");
...
/// Regression: on the DEFAULT ccode-build host —
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    ...
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::CcodeBuildPlan.as_ref().to_string(),
    );
...
async fn cancel_pending_shell_child_presents_one_cancelled_finish() {
    ...
    while let Ok(command) = parent_cmd_rx.try_recv() {
        if matches!(
                command,
                SessionCommand::CcodeSessionNotification {
                    notification: SessionNotification {
...
async fn cancel_pending_at_promote_removes_fresh_worktree_preserves_resumed() {
    ...
    ccode_worktree::WorktreeBuilder::new(&repo, &fresh)
        .standalone(true)
        .create()
        .unwrap();
    assert!(fresh.exists());
    ...
    ccode_worktree::WorktreeBuilder::new(&repo, &resumed)
        .standalone(true)
        .create()
        .unwrap();
...
fn subagent_auth_type_rule() {
    use crate::agent::auth_method::{CACHED_TOKEN_AUTH_METHOD_ID, CCODE_API_KEY_METHOD_ID};
    use ccode_chat::AuthType;
    let session = acp::AuthMethodId::new(CACHED_TOKEN_AUTH_METHOD_ID);
    let api_key = acp::AuthMethodId::new(CCODE_API_KEY_METHOD_ID);
    let byok = byok_model_entry("ccode-byok");
    let plain = test_model_entry("ccode-plain");
    assert_eq!(
...
fn fresh_tool_model_accepts_visible_key_and_internal_id() {
    ...
    models.insert("ccode-3".to_string(), test_model_entry("ccode-3-2025-02-15"));
    assert!(
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(!error.contains("ccode models"));
    }
    assert!(
            super::handle_request::task_model_override_error(
                Some("oauth-only"),
...
fn test_sampling_config(model_slug: &str) -> ccode_sampling_types::SamplingConfig {
...
fn spawn_test_parent_chat_state(model_slug: &str) -> ccode_chat::ChatStateHandle {
    let (mock, _persistence_rx) = ccode_chat::MockChatPersistence::new();
    let (event_tx, _event_rx) = mpsc::unbounded_channel();
    let token = tokio_util::sync::CancellationToken::new();
    ccode_chat::ChatStateActor::spawn(
        vec![],
