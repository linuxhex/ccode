...
use ccode_tools::implementations::ccode_build::task::backend::ChannelBackend;
#[test]
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = ccode_subagent::context::normalize_forked_context(
        items,
    );
...
fn normalize_forked_context_consecutive_users() {
    ...
    let (conv, prefix_len) = ccode_subagent::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = ccode_subagent::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = ccode_subagent::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .find_map(|item| {
            if let ConversationItem::User(u) = item {
                let text: String = u
                    ...
                    .iter()
                    .filter_map(|p| match p {
                        ccode_sampling_types::conversation::ContentPart::Text {
                            text,
                        } => Some(text.as_ref()),
                        _ => None,
                    })
            ...
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                    p,
                    ccode_sampling_types::conversation::ContentPart::Text { text } if text.contains("<background_context>")
                )
                    })
            } else {
                false
...
fn snapshot_ref_field_in_meta_roundtrips() {
    let meta = SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/ccode-wt/sa-snap".into()),
        snapshot_ref: Some("refs/ccode/subagent-snapshots/sa-snap".into()),
        effective_model_id: None,
    };
    let json = serde_json::to_string(&meta).unwrap();
    assert!(json.contains("snapshot_ref"));
    assert!(json.contains("refs/ccode/subagent-snapshots/sa-snap"));
    let parsed: SubagentMeta = serde_json::from_str(&json).unwrap();
    assert_eq!(
            parsed.snapshot_ref.as_deref(),
            Some("refs/ccode/subagent-snapshots/sa-snap")
        );
}
#[test]
...
fn snapshot_test_meta(id: &str) -> SubagentMeta {
    SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/ccode-wt/subagent-x".into()),
        snapshot_ref: None,
        effective_model_id: None,
    }
}
...
fn update_subagent_meta_snapshot_ref_persists_to_disk() {
    ...
    assert_eq!(
            ...
            Some("refs/ccode/subagents/sa-write")
        );
    assert_eq!(reread.status, "completed");
    assert_eq!(
            reread.worktree_path.as_deref(),
            Some("/tmp/ccode-wt/subagent-x")
        );
}
...
fn update_subagent_meta_snapshot_ref_reports_failure_when_meta_missing() {
    ...
    assert!(!update_subagent_meta_snapshot_ref(
            dir.path(),
            "refs/ccode/subagents/sa-missing",
            "completed"
        ));
}
...
fn snapshot_ref_write_promotes_nonterminal_status_to_terminal() {
    ...
    assert!(update_subagent_meta_snapshot_ref(
            dir.path(),
            "refs/ccode/subagents/x",
            "completed"
        ));
    let data = std::fs::read_to_string(dir.path().join("meta.json")).unwrap();
    let reread: SubagentMeta = serde_json::from_str(&data).unwrap();
    assert_eq!(
            Some("refs/ccode/subagents/x"),
            reread.snapshot_ref.as_deref()
        );
...
async fn completion_snapshot_sequence_persists_ref_then_removes_worktree() {
    ccode_test_utils::require_git!();
    use ccode_test_utils::git::{git_commit_all, init_git_repo};
    ...
    ccode_worktree::WorktreeBuilder::new(&repo, &wt)
        .standalone(true)
        .create()
        .unwrap();
    ...
    let ref_name = "refs/ccode/subagents/glue-1";
...
fn subagent_session_metadata_roundtrip() {
    ...
    assert_eq!(session_meta.model_id.as_deref(), Some("ccode-4.5"));
...
fn upload_lifecycle_spawn_then_completion_preserves_fields() {
    ...
    assert_eq!(spawn_gcs.model_id.as_deref(), Some("ccode-4.5"));
    ...
    assert_eq!(completion_gcs.model_id.as_deref(), Some("ccode-4.5"));
...
fn resume_source_worktree_reuse() {
    let source_with_worktree = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.ccode/worktrees/myrepo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
    ...
    assert_eq!(
            worktree.as_deref(),
            Some(Path::new(
                "/home/user/.ccode/worktrees/myrepo/subagent-sub-wt",
            )),
...
fn resume_inherited_cwd_requires_existing_non_worktree_dir() {
    ...
    let missing = ResumeSourceData {
        child_cwd: "/no/such/dir/ccode-missing".into(),
        ..present.clone()
    };
...
fn select_override_cwd_resume_never_falls_through_to_request_cwd() {
    let source = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.ccode/worktrees/repo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
...
fn token_estimation_for_window_safety() {
    ...
    let estimated = ccode_chat::estimate_conversation_tokens(&conversation);
    assert!(estimated > 0, "should produce non-zero estimate");
    assert!(
            estimated < 100,
            "short conversation should have small token estimate"
        );
    assert_eq!(ccode_chat::estimate_conversation_tokens(&[]), 0);
}
#[test]
fn token_estimation_accounts_for_images() {
    use ccode_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
    ...
    let text_tokens = ccode_chat::estimate_conversation_tokens(&text_only);
    ...
    let image_tokens = ccode_chat::estimate_conversation_tokens(&with_image);
    assert_eq!(
            image_tokens,
            text_tokens + 765,
...
fn durable_fallback_roundtrips_child_cwd_and_worktree() {
    ...
    let meta = SubagentMeta {
        ...
        worktree_path: Some("/tmp/ccode-wt/sa-dur".into()),
        snapshot_ref: None,
        effective_model_id: Some("ccode-3".into()),
    };
    ...
    assert_eq!(loaded.worktree_path.as_deref(), Some("/tmp/ccode-wt/sa-dur"));
...
fn durable_fallback_rejects_running_status() {
    let dir = std::env::temp_dir()
        .join("ccode-test-durable-status")
        .join(uuid::Uuid::now_v7().to_string());
...
fn drain_cancelled_finish_cmds(
    ...
    while let Ok(cmd) = cmd_rx.try_recv() {
        if let SessionCommand::CcodeSessionNotification { notification } = cmd
...
fn drain_cancelled_finish_broadcasts(
    ...
    while let Ok(msg) = gateway_rx.try_recv() {
        ...
        assert_eq!(args.request.method.as_ref(), "ccode.dev/session_notification");
...
async fn reconcile_reemits_shared_actor_terminal_outcome() {
    ...
    let finish = std::iter::from_fn(|| cmd_rx.try_recv().ok())
        .find_map(|command| {
            let SessionCommand::CcodeSessionNotification { notification } = command else {
                return None;
            };
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
            ccode_subagent::validate_resume_identity(
                "general-purpose",
                None,
                &source,
            )
...
fn durable_meta_roundtrips_effective_model_id() {
    let dir = std::env::temp_dir()
        .join("ccode-test-model-roundtrip")
        .join(uuid::Uuid::now_v7().to_string());
    let _ = std::fs::create_dir_all(&dir);
    let meta = SubagentMeta {
...
async fn read_parent_sampling_config_keeps_auto_catalog_id_with_routing_slug() {
    ...
    models.insert("auto".to_string(), test_model_entry("ccode-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "ccode-4.5", "composer-2-fast", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "ccode-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
#[tokio::test]
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    let mut models = indexmap::IndexMap::new();
    models.insert("ccode-4.5".to_string(), test_model_entry("ccode-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "ccode-4.5", "auto", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "ccode-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
...
async fn read_parent_sampling_config_resolves_backend_search_from_catalog() {
    let mut entry = test_model_entry("ccode-4.5");
    entry.info.supports_backend_search = true;
    let mut models = indexmap::IndexMap::new();
    models.insert("auto".to_string(), entry);
    let mut ctx = ctx_with_parent_chat_state("auto", "ccode-4.5", "auto", models);
...
async fn runtime_override_wins_over_subagents_models_pin_in_precedence_path() {
    use ccode_agent::config::ModelOverride;
    let build_ctx = || {
        let mut models = indexmap::IndexMap::new();
...
async fn resolve_subagent_inherits_parent_model_without_pins() {
    use ccode_agent::config::ModelOverride;
    for parent_model in ["ccode-4.5", "composer-2-fast", "my-custom-byok-model"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
...
/// (`ccode-4.5`) and a custom parent (`composer-2-fast`)
...
async fn resolve_subagent_config_override_pin_applies_for_any_parent() {
    use ccode_agent::config::ModelOverride;
    for parent_model in ["ccode-4.5", "composer-2-fast"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
        ctx.sampling_config.model = parent_model.to_string();
        ctx.model_id = acp::ModelId::new(parent_model);
        ctx.available_models
...
async fn resolve_subagent_agent_definition_pin_applies_for_light_parent() {
    use ccode_agent::config::ModelOverride;
    let mut ctx = ctx_with_toggle(HashMap::new());
    ctx.sampling_config.model = "ccode-4.5".to_string();
    ctx.model_id = acp::ModelId::new("ccode-4.5");
    ctx.available_models
        .insert("pinned-model".to_string(), test_model_entry("pinned-model"));
...
async fn resolve_subagent_config_override_unknown_model_falls_through_to_inherit() {
    ...
    let (config, model_id) = resolve_subagent_sampling_config(
        ...
        .await;
    assert_eq!(config.model, "ccode-4.5");
    assert_eq!(model_id.0.as_ref(), "ccode-4.5");
}
...
async fn subagent_override_provider_model_spawns_cache_only_credentials() {
    ...
    ctx.model_id = acp::ModelId::new("ccode-4.5");
    ctx.available_models = models;
    ctx.auth = Some(crate::auth::CcodeAuth {
...
fn non_cursor_persona_injected_as_system_reminder() {
    use ccode_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ccode_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ccode_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ccode_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ccode_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ccode_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ccode_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ccode_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(&result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ccode_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn make_test_skill(
    ...
    ccode_tools::implementations::skills::types::SkillInfo {
        ...
        scope: ccode_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
    ...
    local
        .run_until(async {
            ...
            let cmd = tokio::time::timeout(
                ...
                .expect("channel open");
            cancel.cancel();
            let SessionCommand::CcodeSessionNotification { notification } = cmd else {
                panic!("expected CcodeSessionNotification");
            };
