...
//! `ccode-tools` coordinator actor. This module keeps shell-specific
...
use ccode_agent::config::{McpInheritance, ModelOverride, PermissionMode};
use ccode_sampling_types::conversation::ConversationItem;
use ccode_subagent::ResumeSourceData;
use ccode_tools::implementations::ccode_build::task::coordinator::{
    ChildCompletion, ChildControl, ChildReporter, ChildRunOutput, LocalBoxFuture, StartedChild,
    SubagentProgress,
};
use ccode_tools::implementations::ccode_build::task::types::*;
use ccode_tools::types::tool::ToolKind;
use ccode_workspace::file_system::AsyncFileSystem;
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn ccode_tools::implementations::lsp::LspBackend>>,
    ...
    pub sampling_config: ccode_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::CcodeAuth>,
    pub parent_cwd: PathBuf,
    pub parent_session_id: String,
    /// The parent's cutoff at spawn, applied to the child's first turn. `None` if unset.
    pub inherited_tool_overrides: Option<ccode_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    ...
    pub parent_terminal_backend: Option<Arc<dyn ccode_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<ccode_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<ccode_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: ccode_tools::implementations::ccode_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: ccode_tools::implementations::ccode_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: ccode_tools::implementations::ccode_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<ccode_chat::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<ccode_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<ccode_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<ccode_hooks::discovery::HookRegistry>>,
    pub permission_handle: Option<ccode_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<ccode_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: ccode_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<ccode_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "ccode-build").
    ...
    pub parent_tool_definitions: Option<Vec<ccode_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<ccode_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: ccode_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: ccode_tools::types::compat::CompatConfig,
    /// Shared completion reservations held by auto-wake prompts.
    pub task_completion_reservations:
        Option<ccode_tools::reminders::task_completion::TaskCompletionReservations>,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut ccode_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("CCODE_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("CCODE_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &ccode_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (ccode_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &ccode_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (ccode_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &ccode_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &ccode_sampler::SamplerConfig,
) {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = ccode_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(ccode_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[ccode_sampling_types::conversation::ConversationItem],
) -> usize {
    conversation
        .iter()
...
struct InitialContext {
    ...
    conversation: Vec<ccode_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<ccode_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        ccode_subagent::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[ccode_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<ccode_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = ccode_chat::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        ccode_subagent::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
    ...
    if let Some(source) = resume_source {
        ...
        return match storage.copy_session_data_sync(
            ...
            Ok(result) => {
                ...
                let estimated_tokens = ccode_chat::estimate_conversation_tokens(&conversation);
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &ccode_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<ccode_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = ccode_subagent::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def = ccode_subagent::discover_agent_definition(
        subagent_type,
        &resolution_context,
    )?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    ccode_subagent::available_agent_names(
        &ccode_subagent::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<ccode_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = ccode_subagent::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match ccode_subagent::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(ccode_subagent::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(ccode_subagent::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(ccode_subagent::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            ccode_subagent::ResolutionError::PersonaResolution(_)
            | ccode_subagent::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match ccode_subagent::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(ccode_subagent::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            ccode_subagent::ResolutionError::Unknown { .. }
            | ccode_subagent::ResolutionError::PersonaResolution(_)
            | ccode_subagent::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    ccode_subagent::subagent_harness_flavor_is_representable(agent_type)
}
/// Apply the harness-dependent toolset/prompt re-selection to a resolved
/// agent definition.
///
/// The harness flavor (alternate vs ccode-build) normally follows the PARENT
/// agent: `CcodeBuildOrchestrator` parents give children
...
/// (so a ccode-build session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut ccode_agent::config::AgentDefinition,
) {
    let resolution_context = ccode_subagent::HarnessToolsetContext {
    ...
    ccode_subagent::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
...
fn summarize_tool_config(
    config: &ccode_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_permission_mode(
    requested: ccode_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> ccode_agent::config::PermissionMode {
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    ccode_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
fn cancellation_error_message(
    category: Option<ccode_fs::events::types::CancellationCategory>,
...
fn inject_subagent_completed_prompt(
    ...
    task_completion_reservations: &Option<
        ccode_tools::reminders::task_completion::TaskCompletionReservations,
    >,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
    ...
    let summary =
        ccode_tools::implementations::ccode_build::task::completion_summary(request, result);
    let message = ccode_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = ccode_tools::reminders::wrap_reminder(&message);
...
fn emit_subagent_notification(
    ...
    if let Some(cmd_tx) = parent_cmd_tx {
        let _ = cmd_tx.send(SessionCommand::CcodeSessionNotification {
...
/// `ccode.dev/subagent/list_running` on reconnect.
    ...
    tokio::task::spawn_local(async move {
        ...
        loop {
            ...
            if let Some(ref cmd_tx) = parent_cmd_tx {
                let _ = cmd_tx.send(SessionCommand::CcodeSessionNotification { notification });
            }
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("ccode.dev/session_notification", params.into());
...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    backend: &ccode_tools::implementations::ccode_build::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    gateway: &GatewaySender,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
