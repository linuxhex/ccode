...
use ccode_tools::implementations::{ccode_build, opencode};
pub(super) fn canonical_total_tokens(totals: &ccode_chat::UsageTotals) -> u64 {
    totals.total_tokens()
}
...
pub(super) async fn record_subagent_usage(
    ...
    by_model: Option<Vec<(String, ccode_chat::UsageTotals)>>,
...
/// `ccode-tools` coordinator actor and reached only through `reporter`.
#[tracing::instrument(
...
pub(crate) async fn run_shell_child(
    ...
    let mut effective_runtime = ccode_subagent::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = ccode_subagent::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != ccode_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != ccode_tool_types::SubagentIsolationMode::None {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(&source_cwd) {
            ...
            Err(e) => {
                ...
                std::env::temp_dir()
                    .join("ccode-subagent-worktrees")
                    .join(&request.id)
            }
        };
        let source_clone = source_cwd;
        let subagent_id = request.id.clone();
        let creation_mode: ccode_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
            let mut builder = ccode_worktree::WorktreeBuilder::new(&source_clone, &dest)
                .working_tree_mode(ccode_worktree::WorkingTreeMode::PreserveWorkingTree)
                .creation_mode(creation_mode)
                .worktree_kind(ccode_worktree::WorktreeKind::Subagent)
    ...
    effective_runtime.capability_mode = ccode_subagent::intersect_capability_modes(
    ...
    let allow_nested_subagents =
        child_depth < ccode_tools::implementations::ccode_build::task::MAX_SUBAGENT_DEPTH;
    ccode_subagent::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = ccode_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let parent_traceparent = ccode_fs::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = ccode_chat::estimate_conversation_tokens(&forked_conversation);
    ...
    let attribution_callback: Option<ccode_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = ccode_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<ccode_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                ccode_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == ccode_hooks::event::HookEventName::Stop {
                            s.event = ccode_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    ccode_agent::config::McpServerRef::Named(name) => {
                    ...
                    ccode_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            ccode_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        ccode_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
        ccode_chat::CompactionMode::Summary,
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            ccode_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        None,
        ...
        if inherit_skills {
            ...
            ccode_agent::prompt::skills::SkillsConfig::default()
        },
        if inherit_skills {
        ...
        ctx.yolo_mode
            || matches!(
                ...
                ccode_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        ccode_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        ccode_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        ccode_telemetry::events::Outcome::Cancelled
    } else {
        ccode_telemetry::events::Outcome::Error
    };
    ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/ccode/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
