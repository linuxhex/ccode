...
use crate::auth::{AuthManager, AuthMode, CcodeAuth, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (ccode.dev
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(ccode_fs::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
...
/// `ccode.dev/...` extension method, for injection into the agent's inbound ACP
/// stream by the leader's own watcher tasks (config hot-reload, skills).
///
/// The wire method is written **`_`-prefixed** (`_ccode.dev/internal/...`):
...
/// Start a skills file watcher and wire it to inject `ccode.dev/internal/reload_skills`
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = ccode_agent::prompt::workspace_user::optional_workspace_user_dir();
        ...
        while let Some(change) = skills_rx.recv().await {
            ...
            let (id, method) = match change {
                crate::config::watcher::DiscoveryChange::Skills if !created_discovery_dir => {
                    ...
                    ("skills-reload", "ccode.dev/internal/reload_skills")
                }
                ...
                crate::config::watcher::DiscoveryChange::Workflows => {
                    ...
                    ("workflows-reload", "ccode.dev/internal/reload_workflows")
                }
            };
...
/// ([`ccode_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    ccode_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    ccode_telemetry::unified_log::set_version(ccode_version::VERSION);
    ...
    ccode_fs::queue::cleanup_orphaned_uploads(
        &ccode_home::ccode_home(),
        ccode_fs::queue::DEFAULT_MAX_AGE,
    );

    // Log the client that launched us (set by ccode-desktop when spawning `ccode agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("CCODE_CLIENT_VERSION") {
        crate::unified_log::info(
            "CCODE_CLIENT_VERSION",
            None,
    ...
    // blocking stdin reads (see `ccode_acp::spawn_stdin_line_reader`): on
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `ccode agent stdio` is a local/interactive entrypoint (spawned by
            // ccode-desktop), so it needs the gate like the leader and pager paths;
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    ccode_telemetry::unified_log::set_version(ccode_version::VERSION);
    // `ccode agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `ccode agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a ccode.dev session. \
        Run `ccode login` to sign in, or use `ccode agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.ccode_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'ccode login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `ccode login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&ccode_home::ccode_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
        ...
        let auth_manager = Arc::new(AuthManager::new(&ccode_home::ccode_home(), ctx.clone()));
        if crate::agent::auth_method::has_ccode_api_key_env()
    ...
    // Capture the ccode build URL for the first-connection callback
    let ccode_code_url = format!("{}/build", ctx.ccode_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open Ccode Build: {} (press Enter to open in browser)",
                ccode_code_url
            );
            eprintln!();
            let url_for_open = ccode_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&CcodeAuth>, session: &CcodeAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's ccode.dev relay connection according to the start policy,
/// returning the slot where the [`RelayHandle`](crate::agent::relay::RelayHandle)
/// is parked once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `ccode agent leader`
...
///   duplication of mirroring every agent message to ccode.dev.
///
...
/// When a ccode.dev session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the ccode.dev relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: ccode_version::VERSION.to_string(),
    })
    ...
    let auth: Option<CcodeAuth> = crate::auth::try_ensure_session_noninteractive(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<CcodeAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    let auth_for_prefetch: Option<CcodeAuth> = auth.clone();
    let endpoints_for_prefetch = agent_config.endpoints.clone();
    ...
    local_set
        .run_until(async move {
            ...
            // Bridge websocket messages to agent (from ccode.dev relay)
            ...
            // Start (or arm) the ccode.dev relay. Eager by default — a bare
            // `ccode agent leader` (devbox / systemd) has no local IPC clients
            ...
            let relay_handle_slot = if let Some(relay_config) = relay_config {
                ...
                info!("Relay disabled: no ccode.dev session token (BYOK / local-only leader)");
                Rc::new(std::cell::RefCell::new(None))
            };
            ...
            let auth_scope = agent_config.ccode_com_config.auth_scope();
            // Gated on user_ccode_home() so a cwd-relative .ccode/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = ccode_config::user_ccode_home()
                    ...
                    match update {
                        ...
                        ConfigUpdate::ProjectMcpServersChanged { cwd } => {
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "ccode.dev/internal/reload_project_mcp_servers",
                        ...
                        ConfigUpdate::ModelsChanged => {
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "ccode.dev/internal/reload_models",
                                serde_json::json!({}),
                            );
                            let mut tx = acp_tx_for_config.lock().await;
                            if let Err(e) = tx.write_all(line.as_bytes()).await {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.ccode/models_cache.json
                            // (another ccode process fetched a fresher /v1/models
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "ccode.dev/internal/reload_models_cache",
                                serde_json::json!({}),
                            );
                            let mut tx = acp_tx_for_config.lock().await;
                            if let Err(e) = tx.write_all(line.as_bytes()).await {
                        ...
                        ConfigUpdate::Ui {
                            ...
                            let notification = serde_json::json!({
                                "jsonrpc": "2.0",
                                "method": "ccode.dev/config_changed",
                                "params": {
                                    "section": "ui",
                                    "changes": {
                                        "theme": theme,
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> CcodeAuth {
        CcodeAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::CCODE_OAUTH2_ISSUER.to_string()),
    ...
    /// constructor (`for_session`) with a relay-eligible ccode.dev OIDC session.
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::CcodeComConfig {
            ccode_ws_url: format!("ws://{addr}"),
            ccode_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
        crate::agent::relay::RelayConfig::for_session(&auth, &cfg, None, None)
            .expect("ccode.dev OIDC session must be relay-eligible")
    }
    ...
    /// `ccode agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the ccode.dev relay eagerly.
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `ccode agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
    ...
    fn internal_reload_request_line_uses_wire_ext_prefix() {
        ...
        assert_eq!(
            msg["method"], "_ccode.dev/internal/reload_models",
        ...
        let msg: serde_json::Value = serde_json::from_str(line.trim_end()).unwrap();
        assert_eq!(msg["method"], "_ccode.dev/internal/auth_cleared");
    }
