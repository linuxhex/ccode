...
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::CcodeAuth {
    crate::auth::CcodeAuth {
        key: key.into(),
        auth_mode: mode,
...
fn resolve_subscription_tier_prefers_display_then_api_key_then_jwt() {
    ...
    let api = auth_with_mode(crate::auth::AuthMode::ApiKey, "ccode-not-a-jwt");
    assert_eq!(
...
fn jwt_claim_matches_user_subscription_tier_rejects_stale_and_unknown() {
    ...
    assert!(!jwt_claim_matches_user_subscription_tier("free", "CcodePro"));
...
fn harness_pair(id: &str) -> Vec<ccode_sampling_types::conversation::ConversationItem> {
    use ccode_sampling_types::ToolCall;
    use ccode_sampling_types::conversation::ConversationItem;
    vec![
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_ccode_build() {
    let prev = std::env::var("CCODE_AGENT").ok();
    unsafe {
        std::env::remove_var("CCODE_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("CCODE_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return ccode-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = ccode_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `ccode-build-plan`, models in the catalog that still declare
/// `agent_type = "ccode-build"` explicitly must NOT preempt an ACP
/// profile. Any value in the `ccode-build*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_ccode_build_family() {
    let prev = std::env::var("CCODE_AGENT").ok();
    unsafe {
        std::env::remove_var("CCODE_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["ccode-build", "ccode-build-plan", "ccode-build-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for ccode-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("CCODE_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use ccode_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".ccode").join("agents");
    ...
    assert_eq!(
        def.model,
        ccode_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("CCODE_AGENT", v) },
        None => unsafe { std::env::remove_var("CCODE_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `ccode-build` (a client profile name) to
/// `ccode-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("ccode-build", "ccode-build-plan"));
    assert!(harnesses_are_compatible("ccode-build-plan", "ccode-build"));
    assert!(harnesses_are_compatible("ccode-build", "ccode-build"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("ccode-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "ccode-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_ccode_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "ccode-build-plan"),
        "ccode-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_ccode_build() {
    assert_eq!(
        resolve_required_agent_type(None, "ccode-build"),
        "ccode-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "ccode-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "ccode-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "ccode-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use ccode_tools::computer::local::{LocalFs, LocalTerminalBackend};
    use ccode_tools::notification::ToolNotificationHandle;
    use ccode_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = ccode_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: ccode_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: ccode_tools::implementations::ccode_build::image_gen::ImageGenConfig::default(),
        video_gen_config: ccode_tools::implementations::ccode_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: ccode_tools::implementations::ccode_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: ccode_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = ccode_hunk::HunkTrackerActor::spawn(
        "test".to_string(),
        std::path::PathBuf::from("/tmp"),
        hunk_event_tx,
        ccode_hunk::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        hunk_tracker_handle,
        chat_state_handle: ccode_chat::ChatStateHandle::noop(),
        ...
        permission_handle: ccode_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "ccode-build".to_string(),
...
async fn lookup_session_model_returns_per_session_model() {
    ...
    let sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("ccode-3-fast", false, None)),
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("ccode-3");
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("ccode-3", false, None)),
        (sid_b.clone(), make_test_handle("ccode-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use ccode_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use ccode_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("ccode-3", false, Some("ccode-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("ccode-3", false, Some("ccode-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("ccode-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("ccode-3", true, Some("ccode-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("ccode-3", true, Some("ccode-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("ccode-tui"), false);
...
fn parse_code_nav_capability_present_and_true() {
    ...
    meta.insert(
        "ccode.dev/codeNavigation".to_string(),
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("ccode-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("ccode-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::CcodeWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::CcodeAuth {
                key: "eligible".into(),
                auth_mode: crate::auth::AuthMode::WebLogin,
                ..crate::auth::CcodeAuth::test_default()
            });
            use acp::Agent as _;
            ...
            agent
                .ext_method(acp::ExtRequest::new(
                    "ccode.dev/internal/auth_cleared",
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    use crate::auth::{AuthManager, CcodeComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), CcodeComConfig::default()));
...
fn build_agent_with_auth(auth: crate::auth::CcodeAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, CcodeComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let ccode_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("CCODE_HOME", ccode_home.path());
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), CcodeComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("ccode-3", false, None);
...
/// Drain the agent gateway, returning the first `ccode.dev/sessions/changed`
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<ccode_acp::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let ccode_acp::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
/// an `ccode.dev/sessions/changed` upsert carrying the *overridden* activity, so
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent
        .sessions
        .borrow_mut()
        .insert(sid.clone(), make_test_handle("ccode-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("ccode-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("ccode-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("ccode-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("ccode-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("ccode-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::CcodeWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::CcodeTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::CcodeWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
fn bg_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":1,"method":"_ccode.dev/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_backgrounded","task_id":"{task_id}","command":"sleep 99","cwd":"/tmp"}}}}}}"#
    )
}
fn completed_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":2,"method":"_ccode.dev/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_completed","task_snapshot":{{"task_id":"{task_id}","completed":true}}}}}}}}"#
    )
}
...
fn orphaned_tasks_ignores_unrelated_updates() {
    let tmp = tempfile::tempdir().unwrap();
    let bg = bg_line("t1");
    let unrelated = r#"{"timestamp":1,"method":"_ccode.dev/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"auto_compact_started","percentage":80}}}"#;
...
fn orphaned_tasks_filters_rewind_dead_branches() {
    ...
    let rewind = r#"{"timestamp":3,"method":"_ccode.dev/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":0,"created_at":"2025-01-01T00:00:00Z"}}}"#;
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::CCODE_COM_METHOD_ID,
...
/// BYOK guard. Users with `ccode.api_key` must continue to report `ApiKey`
...
#[tokio::test(flavor = "current_thread")]
async fn auth_type_ccode_api_key_no_current_returns_api_key() {
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::CCODE_API_KEY_METHOD_ID,
    ));
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(
        agent.auth_type(),
        ccode_chat::AuthType::ApiKey,
        "ccode.api_key auth must report ApiKey -- BYOK has no session-token \
...
async fn auth_type_session_based_with_current_returns_session_token() {
    use crate::auth::CcodeAuth;
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::OIDC_METHOD_ID,
    ));
    agent.auth_manager.hot_swap(CcodeAuth::test_default());
    assert!(agent.auth_manager.current().is_some());
    assert_eq!(agent.auth_type(), ccode_chat::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), ccode_chat::AuthType::ApiKey,);
}
...
/// Minimal agent whose `ccode_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.ccode_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
/// Deployment-key / managed-config user: `CCODE_API_KEY` resolves and the kill
/// switch is off, so a dead `cached_token` MUST fall through to `ccode.api_key`
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    use crate::agent::auth_method::{CCODE_API_KEY_ENV_VAR, CCODE_API_KEY_METHOD_ID};
    use ccode_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("CCODE_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(CCODE_API_KEY_ENV_VAR, "test-deployment-key");
    let agent = build_minimal_agent_for_tests();
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(CCODE_API_KEY_METHOD_ID),
        "deployment-key user (CCODE_API_KEY set, no kill switch) must fall \
         through to ccode.api_key on a dead cached_token -- not interactive login",
    );
}
/// Forced-IdP deployment: even with `CCODE_API_KEY` present, the admin kill
/// switch keeps the fallthrough on interactive `ccode.dev` (api-key auth is
/// neither advertised nor an eligible fallthrough).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{CCODE_COM_METHOD_ID, CCODE_API_KEY_ENV_VAR};
    use ccode_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("CCODE_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(CCODE_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(CCODE_COM_METHOD_ID),
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive ccode.dev so CCODE_API_KEY can't bypass forced IdP login",
    );
}
/// No advertiseable credentials at all (no env key, no kill switch): the user
/// genuinely needs to log in, so the fallthrough is interactive `ccode.dev`.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_ccode_com_without_credentials() {
    use crate::agent::auth_method::{
        CCODE_COM_METHOD_ID, LEGACY_CCODE_API_KEY_ENV_VAR, CCODE_API_KEY_ENV_VAR,
    };
    use ccode_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("CCODE_DISABLE_API_KEY_AUTH");
    let _new = EnvGuard::unset(CCODE_API_KEY_ENV_VAR);
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(CCODE_COM_METHOD_ID),
        "no API-key creds and no kill switch -> interactive ccode.dev login",
    );
}
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use ccode_tools::implementations::ccode_build::video_gen::{
...
async fn prepare_video_gen_config_respects_feature_flag() {
    use ccode_tools::implementations::ccode_build::video_gen::VideoGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
    assert!(matches!(
        agent.prepare_video_gen_config(),
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use ccode_tools::implementations::ccode_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
/// Build traffic via the `x-ccode-client-identifier` header. If this header is
...
/// Regression: `ccode.dev/auth/info` must return profile fields even when the
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::CcodeAuth::test_default());
    assert!(
...
fn parse_session_kind_matrix() {
    ...
    let cases: &[(&str, serde_json::Value, SessionKind)] = &[
        (
            "chat",
            json!({"ccode.dev/session": {"kind": "chat"}}),
            SessionKind::Chat,
        ),
        (
            "build",
            json!({"ccode.dev/session": {"kind": "build"}}),
            SessionKind::Build,
        ),
        (
            "chat_malformed_sibling",
            json!({"ccode.dev/session": {"kind": "chat", "facets": "not-a-map"}}),
            SessionKind::Chat,
        ),
        (
            "unknown_kind",
            json!({"ccode.dev/session": {"kind": "frob"}}),
            SessionKind::Build,
        ),
...
fn chat_initial_model_matrix() {
    let cases: &[(&str, bool, Option<&str>, Option<&str>)] = &[
        ("chat_with_model", true, Some("ccode-4.5"), Some("ccode-4.5")),
        ("chat_without_model", true, None, None),
        ("build_with_model", false, Some("ccode-4.5"), None),
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = ccode_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(ccode_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<ccode_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::CCODE_DISABLE_CUSTOM_BRIDGE_ENV);
    run_local_for_bridge_test(|| async {
        let agent = build_minimal_agent_for_tests();
        let params = serde_json::json!({ "sessionId": "sess-local" });
        let err = agent
            .ext_method(acp::ExtRequest::new(
                "ccode.dev/rewind/points",
                std::sync::Arc::from(serde_json::value::to_raw_value(&params).unwrap()),
            ))
            .await
...
/// `test_cancel_ends_in_flight_turn_and_frees_slot` (ccode-agent-sdk).
#[test]
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("ccode-tui"));
...
/// Drive `ccode.dev/internal/evict_sessions` through the real `ext_notification`
...
/// `ccode.dev/internal/evict_sessions` notification — the realistic shape of a
...
async fn drive_disconnect_many(agent: &MvpAgent, sids: &[&acp::SessionId]) {
    ...
    agent
        .ext_notification(acp::ExtNotification::new(
            "ccode.dev/internal/evict_sessions",
            raw.into(),
        ))
        .await
        .expect("evict_sessions notification must be handled");
}
/// Drive `ccode.dev/session/close` through the real `ext_method` dispatch
...
/// Mixed batch in a *single* `ccode.dev/internal/evict_sessions` notification —
...
/// `ccode.dev/session/close` dispatch (`ext_method` → `handle_session_close`),
...
/// `ccode.dev/folder_trust/request` round-trip.
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<ccode_acp::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, CcodeComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".ccode/roles");
    let personas = cwd.join(".ccode/personas");
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                ccode_subagent::config::SubagentRole {
...
/// End-to-end gate wiring: project `.ccode/roles` / `personas` alone must drive
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _env = EnvGuard::set("CCODE_HOME", home.path());
    let _sim = EnvGuard::set(ccode_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
    let repo = tempfile::tempdir().unwrap();
    git2::Repository::init(repo.path()).unwrap();
    write_project_subagent_definitions(repo.path());
    run_local_for_bridge_test(|| async {
...
/// Pull the next `ccode.dev/folder_trust/request` reverse-request off the gateway and
/// answer it with `outcome`. Returns the request's decoded params.
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<ccode_acp::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let ccode_acp::AcpClientMessage::ExtMethod(args) = msg else {
        panic!("expected an ext_method reverse-request, got a different message");
    };
    assert_eq!(args.request.method.as_ref(), "ccode.dev/folder_trust/request");
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use ccode_test_support::EnvGuard;
    use ccode_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("CCODE_HOME", home.path());
    let _sim = EnvGuard::set(ccode_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, ccode_acp::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(ccode_acp::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use ccode_test_support::EnvGuard;
    use ccode_hooks_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("CCODE_HOME", home.path());
    let _sim = EnvGuard::set(ccode_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(ccode_acp::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> ccode_announcements::RemoteAnnouncement {
    ccode_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<ccode_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = ccode_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = ccode_announcements::RemoteAnnouncement {
...
async fn emit_announcements_gate_emits_updates_baseline_and_bumps_gen() {
    ...
    let recv_gen =
        |rx: &mut tokio::sync::mpsc::UnboundedReceiver<ccode_acp::AcpClientMessage>| {
            ...
            let ccode_acp::AcpClientMessage::ExtNotification(args) = msg else {
                panic!("expected ExtNotification, got another message kind");
            };
            assert_eq!(args.request.method.as_ref(), "ccode.dev/announcements/update");
...
mod direct_hub_cloud_removed {
    ...
    fn cloud_server_id_meta_is_hard_error() {
        let meta = serde_json::json!({ "ccode.dev/cloud_server_id": "srv-123" });
    ...
    fn cloud_server_id_null_still_present_is_hard_error() {
        let meta = serde_json::json!({ "ccode.dev/cloud_server_id": null });
        let err = reject_direct_hub_cloud_meta(meta.as_object()).expect_err("must reject");
        assert_direct_hub_error(err);
    }
    #[test]
    fn cloud_server_id_with_gateway_meta_still_hard_error() {
        let meta = serde_json::json!({
            "ccode.dev/cloud_server_id": "srv-legacy",
            "envId": "env-1",
            "ccode.dev/cloud_existing_workspace": {
                "server_id": "ws-1",
                "cwd": "/workspace"
            }
        });
    ...
    async fn emit_settings_update_carries_permission_mode_from_cfg() {
        ...
        local
            .run_until(async {
                ...
                assert_eq!(args.request.method.as_ref(), "ccode.dev/settings/update");
