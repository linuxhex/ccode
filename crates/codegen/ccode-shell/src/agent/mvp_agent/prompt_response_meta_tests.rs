...
fn includes_baseline_keys_without_usage() {
    let meta = build_prompt_response_meta(args("sess-1", "prompt-1", 42_000, "ccode-4.5"));
    ...
    assert_eq!(meta["modelId"], "ccode-4.5");
...
fn enriches_meta_with_camelcase_token_keys() {
    ...
    let meta = build_prompt_response_meta(PromptResponseMetaArgs {
        ...
        ..args("sess-1", "prompt-1", 1_700, "ccode-4.5")
    });
...
fn tool_overrides_land_as_camelcase_meta_key() {
    let overrides = ccode_sampling_types::ToolOverrides {
