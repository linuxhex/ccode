...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use ccode_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = ccode_pathss::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                ccode_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    ccode_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                ccode_workspace::permission::AccessKind::Read(Some(".env".into())),
                ccode_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        ccode_workspace::permission::Decision::PolicyDeny(_)
                    ),
...
async fn subagent_spawn_context_inherits_parent_configured_cutoff() {
    ...
    let cutoff = ccode_sampling_types::ToolOverrides {
        x_search: Some(ccode_sampling_types::XSearchOptions {
            date_bound: Some(
                ccode_sampling_types::SearchDateBound::new(None, Some("2020-01-01".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
