...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&ccode_sampling_types::ToolOverrides>,
) {
...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        ccode_telemetry::unified_log::info("agent initialized", None, None);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let opts = ccode_worktree::AutoGcOptions::from_resolved(auto_gc_policy);
            if let Err(e) = ccode_worktree::WorktreeDb::open_default()
                .and_then(|db| ccode_worktree::maybe_auto_gc(&db, &opts))
            {
        ...
        {
            let root = crate::util::ccode_home::ccode_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    ccode_workspace::permission::cleanup_stale_permission_state(
        ...
        ccode_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_ccode_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("ccode-web") => client_type = ClientType::CcodeWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("ccode-code-extension") => client_type = ClientType::Extension,
                Some("ccode-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        *self.client_type.borrow_mut() = client_type;
        ...
        tracing::info!(
            ...
            "code-nav capability initialized from initialize request; \
             index will start lazily on first ccode.dev/code/* request if eligible"
        );
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        if !self.cfg.borrow().ccode_com_config.api_key_auth_disabled()
            && auth_method::read_ccode_api_key_env().is_err()
            && let Some(api_key) = crate::auth::read_api_key(
                &crate::util::ccode_home::ccode_home(),
            )
        {
            unsafe { std::env::set_var("CCODE_API_KEY", &api_key) };
            tracing::info!("auth: loaded API key from auth.json (ccode::api_key scope)");
            ccode_telemetry::unified_log::info(
                "auth: loaded API key from auth.json (ccode::api_key scope)",
                None,
                None,
            );
        }
        ...
        let has_external_api_key = auth_method::should_advertise_ccode_api_key(
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.ccode_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.ccode_com_config.auth_provider_label.clone(),
                cfg.ccode_com_config.auth_provider_command.is_some(),
                cfg.ccode_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                label = ?login_label,
                has_auth_provider,
                "auth: advertising ccode.dev auth method",
            );
        }
        let preferred_method = self.cfg.borrow().ccode_com_config.preferred_method;
        ...
        ccode_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!({
                "ccode_home": crate::util::ccode_home::ccode_home().display().to_string(),
        ...
        debug_assert!(
            !has_external_api_key
                || matches!(
                    ...
                    Some(auth_method::AuthMethodKind::CcodeApiKey)
                ),
            "BYOK invariant violated: ccode.api_key MUST be auth_methods.first() \
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("CCODE_AGENT_METADATA");
                    serde_json::json!({
                    "ccodeShell": true,
                    ...
                    // channel (`ccode.dev/mcp/sdk_call`); the SDK reads this to enable transport="acp".
                    (ccode_mcp::wire::MCP_SDK): true,
                    // `session/new` / `session/load` accept per-session plugin roots in
                    // `_meta.pluginDirs`; the SDKs gate `CcodeOptions.plugins` on this.
                    (SESSION_PLUGIN_DIRS_CAPABILITY_KEY): true,
                    "currentWorkingDirectory": current_working_directory.to_string_lossy().to_string(),
                    "agentVersion": ccode_version::VERSION,
                    "agentId": agent_id(),
                    ...
                    // disabled feature produces zero `ccode.dev/recap` traffic.
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().ccode_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::CCODE_API_KEY_METHOD_ID => {
                if self.cfg.borrow().ccode_com_config.api_key_auth_disabled() {
                ...
                if sampling_config.api_key.is_none() {
                    if let Ok(api_key) = auth_method::read_ccode_api_key_env() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Set CCODE_API_KEY or add api_key/env_key to config.toml.",
                                ),
                        );
                    }
                }
                ...
                log_event(ccode_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().ccode_com_config.preferred_method,
            ...
            auth_method::CCODE_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let ccode_ctx = self.auth_manager.ccode_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_ccode_oauth(cli_oauth);
                    ...
                    })?;
                {
                    ...
                    tracing::debug!("auth: ccode.dev/oidc handler set api_key (SessionToken)");
                    ccode_telemetry::unified_log::debug(
                        "auth: ccode.dev/oidc handler set api_key (SessionToken)",
                        None,
                        None,
                    );
                }
    ...
    async fn new_session(
        ...
        ccode_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || ccode_telemetry::external::is_active() {
            ...
            let perm = if session_yolo_mode {
                ccode_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                ccode_telemetry::enums::PermissionMode::Auto
            } else {
                ccode_telemetry::enums::PermissionMode::Ask
            };
            tokio::spawn(async move {
                let git = ccode_telemetry::context::collect_git_context(&cwd_str);
                let ev = ccode_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                ccode_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match ccode_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
        ...
        if let Some(obj) = meta.as_object_mut() {
            obj.insert("ccode.dev/sessionConfig".to_string(), session_config_value);
            obj.insert("ccode.dev/sessionDetail".to_string(), session_detail_value);
    ...
    async fn load_session(
        ...
        let persist_data = arguments
            .meta
            .as_ref()
            .and_then(|m| m.get("ccode.dev/persist"))
            .cloned();
        ...
        let git_root = ccode_workspace::session::git::find_git_root_from_path(
                cwd.as_path(),
            )
            .ok();
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        ccode_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        ccode_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        ccode_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => ccode_telemetry::events::PlanModeState::Inactive,
        };
        ...
        let restore_code_requested = request_meta
            .as_ref()
            .and_then(|m| m.get("ccode.dev/restore_code"))
            .and_then(|v| v.as_bool())
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            ccode_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = ccode_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: ccode_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
                supplied_cwd = %cwd.as_str(),
                persisted_cwd = %summary.info.cwd,
                target_sha = %target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a ccode worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            ccode_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use ccode_workspace::session::git::RestoreKind;
            let outcome = ccode_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let load_envrc = {
            let skip_envrc = request_meta
                .as_ref()
                .and_then(|m| m.get("ccode.dev/skip_envrc"))
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            if skip_envrc {
                false
        ...
        let preloaded_envrc = ccode_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let prompt_display_cwd = request_meta
            .as_ref()
            .and_then(|m| m.get("ccode.dev/display_cwd"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
        ...
        if let Some((parent_cmd_tx, session_cwd)) = orphan_parent {
            ...
            crate::agent::subagent::reconcile_orphaned_subagents_with_backend(
                    ...
                    &ccode_tools::implementations::ccode_build::task::backend::ChannelBackend::new(
        ...
        let is_ccode_build = persisted_model.0.starts_with("ccode-build");
        let same_family_fallback = if is_ccode_build {
            available.keys().find(|id| id.0.starts_with("ccode-build")).cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("ccode-build")).cloned()
        };
        ...
        if let Some(persist) = persist_data {
            response_meta_map.insert("ccode.dev/persist".to_string(), persist);
        }
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    ccode_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = ccode_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = ccode_workspace::session::git::detect_head_divergence(
            ...
            .and_then(|h| h.current_prompt_id.lock().ok().and_then(|g| g.clone()))
        {
            response_meta_map
                .insert(
                    "ccode.dev/runningPromptId".to_string(),
        ...
        response_meta_map.insert("ccode.dev/sessionConfig".to_string(), session_config_value);
        response_meta_map.insert("ccode.dev/sessionDetail".to_string(), session_detail_value);
        ...
        if self.product_analytics_enabled() {
            log_event(ccode_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            ccode_fs::trace_context::link_current_span_to_meta(
        ...
        if let Some(ctx) = trace_context.clone() {
            ...
            let mut prompt_metadata = PromptMetadata {
                ...
                shell_version: Some(ccode_version::VERSION.to_string()),
                workspace_type: None,
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match ccode_sampling_types::ToolOverridesUpdate::parse(value) {
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: ccode_fs::trace_context::current_traceparent(),
                json_schema,
                send_now,
                admission: None,
            ...
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new(
                        "ccode.dev/session/prompt_complete",
                        params.into(),
                    ),
                );
        }
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<ccode_chat::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = ccode_tty::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = ccode_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = ccode_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            ccode_fs::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            "ccode.dev/getApiKey" | "ccode.dev/setApiKey" => {
                crate::extensions::auth::handle(self, &args).await
            }
            "ccode.dev/session/info" | "ccode.dev/session/close" | "ccode.dev/session/list"
            | "ccode.dev/sessions/list" => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            "ccode.dev/workspaces/list" => {
                crate::agent::handlers::workspaces::handle(self, &args).await
            }
            "ccode.dev/session/updates" => {
                crate::extensions::session_updates::handle(&args, &self.gateway).await
            }
            "ccode.dev/session/state" => {
                crate::extensions::session_state::handle_state(&args).await
            }
            "ccode.dev/session/import" => {
                crate::extensions::session_state::handle_import(&args).await
            }
            "ccode.dev/session/load_history" => {
                crate::extensions::chat_conversation_history::handle(self, &args).await
            }
            "ccode.dev/session/search" => {
                crate::extensions::session_search::handle(&args).await
            }
            "ccode.dev/session/resolve_local_for_worktree_resume"
            | "ccode.dev/session/rehydrate" => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            "ccode.dev/session/rename" | "ccode.dev/session/delete"
            | "ccode.dev/session/update_mcp_servers" | "ccode.dev/session/fork"
            | "ccode.dev/internal/reload_all_mcp_servers"
            | "ccode.dev/internal/reload_project_mcp_servers" | "ccode.dev/internal/reload_skills"
            | "ccode.dev/internal/reload_workflows" | "ccode.dev/internal/reload_models"
            | "ccode.dev/internal/reload_models_cache" | "ccode.dev/internal/auth_cleared"
            | "ccode.dev/plugins/reload" | "ccode.dev/commands/list" => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            "ccode.dev/session/repair" => crate::extensions::repair::handle(self, &args).await,
            "ccode.dev/session/usage" => crate::extensions::usage::handle(self, &args).await,
            "ccode.dev/memory/flush" | "ccode.dev/memory/rewrite" => {
                crate::extensions::memory::handle(self, &args).await
            }
            "ccode.dev/skills/refresh-baseline" => {
                self.refresh_skill_baseline_for_all_sessions();
                crate::extensions::to_ext_response(
                    Ok(serde_json::json!({"ok": true})),
                )
            }
            "ccode.dev/interject" => crate::extensions::interject::handle(self, &args).await,
            "ccode.dev/feedback" | "ccode.dev/feedback/dismiss" | "ccode.dev/btw" => {
                crate::extensions::feedback::handle(self, &args).await
            }
            "ccode.dev/recap" => crate::extensions::recap::handle(self, &args).await,
            "ccode.dev/cloud/terminate" => {
                crate::extensions::auth_gate::require_ccode_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `ccode login` to authenticate.",
                )?;
            ...
            "ccode.dev/billing" => crate::extensions::billing::handle(self, &args).await,
            "ccode.dev/auto-topup-rule" => {
                crate::extensions::billing::handle(self, &args).await
            }
            "ccode.dev/share_session" => crate::extensions::share::handle(self, &args).await,
            "ccode.dev/privacy/setCodingDataRetention" => {
                crate::extensions::privacy::handle(self, &args).await
            }
            "ccode.dev/rollout/survey" => {
                crate::extensions::rollout::handle(self, &args).await
            }
            "ccode.dev/prompt_history" => {
                crate::extensions::prompt_history::handle(self, &args).await
            }
            "ccode.dev/suggest" => crate::extensions::suggest::handle(self, &args).await,
            "ccode.dev/suggestPrompt" => crate::extensions::suggest::handle(self, &args).await,
            s if s.starts_with("ccode.dev/auth/") => {
                crate::extensions::auth::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/session_summaries/") => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/git/worktree/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            s if s.starts_with("ccode.dev/git/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::git::handle(self, &ops, &args).await
            }
            s if s.starts_with("ccode.dev/compact_conversation") => {
                crate::extensions::memory::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/plugins/") => {
                crate::extensions::plugins::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/marketplace/") => {
                crate::extensions::marketplace::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/hooks/") => {
                crate::extensions::hooks::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/hunk-tracker/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::hunk_tracker::handle(self, &ops, &args).await
            }
            s if s.starts_with("ccode.dev/pr/") => {
            ...
            s if s.starts_with("ccode.dev/search/") => {
                crate::extensions::search::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/bundle/") => {
                crate::extensions::bundle::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/code/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::code_nav::handle(self, &ops, &args).await
            }
            s if s.starts_with("ccode.dev/skills/") || s == "ccode.dev/workflows/list" => {
            ...
            s if s.starts_with("ccode.dev/review") => {
                crate::extensions::feedback::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/debug/") => {
                crate::extensions::debug::handle(self, &args).await
            }
            s if s.starts_with("ccode.dev/rewind") => {
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "ccode.dev/yolo_mode_changed"
        ...
        if args.method.as_ref() == "ccode.dev/permissions/reset" {
            let sessions = self.sessions.borrow();
        ...
        if args.method.as_ref() == "ccode.dev/internal/evict_sessions" {
            self.handle_evict_sessions(&args.params).await;
        }
        if args.method.as_ref() == "ccode.dev/toggle_plan_mode"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if matches!(
            args.method.as_ref(),
            "ccode.dev/queue/remove"
                | "ccode.dev/queue/reorder"
                | "ccode.dev/queue/clear"
                | "ccode.dev/queue/edit"
                | "ccode.dev/queue/interject"
        )
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        ...
        if args.method.as_ref() == "ccode.dev/terminal/pty/input"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref() == "_ccode.dev/session/update" {
            if let Ok(notification) = serde_json::from_str::<
                SessionNotification,
            >(args.params.get()) {
                tracing::info!(
                    "Storing ccode session notification: session_id={}",
                    ...
                    .get(&notification.session_id)
                {
                    let _ = handle
                        .cmd_tx
                        .send(crate::session::SessionCommand::CcodeSessionNotification {
                            notification,
                        });
                } else {
                    tracing::warn!(
                        "Received ccode session notification for unknown session: {}",
                        notification.session_id.0
                    );
                }
            } else {
                tracing::warn!("Failed to parse ccode session notification params");
            }
        }
        if args.method.as_ref() == "ccode.dev/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "ccode.dev/telemetry/multi_agent_followup" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| ccode_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "ccode.dev/telemetry/multi_agent_apply" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "ccode.dev/telemetry/multi_agent_discard" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| ccode_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == ccode_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                ccode_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            ccode_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
