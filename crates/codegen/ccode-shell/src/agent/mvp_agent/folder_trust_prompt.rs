...
//! (`ccode.dev/folder_trust/request`) that asks a GUI client (ccode-desktop) to decide
...
//! `ccode.dev/folderTrust.interactive` AND the folder-trust feature flag is on AND the
//! verdict is [`ccode_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.ccode/lsp.json` servers start on the NEXT session open (the durable grant
...
/// ACP `ccode.dev/folder_trust/request` payload (agent → GUI client). Serialized as
...
/// ACP `ccode.dev/folder_trust/request` response (GUI client → agent).
...
impl MvpAgent {
    /// Parse the `ccode.dev/folderTrust.interactive` capability from an initialize
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("ccode.dev/folderTrust"))
    ...
    /// `ccode.dev/folderTrust.interactive` AND [`folder_trust::prompt_warranted`]
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = ccode_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                ccode_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("ccode.dev/folder_trust/request", raw_params.into());
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a ccode_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a ccode_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.ccode/hooks`, `.cursor/hooks.json`),
...
mod tests {
    ...
    fn parse_interactive_trust_capability_present_and_true() {
        ...
        meta.insert(
            "ccode.dev/folderTrust".to_string(),
