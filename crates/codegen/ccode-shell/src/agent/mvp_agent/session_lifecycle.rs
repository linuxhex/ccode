...
impl MvpAgent {
    ...
    /// session end — a terminal/explicit close (`ccode.dev/session/close`). It must
    ...
    pub(crate) fn remove_session(&self, id: &acp::SessionId) {
        ...
        let _ = self
            .subagent_event_tx
            .send(ccode_tools::implementations::ccode_build::task::types::SubagentEvent::DiscardSessionCompletions {
    ...
    /// (`ccode.dev/session/close`). Finalizes the cloud replica (genuine session
    ...
    /// `ccode.dev/sessions/changed` notification with the session in `removed` so
    ...
    /// `ccode.dev/sessions/changed` notification with the current entry in
    ...
    /// Emit an `ccode.dev/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `ccode.dev/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `ccode.dev/sessions/changed` special-case in `leader/server.rs`).
...
/// Field names are the wire contract of `ccode.dev/debug/agent`'s `registries`
