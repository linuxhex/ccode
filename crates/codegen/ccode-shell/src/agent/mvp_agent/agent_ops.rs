...
use super::*;
use ccode_tools::implementations::ccode_build::task::backend::SubagentBackend;
...
impl MvpAgent {
    ...
    fn has_proxy_credentials(&self) -> bool {
        self.cfg.borrow().endpoints.deployment_key.is_some()
            || self.auth_manager.current_or_expired().is_some_and(|a| a.is_ccode_auth())
    }
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::CcodeAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
    ...
    /// Requires feature flag AND ccode authentication (OIDC or legacy WebLogin).
    ...
    /// must not block the ACP response (ccode-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled ccode-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_ccode_auth() || a.auth_mode == crate::auth::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            let (user_id, team_id) = if auth.is_ccode_auth() {
                (Some(auth.user_id), auth.team_id)
            } else {
                (None, auth.team_id)
            };
            ccode_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                ccode_version::VERSION.to_owned(),
                subscription_tier,
    ...
    ///
    /// Used by the `ccode.dev/commands/list` ext method and the
    ...
    /// Return the current CcodeAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::CcodeAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &ccode_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// discovery on ccode-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<ccode_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    ccode_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    ccode_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match ccode_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => ccode_workspace::WorkspaceOps::local(handle),
            Err(e) => {
                tracing::error!(
                    error = %e,
    ...
    ///     `ccode.dev`, `oidc`) -- even if the in-memory token is currently
    ///     expired or missing.
    ///
    /// Returns `ApiKey` only when the auth method is BYOK (`ccode.api_key`) or
    ...
    pub(crate) fn auth_type(&self) -> ccode_chat::AuthType {
    ...
    /// When `cached_token` cannot proceed, prefer non-interactive `ccode.api_key`
    /// iff `should_advertise_ccode_api_key`; otherwise `ccode.dev`. Returns `None`
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let preferred = self.cfg.borrow().ccode_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_ccode_api_key(
                self.cfg.borrow().ccode_com_config.api_key_auth_disabled(),
    ...
    /// `use_oauth` only when the target is interactive `ccode.dev`. When
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let meta = if method_id.0.as_ref() == auth_method::CCODE_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::CcodeAuth) {
        if !crate::util::config::resolve_remote_fetch_enabled() {
            tracing::debug!("post-auth settings refresh skipped: remote_fetch disabled");
            return;
        }
        let is_ccode = auth.is_ccode_auth();
        ...
        let (
            ...
            let ccode_user_id = is_ccode.then(|| user_id.clone());
            let ccode_team_id = is_ccode.then(|| team_id.clone()).flatten();
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<ccode_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    /// funnels through here. Emits `ccode.dev/announcements/update` and advances
    ...
    pub(super) fn emit_announcements(&self, mode: AnnouncementsPushMode) {
        ...
        let accepted = self
            ...
            .forward_fire_and_forget(
                acp::ExtNotification::new("ccode.dev/announcements/update", params.into()),
            );
        if !accepted {
            return;
        }
    ...
    /// Next generation for an `ccode.dev/announcements/update` push. Strictly
    ...
    pub(super) async fn send_model_auto_switched(
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&notification) {
            let _ = self
                .gateway
                .ext_notification(
                    acp::ExtNotification::new("ccode.dev/session_notification", params.into()),
                )
                .await;
        }
    }
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if matches!(preferred, Some(crate::auth::PreferredAuthMethod::Oidc))
            ...
            && credentials.auth_type == ccode_chat::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = ccode_chat::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == ccode_chat::AuthType::ApiKey
    ...
    /// Whether the current session is a personal ccode.dev account on a gated
    ...
    /// Premium upsell prose (see `ImageGenConfig`/`VideoGenConfig`'s
    /// `tier_restricted`).
    ///
    ...
    fn is_tier_restricted_capability(&self) -> bool {
        ...
        if !auth.is_ccode_auth() || auth.team_id.is_some() {
            return false;
        }
        let tier = self
            .cfg
    ...
    ///
    /// Both BYOK and session (OAuth) users go direct to `ccode_api_base_url`.
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> ccode_tools::implementations::ccode_build::image_gen::ImageGenConfig {
        ...
        let cfg = self.cfg.borrow();
        let base_url = cfg.endpoints.ccode_api_base_url.clone();
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| ccode_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("ccode-ccode-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        &self,
    ) -> ccode_tools::implementations::ccode_build::deploy_app::AppBuilderDeployerConfig {
        use ccode_tools::implementations::ccode_build::deploy_app::AppBuilderDeployerConfig;
        AppBuilderDeployerConfig::Disabled
    }
    /// Build video generation config. Video tools call the ccode API directly.
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> ccode_tools::implementations::ccode_build::video_gen::VideoGenConfig {
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `CCODE_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `CCODE_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    /// - `allow_local`: `[toolset.web_fetch] allow_local` > `CCODE_WEB_FETCH_ALLOW_LOCAL` > false
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> ccode_tools::implementations::ccode_build::web_fetch::WebFetchConfig {
        use ccode_tools::implementations::ccode_build::web_fetch::WebFetchConfig;
        let cfg = self.cfg.borrow();
    ...
    pub fn with_models(
        ...
        if !cfg.ccode_com_config.api_key_auth_disabled() {
        ...
        let has_ccode_auth = auth_manager
            .current_or_expired()
            .is_some_and(|a| a.is_ccode_auth());
        let relay_sync_enabled = tui_mode && relay_config_enabled && has_ccode_auth;
        ...
        if relay_sync_enabled {
            tracing::info!("[ccode] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_ccode_auth {
            tracing::info!("[ccode] Relay sync: DISABLED (no auth - run 'ccode login' first)");
        ...
        let instance = Self {
            ...
            plugin_registry_handle: ccode_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: ccode_tools::implementations::ccode_build::task::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().ccode_com_config.auth_provider_command.clone(),
    ...
    /// Handle `ccode.dev/internal/evict_sessions` — the leader server tells us a
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<ccode_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// `ccode.dev/subagent/cancel`. Active/pending → cancelled (a finish follows);
    /// already-finished → its terminal status; unknown id → `NotFound`.
    pub async fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> ccode_tools::implementations::ccode_build::task::types::SubagentCancelOutcome {
    ...
    pub(crate) async fn list_running_subagents(
        ...
        ccode_tools::implementations::ccode_build::task::types::SubagentInspection,
    > {
        ccode_tools::implementations::ccode_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub(crate) async fn query_subagent(
        ...
        ccode_tools::implementations::ccode_build::task::types::SubagentSnapshot,
    > {
        ccode_tools::implementations::ccode_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<ccode_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or CCODE_RELAY_SYNC_ENABLED)
            ...
            while state_rx.changed().await.is_ok() {
                ...
                if let Ok(params) = serde_json::value::to_raw_value(&notification) {
                    let ext_notification = acp::ExtNotification::new(
                        "ccode.dev/session_notification",
                        params.into(),
                    );
    ...
    /// Get hooks list for a session (for `ccode.dev/hooks/list` extension).
    pub async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<ccode_hooks_types::HooksListResponse> {
        let handle = self.get_session_handle(session_id)?;
        handle.get_hooks_list().await
    }
    /// Execute a hooks management action (for `ccode.dev/hooks/action`).
    pub async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: ccode_hooks_types::HooksAction,
    ) -> Option<ccode_hooks_types::ActionOutcome> {
        if matches!(action, ccode_hooks_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&ccode_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
        handle.execute_hooks_action(action).await
    }
    /// Execute a plugins management action (for `ccode.dev/plugins/action`).
    pub async fn execute_plugins_action(
        &self,
        session_id: &acp::SessionId,
        action: ccode_hooks_types::PluginsAction,
    ) -> Option<ccode_hooks_types::ActionOutcome> {
        let is_reload = matches!(action, ccode_hooks_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(|o| &o.status),
            Some(ccode_hooks_types::OutcomeStatus::Success)
        );
    ...
    /// Get a snapshot of the shared plugin registry (for `ccode.dev/plugins/list`).
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<ccode_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn trace_upload_config_snapshot(
        ...
        let auth_token = if cfg.endpoints.deployment_key.is_none() {
            self.auth_manager
                .current_or_expired()
                .filter(|auth| auth.is_ccode_auth())
                .map(|auth| auth.key)
        } else {
            None
        };
    ...
    /// Build the `ccode.dev/sessionConfig` and `ccode.dev/sessionDetail` `_meta` values
    ...
    pub(super) fn session_config_meta(
        ...
        let detail = session_config::CcodeSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<ccode_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<ccode_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, ccode_chat::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let capture = ccode_chat::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                ccode_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let ccode_home = crate::util::ccode_home::ccode_home();
    ...
    /// 5. `CCODE_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `CCODE_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`ccode_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<ccode_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> ccode_agent::AgentDefinition {
        use ccode_agent::AgentDefinition;
        let ccode_agent_env_set = std::env::var("CCODE_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(ccode_agent::config::is_strict_harness_agent_type);
        if !ccode_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = ccode_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
                agent_name = %def.name,
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = ccode_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = %name,
        ...
        let agent_name = std::env::var("CCODE_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("ccode-build-concise") | Some("ccode_build_concise") => {
                AgentDefinition::ccode_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::ccode_build_plan()
                    }
                }
            }
            Some(name) => {
                ccode_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::ccode_build_plan)
            }
            None => AgentDefinition::ccode_build_plan(),
        };
        if !ccode_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = ccode_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
                model_agent_type = %required,
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs_notify_config = init
            ...
            .meta
            .as_ref()
            .and_then(|m| m.get("ccode.dev/fs_notify"))
            .and_then(|v| {
                use crate::session::{ClientFsConfig, ClientFsMode};
                use ccode_fsnotify::FsConfig;
        ...
        let fs: Arc<dyn ccode_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let hunk_plan = plan_hunk_tracking(
            init
                ...
                .meta
                .as_ref()
                .and_then(|m| m.get("ccode.dev/hunkTracker"))
        ...
        let incremental_bash_output = init
            ...
            .meta
            .as_ref()
            .and_then(|m| m.get("ccode.dev/incrementalBashOutput"))
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        let no_color = init
            .client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("ccode.dev/bashOutputNoColor"))
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (ccode_hunk::HunkTrackerHandle::noop(), None),
        };
        let has_ccode_auth = self.auth_manager.current().is_some_and(|a| a.is_ccode_auth());
        let loc_tracking_enabled = hunk_tracking_enabled && has_ccode_auth
            && (self
                .cfg
                .borrow()
                .remote_settings
                ...
                .unwrap_or(false)
                || std::env::var("CCODE_LOC_TRACKING")
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = ccode_hunk::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = ccode_hunk::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
        ...
        let mut session_env = ccode_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                ccode_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            ccode_agent::config::ModelOverride::Override(id) => {
            ...
            ccode_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = ccode_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = ccode_tools::util::ccode_home::ccode_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".ccode").join("lsp.json");
                tracing::warn!(
                    cwd = %tool_ctx.cwd,
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = ccode_chat::Credentials {
            ...
            let attribution_callback: Option<
                ccode_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = ccode_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
            ...
            let git_head_changed = init
                ...
                .meta
                .as_ref()
                .and_then(|m| m.get("ccode.dev/gitHeadChanged"))
                .and_then(|v| v.as_bool());
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    ccode_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        ccode_hunk::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                        ...
                        ccode_hunk::LocAggregate::LinesReverted {
