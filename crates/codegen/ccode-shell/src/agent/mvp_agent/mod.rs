...
use ccode_telemetry::id::{agent_id, agent_instance_id};
use ccode_telemetry::session_ctx::log_event;
use ccode_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use ccode_workspace::permission::{ClientType, PermissionEvent};
...
use ccode_sampler::SamplerConfig as SamplingConfig;
...
pub(crate) fn reject_direct_hub_cloud_meta(
    ...
    if session_meta.and_then(|m| m.get("ccode.dev/cloud_server_id")).is_some() {
...
/// If `persist_data` is provided, it will be included in the meta under `ccode.dev/persist`.
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::CcodeAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `PremiumPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "CcodePro" => jwt_claim == "premium",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "PremiumPro" => jwt_claim == "premium_heavy",
        "PremiumLite" => jwt_claim == "premium_lite",
        _ => false,
    }
}
...
/// `_meta["ccode.dev/session"].kind` → [`SessionKind`]; absent/unknown/malformed → `Build`.
fn parse_session_kind(
    meta: Option<&acp::Meta>,
) -> crate::session::unified_list::SessionKind {
    use crate::session::unified_list::SessionKind;
    use serde::Deserialize;
    meta.and_then(|m| m.get("ccode.dev/session"))
...
pub(crate) const SESSION_PLUGIN_DIRS_CAPABILITY_KEY: &str = "ccode.dev/pluginDirs";
...
/// Used to stamp `ccode.dev/leaderClientId` onto replay notifications so the leader can
...
fn mark_as_replay(
    ...
    if let Some(persist) = persist_data {
        obj.insert("ccode.dev/persist".to_string(), persist.clone());
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a ccode_sampling_types::TokenUsage>,
...
/// Typed payload for the `ccode.dev/settings/update` notification sent to pager
...
fn announcements_push_payload(
    stored: Option<&[ccode_announcements::RemoteAnnouncement]>,
    last_emitted: &[ccode_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<ccode_announcements::RemoteAnnouncement>> {
    let current = ccode_announcements::filter_expired_at(
...
/// Override with `CCODE_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("CCODE_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
/// gates fails.  Used in `ccode.dev/code/status` responses and to generate
...
pub enum CodeNavEligibility {
    ...
    ClientNotWeb,
    /// Client did not advertise `ccode.dev/codeNavigation.enabled`.
...
pub struct MvpAgent {
    ...
    /// ccode.dev chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// channels; a new `authenticate` or `ccode.dev/auth/cancel` cancels the
    /// prior attempt.
    ...
    /// Whether the current client advertised `ccode.dev/codeNavigation.enabled`.
    ...
    /// Whether the current client advertised `ccode.dev/folderTrust.interactive` (it
    ...
    /// list. Set by `enforce_ccode_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.ccode/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<ccode_graph::IndexManagerHandle>>,
    >,
    ...
    /// Local session-registry override: `CCODE_SESSION_REGISTRY` env, else
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        ccode_tools::implementations::ccode_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: ccode_tools::implementations::ccode_build::task::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: ccode_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled ccode-desktop's first
    ...
    workspace_ops: RefCell<Option<ccode_workspace::WorkspaceOps>>,
    ...
    /// Announcements list last actually emitted via `ccode.dev/announcements/update`
    ...
    last_emitted_announcements: RefCell<Vec<ccode_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &ccode_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<ccode_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !ccode_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = ccode_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `ccode-build` →
/// `ccode-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `ccode-build` →
/// `ccode-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use ccode_agent::config::is_strict_harness_agent_type;
    match (
...
struct AuthRequestMeta {
    ...
    /// Pager auth `request_seq` for this attempt. Scopes `ccode.dev/auth/cancel`
...
///
/// Headers injected:
///  - `x-ccode-client-version` -- required by the proxy's version-gate check.
///    Uses `client_version` when provided, otherwise falls back to cli-chat-proxy
///    compile-time `CARGO_PKG_VERSION`.
///  - `X-Ccode-Token-Auth` / `x-authenticateresponse` -- required by the
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-ccode-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
    ...
    headers
        .entry("x-ccode-client-identifier".to_string())
...
/// Parse the client-advertised `ccode.dev/hunkTracker.mode` string. Case-insensitive
/// and trimmed. Absent/blank/`off`/`disabled` => `None`; unknown => `AllDirty`.
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<ccode_hunk::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(ccode_hunk::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<ccode_hunk::TrackingMode>,
}
...
impl MvpAgent {
    ...
    /// - ccode updates (`"_ccode.dev/session/update"`) → `ExtNotification`.
    ///
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<ccode_acp::AcpResult<()>>,
        >,
        mark_replay: bool,
        ...
        let is_ccode = method == "_ccode.dev/session/update";
        if is_ccode {
            if target_client_id.is_none() && !mark_replay {
                ...
                let Ok(mut params) = serde_json::from_str::<
                    serde_json::Value,
                >(raw_params.get()) else {
                    tracing::debug!("replay: skipping ccode update with unparseable params");
                    return;
                };
                if let Some(obj) = params.as_object_mut() {
                    ...
                    if let Some(m) = meta.as_object_mut() {
                        ...
                        if let Some(pd) = persist_data {
                            m.insert("ccode.dev/persist".to_string(), pd.clone());
                        }
                        if let Some(tid) = target_client_id {
                            m.insert("ccode.dev/leaderClientId".to_string(), tid.clone());
                        }
                    }
                }
            ...
            if let Some(tid) = target_client_id {
                stamp_meta_value(&mut notification.meta, "ccode.dev/leaderClientId", tid);
            }
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<ccode_acp::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = ccode_tools::types::TaskSnapshot {
    ...
    ///
    /// Non-ccode auth (API keys, enterprise) always passes. For ccode OAuth2
    /// users, reads `allow_access` from remote settings. Defaults to
    /// `false` (blocked) when remote settings are unavailable.
    pub(super) async fn enforce_ccode_code_access(&self, auth: &crate::auth::CcodeAuth) {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings ccode_build_access_gate)"
            );
            self.retry_subscription_check().await;
        }
    }
    /// Single-shot subscription check called by the pager's "Check
    /// subscription" button (`ccode.dev/auth/check_subscription`). The pager
            ...
            .map(|auth| {
                let gate = if !self.tier_allowed.get() && gate.is_none() {
                    ...
                    Some(crate::auth::GateInfo {
                        message,
                        url: Some(
                            "https://ccode.dev/premium?referrer=ccode-build".to_string(),
                        ),
    ...
    pub(super) async fn maybe_fetch_post_auth_settings(&self) {
        ...
        let is_ccode_auth = auth.is_ccode_auth();
        ...
        tracing::info!("post-auth remote_settings fetch succeeded");
        {
                ...
                && cfg.mode != crate::agent::config::AgentMode::Generic
            {
                ...
                if cfg.storage_mode == StorageMode::Writeback && !is_ccode_auth {
    ...
    /// Fire-and-forget `ccode.dev/settings/update` from the current remote snapshot.
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&payload) {
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new("ccode.dev/settings/update", params.into()),
                );
        }
    }
    ...
    /// cached_token, ccode.dev/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.ccode/bundled/` and the manifest.
...
/// absent — matching the `ccode_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_ccode_code_access` (initial login gate) and
