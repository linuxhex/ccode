...
const CLIENT_VERSION_HEADER: &str = "x-ccode-client-version";

// ============================================================================
// Turn delta wire types (local to ccode-shell until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::ccode_auth_credentials::CcodeAuthCredentials,
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials = crate::util::ccode_auth_credentials::CcodeAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(ccode_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::ccode_auth_credentials::CcodeAuthCredentials,
    ) -> Arc<dyn ccode_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(ccode_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, ccode_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-Ccode-Token-Auth", "ccode-cli")
        } else {
            builder
        }
    }
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = ccode_fs::trace_context::inject_trace_context_into_request(request);
...
mod tests {
    ...
    fn test_signals_to_update() {
        let signals = crate::session::signals::SessionSignals {
            ...
            models_used: vec!["ccode-3".to_string()],
            primary_model_id: Some("ccode-3".to_string()),
        ...
        assert_eq!(update.primary_model_id, Some("ccode-3".to_string()));
