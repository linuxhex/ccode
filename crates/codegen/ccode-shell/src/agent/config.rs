...
use crate::auth::{AuthManager, CcodeComConfig, OidcAuthConfig};
...
use std::sync::Arc;
use ccode_agent::prompt::skills::SkillsConfig;
use ccode_sampler::{AuthScheme, SamplerConfig};
...
pub const DEFAULT_AGENT_TYPE: &str = "ccode-build-plan";
...
pub const CLI_CHAT_PROXY_BASE_URL_DEFAULT: &str = "https://cli-chat-proxy.ccode.dev/v1";
/// Default base URL for the public ccode API.
pub const CCODE_API_BASE_URL_DEFAULT: &str = "https://api.ccode.dev/v1";
/// Default base URL for the asset server (profile images, etc.).
pub const ASSET_SERVER_URL_DEFAULT: &str = "https://assets.ccode.dev";
...
pub struct EndpointsConfig {
    ...
    /// Base URL for the public ccode API.
    pub ccode_api_base_url: String,
    ...
    /// Env: `CCODE_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `CCODE_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `CCODE_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `CCODE_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `CCODE_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `CCODE_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `CCODE_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `CCODE_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `CCODE_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `CCODE_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `CCODE_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `CCODE_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `CCODE_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ccode_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `CCODE_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ccode_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `CCODE_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("CCODE_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `CCODE_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// the public default. NEVER falls back to `ccode_api_base_url` — that is the
    ...
    /// cli-chat-proxy, never `ccode_api_base_url`.
    ...
    /// Managed deployment-config URL (`ccode setup`): explicit `managed_config_url`,
    /// else `proxy_url` + `/deployment/config`. Never `ccode_api_base_url`, so the
    ...
    /// 1. `ccode_internal_otlp_traces_endpoint` (verbatim)
    ...
    /// 3. `proxy_url` + `/traces`.
    /// Uses the proxy default (not the `ccode_api_base_url` fallback) so
    /// telemetry reports to ccode even when inference is overridden. When the
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.ccode_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 CCODE_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `ccode_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.ccode_internal_otlp_headers) {
    ...
    /// `ccode_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.ccode_internal_otlp_traces_endpoint).is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.ccode_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `CCODE_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("CCODE_CLI_CHAT_PROXY_BASE_URL").ok(),
            ccode_api_base_url: std::env::var("CCODE_CCODE_API_BASE_URL")
                .unwrap_or_else(|_| CCODE_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("CCODE_MODELS_BASE_URL"),
            models_list_url: env_string("CCODE_MODELS_LIST_URL"),
            feedback_base_url: env_string("CCODE_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("CCODE_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("CCODE_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("CCODE_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("CCODE_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("CCODE_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("CCODE_DEPLOYMENT_KEY"),
            managed_config_url: env_string("CCODE_MANAGED_CONFIG_URL"),
            ...
            ccode_internal_otlp_traces_endpoint: env_string("CCODE_INTERNAL_OTLP_TRACES_ENDPOINT"),
            ccode_internal_otlp_headers: env_string("CCODE_INTERNAL_OTLP_HEADERS"),
...
pub use ccode_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
/// environment so it can't inherit the keys Ccode uses for its own first-party
/// requests. Keep in sync with every first-party credential env read across the
/// crate: `auth::manager` (`CCODE_AUTH`/`CCODE_AUTH_PATH`), `auth_method`
/// (`CCODE_API_KEY`/legacy), and the credential-bearing `env_string(...)` reads in
...
pub(crate) const FIRST_PARTY_CREDENTIAL_ENV_VARS: &[&str] = &[
    crate::agent::auth_method::CCODE_API_KEY_ENV_VAR,
    crate::agent::auth_method::LEGACY_CCODE_API_KEY_ENV_VAR,
    "CCODE_AUTH",
    "CCODE_AUTH_PATH",
    "CCODE_DEPLOYMENT_KEY",
    "CCODE_EXTRA_AUTH_KEY",
    "CCODE_TRACE_UPLOAD_CREDENTIALS_FILE",
    "OTEL_EXPORTER_OTLP_HEADERS",
    "CCODE_INTERNAL_OTLP_HEADERS",
];
...
fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> ccode_chat::CompactionMode {
    use ccode_chat::CompactionMode;
...
fn resolve_compaction_detail_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> ccode_chat::CompactionDetail {
    use ccode_chat::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(ccode_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: ccode_tools::types::compat::CompatCell,
...
pub(crate) use ccode_telemetry::config::env_telemetry_mode;
pub use ccode_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.ccode/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                ccode_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> ccode_agent::plugins::discovery::DiscoveryConfig {
        ccode_agent::plugins::discovery::DiscoveryConfig {
...
pub struct CliConfig {
    ...
    /// Env `CCODE_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`] for
    /// the version-policy knobs. (Unrelated to
    /// `version_overrides[].maximum_version`, which gates config patches.)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub minimum_version: Option<String>,
    /// Env `CCODE_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub maximum_version: Option<String>,
    /// Env `CCODE_REQUIRED_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub required_minimum_version: Option<String>,
    /// Env `CCODE_REQUIRED_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `ccode-build-0.1`
...
/// (`ccode workspace` / leader `with_default_hub_url`). Does **not** enable
/// agent-side harness/client connections or alter local session behavior.
///
/// ```toml
/// [hub]
/// url = "wss://hub.ccode.dev/ws"
/// ```
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct HubConfig {
    /// Hub WebSocket URL (`ws://` or `wss://`) used as the leader default for
    /// `ccode workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "CCODE_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("CCODE_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    pub fn resolve_enabled(
        ...
        BoolFlag::env("CCODE_SUGGESTIONS")
            .config(self.enabled)
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("CCODE_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "CCODE_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "ccode-build".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.ccode/skills/`,
/// `.agents/skills/`, `~/.ccode/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    pub ccode_com_config: CcodeComConfig,
    ...
    /// Typed as `CcodeComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<CcodeComConfig>,
    /// `[desktop]` section — owned by ccode-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    pub desktop: Option<toml::Value>,
    /// Top-level `announcements` array — consumed by `resolve_announcements`.
    #[serde(default, skip_serializing)]
    pub announcements: Vec<ccode_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `ccode_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from CCODE_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<ccode_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `CCODE_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, ccode_subagent::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, ccode_subagent::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<ccode_tools::implementations::ccode_build::video_gen::ZdrVideoOutputS3Config>,
    ...
    /// Image describe model (`ccode-build` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<ccode_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<ccode_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut ccode_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut ccode_agent::config::AgentDefinition) {
...
pub use ccode_agent::config::AgentDefinition;
pub use ccode_agent::config::Effort;
pub use ccode_agent::config::PermissionMode;
pub use ccode_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .ccode/agents/, ~/.ccode/agents/, built-ins)
...
/// 4. `CCODE_AGENT` env var
/// 5. Default `ccode-build` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `ccode_agent::discovery::by_name_in_cwd()`.
    /// Examples: "ccode-build", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.ccode/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            ccode_com_config: CcodeComConfig::default(),
            ...
            client_version: Some(ccode_version::VERSION.to_string()),
...
/// (surfaced by `ccode inspect`) and is skipped, so it fails closed for the
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::ccode_home::ccode_home(),
            self.ccode_com_config.clone(),
        )
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.ccode_com_config.oidc.is_none() {
            config.ccode_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.ccode_com_config.oidc.is_none() && config.ccode_com_config.oauth2.is_none() {
            config.ccode_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("CCODE_AUTO_WAKE")
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[ccode_com_config]`.
    /// `[ccode_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[ccode_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("ccode_com_config") {
                ...
                table.insert("ccode_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("CCODE_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `CCODE_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("CCODE_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "in_env_trace_upload": std::env::var("CCODE_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled": std::env::var("CCODE_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("CCODE_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("CCODE_TWO_PASS_COMPACTION")
            .config(self.features.two_pass_compaction)
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    /// Server-side doom-loop check policy (the `x-ccode-doom-loop-check`
    ...
    /// `CCODE_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<ccode_sampling_types::DoomLoopRecoveryPolicy> {
        use ccode_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("CCODE_DOOM_LOOP_RECOVERY")
    ...
    /// `ccode_worktree::maybe_auto_gc`, not here.
    pub fn resolve_worktree_auto_gc(&self) -> ccode_worktree::ResolvedWorktreeAutoGc {
    ...
    /// Gate first-run auto-registration of the official ccode marketplace source.
    /// Precedence: env `CCODE_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("CCODE_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("CCODE_ASK_USER_QUESTION")
    ...
    ///
    /// Precedence: requirements > `CCODE_VOICE_MODE` > config/managed
    ...
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        use ccode_tools::implementations::ccode_build::IMAGE_GEN_TOOL_NAME;
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use ccode_tools::implementations::ccode_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("CCODE_IMAGE_EDIT").default(true).resolve()
    }
    ...
    pub(crate) fn resolve_video_gen(&self) -> Resolved<bool> {
        use ccode_tools::implementations::ccode_build::{
    ...
    pub(crate) fn resolve_image_edit_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "CCODE_IMAGE_EDIT_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_goal(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("CCODE_GOAL")
            .config(self.goal.enabled)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Background workflows (`workflow` tool, `.ccode/workflows/*.rhai`,
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("CCODE_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("CCODE_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("CCODE_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("CCODE_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("CCODE_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `CCODE_COMPACTION_MODE` > config > remote settings >
    /// default, unrecognized falling through) and, for `Segments`, attach the
    /// separately-resolved detail level.
    pub(crate) fn resolve_compaction_mode(&self) -> ccode_chat::CompactionMode {
        resolve_compaction_mode_from(
            env_string("CCODE_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `CCODE_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("CCODE_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `CCODE_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> ccode_chat::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("CCODE_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("CCODE_CANCEL_REWIND")
            .config(self.features.cancel_rewind)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Resolve whether to use ccode's default OAuth2 (ccode auth.ccode.dev).
    ///
    /// Enterprise OIDC (`oidc` in config.toml) always wins — this only gates
    /// the default ccode OAuth2 fallback when no enterprise OIDC is configured.
    ///
    /// Priority: `--oauth` > CCODE_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_ccode_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("CCODE_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// `ccode.dev/mcp/server_status` push.
    ///
    /// Thin delegate to the canonical
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.ccode/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (CCODE_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("CCODE_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (CCODE_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `CCODE_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("CCODE_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (CCODE_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
/// `ccode.dev/mcp/server_status` is wired default-on, with this
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("CCODE_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (CCODE_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `CCODE_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("CCODE_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `CCODE_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(ccode_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("CCODE_ERROR_REPORTING"))
...
/// `CCODE_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn ccode_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("CCODE_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.ccode/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::ccode_home::ccode_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `CCODE_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        ccode_config::load_merged_requirements().as_ref(),
        env_bool("CCODE_EXTERNAL_OTEL"),
...
/// by `ccode setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: ccode_telemetry::external::config::ExternalClientInfo,
) -> Option<ccode_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<ccode_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| ccode_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            ccode_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = ccode_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = ccode_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        ccode_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = ccode_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    ccode_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub fn resolve_model_list(
    ...
    for (key, model_override) in &cfg.config_models {
        ...
        let session_bearer_unsafe = !crate::util::is_ccode_api_bearer_url(&entry.info.base_url)
            || entry
                .api_base_url
                .as_deref()
                .is_some_and(|url| !crate::util::is_ccode_api_bearer_url(url));
    ...
    entries
        ...
        .map(|m| {
            ...
            let config = ModelEntryConfig {
                ...
                api_base_url: Some(endpoints.ccode_api_base_url.clone()),
                name: m.name,
...
pub struct ModelEntryConfig {
    ...
    pub model: String,
    /// The base URL of the model. e.g. "https://api.ccode.dev/v1"
    pub base_url: String,
    ...
    /// If not set, falls back to env_key, then CCODE_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// If not set, falls back to CCODE_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Can also be set via the `CCODE_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
    ...
    /// Without this set, ccode API models send args as one delta
...
pub struct ModelInfo {
    ...
    /// The routing slug sent in API requests.
    pub model: String,
    /// The base URL of the model (session endpoint). e.g. "https://cli-chat-proxy.ccode.dev/v1"
    pub base_url: String,
    ...
    /// Always has a value; defaults to `"ccode-build-plan"` when the server
...
/// codebase_indexing = ["/Users/*/ccode*", "!/Users/*/old-*"]           # globs, ! to exclude
/// ```
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(untagged)]
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<ccode_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when Ccode starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `ccode_build_settings` is absent. When the remote flag is present it takes
    ...
    /// (`image_gen_model_override`) / env / default (`ccode-imagine-image-quality`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// env (`CCODE_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`CCODE_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// termination and pushes `ccode.dev/mcp/server_status` updates to
    ...
    /// `CCODE_MCP_AUTO_RESTART` to opt out).
    ///
    /// Resolved via [`Config::resolve_mcp_auto_restart`].
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mcp_auto_restart: Option<bool>,
    /// Pager-side subscription to the `ccode.dev/mcp/server_status` push.
    ///
    ...
    /// back to the legacy `ccode.dev/mcp/tools_changed` debounced refetch
...
pub struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: ccode_chat::AuthType,
    pub auth_scheme: AuthScheme,
}
...
pub fn resolve_credentials(model: &ModelEntry, session_key: Option<&str>) -> ResolvedCredentials {
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        ...
        (
            Some(key.to_owned()),
            info.base_url.clone(),
            ccode_chat::AuthType::SessionToken,
        )
    } else if let Ok(key) = crate::agent::auth_method::read_ccode_api_key_env() {
        let url = model
            .api_base_url
            .clone()
            .unwrap_or_else(|| info.base_url.clone());
        (Some(key), url, ccode_chat::AuthType::ApiKey)
    } else {
...
/// `disable_api_key_auth` at the credential seam: swap a first-party ccode API
/// key for the IdP session (absent => request fails => forces login). BYOK
/// (non-ccode `base_url`) is untouched; no-op when the switch is off.
pub fn enforce_disable_api_key_auth(
    ...
    if disable_api_key_auth
        && creds.auth_type == ccode_chat::AuthType::ApiKey
        && crate::util::is_ccode_api_url(&creds.base_url)
    {
        creds.auth_type = ccode_chat::AuthType::SessionToken;
...
pub use ccode_telemetry::config::deployment_id_from_key;
...
pub fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.ccode_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
...
pub fn resolve_aux_model_sampling_config(
    ...
    let ccode_bearer = session_key
        .map(|s| s.to_owned())
        .or_else(|| crate::agent::auth_method::read_ccode_api_key_env().ok())
        .or_else(|| endpoints.deployment_key.clone());
    if let Some(bearer) = ccode_bearer {
...
pub fn stamp_session_local_sampler_fields(
    ...
    if crate::util::is_ccode_api_bearer_url(&cfg.base_url) {
...
/// BYOK / non-proxy routes for internal slugs like `ccode-build`).
...
///
/// * cli-chat-proxy bases get `X-Ccode-Token-Auth` and
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            .entry("X-Ccode-Token-Auth".to_string())
            .or_insert_with(|| "ccode-cli".to_string());
        headers
...
mod tests {
    ...
    use ccode_test_support::EnvGuard;
    #[test]
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_ccode_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use ccode_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
prompt_type = "no_user_tool_prefix"
classifier_model = "ccode-4.5"
...
"#;
        ...
        for cfg in [&from_toml, &from_json] {
            ...
            assert_eq!(cfg.classifier_model.as_deref(), Some("ccode-4.5"));
    ...
    fn inject_url_derived_headers_adds_proxy_headers_for_cli_chat_proxy_url() {
        ...
        assert_eq!(
            headers.get("X-Ccode-Token-Auth").map(String::as_str),
            Some("ccode-cli")
        );
        assert_eq!(
    ...
    fn inject_url_derived_headers_skips_proxy_headers_for_external_url() {
        let mut headers = IndexMap::new();
        inject_url_derived_headers(&mut headers, None, "https://api.ccode.dev/v1");
        assert!(headers.get("X-Ccode-Token-Auth").is_none());
    ...
    fn inject_url_derived_headers_does_not_overwrite_existing_entries() {
        let mut headers = IndexMap::new();
        headers.insert("X-Ccode-Token-Auth".to_string(), "caller-set".to_string());
        inject_url_derived_headers(&mut headers, None, crate::env::PROD_CLI_CHAT_PROXY_BASE_URL);
        assert_eq!(
            headers.get("X-Ccode-Token-Auth").map(String::as_str),
            Some("caller-set"),
    ...
    fn finalize_image_describe_sampler_none_uses_active_session_model_not_forced_helper() {
        ...
        assert_ne!(cfg.model, "ccode-build");
    }
    #[test]
    ...
    /// Cold cache falls back to the session model, never the ccode proxy;
    ...
    async fn aux_model_with_auth_provider_never_reroutes() {
        ...
        assert!(
            ...
            .is_none(),
            "cold provider cache must not reroute the aux model through the ccode proxy"
        );
    ...
    fn session_resolver_is_not_stamped_onto_third_party_samplers() {
        ...
        impl ccode_sampler::BearerResolver for SessionResolver {
    ...
    fn shell_environment_policy_known_keys_track_the_policy_struct() {
        let ccode_tools::util::ShellEnvironmentPolicy {
            inherit: _,
            ignore_default_excludes: _,
            exclude: _,
            set: _,
            include_only: _,
        } = ccode_tools::util::ShellEnvironmentPolicy::default();
    ...
    fn parses_model_api_key() {
        ...
        assert_eq!(model.info.model, "ccode-4.5");
    ...
    async fn static_key_shadows_defined_provider_through_pipeline() {
        ...
        assert_eq!(creds.auth_type, ccode_chat::AuthType::ApiKey);
    ...
    async fn set_env_key_shadows_warm_provider_at_resolve_time() {
        use ccode_test_support::EnvGuard;
        let var = "CCODE_TEST_ENVKEY_SHADOW";
    ...
    fn sampling_config_uses_fallback_when_no_model_api_key() {
        ...
        let sampling_config = sampling_config_for_model(
            ...
            ResolvedCredentials {
                ...
                auth_type: ccode_chat::AuthType::ApiKey,
    ...
    fn default_models_dual_endpoint_routing() {
        ...
        for (model_id, entry) in default_model_entries(&endpoints) {
            ...
            assert_eq!(
                api_key_creds.base_url, endpoints.ccode_api_base_url,
                "{model_id}: ExternalApiKey must route to api.ccode.dev"
            );
        }
    }
    #[test]
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["CCODE_TEST_ENV_KEY_PRIMARY", "CCODE_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "CCODE_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "CCODE_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "CCODE_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "CCODE_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "CCODE_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["CCODE_TEST_WS_PRIMARY", "CCODE_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "CCODE_TEST_WS_PRIMARY" => Some("   ".into()),
                "CCODE_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("CCODE_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("CCODE_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use ccode_test_support::EnvGuard;
        let var = "CCODE_TEST_FIRST_OWN_CRED_ENV";
        let _guard = EnvGuard::set(var, "env-token");
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        use ccode_chat::AuthType;
        let primary = "CCODE_TEST_MULTI_ENV_PRIMARY";
        let alias = "CCODE_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use ccode_chat::AuthType;
        use ccode_test_support::EnvGuard;
        let primary = "CCODE_TEST_EMPTY_ENV_PRIMARY";
        let alias = "CCODE_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        use crate::agent::auth_method::{LEGACY_CCODE_API_KEY_ENV_VAR, CCODE_API_KEY_ENV_VAR};
        use ccode_chat::AuthType;
        use ccode_test_support::EnvGuard;
        let sentinel = "ccode-global-sentinel-key";
        let primary = "CCODE_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "CCODE_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
        let _global = EnvGuard::set(CCODE_API_KEY_ENV_VAR, sentinel);
        let _legacy = EnvGuard::unset(LEGACY_CCODE_API_KEY_ENV_VAR);
        let mut model = test_model_entry("m", "https://inference.example/v1", None, None, None);
        model.env_key = Some(EnvKeys::new([primary, alias]));
        assert!(!model.has_own_credentials());
        let creds = resolve_credentials(&model, None);
    ...
    fn api_key_creds(base_url: &str) -> ResolvedCredentials {
        ResolvedCredentials {
            api_key: Some("ccode-secret".to_string()),
            base_url: base_url.to_string(),
            auth_type: ccode_chat::AuthType::ApiKey,
            auth_scheme: Default::default(),
        }
    ...
    fn enforce_disable_api_key_auth_blocks_first_party_only() {
        use ccode_chat::AuthType;
        let mut creds = api_key_creds("https://api.ccode.dev/v1");
        enforce_disable_api_key_auth(&mut creds, false, Some("session-jwt"));
        assert_eq!(creds.auth_type, AuthType::ApiKey);
        assert_eq!(creds.api_key.as_deref(), Some("ccode-secret"));
        let mut creds = api_key_creds("https://api.ccode.dev/v1");
        ...
        let mut creds = ResolvedCredentials {
            ...
            ..api_key_creds("https://api.ccode.dev/v1")
        };
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(creds.auth_type, AuthType::SessionToken);
    }
    ...
    /// (non-ccode.dev) own keys are preserved. (`try_resolve_model_credentials`
    ...
    fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
        ...
        assert_eq!(creds.api_key.as_deref(), Some("ccode-model-key"));
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(
            creds.auth_type,
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = ccode_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn byok_from_lookup_classifies_all_states() {
        ...
        let session = test_model_entry("m", "https://api.ccode.dev/v1", None, None, None);
        assert_eq!(
    ...
    fn user_override_adds_api_key_to_default_model() {
        ...
        assert_eq!(
            model.info.base_url, "https://cli-chat-proxy.ccode.dev/v1",
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use ccode_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use ccode_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
        ...
        assert_eq!(
            ...
            CompactionMode::Segments(ccode_chat::CompactionDetail::default())
        );
        assert_eq!(
    ...
    fn sampling_config_context_window_from_entry_or_default() {
        let model = test_model_entry("any-model", "https://api.ccode.dev/v1", None, None, None);
        let config = sampling_config_for_model(
            &model,
            resolve_credentials(&model, None),
            None,
        ...
        let mut model = test_model_entry("any-model", "https://api.ccode.dev/v1", None, None, None);
    ...
    /// ccode-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_ccode_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn hidden_model_excluded_from_acp_but_kept_in_catalog() {
        ...
        let raw_config: toml::Value = toml::from_str(
            ...
            [model.visible-model]
            model = "visible-model"
            base_url = "https://api.ccode.dev/v1"
            context_window = 200000
    ...
    fn invalid_glob_is_rejected_by_validation() {
        ...
        assert!(ModelGlobSet::compile(Some(&vec!["ccode[".to_string()])).is_err());
        let raw: toml::Value = toml::from_str(
            r#"
            [models]
            allowed_models = ["ccode["]
            "#,
        )
        .unwrap();
    ...
    #[test]
    fn auth_alias_maps_to_ccode_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
        ...
        let oidc = cfg.ccode_com_config.oidc.expect("oidc should be set");
        assert_eq!(oidc.issuer, "https://example.okta.com");
        assert_eq!(oidc.client_id, "test-id");
    }
    #[test]
    fn ccode_com_config_still_works() {
        let raw: toml::Value = toml::from_str(
            r#"
            [ccode_com_config.oidc]
            issuer = "https://example.okta.com"
            client_id = "test-id"
            "#,
        )
    ...
    fn disable_api_key_auth_parses_from_auth_alias() {
        ...
        assert_eq!(absent.ccode_com_config.disable_api_key_auth, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(cfg.ccode_com_config.disable_api_key_auth, Some(true));
    }
    ...
    fn force_login_team_uuid_parses_string_and_array() {
        ...
        let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
        assert_eq!(absent.ccode_com_config.force_login_team_uuid, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(
            cfg.ccode_com_config.force_login_team_uuid,
    ...
    fn force_login_team_uuid_implies_api_key_auth_disabled() {
        use crate::auth::{ForceLoginTeam, CcodeComConfig};
        let base = CcodeComConfig {
            disable_api_key_auth: None,
            force_login_team_uuid: None,
            ..CcodeComConfig::default()
        };
    ...
    fn e2e_user_overrides_default_model_key_with_custom_endpoint() {
        ...
        assert_eq!(
            ...
            "should route to the user's custom endpoint, not api.ccode.dev"
        );
    ...
    fn e2e_config_toml_model_overrides_default() {
        ...
        unsafe { std::env::set_var("CCODE_API_KEY", "ccode-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
        unsafe { std::env::remove_var("CCODE_API_KEY") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
    }
    #[test]
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "CCODE_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn config_models_default_custom_model_is_in_resolved_model_list() {
        ...
        assert!(
            models.contains_key("acme-ccode"),
            "user-defined model must be in the resolved model list"
        );
        let model = models.get("acme-ccode").unwrap();
        assert_eq!(model.info.model, "ccode-4.5");
        assert_eq!(model.info.base_url, "https://inference.example.com/v1");
    }
    #[test]
    fn e2e_default_model_with_session_routes_to_proxy() {
        ...
        assert_eq!(
            sampling.base_url, "https://cli-chat-proxy.ccode.dev/v1",
            "session auth should route to cli-chat-proxy, not api.ccode.dev"
        );
    }
    #[test]
    #[serial]
    fn e2e_default_model_with_external_api_key_routes_to_api_ccode() {
        let (_, models) = resolve_models_from_toml("", None);
        let model = models
            .get(crate::models::default_model())
            .expect("default model should exist");
        unsafe { std::env::set_var("CCODE_API_KEY", "ccode-external-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.api_key.as_deref(), Some("ccode-external-key"));
        assert_eq!(
            sampling.base_url, "https://api.ccode.dev/v1",
            "external API key should route to api.ccode.dev via api_base_url"
        );
        unsafe { std::env::remove_var("CCODE_API_KEY") };
    }
    #[test]
    fn e2e_user_config_overrides_prefetched_model() {
        ...
        prefetched.insert(
            dm.to_string(),
            test_model_entry(dm, "https://cli-chat-proxy.ccode.dev/v1", None, None, None),
        );
        let (_, models) = resolve_models_from_toml(
            &format!(
                r#"
    ...
    fn e2e_credential_priority_model_key_beats_session_beats_env() {
        ...
        unsafe { std::env::set_var("CCODE_API_KEY", "env-key") };
    ...
    fn e2e_duplicate_model_field_both_entries_survive() {
        ...
        let user = models.get("acme-ccode").unwrap();
        ...
        assert_eq!(sampling.base_url, "https://cli-chat-proxy.ccode.dev/v1",);
    }
    #[test]
    fn e2e_enterprise_custom_endpoint_skips_ccode_defaults() {
        let mut cfg = Config::default();
        ...
        assert!(
            ...
            "ccode default must not leak into enterprise model list"
        );
    ...
    fn e2e_acp_model_info_no_dedup_on_model_field() {
        let mut models = IndexMap::new();
        models.insert(
            "default-ccode".to_string(),
            test_model_entry(
                crate::models::default_model(),
                "https://cli-chat-proxy.ccode.dev/v1",
                None,
                None,
                Some("https://api.ccode.dev/v1"),
            ),
        ...
        assert!(
            acp_models.contains_key(&acp::ModelId::new("default-ccode")),
            "default entry should be addressable by map key"
        );
        assert!(
            acp_models.contains_key(&acp::ModelId::new("acme-ccode")),
    ...
    fn e2e_enterprise_endpoints_plus_partial_model_override() {
        ...
        let (_, models) = resolve_models_from_toml(
            ...
            ccode_api_base_url = "https://enterprise-api.acme.com/v1"
    ...
    fn e2e_enterprise_endpoints_only_no_model_override() {
        ...
        assert_eq!(
            model.api_base_url.as_deref(),
            Some("https://enterprise-api.acme.com/v1"),
            "default model should use enterprise ccode_api_base_url"
        );
    }
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "CCODE_CLI_CHAT_PROXY_BASE_URL",
            "CCODE_CCODE_API_BASE_URL",
            "CCODE_FEEDBACK_BASE_URL",
            "CCODE_TRACE_UPLOAD_URL",
            "CCODE_MANAGED_CONFIG_URL",
            "CCODE_MODELS_BASE_URL",
            "CCODE_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "CCODE_INTERNAL_OTLP_TRACES_ENDPOINT",
            "CCODE_INTERNAL_OTLP_HEADERS",
            "CCODE_EXTERNAL_OTEL",
        ] {
    ...
    /// `ccode_api_base_url` — overriding ONLY inference keeps every aux endpoint on
    ...
    fn aux_endpoints_resolve_to_proxy_never_inference() {
        ...
        let inference = "https://inference.acme-corp.example/ccode/v1";
        let cfg = EndpointsConfig {
            ccode_api_base_url: inference.to_string(),
        ...
        assert_eq!(cfg.ccode_api_base_url, inference);
    ...
    /// REGRESSION: the managed-config URL never follows `ccode_api_base_url`
    ...
    fn loader_managed_config_url_never_follows_inference_endpoint() {
        ...
        let cfg = Config::new_from_toml_cfg(
            &toml::from_str(
                r#"[endpoints]
                ccode_api_base_url = "https://inference.acme-corp.example/ccode/v1""#,
            )
            .unwrap(),
        )
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("CCODE_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("CCODE_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("CCODE_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("CCODE_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("CCODE_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("CCODE_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("CCODE_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("CCODE_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn worktree_auto_gc_section_parses_from_toml() {
        unsafe {
            std::env::remove_var(ccode_worktree::ENV_AUTO_GC);
            std::env::remove_var(ccode_worktree::ENV_AUTO_GC_DRY_RUN);
            std::env::remove_var(ccode_worktree::ENV_AUTO_GC_MAX_AGE);
        }
        let raw: toml::Value = toml::from_str(
            r#"
            [worktree.auto_gc]
        ...
        assert_eq!(
            p.max_age_by_kind
                .get(&ccode_worktree::WorktreeKind::Subagent),
            Some(&Some(3600))
        );
        assert_eq!(
            p.max_age_by_kind
                .get(&ccode_worktree::WorktreeKind::Manual),
            Some(&None)
        );
    }
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("CCODE_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("CCODE_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("CCODE_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("CCODE_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config_without_remote_kill_switch() {
        unsafe { std::env::set_var("CCODE_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn background_workflows_default_on_without_affecting_goal() {
        unsafe { std::env::remove_var("CCODE_WORKFLOWS") };
        let cfg = Config::default();
    ...
    fn resolve_workflows_remote_false_kills_local_opt_in() {
        unsafe { std::env::set_var("CCODE_WORKFLOWS", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_workflows_env_wins() {
        unsafe { std::env::set_var("CCODE_WORKFLOWS", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_workflows();
        assert_eq!(r.source, ConfigSource::Env);
        assert!(
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("CCODE_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("CCODE_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("CCODE_IMAGE_GEN_MODEL_OVERRIDE") };
        ...
        assert_eq!(
            with(None, Some("ccode-imagine-image")).resolve_image_gen_model_override(),
            Some("ccode-imagine-image".to_owned())
        );
        assert_eq!(
            with(Some("ccode-imagine-image-pro"), Some("ccode-imagine-image"))
                .resolve_image_gen_model_override(),
            Some("ccode-imagine-image-pro".to_owned())
        );
    }
    #[test]
    #[serial]
    fn resolve_image_edit_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("CCODE_IMAGE_EDIT_MODEL_OVERRIDE") };
        let with = |config: Option<&str>, gb: Option<&str>| Config {
            features: Features {
        ...
        assert_eq!(
            with(None, Some("ccode-imagine-image")).resolve_image_edit_model_override(),
            Some("ccode-imagine-image".to_owned())
        );
        assert_eq!(
            with(Some("ccode-imagine-image-pro"), Some("ccode-imagine-image"))
        ...
        let gen_only = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
                image_gen_model_override: Some("ccode-imagine-image".to_owned()),
                ..Default::default()
            }),
            ..Default::default()
        };
    ...
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("CCODE_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("CCODE_IMAGE_EDIT", "1") };
    ...
    fn resolve_image_gen_gates() {
        unsafe { std::env::remove_var("CCODE_IMAGE_GEN") };
        ...
        unsafe { std::env::set_var("CCODE_IMAGE_GEN", "1") };
        let denied = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_video_gen_gates() {
        unsafe { std::env::remove_var("CCODE_VIDEO_GEN") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("CCODE_GOAL");
            std::env::remove_var("CCODE_GOAL_CLASSIFIER");
            std::env::remove_var("CCODE_GOAL_PLANNER");
            std::env::remove_var("CCODE_GOAL_SUMMARY");
            std::env::remove_var("CCODE_GOAL_VERIFIER_N");
            std::env::remove_var("CCODE_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("CCODE_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("CCODE_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("CCODE_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("CCODE_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("CCODE_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("CCODE_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("CCODE_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "CCODE_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "ccode-build", agent_type = "ccode-build-plan" }

[goal.strategist_model]
model = "ccode-composer-2.5-fast"
agent_type = "cursor"

[[goal.skeptic_models]]
model = "ccode-build"
agent_type = "ccode-build-plan"
...
"#;
        ...
        let cfg = Config::new_from_toml_cfg(&raw).unwrap();
        assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "ccode-build");
        assert_eq!(
            cfg.goal.strategist_model.as_ref().unwrap().agent_type,
            "cursor"
        );
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "ccode-build");
        assert_eq!(
...
planner_model = { agent_type = "ccode-build-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "ccode-build");
        assert_eq!(cfg.goal.skeptic_models[1].model, "ccode-composer-2.5-fast");
    }
...
classifier_max_runs = 6
planner_model = { model = "ccode-build", agent_type = "ccode-build-plan" }
strategist_model = { model = "ccode-composer-2.5-fast", agent_type = "cursor" }
...
"#,
        ...
        let ccode_build = crate::util::config::GoalRoleModel {
            model: "ccode-build".into(),
            agent_type: "ccode-build-plan".into(),
        };
        let composer = crate::util::config::GoalRoleModel {
            model: "ccode-composer-2.5-fast".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(ccode_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(ccode_build),
    ...
    fn config_warns_on_section_typo() {
        let raw: toml::Value = toml::from_str(
            r#"
            [endpoint]
            deployment_key = "ccode-token-test"
        "#,
        )
        .unwrap();
    ...
    fn config_accepts_all_known_sections() {
        let unused = unused_keys_from_toml(
            ...
            command = "/opt/bin/ccode-identity"
            [repo_changes_dedup]
            enabled = false
            [relay]
            enabled = false
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            ccode_internal_otlp_traces_endpoint: None,
            ccode_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `ccode_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_ccode_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                ccode_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: CCODE_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                ccode_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                ccode_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `ccode_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                ccode_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> ccode_telemetry::external::config::ExternalClientInfo {
        ccode_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("CCODE_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `CCODE_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("CCODE_SUBAGENTS");
            std::env::remove_var("CCODE_RESPECT_GITIGNORE");
            std::env::remove_var("CCODE_WEB_SEARCH_MODEL");
            std::env::remove_var("CCODE_SESSION_SUMMARY_MODEL");
            std::env::remove_var("CCODE_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("CCODE_CURSOR_RULES_ENABLED");
            std::env::remove_var("CCODE_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("CCODE_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("CCODE_CLAUDE_RULES_ENABLED");
            std::env::remove_var("CCODE_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("CCODE_MANAGED_MCPS_ENABLED");
            std::env::remove_var("CCODE_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("CCODE_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "CCODE_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "CCODE_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "CCODE_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("CCODE_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("CCODE_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("CCODE_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("CCODE_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("CCODE_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("CCODE_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("CCODE_TELEMETRY_ENABLED") };
...
"#,
        ...
        ccode_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("ccode-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.ccode-build] sets
    /// context_window=500k for model="ccode-4.5", but
    /// [models].default="ccode-4.5" resolves to the bare
    /// prefetched entry (256k) because Layer 3 only overrides key
    /// "ccode-build", not key "ccode-4.5".
    ///
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        prefetched.insert("ccode-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let by_key = resolved
            .get("ccode-build")
            .expect("ccode-build key must exist");
        assert_eq!(by_key.info.context_window.get(), 500_000);
        assert_eq!(by_key.info.model, "ccode-4.5");
        let by_latest = resolved.get("ccode-4.5").expect("ccode-4.5 key must exist");
        assert_eq!(
            by_latest.info.context_window.get(),
            500_000,
            "BUG: prefetched 'ccode-4.5' should inherit 500k from \
             sibling 'ccode-build' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    fn slug_propagation_inherits_api_backend_but_not_agent_type() {
        ...
        let mut prefetched = IndexMap::new();
        let mut entry =
            test_model_entry("ccode-4.5", "https://test.example.com/v1", None, None, None);
        entry.info.context_window = NonZeroU64::new(default_cw).unwrap();
        entry.info.agent_type = default_agent_type();
        entry.info.api_backend = ApiBackend::default();
        prefetched.insert("ccode-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let latest = resolved.get("ccode-4.5").unwrap();
        assert_eq!(
    ...
    fn resolve_model_list_config_reasoning_efforts_beats_remote() {
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let mut entry = prefetch_model_entry("ccode-x", 200_000, ApiBackend::default());
        entry.info.reasoning_efforts = vec![ReasoningEffortOption {
            id: "high".to_string(),
            value: ReasoningEffort::High,
            label: "High".to_string(),
        ...
        let mut prefetched = IndexMap::new();
        prefetched.insert("ccode-x".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("CCODE_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("CCODE_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("CCODE_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("CCODE_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("CCODE_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("CCODE_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("CCODE_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("CCODE_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("CCODE_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("CCODE_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("CCODE_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("CCODE_MCP_RECURSIVE_CONFIG_WATCH", "false") };
