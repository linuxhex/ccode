...
impl ReplayBuffer {
    ...
    fn merge(
        ...
        match (prev, new) {
            ...
            (Some(SessionNotification::CcodeSession(prev)), SessionNotification::CcodeSession(new)) => {
                let (force, first, second) = merge_ccode_session_chunks(*prev, *new);
                (
                    force,
                    SessionNotification::CcodeSession(Box::new(first)),
                    second.map(|s| SessionNotification::CcodeSession(Box::new(s))),
                )
            }
...
/// Merge two consecutive ccode notifications.
fn merge_ccode_session_chunks(
...
fn estimate_payload_bytes(n: &SessionNotification) -> u64 {
    match n {
        ...
        SessionNotification::CcodeSession(n) => match &n.update {
...
mod tests {
    ...
    fn pending_text(buf: &ReplayBuffer) -> Option<String> {
        ...
        match pending {
            ...
            SessionNotification::CcodeSession(_) => None,
        }
    }
    ...
    // ── CcodeSession / ToolCallDeltaChunk tests ──────────────────────────
    ...
    #[test]
    fn ccode_session_same_id_deltas_merge_args_and_preserve_name() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        let n = match flushed {
            SessionNotification::CcodeSession(n) => *n,
            _ => panic!("expected CcodeSession"),
        };
        match n.update {
            crate::extensions::notification::SessionUpdate::ToolCallDeltaChunk {
                tool_call_id,
    ...
    #[test]
    fn ccode_session_different_tool_call_id_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        assert!(matches!(flushed, SessionNotification::CcodeSession(_)));
        assert!(rest.is_some());
        assert!(buf.pending.is_none());
    }

    #[test]
    fn ccode_session_tool_call_delta_is_bufferable() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn ccode_session_after_acp_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        let acp_chunk = msg_chunk("s", 1, "thinking...");
        let ccode_session_chunk = delta_chunk("s", Some("call_1"), 0, Some("bash"), Some("args"));

        assert!(buf.consume_chunk(acp_chunk).is_none());
        let (flushed, rest) = buf.consume_chunk(ccode_session_chunk).expect("should force-flush");

        // Both emitted immediately — different kinds can't merge.
        assert!(matches!(flushed, SessionNotification::Acp(_)));
        assert!(matches!(rest, Some(SessionNotification::CcodeSession(_))));
        assert!(buf.pending.is_none());
    }
}
