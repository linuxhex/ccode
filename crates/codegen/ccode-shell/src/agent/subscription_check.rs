...
//! lift the gate.
//!
//! The pager drives the polling via `ccode.dev/auth/check_subscription`: the 5s
...
/// Subscription tiers that qualify for Ccode Build access.
...
async fn fetch_user_info(
    ...
    let request = http_client
        ...
        .header(
            "X-Ccode-Token-Auth",
            auth_manager.ccode_com_config().token_header.as_str(),
        )
...
/// the paywall is shown (`ccode.dev/auth/check_subscription`).
///
...
mod tests {
    ...
    fn partial_tier_name_is_not_qualifying() {
        ...
        assert!(!QUALIFYING_TIERS.contains(&"Ccode"));
