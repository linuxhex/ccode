...
/// What a [`ConfigWarning`] is about. Serialize-only: `ccode inspect --json`
...
impl WarningTarget {
    /// The config path, e.g. `model."ccode-4.5"` or `auth_provider."litellm"`.
...
pub(crate) fn log_config_warnings(warnings: &[ConfigWarning]) {
    ...
    if !warnings.is_empty() {
        tracing::warn!(
            ...
            "model_override: parsed with warnings; run `ccode inspect` for details"
        );
    }
}
...
mod tests {
    ...
    fn duplicate_compactions_keys_keeps_model() {
        ...
        let model = cfg
            .config_models
            .get("ccode-4.5")
            .expect("ccode-4.5 must remain in catalog");
        assert_eq!(
        ...
        assert!(resolved.contains_key("ccode-4.5"));
    }
    ...
    fn legacy_alias_alone_parses_without_warning() {
        ...
        let model = cfg.config_models.get("ccode-4.5").unwrap();
        assert_eq!(
            model.compactions_remaining,
    ...
    fn invalid_reasoning_effort_skips_field_keeps_model() {
        ...
        let model = cfg
            ...
            .get("ccode-4.5")
            .expect("ccode-4.5 must remain in catalog");
        assert_eq!(model.model.as_deref(), Some("ccode-4.5"));
    ...
    fn unknown_field_warns_but_keeps_known_fields() {
        ...
        let entry = models.get("ccode-4.5").unwrap();
        assert_eq!(entry.model.as_deref(), Some("ccode-4.5"));
        assert_eq!(
    ...
    fn non_table_model_section_warns_and_is_ignored() {
        let (models, warnings) = parse_raw(r#"model = "ccode-4""#);
