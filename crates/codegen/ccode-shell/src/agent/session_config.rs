...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("ccode-build", "Ccode Build"),
            model("ccode-4.5", "Ccode 4.5"),
        ];
        let current = acp::ModelId::from("ccode-build");
        ...
        assert_eq!(selected_models[0].id, "ccode-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("ccode-build", "Ccode Build")];
        let current = acp::ModelId::from("ccode-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("ccode-build", "")];
        let current = acp::ModelId::from("ccode-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "ccode-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "ccode-build".to_string(),
            category: "model".to_string(),
            label: "Ccode Build".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "ccode-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "Ccode Build");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn ccode_session_detail_serializes_camel_case() {
        let detail = CcodeSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["cwd"], "/Users/me/project");
        assert_eq!(v["currentModelId"], "ccode-build");
