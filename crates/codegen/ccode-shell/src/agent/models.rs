...
use crate::auth::{AuthManager, CcodeAuth, CcodeComConfig};
use crate::remote::{FetchModelsResult, fetch_models_blocking};
use crate::sampling::SamplerConfig as SamplingConfig;
use globset::{Glob, GlobSet, GlobSetBuilder};
use ccode_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl ModelFetchAuth {
    ...
    ///
    /// A `deployment_key` outranks an ambient `CCODE_API_KEY` so a stray env key
    ...
    pub(crate) fn resolve(endpoints: &config::EndpointsConfig, has_cached_session: bool) -> Self {
        if endpoints.has_custom_endpoint() {
            ...
            Self::Deployment
        } else if crate::agent::auth_method::has_ccode_api_key_env() {
            Self::ApiKey
        } else {
            Self::Session
        }
...
struct Inner {
    ...
    gateway: RwLock<Option<ccode_acp::AcpAgentGatewaySender>>,
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&ccode_home, CcodeComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: ccode_acp::AcpAgentGatewaySender) {
    ...
    pub fn apply_config(&self, new_config: config::Config) {
        ...
        *self.inner.models.write() = new_catalog;
        ...
        // Push the new catalog to connected clients (`ccode.dev/models/update`).
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<ccode_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<ccode_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    ///
    /// `model_id` may be a routing slug (`config.model`, e.g. `ccode-4.5`)
    /// OR a catalog key; the catalog map is keyed by the config key, which can
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-ccode-build`
    /// → slug `ccode-4.5`). Resolve to the catalog key first so a slug
    ...
    /// Hot-reload the catalog from `~/.ccode/models_cache.json` after an
    ...
    /// When another ccode process sharing `~/.ccode` (a `--no-leader` run, a
    /// newer client, ccode-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `ccode_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::ccode_home::ccode_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.ccode_version.as_deref() != Some(ccode_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            ccode_version: Some(ccode_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Ccode Build" both route to `ccode-build`)
...
pub(crate) fn prefetch_models_and_settings_blocking(
    ...
    // Settings need a ccode.dev session; skip for BYOK.
    ...
    match fetch_models_blocking(endpoints, auth, fetch_auth) {
        Ok(FetchModelsResult { models, etag }) if !models.is_empty() => {
            let api_base_url_override = match fetch_auth {
                ModelFetchAuth::ApiKey => Some(endpoints.ccode_api_base_url.clone()),
                _ => None,
            };
...
fn resolve_prefetch_env_with_auth(auth: Option<CcodeAuth>) -> Option<PrefetchEnv> {
    let _timer = crate::instrumentation_timer!("startup.early_prefetch_launch");
    // Config-aware (not env-only) so the prefetch can't leak the bearer to api.ccode.dev.
...
/// explicit off switch must hold even when a stray login, `CCODE_API_KEY`, or
...
fn resolve_prefetch_env(ccode_com_config: Option<CcodeComConfig>) -> Option<PrefetchEnv> {
    let ccode_home = crate::util::ccode_home::ccode_home();
    let auth_manager = AuthManager::new(&ccode_home, ccode_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<CcodeAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(ccode_com_config: Option<CcodeComConfig>) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(ccode_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
///
/// Sessions persist the routing slug (`[model.X].model`, e.g. `ccode-4.5`);
/// the catalog and `/model` picker use config keys (e.g. `enterprise-ccode-build`).
...
pub fn resolve_model_catalog(
    ...
    // must not stamp `none` onto ccode-4.5, which only offers low/medium/high).
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("ccode-test-models-manager");
        let auth_manager = Arc::new(AuthManager::new(&tmp, CcodeComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    async fn set_current_model_id_change_fires_watch_to_all_subscribers() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("ccode-4"));
    ...
    async fn model_switch_generation_snapshot_reflects_current_state() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("ccode-3"));
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("ccode-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, CcodeComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use ccode_sampling_types::ReasoningEffortOption;
        ...
        let mut no_none = ModelEntry {
            info: config::ModelInfo::fallback("ccode-4.5"),
            api_key: None,
            env_key: None,
            auth_provider: None,
            api_base_url: None,
        ...
        prefetched.insert("ccode-4.5".to_string(), no_none);
        ...
        let catalog = resolve_model_catalog(&cfg, Some(prefetched));
        assert_eq!(
            catalog["ccode-4.5"].info.reasoning_effort,
            Some(ReasoningEffort::High),
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("ccode-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, CcodeComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn first_apply_refresh_reselects_default_model() {
        let mgr = test_manager();
        let mut cfg = config::Config::default();
        cfg.models.default = Some("ccode-3".to_string());

        assert!(!mgr.has_fetched_real_catalog());

        let prefetched = make_prefetched(&["ccode-3", "ccode-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);

        assert!(mgr.has_fetched_real_catalog());
        assert_eq!(mgr.current_model_id().0.as_ref(), "ccode-3");
    }
    ...
    fn subsequent_refresh_reselects_when_model_removed() {
        ...
        // Second refresh with ccode-4 removed.
        let prefetched = make_prefetched(&["ccode-3", "ccode-4.5"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    ...
    fn apply_config_honors_new_preferred_model() {
        ...
        *mgr.inner.cfg.write() = stale_cfg;
        ...
        new_cfg.models.default = Some("ccode-3".to_string());
    ...
    fn apply_config_falls_back_when_preferred_not_in_catalog() {
        ...
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("ccode-nonexistent".to_string());
        mgr.apply_config(new_cfg);
    ...
    fn auth_refresh_then_config_reload_preserves_user_model() {
        ...
        *mgr.inner.etag.write() = None;
        ...
        let prefetched = make_prefetched(&["ccode-3", "ccode-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        assert_eq!(mgr.current_model_id().0.as_ref(), "ccode-4");

        // Config reload with persisted preference.
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("ccode-4".to_string());
        mgr.apply_config(new_cfg);
        assert_eq!(mgr.current_model_id().0.as_ref(), "ccode-4");
    }
    ...
    fn reload_from_disk_cache_applies_external_catalog() {
        ...
        cache.persist(
            &make_prefetched(&["ccode-4.5", "ccode-4.3"]),
            Some("etag-ext"),
            auth_method,
            &mgr.cache_origin(),
        );
        ...
        assert!(mgr.has_fetched_real_catalog());
        assert!(mgr.models().contains_key("ccode-4.5"));
        assert!(mgr.models().contains_key("ccode-4.3"));
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["ccode-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("ccode-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["ccode-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("ccode-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["ccode-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("ccode-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["ccode-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("ccode-legacy"));
    }
    ...
    fn clear_resets_has_fetched_real_catalog() {
        ...
        let prefetched = make_prefetched(&["ccode-4.5", "ccode-4.3"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        let first_available = mgr.available().keys().next().unwrap().clone();
        assert_eq!(
            mgr.current_model_id().0.as_ref(),
    ...
    fn resolve_custom_endpoint_always_wins() {
        let _key = EnvGuard::set("CCODE_API_KEY", "test-key");
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        let _unset = EnvGuard::unset("CCODE_API_KEY");
        let _unset_legacy = EnvGuard::unset("CCODE_CODE_CCODE_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    /// `deployment_key` outranks a stray `CCODE_API_KEY`, but session wins over both.
    #[test]
    #[serial]
    fn resolve_deployment_key_outranks_ambient_api_key() {
        let _key = EnvGuard::set("CCODE_API_KEY", "stray-env-key");
        let endpoints = config::EndpointsConfig {
            deployment_key: Some("deploy-key".to_owned()),
            ..config::EndpointsConfig::default()
        };
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Deployment,
            "managed deployment_key should outrank an ambient CCODE_API_KEY",
        );
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, true),
            ModelFetchAuth::Session,
    ...
    /// once — session auth, ambient `CCODE_API_KEY`, `deployment_key`, AND a
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "ccode-build", Some("Auto")),
            make_entry_config_with_id(Some("ccode-build"), "ccode-build", Some("Ccode Build")),
            make_entry_config_with_id(
                Some("ccode-composer-2.5-fast"),
                "ccode-composer-2.5-fast",
                Some("Ccode Fast"),
            ),
        ];
        ...
        assert!(map.contains_key("ccode-build"));
        assert!(map.contains_key("ccode-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "ccode-build",
            "auto entry should still route to ccode-build"
        );
        assert_eq!(map["ccode-build"].info.model, "ccode-build");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("ccode-build"), "ccode-build", Some("First")),
            make_entry_config_with_id(Some("ccode-build"), "ccode-build", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["ccode-build"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            "auto-ccode-build".to_string(),
            make_model_entry("ccode-build"),
        );
        catalog.insert("ccode-build".to_string(), make_model_entry("ccode-build"));

        let mut cfg = config::Config::default();
        cfg.models.default = Some("ccode-build".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "ccode-build", "must match id, not first slug hit");
    }
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        models.insert(
            "enterprise-ccode-build".to_string(),
            make_model_entry("ccode-4.5"),
        );
        models.insert("ccode-4.3".to_string(), make_model_entry("ccode-4.3"));

        let persisted = acp::ModelId::new("ccode-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "enterprise-ccode-build");
    }

    #[test]
    fn resolve_catalog_key_prefers_exact_key_match() {
        let mut models = IndexMap::new();
        models.insert("ccode-4.5".to_string(), make_model_entry("ccode-4.5"));

        let persisted = acp::ModelId::new("ccode-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("exact key must resolve");
        assert_eq!(key.0.as_ref(), "ccode-4.5");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        let mut models = IndexMap::new();
        models.insert(
            "default-ccode-build".to_string(),
            make_model_entry("ccode-4.5"),
        );
        models.insert("user-ccode-build".to_string(), make_model_entry("ccode-4.5"));

        let persisted = acp::ModelId::new("ccode-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-ccode-build");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        let mut models = IndexMap::new();
        models.insert("ccode-build".to_string(), make_model_entry("ccode-build"));
        models.insert(
            "enterprise-ccode-build".to_string(),
            make_model_entry("ccode-build"),
        );
        ...
        let available = test_available_keys(&["enterprise-ccode-build", "ccode-4.3"]);

        let persisted = acp::ModelId::new("ccode-build");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("ccode-build".to_string(), make_model_entry("ccode-4.5"));
        models.insert("other".to_string(), make_model_entry("ccode-build"));

        let available = test_available_keys(&["ccode-build", "other"]);

        let persisted = acp::ModelId::new("ccode-build");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "ccode-build");
    }
