...
/// Env var that, when set, advertises `ccode.api_key` as a viable auth method.
///
/// Kept as a constant so test code and the production check stay in sync.
pub const CCODE_API_KEY_ENV_VAR: &str = "CCODE_API_KEY";

/// Legacy env var name. Checked as a fallback when `CCODE_API_KEY` is not set,
/// so existing deployments that use the old name keep working.
pub const LEGACY_CCODE_API_KEY_ENV_VAR: &str = "CCODE_CODE_CCODE_API_KEY";

/// Read the API key from the environment.
///
/// Checks `CCODE_API_KEY` first, then falls back to the legacy
/// `CCODE_CODE_CCODE_API_KEY` for backward compatibility.
pub fn read_ccode_api_key_env() -> Result<String, std::env::VarError> {
    std::env::var(CCODE_API_KEY_ENV_VAR).or_else(|_| std::env::var(LEGACY_CCODE_API_KEY_ENV_VAR))
}

/// Returns `true` if either `CCODE_API_KEY` or `CCODE_CODE_CCODE_API_KEY` is set.
pub fn has_ccode_api_key_env() -> bool {
    read_ccode_api_key_env().is_ok()
}

/// Whether `ccode.api_key` should be advertised (and pushed FIRST) when building
/// the `auth_methods` list at `initialize()` time.
///
/// Regression: `ccode.api_key` must stay first when only per-model credentials
/// exist (no global `CCODE_API_KEY`). Deferring it made BYOK users hit the login
...
///
/// `disable_api_key_auth` (`[ccode_com_config] disable_api_key_auth` /
/// `CCODE_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
/// method is never advertised, regardless of available credentials, so
/// `CCODE_API_KEY` can't bypass a deployment's forced IdP login.
pub fn should_advertise_ccode_api_key<'a, I>(disable_api_key_auth: bool, models: I) -> bool
where
    I: IntoIterator<Item = &'a ModelEntry>,
{
    ...
    has_ccode_api_key_env() || models.into_iter().any(ModelEntry::has_own_credentials)
}
...
pub struct AuthMethodsBuildInputs<'a> {
    /// True if `ccode.api_key` should be advertised AT ALL. Caller computes via
    /// [`should_advertise_ccode_api_key`]. When `preferred_method` is `Oidc`,
    ...
    /// Optional display label for the login method (`ccode.dev` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `ccode_com_config.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `ccode.dev` method).
...
pub struct BuiltAuthMethods {
    ...
    /// `cached_token` wins over `ccode.api_key` when both are present. When
...
/// `has_external_api_key` is true, the **first** entry MUST be `ccode.api_key`.
...
/// 1. `ccode.api_key`     (if `has_external_api_key`)
...
/// - `ccode.api_key`  else if `has_external_api_key`
/// - `None`         otherwise
///
/// Pinned (`preferred_method`):
/// - `ApiKey`: only `ccode.api_key` if available; else empty list + `None` (fail).
/// - `Oidc`: `cached_token` (if any) + interactive login; never `ccode.api_key`.
...
fn build_pinned_api_key(has_external_api_key: bool) -> BuiltAuthMethods {
    ...
    BuiltAuthMethods {
        methods: vec![ccode_api_key_auth_method()],
        default_auth_method_id: Some(acp::AuthMethodId::new(CCODE_API_KEY_METHOD_ID)),
    }
}
...
fn build_unpinned(
    ...
    if has_external_api_key {
        methods.push(ccode_api_key_auth_method());
        default_auth_method_id = Some(acp::AuthMethodId::new(CCODE_API_KEY_METHOD_ID));
    }

    if has_cached_token {
        methods.push(cached_token_auth_method());
        // cached_token wins over ccode.api_key for default_auth_method_id so
        ...
        if overrode_api_key {
            ccode_telemetry::unified_log::info(
                "auth method priority: cached_token overrides ccode.api_key for default_auth_method_id",
                None,
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.ccode_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.ccode_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(ccode_com_auth_method(login_label, has_auth_provider_command));
    }
}
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            CCODE_API_KEY_METHOD_ID => Self::CcodeApiKey,
            CACHED_TOKEN_AUTH_METHOD_ID => Self::CachedToken,
            CCODE_COM_METHOD_ID => Self::CcodeCom,
    ...
    pub fn is_api_key(self) -> bool {
        matches!(self, Self::CcodeApiKey)
    }

    /// `true` for session-based methods (cached_token, ccode.dev, oidc).
    pub fn is_session_based(self) -> bool {
        matches!(self, Self::CachedToken | Self::CcodeCom | Self::Oidc)
    }

    /// Requires user interaction (browser, OIDC redirect, or external auth command).
    pub fn needs_interactive_login(self) -> bool {
        matches!(self, Self::CcodeCom | Self::Oidc)
    }
...
/// `true` for session-based ACP methods (cached_token, ccode.dev, oidc).
...
/// `CCODE_API_KEY` is set, which demoted live OIDC sessions to non-refreshable
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `ccode login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `ccode login`, set CCODE_API_KEY, or add api_key to ~/.ccode/config.toml.";

/// Next ACP method id when `cached_token` cannot proceed (missing / expired /
/// legacy WebLogin), or `None` when fallthrough is forbidden.
///
/// Unpinned: prefer non-interactive `ccode.api_key` when advertiseable, else
/// interactive `ccode.dev`.
///
...
pub const PREFERRED_API_KEY_UNAVAILABLE: &str = "preferred_method=api_key but no API key is configured (set CCODE_API_KEY or model api_key/env_key in config.toml).";

/// Error when `preferred_method=oidc` but the session path cannot proceed.
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `ccode login` to authenticate.";

pub const CCODE_API_KEY_METHOD_ID: &str = "ccode.api_key";
pub fn ccode_api_key_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(
            acp::AuthMethodId::new(CCODE_API_KEY_METHOD_ID),
            "ccode.api_key".to_string(),
        )
        .description(Some(format!(
            "{CCODE_API_KEY_ENV_VAR} or api_key/env_key in config.toml"
        ))),
    )
}
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.ccode/auth.json".to_string())),
    )
}

pub const CCODE_COM_METHOD_ID: &str = "ccode.dev";

/// ccode OAuth2/OIDC auth. Method id `"ccode.dev"` kept for ACP wire-compat.
pub fn ccode_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("Ccode");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(CCODE_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    /// `cached_token` to non-interactive `ccode.api_key` (not browser OAuth).
    ...
    /// `ccode.api_key` first, while `default_auth_method_id` prefers session;
    /// after session fails, this helper must still pick `ccode.api_key`.
    #[test]
    ...
    /// No advertiseable API-key credentials → interactive `ccode.dev`.
    #[test]
    fn after_cached_token_unavailable_falls_to_ccode_com_without_api_key() {
        assert_eq!(
    ...
    fn auth_method_kind_classifier_matrix() {
        ...
        let api_id = acp::AuthMethodId::new(CCODE_API_KEY_METHOD_ID);
    ...
    // Reordering so `ccode.api_key` is after login methods must fail the tests below.

    /// BYOK with only per-model `env_key` must list `ccode.api_key` first.
    #[test]
    fn enterprise_byok_first_method_is_ccode_api_key() {
        ...
        assert_eq!(
            ...
            Some(AuthMethodKind::CcodeApiKey),
            "BYOK enterprise-style: auth_methods.first() MUST be ccode.api_key \
        ...
        assert!(
            ...
            "first method MUST NOT need interactive login when ccode.api_key is available",
        );
    }

    /// BYOK + cached session token: ccode.api_key stays first in the methods
    /// list (skips login screen), but `default_auth_method_id` is
    /// `cached_token` (keeps OIDC refresh alive).
    #[test]
    fn byok_with_cached_token_keeps_ccode_api_key_first() {
        let inputs = AuthMethodsBuildInputs {
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::CcodeApiKey),
            "ccode.api_key MUST precede cached_token in advertised order",
        );
    ...
    /// `ccode.dev` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): only `ccode.dev` is
    ...
    #[test]
    fn fresh_user_only_advertises_ccode_com_and_requires_login() {
        let built = build_auth_methods(default_inputs());

        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::CcodeCom));
        assert!(built.default_auth_method_id.is_none());
        assert_eq!(built.methods.len(), 1);
    }

    /// Enterprise OIDC replaces `ccode.dev` (mutually exclusive). ccode.api_key,
    /// when present, still leads.
    #[test]
    fn enterprise_oidc_replaces_ccode_com_but_ccode_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::CcodeApiKey));
        assert!(
            built
                .methods
                .iter()
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::CcodeCom),
            "ccode.dev and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `ccode.dev` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let ccode = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::CcodeCom)
            .expect("ccode.dev must be advertised");
        assert_eq!(ccode.name(), "Acme Corp");
        let meta = ccode.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.ccode/config.toml` skeleton from the bug report, walks it through
    /// the same predicate (`should_advertise_ccode_api_key`) and the same
    /// list-builder (`build_auth_methods`) that `MvpAgent::initialize()` uses
    /// in production, and asserts that `auth_methods.first()` is `ccode.api_key`
    /// (which causes the pager to skip the login screen).
    ///
    /// This is the test that *would have caught* that regression -- if you mentally
    /// re-introduce that bug (push ccode.api_key LAST when has_external_api_key
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        let _global = EnvGuard::unset(CCODE_API_KEY_ENV_VAR);
        ...
        // login method. Confirms the predicate isn't trivially true.
        {
            ...
            let has_external_api_key = should_advertise_ccode_api_key(false, models.values());
            ...
            assert_ne!(
                first_kind(&built.methods),
                Some(AuthMethodKind::CcodeApiKey),
                "without env_key resolved, ccode.api_key must NOT be advertised first",
            );
        }

        // With the env var present (the actual enterprise scenario), the predicate
        // returns true and the builder MUST put `ccode.api_key` first so the
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // ccode.dev login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
                first_kind(&built.methods),
                Some(AuthMethodKind::CcodeApiKey),
                "BYOK: ccode.api_key must be auth_methods.first(); deferred-to-last \
    ...
    /// `CCODE_API_KEY` alone (no per-model creds) also triggers
    /// advertising `ccode.api_key` as the first method. Historical "external
    /// key" path; covered here so the predicate keeps treating env-var-only
    /// users the same as per-model users.
    #[test]
    #[serial]
    fn global_external_api_key_advertises_ccode_api_key_first() {
        let _set = EnvGuard::set(CCODE_API_KEY_ENV_VAR, "ccode-external-key");
    ...
    /// AND per-model env_key), so the builder never advertises `ccode.api_key`
    /// and the pager sends the user to the deployment's login method instead.
    #[test]
    #[serial]
    fn disable_api_key_auth_suppresses_ccode_api_key_method() {
        let _set = EnvGuard::set(CCODE_API_KEY_ENV_VAR, "ccode-external-key");
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);

        // Flag off: today's behavior (advertised first).
        assert!(should_advertise_ccode_api_key(false, models.values()));

        // Flag on: never advertised, regardless of credentials.
        let has_external_api_key = should_advertise_ccode_api_key(true, models.values());
        assert!(!has_external_api_key);
        let built = build_auth_methods(AuthMethodsBuildInputs {
            has_external_api_key,
            ..default_inputs()
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::CcodeApiKey),
            "ccode.api_key must not be advertised when disable_api_key_auth is set",
        );
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::CcodeCom),
    ...
    /// Legacy `CCODE_CODE_CCODE_API_KEY` env var is accepted as a fallback
    /// when `CCODE_API_KEY` is not set, ensuring existing deployments keep working.
    #[test]
    #[serial]
    fn legacy_env_var_fallback_advertises_ccode_api_key() {
        let _unset_new = EnvGuard::unset(CCODE_API_KEY_ENV_VAR);
        let _set_legacy = EnvGuard::set(LEGACY_CCODE_API_KEY_ENV_VAR, "ccode-legacy-key");
        assert!(has_ccode_api_key_env());
        assert_eq!(read_ccode_api_key_env().unwrap(), "ccode-legacy-key");
    ...
    /// When both `CCODE_API_KEY` and `CCODE_CODE_CCODE_API_KEY` are set,
    /// the new name takes precedence.
    #[test]
    #[serial]
    fn new_env_var_takes_precedence_over_legacy() {
        let _new = EnvGuard::set(CCODE_API_KEY_ENV_VAR, "new-key");
        let _legacy = EnvGuard::set(LEGACY_CCODE_API_KEY_ENV_VAR, "old-key");
        assert_eq!(read_ccode_api_key_env().unwrap(), "new-key");
    }

    // -- ccode login --legacy regression coverage ------------------------
    //
    // `ccode login --legacy` produces a CcodeAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `CCODE_AUTH` env var (or via legacy
    ...
    // to-end. A regression in CCODE_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `CCODE_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn ccode_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, CcodeAuth, CcodeComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("CCODE_AUTH_PATH");
        let _g2 = EnvGuard::unset(CCODE_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `ccode login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = CcodeAuth {
            ...
            refresh_token: None,
            expires_at: None,
            ..CcodeAuth::test_default()
        };

        // Provide it via CCODE_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("CCODE_AUTH", &legacy_json);

        // AuthManager picks it up from the env var directly (no file needed).
        let dir = tempfile::tempdir().unwrap();
        let cfg = CcodeComConfig::default();
        let mgr = AuthManager::new(dir.path(), cfg);
        let current = mgr.current();
        assert!(
            current.is_some(),
            "legacy token in CCODE_AUTH env MUST be loaded directly -- if this fails, \
        ...
        // With only this legacy token (no ccode api key), first method must be
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("CCODE_AUTH");
        let _g2 = EnvGuard::unset("CCODE_AUTH_PATH");
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::CcodeCom),
            "no cached token AND no api key: pager must show login (ccode.dev first)",
        );
    }
    ...
    fn pin_api_key_with_key_only_advertises_api_key() {
        ...
        assert_eq!(method_ids(&built), vec![CCODE_API_KEY_METHOD_ID]);
        assert_eq!(default_id(&built), Some(CCODE_API_KEY_METHOD_ID));
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, CCODE_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![CCODE_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
