//! `ccode mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = ccode_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.ccode/config.toml".to_string(),
...
// ── Managed (ccode.dev) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "ccode.dev";
...
/// Discover managed `ccode_com_*` servers if the user has ccode auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    let ccode_home = ccode_tools::util::ccode_home::ccode_home();
    let ccode_com_config = CcodeComConfig::default();
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&ccode_home, ccode_com_config));
    ...
    if !snapshot.is_managed_mcp_eligible() {
        return managed_skipped(format!("{:?} auth (not ccode OIDC)", snapshot.auth_mode));
    }

    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `ccode login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use ccode_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `ccode mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `ccode mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `ccode mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `ccode mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
