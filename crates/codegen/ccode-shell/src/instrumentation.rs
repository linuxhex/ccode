//! Shim — see `ccode_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `ccode_shell::instrumentation_timer!`). Keeping the macro here
...
pub use ccode_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `ccode_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `ccode_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `ccode_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `ccode_telemetry::instrumentation`.
#[macro_export]
