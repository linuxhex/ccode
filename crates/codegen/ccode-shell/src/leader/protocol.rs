...
pub enum ClientMode {
    /// Headless mode (ccode agent, ccode agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (ccode agent stdio, ccode -p) - uses local IPC.
...
pub struct ClientCapabilities {
    ...
    /// Whether this client has advertised `ccode.dev/codeNavigation.enabled`.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `ccode update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `ccode update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `ccode update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["ccode-a".into(), "ccode-b".into()],
            pid: 4242,
        };
