//! Leader-follower IPC architecture for ccode-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.ccode/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use ccode_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::CcodeBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = ccode_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`ccode -p`).
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<CcodeBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(CcodeBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<CcodeBuildEnvironment> {
    let environments: &[CcodeBuildEnvironment] = &[CcodeBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<CcodeBuildEnvironment> {
    let environments: &[CcodeBuildEnvironment] = &[CcodeBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::ccode_home::ccode_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!({
                            ...
                            "reason": reason,
                            "killer_ver": ccode_version::VERSION,
                        })),
                    );
                }
                Err(e) => {
...
impl From<&crate::auth::CcodeComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::CcodeComConfig) -> Self {
        Self {
            ccode_ws_url: c.ccode_ws_url.clone(),
            ccode_ws_origin: c.ccode_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`ccode -p` where hanging forever is unacceptable.
...
///     "ccode-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
///
/// The `env_urls.ccode_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "ccode-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.ccode_ws_url);
...
///
/// For a **managed install** — the running binary lives under `ccode_home`
/// (e.g. `~/.ccode/...`) — prefer the managed `~/.ccode/bin/ccode` symlink. After an
/// auto-update or `ccode update` atomically swaps that symlink, `current_exe()`
...
/// `ccode_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `ccode_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.ccode/bin/ccode` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::ccode_home::ccode_home())
}
fn resolve_binary_with_home(ccode_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(ccode_home, std::env::current_exe().ok())
}
/// Binary file name for the managed ccode install (`ccode` / `ccode.exe`).
fn managed_ccode_bin_name() -> &'static str {
    if cfg!(windows) { "ccode.exe" } else { "ccode" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = ccode_home.join("bin").join(managed_ccode_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, ccode_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--ccode-ws-url").arg(&env_urls.ccode_ws_url);
    cmd.arg("--ccode-ws-origin").arg(&env_urls.ccode_ws_origin);
    ...
    let log_path = crate::util::ccode_home::ccode_home().join("leader.log");
    ...
    let leader_log = std::env::var("CCODE_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "ccode_shell=info,ccode_acp=warn,ccode_mcp=warn".into());
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            ccode_ws_url: "wss://test.invalid".into(),
            ccode_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("ccode"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("ccode-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("ccode")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("ccode-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("ccode");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("ccode-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_ccode_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_ccode_bin_name());
