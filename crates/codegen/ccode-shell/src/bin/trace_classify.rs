...
//!       [--output out.jsonl] \
//!       [--model ccode-4.5] \
//!       [--api-base-url https://api.ccode.dev/v1] \
//!       [--api-key <key> | $CCODE_API_KEY | <ccode-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use ccode_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "ccode-4.5")]
    model: String,

    /// Sampler base URL.
    #[arg(long, default_value = "https://api.ccode.dev/v1")]
    api_base_url: String,

    /// API key. Overrides `$CCODE_API_KEY` when set; falls back to
    /// `$CCODE_API_KEY`, then `<ccode-home>/auth.json` (`ccode::api_key`
    ...
    /// shell uses (`$CCODE_HOME` or `~/.ccode`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    ccode_home: Option<PathBuf>,
}
...
mod tests {
    ...
    #[test]
    fn cli_ccode_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.ccode_home, Some(PathBuf::from("/tmp/scratch-ccode")));
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["ccode-4.5"]);
        assert_eq!(by_id("api_base_url"), vec!["https://api.ccode.dev/v1"]);
