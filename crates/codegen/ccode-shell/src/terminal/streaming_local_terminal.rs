...
/// after we stop waiting. Mirrors the bound the `ccode-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("CCODE_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for ccode_acp::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
