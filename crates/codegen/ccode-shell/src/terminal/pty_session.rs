...
const NOTIFICATION_METHOD: &str = "ccode.dev/terminal/pty/notification";
...
/// `CCODE_SHELL`) since `$SHELL` is absent.
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use ccode_config::shell::{WindowsShell, detect_windows_shell};
