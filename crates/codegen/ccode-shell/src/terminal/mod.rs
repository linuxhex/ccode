...
/// Resolved absolute path to bash. On Unix uses the `ccode_config` shell
/// resolution cascade (`$CCODE_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        ccode_config::shell::unix_shell_path(ccode_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `ccode-tools`.
pub use ccode_tools::util::pager_env;
...
/// tool output. Used when the client sets `ccode.dev/bashOutputNoColor: true` (e.g.
/// ccode-pager which renders its own UI and doesn't need raw ANSI codes).
///
