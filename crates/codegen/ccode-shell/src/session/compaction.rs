...
use std::sync::Arc;
use ccode_chat::compaction_utils::{
...
use ccode_sampling_types::{ApiBackend, ConversationItem};
...
/// Override with `CCODE_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("CCODE_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("CCODE_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        ccode_tokens::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("CCODE_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target: "two_pass",
                "two_pass: DEBUG CCODE_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(ccode_chat::estimate_item_tokens)
            .sum::<u64>();
...
/// (`ccode-chat` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                ccode_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
        ...
        self.send_ccode_notification(CcodeSessionUpdate::AutoCompactCompleted {
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            ccode_telemetry::session_ctx::log_event(
                ccode_telemetry::events::AutoCompactSuppressed {
    ...
    pub(crate) async fn surface_compact_auth_failure(&self, err: acp::Error) -> acp::Error {
        ...
        acp::Error::auth_required().data(crate::sampling::error::terminal_error_data(
            message,
            Some(401),
            ccode_sampler::SamplingErrorKind::Auth,
        ))
    }
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    ccode_chat::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    ccode_chat::estimate_conversation_tokens(&full_conv),
                );
                if ccode_tokens::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        trigger: ccode_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            ccode_telemetry::events::CompactionTrigger::Manual => "manual",
            ccode_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = ccode_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            ccode_hooks::event::HookEventName::PreCompact,
            ccode_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            ccode_chat::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            ccode_chat::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            ccode_chat::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let effective_tool_defs: Vec<ccode_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            ccode_chat::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<ccode_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(ccode_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<ccode_sampling_types::HostedTool> =
        ...
        let estimated_input_tokens =
            ccode_chat::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, ccode_telemetry::events::CompactionTrigger::Auto);
        let wall_clock_budget_secs = self
            .agent
            .borrow()
            .compaction_policy()
        ...
        let fr_config = ccode_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match ccode_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(ccode_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(ccode_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(ccode_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = ccode_chat::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    ccode_chat::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                ccode_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                ccode_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use ccode_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn ccode_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn ccode_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: ccode_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        self.dispatch_hook(
            ccode_hooks::event::HookEventName::PostCompact,
            ccode_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &ccode_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn run_compact_only(
        ...
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::AutoCompactFired {
            tokens_before: trigger_info.tokens_used,
            percentage: trigger_info.percentage,
        });
        self.signals_handle()
            .record_compaction(trigger_info.tokens_used);
        self.send_ccode_notification(CcodeSessionUpdate::AutoCompactStarted {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                ccode_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
        ...
        match result {
            ...
            Err(e) => {
                ...
                if self
                    ...
                    == SUPPRESS_NONE
                {
                    self.send_ccode_notification(CcodeSessionUpdate::AutoCompactFailed {
                        error: String::new(),
                    })
                    .await;
                }
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<ccode_sampling_types::ToolSpec>,
        user_context: Option<&str>,
    ...
    fn persist_compaction_checkpoint(
        ...
        use crate::extensions::notification::{
            CompactionCheckpointFile, CompactionCheckpointInfo, SessionUpdate as CcodeSessionUpdate,
        };
        ...
        self.persist_ccode_update_only(CcodeSessionUpdate::CompactionCheckpoint(Box::new(info)));
        tracing::info!(
            prompt_index_at_compaction,
...
mod inline_auto_compact_flow_tests {
    ...
    async fn create_test_actor(
        ...
        gateway_tx: mpsc::UnboundedSender<ccode_acp::AcpClientMessage>,
        ...
        let hunk_tracker_handle = ccode_hunk::HunkTrackerActor::spawn(
            ...
            cwd.to_path_buf(),
            hunk_tx,
            ccode_hunk::TrackingMode::AgentOnly,
        ...
        let chat_state_handle = ccode_chat::ChatStateActor::spawn(
            vec![],
            ccode_sampling_types::SamplingConfig {
            ...
            Box::new(ccode_chat::NullChatPersistence),
            chat_event_tx,
            tokio_util::sync::CancellationToken::new(),
        );
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: ccode_chat::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            extension_registry: ccode_lifecycle::LocalExtensionRegistry::default(),
            ...
            vcs_kind: ccode_workspace::session::git::VcsKind::Git,
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<ccode_acp::AcpClientMessage>();
        ...
        local
            .run_until(async {
                ...
                while let Ok(msg) = persistence_rx.try_recv() {
                    if let PersistenceMsg::Update(SessionUpdate::Ccode(notif)) = msg
                        && let CcodeSessionUpdate::RetryState(
                            crate::extensions::notification::RetryState::Failed {
                                error_type,
                                message,
                            },
                ...
                async fn notification_for(reason: SuppressReason) -> String {
                    ...
                    while let Ok(msg) = persistence_rx.try_recv() {
                        if let PersistenceMsg::Update(
                            crate::session::storage::SessionUpdate::Ccode(notif),
                        ) = msg
                ...
                while let Ok(msg) = persistence_rx.try_recv() {
                    if let PersistenceMsg::Update(SessionUpdate::Ccode(notif)) = msg {
                        match &notif.update {
                            ...
                            CcodeSessionUpdate::AutoCompactFailed { error } => {
                                assert!(
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use ccode_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    async fn forked_release_still_over_threshold_suppresses_auto() {
        ...
        local
            .run_until(async {
                ...
                while let Ok(msg) = persistence_rx.try_recv() {
                    if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(
                        notif,
                    )) = msg
                        && matches!(
                            &notif.update,
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): Ccode Build usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("Ccode Build usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> ccode_sampler::SamplingErrorInfo {
        ccode_sampler::SamplingErrorInfo {
            kind: ccode_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = ccode_sampler::SamplingErrorInfo {
                    kind: ccode_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = ccode_chat::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = ccode_chat::CompactionMode::Summary;
