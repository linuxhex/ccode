//! Session-level fs-watch policy over [`ccode_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `ccode_fsnotify`. This module
...
use ccode_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
/// Dedup key for `ccode.dev/git_head_changed`, shared by the watcher's `GitHead`
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> ccode_workspace::file_system::FileIndexDelta {
    use ccode_workspace::file_system::FileIndexDelta;
...
pub(crate) struct CapabilityInputs {
    ...
    pub hunk_tracking: bool,
    pub code_nav: bool,
    /// `ccode.dev/gitHeadChanged`; opt-in (absent => off).
...
impl ClientNotify {
    async fn on_change(&self, paths: &[PathBuf], kind: FsEventKind) {
        ...
        match self.mode {
            ClientFsMode::Events => {
                // Present-tense strings are the `ccode.dev/fs_notify` wire protocol;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = ccode_workspace::session::git::get_branch(&self.cwd).await;
        let commit = ccode_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = ccode_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = ccode_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: ccode_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                ccode_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || ccode_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = ccode_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(ccode_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: ccode_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
