...
//! `ccode-fs`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use ccode_fs::events::tracker::EventTracker;
pub(crate) use ccode_fs::events::types::{
...
) -> Option<ccode_sampling_types::PriorTurnInterrupt> {
    use ccode_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run ccode-build flavor. Distinct from
...
/// (`ccode-fs`). The two enums are intentional mirrors and live in
