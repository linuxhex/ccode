...
//!   under `~/.ccode/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.ccode/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.ccode/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.ccode/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.ccode/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.ccode/`.
...
/// `CleanAll` mode. Placed under `~/.ccode/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(ccode_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(ccode_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                ccode_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || ccode_worktree::remove_worktree(&path_owned)).await;
