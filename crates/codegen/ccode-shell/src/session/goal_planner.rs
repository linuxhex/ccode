...
///
/// [`SubagentRuntimeOverrides::harness_agent_type`]: ccode_tools::implementations::ccode_build::task::types::SubagentRuntimeOverrides::harness_agent_type
...
pub(crate) struct RoleSpawnOverride {
    ...
    /// Resolved harness `agent_type` (e.g. `"ccode-build-plan"`)
...
mod tests {
    ...
    fn planner_template_default_render_preserves_wording_and_has_no_placeholders() {
        ...
        // (ccode-build) tool names; guards against accidental wording drift.
    ...
    async fn fail_open_retry_emits_spawn_failed_event() {
        ...
        let writer = ccode_fs::events::EventWriter::open(dir.path());
        let ov = RoleSpawnOverride {
            model: Some("m".into()),
            agent_type: Some("t".into()),
        };
