...
//! [`ccode_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`ccode_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`ccode_agent::Agent`] owns an [`ccode_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`ccode_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use ccode_agent::config::AgentDefinition;
use ccode_agent::error::AgentBuildError;
use ccode_agent::prompt::context::PromptAudience;
use ccode_agent::prompt::skills::SkillsConfig;
use ccode_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use ccode_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use ccode_tools::implementations::ccode_build::ask_user_question::types::UserQuestionRequest;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<ccode_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<ccode_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use ccode_tools::implementations::ccode_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(ccode_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(ccode_tools::types::resources::SchedulerBackgroundLoops(
                *scheduler_background_loops,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(ccode_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use ccode_tools::implementations::ccode_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            ccode_tools::computer::local::LocalTerminalBackend::new_local(
                ccode_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: ccode_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(ccode_tools::types::tool::ToolKind::Task)
            .expect("CcodeBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("CcodeBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_ccode_build())
                    .await
