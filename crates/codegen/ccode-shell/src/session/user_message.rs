...
// Re-export from ccode-chat — canonical definition lives there.
pub use ccode_chat::compaction_utils::extract_user_query;
    ...
    match result {
        Ok(Ok(status)) => {
            ...
            let status = if vcs_kind.is_jj() {
                ...
                ccode_agent::prompt::user_message::normalize_git_status(&status)
            };
...
// Tests for extract_user_query now live in ccode_chat::compaction_utils.

#[cfg(test)]
mod tests {
    use super::*;
    use ccode_workspace::file_system::FsError;
    ...
    async fn construct_user_message_returns_without_git_status_on_bad_dir() {
        let dir = std::path::Path::new("/tmp/nonexistent-ccode-test-dir");
