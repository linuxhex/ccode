...
//!
//! Canonical definitions live in `ccode_prompt`; this re-export keeps every existing
//! `crate::session::prompt_queue::*` and cross-crate `ccode_shell::session::prompt_queue::*`
//! path resolving without edits.

pub use ccode_prompt::{
...
pub const QUEUE_CHANGED_METHOD: &str = "ccode.dev/queue/changed";
...
mod tests {
    ...
    fn queue_changed_serializes_camel_case_with_session_id() {
        let payload = QueueChanged {
            ...
            entries: vec![QueueEntryWire {
                id: "p1".to_string(),
                version: 0,
                owner: Some("ccode-tui".to_string()),
                last_editor: None,
    ...
    fn queue_entry_wire_round_trips_last_editor() {
        let entry = QueueEntryWire {
            ...
            version: 3,
            owner: Some("ccode-tui".to_string()),
            last_editor: Some("ccode-vscode".to_string()),
            kind: "prompt".to_string(),
            text: "hello".to_string(),
            position: 0,
            combined_texts: None,
        ...
        assert_eq!(json["lastEditor"], "ccode-vscode");
