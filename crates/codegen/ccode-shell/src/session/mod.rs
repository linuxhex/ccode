...
pub use ccode_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource, FsNotifyError, GitMetaKind};
...
pub(crate) fn is_cursor_user_template(
    _template: &ccode_agent::prompt::user_message::UserMessageTemplate,
) -> bool {
    false
}
/// `false` twin of [`is_cursor_system_template`]; see [`is_cursor_user_template`].
pub(crate) fn is_cursor_system_template(
    _template: &ccode_agent::prompt::context::TemplateOverride,
) -> bool {
    false
}
...
/// Client-requested fs notification mode (was ccode_fsnotify::FsNotifyMode).
...
pub use ccode_shared::session::info;
pub mod managed_mcp;
