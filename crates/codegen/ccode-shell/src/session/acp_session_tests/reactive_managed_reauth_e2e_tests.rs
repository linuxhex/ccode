...
//! `ccode.dev/mcp/server_status` pushes that clients consuming only `server_status`
...
//! handshake against rmcp 2.1 in `ccode-mcp/tests/repro_sse_flood.rs`): a
...
use ccode_mcp::servers::{ClientStateKind, HttpConfig, McpClient};

const MANAGED: &str = "ccode_com_testconnector";
...
async fn actor_with_proxy(
    proxy_base: &str,
    gw_tx: tokio::sync::mpsc::UnboundedSender<ccode_acp::AcpClientMessage>,
...
/// Drain all `ccode.dev/mcp/server_status` pushes currently queued on `gw_rx`.
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use ccode_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `ccode_com_*` tools enter entry-B.
