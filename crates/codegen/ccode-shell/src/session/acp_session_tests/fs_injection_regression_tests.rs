use ccode_tools::computer::local::{LocalTerminalBackend, MockFs};
...
async fn tool_bridge_routes_writes_through_injected_fs() {
    ...
    let config = ToolServerConfig {
        tools: vec![
            ...
            ToolConfig {
                id: "CcodeBuild:search_replace".into(),
                params: Some(
    ...
    let ctx = SessionContext {
        ...
        fs,
        cwd: cwd.clone(),
        session_folder: std::env::temp_dir().join("ccode-test-fs"),
        ...
        skills: vec![],
        state_path: std::env::temp_dir().join("ccode-test-fs/tool_state.json"),
        memory_backend: None,
