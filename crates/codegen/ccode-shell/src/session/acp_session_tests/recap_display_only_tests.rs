...
fn drained_session_recap(rx: &mut tokio::sync::mpsc::UnboundedReceiver<PersistenceMsg>) -> bool {
    ...
    while let Ok(msg) = rx.try_recv() {
        if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(n)) = msg
            && matches!(
                n.update,
        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_ccode_req_id.starts_with("ccode-recap-"),
                        "req id: {}",
                        artifact.x_ccode_req_id
                    );
...
async fn manual_recap_over_budget_trims_persisted_request_and_is_display_only() {
    ...
    local
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    let est = ccode_chat::estimate_conversation_tokens(&artifact.chat_history);
                    assert!(
                        est <= PROMPT_BUDGET,
...
/// shape, so it also covers the laxer ccode ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use ccode_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use ccode_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // ccode backend => strip_reasoning=false; the over-budget branch strips anyway.
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = ccode_sampling_types::build_messages_request(&req);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = ccode_sampling_types::ApiBackend::Responses;
            ...
            let conv_id = recap_req
                .header("x-ccode-conv-id")
                .expect("recap must send x-ccode-conv-id");
            assert!(
...
async fn recap_request_sends_hosted_tools_under_backend_search() {
    use ccode_sampling_types::HostedTool;
    use ccode_test_support::MockInferenceServer;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = ccode_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use ccode_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use ccode_test_support::MockInferenceServer;
