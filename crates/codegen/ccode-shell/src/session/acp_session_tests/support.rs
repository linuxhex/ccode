...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> ccode_hub_sdk::ObservabilityBridge {
    ccode_hub_sdk::ObservabilityBridge::new(
        None,
        ccode_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> ccode_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<ccode_sampling_types::HostedTool>,
) -> ccode_agent::Agent {
    let base = test_agent_default().await;
    ccode_agent::Agent::new(
        base.definition().clone(),
        ccode_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        ccode_agent::ReminderPolicy::default(),
        ccode_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> ccode_agent::Agent {
    use ccode_tools::implementations::ccode_build::update_goal::UpdateGoalTool;
    use ccode_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// Ccode-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_ccode_build_agent_with_todo() -> ccode_agent::Agent {
    use ccode_tools::implementations::ccode_build::todo::TodoWriteTool;
    use ccode_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> ccode_agent::Agent {
    use ccode_tools::implementations::ccode_build::enter_plan_mode::EnterPlanModeTool;
    use ccode_tools::implementations::ccode_build::exit_plan_mode::ExitPlanModeTool;
    use ccode_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<ccode_tools::registry::types::ToolConfig>,
) -> ccode_agent::Agent {
    test_agent_from_config(
        ccode_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        ccode_agent::AgentDefinition::default_ccode_build(),
        std::sync::Arc::new(ccode_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_user_message_template(
    template: ccode_agent::prompt::user_message::UserMessageTemplate,
) -> ccode_agent::Agent {
    let mut definition = ccode_agent::AgentDefinition::default_ccode_build();
...
async fn test_agent_from_config(
    ...
    definition: ccode_agent::AgentDefinition,
    backend: std::sync::Arc<dyn ccode_tools::computer::types::TerminalBackend>,
) -> ccode_agent::Agent {
    use ccode_tools::computer::local::LocalFs;
    use ccode_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("ccode-test"),
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<ccode_acp::AcpClientMessage>,
    ...
    tool_context.task_completion_reservations =
        Some(ccode_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(ccode_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: ccode_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
