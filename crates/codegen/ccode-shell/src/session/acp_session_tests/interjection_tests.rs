...
mod interjection_broadcast_tests {
    ...
    /// `handle_interjection` depends on: method `ccode.dev/session/interjection`
            ...
            .run_until(async {
                ...
                while let Ok(msg) = gateway_rx.try_recv() {
                    if let ccode_acp::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "ccode.dev/session/interjection"
                    {
                        payload =
                ...
                let payload = payload.expect("an ccode.dev/session/interjection broadcast");
                assert_eq!(
