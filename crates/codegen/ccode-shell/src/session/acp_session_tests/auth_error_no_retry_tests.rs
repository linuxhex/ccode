...
fn auth_manager_with_refresher(
    ...
    let am = Arc::new(AuthManager::new(dir.path(), CcodeComConfig::default()));
    am.hot_swap(CcodeAuth {
...
/// attribution.
fn auth_error() -> ccode_sampler::SamplingErrorInfo {
    ccode_sampler::SamplingErrorInfo {
        kind: ccode_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: ccode_chat::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        ccode_chat::AuthType::SessionToken => "cached_token",
        ccode_chat::AuthType::ApiKey => "ccode.api_key",
    };
...
/// `ApiKey` (session-token cache miss + `CCODE_API_KEY`).
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(ccode_chat::Credentials {
...
/// `CCODE_API_KEY`) must NOT attempt an OIDC session-token refresh. The
...
fn model_not_found_error() -> ccode_sampler::SamplingErrorInfo {
    ccode_sampler::SamplingErrorInfo {
            kind: ccode_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model ccode-build does not exist or your team does not have access".into(),
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `ccode logout`, got: {msg}"
            );
            assert!(
                msg.contains("ccode login"),
                "hint must mention `ccode login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> ccode_sampler::SamplingErrorInfo {
    ccode_sampler::SamplingErrorInfo {
            kind: ccode_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.ccode.dev/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_ccode_token_auth=ccode-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
fn session_token_auth_gate_truth_table() {
    ...
    // Session method + Unknown BYOK: refresh only against a first-party ccode
...
/// Negative: a genuine `ccode.api_key` method keeps its configured key on the
...
/// End-to-end for the frozen-gate bug: a session born on `ccode.api_key` (gate
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = ccode_sampler::SamplerConfig {
...
/// never ride a later request to `api.ccode.dev`. Mirrors the forward direction in
...
async fn switch_to_first_party_model_drops_minted_provider_token() {
    ...
    local
        .run_until(async {
            ...
            let (actor, _rx) =
                make_actor_with_auth_and_credentials(None, ccode_chat::AuthType::ApiKey, token)
                    .await;
            ...
            let cfg = ccode_sampler::SamplerConfig {
                ...
                base_url: "https://api.ccode.dev/v1".to_string(),
                model,
                max_completion_tokens: None,
                temperature: None,
                top_p: None,
...
async fn sampler_non_auth_kind_401_on_provider_model_still_recovers() {
    ...
    local
        .run_until(async {
            ...
            error.kind = ccode_sampler::SamplingErrorKind::Api;
