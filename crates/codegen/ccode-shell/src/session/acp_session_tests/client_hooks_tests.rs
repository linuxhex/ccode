...
fn install_client_hook(
    actor: &SessionActor,
    event: ccode_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
fn spawn_deny_responder(
    gateway_rx: tokio::sync::mpsc::UnboundedReceiver<ccode_acp::AcpClientMessage>,
    reason: &'static str,
) {
    ...
    tokio::task::spawn_local(async move {
        while let Some(msg) = gateway_rx.recv().await {
            match msg {
                ccode_acp::AcpClientMessage::ExtMethod(args) => {
                ...
                ccode_acp::AcpClientMessage::SessionNotification(args) => {
    ...
    local
        .run_until(async {
            ...
            actor.fire_hook(
                ccode_hooks::event::HookEventName::Stop,
                None,
                ccode_hooks::event::HookPayload::Stop {
            ...
            let ccode_acp::AcpClientMessage::ExtNotification(args) = msg else {
                panic!("expected an ccode.dev/hooks/event ext notification");
            };
            assert_eq!(args.request.method.as_ref(), "ccode.dev/hooks/event");
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                ccode_tools::registry::types::ToolConfig::for_tool::<
                    ccode_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        ccode_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
async fn post_tool_use_and_failure_never_double_fire() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_ccode_build_agent_with_todo().await;
                ...
                |rx: &mut tokio::sync::mpsc::UnboundedReceiver<ccode_acp::AcpClientMessage>| {
                    ...
                    while let Ok(msg) = rx.try_recv() {
                        if let ccode_acp::AcpClientMessage::ExtNotification(args) = msg
                            && args.request.method.as_ref() == "ccode.dev/hooks/event"
                        {
                            let params: serde_json::Value =
                                serde_json::from_str(args.request.params.get()).unwrap();
...
fn file_registry_with_stop_spec(
    event: ccode_hooks::event::HookEventName,
    script: &str,
) -> ccode_hooks::discovery::HookRegistry {
    let (mut registry, _) = ccode_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![ccode_hooks::config::HookSpec {
...
/// but still delivers the observe `ccode.dev/hooks/event` notification.
#[tokio::test(flavor = "current_thread")]
            ...
            *actor.hook_registry.borrow_mut() =
            ...
            tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    match msg {
                        ccode_acp::AcpClientMessage::ExtMethod(args) => {
                            if args.request.method.as_ref() == "ccode.dev/hooks/run" {
                                runs.set(runs.get() + 1);
                            }
                        ...
                        ccode_acp::AcpClientMessage::ExtNotification(args) => {
                            if args.request.method.as_ref() == "ccode.dev/hooks/event" {
...
/// `"subagent_stop"` on the wire, matching `CCODE_HOOK_EVENT`.
#[tokio::test(flavor = "current_thread")]
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                ccode_hooks::event::HookEventName::SubagentEnd,
                None,
                ccode_hooks::event::HookPayload::SubagentStop {
                    phase: ccode_hooks::event::SubagentStopPhase::Observe,
