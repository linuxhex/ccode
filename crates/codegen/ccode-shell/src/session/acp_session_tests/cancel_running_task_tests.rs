...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(ccode_sampler::SamplerConfig {
...
async fn cancel_running_task_teardown_clears_running_and_pending_work() {
    ...
    local
        .run_until(async {
            ...
            let cwd = AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
            let fs = Arc::new(
                ccode_workspace::file_system::MockFs::new(cwd.to_path_buf()),
            );
            let terminal = Arc::new(DummyTerminal {});
            let (hunk_tx, _hunk_rx) = tokio::sync::mpsc::unbounded_channel();
            let hunk_tracker_handle = ccode_hunk::HunkTrackerActor::spawn(
            ...
            agent
                .tool_bridge()
                .update_resource(
                    ccode_tools::implementations::ccode_build::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
...
/// user's next queued prompt) and rebroadcasts `ccode.dev/queue/changed`. The
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![ccode_sampling_types::ToolCall {
                    id: "call-1".into(),
    ...
    local
        .run_until(async {
            ...
            let cfg = ccode_sampler::SamplerConfig {
                ...
                temperature: None,
                top_p: None,
                api_backend: ccode_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                query_params: Default::default(),
                env_http_headers: Default::default(),
            ...
            let (sampler_event_tx, _sampler_event_rx) = tokio::sync::mpsc::unbounded_channel::<
                ccode_sampler::SamplingEvent,
            >();
            let sampler_handle = ccode_sampler::SamplerActor::spawn(
                cfg,
                ccode_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                ccode_acp::AcpClientMessage,
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(
                        ccode_sampling_types::UserItem {
                            content: vec![ccode_sampling_types::ContentPart::Text {
                                text: "hi".into(),
                            }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use ccode_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    ccode_sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
