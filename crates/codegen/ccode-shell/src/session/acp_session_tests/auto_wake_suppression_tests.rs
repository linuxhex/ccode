use super::support::*;
use super::*;
use ccode_tools::reminders::task_completion::consumed_completion_ids;
use ccode_tools::types::output::{BashOutput, TextOutput, ToolOutput};
...
async fn cancel_barrier_rejects_task_completion_wake_without_reporting_it() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                res.get::<ccode_tools::types::resources::State<
                    ccode_tools::reminders::task_completion::ReportedTaskCompletions,
                >>()
                .is_none(),
                "declined admission must not report before user re-engagement"
            );
            drop(res);
            let reminder = ccode_tools::reminders::TaskCompletionReminder;
            let reminders = ccode_tools::types::tool::Reminder::collect_reminders(
                &reminder,
                resources,
        ...
        .run_until(async {
            ...
            assert!(
                resources
                    .lock()
                    .await
                    .get::<ccode_tools::types::resources::State<
                        ccode_tools::reminders::task_completion::ReportedTaskCompletions,
                    >>()
                    .is_none(),
...
async fn genuine_user_start_consumes_deferred_completions_without_notification_turn() {
    ...
    local
        .run_until(async {
            ...
            let body = ccode_tools::reminders::task_completion::format_monitor_completion(
                &ccode_tools::types::TaskSnapshot {
                    ...
                    signal: None,
                    completed: true,
                    kind: ccode_tools::computer::types::TaskKind::Monitor,
                    block_waited: false,
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use ccode_tools::reminders::task_completion::ReportedTaskCompletions;
...
struct OneTaskTerminal {
    tasks: Vec<ccode_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl ccode_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: ccode_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        ccode_tools::computer::types::TerminalRunResult,
        ccode_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<ccode_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> ccode_tools::computer::types::KillOutcome {
        ccode_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<ccode_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<ccode_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> ccode_tools::computer::types::TaskSnapshot {
    ccode_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
