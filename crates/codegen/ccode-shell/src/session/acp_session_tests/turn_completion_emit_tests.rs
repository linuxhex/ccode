...
//! notification the real `send_ccode_notification` persists — not a
...
fn is_turn_completed(m: &PersistenceMsg) -> bool {
    matches!(
        m,
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(n))
            if matches!(n.update, CcodeSessionUpdate::TurnCompleted { .. })
    )
}
...
fn turn_completed_fields(msgs: &[PersistenceMsg]) -> Option<(String, String, Option<String>)> {
    msgs.iter().find_map(|m| match m {
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(n)) => match &n.update {
            ...
            *actor
            ...
            while let Ok(msg) = gateway_rx.try_recv() {
                if let ccode_acp::AcpClientMessage::ExtNotification(args) = msg
                    && args.request.method.as_ref() == "ccode.dev/session_notification"
                    && let Ok(v) =
