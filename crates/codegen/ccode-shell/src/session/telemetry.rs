use ccode_telemetry::events::SessionHarness;
use ccode_workspace::permission::Decision;
...
pub(crate) fn skill_source_label(skill_path: &str, cwd: &str) -> &'static str {
    ...
    let user = crate::util::ccode_home::ccode_home()
        .parent()
...
pub(crate) fn format_hook_name(spec: &ccode_hooks::config::HookSpec) -> String {
...
/// project dirs contain `/.ccode/`.)
fn format_hook_source(spec: &ccode_hooks::config::HookSpec) -> &'static str {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &ccode_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: ccode_telemetry::enums::PermissionMode,
    pub mcp_server_names: Vec<String>,
    pub lsp_server_names: Vec<String>,
    pub memory_enabled: bool,
    pub auto_update: Option<bool>,
    pub cwd: String,
    pub skills_config: ccode_agent::prompt::skills::SkillsConfig,
    /// Resolved vendor-compat config, so recorded skill / AGENTS.md names match
    /// what the session actually discovers.
    pub compat: ccode_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names = ccode_agent::prompt::agents_md::read_agents_config_with_paths(
            &self.cwd,
            self.compat,
        )
        .await
        ...
        .collect();
        let skill_names = ccode_agent::prompt::skills::list_skills_with_plugins(
            Some(&self.cwd),
            &self.skills_config,
        ...
        SessionHarness {
            ...
            is_git_repo: ccode_telemetry::context::collect_git_context(&self.cwd).is_git_repo,
