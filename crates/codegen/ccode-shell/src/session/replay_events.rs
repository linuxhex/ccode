...
use crate::extensions::notification::SessionNotification as CcodeSessionNotification;
...
///
/// One-shot ccode events (RetryState, ImageCompressed, HookExecution,
/// etc.) take the direct `send_ccode_notification` path for per-event hooks and persistence.
#[derive(Debug, Clone)]
pub(crate) enum SessionNotification {
    Acp(Box<AcpSessionNotification>),
    CcodeSession(Box<CcodeSessionNotification>),
}
...
impl SessionNotification {
    pub(crate) fn session_id(&self) -> &acp::SessionId {
        match self {
            ...
            Self::CcodeSession(n) => &n.session_id,
        }
    }
...
impl From<CcodeSessionNotification> for SessionNotification {
    fn from(n: CcodeSessionNotification) -> Self {
        Self::CcodeSession(Box::new(n))
    }
}
...
impl SessionNotification {
    ...
    pub(crate) fn expect_acp(&self) -> &AcpSessionNotification {
        match self {
            Self::Acp(n) => n,
            Self::CcodeSession(_) => panic!("expected Acp notification, got CcodeSession"),
        }
    }
