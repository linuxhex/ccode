...
use crate::extensions::notification::{
    SessionNotification as CcodeSessionNotification, SessionUpdate as CcodeSessionUpdate,
};
...
impl GoalNotifySender {
    ...
    /// latch updates that must not run the `send_ccode_notification`
    /// rewind-window-close side effect.
    pub(crate) fn send_update(&self, update: CcodeSessionUpdate) {
    ...
    fn dispatch_update(&self, update: CcodeSessionUpdate, persist: bool) {
...
/// drop its goal state.
pub(crate) fn build_goal_cleared() -> CcodeSessionUpdate {
    CcodeSessionUpdate::GoalUpdated {
        goal_id: String::new(),
...
mod tests {
    ...
    fn planning_field(update: &CcodeSessionUpdate) -> Option<bool> {
        match update {
            CcodeSessionUpdate::GoalUpdated { planning, .. } => *planning,
            _ => panic!("expected GoalUpdated"),
        }
    }
    ...
    fn build_goal_updated_verifying_flag_reflects_latch() {
        ...
        let mut o = make_base_orchestration();
        let verifying = |u: &CcodeSessionUpdate| match u {
            CcodeSessionUpdate::GoalUpdated {
                verifying_completion,
                ..
            } => *verifying_completion,
    ...
    fn build_goal_updated_gates_per_model_breakdown_on_two_or_more_models() {
        ...
        let mut o = make_base_orchestration();
        o.live_tokens_by_model = vec![("ccode-4".into(), 5_000)];
        ...
        o.live_tokens_by_model = vec![("ccode-4".into(), 5_000), ("ccode-3".into(), 3_000)];
        match build_goal_updated(&o, 0, 0) {
            CcodeSessionUpdate::GoalUpdated {
                live_tokens_by_model,
                ..
                ...
                live_tokens_by_model,
                vec![("ccode-4".to_owned(), 5_000), ("ccode-3".to_owned(), 3_000)]
            ),
            _ => panic!("expected GoalUpdated"),
        }
    }
