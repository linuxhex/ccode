...
//!
//! Receives [`ccode_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`ccode_mcp::liveness`]),
//! - the [`ccode_mcp::servers::CcodeClientHandler`] (server-pushed
...
//! `ccode.dev/mcp/server_status` and the payload schema defined by
...
pub const SERVER_STATUS_METHOD: &str = "ccode.dev/mcp/server_status";
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`ccode_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `ccode_com_` prefix)
    /// or `local` (user `.ccode/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`ccode_mcp::servers::ClientStateKind::Ready`]
...
/// [`ccode_mcp::servers::McpClientEventKind`]). Used as the
...
pub fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit ccode.dev"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && ccode_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///   `ConfigRemoved` keys,
/// - emit one ACP `ccode.dev/mcp/server_status` push per surviving
///   buffer entry, via the provided gateway.
///
/// `gateway` is a [`ccode_acp::AcpAgentGatewaySender`] (forwarded
...
/// 3. `flush_window` — emit ACP `ccode.dev/mcp/server_status` per
///    surviving entry.
...
mod tests {
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "ccode_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `ccode_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("ccode_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use ccode_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("ccode_com_slack"), // managed
    ...
    fn discard_gateway() -> ccode_acp::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        ccode_acp::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = ccode_acp::AcpAgentGatewaySender::new(gw_tx);
