...
pub enum SessionCommand {
    ...
    RestorePlanApproval,
    GetToolOverrides {
        respond_to: oneshot::Sender<Option<ccode_sampling_types::ToolOverrides>>,
    },
    ...
    SetToolOverrides {
        overrides: ccode_sampling_types::ToolOverrides,
    },
    Prompt {
        prompt_id: String,
        ...
        tool_overrides_update: Option<ccode_sampling_types::ToolOverridesUpdate>,
    ...
    /// rather than the agent-level default (e.g. `ccode-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-ccode-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<ccode_chat::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<ccode_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.ccode/hooks`,
    ...
    /// Out-of-band history repair (`ccode.dev/session/repair`): fix tool-pairing
    ...
    RepairHistory {
        dry_run: bool,
        respond_to:
            oneshot::Sender<anyhow::Result<ccode_chat::compaction_utils::HistoryRepairReport>>,
    },
    GetRewindPoints {
    ...
    /// ccode extension session notification - client-side events to store in persistence
    CcodeSessionNotification {
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, ccode_chat::UsageTotals)>,
    ...
    /// mid-session via the `ccode.dev/session/update_mcp_servers` extension method).
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<ccode_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<ccode_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<ccode_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<ccode_hooks_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: ccode_hooks_types::HooksAction,
        respond_to: oneshot::Sender<ccode_hooks_types::ActionOutcome>,
    },
    ...
    /// This session's plugin registry, as served by `ccode.dev/plugins/list`.
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<ccode_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// from the notification bridge, which does not go through `send_ccode_notification`).
    ...
    /// serialized mailbox; the rebroadcast of `ccode.dev/queue/changed` is the
    ...
    /// `ccode.dev/queue/changed` is the truth signal for every attached client.
    ...
    /// tool-free model call (default `ccode-build-0.1` when available via
    ...
    /// `ccode-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    Interject {
        ...
        /// `ccode.dev/session/interjection` so the originating pager can dedup its
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<ccode_chat::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<ccode_sampling_types::conversation::ConversationItem>>>,
    },
