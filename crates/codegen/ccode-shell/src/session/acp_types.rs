...
impl TokenUsageCategory {
    ...
    pub fn skills_listing(text: &str, skill_count: usize) -> Self {
        Self {
            ...
            tokens: ccode_tokens::estimate_tokens(text),
...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for ccode-build in remote settings).
...
impl ContextInfo {
    ...
    pub fn from_notification(used: u64, total: u64) -> Self {
        Self {
            used,
            total,
            usage_pct: ccode_tokens::usage_percentage_u8(used, total),
            free_tokens: ccode_tokens::free_tokens(total, used),
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `ccode-build`).
...
pub fn is_coding_model_slug(model: &str) -> bool {
    matches!(model, "ccode-build" | "ccode-4.5")
}
...
/// Full wire response for `ccode.dev/session/info`.
///
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "ccode-build"));
        assert!(should_show_model_fingerprint(false, "ccode-4.5"));
    ...
    fn desktop_client_type_deserializes_and_round_trips() {
        ...
        let submission = input.to_submission(Some("ccode-3".into()), None, None, Some(5));
        assert_eq!(
