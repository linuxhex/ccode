    ...
    impl ChatReducer {
        ...
        fn process(&mut self, update: &SessionUpdate) -> Vec<ConversationItem> {
            match update {
                ...
                SessionUpdate::Ccode(n) => self.handle_ccode_ext(&n.update),
            }
        }
        ...
        fn handle_ccode_ext(
            ...
            use crate::extensions::notification::SessionUpdate as CcodeUpdate;

            match update {
                CcodeUpdate::CompactionCheckpoint(_) => {
                    self.reset();
...
/// Method name for ccode extension session/update notifications.
pub(crate) const CCODE_SESSION_UPDATE_METHOD: &str = "_ccode.dev/session/update";

/// A unified session update that can be either an ACP notification or an ccode extension notification.
...
pub enum SessionUpdate {
    ...
    /// ccode extension session notification (e.g., diff_review)
    Ccode(Box<SessionNotification>),
}
        ...
        S: serde::Serializer,
    {
        ...
        match self {
            ...
            SessionUpdate::Ccode(notification) => {
                map.serialize_entry("method", CCODE_SESSION_UPDATE_METHOD)?;
                map.serialize_entry("params", notification)?;
            }
        }
        map.end()
...
pub(crate) struct SessionUpdateEnvelope {
    ...
    /// Either "session/update" for ACP or "_ccode.dev/session/update" for ccode extensions.
    pub method: String,
...
impl SessionUpdateEnvelope {
    ...
    pub(crate) fn from_update(update: &SessionUpdate) -> Result<Self, serde_json::Error> {
        ...
        match update {
            ...
            SessionUpdate::Ccode(notification) => Ok(Self {
                timestamp,
                method: CCODE_SESSION_UPDATE_METHOD.to_string(),
                params: serde_json::to_value(notification)?,
            }),
        }
    }
    ...
    pub(crate) fn into_update(self) -> Result<SessionUpdate, serde_json::Error> {
        if self.method == CCODE_SESSION_UPDATE_METHOD {
            let notification: SessionNotification = serde_json::from_value(self.params)?;
            Ok(SessionUpdate::Ccode(Box::new(notification)))
        } else {
    ...
    pub(crate) fn from_str(line: &str) -> Result<SessionUpdate, serde_json::Error> {
        ...
        if let Ok(envelope) = serde_json::from_str::<BorrowedEnvelope<'_>>(line) {
            ...
            return if envelope.method == Some(CCODE_SESSION_UPDATE_METHOD) {
...
pub struct PersistedData {
    ...
    /// All session updates (ACP updates and ccode extension updates) in chronological order
...
pub enum AppendCwdSwitchError {
    NotCommitted(io::Error),
    Committed {
        acknowledgement: ccode_chat::StrictAppendAck,
        source: io::Error,
    },
}
...
pub trait StorageAdapter: Send + Sync {
    ...
    /// Append a session update (ACP update or ccode extension update) and increment counter
    ...
    ) -> Result<ccode_chat::StrictAppendAck, AppendCwdSwitchError> {
...
pub(crate) fn filter_rewind_lines<'a>(lines: Vec<&'a str>) -> Vec<&'a str> {
    ...
    for line in &lines {
        let (raw_params, is_ccode) = if let Ok(env) = serde_json::from_str::<RawLinePeek<'_>>(line) {
            let raw = env.params.map(|p| p.get()).unwrap_or(line);
            let is_ccode = env.method == Some(CCODE_SESSION_UPDATE_METHOD);
            (raw, is_ccode)
        } else {
            (*line, false)
        };
...
pub fn filter_rewind_updates(updates: Vec<SessionUpdate>) -> Vec<SessionUpdate> {
    let has_rewinds = updates.iter().any(|u| {
        matches!(
            u,
            SessionUpdate::Ccode(n) if matches!(
                n.update,
    ...
    for update in updates {
        ...
        if let SessionUpdate::Ccode(ref n) = update
...
/// Returns only `SessionUpdate::Acp` updates (ccode-extension updates like
...
/// Like [`load_updates_for_replay`], but resolves the session under a specific ccode home.
pub fn load_updates_for_replay_at(
    session_id: &str,
    ccode_home: &std::path::Path,
) -> std::io::Result<Option<Vec<acp::SessionUpdate>>> {
    let sessions_root = ccode_home.join("sessions");
    let Some(session_dir) =
...
pub enum PromptExtractEvent {
    ...
    /// A `RewindMarker` ccode update: truncate accumulated prompts to this index.
    ///
...
/// - ccode `"rewind_marker"`      → `update.target_prompt_index`
...
/// Peek inside ACP or ccode `params` to read the `update.sessionUpdate` tag and
...
pub(crate) fn parse_prompt_extract_event(line: &str) -> PromptExtractEvent {
    ...
    if !is_ccode && tag == *USER_MESSAGE_CHUNK {
...
mod tests {
    ...
    /// Wrap a ccode notification as the envelope stored in updates.jsonl.
    fn ccode_envelope(session_update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_ccode.dev/session/update","params":{{"sessionId":"s","update":{session_update_json}}}}}"#
        )
    }
    ...
    #[test]
    fn ccode_rewind_marker_yields_rewind_to() {
        let line = ccode_envelope(
    ...
    #[test]
    fn ccode_rewind_to_zero_yields_rewind_to_zero() {
        let line = ccode_envelope(
    ...
    #[test]
    fn ccode_diff_review_yields_not_user() {
        let line = ccode_envelope(r#"{"sessionUpdate":"diff_review","content":[]}"#);
        assert_eq!(
            parse_prompt_extract_event(&line),
            PromptExtractEvent::NotUserMessage
        );
    ...
    /// A valid JSON object that has no recognisable ACP/ccode shape — NotUserMessage.
    #[test]
    ...
    fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
        ...
        // ccode-style line persisted by an older binary: no _meta at all.
        let old_ccode = r#"{"timestamp":2,"method":"_ccode.dev/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"}}}"#;
        let raw = format!("{a1}\n{old_ccode}\n");
        ...
        let new_ccode = r#"{"timestamp":2,"method":"_ccode.dev/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"},"_meta":{"eventId":"ev2"}}}"#;
        let raw = format!("{a1}\n{new_ccode}\n");
        let prepared = prepare_replay_lines(&raw, Some("ev1"));
        assert!(!prepared.mark_replay);
        assert_eq!(prepared.lines.len(), 1);
    ...
    fn fast_reject_rewind_marker_in_content() {
        ...
        // (b) an ACP (non-ccode-ext) update carrying rewind_marker in content is NOT a
        // real ccode-ext rewind_marker → NotUserMessage (no RewindTo).
    ...
    fn prepare_replay_reports_spawn_without_finish() {
        let spawn = |id: &str, child: &str| {
            format!(
                r#"{{"method":"_ccode.dev/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_spawned","subagent_id":"{id}","parent_session_id":"s","child_session_id":"{child}","subagent_type":"general-purpose","description":"task"}},"_meta":{{"eventId":"s-1"}}}}}}"#
            )
        };
        let finish = |id: &str| {
            format!(
                r#"{{"method":"_ccode.dev/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_finished","subagent_id":"{id}","child_session_id":"c{id}","status":"completed","tool_calls":0,"turns":0,"duration_ms":0}},"_meta":{{"eventId":"s-2"}}}}}}"#
            )
        };
    ...
    #[test]
    fn from_str_unknown_ccode_variant_deserializes_via_envelope() {
        ...
        let line = ccode_envelope(r#"{"sessionUpdate":"git_branch_update","branch":"main"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Ccode(notif) => {
                assert_eq!(
                    notif.update,
                    crate::extensions::notification::SessionUpdate::Unknown
                );
            }
            SessionUpdate::Acp(_) => panic!("expected Ccode variant"),
        }
    }

    #[test]
    fn from_str_known_ccode_variant_still_works() {
        let line = ccode_envelope(r#"{"sessionUpdate":"memory_flush_started"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Ccode(notif) => {
                assert_eq!(
