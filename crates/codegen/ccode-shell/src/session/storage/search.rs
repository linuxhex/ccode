...
//! other concurrently running ccode processes (which may wipe or downgrade
...
use super::{
    ...
    CCODE_SESSION_UPDATE_METHOD, collect_prompts_from_events,
};
...
/// User-configurable overrides via `~/.ccode/settings.json` are deferred
/// to a follow-up.
...
///
/// TODO: When multiple ccode processes run concurrently, they each have
...
fn search_db_path(root_dir: &Path) -> PathBuf {
    ...
    ccode_journal::JournalMode::for_db_path(&path).effective_db_path(&path)
}
...
fn collect_all_indexable_content_single_pass(updates_path: &Path) -> io::Result<(String, u64)> {
    ...
    for line_result in reader.lines() {
        ...
        let (raw_params, is_ccode) = if let Ok(env) = serde_json::from_str::<RawLinePeek<'_>>(trimmed)
        {
        ...
        // Control events (rewind markers) come from ccode extensions
        // ("_ccode.dev/session/update"). Dispatch on source first, then tag.
        if !is_ccode {
            // ── ACP content events ──────────────────────────────────
            match tag {
...
fn collect_delta_content(updates_path: &Path, offset: u64) -> io::Result<DeltaResult> {
    ...
    for line_result in reader.lines() {
        ...
        match tag {
            Some(t) if is_ccode && t == *REWIND_MARKER => {
                return Ok(DeltaResult::NeedsFullReread);
            }
            Some(t) if !is_ccode && t == *USER_MESSAGE_CHUNK => {
                flush_assistant(&mut current_assistant, &mut assistant_texts);
                if let Ok(peek) = serde_json::from_str::<UserContentPeek<'_>>(raw_params)
                    && let Some(content) = peek.update.content
                    && content.content_type == Some("text")
            ...
            Some("agent_message_chunk") if !is_ccode => {
...
mod tests {
    ...
    fn ccode_update(session_update_json: &str) -> String {
        format!(
