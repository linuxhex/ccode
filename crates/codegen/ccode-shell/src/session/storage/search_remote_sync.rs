...
///
/// Parsed from `[session_search.remote_sync]` in `~/.ccode/config.toml`.
/// Default: disabled.
...
pub async fn maybe_upload_index(
    ...
    gcs_config: ccode_fs::TraceExportConfig,
...
impl ProxyHttpClient for crate::upload::gcs::TraceExportConfigWithAuth {
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        <Self as ccode_fs::gcs::StorageConfig>::proxy_http_client(self)
    }
}
