...
async fn write_compaction_segment_numbers_and_indexes_resume_safely() {
    ...
    let seg = |summary: &str| CompactionSegmentFile {
        ...
        detail: ccode_chat::CompactionDetail::Verbose,
    ...
    let base = adapter
        .session_dir(&info)
        .join(ccode_chat::compaction_transcript::COMPACTION_DIR);
...
async fn test_jsonl_round_trip() {
    ...
    let new_model = acp::ModelId::new("ccode-4.3");
...
async fn load_session_without_updates_defers_rewind_points() {
    use ccode_workspace::session::file_state::RewindPoint;
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use ccode_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
#[tokio::test]
async fn test_ccode_session_update_round_trip() {
    use crate::extensions::notification::{
        DiffContent, SessionNotification as CcodeSessionNotification,
        SessionUpdate as CcodeSessionUpdateType,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let ccode_notification = CcodeSessionNotification {
        session_id: acp::SessionId::new("test-session-123"),
        update: CcodeSessionUpdateType::DiffReview {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Ccode(Box::new(ccode_notification.clone())))
        .await
        .unwrap();
    ...
    assert_eq!(
            loaded.updates.len(),
            2,
            "Should have 2 updates (1 ccode + 1 ACP)"
        );
    match &loaded.updates[0] {
        SessionUpdate::Ccode(notification) => {
            assert_eq!(notification.session_id.0.as_ref(), "test-session-123");
            match &notification.update {
                CcodeSessionUpdateType::DiffReview { content } => {
        ...
        _ => panic!("Expected ccode update as first item"),
    }
...
async fn test_subagent_notifications_round_trip() {
    use crate::extensions::notification::{
        SessionNotification as CcodeSessionNotification,
        SessionUpdate as CcodeSessionUpdateType,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let spawned = CcodeSessionNotification {
        session_id: acp::SessionId::new("parent-session"),
        update: CcodeSessionUpdateType::SubagentSpawned {
    ...
    adapter.append_update(&info, &SessionUpdate::Ccode(Box::new(spawned))).await.unwrap();
    let finished = CcodeSessionNotification {
        session_id: acp::SessionId::new("parent-session"),
        update: CcodeSessionUpdateType::SubagentFinished {
            subagent_id: "child-001".to_string(),
            child_session_id: "child-001".to_string(),
    ...
    adapter.append_update(&info, &SessionUpdate::Ccode(Box::new(finished))).await.unwrap();
    let loaded = adapter.load_session(&info).await.unwrap();
    assert_eq!(loaded.updates.len(), 2);
    match &loaded.updates[0] {
        SessionUpdate::Ccode(notification) => {
            match &notification.update {
                CcodeSessionUpdateType::SubagentSpawned {
                    subagent_id,
                    child_session_id,
                    description,
                    subagent_type,
        ...
        other => panic!("Expected Ccode update, got {other:?}"),
    }
    match &loaded.updates[1] {
        SessionUpdate::Ccode(notification) => {
            match &notification.update {
                CcodeSessionUpdateType::SubagentFinished {
                    subagent_id,
                    status,
                    tool_calls,
                    turns,
    ...
    assert_eq!(spawned_json["method"], "_ccode.dev/session/update");
    ...
    assert_eq!(finished_json["method"], "_ccode.dev/session/update");
...
fn checkpoint_record_with_path(id: &str, checkpoint_file: &str) -> SessionUpdate {
    use crate::extensions::notification::{
        CompactionCheckpointInfo, SessionNotification as CcodeSessionNotification,
        SessionUpdate as CcodeSessionUpdateType,
    };
    SessionUpdate::Ccode(
        Box::new(CcodeSessionNotification {
            session_id: acp::SessionId::new("ckpt-src"),
            update: CcodeSessionUpdateType::CompactionCheckpoint(
...
#[tokio::test]
async fn test_copy_session_data_transforms_ccode_updates() {
    use crate::extensions::notification::{
        DiffContent, SessionNotification as CcodeSessionNotification,
        SessionUpdate as CcodeSessionUpdateType,
    };
    ...
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let source_info = Info {
        id: acp::SessionId::new("source-ccode"),
        cwd: "/source".to_string(),
    };
    adapter.init_session(&source_info, default_model_id()).await.unwrap();
    let ccode_notification = CcodeSessionNotification {
        session_id: acp::SessionId::new("source-ccode"),
        update: CcodeSessionUpdateType::DiffReview {
            content: vec![DiffContent {
                    diff: acp::Diff::new(
                        std::path::PathBuf::from("/test/file.rs"),
    ...
    adapter
        .append_update(&source_info, &SessionUpdate::Ccode(Box::new(ccode_notification)))
        .await
        .unwrap();
    let target_info = Info {
        id: acp::SessionId::new("fork-source-ccode-abcd1234"),
...
async fn test_copy_session_data_with_model_override() {
    ...
    let options = CopySessionOptions {
        ...
        new_model_id: Some("ccode-3".to_string()),
        target_prompt_index: None,
        ..Default::default()
    };
    adapter.copy_session_data(&source_info, &target_info, options).await.unwrap();
    let loaded = adapter.load_session(&target_info).await.unwrap();
    assert_eq!(loaded.summary.current_model_id.0.as_ref(), "ccode-3");
    assert_eq!(
            loaded.summary.parent_session_id,
...
async fn test_load_prompts_only_applies_rewind_truncation() {
    use crate::extensions::notification::{
        SessionNotification as CcodeNotification, SessionUpdate as CcodeSessionUpdate,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = Info {
    ...
    let rewind = CcodeNotification {
        session_id: info.id.clone(),
        update: CcodeSessionUpdate::RewindMarker {
    ...
    for update in [
        ...
        SessionUpdate::Ccode(Box::new(rewind)),
...
async fn test_append_feedback_creates_file_and_persists() {
    ...
    let user_entry = LocalFeedbackEntry::UserFeedback(UserFeedbackEntry {
        ...
        submission: Some(FeedbackSubmission {
            ...
            rating_value: Some(1),
            model_id: Some("ccode-3-fast".into()),
            resolved_model_id: Some("ccode-4.5".into()),
            ..Default::default()
        }),
    });
...
fn fork_filter_removes_synthetic_user_messages() {
    use ccode_sampling_types::conversation::*;
    let mut items = vec![
    ...
    if !fork_filter {
        ...
        assert!(matches!(
                adapter
                    ...
                    .await
                    .unwrap(),
                ccode_chat::StrictAppendAck::AlreadyPresent(item)
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    ccode_sandbox::set_configured_profile("workspace");
    let expected = ccode_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_turn_with_reasoning_between_user_and_assistant() {
    ...
    let mut items = vec![
            ConversationItem::system("sys"),
            ConversationItem::user("q"),
            ConversationItem::Reasoning(ccode_sampling_types::synthesized_reasoning_item(
                "thinking",
            )),
...
fn fork_filter_keeps_multi_tool_turn_with_reasoning_between_results() {
    ...
    let mut items = vec![
            ...
            ConversationItem::Reasoning(ccode_sampling_types::synthesized_reasoning_item("mid")),
...
fn scan_session_dirs_finds_all_sessions() {
    ...
    let cwd = crate::util::ccode_home::encode_cwd_dirname("/home/user/project");
...
fn scan_session_dirs_filters_by_cwd() {
    let tmp = TempDir::new().unwrap();
    let now = chrono::Utc::now();
    let cwd_a = crate::util::ccode_home::encode_cwd_dirname("/home/user/project-a");
    let cwd_b = crate::util::ccode_home::encode_cwd_dirname("/home/user/project-b");
...
fn scan_session_dirs_skips_non_directory_entries() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::ccode_home::encode_cwd_dirname("/project");
...
async fn list_sessions_recent_returns_most_recent_by_mtime() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::ccode_home::encode_cwd_dirname("/workspace");
...
async fn list_sessions_sorts_by_last_active_at_over_updated_at() {
    ...
    let cwd = crate::util::ccode_home::encode_cwd_dirname(cwd_path);
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd,
...
async fn list_sessions_recent_sorts_by_updated_at() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::ccode_home::encode_cwd_dirname("/ws");
    let now = chrono::Utc::now();
...
async fn list_sessions_recent_spans_multiple_workspaces() {
    let tmp = TempDir::new().unwrap();
    let cwd_a = crate::util::ccode_home::encode_cwd_dirname("/project-a");
    let cwd_b = crate::util::ccode_home::encode_cwd_dirname("/project-b");
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd_a,
...
/// path end-to-end (loader + serde + ccode_sampling_types::
...
/// the legacy ccode-build / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"ccode-build"}"#,
        ],
    );
    assert_eq!(
            items.len(),
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let ccode_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"ccode-build"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"ccode-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    let ccode_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"ccode-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"ccode-build"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();
...
async fn retry_after_lost_ack_converges_memory_and_disk_to_authoritative_item() {
            ...
            let result = storage
                ...
                .map_err(|error| match error {
                    crate::session::storage::AppendCwdSwitchError::NotCommitted(
                        error,
                    ) => ccode_chat::StrictAppendError::NotCommitted(error),
                    crate::session::storage::AppendCwdSwitchError::Committed {
                        acknowledgement,
                        source,
                    } => {
                        ccode_chat::StrictAppendError::Committed {
                            acknowledgement,
                            source,
                        }
                    }
    ...
    let chat = ccode_chat::ChatStateActor::spawn(
        vec![],
    ...
    assert!(matches!(
            ...
            .await,
            Err(ccode_chat::StrictAppendError::Indeterminate(_))
        ));
...
async fn acknowledged_chat_append_preserves_existing_file_bytes_and_appends_once() {
    ...
    assert!(matches!(
            adapter
                ...
                .await
                .unwrap(),
            ccode_chat::StrictAppendAck::Appended
        ));
