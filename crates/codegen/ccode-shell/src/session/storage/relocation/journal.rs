...
impl RelocationJournal {
    ...
    pub(super) fn validate(&self, ccode_home: &Path) -> Result<()> {
        ...
        if session_dir_at(ccode_home, &self.source_cwd, &self.session_id)
            == session_dir_at(ccode_home, &self.target_cwd, &self.session_id)
        {
            return Err(RelocationError::Inconsistent(
...
pub(super) fn acquire(ccode_home: &Path, session_id: &str) -> Result<RelocationLease> {
    validate_component("session id", session_id)?;
    let dir = relocation_dir(ccode_home);
...
pub(super) fn read(ccode_home: &Path, session_id: &str) -> Result<RelocationJournal> {
    validate_component("session id", session_id)?;
    let path = journal_path(ccode_home, session_id);
...
pub(super) fn write(
    ...
    let path = journal_path(ccode_home, &journal.session_id);
...
pub(super) fn relocation_dir(ccode_home: &Path) -> PathBuf {
    ccode_home.join(RELOCATION_DIR)
}

pub(super) fn journal_path(ccode_home: &Path, session_id: &str) -> PathBuf {
    relocation_dir(ccode_home).join(format!("{session_id}.json"))
}

pub(super) fn session_dir_at(ccode_home: &Path, cwd: &str, session_id: &str) -> PathBuf {
    ccode_home
        .join("sessions")
...
pub(super) fn sync_namespace(ccode_home: &Path) -> Result<()> {
    let dir = relocation_dir(ccode_home);
