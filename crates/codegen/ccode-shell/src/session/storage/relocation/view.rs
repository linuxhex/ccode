...
impl RelocationView {
    pub(crate) fn load(ccode_home: &Path) -> Result<Self> {
        Self::load_for_sessions_root(&ccode_home.join("sessions"))
    }
    pub(crate) fn journal_ids(ccode_home: &Path) -> Result<Vec<String>> {
        Ok(load_journals(ccode_home)?.into_keys().collect())
    }
    pub(crate) fn load_for_sessions_root(sessions_root: &Path) -> Result<Self> {
        let ccode_home = sessions_root
            .parent()
            .ok_or_else(|| RelocationError::Inconsistent("sessions root has no parent".into()))?;
        let journals = load_journals(ccode_home)?;
        let (all_candidates, persisted_candidates) = load_candidates(sessions_root)?;
        Ok(Self {
            ccode_home: ccode_home.into(),
    ...
    fn select(
        ...
        let selected = if let Some(relocation) = self.journals.get(session_id) {
            let expected =
                journal::session_dir_at(&self.ccode_home, authoritative_cwd(relocation), session_id);
            let path = self
                .all_candidates
                .get(session_id)
            ...
            RelocationStorage::new(self.ccode_home.clone())
...
fn load_journals(ccode_home: &Path) -> Result<HashMap<String, RelocationJournal>> {
    let dir = journal::relocation_dir(ccode_home);
    ...
    for entry in entries {
        ...
        journals.insert(session_id.to_owned(), journal::read(ccode_home, session_id)?);
    }
    Ok(journals)
}
