...
impl RelocationStorage {
    pub(crate) fn new(ccode_home: PathBuf) -> Self {
        Self {
            ccode_home,
            #[cfg(test)]
            fault: None,
    ...
    #[cfg(test)]
    fn with_fault(ccode_home: PathBuf, fault: TestFault) -> Self {
        Self {
            ccode_home,
            fault: Some(fault),
        }
    ...
    pub(crate) fn acquire(&self, session_id: &str) -> Result<RelocationLease> {
        journal::acquire(&self.ccode_home, session_id)
    }

    pub(crate) fn read_journal(&self, session_id: &str) -> Result<RelocationJournal> {
        journal::read(&self.ccode_home, session_id)
    }

    fn write_journal(&self, journal: &RelocationJournal) -> std::result::Result<(), WriteFailure> {
        journal::write(&self.ccode_home, journal, self.journal_fault(journal.phase))
    }
    ...
    pub(crate) fn sync_journal_namespace(&self) -> Result<()> {
        ...
        journal::sync_namespace(&self.ccode_home)
    }
    ...
    pub(crate) fn stage_and_publish(
        ...
        journal.validate(&self.ccode_home)?;
        let source =
            journal::session_dir_at(&self.ccode_home, &journal.source_cwd, &journal.session_id);
    ...
    pub(crate) fn recover_all(&self) -> Result<()> {
        for session_id in RelocationView::journal_ids(&self.ccode_home)? {
    ...
    pub(crate) fn finalize_terminal(
        ...
        proof.journal.validate(&self.ccode_home)?;
        ...
        let path = journal::journal_path(&self.ccode_home, &lease.session_id);
    ...
    fn finish_rollback(&self, journal: &mut RelocationJournal) -> Result<TerminalRelocation> {
        ...
        let result = (|| {
            ...
            self.remove_transaction_directory(&self.staging_dir(journal))?;
            let target =
                journal::session_dir_at(&self.ccode_home, &journal.target_cwd, &journal.session_id);
            if target.exists() {
                self.validate_target(journal)?;
            }
    ...
    fn validate_request(&self, lease: &RelocationLease, request: &RelocationRequest) -> Result<()> {
        ...
        if journal::session_dir_at(&self.ccode_home, &request.source_cwd, &request.session_id)
            == journal::session_dir_at(&self.ccode_home, &request.target_cwd, &request.session_id)
        {
            return Err(RelocationError::Inconsistent(
    ...
    pub(super) fn validate_authoritative_dir(
        ...
        if path
            != journal::session_dir_at(&self.ccode_home, expected_cwd, &journal.session_id).as_path()
        {
            return Err(RelocationError::Inconsistent(
    ...
    fn target_parent(&self, cwd: &str) -> PathBuf {
        self.ccode_home
            .join("sessions")
            .join(ccode_config::encode_cwd_dirname(cwd))
    }

    fn ensure_target_parent(&self, cwd: &str) -> Result<PathBuf> {
        let sessions = self.ccode_home.join("sessions");
        fs::create_dir_durable(&sessions)?;
        let encoded = ccode_config::encode_cwd_dirname(cwd);
    ...
    fn staging_dir(&self, journal: &RelocationJournal) -> PathBuf {
        self.ccode_home
            .join("sessions")
            .join(ccode_config::encode_cwd_dirname(&journal.target_cwd))
