...
use crate::extensions::notification::SessionUpdate as CcodeSessionUpdate;
use crate::extensions::notification::{
    RetryState, SessionNotification as CcodeSessionNotification, is_reauthable_failure,
};
...
use ccode_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use ccode_agent::prompt::skills::SkillsConfig;
use ccode_sampler::SamplerConfig as SamplingConfig;
use ccode_sampling_types::truncate_bytes;
...
use ccode_tools::types::compat::CompatConfig;
use ccode_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use ccode_workspace::file_system::CodebaseIndexManager;
use ccode_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use ccode_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
pub(crate) use spawn::*;
/// Client-registered hook gates (the `ccode.dev/hooks/run` reverse request).
mod hooks;
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<ccode_sampling_types::ToolOverridesUpdate>,
...
impl ccode_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        ccode_tools::types::resources::ManagedGatewayToolCallResponse,
    ...
    > {
        let auth_key = self
            ...
            .ok_or_else(|| ccode_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                ccode_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                ccode_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = ccode_tool_protocol::ToolId::new(caller)
                    .unwrap_or_else(|_| ccode_tool_protocol::ToolId::new("use_tool").expect("valid"));
                ccode_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            ccode_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            ccode_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, ccode_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, ccode_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, ccode_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, ccode_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `ccode-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<ccode_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: ccode_chat::ChatStateHandle,
    ...
    pub(crate) tool_overrides: std::cell::RefCell<Option<ccode_sampling_types::ToolOverrides>>,
    ...
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<ccode_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<ccode_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<ccode_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-ccode-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<ccode_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<ccode_fs::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<ccode_agent::Agent>,
    /// Dedup slot for `ccode.dev/git_head_changed`, shared with the fs-watch
    /// `GitHead` consumer (see `git_head_dedup_key`).
    pub(crate) last_reported_branch: Arc<parking_lot::Mutex<Option<String>>>,
    /// Client opted into `ccode.dev/gitHeadChanged`. When false (headless/SDK),
    ...
    /// Kill-switch (`CCODE_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<ccode_tools::implementations::ccode_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, ccode_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: ccode_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<ccode_hooks::discovery::HookRegistry>>>,
    /// Client hooks from `session/new` `_meta["ccode.dev/hooks"]`; gated in
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<ccode_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<ccode_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: ccode_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `ccode-sampler` actor.
    ///
    ...
    /// (system prompt, [`ccode_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: ccode_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Fire-and-forget — failures are logged but do not interrupt the turn.
    async fn send_after_turn_event(&self, payload: ccode_tool_protocol::turn_hook::AfterTurnPayload) {
        self.workspace_ops
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == ccode_tools::implementations::ccode_build::SCHEDULER_CREATE_TOOL_NAME
            }),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == ccode_tools::implementations::ccode_build::workflow::WORKFLOW_TOOL_NAME
            }),
    ...
    async fn send_feedback_notification(&self, request: crate::session::feedback::FeedbackRequest) {
        self.send_ccode_notification(CcodeSessionUpdate::FeedbackRequest(request.into()))
            .await;
    }
}
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<ccode_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `ccode prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &ccode_agent::PromptContext) {
...
) -> Option<ccode_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use ccode_tools::types::output::{MCPOutput, ToolOutput};
    use ccode_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl ccode_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl ccode_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> ccode_tool_protocol::ToolId {
            ccode_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::ccode_tool_runtime::ListToolsContext,
        ) -> ccode_tool_types::ToolDescription {
            ccode_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            _ctx: ccode_tool_runtime::ToolCallContext,
            _args: serde_json::Value,
        ) -> Result<ToolOutput, ccode_tool_runtime::ToolError> {
...
mod tool_meta_stamp_tests {
    //! Pin the `ccode.dev/tool` stamps on the harness emission paths: the early
    ...
    use super::*;
    use tokio::sync::mpsc;
    use ccode_tools::registry::types::ToolConfig;
    use ccode_tools::tool_taxonomy::TOOL_META_KEY;
    use ccode_workspace::permission::PermissionCommand;
    ...
    /// The `ccode.dev/tool` object from an event's `_meta`, if present.
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "CcodeBuild:read_file".to_string(),
                    )])
                    .await,
                );
                ...
                let t = tool_meta(early.as_ref()).expect("early ToolCall carries ccode.dev/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["namespace"], "ccode_build");
                assert!(t.get("input").is_none(), "identity-only before parse");
                let refined = refined.expect("refinement ToolCallUpdate emitted");
                let t = tool_meta(refined.as_ref()).expect("refinement carries ccode.dev/tool");
        ...
        local
            .run_until(async {
                ...
                let t = tool_meta(update.meta.as_ref())
                    .expect("permission-request ToolCallUpdate carries ccode.dev/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["input"]["path"], "/tmp/stamp.txt");
            })
...
mod managed_gateway_tool_tests {
    ...
    use ccode_tools::types::output::{MCPOutput, ToolOutput};
    use ccode_tools::types::tool::{ToolKind, ToolNamespace};
    use ccode_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<ccode_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
