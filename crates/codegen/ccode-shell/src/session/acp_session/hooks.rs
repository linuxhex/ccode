...
//!
//! Hooks registered at `session/new` (`_meta["ccode.dev/hooks"]`) come in two flavors,
//! both matched by the agent ([`ccode_hooks::matcher::HookMatcher`], shared with
//! file hooks):
//! - **Gates** (awaited reverse *requests* `ccode.dev/hooks/run`):
...
//! - **All other events**: fire-and-forget *notifications* `ccode.dev/hooks/event`,
...
use ccode_telemetry::events::ClientHookGateOutcome;
...
const HOOK_EVENT_METHOD: &str = "ccode.dev/hooks/event";
const HOOK_RUN_METHOD: &str = "ccode.dev/hooks/run";
...
/// Outcome of the `ccode.dev/hooks/run` reverse request, before interpreting it as a
    ...
    match outcome {
        ReverseOutcome::Responded(raw) => {
            match serde_json::from_str::<ClientHookResponse>(raw.get()) {
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "ccode.dev/hooks/run returned an unknown decision value; failing open"
                            );
                ...
                Err(err) => {
                    tracing::warn!(%err, "malformed ccode.dev/hooks/run response; failing open");
                    (
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "ccode.dev/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("ccode.dev/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| {
            ccode_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value)
        })
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::HookBlocked {
    ...
    /// Fan one `ccode.dev/hooks/run` gate dispatch out to every matching callback,
    ...
    fn client_gate_responses<'a>(
        ...
        let default_timeout =
            if envelope.hook_event_name.traits().gate == ccode_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                ccode_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    ccode_telemetry::session_ctx::log_event(
                        ccode_telemetry::events::ClientHookGate {
    ...
    /// `ccode.dev/hooks/run` once per matching callback with the shared `envelope` (the
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> ccode_hooks::dispatcher::StopDispatchResult {
        use ccode_hooks::result::HookRunResult;

        let mut out = ccode_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed)) in ordered {
            ...
            let detail = ccode_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                ccode_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
    ...
    /// Issue one `ccode.dev/hooks/run` reverse request, bounded by a per-callback `timeout`.
    ...
    /// `ccode.dev/hooks/event` notification to each matching registered callback.
