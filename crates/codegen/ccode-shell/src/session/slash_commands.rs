...
use ccode_tools::implementations::skills::skill::format_skill_name;
...
// ── ccode.dev/commands/list ext method ────────────────────────────────
...
///   matching today's Ccode Build flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use ccode_tools::implementations::skills::skill::{
...
/// `ccode-tools` so it stays identical to the pager's `LoopCommand` and the
...
mod tests {
    ...
    fn listing(name: &str) -> crate::session::workflow::registry::WorkflowListing {
        crate::session::workflow::registry::WorkflowListing {
            ...
            when_to_use: None,
            source: "project",
            path: Some(format!(".ccode/workflows/{name}.rhai")),
        }
    }
