...
use crate::extensions::notification::{SessionNotification, SessionUpdate as CcodeSessionUpdate};
...
pub enum PendingKind {
    ...
    Question,
    /// `ccode.dev/exit_plan_mode` plan approval.
    PlanApproval,
}

/// Whether a blocking plan-approval reverse-request is parked in `pending`.
///
/// The resume re-park issues `ccode.dev/exit_plan_mode` from a detached task
...
fn broadcast(gateway: &GatewaySender, session_id: &acp::SessionId, update: CcodeSessionUpdate) {
...
impl PendingInteractionGuard {
    ...
    pub fn new(
        ...
        broadcast(
            &gateway,
            &session_id,
            CcodeSessionUpdate::PendingInteraction {
    ...
    fn drop(&mut self) {
        ...
        if removed {
            broadcast(
                &self.gateway,
                &self.session_id,
                CcodeSessionUpdate::InteractionResolved {
