...
/// Memory log target — matches `ccode_telemetry::memory_log::TARGET`.
const LOG: &str = "ccode_memory";
...
pub fn should_flush(
    ...
    let should = ccode_tokens::exceeds_threshold_with_headroom(
        total_tokens,
        context_window,
...
pub fn is_duplicate(content: &str, db_path: &std::path::Path) -> bool {
    ...
    let conn = match ccode_journal::JournalMode::for_db_path(db_path).open_readonly(db_path) {
        Ok(c) => c,
        Err(_) => {
