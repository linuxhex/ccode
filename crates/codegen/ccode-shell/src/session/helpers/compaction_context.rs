...
//! shell-specific types (`ccode_tools::MemoryBackend`, memory context).
//!
//! The core [`CompactionStateContext`] struct and its builder live in
//! `ccode_chat::compaction_utils`. This module adds system-reminder
//! rendering that requires dependencies not available in `ccode-chat`.
//!
//! The three **common** active-agent sections (background tasks, TODO list,
//! running subagents) are formatted by
//! [`ccode_compaction::reminder`] so ccode-chat and ccode-build stay in lockstep.
...
pub fn to_system_reminder_sync(
    ...
    skills: &[ccode_tools::implementations::skills::types::SkillInfo],
...
pub async fn to_system_reminder(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[ccode_tools::implementations::skills::types::SkillInfo],
    memory_backend: Option<&dyn ccode_tools::types::memory_backend::MemoryBackend>,
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
...
fn to_system_reminder_inner(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[ccode_tools::implementations::skills::types::SkillInfo],
    memory_results: &[ccode_tools::types::memory_backend::MemorySearchResult],
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
    ...
    if let Some(listing) =
        ccode_tools::types::skill_discovery_tracker::format_compaction_skill_listing(skills)
    {
    ...
    if !ctx.connected_mcp_servers.is_empty() {
        use ccode_tools::implementations::search_tool::format_compaction_server_line;
