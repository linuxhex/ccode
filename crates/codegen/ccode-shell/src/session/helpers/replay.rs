...
pub fn find_latest_compaction_checkpoint(
    ...
    for line in raw_contents.lines() {
        ...
        if env.method != Some("_ccode.dev/session/update") {
            continue;
        }
        ...
        >(raw_params.get())
            && let CcodeSessionUpdate::CompactionCheckpoint(info) = notification.update
        {
            latest = Some(*info);
        }
    }
        ...
        match update {
            SessionUpdate::Ccode(notification) => {
                match &notification.update {
                    CcodeSessionUpdate::CompactionCheckpoint(info) => {
                        return self.handle_checkpoint(info, session_dir);
                    }
                    CcodeSessionUpdate::RewindMarker {
                        target_prompt_index,
                        ..
                    } => {
                    ...
                    // Other ccode notifications are informational — skip them.
                    _ => {}
                }
            }
...
mod tests {
    ...
    fn make_rewind_marker(target: usize) -> SessionUpdate {
        SessionUpdate::Ccode(Box::new(CcodeNotification {
    ...
    fn make_checkpoint(
        ...
        SessionUpdate::Ccode(Box::new(CcodeNotification {
            session_id: acp::SessionId::new("test"),
            update: CcodeSessionUpdate::CompactionCheckpoint(Box::new(CompactionCheckpointInfo {
