//! ccode-build's L5 wiring onto the shared full-replace engine
//! (`ccode_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](ccode_compaction::sample_full_replace_summary);
//! this module adapts ccode-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](ccode_compaction::FullReplaceError).
...
use ccode_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use ccode_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for ccode-build's full-replace pass.
///
...
/// else the structured ccode-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the ccode-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured ccode-build), ignoring the shared
...
/// Map ccode-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces ccode-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    ccode_telemetry::session_ctx::log_event(CompactionRetryDegraded {
