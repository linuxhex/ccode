...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(ccode_version::VERSION.to_string());
    s
}
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if let Some(user_meta) = crate::agent::mvp_agent::parse_json_object_env("CCODE_USER_METADATA") {
    ...
    if telemetry_enabled {
        ...
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `CCODE_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `CCODE_TELEMETRY_ENABLED`. These are analytics data that
...
pub struct FeedbackManager {
    ...
    upload_queue_stats: std::sync::OnceLock<Arc<ccode_fs::queue::UploadQueueStats>>,
}
...
impl FeedbackManager {
    ...
    pub fn set_upload_queue_stats(&self, stats: Arc<ccode_fs::queue::UploadQueueStats>) {
    ...
    /// `ccode.dev/debug/trigger_feedback` ACP extension method to exercise
    ...
    pub async fn shutdown(&self, queue: Option<&ccode_fs::queue::UploadQueue>) {
...
mod tests {
    ...
    async fn test_shutdown_with_upload_queue_drains() {
        ...
        use std::sync::Arc;
        use ccode_fs::queue::{TraceExportSource, UploadQueue, UploadRetryPolicy};
...
mod author_identity_tests {
    ...
    fn text_submission() -> FeedbackSubmission {
        ...
        s.model_id = Some("ccode-4".to_string());
        s
    }
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        let _email =
            ccode_test_support::env::EnvGuard::set("CCODE_TEST_WORK_EMAIL", "ada@corp.example");
        let _name =
            ccode_test_support::env::EnvGuard::set("CCODE_TEST_WORK_NAME", "Ada Lovelace");

        // The loader expands `$VAR` at load, exactly as a trusted config tier ships it.
        let mut value = toml::from_str::<toml::Value>(
            r#"
[feedback.user]
name = ["$CCODE_TEST_WORK_NAME"]
email = ["$CCODE_TEST_WORK_EMAIL"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(body["modelId"], "ccode-4");
        ...
        assert_eq!(persisted.model_id.as_deref(), Some("ccode-4"));
    }

    /// `CCODE_USER_METADATA` is merged into the submission and travels with it:
    ...
    async fn workflow_merges_user_metadata_into_submission() {
        let _guard = ccode_test_support::env::EnvGuard::set(
            "CCODE_USER_METADATA",
