...
//! content blocks, the session calls a vision-capable Ccode model
...
use ccode_chat::compaction_utils::{extract_real_user_queries, extract_user_query};
use ccode_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
use ccode_tools::util::truncate::truncate_middle;
...
impl ImageDescribeCache {
    ...
    pub async fn get_or_describe(
        &self,
        client: ccode_sampler::SamplingClient,
        model: &str,
        raw_bytes: &[u8],
        mime_type: &str,
        outline: Option<&str>,
...
mod tests {
    use super::*;
    use ccode_sampling_types::conversation::{ConversationItem, UserItem};
    #[test]
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![ccode_sampling_types::conversation::ContentPart::Text {
                text: text.into(),
            }],
            synthetic_reason: None,
            ..Default::default()
