...
use ccode_fs::queue::UploadQueue;
...
pub struct SessionHandle {
    ...
    pub chat_state_handle: ccode_chat::ChatStateHandle,
    ...
    /// `ccode.dev/codeNavigation.enabled`.  Stored per-session so that in leader
    ...
    /// Exposed so the `ccode.dev/toggle_plan_mode` handler can toggle plan mode
    ...
    /// Set via `ccode.dev/debug/arm_auto_compact`.
    pub force_compact: std::sync::Arc<std::sync::atomic::AtomicBool>,
    pub permission_handle: ccode_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn ccode_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    pub(crate) async fn get_model_metadata(&self) -> ccode_chat::ModelMetadata {
            ...
            .is_ok()
        {
            ...
            ccode_chat::ModelMetadata::default()
        }
    }
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<ccode_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub async fn get_hooks_list(&self) -> Option<ccode_hooks_types::HooksListResponse> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<ccode_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
