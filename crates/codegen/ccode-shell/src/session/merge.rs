...
//!
//! Used by both the ACP `ccode.dev/session/list` handler and the `ccode sessions`
...
pub(crate) async fn fetch_lanes(
    ...
    let repo_urls_fut = async {
        cwd.map(|c| {
            ccode_workspace::session::git::resolve_normalized_remote_urls(std::path::Path::new(
                c,
            ))
        })
        .unwrap_or_default()
...
mod tests {
    ...
    fn all_metadata_fields_populated_together() {
        ...
        assert_eq!(merged[0].repo_name.as_deref(), Some("project"));
