...
pub(crate) fn bounded_goal_transcript(items: &[ConversationItem]) -> String {
    ...
    for item in items.iter().rev() {
        ...
        let capped = ccode_tools::util::truncate_str(trimmed, ITEM_MAX_BYTES);
...
pub(crate) fn build_goal_evaluator_request(
    ...
    ConversationRequest {
        ...
        x_ccode_conv_id: Some(session_id.to_owned()),
        x_ccode_req_id: Some(format!("ccode-goal-eval-{}", uuid::Uuid::new_v4())),
        x_ccode_session_id: Some(session_id.to_owned()),
