...
use crate::util::ccode_home::ccode_home;
...
/// A feedback entry persisted to `~/.ccode/sessions/.../feedback.jsonl`.
///
...
pub enum PersistenceMsg {
    /// A session update (ACP update or ccode extension update)
    ...
    AppendCwdSwitchAndAck {
        ...
        respond_to: tokio::sync::oneshot::Sender<
            Result<ccode_chat::StrictAppendAck, ccode_chat::StrictAppendError>,
        >,
    },
    ...
    CurrentModel {
        ...
        /// The active agent definition name (e.g. `"ccode-build"`).
...
pub use ccode_shared::session::session_dir;
...
pub fn session_exists_for_cwd(session_id: &str, cwd: &str) -> bool {
    let sessions_root = crate::util::ccode_home::ccode_home().join("sessions");
...
fn session_exists_for_cwd_in_root(session_id: &str, cwd: &str, sessions_root: &Path) -> bool {
    let encoded = crate::util::ccode_home::encode_cwd_dirname(cwd);
...
/// `ccode -r <remote_id>` in the same cwd, this function returns the already-restored
...
fn find_local_child_for_remote_in_root(
    ...
    // `ccode -r <remote_id>` before this fix was deployed.
...
/// Searches across ALL cwd directories under `~/.ccode/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.ccode/sessions/`.
///
...
pub fn resolve_local_session_any_cwd_result(session_id: &str) -> io::Result<Option<String>> {
    resolve_local_session_any_cwd_in_root(session_id, &ccode_home().join("sessions"))
...
fn resolve_local_session_any_cwd_in_root(
    ...
    Ok(session_path
        .parent()
        .and_then(crate::util::ccode_home::decode_cwd_from_dirname))
}
...
pub(crate) fn find_persisted_session_dir_by_id_result(
    ...
    find_persisted_session_dir_by_id_in_root_result(session_id, &ccode_home().join("sessions"))
}
...
pub(crate) fn find_any_session_dir_by_id_result(session_id: &str) -> io::Result<Option<PathBuf>> {
    storage_view(&ccode_home().join("sessions"))
...
pub fn find_summary_by_session_id(session_id: &str) -> Option<Summary> {
    find_summary_by_session_id_in_root(session_id, &ccode_home().join("sessions"))
}
...
pub fn local_summaries_for_cwd_sync(cwd: &str) -> io::Result<Vec<Summary>> {
    local_summaries_for_cwd_sync_in_root(cwd, &ccode_home().join("sessions"))
}
...
pub fn resumed_session_sandbox_profile(
    ...
    resumed_session_sandbox_profile_in_root(session_id, cwd, &ccode_home().join("sessions"))
}
...
pub struct Summary {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub request_id: Option<String>,
    /// Absolute path to the `.ccode` directory, used by reconstruction.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ccode_home: Option<String>,
...
/// Current `ccode_home` as a UTF-8 string, or `None` if the path isn't valid UTF-8.
pub fn ccode_home_string() -> Option<String> {
...
impl Summary {
    pub fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            ccode_workspace::session::git::resolve_persisted_session_git_metadata_sync(
                ...
                PersistenceMsg::Update(update) => {
                    match update {
                        ...
                        SessionUpdate::Ccode(_) => {
                            // ccode notifications are written directly without merging
...
/// Collect MCP server stderr logs from `~/.ccode/logs/mcp/` for inclusion in the session archive.
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = ccode_config::ccode_home().join("logs").join("mcp");
        ...
        StorageMode::Writeback => {
            let auth_manager = auth_manager.ok_or_else(|| {
                io::Error::new(
                    ...
                    "Writeback storage mode requires authentication. Run 'ccode login' first.",
                )
            })?;
...
/// worktree containing this session's cwd so `ccode worktree gc` expires by
...
pub async fn list_summaries(cwd: Option<&str>) -> io::Result<Vec<Summary>> {
    let root_dir = crate::util::ccode_home::ccode_home();
...
/// Walk `~/.ccode/sessions/` and delete files with mtime older than `ttl_days`.
...
pub fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "ccode_shell::session::persistence",
...
mod session_exists_tests {
    ...
    fn returns_false_when_root_does_not_exist() {
        let root = std::path::PathBuf::from("/nonexistent/ccode/sessions");
...
mod resumed_sandbox_profile_tests {
    ...
    fn most_recent_cwd_skips_corrupt_summary() {
        ...
        let corrupt_dir = root
            .join(crate::util::ccode_home::encode_cwd_dirname(cwd))
            .join("corrupt");
...
mod session_exists_for_cwd_tests {
    ...
    fn session_under_different_cwd_is_not_considered_present() {
        ...
        let encoded_a = crate::util::ccode_home::encode_cwd_dirname("/project/alpha");
    ...
    fn resolve_local_session_any_cwd_skips_stub_and_finds_real() {
        ...
        let encoded_a = crate::util::ccode_home::encode_cwd_dirname(cwd_a);
        let dir_a = root.join(&encoded_a).join(session_id);
        fs::create_dir_all(&dir_a).unwrap();
        fs::write(dir_a.join("summary.json"), b"{}").unwrap();
        ...
        let encoded_b = crate::util::ccode_home::encode_cwd_dirname(cwd_b);
...
mod find_local_child_tests {
    ...
    fn returns_none_when_no_child_exists() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path().join("sessions");
        let encoded = crate::util::ccode_home::encode_cwd_dirname("/work");
    ...
    /// Regression: a second `ccode -r <remote_id>` must return the existing child
...
mod resolve_local_session_tests {
    ...
    // setting up the on-disk structures under a fake ccode home.
    ...
    fn exact_match_returns_original_id() {
        ...
        // (We can't call resolve_local_session directly because it uses ccode_home(),
    ...
    fn no_match_returns_none() {
        ...
        fs::create_dir_all(root.join(crate::util::ccode_home::encode_cwd_dirname(cwd))).unwrap();
...
mod repo_wide_resolution_tests {
    ...
    fn skips_images_only_stub_and_resolves_real_sibling() {
        ...
        let encoded = crate::util::ccode_home::encode_cwd_dirname(exact_cwd);
