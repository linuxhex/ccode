...
/// Env lane (desktop `CCODE_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`CCODE_CHAT_MODE`); hard-off in release builds.
...
/// Parse `ccode.dev/session/list` params and, under process-wide chat mode, force
...
pub struct ListReq {
    ...
    /// not a git repo. Relaxed responses set `_meta["ccode.dev/listScope"]`.
...
/// `as_str` value (`ccode.dev/listScope`), so no serde derive is needed.
...
pub fn force_kind_chat(req: &mut ListReq) {
    ...
    let mut filters = match meta.remove("ccode.dev/facetFilters") {
        Some(serde_json::Value::Object(map)) => map,
        _ => serde_json::Map::new(),
    };
    filters.insert(
    ...
    meta.insert(
        "ccode.dev/facetFilters".to_owned(),
...
pub struct ExtListResponseMeta {
    #[serde(rename = "ccode.dev/facets")]
    pub facets: FacetSummary,
    #[serde(rename = "ccode.dev/partial")]
    pub partial: PartialInfo,
    /// Present only when the listing relaxed beyond the cwd.
    #[serde(rename = "ccode.dev/listScope", skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            cwd: "/Users/me/ccode".into(),
            hostname: Some("devbox".into()),
            source: "local".into(),
            model_id: Some("ccode-build".into()),
            num_messages: 7,
            last_active_at: Some(updated_at.into()),
            branch: Some("main".into()),
            repo_name: Some("ccode".into()),
            worktree_label: Some("wt".into()),
            git_root_dir: Some("/Users/me/ccode".into()),
            git_remotes: vec!["git@github.com:example/repo.git".into()],
            source_workspace_dir: Some("/Users/me/ccode-src".into()),
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["ccode.dev/session"]["kind"], "build");
        assert_eq!(value["gitRootDir"], "/Users/me/ccode");
        assert_eq!(value["gitRemotes"][0], "git@github.com:example/repo.git");
        assert_eq!(value["sourceWorkspaceDir"], "/Users/me/ccode-src");
    ...
    fn facets_carry_kind_and_cwd() {
        ...
        assert!(matches!(
            ...
            Some(FacetValue::One(serde_json::Value::String(c))) if c == "/Users/me/ccode"
        ));
    }
    #[test]
    fn bare_session_info_is_minimal_plus_meta() {
        ...
        assert_eq!(value["sessionId"], "s1");
        assert_eq!(value["cwd"], "/Users/me/ccode");
        assert_eq!(value["title"], "a summary");
        assert_eq!(value["_meta"]["ccode.dev/session"]["kind"], "build");
    ...
    fn parsed_meta_reads_facet_filters_query_and_limit() {
        let meta = serde_json::json!({
            "ccode.dev/facetFilters": { "kind": ["build"], "starred": true },
            "ccode.dev/query": "antelope",
            "ccode.dev/limit": 5,
        });
    ...
    fn forced_kind_replaces_client_build_filter() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({
                "ccode.dev/facetFilters": { "kind": ["build"] },
            })),
            ..ListReq::default()
        };
        force_kind_chat(&mut req);
    ...
    fn forced_kind_preserves_other_facets() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({
                "ccode.dev/facetFilters": { "kind": ["build"], "starred": [true], "workspace": ["w1"] },
                "ccode.dev/query": "antelope",
                "ccode.dev/limit": 5,
            })),
            ..ListReq::default()
    ...
    fn ccode_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        let am = std::sync::Arc::new(crate::auth::AuthManager::new(
            dir,
            crate::auth::CcodeComConfig::default(),
        ));
        am.hot_swap(crate::auth::CcodeAuth {
            auth_mode: crate::auth::AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::ccode_oauth2_issuer().to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        let addr = spawn_conversations_stub(
            ...
            .await;
        let _env = ccode_test_support::EnvGuard::set(
            "CCODE_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
        let home = tempfile::tempdir().expect("tempdir");
        let client = ConversationsClient::new(ccode_auth_manager(home.path()));
        let mut req = ListReq {
            meta: Some(serde_json::json!({
                "ccode.dev/facetFilters": { "kind": ["build"] },
            })),
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = ccode_test_support::EnvGuard::unset("CCODE_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = ccode_test_support::EnvGuard::set("CCODE_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = ccode_test_support::EnvGuard::set("CCODE_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::CCODE_CHAT_MODE_ENV;
        let _chat_off = ccode_test_support::EnvGuard::unset(CCODE_CHAT_MODE_ENV);
        let _desktop_off =
            ccode_test_support::EnvGuard::unset("CCODE_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                ccode_test_support::EnvGuard::set("CCODE_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = ccode_test_support::EnvGuard::set(CCODE_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        let raw = serde_json::json!({
            "_meta": { "ccode.dev/facetFilters": { "kind": ["build"], "starred": [true] } },
        })
        .to_string();
        {
            let _off = ccode_test_support::EnvGuard::unset(CCODE_CHAT_MODE_ENV);
        ...
        {
            let _on = ccode_test_support::EnvGuard::set(CCODE_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
    ...
    /// Wire pin for the cross-crate `ccode.dev/partial` envelope the pager parses:
    ...
    fn ext_list_response_serializes_partial_reasons() {
        ...
        .expect("serialize");
        assert_eq!(
            healthy["_meta"]["ccode.dev/partial"],
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = ccode_workspace::session::git::normalize_repo_url(this_repo).unwrap();
        ...
        let rows = relax_rows(
            ...
            vec![
                ...
                summary("theirs", Some("git@github.com:ccode-org/other.git"), 9),
            ],
            30,
            facet_registry(),
        )
    ...
    /// Send-side wire pin: `ccode.dev/listScope` present iff the scope relaxed.
    #[test]
    fn ext_list_response_serializes_scope() {
        ...
        assert_eq!(with["_meta"]["ccode.dev/listScope"], serde_json::json!("repo"));
        let without =
            serde_json::to_value(ext_list_response(result(ListScope::Cwd))).expect("serialize");
        assert!(without["_meta"].get("ccode.dev/listScope").is_none());
    }
}
