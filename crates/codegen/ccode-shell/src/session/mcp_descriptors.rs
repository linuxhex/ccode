...
/// sanitizer shared with `ccode-mcp` so the advertised folder matches disk.
...
mod tests {
    ...
    fn sanitize_replaces_unsafe_chars_and_never_empty() {
        assert_eq!(
            sanitize_descriptor_segment("ccode_com_linear"),
            "ccode_com_linear"
        );
    ...
    fn server_dir_is_joined_under_root() {
        let root = Path::new("/home/u/.ccode/projects/enc/mcps");
