...
impl DrainSource {
    ...
    pub(super) fn into_parts(
        ...
        ccode_tools::implementations::ccode_build::update_goal::UpdateGoalInput,
        Option<
            tokio::sync::oneshot::Sender<
                ccode_tools::implementations::ccode_build::update_goal::UpdateGoalAck,
            >,
        >,
    ) {
        match self {
...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: ccode_tools::implementations::ccode_build::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`CCODE_GOAL_PLANNER`) and a plan
...
/// Override with `CCODE_GOAL_REVERIFY_AFTER` (floored at 1).
...
mod fold_tokens_by_model_tests {
    ...
    fn mixed_models_sum_marginals_sorted_desc() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("ccode-3")),
            rec(Some("g1"), 100, 500, Some("ccode-4")), // marginal 400
            rec(Some("g1"), 0, 50, Some("ccode-3")),    // ccode-3 total 150
        ];
        let out = fold_tokens_by_model(&records, "g1", "cur");
        assert_eq!(
            out,
            vec![("ccode-4".to_owned(), 400), ("ccode-3".to_owned(), 150)]
        );
    }
    ...
    fn single_distinct_model_collapses_to_one_entry() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("ccode-4")),
            rec(Some("g1"), 0, 200, None), // folds under current = ccode-4
        ];
        let out = fold_tokens_by_model(&records, "g1", "ccode-4");
        assert_eq!(out, vec![("ccode-4".to_owned(), 300)]);
    }
    ...
    fn other_goal_records_excluded() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("ccode-4")),
            rec(Some("g2"), 0, 999, Some("ccode-4")),
            rec(None, 0, 999, Some("ccode-4")),
        ];
        let out = fold_tokens_by_model(&records, "g1", "cur");
        assert_eq!(out, vec![("ccode-4".to_owned(), 100)]);
    }

    #[test]
    fn last_below_anchor_does_not_underflow() {
        let records = vec![rec(Some("g1"), 500, 100, Some("ccode-4"))];
    ...
    fn captured_model_survives_mid_goal_current_model_switch() {
        // A record captured `ccode-4` at spawn keeps it even though the
        // current model at aggregation time is `ccode-3`.
        let records = vec![rec(Some("g1"), 0, 100, Some("ccode-4"))];
        let out = fold_tokens_by_model(&records, "g1", "ccode-3");
        assert_eq!(out, vec![("ccode-4".to_owned(), 100)]);
    }
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use ccode_tools::implementations::ccode_build::UPDATE_GOAL_TOOL_NAME;
...
impl SessionActor {
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                ccode_tools::implementations::ccode_build::task::types::GoalLoopActive(active),
            )
            .await;
    }
}
