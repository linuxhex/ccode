...
impl SessionActor {
    ...
    pub(super) async fn execute_builtin_slash_command(
        ...
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::SlashCommandUsed {
        ...
        match action {
            ...
            BuiltinAction::SetYolo { enabled } => {
                ...
                if let Some(actual) = yolo_toggle_report(was, actual) {
                    ...
                    ccode_telemetry::session_ctx::log_event(
                        ccode_telemetry::events::YoloToggled {
                            enabled: actual,
                            previous_state: was,
                            trigger: ccode_telemetry::events::YoloTrigger::SlashCommand,
                        },
                    );
                    tracing::info_span!(
            ...
            BuiltinAction::HooksTrust => {
                let msg = match Self::do_hooks_trust_project(&self.session_info.cwd) {
                    Ok(root) => {
                        ccode_telemetry::session_ctx::log_event(
                            ccode_telemetry::events::HookTrusted { success: true },
                        );
                        format!("Trusted: {}.", root.display())
                    }
                    Err(e) => {
                        ccode_telemetry::session_ctx::log_event(
                            ccode_telemetry::events::HookTrusted { success: false },
                        );
                        e
                    }
                };
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    self.send_host_turn_slash_command_output(
                        "Usage: /hooks add <path>\nProvide a path to a hook JSON file or directory under ~/.ccode/.",
                    )
                    .await;
                } else {
                    // CWE-427: Use shared add_hooks_path() which validates
                    // paths are under ~/.ccode/ to prevent hook path injection.
                    match crate::config::add_hooks_path(&path) {
                        Ok(()) => {
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::HookAdded { success: true },
                            );
                        ...
                        Err(e) => {
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::HookAdded { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::HooksRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_hooks_path(&path) {
                        Ok(()) => {
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::HookRemoved { success: true },
                            );
                            self.send_host_turn_slash_command_output(&format!(
                        ...
                        Err(e) => {
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::HookRemoved { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsReload => {
                match &self.plugin_registry_handle {
                    Some(handle) => {
                        ...
                        ccode_telemetry::session_ctx::log_event(
                            ccode_telemetry::events::PluginReloaded { success: true },
                        );
                        self.send_host_turn_slash_command_output(&msg).await;
                    }
                    None => {
                        ccode_telemetry::session_ctx::log_event(
                            ccode_telemetry::events::PluginReloaded { success: false },
                        );
                        self.send_host_turn_slash_command_output(
            ...
            BuiltinAction::SessionInfo => {
                ...
                let context_pct = ccode_tokens::usage_percentage(ctx.used, ctx.total);
            ...
            BuiltinAction::PluginsAdd { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::add_plugin_path(&path_str) {
                        Ok(()) => {
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::PluginAdded {
                                    source: ccode_telemetry::events::PluginSource::LocalPath,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::PluginsRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_plugin_path(&path_str) {
                        Ok(()) => {
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::PluginRemoved { success: true },
                            );
                        ...
                        Err(e) => {
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::PluginRemoved { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsInstall { source, trust } => {
                if source.is_empty() {
                    ...
                    if !trust {
                        let install_source =
                            ccode_agent::plugins::git_install::parse_install_source(
                                &source, cwd,
                            );
                        let source_desc = match &install_source {
                            ccode_agent::plugins::git_install::InstallSource::Git {
                                url,
                                ..
                            } => {
                                format!("remote git repo: {url}")
                            }
                            ccode_agent::plugins::git_install::InstallSource::Local {
                                path,
                                ..
                            } => {
                        ...
                        match crate::plugin::install_plugin(&source, cwd) {
                            Ok(outcome) => {
                                ...
                                ccode_telemetry::session_ctx::log_event(
                                    ccode_telemetry::events::PluginInstalled {
                                        install_kind: kind,
                                        success: true,
                                        trust: true,
                                        error_category: None,
            ...
            BuiltinAction::MemoryBrowse => {
                ...
                self.send_ccode_notification(CcodeSessionUpdate::MemoryFiles { files: file_infos })
                    .await;
                ok_end_turn(0, None)
            }
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn ccode_tools::types::memory_backend::MemoryBackend,
            ...
            BuiltinAction::GoalClear => {
                ...
                self.clear_pending_classifier_completions();
                self.send_ccode_notification(crate::session::goal_orchestrator::build_goal_cleared())
                    .await;
