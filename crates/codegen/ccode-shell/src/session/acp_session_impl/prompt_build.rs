...
fn partition_rules_by_scope(
    files: Vec<ccode_agent::prompt::agents_md::AgentConfigFile>,
    ccode_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<ccode_agent::prompt::user_message::RuleEntry>,
    Vec<ccode_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = ccode_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use ccode_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[ccode_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
        entries.iter().map(|entry| entry.content.as_str()).collect()
    }
    #[test]
    fn partitions_custom_ccode_and_vendor_home_rules_as_user_scope() {
        let files = vec![
    ...
    #[test]
    fn ccode_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.ccode/rules/project.md"),
    ...
    #[test]
    fn nested_ccode_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
            file("/custom/ccode/rules/global.md"),
            file("/custom/ccode/worktrees/repo/.cursor/rules/project.md"),
            file("/custom/ccode/worktrees/repo/src/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/custom/ccode"),
            &[],
            Some(Path::new("/custom/ccode/worktrees/repo")),
        );
        assert_eq!(paths(&user), vec!["/custom/ccode/rules/global.md"]);
        assert_eq!(
            paths(&workspace),
            vec![
...
/// end for ccode ordering), so a failed offload never
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == Some(ccode_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        ...
        use ccode_agent::prompt::agents_md::read_agents_config_with_paths;
        use ccode_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use ccode_workspace::file_system::{git_status_short, jj_status};
        use ccode_workspace::session::git::VcsKind;
    ...
    ) -> Vec<ccode_agent::prompt::user_message::McpServerEntry> {
        use ccode_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &ccode_chat::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = ccode_sampler::SamplingClient::new(sampler_config).map_err(|e| {
