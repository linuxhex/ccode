...
/// - `file_path` — ccode_build (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), ccode_build_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — ccode_build (`read_file`, via `#[serde(rename)]`)
///
...
/// `list_dir`=`target_directory`. Grammar lives in `ccode_chat`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<ccode_chat::compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    ccode_chat::compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = ccode_tools::util::strip_redundant_session_cd(
            &command,
        ...
        // execute tool by kind so the ccode.dev/tool identity still stamps.
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(ccode_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from ccode-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, ccode-shell would sanitize
