...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<ccode_chat::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<ccode_mcp::servers::McpClientEvent>();
        {
        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: ccode_telemetry::memory_log::TARGET,
                        ...
                        tracing::debug!(target: ccode_telemetry::memory_log::TARGET,
                ...
                event = chat_state_event_rx.recv() => {
                    match event {
                        Some(ccode_chat::ChatStateEvent::ConversationReset { new_len }) => {
                        ...
                        Some(ccode_chat::ChatStateEvent::ImageBudget {
                            body_bytes,
                            trigger_bytes,
                        ...
                        Some(ccode_chat::ChatStateEvent::PromptIndexChanged { .. }) |
                        Some(ccode_chat::ChatStateEvent::TokensUpdated { .. }) => {
                ...
                maybe_cmd = cmd_rx.recv() => {
                    let Some(cmd) = maybe_cmd else {
                        ...
                        let envelope = session.fire_hook(
                            ccode_hooks::event::HookEventName::SessionEnd,
                            None,
                            ccode_hooks::event::HookPayload::SessionEnd {
                        ...
                        if let Some(registry) = session.hook_registry.borrow().clone() {
                            ...
                            let results = ccode_hooks::dispatcher::dispatch_non_blocking(
                                &registry,
                                ccode_hooks::event::HookEventName::SessionEnd,
                                &envelope,
                                &ctx,
                        ...
                        if !session.startup_hints.is_subagent {
                            if let Some(storage) = session.memory.storage() {
                                ...
                                if let crate::session::memory::hooks::SessionEndResult::Written(ref path_str) = result {
                                    ...
                                    session.send_ccode_notification(CcodeSessionUpdate::MemorySessionSaved {
                                        path: path_str.clone(),
                                    }).await;
                                }
                            }
                        ...
                        }
                        {
                            ...
                            if let Some(signals) = session.signals_handle().snapshot().await {
                                ccode_telemetry::session_ctx::log_event(
                                    ccode_telemetry::events::SessionEnded {
                    ...
                    match cmd {
                        ...
                        SessionCommand::OverrideModelName { model_name, extra_headers, context_window } => {
                            ...
                            if let Some(mut cfg) = session.chat_state_handle.get_sampling_config().await {
                                ...
                                // the agent-level default (e.g. "ccode-4.5").
                                ...
                                if let Some(r) = crate::agent::config::try_resolve_model_credentials(model_name.as_str(), existing.api_key.as_deref()) {
                                    session.chat_state_handle.update_credentials(ccode_chat::Credentials {
                                        api_key: r.api_key,
                        ...
                        SessionCommand::GetHooksList { respond_to } => {
                            ...
                            let _ = respond_to.send(ccode_hooks_types::HooksListResponse {
                                hooks,
                                project_trusted,
                        ...
                        SessionCommand::NotifyPluginUpdates { updates } => {
                            session
                                .send_ccode_notification(
                                    CcodeSessionUpdate::PluginUpdatesInstalled { updates },
                                )
                                .await;
                        }
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        ccode_tools::implementations::ccode_build::task::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = ccode_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::CcodeSessionNotification { notification } => {
                            session.handle_ccode_session_notification(notification).await;
                        }
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            // also fires for `<cwd>/.ccode/config.toml` edits,
                            ...
                            {
                                let _ = tx.send(
                                    ccode_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::ToggleMcpServer { server_name, enabled, server_config, respond_to } => {
                            session.events.emit(ccode_fs::events::Event::McpServerToggled {
                            ...
                            if is_managed_gateway {
                                ...
                                tokio::task::spawn_local(async move {
                                    ...
                                    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
                                        notifications.forward_fire_and_forget(acp::ExtNotification::new("ccode.dev/mcp/tools_changed", params.into()));
                                    }
                                    let _ = respond_to.send(Ok(()));
                                });
                                continue;
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                ccode_hooks::event::HookEventName::SessionStart,
                                None,
                                ccode_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[ccode_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = ccode_chat::compaction_utils::extract_user_query(&raw);
