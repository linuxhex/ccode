...
/// Running-turn display fields for `ccode.dev/queue/changed` (clients paint turn-start UI).
...
impl SessionActor {
    ...
    fn broadcast_queue_changed_inner(&self, state: &State, running: Option<RunningPromptDisplay>) {
        ...
        tracing::debug!(
            ...
            session = self.session_info.id.0.as_ref(),
            "broadcasting ccode.dev/queue/changed to subscribers",
        );
    ...
    ///
    /// Always re-broadcasts `ccode.dev/queue/changed` so every client reconciles
    ...
    /// 3. Re-broadcast `ccode.dev/queue/changed` so every subscriber renders the
    ...
    /// [`ccode_prompt::combine_prefix_len`]. `skip_ids` holds rows under
    ...
    pub(super) fn combine_front_pending_inputs(
        ...
        use ccode_prompt::{CombineGate, combine_prefix_len};
    ...
    fn combine_gate(item: &InputItem) -> ccode_prompt::CombineGate<'_> {
    ...
    fn append_text_to_prompt(item: &mut InputItem, extra: &str) {
        use ccode_prompt::TEXT_SEPARATOR;
    ...
    fn stamp_combined_display_texts_meta(item: &mut InputItem) {
        use ccode_prompt::stamp_combined_display_texts;
