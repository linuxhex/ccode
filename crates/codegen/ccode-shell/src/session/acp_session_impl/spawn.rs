...
fn drop_cli_catchall_allows(
    rules: Vec<ccode_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<ccode_workspace::permission::types::PermissionRule>,
    Vec<ccode_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if ccode_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use ccode_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use ccode_workspace::permission::rules::parse_permission_rule;
    use ccode_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: ccode_chat::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<ccode_sampler::SharedAttributionCallback>,
    ...
    compaction_mode: ccode_chat::CompactionMode,
    ...
    web_search_sampling_config: Option<ccode_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: ccode_tools::implementations::ccode_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: ccode_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<ccode_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<ccode_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<ccode_workspace::permission::PermissionHandle>,
    api_key_provider: Option<ccode_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<ccode_hooks::discovery::HookRegistry>>,
    workspace_ops: ccode_workspace::WorkspaceOps,
    cli_permission_rules: Vec<ccode_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn ccode_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        ccode_tools::implementations::ccode_build::scheduler::types::SchedulerHandle,
    >,
    ...
    ccode_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(ccode_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            ccode_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
                project_trusted,
            )
            .await;
        let yolo_pin = ccode_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("ccode: --allow catch-all ignored: {reason}");
            }
        }
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        ccode_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(ccode_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if ccode_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    ccode_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn ccode_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            ccode_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        ccode_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            ccode_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("CCODE_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "CCODE_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = ccode_sampling_types::SamplingConfig {
    ...
    let actor_pruning_config = ccode_chat::PruningConfig {
    ...
    let chat_state_handle = ccode_chat::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    let task_completion_reservations =
        ccode_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        ccode_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        ccode_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        ccode_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn ccode_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn ccode_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn ccode_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(ccode_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(ccode_tools::computer::local::LocalFs)
        };
    ...
    let harness_metrics = if telemetry_enabled || ccode_telemetry::external::is_active() {
    ...
    let compaction_policy = ccode_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        ccode_tools::implementations::ccode_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn ccode_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target: ccode_telemetry::memory_log::TARGET,
                error = %e,
        ...
        let backend: std::sync::Arc<dyn ccode_tools::types::memory_backend::MemoryBackend> =
        ...
        ccode_telemetry::session_ctx::log_event(
            ccode_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        ccode_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let mcp_state = {
        ...
        if !acp_mcp_servers.is_empty() {
            ...
            tracing::info!(
                ...
                "Registered in-process SDK MCP servers (ccode.dev/mcp/sdk_call)"
            );
        }
    ...
    let resolved_task_output =
        ccode_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        ccode_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        ccode_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<ccode_tools::implementations::ccode_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ClientType::CcodeTUI => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
        ClientType::CcodeWeb => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Web,
        ...
        ClientType::CcodePager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = ccode_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = ccode_sampler::RetryPolicy {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<ccode_sampler::SamplingEvent>();
    let sampler_handle = ccode_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<ccode_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<ccode_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = ccode_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        ccode_tools::implementations::ccode_build::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        ccode_tools::implementations::ccode_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use ccode_tools::implementations::ccode_build::workflow::WorkflowLaunchAck;
            while let Some((req, ack)) = workflow_launch_rx.recv().await {
                if !background_workflows_enabled {
                    let _ = ack.send(WorkflowLaunchAck::Rejected {
                        ...
                        detail: "Background workflows are disabled for this session \
                                 ([workflows] enabled = false / CCODE_WORKFLOWS=0 / remote flag)."
                            .into(),
                    });
                    continue;
                }
                ...
                if input.validate_only {
                    ...
                    let agent_budget = input
                        .agent_budget
                        .unwrap_or(ccode_workflow::DEFAULT_AGENT_BUDGET);
                    tokio::spawn(async move {
                        let verdict = tokio::task::spawn_blocking(move || {
                            ccode_workflow::validate_script_with_agent_budget(
                                &script,
                                probe_args,
                                agent_budget,
                            )
    ...
    let obs_bridge = {
        let sid = ccode_tool_protocol::SessionId::new(&*session_info.id.0)
            .unwrap_or_else(|_| ccode_tool_protocol::SessionId::new("unknown").expect("valid"));
        ccode_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<ccode_sampling_types::ToolOverrides>,
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        vcs_kind: {
            ...
            match ccode_workspace::session::git::discover_git_root(root) {
                ccode_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    ccode_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => ccode_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(ccode_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(ccode_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            ccode_tools::implementations::ccode_build::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                ccode_tools::implementations::ccode_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                ccode_telemetry::session_ctx::log_event(
                    ccode_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use ccode_tools::implementations::ccode_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use ccode_tools::implementations::ccode_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = ccode_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            ccode_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        ccode_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, ccode_agent::AgentBuildError>,
    >();
            ...
            local.block_on(&rt, async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent": tp })
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let span = ccode_fs::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    ccode_sampling_types::conversation::ContentPart::Text { text }
