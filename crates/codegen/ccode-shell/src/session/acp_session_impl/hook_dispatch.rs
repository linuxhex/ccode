...
) -> ccode_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> ccode_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use ccode_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
pub(super) fn notification_hook_for_update(
    ...
    match update {
        CcodeSessionUpdate::DiffReview { .. } => Some((
            "permission_prompt".into(),
            Some("Diff review requested".into()),
            None,
            Some("info".into()),
        )),
        CcodeSessionUpdate::AutoRecoveryExhausted { error, .. } => Some((
            "agent_error".into(),
            Some(error.clone()),
            None,
            Some("error".into()),
        )),
        CcodeSessionUpdate::RetryState(RetryState::Exhausted { reason, .. }) => Some((
            "agent_error".into(),
            Some(reason.clone()),
            None,
            Some("error".into()),
        )),
        CcodeSessionUpdate::RetryState(RetryState::Failed { message, .. }) => Some((
            "agent_error".into(),
            Some(message.clone()),
            None,
            Some("error".into()),
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> ccode_hooks::runner::RunContext<'_> {
        ccode_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_annotation(&self, message: &str) {
        self.send_ccode_notification(CcodeSessionUpdate::HookAnnotation {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[ccode_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, ccode_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
        use crate::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
        use ccode_hooks::result::HookRunResult;
        ...
        self.send_ccode_notification(CcodeSessionUpdate::HookExecution {
    ...
    pub(super) async fn dispatch_hook(
        ...
        payload: ccode_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            ccode_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                ccode_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    ccode_telemetry::events::HookOutcome::Success,
                ),
                ccode_hooks::result::HookRunResult::Blocked {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    ccode_telemetry::events::HookOutcome::Blocked,
                ),
                ccode_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    ccode_telemetry::events::HookOutcome::Error,
                ),
                ccode_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn hook_updates_do_not_fire_notification_hook() {
        let execution = CcodeSessionUpdate::HookExecution {
        ...
        let annotation = CcodeSessionUpdate::HookAnnotation {
    ...
    fn retry_in_progress_does_not_fire_notification_hook() {
        let update = CcodeSessionUpdate::RetryState(RetryState::Retrying {
            attempt: 1,
            max_retries: 3,
            reason: "timeout".into(),
        });
    ...
    fn feedback_request_does_not_fire_notification_hook() {
        let update = CcodeSessionUpdate::FeedbackRequest(FeedbackRequestNotification {
    ...
    fn diff_review_fires_permission_prompt() {
        let update = CcodeSessionUpdate::DiffReview { content: vec![] };
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = CcodeSessionUpdate::TaskCompleted {
            task_snapshot: ccode_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
