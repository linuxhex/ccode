...
impl SessionActor {
    pub(super) async fn handle_set_session_model(
        ...
        self.chat_state_handle
            .update_sampling_config(ccode_sampling_types::SamplingConfig {
                ...
                if let ConversationItem::System(sys) = item {
                    if use_concise {
                        sys.content = std::sync::Arc::<str>::from(
                            ccode_agent::prompt::template::COMPACT_SYSTEM_PROMPT,
                        );
                    } else {
                        sys.content =
    ...
    /// [`ccode_agent::AgentDefinition`], replaces `self.agent`,
