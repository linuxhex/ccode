...
impl SessionActor {
    ...
    pub(super) async fn get_rewind_points(&self) -> RewindPointsResponse {
        ...
        let file_meta_map: std::collections::HashMap<
            usize,
            &ccode_workspace::session::file_state::RewindPointMeta,
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            ccode_workspace::session::file_state::FlexiblePath,
            Option<String>,
        ...
        if wants_conversation_rewind {
            ...
            self.persist_ccode_update_only(CcodeSessionUpdate::RewindMarker {
                target_prompt_index: target_index,
    ...
    /// Out-of-band history repair (`ccode.dev/session/repair`) for a resident
    /// session: run `ccode_chat::compaction_utils::repair_history` inside
    ...
    pub(super) async fn handle_repair_history(
        &self,
        dry_run: bool,
    ) -> anyhow::Result<ccode_chat::compaction_utils::HistoryRepairReport> {
        ...
        if turn_flag.load(std::sync::atomic::Ordering::SeqCst) {
            anyhow::bail!(ccode_chat::commands::RepairHistoryBlocked);
        }
