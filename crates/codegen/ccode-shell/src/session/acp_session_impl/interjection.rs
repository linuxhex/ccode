...
// ccode-interjection-core crate so the server-side agent loop can adopt the
...
pub(crate) use ccode_interjection::{InterjectionBuffer, drain_formatted, format_interjection};

/// Shell instantiation of the shared entry type: images are ACP content.
pub(crate) type PendingInterjection = ccode_interjection::PendingInterjection<acp::ImageContent>;
...
/// `ccode.dev/session/interjection` broadcast, so a live echo would duplicate it.
