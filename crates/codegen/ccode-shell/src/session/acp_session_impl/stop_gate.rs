...
use ccode_hooks::{dispatcher, result};
...
/// `description` instead.
fn stop_entry_from_task(task: &ccode_tools::types::TaskSnapshot) -> StopBackgroundTask {
    let command_text =
...
fn stop_entry_from_subagent(
    summary: &ccode_tools::implementations::ccode_build::task::types::ActiveSubagentSummary,
) -> StopBackgroundTask {
    StopBackgroundTask {
...
fn stop_cron_from_scheduled(
    task: &ccode_tools::implementations::ccode_build::scheduler::types::ScheduledTask,
) -> StopSessionCron {
    StopSessionCron {
        id: task.id.clone(),
        schedule:
            ccode_tools::implementations::ccode_build::scheduler::interval::interval_to_human(
                task.interval_secs,
            ),
...
impl SessionActor {
    ...
    pub(crate) async fn list_active_subagents(
        &self,
    ) -> Vec<ccode_tools::implementations::ccode_build::task::types::ActiveSubagentSummary> {
    ...
    pub(super) async fn run_stop_gate(
        ...
        // `fire_hook`: client hooks get the awaited `ccode.dev/hooks/run` request
...
mod stop_gate_snapshot_tests {
    ...
    fn task_snapshot(
        kind: ccode_tools::computer::types::TaskKind,
    ...
    fn subagent_summary_maps_to_stop_entry() {
        let summary =
            ccode_tools::implementations::ccode_build::task::types::ActiveSubagentSummary {
    ...
    fn scheduled_task_maps_to_stop_cron() {
        let task =
            ccode_tools::implementations::ccode_build::scheduler::types::ScheduledTask::new(
                300,
