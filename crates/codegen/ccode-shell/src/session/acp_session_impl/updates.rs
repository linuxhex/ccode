...
//! its buffered/transient/direct variants, ccode-notification handling, and
...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, ccode_chat::UsageTotals)],
    ...
    /// Producer for the **high-frequency streaming path** with an ccode
    /// extension payload. Routes through `event_tx` -> `ReplayBuffer` ->
    /// `emit_buffered` so chunks get merged + debounced + emitted.
    ///
    /// For one-shot ccode events (RetryState, ImageCompressed, HookExecution,
    /// AutoCompactCompleted, etc.), use `send_ccode_notification` instead.
    ///
    /// The frequency-based split (`send_buffered_ccode_update` vs `send_ccode_notification`)
    /// mirrors the ACP-side split between `send_update` (high-frequency,
    /// buffered) and `emit_notification_direct` (low-frequency, direct).
    pub(super) async fn send_buffered_ccode_update(&self, update: CcodeSessionUpdate) {
        self.close_rewind_window().await;
        let notification = CcodeSessionNotification {
    ...
    /// - **ccode** (`ToolCallDeltaChunk`) -> inlines a gateway
    ...
    pub(super) async fn emit_buffered(&self, notification: SessionNotification) {
        match notification {
            ...
            SessionNotification::CcodeSession(n) => {
                self.log_outbound_ccode_session_buffered(&n);
                if self
                    .notifications
                    .gateway_enabled
    ...
    /// Tracing log for buffered ccode notifications emerging from
    /// emit_buffered. Mirrors `log_outbound_notification` for ACP.
    /// Visible with `RUST_LOG=acp_event=info`.
    fn log_outbound_ccode_session_buffered(&self, notification: &CcodeSessionNotification) {
        if !matches!(
            notification.update,
            CcodeSessionUpdate::ToolCallDeltaChunk { .. }
        ) {
            return;
        }
        tracing::info!(
            target: "acp_event",
            event = "ccode_session_buffered_notification_sent",
            session_id = %self.session_info.id,
            "Sending buffered ccode session notification"
        );
    }
    ...
    /// Handle ccode session notifications - store them in persistence
    ...
    pub(super) async fn handle_ccode_session_notification(
        &self,
        mut notification: CcodeSessionNotification,
    ) {
        if !matches!(
            notification.update,
            CcodeSessionUpdate::SubagentProgress { .. }
        ) {
            tracing::debug!("storing ccode session notification");
        }
        {
        ...
        match &notification.update {
            CcodeSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    ccode_hooks::event::HookEventName::SubagentStart,
                    None,
                    ccode_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = ccode_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        ccode_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
        ...
        let _ = self
            ...
            .persistence_tx
            .send(PersistenceMsg::Update(
                crate::session::storage::SessionUpdate::Ccode(Box::new(notification)),
            ));
    }
    /// Persist an ccode extension notification to `updates.jsonl` **without** sending it
    ...
    pub(super) fn persist_ccode_update_only(&self, update: CcodeSessionUpdate) {
        let notification = CcodeSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
        ...
        if self
            ...
            .is_err()
        {
            tracing::warn!("Failed to send ccode update to persistence channel");
        }
    }
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            ccode_hooks::event::HookEventName::Notification,
            None,
            ccode_hooks::event::HookPayload::Notification {
    ...
    /// Send an ccode extension notification to the client
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_ccode_notification(&self, update: CcodeSessionUpdate) {
        self.send_ccode_notification_with_extra_meta(update, None)
            .await;
    }
    /// [`Self::send_ccode_notification`] with caller-supplied `_meta` keys merged
    /// into the standard eventId/timestamp meta. Caller keys win on collision.
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_ccode_notification_with_extra_meta(
        &self,
...
#[cfg(test)]
mod ccode_session_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
    fn persisted_ccode_session_event_id(
        prx: &mut tokio::sync::mpsc::UnboundedReceiver<PersistenceMsg>,
    ) -> String {
        loop {
            match prx.try_recv().expect("an ccode line must be persisted") {
                PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(notif)) => {
                    return notif
                        ...
                        .and_then(|m| m.get("eventId"))
                        .and_then(|v| v.as_str())
                        .expect("persisted ccode lines must carry an eventId")
                        .to_string();
                }
                _ => continue,
            }
    ...
    /// `send_ccode_notification` (own emission) and
    /// `handle_ccode_session_notification` (inbound/forwarded, meta-less) —
    ...
    #[tokio::test]
    async fn actor_persisted_ccode_session_lines_carry_event_id() {
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    tokio::sync::mpsc::unbounded_channel::<ccode_acp::AcpClientMessage>();
                let (persistence_tx, mut prx) =
                    tokio::sync::mpsc::unbounded_channel::<PersistenceMsg>();
                let actor = create_test_actor(0, 256_000, 85, gateway_tx, persistence_tx).await;
                actor
                    .send_ccode_notification(CcodeSessionUpdate::HookAnnotation {
                        message: "own emission".into(),
                    })
                    .await;
                let own_id = persisted_ccode_session_event_id(&mut prx);
                assert!(own_id.starts_with("test-actor-"));
                actor
                    .handle_ccode_session_notification(CcodeSessionNotification {
                        session_id: acp::SessionId::new("test-actor"),
                        update: CcodeSessionUpdate::HookAnnotation {
                            message: "inbound".into(),
                        },
                        meta: None,
                    })
                    .await;
                let inbound_id = persisted_ccode_session_event_id(&mut prx);
                assert!(inbound_id.starts_with("test-actor-"));
                assert_ne!(own_id, inbound_id);
                actor.persist_ccode_update_only(CcodeSessionUpdate::HookAnnotation {
                    message: "persist-only".into(),
                });
                let persist_only_id = persisted_ccode_session_event_id(&mut prx);
