...
pub(super) struct MemoryFlushSnapshot {
    counts: ccode_chat::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &ccode_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                ccode_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                ccode_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        ccode_telemetry::session_ctx::log_event(
            ccode_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_inner(
        ...
        self.send_ccode_notification(CcodeSessionUpdate::MemoryDreamCompleted {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            x_ccode_conv_id: Some(format!("dream-{}", uuid::Uuid::new_v4())),
            x_ccode_req_id: Some(format!("ccode-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: ccode_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        let flush_start = std::time::Instant::now();

        self.send_ccode_notification(CcodeSessionUpdate::MemoryFlushStarted)
            .await;
        ...
        let result = async {
            ...
            ccode_telemetry::session_ctx::log_event(
                ccode_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                items,
                model: Some(model),
                x_ccode_conv_id: Some(format!("flush-{}", uuid::Uuid::new_v4())),
                x_ccode_req_id: Some(format!("ccode-flush-{}", uuid::Uuid::new_v4())),
                x_ccode_session_id: Some(session_id.clone()),
                x_ccode_agent_id: Some(ccode_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: ccode_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        ccode_telemetry::session_ctx::log_event(
            ccode_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => ccode_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => ccode_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => ccode_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => ccode_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::MemoryFlushed {
        ...
        self.send_ccode_notification(CcodeSessionUpdate::MemoryFlushCompleted {
            result: outcome,
            path: flush_path,
        })
        .await;
    ...
    pub(super) async fn snapshot_memory_flush_state(&self) -> MemoryFlushSnapshot {
        ...
        let chat_history = crate::sampling::conversation_to_chat_messages(
            ccode_chat::compaction_utils::prepare_conversation_for_summarization(conversation),
        );
        MemoryFlushSnapshot {
            counts,
            chat_history,
    ...
    /// LLM call using the `ccode-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let request = ConversationRequest {
            items,
            tools: vec![],
            model: Some("ccode-build".to_owned()),
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    ccode_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                ccode_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = ccode_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = ccode_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                ccode_sampler::collect_response(events).await
            }
        };
