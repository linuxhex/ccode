...
impl SessionActor {
    ...
    /// Emit `ccode.dev/git_head_changed` after an edit/shell command that may have
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            ccode_workspace::session::git::get_worktree_info(cwd),
            ccode_workspace::session::git::get_branch(cwd),
        );
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification = acp::ExtNotification::new("ccode.dev/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<ccode_tools::implementations::ccode_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) async fn handle_completion(&self, prompt_id: String, result: PromptTurnResult) {
        ...
        // replayed `_ccode.dev/session/update` rail so a viewer that re-attaches
    ...
    /// persists + forwards via `send_ccode_notification`. Retiring
    ...
    pub(super) async fn emit_turn_completed(
        ...
        self.send_ccode_notification_with_extra_meta(
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use ccode_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> ccode_hooks::event::StopFailureKind {
        use ccode_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &ccode_agent::plugins::install_registry::InstallError,
    ) -> String {
