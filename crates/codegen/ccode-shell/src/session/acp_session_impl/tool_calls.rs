...
fn is_interruptible_wait_tool(tool_name: &str, args: &serde_json::Value) -> bool {
    match tool_name {
        ...
        | "get_terminal_command_output" => ccode_tool_types::task_output_waits_from_json(args),
...
fn interrupted_wait_tool_result_with_msg(args: &serde_json::Value, msg: &str) -> ToolRunResult {
    use ccode_tool_types::{TaskOutputOutput, TaskOutputResult};
    let task_id = args
        .get("task_ids")
...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// Ccode-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as ccode edits). Deleting
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &ccode_tools::implementations::ccode_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
///
/// Uses `ccode_acp`'s TYPED [`AcpChannelFailure`](ccode_acp::AcpChannelFailure)
...
fn ext_method_no_client(err: &acp::Error) -> bool {
    matches!(
        ccode_acp::acp_channel_failure(err),
        Some(ccode_acp::AcpChannelFailure::SendFailed)
    )
}
...
impl SessionActor {
    /// Merge the canonical `ccode.dev/tool` identity envelope into a tool-call
    ...
    pub(super) fn stamp_tool_meta(
        ...
        ccode_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) async fn execute_tool_calls(
        ...
        for call in tool_calls.into_iter() {
            ...
            self.observability_bridge
                .emit(
                    ccode_tool_protocol::session_event::SessionEvent::ToolCallStarted {
                ...
                .await?
            {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(ccode_fs::events::Event::McpToolCallCompleted {
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => ccode_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && ccode_mcp::servers::is_auth_rejection_message(
            ...
            self.observability_bridge
                .emit(
                    ccode_tool_protocol::session_event::SessionEvent::ToolCallCompleted {
                ...
                .await;
            let (ext_file_path, ext_parameters) = if ccode_telemetry::external::is_active() {
            ...
            ccode_telemetry::session_ctx::log_event(
                ccode_telemetry::events::ToolCallCompleted {
                    tool_name: prepared.tool_name.clone(),
                    outcome: tool_outcome,
                    duration_ms,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(ccode_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                ccode_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                ccode_hooks::event::HookEventName::PreToolUse,
                None,
                ccode_hooks::event::HookPayload::PreToolUse {
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                ccode_workspace::permission::AccessKind::Read(p) => (
                    ccode_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                ccode_workspace::permission::AccessKind::Edit(p) => {
                    (ccode_telemetry::events::AccessKind::Edit, p.clone())
                }
                ccode_workspace::permission::AccessKind::Bash(cmd) => {
                    (ccode_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                ccode_workspace::permission::AccessKind::Grep { path, glob } => (
                    ccode_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                ccode_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (ccode_telemetry::events::AccessKind::Mcp, name.clone())
                }
                ccode_workspace::permission::AccessKind::WebFetch(u) => {
                    (ccode_telemetry::events::AccessKind::Web, u.clone())
                }
                ccode_workspace::permission::AccessKind::WebSearch(q) => {
                    (ccode_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            ccode_telemetry::session_ctx::log_event(
                ccode_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            let edit_path_context = matches!(&access_kind, AccessKind::Edit(_)).then(|| {
                ccode_workspace::permission::types::EditPathContext {
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        ccode_fs::events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        ccode_fs::events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        ccode_fs::events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        ccode_fs::events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (ccode_telemetry::events::PermissionOutcome::Allow, None)
                }
                Decision::Reject(reason) | Decision::PolicyDeny(reason) => (
                    ccode_telemetry::events::PermissionOutcome::Deny,
                    Some(reason.to_string()),
                ),
                Decision::Cancelled => (
                    ccode_telemetry::events::PermissionOutcome::Cancelled,
                    None,
                ),
                Decision::FollowupMessage(_) => (
                    ccode_telemetry::events::PermissionOutcome::Followup,
                    None,
                ),
            };
            tracing::info_span!(
            ...
            .in_scope(|| {});
            ccode_telemetry::session_ctx::log_event(
                ccode_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        ccode_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        ccode_hooks::event::HookEventName::PermissionDenied,
                        ccode_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    /// Issue the `ccode.dev/exit_plan_mode` reverse-request and await the user's
    ...
    pub(super) async fn request_plan_approval(
        ...
        ccode_tools::implementations::ccode_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use ccode_tools::implementations::ccode_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                ccode_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
        ...
        let (title, kind, locations, content) = match tool_call_input {
                ...
                (
                    ...
                    vec![
                        acp::ToolCallLocation::new(read_file.path)
                            ...
                            .line(
                                ccode_tools::normalization::norm_offset_i64(read_file.offset)
                                    .map(|l| l as u32),
                            ),
                    ],
                    Vec::new(),
            ...
            ToolInput::WaitTasks(wait) => (
                format!(
                    ...
                    wait.task_ids.len(),
                    match wait.mode {
                        ccode_tool_types::WaitMode::WaitAny => "wait_any",
                        ccode_tool_types::WaitMode::WaitAll => "wait_all",
                    }
                ),
                acp::ToolKind::Other,
                vec![],
            ...
            ToolInput::Skill(skill) => {
                ccode_telemetry::session_ctx::log_event(
                    ccode_telemetry::events::SkillDispatched {
    ...
    async fn handle_tool_parse_error(
        ...
        call_id: &str,
        function_name: &str,
        err: ccode_tool_runtime::ToolError,
        raw_arguments: &str,
        model_id: &str,
    ) -> Result<(), acp::Error> {
        tracing::error!(
    ...
    /// `ccode_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use ccode_telemetry::enums::PrCreationSource;
        use ccode_tools::util::git_detect;
        match &result.output {
            ccode_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    ccode_telemetry::session_ctx::log_event(
                        ccode_telemetry::events::PrMerged {},
                    );
                }
            }
            ccode_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: ccode_tools::util::git_detect::PrRef,
        source: ccode_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            ccode_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                ccode_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let ccode_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                ccode_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | ccode_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            ccode_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            ccode_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
        ...
        // P0 阶段A：对可压缩工具的输出先做 Snip + Budget 截断再入历史，
        // 避免单次超大输出（如万行文件、长 bash 日志）在压缩触发前就
        // 撑爆上下文。编排类工具（Agent/Task/…）不进白名单，原样保留。
        let tool_chat = crate::session::micro_compact_gate::snip_and_budget_tool_result(
            tool_chat,
            requested_tool_name,
        );
        self.chat_state_handle.push_tool_result(tool_chat);
        ...
        if !extracted_images.is_empty() {
            ...
            if let Some((notice, notes)) = crate::session::image_normalize::dropped_to_envelope(
                ...
                self.send_ccode_notification(CcodeSessionUpdate::ImageDropped { notes })
                    .await;
            }
    ...
    /// Translate one [`ccode_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: ccode_sampler::SamplingEvent,
    ) {
        use ccode_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::ToolCallDelta {
                ...
                self.send_buffered_ccode_update(CcodeSessionUpdate::ToolCallDeltaChunk {
                    tool_call_id: id,
                    tool_index,
                    name,
                    arguments_delta,
            ...
            SamplingEvent::Retrying {
                ...
                if kind == ccode_sampler::SamplingErrorKind::DoomLoopDetected {
            ...
            SamplingEvent::Failed { request_id, error } => {
                ccode_telemetry::unified_log::error(
...
fn execute_tool_call_parts(
    ...
    let display = ccode_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use ccode_tools::types::ToolInput;
    use ccode_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use ccode_tools::implementations::ccode_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use ccode_tools::implementations::opencode::write::WriteInput;
        ToolInput::Write(WriteInput {
            file_path: path.into(),
            content: "x".into(),
        })
    }
    /// Ccode edit tools are plan-file-only while plan mode is active — the
    /// enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn ccode_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "ccode tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use ccode_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
    ...
    fn non_edit_tools_not_gated() {
        use ccode_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use ccode_tools::implementations::ccode_build::exit_plan_mode::ExitPlanModeExtResponse;
    ...
    fn ext_method_no_client_defaults_false_for_untagged_error() {
        assert!(!ext_method_no_client(&ccode_acp::acp_internal_error(
...
mod wait_interrupt_tests {
    ...
    use ccode_tools::types::output::ToolOutput;
    use ccode_tool_types::TaskOutputOutput;
    ...
    async fn pending_interjection_aborts_in_flight_wait() {
        ...
        use ccode_interjection::PendingInterjection;
