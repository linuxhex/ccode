...
impl WorkflowNotifySender {
    ...
    fn dispatch(&self, update: CcodeSessionUpdate, persist: bool) {
        let mut meta = None;
...
mod tests {
    ...
    fn phase_states_derive_from_current() {
        ...
        match build_workflow_updated(&state, 0, 0) {
            CcodeSessionUpdate::WorkflowUpdated { phases, .. } => {
    ...
    fn completed_run_marks_current_phase_done() {
        ...
        t.apply_outcome(
            "wf_2",
            &ccode_workflow::WorkflowOutcome::Completed {
                result: serde_json::json!("done"),
            },
        );
        let state = t.get("wf_2").unwrap();
        match build_workflow_updated(&state, 0, 0) {
            CcodeSessionUpdate::WorkflowUpdated { phases, status, .. } => {
