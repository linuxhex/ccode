...
use ccode_workflow::{WorkflowMeta, extract_meta};
...
pub(crate) enum ResolveError {
    ...
    Meta(#[from] ccode_workflow::MetaError),
}

pub(crate) fn project_root(session_cwd: &Path) -> PathBuf {
    ccode_workspace::session::git::find_git_root_from_path(session_cwd)
        .unwrap_or_else(|_| session_cwd.to_path_buf())
}

pub(crate) fn user_workflow_dir() -> PathBuf {
    crate::util::ccode_home::ccode_home().join("workflows")
}
...
impl WorkflowRegistry {
    pub(crate) fn scan(session_cwd: Option<&Path>) -> Self {
        ...
        {
            dirs.push((project_root(cwd).join(".ccode").join("workflows"), "project"));
        }
...
pub(crate) fn resolve_by_path(
    ...
    if !in_project && !in_user_or_session {
        return Err(ResolveError::UntrustedPath {
            path: candidate.display().to_string(),
            reason: "outside the project, ccode home, and session workflow runs".into(),
        });
    }
...
pub(crate) fn save_project_workflow(
    ...
    let dir = canonical_root.join(".ccode").join("workflows");
...
mod tests {
    ...
    fn inline_resolution_validates_name_and_size() {
        ...
        assert!(matches!(
            ...
            Err(ResolveError::Meta(ccode_workflow::MetaError::InvalidName))
        ));
        assert!(matches!(
    ...
    fn deterministic_scan_uses_git_root_and_skips_invalid_filename() {
        ...
        let wf_dir = dir.path().join(".ccode").join("workflows");
    ...
    fn project_workflows_follow_folder_trust() {
        let dir = tempfile::tempdir().unwrap();
        git2::Repository::init(dir.path()).unwrap();
        let workflows = dir.path().join(".ccode/workflows");
    ...
    fn discovery_and_explicit_paths_reject_symlink_files() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let project = dir.path().join("project");
        let workflows = project.join(".ccode/workflows");
    ...
    fn save_through_symlinked_session_root_stays_in_canonical_project() {
        ...
        assert_eq!(
            ...
            project.join(".ccode/workflows/safe.rhai")
        );
    }
    ...
    fn save_rejects_symlinked_registry_directory() {
        ...
        symlink(&attacker, project.join(".ccode/workflows")).unwrap();
