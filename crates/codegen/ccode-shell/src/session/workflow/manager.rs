...
use ccode_workflow::{Journal, WorkflowOutcome, WorkflowRunParams};
...
pub(crate) const WORKFLOW_DEFAULT_AGENT_BUDGET: u64 = ccode_workflow::DEFAULT_AGENT_BUDGET;
...
impl WorkflowManager {
    ...
    pub(crate) fn launch(
        ...
        let (run_id, journal, state) = match &spec.resume_run_id {
            Some(run_id) => {
                ...
                if existing.status
                    ...
                    && existing.agents_used >= ccode_workflow::MAX_AGENT_BUDGET
                {
                    return Err(LaunchError::NotResumable(
        ...
        let exec = tokio::task::spawn_blocking(move || {
            ccode_workflow::run_workflow(WorkflowRunParams {
                script,
                args,
                journal,
                host_tx,
    ...
    pub(crate) fn test_bundle_with_session_dir(
        ...
        let notify = super::notify::WorkflowNotifySender::new(
            ...
            ccode_acp::AcpAgentGatewaySender::new(gateway_tx),
            persist_tx,
            store.clone(),
        );
    ...
    fn cancel_children_for_run(&self, run_id: &str) -> bool {
        ...
        self.subagent_event_tx
            .send(
                ccode_tools::implementations::ccode_build::task::types::SubagentEvent::Cancel(
                    ccode_tools::implementations::ccode_build::task::types::SubagentCancelRequest {
                        parent_session_id: Some(self.session_id.clone()),
                        target: ccode_tools::implementations::ccode_build::task::types::SubagentCancelTarget::WorkflowRunId(
                            run_id.to_owned(),
                        ),
                        respond_to,
                    },
...
mod tests {
    ...
    type CancelLog = Arc<
        parking_lot::Mutex<
            Vec<ccode_tools::implementations::ccode_build::task::types::SubagentCancelTarget>,
        >,
    >;
    ...
    async fn pause_marks_user_paused_and_resume_replays() {
        ...
        use ccode_tools::implementations::ccode_build::task::types::SubagentEvent;
        ...
        use ccode_tools::implementations::ccode_build::task::types::SubagentResult;
    ...
    async fn workflow_spawns_await_to_completion() {
        ...
        assert_eq!(
            ...
            ccode_tools::implementations::ccode_build::task::types::ModelOverrideProvenance::Tool,
