...
//! window containing a [`ccode_mcp::servers::McpClientEventKind::TransportClosed`]
//! or [`ccode_mcp::servers::McpClientEventKind::HandshakeFailed`] key for a
...
//!    [`ccode_mcp::servers::start_mcp_server`] sets
...
//!    push and stop.
//! 5. **Already-Empty** — see the [`ccode_mcp::servers::ClientStateKind::Empty`]
...
use ccode_mcp::servers::{McpClientEventKind, McpServerName};
...
pub trait RestartActions {
    ...
    /// Push an already-built `ccode.dev/mcp/server_status` payload to the
...
/// ACP `ccode.dev/mcp/server_status` notification. Failures are logged and
