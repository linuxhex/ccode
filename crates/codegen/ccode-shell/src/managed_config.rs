...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    ccode_config::MANAGED_CONFIG_FILENAME,
    ccode_config::REQUIREMENTS_FILENAME,
    ccode_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    ccode_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            ccode_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", ccode_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            ccode_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// would just 401).
fn eligible_team_principal(auth: CcodeAuth) -> Option<CcodeAuth> {
...
/// config is a ccode.dev feature with one ccode.dev auth.
fn read_active_team_auth() -> Option<CcodeAuth> {
    let home = crate::util::ccode_home::ccode_home();
...
pub fn clear_orphan() {
    ...
    if ccode_config::fail_closed_policy_armed_at(&home) {
        tracing::info!(
...
/// files (TUI tick vs `ccode login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `ccode setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `CCODE_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("CCODE_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `ccode-http` classifier.
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = ccode_config::signed_policy::stored_envelope_nonce(
        &crate::util::ccode_home::ccode_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            ccode_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
/// Override with `CCODE_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("CCODE_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** CcodeBuildDeployment UUID (the id
...
/// Resolve deployment key from `CCODE_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (ccode setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("CCODE_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.ccode/`, trying the
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<CcodeAuth>,
...
/// read-only `ccode setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if ccode_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(ccode_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        ccode_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(ccode_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            ccode_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(ccode_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &ccode_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<ccode_config::signed_policy::SignatureEnvelope> {
    use ccode_config::signed_policy::now_unix;
    ...
    let claim = match ccode_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &ccode_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `ccode login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<CcodeAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `ccode setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(CcodeAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `ccode setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !ccode_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::ccode_home::ccode_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            ccode_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `ccode setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.ccode`.
    Installed,
...
/// Result of `ccode setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `ccode setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `ccode setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `ccode setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `ccode setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && ccode_config::signed_policy::verification_active()
...
/// Run the `ccode setup` sync for the current principal. The caller must check
