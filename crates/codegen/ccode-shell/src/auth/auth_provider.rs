...
/// grandchildren -- and the `CCODE_AUTH_PROVIDER_*` credentials in their env --
...
async fn run_capped(
    ...
    // the direct child.
    let mut group = ccode_tools::util::ProcessGroup::new()
...
async fn mint_provider_token(
    ...
    if let Some(prev) = previous {
        cmd.env("CCODE_AUTH_PROVIDER_ACCESS_TOKEN", &prev.token);
        if let Some(refresh) = &prev.refresh_token {
            cmd.env("CCODE_AUTH_PROVIDER_REFRESH_TOKEN", refresh);
        }
        if let Some(expires_at) = prev.expires_at {
            cmd.env("CCODE_AUTH_PROVIDER_EXPIRES_AT", expires_at.to_rfc3339());
        }
    }
