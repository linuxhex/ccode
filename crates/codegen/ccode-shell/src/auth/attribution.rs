...
//!
//! 1. [`ccode_telemetry::unified_log::warn`] for the local
//!    `~/.ccode/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`ccode_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`ccode_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`ccode_sampler::SamplerConfig::attribution_callback`] at every
...
use ccode_sampler::{Auth401AttributionCallback, SamplingConsumer};
use ccode_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`ccode_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `ccode_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn ccode_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `ccode_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `ccode-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `ccode-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.ccode/logs/unified.jsonl) + scrubbed
...
/// and the mint/expiry fields from the resulting `CcodeAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `CcodeAuth` (or
    ...
    // `ccode_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.ccode/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> CcodeAuth {
        CcodeAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    /// `ShellAttribution` implements `ccode_tools::Auth401AttributionCallback`
