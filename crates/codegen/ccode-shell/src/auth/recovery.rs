...
use crate::auth::model::CcodeAuth;
use crate::auth::token_type::TokenType;
use ccode_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&CcodeAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        ccode_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<CcodeAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<CcodeAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<CcodeAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.ccode_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<CcodeAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            ccode_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<CcodeAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<CcodeAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &CcodeAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, CcodeAuth};
    ...
    fn rejected_cred() -> Option<CcodeAuth> {
        Some(CcodeAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), CcodeComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<CcodeAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.ccode_com_config().auth_scope();
        let fresh = CcodeAuth {
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.ccode_com_config().auth_scope();
        let expired_different = CcodeAuth {
