...
use crate::auth::model::{CcodeAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: CcodeAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.ccode_com_config.token_header;
    ...
    let response = http_client
        ...
        .header("X-Ccode-Token-Auth", token_header.as_str())
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut CcodeAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: CcodeAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut CcodeAuth, user_info: UserInfo) {
