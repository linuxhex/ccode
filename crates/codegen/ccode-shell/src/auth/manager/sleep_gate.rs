...
impl SleepGate {
    pub(super) fn raise(&self) {
        *self.raised_at.write() = Some(GateRaise::now());
        ccode_telemetry::unified_log::warn("auth.sleep.gate_set", None, None);
    }
...
impl AuthManager {
    ...
    /// [`ccode_power::PowerState`] for the canonical explanation of what a
    ...
    pub(crate) fn is_dark_wake(&self) -> bool {
        ...
        matches!(
            ccode_power::current_power_state(),
            ccode_power::PowerState::DarkWake
        )
    }
    ...
    pub fn start_system_power_listener(self: &Arc<Self>) {
        ...
        let listener = ccode_power::SystemPowerListener::start(move |event| {
            if let Some(this) = weak.upgrade() {
                let imminent = matches!(event, ccode_power::PowerEvent::WillSleep);
