...
use crate::auth::{AuthManager, CcodeAuth, CcodeComConfig, parse_output};
use crate::util::ccode_home;
...
/// session.
fn is_cached_credential_compatible(auth: &CcodeAuth, ccode_com_config: &CcodeComConfig) -> bool {
    let expected_issuer = ccode_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| ccode_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(ccode_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("CCODE_LOGIN_DEVICE_FLOW")
...
/// ccode OAuth2 provider supports it. Enterprise OIDC (`oidc=Some`) always uses
/// the loopback flow, mirroring `run_auth_flow_inner`'s precedence.
async fn cli_should_use_device(
    config: &CcodeComConfig,
    login_override: LoginTransportOverride,
) -> bool {
    !crate::auth::oidc::is_configured(config) && should_use_device_flow(login_override).await
}

/// Whether interactive ccode OAuth2 login uses the RFC 8628 device flow (vs loopback).
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `CCODE_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `ccode_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("CCODE_LOGIN_DEVICE_FLOW");
...
/// How login presents itself; surfaced to the TUI via `ccode.dev/auth/get_url`.
...
impl AuthUrlMode {
    /// Wire string for the `ccode.dev/auth/get_url` ACP response.
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.ccode_com_config());
...
pub async fn run_auth_flow_with_stderr_bridge(
    auth_manager: &Arc<AuthManager>,
    ccode_com_config: &CcodeComConfig,
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = ccode_com_config.oidc.is_some(),
        has_oauth2 = ccode_com_config.oauth2.is_some(),
        has_external_auth = ccode_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if reauth {
        auth_manager.clear()?;
        // Also remove the legacy accounts.ccode.dev scope so stale tokens
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, ccode_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, ccode_com_config)
        }) {
            ccode_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = ccode_com_config.auth_provider_command {
    ...
    // `ccode login --devbox` uses run_devbox_login and is not gated here.
    if !ccode_com_config.blocks_automatic_oidc()
    ...
    // ccode OAuth2 also defaults to loopback; the device flow (robust on
    // remote/SSH where the loopback redirect can't reach the CLI) is opt-in via
    // --device-auth / CCODE_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(ccode_com_config) {
        return crate::auth::oidc::run_login_flow(ccode_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = ccode_com_config.oauth2 {
    ...
    tracing::error!(
        "auth: no OAuth2 configuration available (neither enterprise OIDC nor ccode OAuth2 configured)"
    );
    anyhow::bail!(
        "No OAuth2 configuration available. Run `ccode login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(ccode_com_config: &CcodeComConfig) -> Option<CcodeAuth> {
    let ccode_home = ccode_home::ccode_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&ccode_home, ccode_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(ccode_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(ccode_com_config).await {
        return Some(auth);
    }
    let ccode_home = ccode_home::ccode_home();
    let auth_manager = Arc::new(AuthManager::new(&ccode_home, ccode_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, ccode_com_config).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<CcodeAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_ccode_auth() && a.refresh_token.is_some())
}
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = ccode_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: CcodeAuth) -> CcodeAuth {
...
fn report_signed_in(auth: &CcodeAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<CcodeAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(ccode_com_config).await)
    } else {
        ensure_authenticated(ccode_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `ccode login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `CCODE_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // oauth2=None) always uses the loopback flow; only the ccode OAuth2 provider
    // supports the device flow. Without this guard, `ccode login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.ccode_com_config, login_override).await {
        if config.ccode_com_config.oauth2.is_none() {
            // No OIDC and no oauth2 here, so `--oauth` can't help.
            anyhow::bail!("Sign-in is not available for this deployment. Set CCODE_API_KEY instead.");
        }
        let ccode_home = ccode_home::ccode_home();
        let auth_manager = Arc::new(AuthManager::new(&ccode_home, config.ccode_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.ccode_com_config) {
            eprintln!(
    ...
    // config was actually applied; `ccode setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `CCODE_API_KEY` / `CCODE_CODE_CCODE_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        ccode_telemetry::external::set_identity(
            ccode_telemetry::external::IdentityAttrs::default(),
        );
        ccode_telemetry::external::flush();
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_ccode_api_key_env(),
    })
}

/// `ccode logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let ccode_home = ccode_home::ccode_home();
    let auth_manager = AuthManager::new(&ccode_home, config.ccode_com_config.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via CCODE_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("CCODE_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::auth::config::CCODE_OAUTH2_ISSUER;
    use crate::env::EnvVarGuard;
    use chrono::Utc;

    /// Run `f` with `CCODE_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("CCODE_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("CCODE_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("CCODE_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A ccode.dev first-party (ccode.dev-issuer) OIDC session — `is_ccode_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> CcodeAuth {
        CcodeAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(CCODE_OAUTH2_ISSUER.to_string()),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), CcodeComConfig::default());
        ...
        // Third-party external (no ccode.dev issuer) stays excluded.
        mgr.hot_swap(CcodeAuth {
            oidc_issuer: None,
            auth_mode: AuthMode::External,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only ccode_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), CcodeComConfig::default()));
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = CcodeComConfig {
            auth_provider_command: Some("printf '%s' ccode-ext-token".to_string()),
        ...
        assert_eq!(auth.map(|a| a.key), Some("ccode-ext-token".to_string()));
    }
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), CcodeComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("CCODE_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("CCODE_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `ccode login` must use loopback, not device —
        ...
        // The ccode OAuth2 provider (oidc=None, oauth2=Some) does use device.
        let ccode_cfg = CcodeComConfig::default();
        assert!(ccode_cfg.oauth2.is_some() && ccode_cfg.oidc.is_none());
        assert!(cli_should_use_device(&ccode_cfg, LoginTransportOverride::ForceDevice).await);
    }
    ...
    fn oidc_auth(issuer: &str) -> CcodeAuth {
        CcodeAuth {
    ...
    fn pinned_cfg(team: &str) -> CcodeComConfig {
        CcodeComConfig {
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = CcodeAuth {
            key: "fresh-token-from-disk".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() + chrono::Duration::hours(1)),
            refresh_token: Some("new-rt".into()),
            oidc_issuer: Some(CCODE_OAUTH2_ISSUER.into()),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = CcodeComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = CcodeAuth {
        ...
        // CCODE_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Ccode: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `ccode login` passes `on_stderr=None`; stderr must be inherited so
