...
//! attempt (or an explicit `ccode.dev/auth/cancel`) cancels the previous one, so
...
//! scopes explicit cancels so a delayed `ccode.dev/auth/cancel` cannot tear down a
//! successor login.
...
pub(crate) struct AttemptChannels {
    /// Forwards pasted codes from `ccode.dev/auth/submit_code` to the flow.
    code_tx: tokio::sync::mpsc::Sender<String>,
    /// Yields the auth URL to `ccode.dev/auth/get_url`. `Option` so
