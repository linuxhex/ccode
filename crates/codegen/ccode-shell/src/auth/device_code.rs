...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, CcodeAuth};
...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `ccode login` or set CCODE_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
/// `x-ccode-client-surface` header so device-flow metrics can separate logins a
...
pub async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-ccode-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
                ("scope", scope_str.as_str()),
                ("referrer", "ccode-build"),
            ]),
        &url,
    )
    .send()
...
///
/// On success, persists credentials to `~/.ccode/auth.json` and returns
/// the authenticated `CcodeAuth`.
///
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `ccode login --device-auth` again.");
        }
...
pub async fn run_device_code_login_channels(
    ...
    // `ccode.dev/auth/get_url` isn't blocked on a slow/hanging browser launch
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, CcodeComConfig};
    ...
    fn auth_manager_with_ccode_home(
        ccode_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(ccode_home, CcodeComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let ccode_home = temp_dir.path().join(".ccode");
        std::fs::create_dir_all(&ccode_home).unwrap();
        let auth_manager = auth_manager_with_ccode_home(&ccode_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access ccode-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            "sub": "user-42",
            "iss": "https://auth.ccode.dev",
            "aud": "client-id",
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access ccode-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access ccode-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    /// persist nothing.
    fn assert_build_auth_rejected(cfg: CcodeComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let ccode_home = temp_dir.path().join(".ccode");
        std::fs::create_dir_all(&ccode_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&ccode_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !ccode_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::CcodeAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_ccode_home(temp_dir.path(), "http://127.0.0.1:9");
