...
use crate::auth::config::CcodeComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use ccode_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, CcodeAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<CcodeAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<ccode_power::SystemPowerListener>>,
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<CcodeAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    pub fn new(ccode_home: &Path, ccode_com_config: CcodeComConfig) -> Self {
        let scope = ccode_com_config.auth_scope();
        let proxy_base_url =
        ...
        ccode_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "ccode_home": ccode_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "CCODE_HOME": std::env::var("CCODE_HOME").unwrap_or_else(|_| "(unset)".into()),
                "CCODE_AUTH_PATH": std::env::var("CCODE_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "CCODE_AUTH": std::env::var("CCODE_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // CCODE_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("CCODE_AUTH") {
            if let Ok(auth) = serde_json::from_str::<CcodeAuth>(&inline_json) {
            ...
            tracing::warn!("CCODE_AUTH set but failed to parse as JSON, falling back to file");
        }

        // CCODE_AUTH_PATH: custom file path (overrides default $CCODE_HOME/auth.json).
        let path = std::env::var("CCODE_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| ccode_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &CcodeAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.ccode_com_config)?;
    ...
    fn vet_cached(&self, auth: CcodeAuth) -> Option<CcodeAuth> {
    ...
    pub(crate) fn current(&self) -> Option<CcodeAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<CcodeAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&CcodeAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<CcodeAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<CcodeAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<CcodeAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &CcodeAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &CcodeAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &CcodeAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.ccode_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `CcodeAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: CcodeAuth) -> std::io::Result<CcodeAuth> {
        ...
        match &write_result {
            Ok(()) => ccode_telemetry::unified_log::info(
            ...
            Err(e) => ccode_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: CcodeAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut CcodeAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn ccode_com_config(&self) -> &CcodeComConfig {
        &self.ccode_com_config
    }
    ...
    pub(crate) async fn run_external_refresh_command(&self, command: &str) -> Option<CcodeAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: CcodeAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<CcodeAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> CcodeAuth {
        self.inner.read().clone().unwrap_or_else(|| CcodeAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: CcodeAuth) -> Option<CcodeAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<CcodeAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<CcodeAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<CcodeAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in CCODE_AUTH mode).
            DiskAuthState::Ok => {
                ccode_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<CcodeAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<CcodeAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<CcodeAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.ccode_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`ccode login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.ccode_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<CcodeAuth, AuthError> {
        if self.ccode_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        ccode_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `ccode_power::PowerState` for the canonical
    ...
    pub(crate) fn is_different_token(&self, candidate: &CcodeAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<ccode_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[ccode_com] auth_token_ttl` to drive a real schedule.
...
impl ccode_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.ccode_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.ccode_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.ccode_com_config.preferred_method,
    ...
    non_empty_key(crate::agent::auth_method::read_ccode_api_key_env().ok())
...
impl AuthManager {
    /// `ccode::api_key` from this manager's auth file, memoized on
...
) -> ccode_tools::types::SharedApiKeyProvider {
