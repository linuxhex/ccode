...
use super::model::{API_KEY_SCOPE, AuthMode, AuthStore, CcodeAuth, lookup_auth};
...
/// Falls back to the legacy `https://accounts.ccode.dev/sign-in` scope key
/// when the requested scope is not found (devbox auth.json migration).
pub fn read_token_by_scope(ccode_home: &Path, scope: &str) -> anyhow::Result<String> {
    let path = ccode_home.join("auth.json");
    let store =
        read_auth_json(&path).map_err(|_| anyhow::anyhow!("Not logged in. Run `ccode login`."))?;
    lookup_auth(&store, scope).map(|a| a.key).ok_or_else(|| {
        anyhow::anyhow!("Your auth token is invalid. Run `ccode login` to re-authenticate.")
    })
}

/// Read the API key from the `ccode::api_key` scope in auth.json.
pub fn read_api_key(ccode_home: &Path) -> Option<String> {
    let path = ccode_home.join("auth.json");
    let map = read_auth_json(&path).ok()?;
    map.get(API_KEY_SCOPE).map(|a| a.key.clone())
}

/// Store a plain API key in auth.json under the `ccode::api_key` scope.
///
...
pub fn store_api_key(ccode_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = ccode_home.join("auth.json");
...
/// Remove the `ccode::api_key` scope from auth.json.
pub fn clear_api_key(ccode_home: &Path) -> std::io::Result<()> {
    let path = ccode_home.join("auth.json");
