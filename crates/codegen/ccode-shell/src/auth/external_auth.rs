...
/// Parse stdout into a session-credential `CcodeAuth`.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<CcodeAuth> {
...
pub(crate) async fn run_external_refresh(command: &str) -> Option<CcodeAuth> {
...
pub(crate) async fn refresh_with_command(command: &str, prev_auth: &CcodeAuth) -> Option<CcodeAuth> {
...
mod tests {
    ...
    #[test]
    fn parse_output_issuer_claim_enables_ccode_auth() {
        ...
        // ccode.dev issuer claim → first-party session (relay-eligible).
        let auth = parse_output(&ok(
            r#"{"access_token":"t","expires_in":900,"issuer":"https://auth.ccode.dev"}"#,
        ))
        .unwrap();
        assert_eq!(auth.oidc_issuer.as_deref(), Some("https://auth.ccode.dev"));
        assert!(auth.is_ccode_auth());

        // Non-ccode.dev issuer is stored but stays third-party.
        let auth = parse_output(&ok(
    ...
    #[tokio::test]
    async fn sets_ccode_auth_expired_env_on_refresh() {
        let auth = run_external_refresh("echo $CCODE_AUTH_EXPIRED")
            .await
            .unwrap();
