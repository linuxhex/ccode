...
/// Default scopes for the ccode OAuth2 provider. Includes `ccode-cli:access`
...
pub enum PreferredAuthMethod {
    /// `CCODE_API_KEY` / auth.json `ccode::api_key` / per-model BYOK (`ccode.api_key`).
    ApiKey,
    /// OIDC / OAuth2 session (`cached_token`, interactive `ccode.dev` / `oidc`,
    /// including devbox-minted OIDC).
    Oidc,
}
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct CcodeComConfig {
    pub ccode_ws_origin: String,
    pub ccode_ws_url: String,
    ...
    /// Login button label (env: `CCODE_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `CCODE_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub auth_token_ttl: Option<u64>,
    /// Admin kill switch: when `Some(true)`, the `ccode.api_key` auth method is
    /// neither advertised nor accepted, so `CCODE_API_KEY`/per-model credentials
    /// can't bypass the deployment's IdP login. Env: `CCODE_DISABLE_API_KEY_AUTH`.
...
/// Customer OIDC Identity Provider configuration (`[ccode_com_config.oidc]`).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// OAuth2 provider configuration (`CCODE_OAUTH2_ISSUER` / `CCODE_OAUTH2_CLIENT_ID`).
///
...
pub const CCODE_OAUTH2_ISSUER: &str = "https://auth.ccode.dev";
...
const PROD_ACCOUNTS_APP_ORIGINS: &[&str] = &["https://accounts.ccode.dev"];
...
const CCODE_OAUTH2_LOCAL_ISSUER: &str = "http://localhost:22255";
const DEFAULT_OAUTH2_REFERRER: &str = "ccode-build";
/// Returns `true` when `CCODE_LOCAL_AUTH=1` is set,
/// indicating the local accounts-app should be used as the OAuth2 issuer.
pub fn use_local_auth() -> bool {
    std::env::var("CCODE_LOCAL_AUTH")
        .map(|v| !v.is_empty() && v != "0")
        .unwrap_or(false)
}
/// Returns the active ccode OAuth2 issuer — the local-dev issuer when
/// `CCODE_LOCAL_AUTH=1` is set, otherwise the production issuer.
pub fn ccode_oauth2_issuer() -> &'static str {
    if use_local_auth() {
        CCODE_OAUTH2_LOCAL_ISSUER
    } else {
        CCODE_OAUTH2_ISSUER
    }
}
/// Returns `true` if `issuer` is a recognised ccode OAuth2 issuer
/// (production **or** local-dev). Use this instead of comparing against
/// [`CCODE_OAUTH2_ISSUER`] directly so that local-dev sessions are still
/// treated as first-party ccode auth.
pub fn is_ccode_oauth2_issuer(issuer: &str) -> bool {
    issuer == CCODE_OAUTH2_ISSUER || issuer == CCODE_OAUTH2_LOCAL_ISSUER
}
/// auth.json scope key used by the pre-OIDC `ccode login --legacy` flow.
/// Matches the key format produced by the original `accounts.ccode.dev` relay auth.
pub const LEGACY_AUTH_SCOPE: &str = "https://accounts.ccode.dev/sign-in";
impl CcodeComConfig {
    /// Whether `ccode.api_key` auth is disabled. Pinning a team
    ...
    /// `CCODE_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
    ...
    /// pin is fail-closed. Explicit `ccode login --devbox` / `--api-key` bypass
    ...
    pub fn auth_scope(&self) -> String {
        if let Some(ref oidc) = self.oidc {
            ...
            unreachable!("oauth2 config is always present (ccode default or env override)")
        }
    }
}
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("CCODE_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("CCODE_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("CCODE_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("CCODE_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("CCODE_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("CCODE_OAUTH2_REFERRER")
...
impl Default for CcodeComConfig {
    fn default() -> Self {
        ...
        let oauth2 = if oidc.is_some() {
            ...
            Some(
                OAuth2ProviderConfig::from_env().unwrap_or_else(|| OAuth2ProviderConfig {
                    issuer: ccode_oauth2_issuer().to_owned(),
        ...
        Self {
            ccode_ws_origin: std::env::var("CCODE_WS_ORIGIN")
                .unwrap_or_else(|_| PROD_WS_ORIGIN.to_owned()),
            ccode_ws_url: std::env::var("CCODE_WS_URL")
                .unwrap_or_else(|_| PROD_RELAY_WS_URL.to_owned()),
            token_header: "ccode-cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("CCODE_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("CCODE_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("CCODE_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("CCODE_DISABLE_API_KEY_AUTH")
                .ok()
...
/// Parse a boolean env-var value for ccode's on/off flags. A bare presence
/// enables the flag, but the common falsy spellings (`0`, `false`, `off`,
/// `no`, empty) count as disabled — so e.g. `CCODE_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `CCODE_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("CCODE_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("CCODE_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("CCODE_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("CCODE_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("CCODE_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
mod tests {
    ...
    fn team_auth_scope_is_base_scope() {
        let cfg = OAuth2ProviderConfig {
            issuer: "https://auth.ccode.dev".into(),
            client_id: "client-123".into(),
            scopes: default_team_oauth2_scopes(),
            principal_type: Some("Team".into()),
            principal_id: Some("team-abc".into()),
            referrer: Some("ccode-build".into()),
        };
        assert_eq!(cfg.auth_scope(), "https://auth.ccode.dev::client-123");
    }
    #[test]
    ...
    /// (served from accounts.ccode.dev) delivers the code via `fetch(..., cors)`, so
    ...
    fn allowed_accounts_app_origins_are_frozen() {
        assert_eq!(PROD_ACCOUNTS_APP_ORIGINS, &["https://accounts.ccode.dev"]);
        assert_eq!(allowed_accounts_app_origins(), PROD_ACCOUNTS_APP_ORIGINS);
    }
    /// FROZEN client contract: the 10 scopes the ccode OAuth2 client requests.
    ...
    fn preferred_method_deserializes_from_toml() {
        let cfg: CcodeComConfig = toml::from_str(
            r#"
        ...
        let cfg: CcodeComConfig = toml::from_str("").expect("parse empty");
