...
use crate::util::ccode_auth_credentials::CcodeAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::CcodeAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| ccode_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: CcodeAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = CcodeAuthCredentials::new(None);
...
/// Resolves the embedding credentials for `embed_base_url`, attaching the ccode
/// session credential only to ccode-operated endpoints over `https`.
pub(crate) fn embedding_session_credentials(
    embed_base_url: &str,
    auth_manager: Option<&Arc<AuthManager>>,
    api_key_provider: Option<ccode_tools::types::SharedApiKeyProvider>,
) -> ccode_memory::EndpointScopedCredentials {
    ...
    ccode_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
        crate::util::is_ccode_api_bearer_url,
        auth_credentials,
        api_key_provider,
    )
}
...
///
/// - `"ccode-shell"`   — classic Ccode CLI / TUI (ccode-shell)
/// - `"ccode-pager"`   — new Ccode Pager / TUI (ccode-pager)
/// - `"ccode-desktop"` — Ccode Desktop app
/// - `"ccode-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-ccode-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-ccode-client-identifier`
///
/// to the underlying `ccode-fs::StorageClient` via
...
///   - `user_token` (normal OIDC users via com scope) → sends Bearer + `X-Ccode-Token-Auth`
...
pub fn build_storage_client_for_proxy(
...
) -> ccode_fs::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn ccode_fs::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        ccode_fs::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(ccode_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = CcodeAuthCredentials::new(user_token);
...
/// Bridge that lets `StorageClient` (which lives in ccode-fs)
...
impl ccode_fs::storage_client::Auth401AttributionCallback for StorageClientAttributionBridge {
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = CcodeAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        ccode_telemetry::external::set_identity(
            ccode_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `ccode-pager` and `ccode-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> ccode_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let ccode_com_config = crate::auth::CcodeComConfig::default();
    let exporter = ccode_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = ccode_com_config.token_header.clone();
    let ccode_home = crate::util::ccode_home::ccode_home();
    let bootstrap = Arc::new(AuthManager::new(&ccode_home, ccode_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    ccode_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::CcodeComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use ccode_auth::AuthCredentialProvider;
    /// Serializes tests that pin `CCODE_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `CCODE_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("CCODE_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("CCODE_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("CCODE_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("CCODE_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> CcodeAuth {
        CcodeAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<CcodeAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::CcodeComConfig::default(),
        ));
        ...
        impl crate::auth::refresh::TokenRefresher for OkRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(CcodeAuth {
                    key: "fresh".into(),
                    auth_mode: crate::auth::AuthMode::Oidc,
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let mgr = make_manager(
            &dir,
            Some(make_auth("ccode-session-token", ChronoDuration::hours(1))),
        );
        let api_key_provider: ccode_tools::types::SharedApiKeyProvider =
            Arc::new(crate::auth::manager::SharedAuthKeyProvider(mgr.clone()));
        for denied in ["https://byok.attacker.example/v1", "http://api.ccode.dev/v1"] {
            let resolved =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use ccode_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("ccode-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("ccode-token-EX").as_str())
        );
        assert!(dep.api_key_id.is_none());
        let api_auth = CcodeAuth {
    ...
    /// user token, matching the precedence in `CcodeAuthCredentials::apply`.
