...
use super::is_ccode_oauth2_issuer;
...
pub(super) const LEGACY_SCOPE: &str = "https://accounts.ccode.dev/sign-in";

/// auth.json scope key for plain API key auth (desktop login, `ccode login --api-key`).
pub const API_KEY_SCOPE: &str = "ccode::api_key";
...
/// server enrichment opts in. Single source for `CcodeAuth`, `AuthMeta`, and
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from ccode-desktop login or `ccode login --api-key`)
    ApiKey,
}
...
pub struct CcodeAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_ccode_code_access: Option<bool>,
    ...
    /// provider's `issuer` claim. In both modes an ccode.dev issuer marks the
    /// credential first-party (`is_ccode_auth`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl std::fmt::Debug for CcodeAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CcodeAuth")
...
impl CcodeAuth {
    ...
    /// `true` when the token comes from a first-party ccode account —
    /// either an OIDC login against https://auth.ccode.dev (or the local-dev
    /// equivalent), or an external auth provider that declared an ccode
    ...
    pub fn is_ccode_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::Oidc | AuthMode::External => self
                .oidc_issuer
                .as_deref()
                .is_some_and(is_ccode_oauth2_issuer),
            AuthMode::ApiKey | AuthMode::WebLogin => false,
        }
    }

    /// `true` when this auth can access ccode.dev managed MCP connectors.
    pub fn is_managed_mcp_eligible(&self) -> bool {
        self.is_ccode_auth() || self.auth_mode == AuthMode::WebLogin
    }
    ...
    /// (`is_ccode_auth`), matching the built-in devbox login they replace.
    /// Plain API keys never do.
    pub fn is_session_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::WebLogin | AuthMode::Oidc => true,
            AuthMode::External => self.is_ccode_auth(),
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &CcodeAuth) {
...
#[cfg(test)]
impl CcodeAuth {
    /// Returns a `CcodeAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// CcodeAuth { key: "my-key".into(), ..CcodeAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, CcodeAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `ccode login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<CcodeAuth> {
...
/// Early-invalidation buffer. Override with `CCODE_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("CCODE_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &CcodeAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &CcodeAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> CcodeAuth {
        CcodeAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    #[test]
    fn is_ccode_auth_matrix() {
        use crate::auth::CCODE_OAUTH2_ISSUER;
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| CcodeAuth {
            oidc_issuer: issuer.map(str::to_owned),
            ..make_auth(mode)
        };

        // Only Oidc/External qualify, and only with an ccode.dev issuer.
        assert!(with_issuer(AuthMode::Oidc, Some(CCODE_OAUTH2_ISSUER)).is_ccode_auth());
        assert!(with_issuer(AuthMode::External, Some(CCODE_OAUTH2_ISSUER)).is_ccode_auth());
        assert!(!with_issuer(AuthMode::Oidc, None).is_ccode_auth());
        assert!(!with_issuer(AuthMode::External, None).is_ccode_auth());
        assert!(!with_issuer(AuthMode::Oidc, Some("https://idp.acme.example")).is_ccode_auth());
        assert!(!with_issuer(AuthMode::External, Some("https://idp.acme.example")).is_ccode_auth());

        // ApiKey / WebLogin stay false even with an ccode.dev issuer set.
        assert!(!with_issuer(AuthMode::ApiKey, Some(CCODE_OAUTH2_ISSUER)).is_ccode_auth());
        assert!(!with_issuer(AuthMode::WebLogin, Some(CCODE_OAUTH2_ISSUER)).is_ccode_auth());
    }
    ...
    fn is_session_auth_requires_first_party_for_external() {
        ...
        assert!(with_issuer(AuthMode::External, Some(CCODE_OAUTH2_ISSUER)).is_session_auth());
        ...
        // Plain API keys never do.
        assert!(!with_issuer(AuthMode::ApiKey, Some(CCODE_OAUTH2_ISSUER)).is_session_auth());
    }
    ...
    fn user_info_subscription_tier_present() {
        ...
        assert_eq!(info.subscription_tier.as_deref(), Some("PremiumPro"));
    }
