...
use super::super::config::{ForceLoginTeam, CcodeComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, CcodeAuth};
use base64::Engine;
...
pub fn is_configured(config: &CcodeComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &CcodeComConfig) -> Option<ForceLoginTeam> {
...
mod tests {
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "ccode-cli:access".into()],
            audience: None,
        };
        ...
        let discovery = Discovery {
            authorization_endpoint: "https://auth.ccode.dev/authorize".into(),
            token_endpoint: "https://auth.ccode.dev/token".into(),
            jwks_uri: None,
            id_token_signing_alg_values_supported: None,
        };
        let pkce = Pkce {
        ...
        assert!(url.contains("referrer=ccode-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("ccode-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.ccode.dev/authorize".into(),
            token_endpoint: "https://auth.ccode.dev/token".into(),
        ...
        assert!(url.contains("referrer=ccode-desktop"));
        assert!(!url.contains("referrer=ccode-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access ccode-cli:access api:access",
            "principal_type": "Team",
