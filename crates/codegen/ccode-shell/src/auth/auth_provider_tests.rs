...
async fn provider_refresh_sets_expired_env() {
    let provider = AuthProviderRef::new(
        ...
        AuthProviderConfig {
            command: "printf 'tok-%s' \"${CCODE_AUTH_EXPIRED:-0}\"".to_owned(),
            args: None,
            token_ttl_secs: Some(3600),
            timeout_secs: None,
            cwd: None,
    ...
    assert_eq!(
        ...
        Some("tok-0"),
        "first mint runs without CCODE_AUTH_EXPIRED"
    );
    test_expire_provider_token("test-expired-env");
    assert_eq!(
        provider.ensure_fresh_token(None).await.rotated().as_deref(),
        Some("tok-1"),
        "re-mints run with CCODE_AUTH_EXPIRED=1"
    );
}
...
/// via `CCODE_AUTH_PROVIDER_*`, so a refresh-grant command can refresh instead of
...
async fn re_mint_hands_the_prior_token_back_to_the_command() {
    let provider = AuthProviderRef::new(
        ...
        AuthProviderConfig {
            command: "printf 'seen-%s' \"${CCODE_AUTH_PROVIDER_ACCESS_TOKEN:-none}\"".to_owned(),
            args: None,
            token_ttl_secs: Some(3600),
            timeout_secs: None,
            cwd: None,
