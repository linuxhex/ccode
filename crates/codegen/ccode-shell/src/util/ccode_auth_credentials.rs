...
use std::sync::Arc;
/// Credentials for authenticating with ccode backend services.
///
...
/// User token (ccode users) sends `Bearer` + `X-Ccode-Token-Auth: ccode-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct CcodeAuthCredentials {
...
impl std::fmt::Debug for CcodeAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CcodeAuthCredentials")
            .field(
                "user_token",
...
impl CcodeAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your CCODE_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `ccode login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> CcodeAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> CcodeAuthCredentials {
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            builder
                ...
                .header(
                    obfstr::obfstr!("X-Ccode-Token-Auth"),
                    obfstr::obfstr!("ccode-cli"),
                )
        } else {
            builder
        };
...
impl ccode_auth::HttpAuth for CcodeAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        CcodeAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = CcodeAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = CcodeAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = CcodeAuthCredentials::new(Some("static-token".into()));
