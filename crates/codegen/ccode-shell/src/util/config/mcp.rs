...
use ccode_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use ccode_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `ccode-config-types` (config
...
pub use ccode_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use ccode_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.ccode/config.toml` at each level.
/// Project-scoped `.ccode/config.toml` entries override global `~/.ccode/config.toml`
...
/// `.ccode/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.ccode/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.ccode/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.ccode/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .ccode/config.toml"
                );
...
pub fn mcp_preferences_path() -> PathBuf {
    ccode_config::ccode_home().join("mcp_preferences.json")
}
...
/// Used to surface setup-required rows and drive `ccode.dev/mcp/setup`.
pub fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&ccode_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    ccode_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.ccode/config.toml`.
...
/// Upsert an MCP server entry in `~/.ccode/config.toml`.
///
...
/// config file, e.g. a project-scoped `.ccode/config.toml`.
...
/// Delete an MCP server entry from `~/.ccode/config.toml`.
///
...
/// config file, e.g. a project-scoped `.ccode/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = ccode_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.ccode/config.toml` files.
...
// `.mcp.json` discovery moved to `ccode-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use ccode_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.ccode/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::ccode_home::ccode_home().join("config.toml")
}

/// Path to the user-level config file (`~/.ccode/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.ccode/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".ccode").join("config.toml")
}
...
/// Overrides `[cli] session_registry`; usable before `~/.ccode/config.toml` exists.
pub const SESSION_REGISTRY_ENV_VAR: &str = "CCODE_SESSION_REGISTRY";

pub fn session_registry_from_env_opt() -> Option<bool> {
    ccode_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = ccode_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = ccode_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = ccode_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.ccode/skills", "~/.ccode/skills/special/SKILL.md"]
ignore = ["~/.ccode/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.ccode/skills", "~/.ccode/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.ccode/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${CCODE_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        ...
        std::fs::create_dir_all(&ccode_dir).unwrap();
        std::fs::write(
            ccode_dir.join("config.toml"),
            r#"
