...
/// Re-exported from `ccode-announcements` for backward compatibility.
pub use ccode_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `CCODE_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("CCODE_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid CCODE_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `CCODE_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
