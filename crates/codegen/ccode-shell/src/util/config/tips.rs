...
/// `CCODE_TIPS_OVERRIDE` env var overrides everything (debug builds only).
...
pub fn resolve_tips(
    ...
    if let Ok(raw) = std::env::var("CCODE_TIPS_OVERRIDE") {
...
pub fn resolve_tips_from_disk(
    ...
    crate::util::tips::pick_and_advance(&all, ccode_home)
}
...
/// Parse a `CCODE_SLASH_COMMAND_TAGS` payload (a JSON object of string→string)
...
fn parse_slash_command_tags_json(raw: Option<&str>) -> std::collections::HashMap<String, String> {
    ...
    match serde_json::from_str::<std::collections::BTreeMap<String, String>>(raw) {
        ...
        Err(e) => {
            tracing::warn!(
                error = %e,
                "ignoring malformed CCODE_SLASH_COMMAND_TAGS; expected a JSON object of string values"
            );
...
/// Read per-command tags from the `CCODE_SLASH_COMMAND_TAGS` env var. Unset →
/// empty; malformed → warn + empty.
fn slash_command_tags_from_env() -> std::collections::HashMap<String, String> {
    parse_slash_command_tags_json(std::env::var("CCODE_SLASH_COMMAND_TAGS").ok().as_deref())
}
...
/// settings (base) → local `[slash_command_tags]` → `CCODE_SLASH_COMMAND_TAGS`
...
mod tests {
    ...
    // so ambient `CCODE_SLASH_COMMAND_TAGS` can't affect these assertions.
    #[test]
