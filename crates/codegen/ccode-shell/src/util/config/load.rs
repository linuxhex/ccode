...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("CCODE_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.ccode/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.ccode/config.toml` `[permission]` section
///
...
"#;
            ...
            && let Some(TomlValue::Table(models)) = table.get("models")
        {
            let default = models
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            assert_eq!(default.as_deref(), Some("ccode-code-fast-1"));
        } else {
