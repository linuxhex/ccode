...
///
/// Reads `stable_version` from `~/.ccode/version.json` (written by the
...
///
/// This is a lightweight duplicate of `ccode_update::channel_name()` for
/// use in `ccode-shell` which cannot depend on `ccode-update`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    use std::sync::OnceLock;
    static NAME: OnceLock<Option<&'static str>> = OnceLock::new();
    *NAME.get_or_init(|| {
        let version_path = crate::util::ccode_home::ccode_home().join("version.json");
        let content = std::fs::read_to_string(&version_path).ok()?;
        let parsed: serde_json::Value = serde_json::from_str(&content).ok()?;
        let stable = parsed.get("stable_version")?.as_str()?;
        let current = semver::Version::parse(ccode_version::VERSION).ok()?;
...
impl VersionKnob {
    ...
    pub(crate) fn env_var(self) -> &'static str {
        match self {
            VersionKnob::Minimum => "CCODE_MINIMUM_VERSION",
            VersionKnob::Maximum => "CCODE_MAXIMUM_VERSION",
            VersionKnob::RequiredMinimum => "CCODE_REQUIRED_MINIMUM_VERSION",
            VersionKnob::RequiredMaximum => "CCODE_REQUIRED_MAXIMUM_VERSION",
        }
    }
...
mod tests {
    ...
    fn env_tightens_but_cannot_loosen() {
        ...
        let tighten = |var: &str| match var {
            "CCODE_MINIMUM_VERSION" => Some("0.2.150".to_string()),
            "CCODE_MAXIMUM_VERSION" => Some("0.2.180".to_string()),
            _ => None,
        };
        ...
        let loosen = |var: &str| match var {
            "CCODE_MINIMUM_VERSION" => Some("0.2.1".to_string()),
            "CCODE_MAXIMUM_VERSION" => Some("0.2.999".to_string()),
            _ => None,
        };
    ...
    fn a_user_bound_cannot_cancel_a_managed_hard_bound() {
        ...
        let low_ceiling =
            |var: &str| (var == "CCODE_REQUIRED_MAXIMUM_VERSION").then(|| "0.2.50".to_string());
        ...
        let high_floor =
            |var: &str| (var == "CCODE_REQUIRED_MINIMUM_VERSION").then(|| "0.2.200".to_string());
        ...
        let both = |var: &str| match var {
            "CCODE_REQUIRED_MINIMUM_VERSION" => Some("99.0.0".to_string()),
            "CCODE_REQUIRED_MAXIMUM_VERSION" => Some("0.0.1".to_string()),
            _ => None,
        };
