...
/// [`ccode_config::env_bool`] parser (`CCODE_TOOLS_FIND_BFS` /
/// `CCODE_TOOLS_GREP_UGREP`, plus the `CCODE_FIND_BFS` / `CCODE_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.ccode/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`ccode_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = ccode_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = ccode_config::env_bool(primary).or_else(|| ccode_config::env_bool(alias));
    ...
    (
        resolve("CCODE_TOOLS_FIND_BFS", "CCODE_FIND_BFS", "find_bfs"),
        resolve("CCODE_TOOLS_GREP_UGREP", "CCODE_GREP_UGREP", "grep_ugrep"),
    )
}
...
) -> Option<ccode_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<ccode_tools::util::ShellEnvironmentPolicy>() {
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "CCODE_LOGIN_ENV";
...
const ENV_SCHEDULER_BACKGROUND_LOOPS: &str = "CCODE_SCHEDULER_BACKGROUND_LOOPS";
...
///
/// Precedence: requirements > env (`CCODE_SCHEDULER_BACKGROUND_LOOPS`) > user
...
/// the shared [`ccode_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "CCODE_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`CCODE_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            ccode_tools::implementations::ccode_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`CCODE_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        ccode_tools::implementations::ccode_build::ask_user_question::response_timeout_env_secs(),
...
) -> ccode_tools::implementations::ccode_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use ccode_tools::implementations::ccode_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = ccode_tools::implementations::ccode_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes CCODE_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
...
mod shell_env_policy_tests {
    use super::*;
    use ccode_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
