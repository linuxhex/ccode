pub mod config;
pub mod ccode_auth_credentials;
pub mod hooks;
pub(crate) mod subprocess;
pub(crate) mod user_identity;

// The foundation utilities live in `ccode-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `ccode_shell::util::…` users compile
// unchanged.
pub use ccode_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let ccode_rules = ccode_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == ccode_home || parent == ccode_rules)
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn ccode_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_ccode_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
            Path::new("/custom/ccode/worktrees/repo/src/AGENTS.md"),
