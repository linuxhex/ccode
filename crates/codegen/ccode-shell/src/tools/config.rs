...
use ccode_sampler::SamplerConfig;
use ccode_tools::implementations::ccode_build;
use ccode_tools::registry::types::ToolConfig;

/// Production ccode-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production ccode-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `CCODE_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `CCODE_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> ccode_tools::implementations::ccode_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("CCODE_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| ccode_config::env_bool("CCODE_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `ccode_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `CcodeBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `CcodeBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "CcodeBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "CcodeBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"CcodeBuild:read_file"));
        assert!(ids.contains(&"CcodeBuild:search_replace"));
        assert!(ids.contains(&"CcodeBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"CcodeBuildHashline:hashline_read"));
        assert!(ids.contains(&"CcodeBuildHashline:hashline_edit"));
        assert!(ids.contains(&"CcodeBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            ccode_agent::config::AgentDefinition::plan(),
            ccode_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "CcodeBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"CcodeBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"CcodeBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"CcodeBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production ccode-build must set the 10h foreground ceiling"
        );
    }
