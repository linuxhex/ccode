//! Notification bridge: translates `ccode-tools` `ToolNotification` events
//! into `ccode-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use ccode_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use ccode_tools::types::output::{BashOutput, ToolOutput};
use ccode_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    /// `ccode.dev/incrementalBashOutput` capability.
    ...
    pub task_wake_suppressed: ccode_tools::reminders::task_completion::TaskWakeSuppressed,
    ...
    /// next-turn exit reminder. Ccode-build leaves this `false` — its
...
fn stamp_scheduler_meta(
    ...
    meta.insert("ccode.dev/schedulerGeneration".to_owned(), generation.into());
    meta.insert("ccode.dev/schedulerRevision".to_owned(), revision.into());
}
...
async fn handle_scheduled_task_removed(
    config: &NotificationBridgeConfig,
    removed: ccode_tools::notification::ScheduledTaskRemoved,
    ...
    let result: Result<Box<serde_json::value::RawValue>, String> = async {
        ...
        let update = crate::session::storage::SessionUpdate::Ccode(Box::new(notification));
        ...
        Ok(params) => {
            ...
            config
                ...
                .forward_fire_and_forget(acp::ExtNotification::new(
                    "ccode.dev/scheduled_task_deleted",
                    params.into(),
                ));
            Ok(())
        }
...
async fn emit_current_mode_update(
    config: &NotificationBridgeConfig,
    mode: ccode_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::BashExecutionBackgrounded(bg) => {
            ...
            let _ = config.persistence.tx.send(PersistenceMsg::Update(
                crate::session::storage::SessionUpdate::Ccode(Box::new(notification.clone())),
            ));
            let params = serde_json::to_value(&notification)
            ...
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("ccode.dev/task_backgrounded", params.into());
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == ccode_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    ccode_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    ccode_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = ccode_tools::reminders::wrap_reminder(&body);
            ...
            if let Some(params) = params {
                let notification: acp::ExtNotification =
                    acp::ExtNotification::new("ccode.dev/task_completed", params.into());
                config.gateway.forward_fire_and_forget(notification);
            }
            let _ = config
                .session_cmd_tx
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, ccode_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    ...
                    .tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, ccode_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
        ...
        ToolNotification::ScheduledTaskFired(fired) => {
                ...
                serde_json::to_value(&fired_notif).and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "ccode.dev/scheduled_task_fired",
                        params.into(),
                    ));
            }
        }
        ...
        ToolNotification::ScheduledTaskCreated(created) => {
                ...
                .and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "ccode.dev/scheduled_task_created",
                        params.into(),
                    ));
            }
        }
...
mod tests {
    use super::*;
    use ccode_tools::computer::types::TaskKind;
    ...
    fn make_test_config_full() -> (
        NotificationBridgeConfig,
        mpsc::UnboundedReceiver<ccode_acp::AcpClientMessage>,
    ...
    fn make_test_config_full_raw() -> (
        ...
        let gateway = ccode_acp::AcpAgentGatewaySender::new(gateway_tx);
    ...
    /// drain it). The pager's `ccode.dev/task_completed` notification still fires.
    #[tokio::test]
    async fn bash_task_completed_suppresses_auto_wake_during_goal_loop() {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let ccode_acp::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "ccode.dev/task_completed"
            {
                found_ext = true;
            }
        }
        assert!(
            found_ext,
            "ccode.dev/task_completed ExtNotification must still be sent for UI"
        );
    }
    ...
    fn task_completed_will_wake(
        gateway_rx: &mut mpsc::UnboundedReceiver<ccode_acp::AcpClientMessage>,
    ) -> Option<bool> {
        while let Ok(msg) = gateway_rx.try_recv() {
            if let ccode_acp::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "ccode.dev/task_completed"
    ...
    async fn task_completed_notification_stamps_will_wake() {
        ...
        *config
        ...
        while let Ok(message) = persistence_rx.try_recv() {
            if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(update)) =
                message
                && matches!(
                    &update.update,
        ...
        assert!(
            persisted,
            "declined admission must still persist ccode.dev/task_completed"
        );
    }
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(ccode_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            ccode_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
        ...
        match msg {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(notif)) => {
                assert!(matches!(
                    &notif.update,
                    crate::extensions::notification::SessionUpdate::ScheduledTaskCreated { .. }
                ));
                let meta = notif.meta.as_ref().expect("scheduler metadata");
                assert_eq!(meta["ccode.dev/schedulerGeneration"], "generation-a");
                assert_eq!(meta["ccode.dev/schedulerRevision"], 1);
                assert!(
                    notif
                        .meta
                        .as_ref()
                    ...
                    "persisted ccode bridge lines must carry an eventId"
                );
            }
            _ => panic!("expected PersistenceMsg::Update(Ccode(ScheduledTaskCreated))"),
        }
    }
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification = ToolNotification::BashOutputChunk(
            ccode_tools::notification::types::BashOutputChunk {
                base: ccode_tools::notification::types::BashNotificationBase {
        ...
        let broadcast_id = match gateway_rx.try_recv().expect("chunk must be broadcast") {
            ccode_acp::AcpClientMessage::SessionNotification(args) => args
                .request
                .meta
                .as_ref()
                .and_then(|m| m.get("eventId"))
    ...
    async fn scheduled_task_removed_is_persisted() {
        let (config, _gateway_rx, mut persistence_rx, _cmd_rx) = make_test_config_full();
        let removed = ccode_tools::notification::ScheduledTaskRemoved {
            task_id: "loop-1".into(),
            generation: "generation-a".into(),
            revision: 2,
        };
        ...
        match msg {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(notif)) => {
                ...
                assert!(
                    ccode_persisted_event_id(&notif).is_some(),
                    "the persisted deletion line must be stamped"
                );
                let meta = notif.meta.as_ref().expect("scheduler metadata");
                assert_eq!(meta["ccode.dev/schedulerGeneration"], "generation-a");
                assert_eq!(meta["ccode.dev/schedulerRevision"], 2);
            }
            _ => panic!("expected PersistenceMsg::Update(Ccode(ScheduledTaskDeleted))"),
        }
    }
    #[tokio::test]
    async fn acknowledged_scheduler_removal_appends_before_ack_and_broadcast() {
        ...
        let persistence = async {
            let PersistenceMsg::AppendUpdateDurablyAndAck {
                update: crate::session::storage::SessionUpdate::Ccode(notification),
                respond_to,
            } = persistence_rx.recv().await.expect("durable append")
            else {
                panic!("expected durable scheduler tombstone");
            };
            assert_eq!(notification.meta.unwrap()["ccode.dev/schedulerRevision"], 17);
        ...
        assert!(matches!(
            gateway_rx.try_recv(),
            Ok(ccode_acp::AcpClientMessage::ExtNotification(_))
        ));
    }
    fn ccode_persisted_event_id(
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            ccode_tools::notification::types::BashExecutionBackgrounded {
                base: ccode_tools::notification::types::BashNotificationBase {
        ...
        match persistence_rx.try_recv().expect("must persist") {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ccode(notif)) => {
                assert!(ccode_persisted_event_id(&notif).is_some());
            }
            _ => panic!("expected Ccode update"),
        }
    }
    #[tokio::test]
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            ccode_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
        ...
        let ccode_acp::AcpClientMessage::ExtNotification(fired) = fired else {
            panic!("expected scheduler fire notification");
        };
        let value: serde_json::Value = serde_json::from_str(fired.request.params.get()).unwrap();
        assert_eq!(value["_meta"]["ccode.dev/schedulerGeneration"], "generation-a");
        assert_eq!(value["_meta"]["ccode.dev/schedulerRevision"], 3);
    }
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        let notification =
            ToolNotification::PlanModeExited(ccode_tools::notification::types::PlanModeExited {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let ccode_acp::AcpClientMessage::SessionNotification(args) = msg
    ...
    /// Default (ccode) polarity: the exit_plan_mode tool result is the model's
    ...
    async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
        ...
        let notification =
            ToolNotification::PlanModeExited(ccode_tools::notification::types::PlanModeExited {
                tool_call_id: "tc-exit-ccode".into(),
                plan_content: Some("- step 1".into()),
                plan_file_path: "/tmp/test-session/plan.md".into(),
            });
        let mut offsets = HashMap::new();
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification = ToolNotification::PlanModeEntered(
            ccode_tools::notification::types::PlanModeEntered {
