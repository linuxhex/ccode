...
use ccode_tools::types::{KillOutcome, TaskSnapshot};
...
/// Wire DTO for the `ccode.dev/task/kill` ext request.
///
/// `pub` (with both serde directions) so ACP clients (ccode-pager) build
...
/// Wire DTO for the `ccode.dev/task/kill` ext response payload (nested under
...
/// Wire DTO for the `ccode.dev/subagent/cancel` ext request.
///
/// `pub` (with both serde directions) so ACP clients (ccode-pager) build
...
/// Wire DTO for the `ccode.dev/subagent/cancel` response payload (under `result` in
...
/// Handle `ccode.dev/scheduler/*` extension methods.
...
/// Handle `ccode.dev/subagent/*` extension methods.
pub async fn handle_subagent(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ccode.dev/subagent/list_running" => {
...
mod tests {
    ...
    // ── ccode.dev/subagent/cancel outcome wire DTO ──────────────────────────
