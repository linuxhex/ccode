//! ACP extension handler for bulk session updates (`ccode.dev/session/updates`).
//!
...
//! the update type (`"session/update"` for ACP, `"_ccode.dev/session/update"` for
//! ccode extensions) and extract the notification payload from `params`.
//!
...
/// Send updates as chunked `_ccode.dev/session/updates/chunk` notifications.
    ...
    for (chunk_idx, chunk) in lines.chunks(chunk_size).enumerate() {
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            gateway.forward_fire_and_forget(acp::ExtNotification::new(
                "ccode.dev/session/updates/chunk",
...
mod tests {
    ...
    fn dummy_gateway() -> ccode_acp::AcpAgentGatewaySender {
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("ccode.dev/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_tail_request_with_rewind_returns_only_live_timeline() {
        ...
        std::fs::write(
            ...
            [
                ...
                r#"{"timestamp":5,"method":"_ccode.dev/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":1}}}"#,
    ...
    fn xai_rewind(target: usize) -> String {
        format!(
            r#"{{"timestamp":0,"method":"_ccode.dev/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"rewind_marker","target_prompt_index":{target}}}}}}}"#
        )
    }
