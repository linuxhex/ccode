//! `ccode.dev/session/usage` — cumulative session token/cost as [`PromptUsage`].
//!
//! Projects the in-memory [`ccode_chat::UsageLedger`] (main-loop + folded
...
/// Wire response for `ccode.dev/session/usage`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ccode.dev/session/usage" => handle_session_usage(agent, args).await,
...
mod tests {
    ...
    fn response_serializes_ledger_as_prompt_usage_wire_shape() {
        ...
        ledger.record_main_loop_call("ccode-build", &usage(100, 10), Some(50), Some(20_000_000));
        ...
        assert_eq!(v["usage"]["modelUsage"]["ccode-build"]["inputTokens"], 100);
