...
//! using the ccode-graph index.
//!
//! ## Extension Methods
//!
//! | Method | Description |
//! |--------|-------------|
//! | `ccode.dev/code/goto-definition` | Definition location(s) for symbol at position |
//! | `ccode.dev/code/goto-references` | Reference location(s) for symbol at position |
//! | `ccode.dev/code/find-definitions` | All definitions of a symbol by name |
//! | `ccode.dev/code/find-references` | All references to a symbol by name |
//! | `ccode.dev/code/status` | Indexing status |
...
/// Reason string for the `ccode.dev/code/status` response.
///
...
pub async fn handle(
    ...
    use ccode_workspace::workspace_ops::*;

    match args.method.as_ref() {
        "ccode.dev/code/goto-definition" => {
        ...
        "ccode.dev/code/goto-references" => {
            let req: GotoRequest = serde_json::from_str(args.params.get())
                .map_err(|e| acp::Error::invalid_params().data(format!("invalid params: {e}")))?;
            let cwd = resolve_cwd(agent, req.cwd.clone(), req.session_id.as_ref())?;
            let was_newly_started =
        ...
        "ccode.dev/code/find-definitions" => {
        ...
        "ccode.dev/code/find-references" => {
            let req: FindSymbolRequest = serde_json::from_str(args.params.get())
                .map_err(|e| acp::Error::invalid_params().data(format!("invalid params: {e}")))?;
            let cwd = resolve_cwd(agent, req.cwd.clone(), req.session_id.as_ref())?;
            let was_newly_started =
...
fn to_code_nav_ext_response(resp: ccode_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for ccode-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise ccode.dev/codeNavigation.enabled to use code navigation"
        }
