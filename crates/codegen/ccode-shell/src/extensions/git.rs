...
use ccode_workspace::session::git::{
...
/// Request for ccode.dev/git/checkout_commit extension method.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
async fn resolve_git_root(
    ...
    if let Some(sid) = session_id {
        if let Some(cwd) = agent.get_session_cwd(sid) {
            let result = ops
                .dispatch(
                    &ccode_workspace::workspace_ops::GitResolveRootReq { cwd },
                    None,
                )
                .await
                .map_err(|e| {
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "ccode.dev/git/status" => {
            ...
            if let Some(ref git_root) = git_root {
                let current_commit = ops
                    .dispatch(
                        &ccode_workspace::workspace_ops::GitCurrentCommitReq {
        ...
        "ccode.dev/git/checkout_session_head" => {
            ...
            let result = ops
                .dispatch(
                    &ccode_workspace::workspace_ops::GitCheckoutCommitReq {
                        git_root: git_root.clone(),
                        head_commit,
