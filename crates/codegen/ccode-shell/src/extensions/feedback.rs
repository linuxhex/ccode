//! `ccode.dev/feedback`, `ccode.dev/feedback/dismiss`, `ccode.dev/btw`, and `ccode.dev/review/*`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ccode.dev/feedback" | "ccode.dev/feedback/dismiss" => {
            tracing::info!("handling user feedback");
            handle_feedback(agent, args).await
        }
        m if m.starts_with("ccode.dev/review") => {
...
/// Handle `ccode.dev/btw` -- a side question that doesn't interrupt the current turn.
...
async fn handle_feedback(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !agent.cfg.borrow().is_feedback_enabled() {
        return Err(acp::Error::internal_error().data(
            "Feedback is disabled. To enable, set CCODE_FEEDBACK_ENABLED=true or \
...
///
/// Methods:
/// - `ccode.dev/review/comment`: record a new inline code comment to cloud storage
/// - `ccode.dev/review/comment/delete`: record a tombstone event for a deleted comment
async fn handle_review(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ccode.dev/review/comment" => {
        ...
        "ccode.dev/review/comment/delete" => {
