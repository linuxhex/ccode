//! `ccode.dev/session/repair` — out-of-band recovery for sessions bricked by
...
//!
//! Repairs via [`ccode_chat::compaction_utils::repair_history`]. Resident
...
use ccode_chat::compaction_utils::HistoryRepairReport;
...
/// Response payload for `ccode.dev/session/repair`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ccode.dev/session/repair" => handle_session_repair(agent, args).await,
...
/// back atomically. `ccode_root` is injectable for tests.
async fn repair_on_disk(ccode_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &ccode_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(ccode_root.to_path_buf());
    ...
    let report = ccode_chat::compaction_utils::repair_history(&mut chat_history);
