//! `ccode.dev/share_session` extension handler.
//!
...
async fn handle_share_session(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let auth = require_ccode_auth_for_share(&agent.auth_manager)?;
...
async fn upload_share_data_to_gcs(
    ...
    if let Err(e) = ccode_fs::gcs::upload_bytes_signed(
...
fn require_ccode_auth_for_share(
    auth_manager: &crate::auth::AuthManager,
) -> Result<crate::auth::CcodeAuth, acp::Error> {
    super::auth_gate::require_ccode_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `ccode login` to authenticate.",
    )
}
...
mod tests {
    ...
    use crate::auth::{AuthMode, CcodeAuth};
    ...
    fn make_auth_manager_with_token_expiring_in(
        ...
        // We must explicitly set oidc_issuer to a first-party ccode issuer.
        // Only OIDC tokens against https://auth.ccode.dev (or the local-dev equivalent)
        // return true from is_ccode_auth(). This is required for the share tests to
        // exercise the happy path through require_ccode_auth_for_share.
        let auth = CcodeAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some("https://auth.ccode.dev".to_string()),
            key: "test-key".into(),
    ...
    fn share_works_outside_the_5m_early_invalidation_window() {
        ...
        assert!(require_ccode_auth_for_share(&mgr).is_ok());
    }
    ...
    fn share_succeeds_inside_the_5m_early_invalidation_window() {
        ...
        let res = require_ccode_auth_for_share(&mgr);
        assert!(
            res.is_ok(),
            "require_ccode_auth_for_share must succeed for a still-valid buffered ccode token"
        );
    }
    ...
    fn share_fails_with_no_auth_at_all() {
        ...
        assert!(require_ccode_auth_for_share(&mgr).is_err());
    }

    #[test]
    fn share_rejects_non_ccode_auth_with_actionable_ccode_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            CcodeComConfig::default(),
        ...
        // API key is the simplest non-ccode credential (External and enterprise OIDC
        // are also rejected the same way).
        let non_ccode = CcodeAuth {
            auth_mode: AuthMode::ApiKey,
            key: "ccode-test-key".into(),
            create_time: Utc::now(),
            ..Default::default()
        };
        mgr.hot_swap(non_ccode);

        let err = require_ccode_auth_for_share(&mgr)
            .expect_err("non-ccode accounts (API key, External, enterprise IdP) must be rejected");

        // This is the key assertion the review asked for: we must test the *exact*
        // actionable data string for the non-ccode path (distinct from the generic
