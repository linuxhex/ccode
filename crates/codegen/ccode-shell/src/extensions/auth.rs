//! `ccode.dev/auth/*` and legacy `ccode.dev/{get,set}ApiKey` extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ccode.dev/auth/getBearerToken" => handle_get_bearer_token(agent).await,
        "ccode.dev/getApiKey" => handle_get_api_key(),
        "ccode.dev/setApiKey" => handle_set_api_key(args),
        "ccode.dev/auth/submit_code" => handle_submit_code(agent, args),
        "ccode.dev/auth/get_url" => handle_get_url(agent).await,
        "ccode.dev/auth/cancel" => handle_cancel(agent, args),
        "ccode.dev/auth/logout" => handle_logout(agent, args).await,
        "ccode.dev/auth/info" => handle_info(agent),
        "ccode.dev/auth/check_subscription" => handle_check_subscription(agent).await,
...
fn handle_get_api_key() -> ExtResult {
    let key = crate::agent::auth_method::read_ccode_api_key_env().ok();
...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&ccode_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("CCODE_API_KEY") };
        } else {
            crate::auth::store_api_key(&ccode_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::set_var("CCODE_API_KEY", k) };
        }
    } else {
        crate::auth::clear_api_key(&ccode_home)
            .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `ccode-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a ccode-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("ccode-asset:///{key}")),
        None => None,
    };
    to_raw_response(&AuthInfoResponse {
        ...
        // matching `AuthManager::allows_data_collection` / CcodeAuth Default.
