//! `ccode.dev/session/state` reads a session's metadata columns; `ccode.dev/session/import`
...
/// `ccode.dev/session/state`: return metadata columns keyed by logical name. Errors when
/// the session isn't found on this host, since it reads a single record whose absence
/// is not an empty result (unlike the collection returned by `ccode.dev/session/updates`).
...
/// `ccode.dev/session/import`: recreate a session on this host from mirrored columns and
...
pub async fn handle_import(args: &acp::ExtRequest) -> ExtResult {
    ...
    if !has_local_session {
        ...
        crate::util::ccode_home::ensure_sessions_cwd_dir(&request.cwd)
...
fn sanitize_summary_for_host(summary: &mut serde_json::Map<String, Value>, id: &str, cwd: &str) {
    ...
    set_or_remove(
        summary,
        "ccode_home",
        crate::session::persistence::ccode_home_string(),
    );
    set_or_remove(
        summary,
        "sandbox_profile",
        ccode_sandbox::configured_profile_name().map(String::from),
    );
}
