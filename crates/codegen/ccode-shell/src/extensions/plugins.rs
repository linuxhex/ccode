//! `ccode.dev/plugins/*` extension handlers.
//!
...
pub fn loaded_plugin_to_info(plugin: &ccode_agent::plugins::LoadedPlugin) -> PluginInfo {
    use ccode_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &ccode_agent::plugins::PluginOrigin) -> PluginOrigin {
    use ccode_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectCcode => PluginOrigin::ProjectCcode,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserCcode => PluginOrigin::UserCcode,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ccode.dev/plugins/action" => {
            let req: ccode_hooks_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use ccode_agent::plugins::PluginOrigin as AgentOrigin;
    use ccode_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> ccode_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        ccode_agent::plugins::LoadedPlugin {
    ...
    fn info_carries_origin_and_marketplace_display_name() {
        let plugin = make_loaded_plugin(AgentOrigin::MarketplaceInstall {
            source_name: Some("ccode Official".to_string()),
        ...
        assert_eq!(info.marketplace_source.as_deref(), Some("ccode Official"));
    }
