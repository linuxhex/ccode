//! `ccode.dev/hooks/*` extension handlers.
//!
...
use ccode_hooks::event::{HookEventEnvelope, HookEventName};
use ccode_hooks::matcher::HookMatcher;
use ccode_hooks_types::{HookEvent, HookHandlerType, HookInfo};
...
pub fn hook_spec_to_info(spec: &ccode_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: ccode_hooks::trust::is_hook_disabled(&spec.name),
    }
}

// Wire types for client-registered hooks (`ccode.dev/hooks/run`); the gate that uses
...
/// for both the `ccode.dev/hooks/run` request (gate) and the `ccode.dev/hooks/event` notification
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[ccode_hooks::event::HookEventName] = &[
    ccode_hooks::event::HookEventName::PreToolUse,
    ccode_hooks::event::HookEventName::Stop,
    ccode_hooks::event::HookEventName::SubagentStop,
];
...
/// Response payload for `ccode.dev/hooks/run` (client to agent). `Default` (used on
...
/// Parse client hooks from `session/new` `_meta["ccode.dev/hooks"]`, shaped
...
pub(crate) fn parse_client_hooks(meta: Option<&acp::Meta>) -> ClientHooks {
    ...
    let Some(map) = meta
        .and_then(|m| m.get("ccode.dev/hooks"))
    ...
    for (event_name, value) in map {
        ...
        let Ok(event) = HookEventName::deserialize(de) else {
            tracing::warn!(event = %event_name, "ignoring unknown ccode.dev/hooks event");
            continue;
        };
        let Some(array) = value.as_array() else {
            tracing::warn!(event = %event_name, "ccode.dev/hooks event value is not an array; skipping");
            continue;
        };
...
/// clear) when the request meta carries `ccode.dev/hooks`, else `None` so a reconnect that
...
pub(crate) fn reconnect_client_hooks(meta: Option<&acp::Meta>) -> Option<ClientHooks> {
    meta.and_then(|m| m.get("ccode.dev/hooks"))
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let group = WireGroup::deserialize(value)
        .inspect_err(|err| tracing::warn!(%event, %err, "ignoring malformed ccode.dev/hooks group"))
        .ok()?;
    if group.hook_callback_ids.is_empty() {
        tracing::warn!(%event, "ignoring ccode.dev/hooks group with no hookCallbackIds");
        return None;
    }
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern)
            if event.traits().matcher == ccode_hooks::event::MatcherPolicy::Ignored =>
        {
        ...
        Some(pattern) => match HookMatcher::new(pattern) {
            ...
            Err(err) => {
                tracing::warn!(%event, pattern, %err, "ignoring ccode.dev/hooks group with invalid matcher");
                return None;
            }
        },
    };
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ccode.dev/hooks/action" => {
            let req: ccode_hooks_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn parse_client_hooks_canonicalizes_subagent_alias() {
        let meta = serde_json::json!({
            "ccode.dev/hooks": { "SubagentEnd": [{ "hookCallbackIds": ["cb"] }] }
        });
        let hooks = parse_client_hooks(meta.as_object());
    ...
    /// Reconnect refresh applies hooks only when the load meta carries `ccode.dev/hooks`:
    ...
    fn reconnect_client_hooks_only_when_key_present() {
        ...
        let cleared = reconnect_client_hooks(serde_json::json!({ "ccode.dev/hooks": {} }).as_object());
        assert!(cleared.is_some_and(|h| h.is_empty()));

        let set = reconnect_client_hooks(
            serde_json::json!({
                "ccode.dev/hooks": { "PreToolUse": [{ "hookCallbackIds": ["cb"] }] }
            })
            .as_object(),
        );
    ...
    fn advertised_blocking_events_are_gates() {
        use ccode_hooks::event::GateKind;
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use ccode_hooks::event::{HookEventEnvelope, HookPayload};
