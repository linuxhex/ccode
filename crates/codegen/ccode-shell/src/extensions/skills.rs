...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    for subdir in &subdirs {
        try_add_source(ccode_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "ccode.dev/skills/add" => {
            ...
            .await
            {
                ccode_telemetry::session_ctx::log_event(
                    ccode_telemetry::events::SkillAdded {
                        added_count: 0,
                        total_skills: 0,
                        success: false,
                    },
            ...
            ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::SkillAdded {
        ...
        "ccode.dev/skills/remove" => {
            ...
            {
                ccode_telemetry::session_ctx::log_event(
                    ccode_telemetry::events::SkillRemoved { success: false },
                );
            ...
            ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::SkillRemoved {
                success: true,
            });
