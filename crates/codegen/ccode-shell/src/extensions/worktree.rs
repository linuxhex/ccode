//! Handler for ccode.dev/git/worktree/* extension methods.
...
const WORKTREE_EXT_LOG: &str = "ccode_worktree";
...
impl WorktreeNotificationSender for GatewayWorktreeNotifier {
    async fn send_worktree_status(&self, progress: WorktreeStatus) {
        ...
        let notification = acp::ExtNotification::new("ccode.dev/git/worktree/status", params.into());
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "ccode.dev/git/worktree/create_from_worktree" => {
            let mut req =
            ...
            let result = ops
                .dispatch(
                    &ccode_workspace::workspace_ops::PrepareWorktreeFromWorktreeReq {
                        inner: req.clone(),
                    },
                    None,
                )
        ...
        "ccode.dev/git/worktree/create_from_worktree_sync" => {
            ...
            let resolved_root = ops
                .dispatch(
                    &ccode_workspace::workspace_ops::GitResolveRootReq {
            ...
            if let Some(git_root) = resolved_root {
                let vcs_kind = ops
                    .dispatch(
                        &ccode_workspace::workspace_ops::DetectVcsKindReq {
                            path: git_root.clone(),
                        },
                        None,
                    )
                    .await
                    .unwrap_or(ccode_workspace::session::git::VcsKind::Git);
                if vcs_kind.is_jj() {
            ...
            let result = ops
                .dispatch(
                    &ccode_workspace::workspace_ops::CreateWorktreeFromWorktreeSyncReq {
                        inner: req.into_wire(),
                    },
                    None,
                )
        ...
        "ccode.dev/git/worktree/list" => {
            let req: ccode_workspace::workspace_ops::WorktreeListReq =
        ...
        "ccode.dev/git/worktree/show" => {
            let req = serde_json::from_str::<ShowWorktreeRequest>(args.params.get())?;
            let op = ccode_workspace::workspace_ops::WorktreeShowReq {
        ...
        "ccode.dev/git/worktree/gc" => {
            ...
            let op = ccode_workspace::workspace_ops::WorktreeGcReq {
                dry_run: req.dry_run,
                max_age_secs,
                force: req.force,
            };
        ...
        "ccode.dev/git/worktree/db/stats" => {
            let result = ops
                .dispatch(
                    &ccode_workspace::workspace_ops::WorktreeDbStatsReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "ccode.dev/git/worktree/db/rebuild" => {
            let result = ops
                .dispatch(
                    &ccode_workspace::workspace_ops::WorktreeDbRebuildReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "ccode.dev/git/worktree/db/path" => {
            let result = ops
                .dispatch(
                    &ccode_workspace::workspace_ops::WorktreeDbPathReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
mod tests {
    ...
    fn list_request_with_filters() {
        let json = r#"{"repo": "ccode", "type": ["session", "fork"], "includeAll": true}"#;
        let req: ListWorktreeRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.repo.as_deref(), Some("ccode"));
    ...
    fn db_path_response_serializes() {
        let resp = WorktreeDbPathResponse {
            path: "/home/user/.ccode/worktrees.db".into(),
        };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"path\":\"/home/user/.ccode/worktrees.db\""));
    }
