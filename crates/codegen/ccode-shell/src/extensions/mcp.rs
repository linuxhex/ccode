...
//!
//! - `ccode.dev/mcp/list` — list available MCP servers (agent-scoped or session-annotated)
//! - `ccode.dev/mcp/call` — invoke an MCP tool directly, outside the LLM loop
//! - `ccode.dev/mcp/servers_updated` — notification pushed when managed configs resolve
//! - `ccode.dev/mcp/server_status` — per-server delta pushed by the
...
// rmcp is quarantined in ccode-mcp; see that crate's docs.
use ccode_mcp::rmcp;
...
/// Agent-only `ccode.dev/mcp/*` ACP method/notification names.
///
/// Unlike [`wire::MCP_CALL`] (the cross-SDK contract, which stays in
/// `ccode_mcp::wire`), these methods are private to the agent↔client channel and
...
pub mod mcp_methods {
    ...
    pub const PREFIX: &str = "ccode.dev/mcp/";

    pub const LIST: &str = "ccode.dev/mcp/list";
    pub const READ_RESOURCE: &str = "ccode.dev/mcp/read_resource";
    pub const AUTH_STATUS: &str = "ccode.dev/mcp/auth_status";
    pub const AUTH_TRIGGER: &str = "ccode.dev/mcp/auth_trigger";
    pub const SETUP: &str = "ccode.dev/mcp/setup";
    pub const TOGGLE: &str = "ccode.dev/mcp/toggle";
    pub const TOGGLE_TOOL: &str = "ccode.dev/mcp/toggle_tool";
    pub const UPSERT: &str = "ccode.dev/mcp/upsert";
    pub const DELETE: &str = "ccode.dev/mcp/delete";

    pub const SERVERS_UPDATED: &str = "ccode.dev/mcp/servers_updated";
    pub const TOOLS_CHANGED: &str = "ccode.dev/mcp/tools_changed";
    pub const INIT_PROGRESS: &str = "ccode.dev/mcp/init_progress";
}
...
// Re-export the `ccode.dev/mcp/server_status` schema +
// method constant from the dispatcher module so external callers
// have a single import point alongside the other `ccode.dev/mcp/*`
// types.
//
...
pub async fn notify_servers_updated(
    gateway: &ccode_acp::AcpAgentGatewaySender,
    ...
    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
        ...
        tracing::info!("Sent ccode.dev/mcp/servers_updated notification to client");
    }
}

// ── Dispatch ────────────────────────────────────────────────────────

/// Inbound `ccode.dev/mcp/*` methods this agent services, resolved from the wire string.
///
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<ccode_tools::bridge::ToolBridge>,
    event_writer: Option<&ccode_fs::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(ccode_fs::events::Event::McpHealthCheck {
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = ccode_fs::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from ccode-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `ccode-mcp` directly.
...
impl ccode_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<ccode_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(ccode_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<ccode_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(ccode_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(ccode_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(ccode_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    /// The emit-only reverse method (`ccode.dev/mcp/sdk_call`) shares the `ccode.dev/mcp/`
    ...
    fn inbound_sdk_call_has_no_forward_route() {
        ...
        assert_eq!(
            ...
            None,
            "inbound ccode.dev/mcp/sdk_call must not resolve to a forward handler"
        );
    ...
    fn test_mcp_list_response_serialization() {
        let resp = McpListResponse {
            servers: vec![
                McpServerEntry {
                    name: "ccode_com_linear".to_string(),
                    display_name: None,
                    source: McpServerSource::Managed,
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("Ccode CLI".to_string()),
                    },
                    source_label: None,
                    setup: None,
                    setup_values: None,
        ...
        assert_eq!(json["servers"][0]["scopeName"], "Ccode CLI");
    ...
    fn disabled_managed_http_rows_keep_non_gateway_placeholder_config() {
        let entry = disabled_server_placeholder_entry("ccode_com_slack");
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        append_disabled_like_handle_list(
            &mut servers,
            &["ccode_com_slack", "ccode_mcp_linear"],
            true,
        );
        assert!(
            servers.is_empty(),
        ...
        let mut servers = build_mcp_catalog_with_gateway_tools(&[], &[], None, &Default::default());
        append_disabled_like_handle_list(&mut servers, &["ccode_com_slack"], false);
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "ccode_com_slack");
    ...
    fn test_disabled_session_state_serialization() {
        let entry = McpServerEntry {
            name: "ccode_com_slack".to_string(),
            display_name: None,
            source: McpServerSource::Managed,
            source_label: None,
            setup: None,
