...
///
/// Priority order: local ccode bash history > shell history > cross-CWD history.
...
fn scan_cross_cwd_prompts() -> Vec<String> {
    let sessions_dir = crate::util::ccode_home::ccode_home().join("sessions");
    ...
    for (dir, _) in dirs.iter().take(MAX_CROSS_CWD_DIRS) {
        ...
        let cwd = match crate::util::ccode_home::decode_cwd_from_dirname(dir) {
