...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ccode.dev/suggest" => handle_suggest(agent, args).await,
        "ccode.dev/suggestPrompt" => handle_suggest_prompt(agent, args).await,
        _ => Err(acp::Error::method_not_found()),
    }
}

/// Request/response for `ccode.dev/suggestPrompt` — predict the user's likely next
...
struct SuggestPromptRequest {
    ...
    /// override, or `ccode-build-0.1` when its catalog offers it). One tier
    /// of the shell-side resolution in
    /// `prompt_suggest::effective_suggest_model`: env > config.toml > remote
    /// > this hint > `ccode-build-0.1` default, catalog-guarded (a
...
/// forever. Reasoning models (e.g. `ccode-build`) can take ~30s on a cold
