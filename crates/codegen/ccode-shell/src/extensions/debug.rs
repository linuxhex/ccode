//! `ccode.dev/debug/*` extension handlers for local client testing.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ccode.dev/debug/trigger_feedback" => {
            tracing::info!("debug: triggering test feedback request");
            handle_trigger_feedback(agent, args).await
        }
        "ccode.dev/debug/arm_auto_compact" => handle_arm_auto_compact(agent, args),
        "ccode.dev/debug/agent" => handle_agent(agent).await,
