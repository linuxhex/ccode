//! `ccode.dev/marketplace/*` extension handlers.
//!
//! Provides marketplace browsing and install endpoints for the pager modal.
//! Delegates to `ccode-marketplace` crate for scanning and install logic.

use agent_client_protocol as acp;
use ccode_hooks_types::{
...
fn load_filtered_marketplace_sources() -> Vec<ccode_marketplace::MarketplaceSource> {
    crate::plugin::load_filtered_marketplace_sources()
}

pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ccode.dev/marketplace/list" => handle_list().await,
        "ccode.dev/marketplace/action" => handle_action(agent, args).await,
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                ccode_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                ccode_marketplace::SourceKind::Local { path } => {
...
async fn handle_action(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            {
                Ok(outcome) => outcome,
                Err(e) => ccode_hooks_types::ActionOutcome {
                    status: ccode_hooks_types::OutcomeStatus::InternalError,
...
fn refresh_sources(
    sources: &[ccode_marketplace::MarketplaceSource],
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                ...
                ccode_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let ccode_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = ccode_marketplace::git::default_cache_root();
            if let Err(e) = ccode_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
    ...
    ccode_hooks_types::ActionOutcome {
        status: ccode_hooks_types::OutcomeStatus::Success,
        message: msg,
        requires_reload: false,
        requires_restart: false,
    }
...
async fn handle_update(
    ...
    use ccode_marketplace::installer;
    use ccode_hooks_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &ccode_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            ccode_marketplace::SourceKind::Local { path } => path.display().to_string(),
            ccode_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match ccode_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        ccode_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = ccode_marketplace::git::default_cache_root();
            match ccode_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                ccode_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = ccode_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = ccode_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = ccode_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, ccode_hooks_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(ccode_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            ccode_marketplace::SourceKind::Git { url, branch } => {
                ...
                match ccode_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    ccode_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &ccode_marketplace::MarketplaceSource,
    ...
    let scan = ccode_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = ccode_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match ccode_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: ccode_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.ccode/config.toml`.
async fn handle_add_source(url: &str) -> ccode_hooks_types::ActionOutcome {
    ...
    let allowlist =
        &ccode_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            ccode_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
                ...
                return ActionOutcome {
                    ...
                    message: format!(
                        "{e}. Not a reachable git repository — to add it anyway (e.g. a \
                         VPN-gated host), run: ccode plugin marketplace add {url} --force"
                    ),
                    requires_reload: false,
                    requires_restart: false,
                };
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if ccode_marketplace::is_official_source_url(u));
    let name = if is_official {
        ccode_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = ccode_config::ccode_home().join("config.toml");
    let ccode_home = ccode_config::ccode_home();
    let _save_guard = crate::util::config::lock_config_writes().await;
    let write = {
        let name = name.clone();
        tokio::task::spawn_blocking(move || {
            let _flock = acquire_init_lock(&ccode_home).ok();
...
/// Remove a marketplace source from `~/.ccode/config.toml` and uninstall all
/// plugins that were installed from it.
async fn handle_remove_source(source_url_or_path: &str) -> ccode_hooks_types::ActionOutcome {
...
fn remove_source_locked(source_url_or_path: &str) -> ccode_hooks_types::ActionOutcome {
    use crate::plugin;
    use ccode_hooks_types::{ActionOutcome, OutcomeStatus};
    ...
    let config_path = ccode_home.join("config.toml");
    let is_official = ccode_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<ccode_home>/.config-init.lock`,
...
fn acquire_init_lock(ccode_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(ccode_home);
    let lock_path = ccode_home.join(".config-init.lock");
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a ccode_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub fn purge_default_skills_installs(ccode_home: &std::path::Path) {
    purge_default_skills_installs_impl(ccode_home, || {
        ccode_agent::plugins::install_registry::InstallRegistry::try_load_from(
            ccode_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}
...
fn purge_default_skills_installs_impl(
    ...
    load_registry: impl FnOnce() -> Result<
        ccode_agent::plugins::install_registry::InstallRegistry,
        ccode_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = ccode_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(ccode_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %ccode_home.join(".config-init.lock").display(),
...
/// Auto-register the official ccode marketplace source on first run.
///
...
pub fn ensure_official_marketplace_source(ccode_home: &std::path::Path) {
    let config_path = ccode_home.join("config.toml");
    ...
    // (settings.json / known_marketplaces.json) under ccode_home. Scoped to
    // ccode_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = ccode_marketplace::load_sources(&parsed);
    let json_sources = ccode_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&ccode_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, ccode_marketplace::SourceKind::Git { url, .. }
            if ccode_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            ccode_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                ccode_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = ccode_marketplace::OFFICIAL_SOURCE_GIT_URL,
                "auto-registered official ccode marketplace source"
            );
        }
        Ok(()) => {}
        Err(e) => {
...
mod official_source_tests {
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            ccode_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                ccode_marketplace::SourceKind::Git { url, .. }
                    if ccode_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            ccode_marketplace::SourceKind::Git { url, .. }
                if url == ccode_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                ccode_marketplace::OFFICIAL_SOURCE_NAME,
                ccode_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"ccode-official":{{"source":{{"source":"git","url":"{}"}}}}}}"#,
                ccode_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_extra_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"extraKnownMarketplaces":{{"ccode-official":{{"source":{{"source":"git","url":"{}"}}}}}}}}"#,
                ccode_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use ccode_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = ccode_marketplace::MarketplaceEntry {
            name: "demo".into(),
            version: Some("1.0.0".into()),
            description: Some("demo".into()),
            category: Some("dev".into()),
            author: Some("ccode".into()),
            tags: vec!["cli".into()],
            ...
            components: Some(ccode_hooks_types::PluginComponents {
                skills: vec![ccode_hooks_types::ComponentItem::new(
                    "code-review",
