//! `ccode.dev/billing` extension handler.
//!
//! Fetches the authenticated user's Ccode Build billing configuration
...
/// Current billing configuration for Ccode Build coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `CcodeBuildBillingConfig` fields
...
/// against both the new `GetCcodeCreditsConfig` and the legacy
/// `GetCcodeBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetCcodeCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `CcodeCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetCcodeBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/ccode/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BillingConfigResponse {
    ...
    /// User-friendly subscription tier name (e.g. "Premium Heavy").
...
///
/// Keeps history to a count + the most recent period so `~/.ccode/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_ccode_auth(
        &agent.auth_manager,
        "Authentication required to fetch billing data",
        "Billing data requires auth with ccode.dev. Run `ccode login` to authenticate.",
    )?;
    ...
    // forwards to the backend `GetCcodeCreditsConfig`.
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-Ccode-Token-Auth",
            crate::auth::CcodeComConfig::default().token_header,
        )
    ...
    // Every prompt / /usage / poll path hits `ccode.dev/billing`; log the fetched
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_ccode_auth(
        ...
        "Auto top-up data requires auth with ccode.dev. Run `ccode login` to authenticate.",
    )?;
...
mod tests {
    ...
    fn billing_unified_log_ctx_includes_credits_and_collapses_history() {
        ...
        assert_eq!(ctx["subscriptionTier"], "Premium");
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetCcodeCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_CCODE_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
