//! `ccode.dev/memory/flush`, `ccode.dev/memory/rewrite`, and `ccode.dev/compact_conversation`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("ccode.dev/compact_conversation") => handle_compact(agent, args).await,
        "ccode.dev/memory/flush" => handle_flush(agent, args).await,
        "ccode.dev/memory/rewrite" => handle_rewrite(agent, args).await,
