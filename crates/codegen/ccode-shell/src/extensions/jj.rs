//! Jujutsu extension handlers — delegates to [`ccode_workspace::session::jj`].

use agent_client_protocol as acp;

use super::{Empty, ExtResult, to_ext_response, to_ext_response_partial};
use ccode_workspace::session::git::{CommitData, StageData};
use ccode_workspace::session::jj;

/// Handle a `ccode.dev/git/*` method for a jj-colocated repo.
///
...
pub async fn try_handle(
    ...
    match method {
        "ccode.dev/git/status" => Some(to_ext_response(jj::status(git_root).await)),
        "ccode.dev/git/info" => Some(to_ext_response(jj::info(git_root).await)),
        // git HEAD points at `@-` in a colocated repo; route to jj so we report
        // the working-copy commit (`@`), consistent with `status`/`info`.
        "ccode.dev/git/current_commit" => Some(to_ext_response(jj::current_commit(git_root).await)),
        "ccode.dev/git/branches" => Some(to_ext_response(jj::list_bookmarks(git_root).await)),

        // jj has no staging area — stage/unstage are no-ops
        "ccode.dev/git/stage" => Some(to_ext_response(Ok(StageData { paths: Vec::new() }))),
        "ccode.dev/git/stage/content" | "ccode.dev/git/unstage" => Some(to_ext_response(Ok(Empty {}))),

        "ccode.dev/git/discard" => {
        ...
        "ccode.dev/git/checkout" => Some(Err(acp::Error::invalid_params()
            .data("checkout is not supported in jj repos; use `jj new` or `jj edit`"))),
        "ccode.dev/git/stash" => Some(Err(acp::Error::invalid_params()
