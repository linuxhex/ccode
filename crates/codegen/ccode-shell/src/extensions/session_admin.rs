...
//! lifecycle:
//!
//! - `ccode.dev/session/rename`                  rename a session locally + remote
//! - `ccode.dev/session/delete`                  delete a session locally + remote
//! - `ccode.dev/session/update_mcp_servers`      mid-session MCP server swap
//! - `ccode.dev/session/fork`                    fork a session into a new one
//! - `ccode.dev/internal/reload_all_mcp_servers` config hot-reload, all sessions
//! - `ccode.dev/internal/reload_project_mcp_servers` config hot-reload, cwd-scoped
//! - `ccode.dev/internal/reload_skills`          skills file watcher fan-out
//! - `ccode.dev/internal/reload_models`          model list hot-reload from config.toml
//! - `ccode.dev/internal/reload_models_cache`    model catalog hot-reload from disk cache
//! - `ccode.dev/internal/auth_cleared`           auth hot-clear cleanup
//! - `ccode.dev/plugins/reload`                  rebuild shared plugin registry
//! - `ccode.dev/commands/list`                   list slash commands
...
use ccode_telemetry::id::agent_id;

#[tracing::instrument(skip_all, fields(method = %args.method))]
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "ccode.dev/session/rename" => handle_session_rename(agent, args).await,
        "ccode.dev/session/delete" => handle_session_delete(agent, args).await,
        "ccode.dev/session/update_mcp_servers" => handle_update_mcp_servers(agent, args).await,
        "ccode.dev/session/fork" => handle_session_fork(agent, args).await,
        "ccode.dev/internal/reload_all_mcp_servers" => handle_reload_all_mcp_servers(agent).await,
        "ccode.dev/internal/reload_project_mcp_servers" => {
            handle_reload_project_mcp_servers(agent, args).await
        }
        "ccode.dev/internal/reload_skills" => handle_reload_skills(agent),
        "ccode.dev/internal/reload_workflows" => handle_reload_workflows(agent),
        "ccode.dev/internal/reload_models" => handle_reload_models(agent),
        "ccode.dev/internal/reload_models_cache" => handle_reload_models_cache(agent),
        "ccode.dev/internal/auth_cleared" => handle_auth_cleared(agent),
        "ccode.dev/plugins/reload" => handle_plugins_reload(agent).await,
        "ccode.dev/commands/list" => handle_commands_list(agent, args).await,
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session rename requires ccode OAuth credentials"),
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `ccode sessions delete <id>` CLI path.
...
async fn soft_delete_chat_conversation(agent: &MvpAgent, conversation_id: &str) -> ExtResult {
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session delete requires ccode OAuth credentials"),
            other => acp::Error::internal_error()
...
/// hot-reload watcher when `<cwd>/.ccode/config.toml`,
...
/// Hot-reload the model catalog from `~/.ccode/models_cache.json` after an
...
async fn handle_plugins_reload(agent: &MvpAgent) -> ExtResult {
    ...
    // Explicit desktop `ccode.dev/plugins/reload`: force a full local-install re-copy.
    agent
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `ccode.dev/commands/list` (the pull used by ccode-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .ccode/plugins under it, plus
