//! ccode.dev chat-product model catalog (`POST /rest/modes`) — the models
//! ccode-web's chat picker shows, distinct from the CLI `/v1/models` build
...
const CCODE_WEB_URL: &str = "https://ccode.dev";
...
pub enum ChatModelsError {
    #[error("no ccode.dev credentials")]
    NoAuth,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("CCODE_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("CCODE_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("CCODE_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| CCODE_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid ccode.dev bearer — deliberately NOT `is_ccode_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-Ccode-Token-Auth",
                self.auth.ccode_com_config().token_header.clone(),
            )
        ...
        let builder = ccode_fs::trace_context::inject_trace_context_into_request(builder);
