...
use crate::auth::{CcodeAuth, CcodeComConfig};
...
const CCODE_CODE_BACKEND_URL: &str = "https://code.ccode.dev";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const CCODE_CODE_WEB_URL: &str = "https://ccode.dev";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("CCODE_CODE_WEB_URL").unwrap_or_else(|_| CCODE_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-Ccode-Token-Auth", CcodeComConfig::default().token_header)
        .header("x-userid", &auth.user_id)
        .header("x-ccode-client-version", ccode_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::ccode_auth_credentials::CcodeAuthCredentials::new(
    ...
    ccode_fs::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn ccode_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("CCODE_CODE_BACKEND_URL")
                .unwrap_or_else(|_| CCODE_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&CcodeAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn ccode_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<CcodeAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-Ccode-Token-Auth so nginx auth subrequest routes to authenticate_ccode_cli_token.
    /// See: crates/codegen/ccode-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-Ccode-Token-Auth",
            required(&CcodeComConfig::default().token_header, "X-Ccode-Token-Auth")?,
        );
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-ccode-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-ccode-client-version",
            HeaderValue::from_static(ccode_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = ccode_fs::trace_context::inject_trace_context_into_request(
...
/// Fetch `ccode_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-ccode-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(ccode_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-ccode-agent-id", agent_id)
        .header("x-ccode-client-version", ccode_version::VERSION)
        .header(
            "x-ccode-client-identifier",
            crate::http::process_client_identifier(),
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.ccode_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_ccode_api_key_env()
                .or_else(|_| {
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set CCODE_API_KEY.".into(),
                    )
                })?;
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-Ccode-Token-Auth", "ccode-cli")
                .header("x-userid", &auth.user_id)
                .header("x-ccode-client-version", ccode_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| ccode_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(ccode_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-ccode-agent-id"),
                            client_identifier: header_str(&headers, "x-ccode-client-identifier"),
                            client_version: header_str(&headers, "x-ccode-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-ccode-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-ccode-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-ccode-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::CcodeComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("ccode-cli"));
    ...
    fn parse_openai_format_uses_id_field() {
        ...
        let result = parse_remote_model_value(&value, "https://api.ccode.dev/v1").unwrap();
        assert_eq!(result.model, "ccode-3");
        assert_eq!(result.base_url, "https://api.ccode.dev/v1");
        assert_eq!(result.name.as_deref(), Some("ccode-3"));
    }
    #[test]
    ...
    fn inference_url_defaults_to_proxy() {
        let ep = endpoints("https://proxy.ccode.dev/v1", None, None);
        assert_eq!(ep.resolve_inference_base_url(), "https://proxy.ccode.dev/v1");
    }
    #[test]
    fn inference_url_uses_models_base_url() {
        let ep = endpoints(
            "https://proxy.ccode.dev/v1",
    ...
    fn list_url_defaults_to_proxy_models() {
        ...
        assert_eq!(
            ...
            "https://proxy.ccode.dev/v1/models"
        );
    }
    #[test]
    fn list_url_derived_from_base_url() {
        ...
        assert_eq!(ep.resolve_models_list_url(), "https://api.ccode.dev/v1/models");
    }
    #[test]
    ...
    /// ApiKey → `ccode_api_base_url` (ApiKey, public default when unset); a custom
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(session.url, "https://cli-chat-proxy.ccode.dev/v1/models");
        ...
        assert_eq!(deployment.url, "https://cli-chat-proxy.ccode.dev/v1/models");
        assert_eq!(deployment.auth, EndpointAuth::Session);
        let api = ListModelsEndpoint::from_endpoints(&cfg, ModelFetchAuth::ApiKey);
        assert_eq!(api.url, "https://inference.acme-corp.example/ccode/v1/models");
    ...
    /// REGRESSION: `ccode setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("CCODE_DEPLOYMENT_KEY", "ccode-token-ENTERPRISE") };
        ...
        assert_eq!(url, "https://cli-chat-proxy.ccode.dev/v1/deployment/config");
        assert!(
        ...
        unsafe { std::env::remove_var("CCODE_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
