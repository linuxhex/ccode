...
/// Body for `PUT /rest/app-chat/conversations/{id}` (ccode-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("CCODE_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    async fn require_ccode_auth(&self) -> Result<CcodeAuth, ConvError> {
    ...
    pub async fn list_conversations(
        ...
        let auth = self.require_ccode_auth().await?;
