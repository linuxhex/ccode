...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.ccode/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** ccode process
    /// sharing the same `~/.ccode` (the writer may also be this process;
...
/// Watches `~/.ccode/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.ccode/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.ccode/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let ccode_home_buf = ccode_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(ccode_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(ccode_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(ccode_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(ccode_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %ccode_home.display(),
                    error = %e,
                    "failed to watch ccode home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.ccode/config.toml` so the loop above
        // watches `<cwd>/.ccode/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.ccode/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.ccode/config.toml` trigger a
    ...
    /// `<cwd>/.ccode/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.ccode/` does not yet
/// exist at session-open time, the `.ccode/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.ccode/` followed by a
/// write to `<cwd>/.ccode/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let ccode_dir = cwd.join(".ccode");
    if let Err(e) = debouncer
        .watcher()
        .watch(&ccode_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .ccode directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let ccode_dir = cwd.join(".ccode");
    if let Err(e) = debouncer.watcher().unwatch(&ccode_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .ccode directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.ccode/` hasn't
...
fn discovery_change_for_path(path: &Path) -> Option<DiscoveryChange> {
    if path.file_name().is_some_and(|name| name == ".ccode") {
...
/// recursively: `ccode_home` (`~/.ccode`, or `$CCODE_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.ccode` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, ccode_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, ccode_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".ccode", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, ccode_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(ccode_home) {
        return true;
    }
...
impl ProjectDiscoveryWatcher {
    pub fn start(cwd: &Path) -> Option<(Self, mpsc::UnboundedReceiver<DiscoveryChange>)> {
        ...
        let project_ccode = project_root.join(".ccode");
        let workflows = project_ccode.join("workflows");
        let (tx, rx) = mpsc::unbounded_channel();
        let project_ccode_for_events = project_ccode.clone();
        let mut debouncer =
            new_filtered_debouncer(SKILLS_DEBOUNCE, move |res: DebounceEventResult| {
                ...
                for event in events
                    .iter()
                    .filter(|event| event.path.starts_with(&project_ccode_for_events))
                {
        ...
        let initial = if project_ccode.is_dir() {
            project_ccode.clone()
        } else {
            project_root.clone()
        };
        ...
        let refresh_dirs = vec![
            (project_ccode, RecursiveMode::NonRecursive),
            (
                project_root.join(".ccode").join("skills"),
                RecursiveMode::Recursive,
            ),
            (
                project_root.join(".ccode").join("commands"),
...
impl SkillsFileWatcher {
    ///
    /// Uses [`collect_skill_config_dirs`](ccode_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = ccode_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &ccode_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &ccode_home) {
        ...
        let mut refresh_dirs = vec![(ccode_home.join("workflows"), RecursiveMode::NonRecursive)];
        if let Some(cwd) = cwd {
            let project_root = crate::session::workflow::registry::project_root(cwd);
            let project_ccode = project_root.join(".ccode");
            let parent_watch = if project_ccode.is_dir() {
                project_ccode.clone()
            } else {
                project_root
            };
            ...
            refresh_dirs.push((project_ccode.clone(), RecursiveMode::NonRecursive));
            refresh_dirs.push((project_ccode.join("workflows"), RecursiveMode::NonRecursive));
        }
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only ccode_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_ccode_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let ccode_home = home.join(".ccode");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &ccode_home, Some(home));

        // ccode_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&ccode_home));
        ...
        assert!(!g(&home.join("repo").join(".ccode")));
    }

    /// Regression for the ~/.ccode inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.ccode/worktrees/`) must not
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        let ccode = Path::new("/tmp/project/.ccode");
        assert_eq!(
            discovery_change_for_path(ccode),
            Some(DiscoveryChange::Skills),
            "first .ccode creation must take the broader skills reload path"
        );
        assert_eq!(
            discovery_change_for_path(&ccode.join("workflows")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&ccode.join("workflows/review.rhai")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&ccode.join("skills/review/SKILL.md")),
            Some(DiscoveryChange::Skills)
        );
    }
    ...
    /// A write to `<ccode_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another ccode process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.ccode/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let ccode_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_ccode = cwd.path().join(".ccode");
        fs::create_dir_all(&project_ccode).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_ccode.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_ccode.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.ccode/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.ccode/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_ccode = new_cwd.path().join(".ccode");
        fs::create_dir_all(&project_ccode).unwrap();
        fs::write(project_ccode.join("config.toml"), "").unwrap();
