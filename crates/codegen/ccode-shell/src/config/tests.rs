...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "CCODE_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$CCODE_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${CCODE_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$CCODE_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$CCODE_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the CCODE_MEMORY env var.
...
/// Run `f` with CCODE_MEMORY explicitly unset.
fn without_ccode_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("CCODE_MEMORY", None, f)
}
/// Run `f` with CCODE_MEMORY set to a specific value.
fn with_ccode_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("CCODE_MEMORY", Some(value), f)
}
#[test]
fn memory_config_default_disabled() {
    without_ccode_memory(|| {
...
fn memory_config_env_var_zero_does_not_enable() {
    with_ccode_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "CCODE_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_ccode_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "CCODE_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_ccode_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "CCODE_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_ccode_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "CCODE_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_ccode_memory(
        ...
        || {
            ...
            assert!(
                mem.enabled,
                "CLI --experimental-memory should override CCODE_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_ccode_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "--no-memory should override --experimental-memory, CCODE_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_ccode_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(!mem.enabled, "--no-memory should override CCODE_MEMORY=1");
        },
    );
}
#[test]
...
"#;
        ...
        assert_eq!(mem.flush.flush_model.as_deref(), Some("ccode-4"));
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$CCODE_TEST_USER $CCODE_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the CCODE_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with CCODE_SUBAGENTS explicitly unset.
fn without_ccode_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("CCODE_SUBAGENTS", None, f)
}
/// Run `f` with CCODE_SUBAGENTS set to a specific value.
fn with_ccode_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("CCODE_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_ccode_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_ccode_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(!sa.enabled, "CCODE_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_ccode_subagents(
        ...
        || {
            ...
            assert!(
                !sa.enabled,
                "CCODE_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_ccode_subagents(
        ...
        || {
            ...
            assert!(
                sa.enabled,
                "--subagents CLI flag should override CCODE_SUBAGENTS=0"
            );
        },
    );
}
...
fn subagents_config_models_parsed() {
    without_ccode_subagents(|| {
        ...
        assert_eq!(sa.models.get("explore").unwrap(), "ccode-3-fast");
        assert_eq!(sa.models.get("plan").unwrap(), "ccode-4.5");
    });
}
#[test]
    ...
    with_ccode_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(sa.enabled, "CCODE_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "ccode-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        "CCODE_MANAGED_MCPS_ENABLED",
        managed_mcps,
        || with_env_var_opt("CCODE_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "CCODE_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "CCODE_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "CCODE_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("CCODE_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_ccode_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_ccode_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "CCODE_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("CCODE_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_ccode_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_ccode_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_ccode_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_ccode_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "CCODE_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".ccode/prompts/impl.md"
        "#;
    ...
    assert_eq!(researcher.model.as_deref(), Some("ccode-3"));
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".ccode/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".ccode").join("roles");
...
fn personas_parse_from_toml() {
    let toml_str = r#"
            ...
            instructions_file = ".ccode/personas/concise.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    ...
    assert_eq!(
            ...
            Some(".ccode/personas/concise.md")
        );
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".ccode").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".ccode").join("bundled");
    std::fs::create_dir_all(workspace.join(".ccode").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".ccode").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".ccode").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".ccode").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".ccode/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".ccode/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".ccode/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".ccode/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".ccode/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".ccode/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".ccode/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".ccode/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.ccode/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
/// a repo `.ccode/config.toml` structurally cannot supply one.)
#[test]
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "CCODE_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
cli_chat_proxy_base_url = "https://cli-chat-proxy.ccode.dev/v1"

[model.ccode-build]
base_url = "https://inference.acme-corp.example/ccode/v1"
...
[endpoints]
deployment_key = "ccode-token-ENTERPRISE"
ccode_api_base_url = "https://inference.acme-corp.example/ccode/v1"
...
"#,
    ...
    assert_eq!(
            ...
            "https://cli-chat-proxy.ccode.dev/v1/deployment/config"
        );
    assert!(
            !cfg.endpoints
...
/// RCE guard: a project `.ccode/config.toml` must never source
...
fn project_config_never_sources_feedback_user() {
    ...
    let ccode = repo.path().join(".ccode");
...
fn apply_requirements_value_overrides_user_settings() {
    ...
    assert_eq!(
            ...
            cfg.endpoints.ccode_api_base_url
        );
    assert_eq!(
...
#[test]
fn validate_hooks_path_rejects_outside_ccode_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
            msg.contains("must be under ~/.ccode/"),
            "should mention ~/.ccode/ restriction, got: {msg}"
        );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let ccode_home = crate::util::ccode_home::ccode_home();
    let traversal = format!("{}/../evil", ccode_home.display());
...
#[test]
fn validate_hooks_path_accepts_ccode_hooks_subdir() {
    let ccode_home = crate::util::ccode_home::ccode_home();
    let valid_path = ccode_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.ccode/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use ccode_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own ccode yolo.
#[test]
...
fn simulate_release_build() -> ccode_test_support::EnvGuard {
    ccode_test_support::EnvGuard::set(ccode_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".ccode"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
#[test]
fn explicit_ccode_root_is_the_only_user_source() {
    let tmp = tempfile::tempdir().unwrap();
    let ambient = tmp.path().join("ambient-home/.ccode");
    let configured = tmp.path().join("configured-ccode-home");
...
/// verdicts untouched). CCODE_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores CCODE_HOME even on panic). No user-global
/// `$CCODE_HOME/config.toml` is seeded: `ccode_home()` is `OnceLock`-cached,
...
/// in `ccode-agent`. An ABSOLUTE plugin path is used so the merged
...
/// CCODE_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use ccode_agent::plugins::{TrustStore, discover_plugins};
    use ccode_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("CCODE_HOME", home.path());
    let _flag = EnvGuard::unset("CCODE_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. CCODE_HOME-isolated (empty
/// store); CCODE_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
fn kill_switched_cold_cwd_stays_allowed_through_plugins_config_read() {
    ...
    let ccode = repo.path().join(".ccode");
    std::fs::create_dir_all(&ccode).unwrap();
    std::fs::write(ccode.join("config.toml"), "[plugins]\npaths = [\"./proj-plugin\"]\n")
        .unwrap();
    let cwd = repo.path();
    let remote = crate::util::config::RemoteSettings {
