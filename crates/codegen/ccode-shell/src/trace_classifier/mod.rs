...
/// two-function shape in `ccode-tools` (F29).
...
/// `ccode-tools` — existing items keep their prior content when
...
/// binary. Wraps a `ccode_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: ccode_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: ccode_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_ccode_conv_id: Some(format!("trace-classifier-{}", uuid::Uuid::new_v4())),
        x_ccode_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
        x_ccode_session_id: session_id_str,
...
pub struct RunArgs {
    ...
    /// Ccode-home directory containing the `auth.json` to consult as
    /// the third API-key fallback. Defaults to
    /// `ccode_shell::util::ccode_home::ccode_home()` when `None`.
    /// Exposed as a CLI flag for testability.
    pub ccode_home: Option<PathBuf>,
}
...
/// Resolve the API key from `--api-key` → `$CCODE_API_KEY` →
...
/// provider command (if `CCODE_AUTH_PROVIDER_COMMAND` is set) is
...
/// the user can act on.
pub async fn resolve_api_key(explicit: Option<&str>, ccode_home: &Path) -> Result<String> {
    if let Some(k) = explicit.map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    let env_key = std::env::var("CCODE_API_KEY").ok();
    if let Some(k) = env_key.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    if let Some(key) = non_interactive_auth_key(ccode_home).await? {
        return Ok(key);
    }
    Err(anyhow!(
        "no API key: pass --api-key, set CCODE_API_KEY, or run `ccode login` to populate \
         <ccode-home>/auth.json. An expired OIDC token is auto-refreshed when a refresh_token \
         is present; if not, re-login is required."
    ))
}

/// Build an `AuthManager` for `ccode_home`, install the
...
/// explicit `ccode_home` so `--ccode-home` overrides work in test
/// fixtures (the upstream helper hardcodes the global `ccode_home()`
/// path).
///
/// Returns:
...
async fn non_interactive_auth_key(ccode_home: &Path) -> Result<Option<String>> {
    use crate::auth::{AuthError, AuthManager, CcodeComConfig};
    ...
    let config = CcodeComConfig::default();
    let auth_provider_command = config.auth_provider_command.clone();
    let manager = std::sync::Arc::new(AuthManager::new(ccode_home, config));
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `ccode login` to re-authenticate, \
             or pass --api-key / set $CCODE_API_KEY to bypass auth.json."
        )),
    }
}
...
async fn build_sampler_client(
    ...
    model: String,
    api_key: Option<&str>,
    ccode_home: Option<&Path>,
) -> Result<ccode_sampler::SamplingClient> {
    let default_home;
    let ccode_home_path: &Path = match ccode_home {
        Some(p) => p,
        None => {
            default_home = crate::util::ccode_home::ccode_home();
            default_home.as_path()
        }
    };
    let resolved = resolve_api_key(api_key, ccode_home_path).await?;
    let config = ccode_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..ccode_sampler::SamplerConfig::default()
    };
    ccode_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use ccode_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        let conv_id = req.x_ccode_conv_id.as_deref().expect("conv id");
        ...
        assert_eq!(req.x_ccode_session_id.as_deref(), Some("sess-x"));
        let req_id = req.x_ccode_req_id.as_deref().expect("req id");
        ...
        assert_eq!(
            req.x_ccode_agent_id.as_deref(),
            Some(ccode_telemetry::id::agent_id().as_str())
        );
    }
    ...
    /// OIDC scope (the same scope `ccode login` writes today and
    /// `AuthManager` reads). `auth_mode: api_key` skips the refresh
    /// path entirely — useful for "plain key, no refresh wanted"
    /// fixtures.
    fn write_auth_json(ccode_home: &Path, key: &str) {
        let scope = crate::auth::CcodeComConfig::default().auth_scope();
        ...
        std::fs::write(ccode_home.join("auth.json"), body.to_string()).expect("write auth.json");
    }
    ...
    fn write_empty_auth_json(ccode_home: &Path) {
        std::fs::write(ccode_home.join("auth.json"), "{}").expect("write auth.json");
    }
    ...
    fn write_fresh_oidc_auth_json(ccode_home: &Path, key: &str) {
        let scope = crate::auth::CcodeComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    fn write_expired_oidc_auth_json_no_refresh(ccode_home: &Path, key: &str) {
        let scope = crate::auth::CcodeComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    async fn resolve_api_key_precedence() {
        ...
        let ccode_home = tmp.path();
        // No auth.json at all → AuthManager sees nothing on disk.
        write_empty_auth_json(ccode_home);
        ...
        assert!(
            resolve_api_key(None, ccode_home).await.is_err(),
            "no flag, no env, no auth.json → error",
        );
        assert!(
            resolve_api_key(Some(""), ccode_home).await.is_err(),
            "empty flag → error",
        );
        assert!(
            resolve_api_key(Some("   "), ccode_home).await.is_err(),
        ...
        assert_eq!(
            resolve_api_key(Some("from-flag"), ccode_home)
                .await
                .expect("ok"),
            "from-flag",
        );

        unsafe { std::env::set_var("CCODE_API_KEY", "from-env") };
        assert_eq!(
            resolve_api_key(None, ccode_home).await.expect("ok"),
            "from-env"
        );
        assert_eq!(
            resolve_api_key(Some("from-flag"), ccode_home)
        ...
        assert_eq!(
            resolve_api_key(Some(""), ccode_home).await.expect("ok"),
            "from-env",
        ...
        unsafe { std::env::set_var("CCODE_API_KEY", "   ") };
        assert!(
            resolve_api_key(None, ccode_home).await.is_err(),
    ...
    async fn resolve_api_key_falls_back_to_auth_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let ccode_home = tmp.path();
        write_auth_json(ccode_home, "from-auth-json");
        // `with_no_env` returns sync; we re-implement the clearing
        // inline so we can `.await` the resolver.
        let key =
            with_env_isolated(async { resolve_api_key(None, ccode_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_skips_auth_json_when_env_provided() {
        ...
        let key = with_env_isolated(async {
            unsafe { std::env::set_var("CCODE_API_KEY", "from-env") };
            let resolved = resolve_api_key(None, ccode_home).await.expect("ok");
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        let msg = with_env_isolated(async {
            let err = resolve_api_key(None, ccode_home)
                .await
        ...
        assert!(
            msg.contains("--api-key")
                && msg.contains("CCODE_API_KEY")
                && msg.contains("ccode login")
    ...
    async fn resolve_api_key_skips_empty_auth_json_entry() {
        ...
        // the unified error.
        write_auth_json(ccode_home, "");
        let err1 = with_env_isolated(async { resolve_api_key(None, ccode_home).await }).await;
        assert!(err1.is_err(), "empty auth.json entry is treated as absent");

        write_auth_json(ccode_home, "   ");
        let err2 = with_env_isolated(async { resolve_api_key(None, ccode_home).await }).await;
        assert!(
            err2.is_err(),
    ...
    async fn resolve_api_key_returns_refreshable_oidc_token() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let ccode_home = tmp.path();
        write_fresh_oidc_auth_json(ccode_home, "fresh-oidc-key");
        let key =
            with_env_isolated(async { resolve_api_key(None, ccode_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_returns_error_when_oidc_token_expired_and_no_refresh_token() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let ccode_home = tmp.path();
        write_expired_oidc_auth_json_no_refresh(ccode_home, "stale-oidc-key");
        let err = with_env_isolated(async { resolve_api_key(None, ccode_home).await }).await;
        assert!(
            err.is_err(),
    ...
    /// * `CCODE_API_KEY` — would return early from `resolve_api_key`.
    /// * `CCODE_AUTH` — inline-JSON credentials override that bypasses
    ///   the on-disk read entirely (`AuthManager::new`).
    /// * `CCODE_AUTH_PATH` — overrides the auth.json path; if set to
    ///   the operator's real `~/.ccode/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `CCODE_AUTH_PROVIDER_COMMAND` — selects an external
    ...
    async fn run_rejects_missing_trace_path() {
        let prev = std::env::var("CCODE_API_KEY").ok();
        unsafe { std::env::remove_var("CCODE_API_KEY") };
        let tmp = tempfile::tempdir().expect("tempdir");
        let args = RunArgs {
            ...
            min_confidence: None,
            include_reasoning: None,
            ccode_home: Some(tmp.path().to_path_buf()),
        };
        ...
        match prev {
            Some(v) => unsafe { std::env::set_var("CCODE_API_KEY", v) },
            None => unsafe { std::env::remove_var("CCODE_API_KEY") },
        }
    }
