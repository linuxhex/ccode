//! `ccode inspect` — configuration introspection.
//!
//! Shows everything Ccode discovers in the current directory: project
...
use ccode_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[ccode_com_config]`
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `CCODE_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .ccode/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `ccode inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = ccode_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = ccode_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = ccode_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        ccode_version: ccode_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == ccode_home.join("rules"))
        || has_rules_directory(file_path, ".ccode")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = ccode_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        ccode_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `ccode_agent::prompt::agents_md::read_agents_config_with_paths`
    ...
    configs
        ...
        .map(|c| {
            let file_type =
                instruction_file_type(&c.file_path, &ccode_home, imported, &extra_rule_prefixes);
            let scope = instruction_scope(&c.file_path, &ccode_home, &vendor_homes, &workspace_root);
...
/// which handles both Ccode TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path, project_trusted: bool) -> PermissionsReport {
    use ccode_workspace::permission::resolution;
...
/// (`[ccode_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let ccode_com_config = config
        .map(|c| c.ccode_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: ccode_com_config.api_key_auth_disabled(),
        disable_api_key_auth: ccode_com_config.disable_api_key_auth,
        force_login_team_uuid: ccode_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[ccode_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = ccode_tools::types::compat::CompatConfig::default();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&ccode_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &ccode_agent::plugins::PluginRegistry,
    skills_config: &ccode_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = ccode_agent::prompt::skills::list_skills_with_plugins(
...
fn skill_entry_source(s: &ccode_agent::prompt::skills::SkillInfo) -> ConfigSource {
    use ccode_tools::implementations::skills::types::SkillScope;
...
fn list_agents(
    ...
    let agents = ccode_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[ccode_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                ccode_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                ccode_agent::plugins::PluginScope::Project => Scope::Project,
                ccode_agent::plugins::PluginScope::User => Scope::User,
                ccode_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: ccode_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    ccode_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = ccode_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `ccode mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.ccode/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_ccode_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.ccode_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `ccode mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use ccode_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use ccode_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for (vendor, path) in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.ccode"), false, &[]);
        ...
        for path in ["/repo/.ccode/rules/team.md", r"C:\repo\.ccode\rules\team.md"] {
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.ccode"), false, &[]),
                "rules"
            );
        }
        for path in [
            ...
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.ccode"), true, &[]),
                "rules"
            );
        }
        for path in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.ccode"), true, &[]);
    ...
    #[test]
    fn ccode_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let ccode_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, ccode_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.ccode/rules/project.md",
    ...
    fn vendor_home_nested_in_workspace_keeps_direct_surfaces_global() {
        ...
        for path in ["/repo/.claude/rules/global.md", "/repo/.claude/CLAUDE.md"] {
            assert!(matches!(
                instruction_scope(path, Path::new("/other/ccode"), &vendor_homes, workspace),
                Scope::Global
            ));
        }
        for path in [
    ...
    #[test]
    fn workspace_scope_wins_inside_ccode_home() {
        let ccode_home = Path::new("/custom/ccode");
        let workspace = Path::new("/custom/ccode/worktrees/repo");
        for path in [
            "/custom/ccode/worktrees/repo/.cursor/rules/project.md",
            "/custom/ccode/worktrees/repo/src/AGENTS.md",
        ] {
            assert!(matches!(
                instruction_scope(path, ccode_home, &[], workspace),
                Scope::Project
        ...
        assert!(matches!(
            instruction_scope("/custom/ccode/rules/global.md", ccode_home, &[], workspace,),
            Scope::Global
        ));
    }

    #[test]
    fn custom_ccode_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.ccode/requirements.toml";
    ...
    fn config_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("ccode-4.5"));

        let human = render_config_warnings(&warnings);
        assert!(human.contains("Config Warnings"), "{human}");
        assert!(
            human.contains("[model.\"ccode-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"ccode-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["key"], "ccode-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let s = skill_fixture("a", "/repo/.ccode/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.ccode/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.ccode/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(skill_entry_source(&s), ConfigSource::User { .. }));

        let s = skill_fixture(
            "d",
            "/home/u/.ccode/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s),
        ...
        let s = skill_fixture("e", "/home/u/.ccode/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.ccode dirs.
        ...
        let registry = ccode_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
