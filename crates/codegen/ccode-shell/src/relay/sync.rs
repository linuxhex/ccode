...
/// Format: https://ccode.dev/build/{sessionId}
pub fn build_share_url(session_id: &str) -> String {
    let base_url =
        std::env::var("CCODE_CODE_WEB_URL").unwrap_or_else(|_| "https://ccode.dev".to_string());
...
fn handle_relay_message(
    ...
    match method {
        Some("initialize") => {
            ...
            if let Some(request_id) = id {
                ...
                *initialized = true;
                ...
                let upsert = json!({
                    "jsonrpc": "2.0",
                    "method": "_ccode.dev/session/upsert",
                    "params": {
                        "sessionId": session_id,
                        "cwd": cwd,
                    }
        ...
        Some("_ccode.dev/relay/initialized") => {
...
mod tests {
    ...
    fn test_build_share_url_default() {
        ...
        assert_eq!(url, "https://ccode.dev/build/test-session-123");
    }
    ...
    fn test_build_share_url_with_uuid() {
        ...
        assert_eq!(
            url,
            "https://ccode.dev/build/01937d8a-1234-7abc-9def-0123456789ab"
        );
    }
