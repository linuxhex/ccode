...
pub use self::types::*;
pub use ccode_sampler::ApiBackend;
pub use ccode_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// ccode-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `ccode-sampler` crate. We re-export the public surface here so
...
// `ccode_sampler::*`. The shell-side `sampling::client::Config`
