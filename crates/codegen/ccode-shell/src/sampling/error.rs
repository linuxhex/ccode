...
//!
//! The canonical error types now live in `ccode_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a ccode-shell dependency).

// Re-export everything from the standalone crate.
pub use ccode_sampling_types::error::*;
...
/// API key / team rate-limit copy. Personal ccode.dev upgrades do not raise API
/// team limits; admins purchase credits or a higher spend-based tier.
/// See https://docs.ccode.dev/developers/rate-limits#rate-limit-tiers
pub const RATE_LIMITED_USER_MESSAGE_API_KEY: &str = "You\u{2019}ve hit your team\u{2019}s API rate limit. Ask a team admin to purchase more credits for higher limits, or try again later. See https://docs.ccode.dev/developers/rate-limits#rate-limit-tiers";
...
pub const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Ccode Build usage limit for now. Get Premium for much higher limits, or try again later: https://ccode.dev/premium?referrer=ccode-build";
...
/// API-key + personal Premium upsell → team credits copy. Else the body
...
/// IC sometimes reuses OAuth free-tier upsell copy on 429s ("upgrade to a Ccode
/// subscription" / ccode.dev/premium). That is wrong for API-key / team auth:
...
fn pushes_consumer_subscription_upsell(detail: &str) -> bool {
    ...
    d.contains("ccode.dev/premium") || d.contains("upgrade to a ccode subscription")
}

/// Map a `SamplingError` to an ACP `Error` for client-facing responses.
/// This stays in ccode-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a Ccode subscription")
                    && crate::agent::auth_method::has_ccode_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (CCODE_API_KEY). \
                         Your cached OAuth session is being used instead. \
                         To use your API key, run `ccode logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                ccode_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: ccode_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != ccode_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == ccode_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = ccode_chat::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &ccode_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn rate_limited_fallback_oauth_vs_api_key() {
        ...
        assert!(
            RATE_LIMITED_USER_MESSAGE_API_KEY
                .contains("https://docs.ccode.dev/developers/rate-limits#rate-limit-tiers")
        );
    ...
    fn format_rate_limited_surfaces_nonempty_server_detail() {
        ...
        // Team console rate-limit copy has no personal Premium upsell — surface as-is.
        let team = "resource-exhausted: Too many requests for team abc. See https://console.ccode.dev/team/default/rate-limits.";
    ...
    fn format_rate_limited_api_key_rewrites_consumer_subscription_upsell() {
        let body = "Some resource has been exhausted: You are sending requests too quickly. \
             Please slow down, or upgrade to a Ccode subscription for higher limits: \
             https://ccode.dev/premium";
        let wire = format!("API error (status 429 Too Many Requests): {body}");
        // OAuth keeps the IC body (personal plan upgrade is correct).
        assert_eq!(format_rate_limited_user_message(Some(&wire), false), body);
        // API key must not push ccode.dev Premium — team credits / rate-limit tiers.
        assert_eq!(
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        unsafe {
            ...
            std::env::remove_var("CCODE_CODE_CCODE_API_KEY");
            if let Some(v) = prev {
                std::env::set_var("CCODE_API_KEY", v);
            }
            if let Some(v) = prev_legacy {
                std::env::set_var("CCODE_CODE_CCODE_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("ccode-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
                message: "The model 'ccode-build' requires a Ccode subscription.".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry: None,
            };
            ...
            assert!(
                msg.contains("ccode logout"),
                "should suggest ccode logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
