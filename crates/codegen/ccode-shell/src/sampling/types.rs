// Re-export all types from the standalone ccode-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use ccode_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `ccode-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `ccode-sampler` crate can reference them without a circular
// dep on `ccode-shell`.

// Tests for the types now live in ccode-sampling-types crate.

use ccode_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `ccode-sampling-types` or
/// `ccode-tools`) so neither of those crates needs to depend on
