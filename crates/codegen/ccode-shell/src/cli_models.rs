//! Data APIs for `ccode models`. Clients own display.
...
/// Status for the `ccode models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `ccode_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    /// [`crate::agent::auth_method::should_advertise_ccode_api_key`] so
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.ccode_com_config.ccode_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_ccode_api_key(
            agent_config.ccode_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    ///
    /// Uses `CCODE_AUTH_PATH` (not `CCODE_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            EnvGuard::unset(CCODE_API_KEY_ENV_VAR),
            EnvGuard::unset(LEGACY_CCODE_API_KEY_ENV_VAR),
            EnvGuard::unset("CCODE_AUTH"),
            EnvGuard::set("CCODE_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("CCODE_DEPLOYMENT_KEY"),
            EnvGuard::unset("CCODE_WS_ORIGIN"),
            EnvGuard::unset("CCODE_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_api_key_env() {
        ...
        let _key = EnvGuard::set(CCODE_API_KEY_ENV_VAR, "ccode-test-key");
    ...
    fn resolve_legacy_api_key_env() {
        let (_dir, _g) = isolate_auth_sources();
        let _key = EnvGuard::set(LEGACY_CCODE_API_KEY_ENV_VAR, "legacy-key");
        assert_eq!(AuthStatus::resolve(&Config::default()), AuthStatus::ApiKey);
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("CCODE_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("ccode.dev".to_owned())
        );
    }
