...
mod imp {
    ...
    pub fn install(crash_dir: &Path, ccode_version: &str) -> bool {
        ...
        unsafe {
            ...
            version.fill(0);
            let copy_len = ccode_version.len().min(format::VERSION_STRING_LEN);
            version[..copy_len].copy_from_slice(&ccode_version.as_bytes()[..copy_len]);
        }
...
mod tests {
    ...
    fn full_install_replaces_minimal_handler() {
        ...
        let dir = std::env::temp_dir().join("ccode-crash-test-replace");
