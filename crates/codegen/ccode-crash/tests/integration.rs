//! Integration tests for ccode-crash.
//!
...
fn subprocess_entry() {
    ...
    let config = ccode_crash::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    ccode_crash::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob = ccode_crash::format::CrashBlob::parse(&data).expect("crash blob should parse");
    ...
    let report =
        ccode_crash::check_previous_crash(tmp.path()).expect("should produce a crash report");
...
fn clean_exit_does_not_produce_crash_report() {
    ...
    let report = ccode_crash::check_previous_crash(tmp.path());
