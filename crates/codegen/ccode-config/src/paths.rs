//! Filesystem locations for ccode config files and binaries.
...
/// The default user ccode directory (`~/.ccode`, canonicalized) used when
/// `CCODE_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`ccode_home()`] is the default without duplicating the computation.
///
...
/// `~/.ccode/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `ccode_worktree::db::resolve_ccode_home` (deliberately standalone crate).
pub fn default_ccode_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".ccode")
}

/// Per-user config directory: `$CCODE_HOME` or `~/.ccode`. Created if needed.
pub fn ccode_home() -> PathBuf {
    CCODE_HOME
        .get_or_init(|| {
            let ccode_home = if let Ok(v) = std::env::var("CCODE_HOME") {
                PathBuf::from(v)
            } else {
                default_ccode_home()
            };
...
/// The user-global ccode home, but only when one genuinely resolves: `Some` when
/// `$CCODE_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`ccode_home()`], this never falls back to a cwd-relative `.ccode`, so callers
/// that *scan* user-global ccode resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.ccode` tree for the user-global one when no home resolves.
pub fn user_ccode_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("CCODE_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(ccode_home)
}

/// Canonical ccode application path: `$CCODE_HOME/bin/ccode` (Unix) or `ccode.exe` (Windows).
pub fn ccode_application() -> PathBuf {
    ccode_application_in(&ccode_home())
}

/// [`ccode_application`] under an explicit home instead of `$CCODE_HOME`.
pub fn ccode_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "ccode.exe" } else { "ccode" };
    home.join("bin").join(name)
}

/// System-wide config directory: `/etc/ccode/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/ccode"))
    } else {
        None
    }
}
...
/// `ccode_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    ccode_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = ccode_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_ccode_home_has_no_verbatim_prefix() {
        ...
        let home = default_ccode_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".ccode"));
    }
