...
impl ManagedConfig {
    pub fn plan(request: ManagedConfigRequest) -> Result<ManagedConfigPlan, ManagedConfigError> {
        ...
        let backup_path_hint = (changes && original.bytes.is_some())
            .then(|| transaction::artifact_hint(&target_path, "ccode-backup"));
        let temp_path_hint = changes.then(|| transaction::artifact_hint(&target_path, "ccode-tmp"));
        let lock_path = transaction::sibling_artifact(&target_path, "ccode.lock");
