//! Ccode-owned direct global hook paths shared by shell discovery and sandbox
//! write-deny: `$CCODE_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$CCODE_HOME/hooks/` (discovered + protected).
    HookDirectory,
    /// `$CCODE_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked CCODE_HOME is not allowed under sandbox write-deny: {path}")]
    SymlinkedCcodeHome { path: PathBuf },
    ...
    #[error("Ccode hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("Ccode hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create Ccode hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create Ccode hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$CCODE_HOME/hooks` dir + `hooks-paths` file (create if missing).
/// Race-resistant create (`create_dir` / `create_new`+`O_NOFOLLOW`); never
/// truncates an existing registry; rejects symlinks/wrong types.
pub fn ensure_ccode_hook_slots(ccode_home: &Path) -> Result<(), GlobalHookSourceError> {
    if path_has_symlink_component(ccode_home) {
        return Err(GlobalHookSourceError::SymlinkedCcodeHome {
            path: ccode_home.to_path_buf(),
        });
    }

    match std::fs::create_dir(ccode_home) {
        Ok(()) => {}
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {}
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(ccode_home).map_err(|source| {
                GlobalHookSourceError::CreateHooksDir {
                    path: ccode_home.to_path_buf(),
                    source,
                }
    ...
    let ccode_meta = std::fs::symlink_metadata(ccode_home).map_err(|source| {
        GlobalHookSourceError::CreateHooksDir {
            path: ccode_home.to_path_buf(),
            source,
        }
    })?;
    if ccode_meta.file_type().is_symlink() || !ccode_meta.file_type().is_dir() {
        return Err(GlobalHookSourceError::SymlinkedCcodeHome {
            path: ccode_home.to_path_buf(),
        });
    }

    let hooks = ccode_home.join("hooks");
    ...
    let registry = ccode_home.join("hooks-paths");
...
/// Resolve Ccode-owned direct global hook sources (`reject_symlinks` for sandbox).
pub fn resolve_global_hook_sources(
    ...
    if let Some(ccode) = ccode_home {
        if reject_symlinks && path_has_symlink_component(ccode) {
            return Err(GlobalHookSourceError::SymlinkedCcodeHome {
                path: ccode.to_path_buf(),
            });
        }

        let hooks = ccode.join("hooks");
        let hooks_paths = ccode.join("hooks-paths");
        if reject_symlinks {
