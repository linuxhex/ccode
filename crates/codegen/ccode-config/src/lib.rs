//! Config file loading for Ccode.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/ccode/managed_config.toml`
//! 2. `$CCODE_HOME/managed_config.toml`
//! 3. `$CCODE_HOME/config.toml`
//! 4. `$CCODE_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/ccode/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.ccode`, admin-forced) — macOS only
//!
...
pub use global_hook_sources::{
    ...
    ensure_ccode_hook_slots, existing_ancestor_chain, is_direct_hook_json_name,
...
pub use paths::{
    ...
    default_ccode_home, encode_cwd_dirname, ensure_sessions_cwd_dir, ccode_application,
    ccode_application_in, ccode_home, sessions_cwd_dir, system_config_dir, user_ccode_home,
};
pub use validation::{
