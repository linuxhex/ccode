...
//! C++/C#/.NET builds.
//!
//! Set `CCODE_SHELL` to override auto-detection: `pwsh`, `powershell`,
...
///
/// If `CCODE_SHELL` is set, it takes precedence over auto-detection.
...
pub fn detect_windows_shell() -> &'static WindowsShell {
    ...
    CACHED.get_or_init(|| {
        // Explicit override via CCODE_SHELL.
        if let Ok(val) = std::env::var("CCODE_SHELL") {
            match val.trim().to_ascii_lowercase().as_str() {
                "pwsh" => {
                    tracing::info!("Windows shell (CCODE_SHELL override): pwsh");
                    return WindowsShell::Pwsh;
                }
                "powershell" => {
                    tracing::info!("Windows shell (CCODE_SHELL override): powershell.exe");
                ...
                "bash" | "gitbash" | "git-bash" => {
                    if let Some(path) = find_git_bash() {
                        tracing::info!(
                            shell = path,
                            "Windows shell (CCODE_SHELL override): Git Bash"
                        );
                        return WindowsShell::GitBash(path);
                    }
                    tracing::warn!(
                        "CCODE_SHELL={val} but Git Bash not found; falling through to auto-detect"
                    );
                }
                "cmd" | "cmd.exe" => {
                    tracing::info!("Windows shell (CCODE_SHELL override): cmd.exe");
                    return WindowsShell::Cmd;
                }
                other => {
                    tracing::warn!(
                        "CCODE_SHELL={other} is not recognized \
...
//
//   1. `$CCODE_SHELL` override, if it names the requested kind and is runnable.
...
fn resolve_unix_shell_path(kind: UnixShellKind) -> String {
    ...
    // 1) Explicit override via $CCODE_SHELL.
    if let Ok(s) = std::env::var("CCODE_SHELL") {
...
///
/// The probe is spawned via `ccode_tty::detach_std_command` so that
...
fn is_executable(path: &std::path::Path) -> bool {
    ...
    // Nix fallback. Detach from the controlling TTY via ccode_tty so
...
mod tests {
    ...
    fn is_command_available_detects_present_and_absent() {
        ...
        assert!(!is_command_available(
            "ccode-definitely-not-a-real-command-xyz"
        ));
    }
