...
pub trait AuthCredentialProvider: HttpAuth + Send + Sync + 'static {
    ...
    /// (`ccode-desktop`, `ccode login`). The `token` field MUST mirror
    ...
    /// Whether `X-Ccode-Token-Auth` should be sent with the bearer token.
    /// `false` for deployment keys (bare Bearer), `true` for user/OAuth tokens.
    /// See `CcodeAuthCredentials::apply()` for the wire format contract.
