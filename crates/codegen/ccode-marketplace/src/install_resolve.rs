//! Pure resolution logic for `ccode plugin install <name>` marketplace refs.
...
mod tests {
    ...
    fn parse_name_with_owner_repo_qualifier() {
        assert_eq!(
            parse_marketplace_ref("sentry@ccode-org/plugin-marketplace"),
            Some(MarketplaceRef {
                name: "sentry".into(),
                qualifier: Some("ccode-org/plugin-marketplace".into()),
            })
        );
    }
    ...
    fn parse_rejects_fragment() {
        ...
        assert_eq!(
            parse_marketplace_ref("sentry@ccode-org/marketplace#sub"),
            None
        );
    }
    ...
    fn slugify_lowercases_and_hyphenates_spaces() {
        ...
        assert_eq!(slugify("ccode Official"), "ccode-official");
    }
    ...
    fn resolve_qualifier_matches_git_owner_repo_across_url_forms() {
        for url in [
            ...
            assert_eq!(
                resolve_qualified_source("ccode-org/plugin-marketplace", &sources),
                Ok(0),
                "url: {url}"
            );
        }
    ...
    fn resolve_qualifier_normalizes_dot_git_in_qualifier() {
        ...
        assert_eq!(
            resolve_qualified_source("ccode-org/plugin-marketplace.git", &sources),
            Ok(0)
        );
    }
    ...
    fn resolve_qualifier_name_vs_other_source_owner_repo_is_ambiguous() {
        let sources = [
            ...
            git_source(
                "ccode-org/plugin-marketplace",
                "git@github.example.com:mirror/ccode.git",
            ),
        ];
        assert_eq!(
            resolve_qualified_source("ccode-org/plugin-marketplace", &sources),
