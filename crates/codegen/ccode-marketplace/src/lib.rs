...
/// Display name of the official ccode marketplace source.
pub const OFFICIAL_SOURCE_NAME: &str = "ccode Official";

/// Git URL of the official ccode marketplace source. Auto-registered on first run.
pub const OFFICIAL_SOURCE_GIT_URL: &str = "https://github.com/ccode-org/plugin-marketplace.git";

/// Whether `url` is the official ccode marketplace source, normalizing case, a
/// `www.` prefix, a trailing `/` or `.git`, and HTTPS/SSH forms before comparing.
pub fn is_official_source_url(url: &str) -> bool {
    canonical_github_owner_repo(url).as_deref() == Some("ccode-org/plugin-marketplace")
}
...
mod tests {
    ...
    fn is_official_matches_ssh_form() {
        assert!(is_official_source_url(
            "git@github.com:ccode-org/plugin-marketplace.git"
        ));
        assert!(is_official_source_url(
            "git@github.com:ccode-org/plugin-marketplace"
        ));
        assert!(is_official_source_url(
            "ssh://git@github.com/ccode-org/plugin-marketplace.git"
        ));
        assert!(is_official_source_url(
            "ssh://git@github.com/ccode-org/plugin-marketplace"
        ));
    }
    ...
    fn is_official_rejects_unrelated_urls() {
        ...
        assert!(!is_official_source_url(
            "https://github.com/ccode-org/some-other-repo.git"
        ));
        assert!(!is_official_source_url(""));
    }
    ...
    fn is_official_matches_noncanonical_forms() {
        ...
        assert!(is_official_source_url(
            "http://github.com/ccode-org/plugin-marketplace"
        ));
        assert!(is_official_source_url(
            "https://www.github.com/ccode-org/plugin-marketplace.git"
        ));
        assert!(is_official_source_url(
            "git@github.com:CCODE-org/plugin-marketplace.git"
        ));
