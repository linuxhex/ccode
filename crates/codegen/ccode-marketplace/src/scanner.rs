...
//!    the lookup order — `.ccode-plugin/marketplace.json` is preferred), use it.
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = ccode_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(ccode_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn url_sourced_entry_carries_keywords() {
        let dir = tempfile::tempdir().unwrap();
        let ccode_dir = dir.path().join(".ccode-plugin");
        std::fs::create_dir_all(&ccode_dir).unwrap();
        std::fs::write(
            ccode_dir.join("marketplace.json"),
            r#"{
                "name": "kw-marketplace",
                "plugins": [{
                    "name": "remote-plugin",
    ...
    #[test]
    fn ccode_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        make_plugin(dir.path(), "ccode-plugin", "1.0.0");
        ...
        std::fs::write(
            ccode_dir.join("marketplace.json"),
            r#"{
                "name": "ccode-marketplace",
                "plugins": [{
                    "name": "ccode-plugin",
                    "description": "From the .ccode-plugin index",
                    "category": "design",
                    "source": { "type": "local", "path": "./plugins/ccode-plugin" },
                    "tags": ["ccode"]
                }]
            }"#,
        )
        ...
        let plugins = scan_marketplace(dir.path()).entries;
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].name, "ccode-plugin");
        assert_eq!(plugins[0].category.as_deref(), Some("design"));
        assert_eq!(plugins[0].tags, vec!["ccode"]);
        assert!(plugins[0].keywords.is_empty());
    }
    ...
    fn write_ccode_file(dir: &Path, file: &str, content: &str) {
        let ccode_dir = dir.join(".ccode-plugin");
        std::fs::create_dir_all(&ccode_dir).unwrap();
        std::fs::write(ccode_dir.join(file), content).unwrap();
    }
    ...
    fn url_entry_gets_components_when_sha_matches() {
        ...
        write_ccode_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn url_entry_without_pinned_sha_gets_no_components() {
        let dir = tempfile::tempdir().unwrap();
        write_ccode_file(dir.path(), "marketplace.json", &url_marketplace_index(""));
        write_ccode_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn malformed_catalog_degrades_to_no_components() {
        ...
        write_ccode_file(dir.path(), "plugin-index.json", "not json");
