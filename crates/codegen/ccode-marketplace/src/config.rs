//! Parse marketplace sources from `~/.ccode/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
//! name = "ccode Official"
//! git = "https://github.com/ccode-org/ccode-marketplace.git"
//!
//! [[marketplace.sources]]
//! name = "Local Dev"
//! path = "~/dev/my-plugins"
...
/// `CCODE_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
pub fn env_require_sha() -> bool {
    ccode_config::env_bool("CCODE_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// instead of `~/.ccode`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn parse_git_source() {
        ...
        let sources = load_sources(&config);
        assert_eq!(sources.len(), 1);
        assert_eq!(sources[0].name, "ccode Official");
        assert!(
            matches!(&sources[0].kind, SourceKind::Git { url, branch } if url.contains("ccode-org") && branch.as_deref() == Some("main"))
        );
    }
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("CCODE_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("CCODE_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("CCODE_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
