...
//!
//! Catalog file lookup, in order: `.ccode-plugin/marketplace.json` (preferred),
//! `.ccode-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.ccode-plugin/marketplace.json` (preferred ccode convention)
/// 2. `.ccode-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_ccode_dir() {
        let dir = tempfile::tempdir().unwrap();
        let ccode_dir = dir.path().join(".ccode-plugin");
        std::fs::create_dir_all(&ccode_dir).unwrap();
        std::fs::write(
            ccode_dir.join("marketplace.json"),
            r#"{"name": "ccode", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "ccode");
    }

    #[test]
    fn load_index_ccode_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".ccode-plugin", "ccode"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "ccode");
    }
