...
//! Cache root: `~/.ccode/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = ccode_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(ccode_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    ccode_config::ccode_home().join("marketplace-cache")
}
...
mod tests {
    ...
    fn cache_hash_is_deterministic() {
        let url = "https://github.com/ccode-org/ccode-marketplace.git";
    ...
    #[test]
    fn default_cache_root_under_ccode() {
