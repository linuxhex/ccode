...
//! `.ccode-plugin/plugin-index.json` (preferred), then
...
use ccode_hooks_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_ccode_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".ccode-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_ccode_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".ccode-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".ccode-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".ccode-plugin",
            r#"{
                "$schema": "https://ccode.dev/ccode/plugin-index.schema.json",
                "version": 1,
