...
use ccode_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use ccode_pager::client_identity::PAGER_CLIENT_VERSION;
use ccode_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use ccode_shell::agent::config::Config as AgentConfig;
use ccode_shell::leader::{
...
use ccode_update::{UpdateConfig, auto_update, enforce_version_policy_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.ccode_ws_origin {
        config.ccode_com_config.ccode_ws_origin = v.clone();
    }
    if let Some(v) = &args.ccode_ws_url {
        config.ccode_com_config.ccode_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &ccode_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.ccode_api_base_url {
        config.endpoints.ccode_api_base_url = v.clone();
    }
}
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Ccode agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `ccode -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use ccode_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(ccode_telemetry::sampling_log::layer())
        .with(ccode_telemetry::instrumentation::layer())
        .with(ccode_telemetry::hooks_log::layer())
        .with(ccode_telemetry::otel_layer::build_otel_layer(
            ccode_telemetry::otel_layer::OtelClientInfo {
                client_name: "ccode-pager",
                client_version: ccode_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            ccode_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    ccode_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    ccode_telemetry::external::init(
        ccode_shell::agent::config::resolve_external_otel_config(
            ccode_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: ccode_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `ccode setup`: rendering + exit codes only; fetch logic lives in `ccode_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use ccode_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `ccode login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export CCODE_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:CCODE_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  ccode setup");
        eprintln!();
        eprintln!("Or add the key to ~/.ccode/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's Ccode administrator."
        );
        std::process::exit(1);
    }
    if json {
        match managed_config::fetch_setup_report().await {
            Ok(report) => {
                ...
                if !report.configured {
                    eprintln!(
                        "Your team doesn't have a managed configuration yet. A team admin can set one up at console.ccode.dev."
                    );
                }
            }
            Err(e) => {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `ccode setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = ccode_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !ccode_shell::util::is_ccode_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a ccode process, removing stale lock");
        ...
        if let Err(e) = ccode_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(ccode_shell::env::CcodeBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, ccode_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = ccode_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = ccode_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "ccode-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&ccode_shell::leader::ControlPayload>,
...
/// Env override for the `ccode workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "CCODE_WORKSPACE_COMMAND";
/// Resolution of the `ccode workspace` gate. `Unknown` is kept separate from
...
/// The `CCODE_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&ccode_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for ccode on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<ccode_shell::util::config::RemoteSettings> {
    join_early_prefetch(ccode_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`ccode workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `ccode workspace`. Check your \
             network connection (run `ccode login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.ccode_com_config.ccode_ws_url;
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a ccode session, or run `ccode workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = ccode_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use ccode_shell::auth::ensure_authenticated;
    ccode_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = ccode_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`ccode workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.ccode/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.ccode_com_config,
        false,
        Some("No cached credentials found. Run `ccode login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.ccode_com_config);
...
struct StdioReplayState {
    ...
    /// `ccode.dev/leader_reconnected` as the primary restored session.
...
fn cache_outgoing_acp_state(msg: &str, state: &std::sync::Mutex<StdioReplayState>) {
    ...
    match method {
        ...
        "ccode.dev/session/close" | "_ccode.dev/session/close" => {
...
const REPLAY_LOAD_REQUEST_ID: &str = "ccode.dev/leader-replay/session-load";
...
/// `ccode.dev/leader_reconnected` with empty params in that case, signalling the
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    ccode_telemetry::sentry::flush_on_shutdown();
    ccode_telemetry::otel_layer::shutdown_otel();
    ccode_telemetry::debug_log::flush();
...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "ccode: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<ccode_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use ccode_pager::app::signal_handler::next_signal_code;
    ...
    let _otel_guard = ccode_telemetry::otel_layer::otel_guard();
    ccode_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => ccode_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(error = %e, "--trust: failed to resolve cwd; folder not trusted")
            }
        }
    }
    let early_prefetch = ccode_shell::agent::models::start_early_prefetch(None);
    ccode_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Ccode Build (pager) - v{}",
            ccode_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                ccode_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(ccode_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = ccode_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("ccode: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = ccode_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&ccode_shell::agent::config::RuntimeResolutionContext {
    ...
    let managed_install = is_managed_install(
        ...
        &ccode_shell::util::ccode_home::ccode_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = ccode_shell::leader::LeaderEnvUrls::from(&agent_config.ccode_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = ccode_acp::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                            biased;
                                    ...
                                    .await
                                {
                                    Ok((new_tx, mut new_rx, _disconnect_rx)) => {
                                        ...
                                        let notification = format!(
                                            r#"{{"jsonrpc":"2.0","method":"ccode.dev/leader_reconnected","params":{params}}}"#
                                        );
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = ccode_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            ccode_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(ccode_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = ccode_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = ccode_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `CCODE_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `ccode dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `ccode dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `CCODE_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !ccode_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.ccode/config.toml and \
             unsetting CCODE_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("CCODE_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<ccode_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(ccode_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<ccode_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(ccode_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    ccode_shell::heap_profile::install(ccode_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "ccode {}\n",
        ccode_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), channel_label,)
    )
}
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = ccode_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    if let Some(code) = ccode_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = ccode_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    ...
    ccode_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    ccode_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        ccode_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        ccode_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    ccode_pager::memory_trace::start(
        ccode_shell::util::ccode_home::ccode_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = ccode_config::validate_requirements() {
        eprintln!("Couldn't start Ccode: {e}");
        eprintln!();
        eprintln!(
            "Update Ccode to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = ccode_telemetry::sentry::init(ccode_telemetry::sentry::Config {
        client: "ccode-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: ccode_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    ccode_pager::docs::extract_user_guide_docs(&ccode_shell::util::ccode_home::ccode_home());
    ccode_crash::install_terminal_restore_only();
    if ccode_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = ccode_shell::util::ccode_home::ccode_home().join("crash");
        if let Some(report) = ccode_crash::check_previous_crash(&crash_dir) {
            eprintln!("Ccode crashed during your last session.");
            eprintln!("  Signal:  {}", report.signal_name);
            eprintln!("  Version: {}", report.app_version);
            eprintln!("  Report:  {}", report.report_path.display());
            eprintln!();
        }
        if !ccode_crash::install(ccode_crash::CrashHandlerConfig {
    ...
    let crashed = ccode_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    ccode_telemetry::debug_log::flush();
    if let Err(e) = result {
        ccode_tty::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("CCODE_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("CCODE_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(ccode_shell::agent::chat_modes::CCODE_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(ccode_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("CCODE_DEBUG_LOG", path);
            std::env::remove_var("CCODE_LOG_FILE");
        }
    }
    ...
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return ccode_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        ccode_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        ccode_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    ccode_shell::config::apply_sandbox(
        None,
    ...
    ccode_shell::http::set_client_name(if is_interactive {
        ccode_workspace::permission::ClientType::CcodePager
    } else {
        ccode_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": ccode_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `ccode-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                ccode_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return ccode_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = ccode_telemetry::otel_layer::otel_guard();
                return ccode_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = ccode_telemetry::otel_layer::otel_guard();
                let config = ccode_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ccode_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ccode_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ccode_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ccode_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return ccode_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ccode_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return ccode_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                ccode_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                ccode_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = ccode_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                ccode_shell::auth::run_cli_logout(&config)?;
                ccode_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = ccode_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(ccode_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == ccode_pager::headless::OutputFormat::Plain
        {
            args.output_format = ccode_pager::headless::OutputFormat::Json;
        }
        return ccode_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            ccode_pager::headless::HeadlessOptions {
    ...
    let result = ccode_pager::app::run(args, bg_update_rx).await;
    ccode_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `ccode` to start.");
            } else {
                eprintln!("Update did not complete. Run `ccode update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `ccode update` child
...
/// fall back to a fresh blocking `ccode update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = ccode_shell::env::CcodeBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                ccode_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("CCODE_NPM_REGISTRY"))
        .ok()
        .or_else(ccode_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = ccode_shell::config::load_effective_config_disk_only()
        && let Some(ch) = ccode_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("CCODE_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<ccode_home>/bin/ccode` resolves to, the
...
/// npm shim hardcodes `~/.ccode`, so a custom `CCODE_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, ccode_home: &std::path::Path) -> bool {
    if ccode_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = ccode_config::ccode_application_in(ccode_home);
...
/// Handle `ccode-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `ccode update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in ccode_shell::leader::discover_leaders().await {
        if d.classification != ccode_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match ccode_shell::leader::LeaderClient::connect(
            socket_path,
            "ccode-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(ccode_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(ccode_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn version_output_writer_preserves_channel_aware_contract() {
        for (label, expected_suffix) in [
            ...
            assert!(output.starts_with("ccode "));
    ...
    fn version_flags_and_doctor_are_distinct_early_intents() {
        let version = PagerArgs::try_parse_from(["ccode", "--version"]).unwrap();
        assert!(version.version);
        assert!(version.command.is_none());
        let short = PagerArgs::try_parse_from(["ccode", "-v"]).unwrap();
        assert!(short.version);
        assert!(short.command.is_none());
        let subcommand = PagerArgs::try_parse_from(["ccode", "version"]).unwrap();
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            ccode_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: ccode_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/ccode-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            ccode_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!ccode_shell::heap_profile::prof_available());
            return;
        }
        assert!(ccode_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(ccode_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(ccode_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        ccode_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_ccode_target() {
        let home =
            std::env::temp_dir().join(format!("ccode-pager-managed-install-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        std::fs::create_dir_all(home.join("bin")).unwrap();
        std::fs::create_dir_all(home.join("downloads")).unwrap();
        assert!(!is_managed_install(
            Some(home.join("bin").join("ccode")),
            &home
        ));
        ...
        let target = home.join("downloads").join("ccode-1.2.3");
        std::fs::write(&target, b"binary").unwrap();
        std::os::unix::fs::symlink(&target, home.join("bin").join("ccode")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("ccode")),
            &home
        ));
        assert!(is_managed_install(Some(target.clone()), &home));
        let pinned = home.join("bin").join("ccode-9.9.9");
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("CCODE_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("CCODE_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `ccode dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(CCODE_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["ccode", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(CCODE_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("CCODE_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("CCODE_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("CCODE_WORKSPACE_COMMAND", "off") };
    ...
    fn cache_session_close_stops_replaying_it() {
        ...
        cache_outgoing_acp_state(
            r#"{"jsonrpc":"2.0","id":3,"method":"_ccode.dev/session/close","params":{"sessionId":"s1"}}"#,
            &state,
        );
        let s = state.lock().unwrap();
    ...
    async fn replay_waits_for_load_response_through_notifications() {
        ...
        let responder = tokio::spawn(async move {
            ...
            response_tx
                .send(
                    r#"{"jsonrpc":"2.0","method":"ccode.dev/leader/version_mismatch","params":{}}"#
                        .to_string(),
                )
                .unwrap();
            response_tx
    ...
    /// `ccode.dev/leader_reconnected` with empty params and the external client
