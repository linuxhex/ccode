...
/// Vendor API keys with `sk-`/`sk_` prefixes and ccode (`ccode-`) keys. `\b`-anchored so
/// `task-`/`disk-`/`risk-` don't fold a stray `sk-`.
static API_KEY_PREFIX_REGEX: LazyLock<Regex> =
    LazyLock::new(|| compile(r"\b(?:sk[-_]|ccode-)[A-Za-z0-9_-]{20,}"));
...
mod tests {
    ...
    fn no_match_returns_borrowed() {
        ...
        assert!(matches!(redact_secrets("model=ccode-3"), Cow::Borrowed(_)));
    }
    ...
    fn redacts_known_secret_shapes() {
        let cases = [
            (
                fixture(&["key: ccode-", "abc123XYZdef456GHIjkl789"]),
                "ccode api key",
            ),
            (
    ...
    fn redacts_sensitive_url_query_params() {
        let out = redact_secrets("callback https://ccode.dev/cb?code=ABC123XYZ&state=xyz789 failed");
    ...
    fn url_regex_excludes_trailing_punctuation() {
        let out = redact_secrets("see `https://ccode.dev/cb?code=ABCD12345`");
