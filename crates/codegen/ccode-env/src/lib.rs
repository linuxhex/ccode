...
//! Backend environment presets for the Ccode CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `CCODE_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CcodeBuildEndpoints {
...
const PRODUCTION_ENDPOINTS: CcodeBuildEndpoints = CcodeBuildEndpoints {
    cli_chat_proxy_base_url: "https://cli-chat-proxy.ccode.dev/v1",
    asset_server_url: "https://assets.ccode.dev",
    relay_ws_url: "wss://code.ccode.dev/ws/code-agent",
    gateway_ws_url: "wss://ccode.dev/ws/gw/",
    ws_origin: "https://ccode.dev",
};
...
pub enum CcodeBuildEnvironment {
    #[default]
    Production,
}
impl CcodeBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        CcodeBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            CcodeBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, CcodeBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            CcodeBuildEnvironment::Production => "CCODE_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> CcodeBuildEndpoints {
        match self {
            CcodeBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `ccode.dev/code` driving a
    ...
    /// `CCODE_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for CcodeBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CcodeBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            CcodeBuildEnvironment::Production.env_prefix(),
            "CCODE_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "CCODE_CCODE_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    /// loop mistakenly connecting to `wss://ccode.dev/ws/gw/`).
    #[test]
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            CcodeBuildEnvironment::Production.relay_ws_url(),
            CcodeBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            CcodeBuildEnvironment::from_flags(false, false),
            CcodeBuildEnvironment::Production
        );
    }
}
