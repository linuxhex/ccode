...
use crate::version::get_installed_ccode_version;
use semver::Version;
use tracing::warn;
use ccode_shell::util::config::VersionPolicy;
...
pub(crate) enum VersionPolicyError {
    #[error(
        "Cannot install Ccode {target}: the minimum allowed version is {minimum}. \
         Run `ccode update` to install the latest allowed version."
    )]
