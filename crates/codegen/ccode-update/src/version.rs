...
use ccode_shell::env::CcodeBuildEnvironment;
use ccode_shell::util::ccode_home::ccode_home;

const TTL_SECONDS_BEFORE_AUTO_UPDATE: Duration = Duration::from_secs(60 * 30);
const NPM_PACKAGE: &str = "@ccode-official/ccode";
pub const GH_RELEASE_REPO: &str = "ccode-org-shared/ccode-build";

/// Primary CLI base URL: Cloudflare-fronted ccode.dev endpoint with edge caching
/// for binaries and origin-respecting no-cache for channel pointers.
pub(crate) const CLI_BASE_URL_PRIMARY: &str = "https://ccode.dev/cli";
...
pub(crate) const CLI_BASE_URL_FALLBACK: &str =
    "https://storage.googleapis.com/ccode-build-public-artifacts/cli";
...
///
/// Constructed once from `CcodeBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `CcodeBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.ccode.dev/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.ccode/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (CCODE_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &CcodeBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: ccode_shell::auth::CcodeComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = ccode_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = CcodeVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<CcodeVersion>(&version_str)
...
pub use ccode_version::installed as get_installed_ccode_version;

/// Version of the managed ccode binary currently on disk, read from the
/// `~/.ccode/bin/ccode` symlink target (`../downloads/ccode-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `ccode update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.ccode/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = ccode_shell::util::ccode_home::ccode_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "ccode")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`ccode-0.1.150-macos-aarch64`, including
/// pre-releases: `ccode-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`ccode-0.1.150`,
/// `ccode-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`ccode-latest`, `ccode-pager-*` when `bin_prefix` is
/// `ccode`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.ccode/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: CcodeVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(ccode_version::VERSION, &stable)
    })
}
...
/// `~/.ccode/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(ccode_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = CcodeVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`ccode-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("ccode-0.2.46-darwin-arm64", Some("0.2.46")),
            ("ccode-0.1.220-linux-x86_64", Some("0.1.220")),
            ("ccode-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("ccode-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("ccode-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("ccode-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("ccode-garbage-darwin-arm64", None),               // unparseable version
            ("ccode-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("ccode-latest", None),                             // symlink alias, not a version
            ("ccode", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "ccode").as_deref(),
                *expected,
        ...
        // prefix but not under "ccode".
        assert_eq!(
            version_from_versioned_binary_name("ccode-pager-0.1.5-darwin-arm64", "ccode-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // CcodeVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: CcodeVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = CcodeVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: CcodeVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<CcodeVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<CcodeVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = CcodeVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use ccode_shell::env::CcodeBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&CcodeBuildEnvironment::Production);
