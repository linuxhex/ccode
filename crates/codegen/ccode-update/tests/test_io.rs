...
//!
//! These tests touch global process state — `CCODE_HOME` (a `OnceLock` in
//! `ccode-config`), `CCODE_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `CCODE_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the CCODE_HOME isolation used in other
//! integration tests.
...
use ccode_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_ccode_home() {
    let _ = test_home();
    reset();
...
/// Write a `CcodeVersion`-shaped JSON file with an arbitrary timestamp.
...
/// `CcodeVersion` JSON layout.
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under CCODE_HOME,
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every ccode launch.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_ccode_version env override
//
// The function honors `CCODE_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_ccode_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("CCODE_TEST_VERSION", "9.9.9");
    }
    let v = ccode_update::version::get_installed_ccode_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        std::env::remove_var("CCODE_TEST_VERSION");
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        ccode_update::version::get_installed_ccode_version()
    };

    unsafe {
        std::env::set_var("CCODE_TEST_VERSION", "0.0.0-test");
    }
    let overridden = ccode_update::version::get_installed_ccode_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("CCODE_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = ccode_update::version::get_installed_ccode_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("CCODE_TEST_VERSION", "not-a-version");
    }
    let v = ccode_update::version::get_installed_ccode_version();
