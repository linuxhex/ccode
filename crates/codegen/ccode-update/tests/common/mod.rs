...
//! its own `OnceLock<CCODE_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the CCODE_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes CCODE_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// CCODE_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `CCODE_HOME`, initialized exactly once per test
/// binary. Once initialized, `ccode_config::ccode_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            ...
            std::env::remove_var("CCODE_INSTALLER");
            std::env::remove_var("CCODE_MANAGED_BY_NPM");
            std::env::remove_var("CCODE_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `CCODE_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_ccode_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("CCODE_TEST_VERSION", v) };
}
...
/// Minimal [`ccode_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> ccode_update::UpdateConfig {
    ccode_update::UpdateConfig {
...
/// Backdate every file in `CCODE_HOME/downloads` by ~2 hours.
///
