...
//!
//! > `~/.ccode/bin/ccode` resolves to a binary that passes the smoke-test, OR it
...
use ccode_update::auto_update::install_internal_from_base;
...
/// (`ccode` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `ccode` should resolve to after an install attempt.
...
/// to both `ccode` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p ccode-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("CCODE_BLITZ_ITERS")
        .ok()
