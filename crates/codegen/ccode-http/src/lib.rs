...
//! `ccode_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `CCODE_POOL_*` /
//! `CCODE_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `CCODE_SAMPLER_SHARED_CLIENT=0` falls back to
...
// `OriginClientInfo` is owned by `ccode-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `ccode-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `ccode-telemetry` consumes
// the same type via `ccode_sampler::OriginClientInfo`. The shell-specific
...
/// `CCODE_CLIENT_VERSION` env vars. Returns `None` when
/// `CCODE_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`CCODE_CLIENT_NAME` env var, default `"ccode-shell"`).
...
/// (`ccode -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-ccode-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `ccode -p`, `run_headless_inner` for
/// `ccode agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn ccode_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(ccode_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        let meta = serde_json::json!({
            "clientIdentifier": "ccode-desktop",
            "clientVersion": "1.2.3",
        })
        .as_object()
        .cloned()
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "ccode-desktop".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("ccode-desktop/1.2.3 ccode-shell/"));
        ...
        assert!(without_version.starts_with("ccode-web ccode-shell/"));
        assert!(!without_version.starts_with("ccode-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "ccode-shell/0.1.171 (macos; aarch64)");
    }
}
