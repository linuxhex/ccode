...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"ccode_com_linear"`).
...
/// SDK-side id echoed back in `ccode.dev/mcp/sdk_call`. A named struct (rather than a
/// `(String, String)` tuple) so callers can't transpose the two strings.
///
/// `Deserialize`d straight from a `_meta["ccode.dev/mcp/servers"]` entry, so the
...
/// The session's in-process SDK MCP servers (declared via `_meta["ccode.dev/mcp/servers"]`,
...
struct AcpMcpRegistry {
    ...
    /// `ccode.dev/mcp/sdk_call` over the ACP connection).
...
pub struct McpState {
    ...
    /// Persisted to `~/.ccode/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: ccode_fs::events::EventWriter,
    ...
    /// `ccode.dev/mcp/server_status` notifications.
    ///
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: ccode_fs::events::EventWriter::noop(),
    ...
    /// `CcodeClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: ccode_fs::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &ccode_fs::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `ccode-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use ccode_telemetry::enums::McpInitStrategy;

/// Parse a non-empty `server__tool` ID with one overlap-aware delimiter and
/// valid [`ccode_tool_protocol::ToolId`] syntax.
pub fn parse_mcp_qualified_name(name: &str) -> Option<(ccode_tool_protocol::ToolId, &str, &str)> {
    ...
    Some((ccode_tool_protocol::ToolId::new(name).ok()?, server, tool))
}
...
impl McpError {
    ...
    pub fn error_category(&self) -> ccode_fs::events::McpErrorCategory {
        use ccode_fs::events::McpErrorCategory;
        match self {
...
///   the frontend via `ccode.dev/mcp/tools_changed` notifications and callable via
///   `ccode.dev/mcp/call`.
...
/// `ccode_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl ccode_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> ccode_tool_protocol::ToolId {
        ...
        ccode_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| ccode_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        ccode_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(ccode_fs::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(ccode_fs::events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            ccode_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(ccode_fs::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(ccode_fs::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &ccode_fs::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, ccode_tool_runtime::ToolError> {
        let mcp_service = client
            .ensure_initialized()
            .await
            .map_err(|e| ccode_tool_runtime::ToolError::custom("process_manager", e.to_string()))?;
        ...
        match result {
            ...
            Ok(Err(e)) => Err(ccode_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
                *is_timeout = true;
                ...
                Err(ccode_tool_runtime::ToolError::custom(
                    "process_manager",
                    format!(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(ccode_fs::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(ccode_fs::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
            ...
            Err(e) => {
                ...
                return Err(ccode_tool_runtime::ToolError::custom(
                    "process_manager",
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(ccode_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(ccode_fs::events::Event::McpTransportDecodeError {
...
enum PendingTransport {
    ...
    /// (`ccode.dev/mcp/sdk_call`). Rebuildable from its `server_id` + invoker, so handshake
...
/// Uses [`CcodeClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, CcodeClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`CcodeClientHandler`] when the server pushes a notification we
...
///
/// Consumers fan these out to ACP `ccode.dev/mcp/server_status` after 50 ms
...
pub struct McpClient {
    ...
    /// [`CcodeClientHandler`]** constructed by
...
/// and the [`CcodeClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to ccode-mcp
...
impl McpClient {
    ...
    /// channel. `server_id` is the id the agent echoes back in `ccode.dev/mcp/sdk_call`; the
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: ccode_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, CcodeClientHandler>, McpError> {
        ...
        match pending {
            ...
            PendingTransport::Acp { server_id, invoker } => {
                // Per-reverse-call backstop on `ccode.dev/mcp/sdk_call`: the larger of the
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("ccode-shell-{server_name}"),
                ccode_version::VERSION.to_string(),
            ),
        )
    ...
    /// Build the [`CcodeClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> CcodeClientHandler {
        CcodeClientHandler {
    ...
    /// references (the [`CcodeClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            ccode_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = ccode_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`ccode_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "ccode_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("ccode-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = ccode_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = ccode_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        ccode_telemetry::session_ctx::log_event(
                            ccode_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: ccode_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    ccode_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            ccode_fs::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = ccode_telemetry::instrumentation::timer("mcp_start_servers");
...
/// `ccode.dev/mcp/server_status` events.
///
...
impl ClientHandler for CcodeClientHandler {
    ...
    // See the [`CcodeClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            ccode_fs::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("ccode_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("CCODE_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("ccode_com_FIGMA", "https://example.com/mcp"));
        ...
        assert!(!is_figma_mcp("ccode_com_linear", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_ccode_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use ccode_hub_sdk::LocalRegistry;
        use ccode_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = ccode_fs::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = ccode_fs::events::EventWriter::open(tmp.path());
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = CcodeClientHandler {
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'ccode_com_notion' handshake failed: Auth required, when send initialize request"
        ));
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            ccode_fs::events::McpErrorCategory::AuthRequired
        ));
    }
