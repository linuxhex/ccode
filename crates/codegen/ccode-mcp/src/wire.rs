//! Single source of truth for the `ccode.dev/mcp/*` ACP wire strings.
//!
...
/// Forward tool-invocation method (client -> agent): `ccode.dev/mcp/call`.
///
...
pub const MCP_CALL: &str = "ccode.dev/mcp/call";

/// Reverse zero-IPC tool-invocation method (agent -> client): `ccode.dev/mcp/sdk_call`.
///
...
pub const MCP_SDK_CALL: &str = "ccode.dev/mcp/sdk_call";

/// `session/new` `_meta` key listing in-process SDK MCP servers: `ccode.dev/mcp/servers`.
pub const MCP_SERVERS: &str = "ccode.dev/mcp/servers";

/// `initialize` `_meta` capability flag advertising in-process SDK MCP support
/// (enables the SDK's `transport="acp"`): `ccode.dev/mcp/sdk`.
pub const MCP_SDK: &str = "ccode.dev/mcp/sdk";
