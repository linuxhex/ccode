...
const MCP_OAUTH_CLIENT_NAME: &str = "Ccode";
...
// mode disabled, multiple `ccode` invocations).
//
// Layer 1 (cross-process): filesystem lock at $CCODE_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    ccode_config::ccode_home().join(format!("mcp_auth_{safe}.lock"))
}
...
mod tests {
    ...
    async fn manager_ready_for_exchange(token_endpoint: String) -> (AuthorizationManager, String) {
        ...
        mgr.configure_client(
            OAuthClientConfig::new("ccode-test-client", "http://127.0.0.1:0/callback")
