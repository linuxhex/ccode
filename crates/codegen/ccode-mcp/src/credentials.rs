...
//!
//! Credentials are stored in `$CCODE_HOME/mcp_credentials.json`, keyed by a
//! composite key derived from the server name and URL. This keeps MCP OAuth
//! tokens isolated from the user's ccode auth (`auth.json`).
//!
...
///
/// Local helper (not shell-base): `ccode-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$CCODE_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$CCODE_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$CCODE_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user ccode home (set $CCODE_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$CCODE_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(ccode_config::user_ccode_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$CCODE_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("ccode-mcp-credentials-test");
