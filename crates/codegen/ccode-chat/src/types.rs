...
/// emitter in `ccode-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (ccode login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, CCODE_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `ccode_sampling_types::SamplingConfig` (which is secret-free).
