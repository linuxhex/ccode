...
use ccode_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource};
...
async fn shared_dedupes_by_directory() {
    ...
    let Ok(a) = ccode_fsnotify::shared(path.clone(), FsConfig::default()) else {
        eprintln!("skipping: OS watcher unavailable (resource limit?)");
        return;
    };
    let before = ccode_fsnotify::stats();
    let b = ccode_fsnotify::shared(path.clone(), FsConfig::default()).unwrap();
    ...
    let after = ccode_fsnotify::stats();
    assert_eq!(
    ...
    let c = ccode_fsnotify::shared(other.path().to_path_buf(), FsConfig::default()).unwrap();
    assert!(!Arc::ptr_eq(&a, &c), "different dirs must not share");
    assert_eq!(
        ccode_fsnotify::stats().created_total - after.created_total,
        1,
    ...
    drop(a);
    drop(b);
    let d = ccode_fsnotify::shared(path, FsConfig::default()).unwrap();
    assert_eq!(
        Arc::strong_count(&d),
        1,
...
///
/// ```bash
/// cargo test -p ccode-fsnotify --test integration \
...
async fn shared_watcher_scaling_demo() {
    ...
    for _ in 0..SESSIONS {
        let Ok(src) = ccode_fsnotify::shared(path.clone(), FsConfig::default()) else {
            eprintln!("skipping: OS watcher unavailable (resource limit?)");
            return;
        };
        handles.push(src);
