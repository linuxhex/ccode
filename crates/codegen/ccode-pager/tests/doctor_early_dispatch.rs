...
fn pager_binary() -> Result<std::path::PathBuf, String> {
    for key in ["PAGER_BINARY", "CARGO_BIN_EXE_ccode-pager"] {
    ...
    Err("PAGER_BINARY/CARGO_BIN_EXE_ccode-pager not set".to_owned())
}
...
fn doctor_json_bypasses_unrelated_startup_state() {
    ...
    let ccode_home = temp.path().join("ccode-home");
    std::fs::create_dir_all(&home).expect("create HOME");
    std::fs::create_dir_all(&ccode_home).expect("create CCODE_HOME");
    ...
    let before = directory_entries(&ccode_home);
    ...
    assert!(!String::from_utf8_lossy(&output.stdout).contains("Ccode Doctor"));

    let after = directory_entries(&ccode_home);
...
fn doctor_fix_without_id_lists_tmux_fixes_from_current_probe_evidence() {
    ...
    let home = temp.path().join("home");
    let ccode_home = temp.path().join("qhome");
...
fn doctor_hostile_home_and_byobu_create_no_config_files() {
    ...
    for (key, value) in [("HOME", "."), ("BYOBU_CONFIG_DIR", "relative")] {
        let mut command = base_pager_command(&binary, &home, &ccode_home, "/bin/bash");
        command
...
fn doctor_fix_without_id_lists_only_applicable_automatic_fixes() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(
        stdout.contains("On your local computer, run: ccode doctor fix ssh-wrap"),
        "{stdout}"
    );
    assert!(!home.join(".bashrc").exists());
...
fn doctor_tmux_fix_yes_writes_only_actual_home_tmux_config() {
    ...
    assert!(!ccode_home.join(".tmux.conf").exists());
}
...
fn doctor_fix_yes_writes_only_actual_home_shell_rc() {
    ...
    assert_eq!(
        ...
        "# >>> ccode doctor >>>\n# >>> terminal.ssh-wrap >>>\nalias ssh='ccode wrap ssh'\n# <<< terminal.ssh-wrap <<<\n# <<< ccode doctor <<<"
    );
    assert!(!ccode_home.join(".bashrc").exists());
}
...
fn doctor_fix_safety_boundaries_are_process_isolated() {
    ...
    assert!(
        stderr.contains("Ccode found an existing SSH alias or function")
...
fn run_pager(
    ...
    let mut command = base_pager_command(binary, home, ccode_home, shell);
