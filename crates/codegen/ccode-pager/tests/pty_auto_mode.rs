...
//!
//! Uses `ccode-pager-pty` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.ccode/auth.json` from `CCODE_AUTH_JSON` (path) or the
//! developer's `~/.ccode/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p ccode-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Duration;

use ccode_pager_pty::{PtyHarness, pager_binary};
use ccode_test_support::TestSandbox;
...
/// Prefer explicit path, else the user's real `~/.ccode/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("CCODE_AUTH_JSON") {
...
fn prepare_sandbox(sandbox: &mut TestSandbox, gate_on: bool) -> Vec<(String, String)> {
    ...
    sandbox.remove_env("CCODE_API_KEY");

    let home = sandbox.home();
    let ccode = sandbox.ccode_home();
    let _ = std::fs::create_dir_all(ccode);
    if let Some(src) = auth_json_source() {
        let dest = ccode.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.ccode/auth.json — may hit device login");
    }
    ...
    // of the developer's shell. `CCODE_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (ccode_config::
    ...
    env.push((
        "CCODE_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("CCODE_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (ccode-pager-pty)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "CCODE_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
