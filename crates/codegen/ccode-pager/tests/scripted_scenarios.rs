...
//!
//! These tests exercise the real `ccode-pager` binary through a PTY using
...
/// enterprise deploy report: with `CCODE_GOAL=1`, `/goal` must
...
/// explicitly off (`CCODE_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `CCODE_FOLDER_TRUST=1` and a git repo that
...
/// also pins `CCODE_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
