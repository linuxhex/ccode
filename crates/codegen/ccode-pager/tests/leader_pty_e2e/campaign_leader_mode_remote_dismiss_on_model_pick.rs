...
#[ignore = "PTY e2e; run with cargo test -p ccode-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared CCODE_HOME so every spawn elects/attaches to the
    ...
    // Session (OAuth) auth, not the harness's default CCODE_API_KEY: the
    ...
    let state_path = ccode_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(ccode_home.join("config.toml")).expect("read config.toml");
    assert!(
