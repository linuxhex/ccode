...
/// turn and the turn-end `ccode.dev/suggestPrompt` call (the mock's fixed mode
...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`ccode-build-0.1`, the shell's built-in default)
    ...
    let overrides = [("CCODE_PROMPT_SUGGESTIONS".to_owned(), "true".to_owned())];
