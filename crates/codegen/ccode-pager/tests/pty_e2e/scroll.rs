...
use ccode_pager_pty::PtyHarness;
pub(crate) use ccode_pager_pty::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`ccode-pager-pty/src/scroll_matrix/runner.rs`) ports this
...
/// `ccode-pager-pty/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `CCODE_SCROLL_SPEED`) appended to the controller's env.
