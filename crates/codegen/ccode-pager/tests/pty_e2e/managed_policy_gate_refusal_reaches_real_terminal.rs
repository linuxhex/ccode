...
async fn managed_policy_gate_refusal_reaches_real_terminal() {
    ...
    let home_path = sandbox.ccode_home();
    std::fs::write(
    ...
    let mut harness = PtyHarness::new_in_sandbox_ops(
        ...
        &["--no-auto-update"],
        &sandbox,
        // CCODE_MANAGED_CONFIG=0 disables the background refetch so the gate decision is deterministic and offline.
        &[
            EnvOp::set("CCODE_MANAGED_CONFIG", "0"),
