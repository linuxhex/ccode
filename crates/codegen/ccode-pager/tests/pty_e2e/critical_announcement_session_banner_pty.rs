...
const PROMO_URL: &str = "https://ccode.dev/zz-promo-cta";
...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    let overrides: Vec<(String, String)> = vec![(
        "CCODE_ANNOUNCEMENTS_OVERRIDE".into(),
...
async fn critical_announcement_hide_button_click_hides_banner() {
    ...
    let long_msg = format!(
        "{CRIT_MSG} elevated error rates persist across regions check status.ccode.dev for updates and retry your request later"
    );
...
/// safe-open path — observed via the `CCODE_TEST_OPEN_URL_FILE` seam, never a
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("CCODE_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `CCODE_ANNOUNCEMENTS_OVERRIDE`.
...
/// `CCODE_TEST_OPEN_URL_FILE` seam + a `TERM_PROGRAM` pin for OSC 8).
fn spawn_with_announcements_and_env(
    ...
    let binary = pager_binary().expect("resolve pager binary");
    let announcement = ("CCODE_ANNOUNCEMENTS_OVERRIDE", override_json);
