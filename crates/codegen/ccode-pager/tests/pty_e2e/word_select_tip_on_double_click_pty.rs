...
async fn word_select_tip_shows_and_ctrl_y_accepts() {
    ...
    let config_path = content.home().join(".ccode").join("config.toml");
...
async fn word_select_tip_skipped_when_contextual_hint_disabled() {
    let content = ContentController::start().await.expect("start content");
    // flash mode + tip explicitly disabled. CCODE_CONTEXTUAL_HINTS is NOT set —
    ...
    // inherited CCODE_CONTEXTUAL_HINTS from the runner's shell can't force
    // tips on and defeat the config opt-out under test.
    let overrides: Vec<(String, String)> = vec![("CCODE_CONTEXTUAL_HINTS".into(), String::new())];
