...
pub(crate) const CONTEXTUAL_HINTS_ENV: &[(&str, &str)] = &[("CCODE_CONTEXTUAL_HINTS", "1")];
...
// `ccode_pager_pty::flows` (re-exported above).
...
/// the harness's default `CCODE_API_KEY` (ApiKey/BYOK mode, no auth.json entry)
/// would never fetch `/v1/settings`. Spawns WITHOUT `CCODE_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    overrides.push(EnvOp::set("CCODE_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// Explicit overrides for a folder-trust run. A self-built ccode auto-trusts,
/// so `CCODE_TEST_VERSION` simulates a release; the gate is pinned both ways.
pub(crate) fn trust_env(feature_on: bool) -> [(&'static str, &'static str); 2] {
    [
        ("CCODE_TEST_VERSION", "0.0.0-sim"),
        ("CCODE_FOLDER_TRUST", if feature_on { "1" } else { "0" }),
    ]
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".ccode")
        .join(ccode_workspace::trust::TRUST_FILE_NAME);
    let store = ccode_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&ccode_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `ccode_pager_pty::flows`
// (re-exported above).
...
/// Budget for session creation plus the `ccode.dev/mcp/list` round-trip.
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let ccode_home = content.home().join(".ccode");
    std::fs::create_dir_all(&ccode_home).expect("create fake CCODE_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(ccode_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in ccode-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.ccode/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{CCODE_HOME|HOME}/.ccode/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let ccode_home = content.home().join(".ccode");
    std::fs::create_dir_all(&ccode_home).expect("create .ccode");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let ccode_home = content.home().join(".ccode");
    std::fs::create_dir_all(&ccode_home).expect("create .ccode");
    // Minimal opt-in only — matches load_config's `{CCODE_HOME|HOME}/.ccode/config.toml`.
...
pub(crate) fn spawn_mouse_toggle_pager(content: &ContentController) -> PtyHarness {
    ...
    PtyHarness::spawn_with_content_env_ops(
        ...
        content,
        &[],
        &[EnvOp::set("CCODE_MOUSE_REPORTING_TOGGLE", "true")],
    )
    .expect("spawn pager")
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "CCODE_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = ccode_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
/// `ccode wrap` run budget. Same contention math as the requirements-version
...
/// Run `ccode wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `CCODE_HOME`, returning the exit code (`None` only while still running at
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("CCODE_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness =
        PtyHarness::new_inherited_env(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env, None)
            .expect("spawn ccode wrap");
    ...
    let code = match wait_for_exit_status(&mut harness, WRAP_TIMEOUT) {
        ...
        Ok(PtyExitPoll::Running) => {
            harness.quit().expect("kill ccode wrap after timeout");
            None
        }
        Ok(PtyExitPoll::PendingStatus) => {
            panic!("ccode wrap exited but portable status remained unavailable for {WRAP_TIMEOUT:?}")
        }
        Err(error) => panic!("poll ccode wrap exit: {error:#}"),
    };
...
/// `ccode wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$CCODE_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("CCODE_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use ccode_pager_pty::host_clipboard::{
...
pub(crate) use ccode_pager_pty::host_clipboard::clipboard_roundtrip_works;
