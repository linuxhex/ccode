...
use crate::common::*;
use ccode_pager_pty::{InferenceEndpoint, InferenceRequestMatcher};
...
async fn minimal_commits_thinking_body_to_scrollback() {
    ...
    std::fs::create_dir_all(content.home().join(".ccode")).expect("mk .ccode");
    std::fs::write(
        content.home().join(".ccode/config.toml"),
