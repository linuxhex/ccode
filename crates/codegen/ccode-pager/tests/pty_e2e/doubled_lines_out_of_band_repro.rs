...
//!
//! The stack is `tmux -> nvim :terminal -> ccode`: nvim/tmux repaint ccode's
//! pane out-of-band (no ccode PTY resize), and ccode's diff renderer only
...
//! into the virtual terminal, bypassing ccode) and then check whether ccode
//! heals it. `NVIM` is set in ccode's env so it sees the embedded-editor
//! context the fix keys off (mirroring a real nvim `:terminal`).
//!
//! The fix: ccode forces a full clear+repaint on `FocusGained` in
//! editor/multiplexer contexts, so the injected row is gone after refocus.
//! The final assertion guards that heal.

use super::common::*;

/// Unique sentinel that ccode would never render on its own.
...
async fn out_of_band_stale_row_heals_on_focus_gained() {
    ...
    let overrides: Vec<(String, String)> =
        vec![("NVIM".into(), "/tmp/ccode-pty-harness-fake-nvim.sock".into())];
    ...
    // virtual screen at col 1 — the static left margin ccode's diff renderer
    ...
    // ccode must not self-heal out-of-band content via ordinary diff redraws:
    // it only rewrites cells whose own model changed, so this row is stranded.
    h.update(Duration::from_millis(300));
    assert!(
        h.contains_text(STALE_MARKER),
        "stale row should survive a normal redraw (ccode's diff renderer doesn't own it)\nscreen:\n{}",
        h.screen_contents()
    );

    // A FocusGained (CSI I) forces a full clear+repaint that re-asserts ccode's
