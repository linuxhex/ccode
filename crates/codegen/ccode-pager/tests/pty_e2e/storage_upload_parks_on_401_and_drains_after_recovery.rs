...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party ccode OAuth (`is_ccode_auth()`:
    // AuthMode::Oidc + ccode issuer); the harness's CCODE_API_KEY is ApiKey mode
    ...
    let overrides = [
        ...
        EnvOp::set("CCODE_TRACE_UPLOAD", "true"),
        EnvOp::set("CCODE_TELEMETRY_TRACE_UPLOAD", "true"),
        EnvOp::set("CCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"),
    ];
    ...
    //
    // Accounting (ccode-fs upload queue):
    ...
    //   not fire inside this 3s window; with `CCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS=2`
