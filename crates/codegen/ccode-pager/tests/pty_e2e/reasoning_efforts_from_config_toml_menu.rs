...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("ccode-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.ccode/config.toml` with a per-model reasoning-effort menu.
    let ccode_home = content.home().join(".ccode");
    std::fs::create_dir_all(&ccode_home).expect("create .ccode");
    // Quote the dotted model id: bare `[model.ccode-4.5]` is TOML key-path syntax (model.ccode-4.5), not the id "ccode-4.5".
    std::fs::write(
