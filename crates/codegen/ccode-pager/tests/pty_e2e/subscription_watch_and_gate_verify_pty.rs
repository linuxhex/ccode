...
const PAID_TIER: &str = "PremiumPro";

/// Display name delivered via `/settings` `subscription_tier_display`.
const PAID_TIER_DISPLAY: &str = "Premium Pro";
...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `premium_heavy` = live
/// `/user` string `PremiumPro`. Must match
...
/// Like [`seed_fake_oauth`], but under the `CCODE_LOCAL_AUTH` dev issuer
/// (`http://localhost:22255`). Two reasons: `is_ccode_oauth2_issuer()` accepts
...
/// call to auth.ccode.dev (hermetic, no CI-network flake). Pair with
/// `CCODE_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let ccode_home = content.home().join(".ccode");
    std::fs::create_dir_all(&ccode_home).expect("create temp .ccode");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    overrides.push(EnvOp::set("CCODE_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let content = ContentController::start_with_models(vec![MockModel::new("ccode-3")])
        .await
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[EnvOp::set("CCODE_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://ccode.dev/premium?referrer=ccode-build",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[EnvOp::set("CCODE_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
