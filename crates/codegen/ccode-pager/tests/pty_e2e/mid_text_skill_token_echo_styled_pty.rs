...
use ccode_pager_pty::StyledLine;
...
/// Seed a user-invocable skill under `dir/.ccode/skills`. The workspace must
/// be a git repo: the live session advertises workspace-local skills only for
/// git workspaces (offline `ccode inspect` scans plain dirs too) — same shape
/// as the mid_text_skill_token_echo.yaml scenario's `git_init: true`.
fn seed_test_skill(dir: &Path) {
    let skill_dir = dir.join(".ccode").join("skills").join("test-skill");
