#!/bin/bash
#
# Ccode CLI installer (enterprise channel) — https://ccode.dev/cli/enterprise-install.sh
#
...
#
# Usage:
#   curl -fsSL https://ccode.dev/cli/enterprise-install.sh | bash            # latest enterprise
#   curl -fsSL https://ccode.dev/cli/enterprise-install.sh | bash -s 0.1.42  # specific version
#   GROK_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://ccode.dev/cli/enterprise-install.sh)
#
...
if [ "$os" = "windows" ]; then
    ...
    chmod +x "$binary_path"
    ln -sf "$binary_path" "$BIN_DIR/ccode"
