#!/bin/bash
#
# Ccode CLI installer — https://ccode.dev/cli/install.sh
#
# Auth: GROK_DEPLOYMENT_KEY (takes precedence) or ~/.ccode/auth.json from `ccode login`.
# Env: GROK_CHANNEL (stable|alpha|enterprise, default: stable), GROK_BIN_DIR, GROK_PROXY_URL
#
# Usage:
#   curl -fsSL https://ccode.dev/cli/install.sh | bash            # latest stable
#   curl -fsSL https://ccode.dev/cli/install.sh | bash -s 0.1.42  # specific version
#   GROK_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://ccode.dev/cli/install.sh)
#
...
# Read a token from ~/.ccode/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_grok_token() {
    local auth_file="$HOME/.ccode/auth.json"
    local scope="$1"
...
OIDC_SCOPE="https://auth.ccode.dev::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.ccode.dev/sign-in"
AUTH_SOURCE=""
...
if [ -n "$GROK_DEPLOYMENT_KEY" ]; then
    ...
    if [ -n "$OIDC_TOKEN" ]; then
        ...
        echo "Auth: using OIDC token from ~/.ccode/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.ccode/auth.json." >&2
    fi
fi
...
BASE_URL_PRIMARY="https://ccode.dev/cli"
BASE_URL_FALLBACK="https://storage.googleapis.com/ccode-build-public-artifacts/cli"
DOWNLOAD_DIR="$HOME/.ccode/downloads"
BIN_DIR="${GROK_BIN_DIR:-$HOME/.ccode/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${GROK_CHANNEL:-stable}"

# Pick a working BASE_URL: try Cloudflare-fronted ccode.dev first, fall back to
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Ccode $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Ccode $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/ccode-$platform"
artifact_base="${BASE_URL}/ccode-${version}-${platform}"
...
echo "  Downloading ccode ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Ccode is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in ccode.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/ccode.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded ccode failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.ccode/bin and ~/.ccode/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/ccode"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/ccode and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.ccode/completions/bash" "$HOME/.ccode/completions/zsh"
"$BIN_DIR/ccode" completions bash > "$HOME/.ccode/completions/bash/ccode.bash" 2>/dev/null || true
"$BIN_DIR/ccode" completions zsh  > "$HOME/.ccode/completions/zsh/_grok"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/ccode" completions fish > "$HOME/.config/fish/completions/ccode.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.ccode/config.toml"
...
if [ -n "$GROK_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${GROK_PROXY_URL:-https://cli-chat-proxy.ccode.dev/v1}"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.ccode/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.ccode/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.ccode/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.ccode/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Ccode $version installed to $BIN_DIR/ccode.exe" >&2
else
    echo "Ccode $version installed to $BIN_DIR/ccode" >&2
fi

# --- Ensure ccode is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so ccode works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/ccode" "$candidate/ccode"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/ccode -> $BIN_DIR/ccode" >&2
...
# Also update shell config so ~/.ccode/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> ccode installer >>>
fish_add_path $HOME/.ccode/bin
# <<< ccode installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> ccode installer >>>
export PATH="$HOME/.ccode/bin:$PATH"
fpath=(~/.ccode/completions/zsh $fpath)
...
# <<< ccode installer <<<'
    ...
    if grep -qs "ccode installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> ccode installer >>>/ { skip=1; next }
            /# <<< ccode installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'ccode' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'ccode' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'ccode' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.ccode/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use ccode from cmd.exe or PowerShell, add %USERPROFILE%\\.ccode\\bin to your PATH." >&2
fi
