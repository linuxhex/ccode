#
# Ccode CLI installer (enterprise channel) for PowerShell — https://ccode.dev/cli/enterprise-install.ps1
#
...
#
# Auth: GROK_DEPLOYMENT_KEY env var (takes precedence) or ~/.ccode/auth.json from `ccode login`.
# Env: GROK_BIN_DIR, GROK_PROXY_URL
#
# Usage:
#   irm https://ccode.dev/cli/enterprise-install.ps1 | iex                                       # latest enterprise
#   & ([scriptblock]::Create((irm https://ccode.dev/cli/enterprise-install.ps1))) -Version 0.1.42 # specific version
#   $env:GROK_VERSION="0.1.42"; irm https://ccode.dev/cli/enterprise-install.ps1 | iex           # specific version (alt)
#   $env:GROK_DEPLOYMENT_KEY="<key>"; irm https://ccode.dev/cli/enterprise-install.ps1 | iex
#
...
if ($PSVersionTable.Platform -and $PSVersionTable.Platform -ne 'Win32NT') {
    Write-Error "This installer is for Windows. On macOS/Linux, use: curl -fsSL https://ccode.dev/cli/enterprise-install.sh | bash"
    exit 1
}

$GrokDir = Join-Path $env:USERPROFILE '.ccode'
...
$OidcScope = 'https://auth.ccode.dev::b1a00492-073a-47ea-816f-4c329264a828'
$LegacyScope = 'https://accounts.ccode.dev/sign-in'
$AuthSource = ''
...
if ($env:GROK_DEPLOYMENT_KEY) {
    ...
    if ($oidcToken) {
        ...
        Write-Host 'Auth: using OIDC token from ~/.ccode/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.ccode/auth.json.' -ForegroundColor DarkGray
    }
}
...
$BaseUrlPrimary = 'https://ccode.dev/cli'
$BaseUrlFallback = 'https://storage.googleapis.com/ccode-build-public-artifacts/cli'
...
# Pick a working BaseUrl: try Cloudflare-fronted ccode.dev first, fall back to
...
if ($AuthSource) {
    Write-Host "Installing Ccode $resolvedVersion ($platform, $AuthSource)..." -ForegroundColor Cyan
} else {
    Write-Host "Installing Ccode $resolvedVersion ($platform)..." -ForegroundColor Cyan
}

# --- Download binary ---

$binaryPath = Join-Path $DownloadDir "ccode-$platform.exe"
$artifactBase = "$BaseUrl/ccode-$resolvedVersion-$platform"
...
foreach ($binName in @('ccode.exe', 'agent.exe')) {
...
Write-Host "  Installed to $BinDir\ccode.exe and $BinDir\agent.exe." -ForegroundColor DarkGray
...
try {
    ...
    & (Join-Path $BinDir 'ccode.exe') completions powershell 2>$null |
        Set-Content (Join-Path $completionsDir 'ccode.ps1') -ErrorAction SilentlyContinue
} catch {}
...
if ($env:GROK_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:GROK_PROXY_URL) { $env:GROK_PROXY_URL } else { 'https://cli-chat-proxy.ccode.dev/v1' }
...
Write-Host "Ccode $resolvedVersion installed to $BinDir\ccode.exe" -ForegroundColor Green
...
if ($pathEntries -notcontains $BinDir) {
    ...
    # Update current session so ccode works immediately.
...
Write-Host ''
Write-Host "Run 'ccode' or 'agent' to get started!" -ForegroundColor Cyan
