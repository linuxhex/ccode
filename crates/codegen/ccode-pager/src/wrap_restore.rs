//! Terminal-mode tracking and restore emission for `ccode wrap`.
//!
//! `ccode wrap` cannot tell a clean child exit from a connection drop: an ssh
...
//! [`ccode_crash::terminal`] (`RESTORE_SEQ`) — pinned mechanically by a
...
/// `ccode_crash::terminal::RESTORE_SEQ`'s ordering tests.
...
/// `ccode_crash::terminal::RESTORE_SEQ` for every element the two
...
mod tests {
    ...
    fn covers_and_orders_every_crash_handler_restore_seq_element() {
        let elements: Vec<Vec<u8>> = ccode_crash::terminal::RESTORE_SEQ
            .split(|&b| b == 0x1b)
