...
//! the generic [`CodeBlockSpan`](ccode_markdown::CodeBlockSpan) API) and
...
use ccode_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](ccode_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](ccode_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a ccode_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `CcodeDay` is the only light theme; every other concrete theme (and the
/// `CcodeNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `ccode_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::CcodeDay)
}
