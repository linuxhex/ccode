...
pub enum SessionEvent {
    ...
    /// Message comes from agent via CcodeSessionUpdate::HookAnnotation.
    HookAnnotation {
        /// The hook message
        message: String,
    },
...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "ccode-4.5".into(),
            new_model_id: "ccode-build".into(),
            reason: "Model \"ccode-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"ccode-4.5\" is no longer available. Switched to \"ccode-build\"."
        );
    }
