...
//! `ccode.dev/log` ACP notifications. Call [`init`] once at startup with
...
use ccode_telemetry::unified_log::{
...
fn push_entry(lvl: LogLevel, msg: &str, sid: Option<&str>, ctx: Option<serde_json::Value>) {
    let entry = ClientLogEntry {
        ...
        ver: Some(ccode_version::VERSION.to_owned()),
        lvl,
...
fn send_entries(entries: Vec<ClientLogEntry>) {
    ...
    handle.spawn(async move {
        let _ = ccode_acp::acp_send(notification, &tx).await;
    });
}
