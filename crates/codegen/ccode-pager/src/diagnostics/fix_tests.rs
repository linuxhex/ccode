...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "ccode doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("ccode doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn tmux_fix_registry_resolves_every_short_and_canonical_id() {
    for (id, handle, _) in automatic_fix_choices() {
        ...
        assert_eq!(
            ...
            format!("ccode doctor fix {handle}")
        );
    }
}
...
fn tmux_managed_items_coexist_and_each_apply_is_one_transaction() {
    ...
    assert_eq!(content.matches("# >>> ccode doctor >>>").count(), 1);
...
fn tmux_stale_plan_and_idempotence_reuse_managed_writer_safety() {
    ...
    assert!(matches!(
        ...
        Err(FixError::TmuxManaged(
            ccode_config::managed_text::ManagedConfigError::StalePlan(_)
        ))
    ));
...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='ccode wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='ccode wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn non_utf8_source_fails_closed_before_conflict_policy() {
    ...
    assert!(matches!(
        ...
        Err(FixError::Managed(
            ccode_config::managed_text::ManagedConfigError::UnsafePath { .. }
        ))
    ));
}
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    let ccode = temp.path().join("ccode");
    std::fs::write(
        &ccode,
        format!(
            "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
    ...
    .unwrap();
    use std::os::unix::fs::PermissionsExt as _;
    std::fs::set_permissions(&ccode, std::fs::Permissions::from_mode(0o755)).unwrap();

    if let Some(bash) = find_on_path("bash") {
        let rc = temp.path().join("bashrc");
        std::fs::write(&rc, "alias ssh='ccode wrap ssh'\n").unwrap();
        let command = format!(
        ...
        ccode_tty::detach_std_command(&mut shell);
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='ccode wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
    ...
    if let Some(fish) = find_on_path("fish") {
        ...
        let fish_ccode = temp.path().join("fish-ccode");
        std::fs::write(
            &fish_ccode,
            format!(
                "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
        ...
        .unwrap();
        std::fs::set_permissions(&fish_ccode, std::fs::Permissions::from_mode(0o755)).unwrap();
        let rc = temp.path().join("config.fish");
        std::fs::write(&rc, "alias ssh 'fish-ccode wrap ssh'\n").unwrap();
        let command = format!(
