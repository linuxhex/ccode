...
/// Passive input-device probe for `ccode doctor` / `/doctor`.
///
...
pub fn apply_voice_probe(report: &mut DiagnosticReport, emit_missing_issue: bool) {
    if !ccode_voice::AUDIO_SUPPORTED {
        return;
    }
    match ccode_voice::input_device_info() {
        Ok(device) => {
        ...
        Err(err) => {
            let error = match err {
                ccode_voice::VoiceError::Config(message) => message,
    ...
    DiagnosticFinding {
        ...
        note: Some(
            "Connect or select a microphone in your system sound settings. On Linux, install a \
             supported audio recorder if none was found on PATH. Then run `/doctor` or `ccode \
...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `ccode wrap` on the local end, so
...
pub(crate) fn collect_startup_warnings_from(
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Ccode also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `ccode wrap ssh <host>` on your local computer or use a terminal that \
    ...
    if ctx.is_tmux_backed() && matches!(tmux.control_mode, probes::TmuxProbeResult::Available(true))
    {
        let message = match fullscreen_active {
            ...
            Some(false) => "Ccode is using inline mode because tmux control mode limits fullscreen",
...
pub(crate) fn wezterm_kitty_keyboard_warning_from(
    ...
    if shape == WezTermShape::SshXtversion {
        ...
        warning.note = Some(
            "For this session, type `\\` and then press Enter. Ccode can't negotiate the Kitty \
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(ccode_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        config_path: None,
        note: Some(format!(
            "Ccode is using the user profile. Compare `.ccode/sandbox.toml` with {}, then rename \
             or remove the conflicting project profile. Project settings can add profile names \
             but can't redefine a user profile.",
            crate::util::display_user_ccode_path("sandbox.toml")
        )),
    })
}

/// Pure SSH `ccode wrap` recommendation — suggests launching the session
/// through `ccode wrap ssh <host>` on the user's local machine, which gives a
...
/// - `!osc52_sink_active` — no wrap is already capturing our output. `ccode
...
pub(crate) fn collect_notification_warnings_with_method(
        ...
        && ctx.brand == TerminalName::Unknown
    {
        let mut warning = TerminalWarning::new(
            ...
            "Ccode is using the terminal bell because the terminal was not recognized",
            None,
            None,
        );
        warning.note = Some(format!(
            ...
            crate::util::display_user_ccode_path("config.toml")
        ));
        warnings.push(warning);
    }
    ...
    if condition == NotificationCondition::Unfocused && !supports_focus_tracking(ctx.brand) {
        let mut warning = TerminalWarning::new(
            ...
            Some(&crate::util::display_user_ccode_path("config.toml")),
        );
        warning.note = Some(
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset `NO_COLOR`, then restart Ccode.".to_string());
        return Some(warning);
    }
    ...
    if is_tmux_backed {
        ...
        warning.note = Some(format!(
            "In the same tmux config, also add `set -g default-terminal \"tmux-256color\"`. Add \
             ...
             `tmux source-file {tmux_config_path}`, or detach and reattach, and restart Ccode."
        ));
        return Some(warning);
    }
...
mod tests {
    ...
    fn sandbox_profile_conflict_warning_reports_conflicts() {
        ...
        assert!(w.note.as_deref().is_some_and(|note| {
            note.contains("rename or remove")
                && note.contains(".ccode/sandbox.toml")
                && note.contains(&crate::util::display_user_ccode_path("sandbox.toml"))
    ...
    // -- ssh_wrap_hint: `ccode wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("ccode wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        ...
        // `ccode wrap` — adoption silences the hint by itself.
        ...
        assert!(findings[1].remediation.as_ref().is_some_and(|remediation| {
            remediation.fix.contains("condition = \"always\"")
                && remediation.config_path.as_deref()
                    == Some(crate::util::display_user_ccode_path("config.toml").as_str())
        }));
    }
    ...
    fn notification_none_protocol_no_warnings() {
        let ctx = TerminalContext {
            brand: TerminalName::CcodeDesktop,
            ..Default::default()
        };
        let query = FakeTmuxQuery::healthy_modern();
        let w = collect_notification_warnings(
    ...
    fn supports_focus_tracking_known_terminals() {
        ...
        assert!(supports_focus_tracking(TerminalName::CcodeDesktop));
