...
fn tmux_config_and_reload_notes_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Automatic setup: `ccode doctor fix dcs-passthrough`\n",
            ...
            "      Automatic setup: `ccode doctor fix tmux-extended-keys`\n",
...
fn limited_color_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Note: Add this export to your shell startup file, such as `~/.zshrc` or `~/.bashrc`, then restart Ccode.\n",
        )
    );
}
...
fn wezterm_xtversion_runtime_evidence_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Note: For this session, type `\\` and then press Enter. Ccode can't negotiate the Kitty keyboard protocol over SSH yet. `enable_kitty_keyboard = true` applies only to local WezTerm sessions.\n",
        )
    );
}
...
fn runtime_startup_findings_are_visible_with_useful_doctor_content() {
    ...
    assert!(output.contains("Ccode is using the terminal bell"));
    assert!(output.contains("If the bell works for you"));
    assert!(output.contains("may not report focus changes"));
    assert!(output.contains(&crate::util::display_user_ccode_path("config.toml")));
...
fn runtime_findings_merge_before_single_formatter_orders_issues_before_recommendations() {
    ...
    let issue = output.find("Ccode is using the terminal bell").unwrap();
