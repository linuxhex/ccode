...
use ccode_announcements::visible_announcements;
...
fn is_critical(a: &ccode_announcements::RemoteAnnouncement) -> bool {
    a.severity.as_deref() == Some("critical")
}

fn is_promo(a: &ccode_announcements::RemoteAnnouncement) -> bool {
...
fn is_live_critical(
    a: &ccode_announcements::RemoteAnnouncement,
    now: chrono::DateTime<chrono::Utc>,
) -> bool {
    is_critical(a) && !ccode_announcements::is_expired_at(a, now)
}
...
fn is_live_promo(
    ...
    is_promo(a) && !ccode_announcements::is_expired_at(a, now)
}
...
pub fn is_dismissible(a: &ccode_announcements::RemoteAnnouncement) -> bool {
...
fn is_hidden(
    ...
    is_dismissible(a) && hidden_ids.contains(&ccode_announcements::announcement_hide_key(a))
}
...
fn usable_cta(a: &ccode_announcements::RemoteAnnouncement) -> Option<(&str, &str)> {
...
pub(crate) fn usable_cta_caption(a: &ccode_announcements::RemoteAnnouncement) -> Option<&str> {
...
fn first_critical_session_announcement<'a>(
    announcements: &'a [ccode_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<&'a ccode_announcements::RemoteAnnouncement> {
...
pub fn promo_cta_target<'a>(
    announcements: &'a [ccode_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<(&'a ccode_announcements::RemoteAnnouncement, &'a str)> {
...
pub fn session_announcement_hide_keys_at(
    ...
    visible_announcements(announcements)
        ...
        .map(ccode_announcements::announcement_hide_key)
        .collect()
}
...
fn render_critical_rows(
    area: Rect,
    buf: &mut Buffer,
    ann: &ccode_announcements::RemoteAnnouncement,
    hide_hovered: bool,
) -> BannerHits {
    use unicode_width::UnicodeWidthStr;
...
mod tests {
    ...
    fn promo(id: &str, message: &str, cta: Option<(&str, &str)>) -> RemoteAnnouncement {
        RemoteAnnouncement {
            ...
            cta: cta.map(|(label, url)| ccode_announcements::AnnouncementCta {
    ...
    fn session_hide_keys_cover_live_criticals_and_promos_only() {
        ...
        let list = vec![
            ...
            promo("promo-1", "upsell", Some(("Go", "https://ccode.dev"))),
            expired_promo,
        ];
    ...
    fn first_promo_selection_filters_severity_and_hidden() {
        let list = vec![
            ...
            promo("p-a", "A promo", Some(("Go", "https://ccode.dev"))),
    ...
    fn first_session_announcement_prefers_critical_over_promo() {
        let list = vec![
            promo("p", "upsell", Some(("Go", "https://ccode.dev"))),
            RemoteAnnouncement {
                id: Some("c".into()),
                severity: Some("critical".into()),
    ...
    fn render_promo_row_non_dismissible_shows_configured_caption() {
        let mut ann = promo("p", &"M".repeat(60), Some(("Go", "https://ccode.dev")));
        ...
        let mut bare = promo("p", &"M".repeat(60), Some(("Go", "https://ccode.dev")));
    ...
    fn promo_cta_target_requires_usable_pair() {
        let full = vec![promo("p", "msg", Some(("Go", " https://ccode.dev/promo ")))];
        let (a, url) = promo_cta_target(&full, &no_hidden()).expect("usable target");
        assert_eq!(a.id.as_deref(), Some("p"));
        assert_eq!(url, "https://ccode.dev/promo");

        let mut label_only = promo("p", "msg", None);
        label_only.cta = Some(ccode_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: None,
            caption: None,
        });
    ...
    fn usable_cta_caption_trims_and_never_resurrects_unusable_cta() {
        let mut p = promo("p", "msg", Some(("Go", "https://ccode.dev")));
    ...
    fn promo_cta_returns_label_and_pinned_flag() {
        let mut pinned = promo("p", "msg", Some(("Upgrade Account", "https://ccode.dev/promo")));
        ...
        let dismissible = [promo("d", "msg", Some(("Go", "https://ccode.dev")))];
    ...
    fn promo_cta_target_yields_to_critical_slot_owner() {
        let promo_ann = promo("p", "upsell", Some(("Go", "https://ccode.dev/promo")));
        ...
        assert_eq!(
            ...
            Some((Some("p"), "https://ccode.dev/promo"))
        );
    ...
    fn usable_cta_rejects_unsafe_schemes() {
        ...
        for good in ["https://ccode.dev/promo", "http://ccode.dev/promo"] {
    ...
    fn render_promo_row_button_and_hide_affordances() {
        let mut ann = promo(
            "p",
            "New promo",
            Some(("Get Premium", "https://ccode.dev/ccode")),
        );
        ann.cta.as_mut().unwrap().caption = Some("or use Ctrl+O".into());
        let anns = [ann];
        ...
        let row0 = buf_row(&buf, area, 0);
        assert!(row0.starts_with("[Get Premium]"), "row0={row0:?}");
        assert!(
    ...
    fn render_promo_row_hover_styles() {
        let anns = [promo("p", "msg", Some(("Go", "https://ccode.dev")))];
        let area = Rect::new(0, 0, 80, 1);
        let theme = Theme::current();
    ...
    fn render_promo_row_truncates_button_never_affordances() {
        let anns = [promo(
            ...
            "msg",
            Some((
                "Upgrade to Premium Heavy for the exclusive preview",
                "https://ccode.dev",
            )),
        )];
        let area = Rect::new(0, 0, 50, 1);
    ...
    fn render_promo_row_narrow_width_drops_hide_cta_text() {
        let anns = [promo("p", "msg body", Some(("Go", "https://ccode.dev")))];
