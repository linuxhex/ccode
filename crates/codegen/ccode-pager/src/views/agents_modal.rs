...
use ccode_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use ccode_shell::agent::config::AgentSelectionConfig;
use ccode_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.ccode` vs `{cwd}/.ccode`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `CCODE_AGENT`,
    /// model `agentType`, then `ccode-build`).
...
/// Skips internal variants (CcodeBuildConcise, CcodeBuildPlan,
/// CcodeBuildPlanNoSubagents, CcodeBuildAskUser, Codex, Opencode,
/// CursorExtended, CcodeBuildOrchestrator).
...
impl AgentsModalState {
    ...
    #[cfg(test)]
    fn search_viewport(&self, width: usize) -> ccode_textarea::SingleLineViewport {
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = ccode_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.ccode/personas` and `{cwd}/.ccode/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = ccode_home.join("bundled").join("personas");
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".ccode").join("personas")),
        (ConfigFileScope::User, ccode_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match ccode_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => ccode_config::ccode_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".ccode").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.ccode/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.ccode/{subdir}` or `{cwd}/.ccode/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let ccode_home = ccode_config::ccode_home();
    let in_user = dunce::canonicalize(ccode_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".ccode").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    ccode_shell::config::load_effective_config()
        .ok()
        .and_then(|root| ccode_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in ccode-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    ccode_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        ccode_agent::config::PromptMode::Extend => "extend",
        ccode_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use ccode_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".ccode").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".ccode").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.ccode/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".ccode").join("personas").join("gone.toml");
