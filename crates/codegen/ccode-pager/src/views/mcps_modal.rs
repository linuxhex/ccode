...
/// Display label for a section header, e.g. `"Managed by ccode.dev (3)"`.
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by ccode.dev ({count})"),
        McpSectionId::Plugin(name) => format!("Plugin: {name} ({count})"),
        McpSectionId::Local => format!("Local ({count})"),
    }
}

/// Base ccode.dev connectors URL (no team). Prefer [`managed_connectors_url`] when opening.
pub const MANAGED_SECTION_CONNECTORS_URL: &str = "https://ccode.dev/connectors";
...
///
/// Priority: `ccode_com_` prefix or managed wire source → Managed; else plugin
...
pub fn section_for(server: &McpServerInfo) -> McpSectionId {
    if server.name.starts_with("ccode_com_") || server.wire_source == McpWireSource::Managed {
        McpSectionId::Managed
...
pub fn is_removable(server: &McpServerInfo) -> bool {
    server.wire_source == McpWireSource::Local && !server.name.starts_with("ccode_com_")
}
...
/// Patch a single server row in-place from an `ccode.dev/mcp/server_status`
/// push.
///
...
mod tests {
    ...
    fn section_description_lines_managed_includes_connectors_url() {
        ...
        assert_eq!(lines[1], "[ccode.dev/connectors]");
        assert!(
        ...
        assert_eq!(with_team[1], "[ccode.dev/connectors?teamId=team-1]");
    }

    #[test]
    fn managed_connectors_url_display_strips_scheme() {
        assert_eq!(managed_connectors_url_display(None), "ccode.dev/connectors");
        assert_eq!(
            managed_connectors_url_display(Some("team-uuid-1")),
            "ccode.dev/connectors?teamId=team-uuid-1"
        );
    }
    ...
    #[test]
    fn section_for_ccode_com_with_plugin_label_is_managed() {
    ...
    #[test]
    fn is_removable_rejects_ccode_com_prefix() {
        let server = server_from_wire("ccode_com_slack", Some("local"), None);
        assert!(!is_removable(&server));
    }
    ...
    fn convert_list_response_classifies_managed_gateway_only_for_gateway_rows() {
        ...
        let legacy_managed = server_from_wire("ccode_com_slack", Some("managed"), None);
