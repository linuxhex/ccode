...
pub struct DashboardRow {
    ...
    /// dim text (e.g. `"ccode my-branch-2 worktree"`). `None` when the
...
mod tests {
    ...
    fn idle_local_agent_without_message_has_blank_secondary() {
        let agent = make_idle_agent_with_model(Some("ccode-4.5"));
    ...
    fn subtitle_worktree_shows_label_branch_and_marker() {
        ...
        agent.session.cwd = PathBuf::from("/home/me/.ccode/worktrees/x/location-picker");
    ...
    fn subtitle_non_worktree_shows_branch_and_folder() {
        let mut agent = make_idle_agent_with_model(None);
        agent.session.cwd = PathBuf::from("/home/me/ccode/crates/foo");
