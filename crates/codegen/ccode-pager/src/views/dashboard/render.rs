...
///
/// ```text
///   ◆ Add responsiveness to /context · ccode my-branch-2 worktree       4 mins
...
fn render_row(
    ...
    if title_avail > 0 {
        ...
        // Subtitle: ` · ccode my-branch-2 worktree`.
...
/// **top border** (e.g. `✗ Session no longer exists`, `✓ Theme: Ccode
...
fn render_dispatch(
    ...
    if state.search_mode {
        ...
        let cursor_column = if state.dispatch.text().is_empty() {
            ...
            let viewport = ccode_textarea::EditBuffer::from_parts(
                state.dispatch.text(),
...
mod tests {
    ...
    fn dashboard_visual_preview() {
        ...
        let rows = vec![
            DashboardRow {
                ...
                subtitle: Some("ccode my-branch-2 worktree".to_string()),
            ...
            DashboardRow {
                ...
                subtitle: Some("ccode my-branch-3 worktree".to_string()),
            ...
            DashboardRow {
                ...
                subtitle: Some("ccode main".to_string()),
            ...
            DashboardRow {
                ...
                label: "Add buttons for /models".to_string(),
                subtitle: Some("ccode mybranch worktree".to_string()),
                state: RowState::Working,
    ...
    fn header_location_renders_from_staged_cwd() {
        ...
        state.cwd = std::path::PathBuf::from("/ccode-staged-cwd-marker");
        ...
        let top_row: String = (0..area.width)
            ...
            .collect();
        assert!(
            top_row.contains("/ccode-staged-cwd-marker"),
    ...
    fn feedback_badge_renders_verbatim_in_neutral_color() {
        ...
        state.error_toast = Some(format!("{check} Theme: Ccode Day"));
        let _ = render_dispatch(&mut buf, Rect::new(0, 0, 80, 3), &theme, &mut state, None);

        let top_row: String = (0..80).map(|x| buf[(x, 0)].symbol().to_string()).collect();
        assert!(
            top_row.contains(&format!("{check} Theme: Ccode Day")),
    ...
    fn render_location_picker_shows_worktree_toggle_in_repo() {
        ...
        let repo = std::env::temp_dir().join("ccode-loc-wt-toggle-repo-test");
    ...
    fn render_location_picker_hides_worktree_toggle_outside_repo() {
        ...
        let mut modal = LocationPickerState::new(
            vec![],
            std::path::PathBuf::from("/ccode-not-a-repo-xyz-12345"),
            std::collections::HashMap::new(),
        );
        let mut buf = Buffer::empty(area);
        render_location_picker(&mut buf, area, &theme, &mut modal);
