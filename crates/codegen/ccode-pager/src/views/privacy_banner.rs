...
pub(crate) const PRIVACY_BANNER_LEGAL_URL: &str = "https://ccode.dev/legal";
...
pub(crate) fn render(
    ...
    let title = Span::styled("Help improve Ccode", Style::default().fg(theme.text_primary));
    let desc = "Allow your sessions to improve SpaceCcode's models.";
    // Drop trailing spans whole rather than clipping mid-word when narrow.
    let line1 = if left.width as usize >= "Help improve Ccode  ".len() + desc.len() {
        Line::from(vec![
            title,
            Span::raw("  "),
