...
use ccode_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    pub request: ccode_acp::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `ccode-shell`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell Ccode what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from ccode\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from ccode\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p ccode-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/ccode-pager
