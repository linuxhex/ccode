...
//!  ccode-3 · yolo                             ← info line (optional)
//! ```
//!
...
use ccode_textarea::{ElementId, ElementKind, TextArea, TextAreaState, TextElement};
...
pub struct PromptWidget {
    ...
    hovered_image_element_id: Option<ccode_textarea::ElementId>,
    /// Image shown immediately after insertion without moving the edit cursor.
    post_insert_image_preview: Option<(ccode_textarea::ElementId, usize)>,
        ...
        let applied = if snap.cursor_in_command {
            ...
            if let Some(ref range) = snap.command_range {
                    ...
                    && self.textarea.text().is_char_boundary(range.end)
                {
                    ...
                    // `/mod ccode-4` yields `/model ccode-4`, not `/model  ccode-4`
            ...
            // Replace the args range (e.g., "gr" → "Ccode 4 Fast").
...
impl PromptWidget {
    ...
    fn handle_key_inner(&mut self, key: &KeyEvent) -> PromptEvent {
        ...
        if changed {
            ...
            if is_backspace_key && !old_text.is_empty() {
                use ccode_telemetry::events::BackspaceNoEffect;
    ...
    pub fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> PromptEvent {
        use ccode_textarea::{MouseAction, TextElementEventKind};
    ...
    fn image_element_near_cursor(&self) -> Option<&ccode_textarea::TextElement> {
    ...
    pub fn textarea(&self) -> &ccode_textarea::TextArea {
        &self.textarea
    }
...
fn strip_all_elements(
    text: &str,
    cursor: usize,
    textarea: &ccode_textarea::TextArea,
) -> (String, usize) {
...
fn map_clean_offset_to_raw(
    text: &str,
    clean_offset: usize,
    elements: &[ccode_textarea::TextElement],
) -> usize {
    if elements.is_empty() {
