...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a ccode_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Ccode Build** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | Ccode Build Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Ccode Build Beta**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = ccode_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", ccode_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a ccode_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a ccode_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [ccode_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a ccode_shell::auth::GateInfo>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("ccode.dev");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Ccode Build is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that Ccode Build
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Ccode Build may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &ccode_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("Premium subscription required");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
        ...
        if centered.height > 2 {
            ...
            let gate_link = p
                ...
                .unwrap_or("https://ccode.dev/premium?referrer=ccode-build");
...
fn masked_auth_token_view(input: &str, cursor_byte: usize, width: usize) -> (String, usize) {
    ...
    let buffer =
        ccode_textarea::EditBuffer::from_parts(masked.display.as_str(), masked.cursor_byte);
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use ccode_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = ccode_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    /// Ccode view when scanned foreign rows exist — even when the native list
    ...
    fn hidden_external_hint_renders_outside_chat_mode() {
        ...
        assert!(
            ...
            "default Ccode filter must pin the hidden-external hint:\n{build_mode}"
        );
        assert!(
    ...
    fn grouped_entries_insert_headers() {
        let entries = vec![
            make_entry("s1", "Fix auth", "ccode"),
            make_entry("s2", "Add streaming", "ccode"),
        ...
        // Groups are sorted alphabetically: fw-1 before ccode.
        // Header positions: 0 (fw-1), 2 (ccode)
        ...
        assert!(
            matches!(&result[2], crate::views::picker::PickerEntry::Header { label } if label == &"ccode")
        );
    }
    ...
    fn grouped_entries_rows_are_indented() {
        let entries = vec![make_entry("s1", "Fix auth", "ccode")];
    ...
    fn extract_user_code_parses_verification_url() {
        assert_eq!(
            extract_user_code("https://accounts.ccode.dev/oauth2/device?user_code=ABCD-EFGH"),
            Some("ABCD-EFGH"),
        );
        // Trailing params after the code are ignored.
        assert_eq!(
            extract_user_code("https://ccode.dev/oauth2/device?user_code=WXYZ-1234&foo=bar"),
            Some("WXYZ-1234"),
        );
        // A param whose name merely ends in `user_code` must not be matched.
        assert_eq!(
            extract_user_code("https://ccode.dev/d?foo_user_code=BAD&user_code=GOOD"),
            Some("GOOD"),
        );
        // No code param, empty code, and unexpected characters all yield None.
        assert_eq!(extract_user_code("https://ccode.dev/oauth2/device"), None);
        assert_eq!(extract_user_code("https://ccode.dev/d?user_code="), None);
        assert_eq!(extract_user_code("https://ccode.dev/d?user_code=AB%20CD"), None);
    }
    ...
    fn device_auth_arm_shows_url_and_no_paste_box() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.ccode.dev/oauth2/device?user_code=ABCD-EFGH";
    ...
    fn device_auth_arm_raw_url_mode_shows_full_url() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.ccode.dev/oauth2/device?user_code=WXYZ-1234";
    ...
    fn raw_url_mode_uses_full_width_for_long_urls() {
        ...
        let url = "https://accounts.ccode.dev/oauth2/device?user_code=WXYZ-1234&extra=0123456789";
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.ccode.dev/oauth2/authorize?client_id=ccode";
    ...
    fn long_ann() -> ccode_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = ccode_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
