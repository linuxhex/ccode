...
mod tests {
    ...
    fn format_cwd_plain_repo() {
        assert_eq!(format_cwd_parts("~/ccode", None), "~/ccode");
    }
    ...
    fn format_cwd_worktree_shows_main_repo() {
        assert_eq!(
            format_cwd_parts("~/wt/session-1", Some("~/ccode")),
            "~/wt/session-1 (worktree of ~/ccode)"
        );
    }
    ...
    fn format_cwd_display_shows_subdir_not_repo_root() {
        ...
        assert_eq!(
            format_cwd_display(Path::new("/work/ccode/frontend/apps"), Some(&info)),
            "/work/ccode/frontend/apps",
        );
    }
    ...
    fn format_cwd_display_cache_miss_shows_raw_cwd() {
        assert_eq!(
            format_cwd_display(Path::new("/work/ccode/frontend/apps"), None),
            "/work/ccode/frontend/apps",
        );
    }
}
