...
/// only one normal component (e.g., `/ccode`), returns that component alone.
/// Does not perform tilde expansion — callers provide absolute paths.
/// Returns `"unknown"` for empty input.
///
/// Examples: `/home/user/fw/1` → `"fw-1"`, `/ccode` → `"ccode"`, `/` → `"/"`.
///
...
/// matters because the fast foreign scan can land rows the default Ccode view
...
///
/// Default is [`Self::Ccode`]: native Ccode sessions only (local / remote /
/// conversation), so `/resume` does not mix Claude/Codex/Cursor foreign
/// sessions into the list. `f` cycles Ccode → External → All → Local →
/// Remote — External first so one press from the default reveals foreign
/// sessions.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SourceFilter {
    /// Native Ccode sessions only — excludes Claude/Codex/Cursor foreign rows.
    #[default]
    Ccode,
    Local,
    Remote,
...
impl SourceFilter {
    ...
    pub fn next(self) -> Self {
        match self {
            Self::Ccode => Self::External,
            Self::External => Self::All,
            Self::All => Self::Local,
            Self::Local => Self::Remote,
            Self::Remote => Self::Ccode,
        }
    }
    ...
    ///
    /// ccode.dev conversations carry `source == "conversation"` and live remotely,
    /// so they pass the `Remote` filter (and `Ccode` / `All`) but not `Local`.
    /// Foreign sources (`claude` / `codex` / `cursor`) only pass `External` and
    /// `All`.
    pub fn matches(self, source: &str) -> bool {
        match self {
            Self::Ccode => !crate::app::is_foreign_picker_source(source),
...
pub(crate) fn capture_picker_selection(
    entries: Option<&[SessionPickerEntry]>,
    content_results: Option<&[ccode_shell::extensions::session_search::SearchSessionHit]>,
    state: &PickerState,
    query: &str,
    grouped: bool,
...
pub(crate) fn build_content_entry_data(
    hits: &[ccode_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
/// Hint shown on the default `Ccode` view when the foreign-session scan loaded
/// Claude/Codex/Cursor entries it hides. Ccode-only: `next(Ccode) == External`
...
pub(crate) fn hidden_external_hint(
    ...
    if source_filter != SourceFilter::Ccode {
        return None;
    }
    let hidden = entries?
        .iter()
...
mod tests {
    ...
    fn repo_name_from_cwd_standard_path() {
        assert_eq!(repo_name_from_cwd("/home/user/ccode"), "user-ccode");
    }
    ...
    fn repo_name_from_cwd_single_dir() {
        assert_eq!(repo_name_from_cwd("/ccode"), "ccode");
    }
    ...
    fn source_filter_matches() {
        // Default Ccode filter: native only (not Claude/Codex/Cursor).
        assert!(SourceFilter::Ccode.matches("local"));
        assert!(SourceFilter::Ccode.matches("remote"));
        assert!(SourceFilter::Ccode.matches("both"));
        assert!(SourceFilter::Ccode.matches("conversation"));
        assert!(!SourceFilter::Ccode.matches("claude"));
        assert!(!SourceFilter::Ccode.matches("codex"));
        assert!(!SourceFilter::Ccode.matches("cursor"));
        ...
        // ccode.dev conversations are remote: visible under Ccode + All + Remote, not Local.
    ...
    fn source_filter_cycles() {
        ...
        assert_eq!(SourceFilter::Ccode.next(), SourceFilter::External);
        ...
        assert_eq!(SourceFilter::Remote.next(), SourceFilter::Ccode);
        assert_eq!(SourceFilter::Ccode.label(), "Ccode");
        assert_eq!(SourceFilter::External.label(), "External");
        assert_eq!(SourceFilter::default(), SourceFilter::Ccode);
    }
    ...
    fn source_filter_filters_entries() {
        ...
        let ccode = filter_session_entries(Some(&entries), "", SourceFilter::Ccode);
        assert_eq!(ccode, vec![0, 1, 2]); // local + remote + both, no foreign
    ...
    fn source_filter_empty_and_unknown_source() {
        ...
        // foreign, so it passes Ccode + All but never Local, Remote, or External.
        assert!(SourceFilter::Ccode.matches(""));
        ...
        assert!(SourceFilter::Ccode.matches("unknown"));
    ...
    fn source_filter_is_active() {
        assert!(!SourceFilter::Ccode.is_active());
    ...
    fn hidden_external_hint_visibility() {
        ...
        // Only the default Ccode view surfaces the hint (with the count).
        assert_eq!(
            hidden_external_hint(Some(&entries), SourceFilter::Ccode).as_deref(),
        ...
        assert_eq!(
            hidden_external_hint(Some(&one), SourceFilter::Ccode).as_deref(),
        ...
        assert!(hidden_external_hint(Some(&native), SourceFilter::Ccode).is_none());
        assert!(hidden_external_hint(None, SourceFilter::Ccode).is_none());
    }
