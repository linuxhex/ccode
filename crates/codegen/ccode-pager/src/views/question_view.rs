...
use ccode_acp::AcpResult;
use ccode_markdown::StreamingMarkdownRenderer;
pub use ccode_tools::implementations::ccode_build::ask_user_question::{
...
/// driven by an ACP `ccode.dev/ask_user_question` request).
///
...
pub enum LocalQuestionKind {
    ...
    CreditLimitUpsell {
        choices: Vec<ccode_telemetry::events::CreditLimitChoice>,
    },
    /// Premium upsell modal: the free-usage paywall (429 +
    ...
    /// `id`.
    FreeUsageUpsell {
        /// Telemetry source for `PremiumUpsellClicked` — distinguishes
        /// the paywall from the restricted-command upsell.
        source: ccode_telemetry::events::PremiumUpsell,
    },
    ...
    AgentTypeMismatch {
        ...
        effort: Option<ccode_shell::sampling::types::ReasoningEffort>,
    },
    DoctorFix {
...
///
/// Created when an `ccode.dev/ask_user_question` ext-method request arrives;
...
pub struct QuestionViewState {
    ...
    /// instead of by an ACP `ccode.dev/ask_user_question` request. `None` for
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `ccode.dev/ask_user_question`
...
impl QuestionViewState {
    ...
    /// questions (e.g. the Premium upsell) have no freeform row, so
...
impl QuestionViewState {
    ...
    pub fn build_accepted_response(
        &self,
    ) -> ccode_tools::implementations::ccode_build::ask_user_question::AskUserQuestionExtResponse
    {
    ...
    pub fn send_ext_response(
        &mut self,
        response: ccode_tools::implementations::ccode_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
...
mod tests {
    ...
    /// `no_freeform` questions (e.g. the Premium upsell) have no "Other"
