...
//! `ccode.dev/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `ccode.dev/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `ccode.dev/suggest` response.
...
// (e.g. ccode-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Parsed response from an ACP `ccode.dev/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `ccode.dev/suggest` response.
...
/// Wire `limit` for `ccode.dev/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `CCODE_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `CCODE_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `ccode.dev/suggest` request.
    /// Resolved at construction from `CCODE_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: ccode_config::env_bool("CCODE_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: ccode_config::env_bool("CCODE_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("CCODE_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `ccode.dev/suggest` response arrives, with the text and
