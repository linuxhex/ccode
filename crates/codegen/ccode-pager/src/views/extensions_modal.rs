...
use ccode_tools::implementations::skills::types::SkillInfo;
...
pub fn fuzzy_matches_hook(hook: &ccode_hooks_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
pub(crate) fn source_has_matching_plugin(
    source: &ccode_hooks_types::MarketplaceScanResult,
    query: &str,
) -> bool {
...
pub fn filtered_marketplace_count(
    sources: &[ccode_hooks_types::MarketplaceScanResult],
    query: &str,
) -> usize {
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<ccode_hooks_types::PluginOrigin>,
) -> ccode_hooks_types::PluginInfo {
    ccode_hooks_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: ccode_hooks_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: ccode_hooks_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: ccode_hooks_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    (u8, String, String),
    Vec<(usize, &'a ccode_hooks_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &ccode_hooks_types::PluginInfo) -> PluginGroup {
    use ccode_hooks_types::{PluginOrigin, PluginScope};

    match &plugin.origin {
        Some(PluginOrigin::ProjectCcode) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserCcode) => PluginGroup::new(2, "origin:user", "User"),
...
pub fn resolve_marketplace_index<'a>(
    sources: &'a [ccode_hooks_types::MarketplaceScanResult],
    target: usize,
    query: &str,
...
pub enum MarketplaceHit<'a> {
    ...
    Plugin {
        source_index: usize,
        source: &'a ccode_hooks_types::MarketplaceScanResult,
        plugin: &'a ccode_hooks_types::MarketplacePluginEntry,
    },
}
...
fn build_hook_groups<'a>(
    hooks: &'a [ccode_hooks_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<(&'a str, Vec<usize>)> {
...
pub fn next_visible_hook(
    hooks: &[ccode_hooks_types::HookInfo],
    current: usize,
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> ccode_telemetry::events::ExtensionsModalTab {
        use ccode_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(ccode_hooks_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(ccode_hooks_types::PluginsAction),
    ...
    RefreshMcpList,
    /// Open ccode.dev connectors page (MCP tab: press `o`).
    ...
    MarketplaceAction(ccode_hooks_types::MarketplaceAction),
...
impl ModalInputField {
    ...
    pub(crate) fn viewport(&self, width: usize) -> ccode_textarea::SingleLineViewport {
...
pub enum ConfirmationAction {
    ...
    Hooks(ccode_hooks_types::HooksAction),
    ...
    Plugins(ccode_hooks_types::PluginsAction),
    /// Replay a marketplace action (uninstall plugin or remove source).
    Marketplace(ccode_hooks_types::MarketplaceAction),
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use ccode_hooks_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use ccode_hooks_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            ccode_hooks_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use ccode_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<ccode_hooks_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<ccode_hooks_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<ccode_hooks_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<ccode_hooks_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[ccode_hooks_types::PluginInfo]) {
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let ccode = ccode_config::ccode_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user ccode home (CCODE_HOME-aware)
    // or a project-scoped `{cwd}/.ccode/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User ccode home (CCODE_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(ccode.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.ccode/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".ccode" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    if let Some(name) = plugin_name("plugins").or_else(|| plugin_name("installed-plugins")) {
        return (format!("Plugin: {name}"), false);
    }
    // Global hooks under $CCODE_HOME/hooks
    let global_hooks = ccode.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.ccode/hooks") || source_dir.contains("/.ccode/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&ccode) {
            let prefix = crate::util::display_ccode_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            ccode_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_ccode_home(path) {
                    crate::util::display_user_ccode_path("skills")
            ...
            ccode_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            ccode_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &ccode_hooks_types::PluginInfo) -> Vec<String> {
    use ccode_hooks_types::McpStatus;
...
fn component_categories(
    components: &ccode_hooks_types::PluginComponents,
) -> [(&'static str, &[ccode_hooks_types::ComponentItem]); 6] {
    use ccode_hooks_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &ccode_hooks_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    let mut groups: std::collections::BTreeMap<
                        String,
                        Vec<(usize, &ccode_hooks_types::HookInfo)>,
                    > = std::collections::BTreeMap::new();
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.ccode/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // ccode-home branch is CCODE_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.ccode/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.ccode/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        let servers = vec![
            make_mcp_server_for_rows("ccode_com_linear", McpWireSource::Managed, vec![]),
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by ccode.dev")),
            "managed section header must appear"
        );
        assert!(
            !rows.labels.iter().any(|l| l == "ccode_com_linear"),
    ...
    fn make_skill(
        name: &str,
        desc: &str,
    ) -> ccode_tools::implementations::skills::types::SkillInfo {
        ccode_tools::implementations::skills::types::SkillInfo {
            name: name.to_string(),
            display_name: None,
            ...
            metadata: None,
            path: "test".to_string(),
            scope: ccode_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = ccode_tools::implementations::skills::types::SkillScope::Plugin;
        skill.config_source = Some(ccode_tools::types::config_source::ConfigSource::Plugin {
    ...
    fn make_plugin(name: &str) -> ccode_hooks_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: ccode_hooks_types::PluginOrigin,
    ) -> ccode_hooks_types::PluginInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> ccode_hooks_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(ccode_hooks_types::PluginsListResponse {
            plugins: vec![
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    ccode_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    ccode_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                ccode_hooks_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> ccode_hooks_types::HookInfo {
        ccode_hooks_types::HookInfo {
            name: name.to_string(),
            event: ccode_hooks_types::HookEvent::PreToolUse,
            handler_type: ccode_hooks_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> ccode_hooks_types::MarketplaceScanResult {
        ccode_hooks_types::MarketplaceScanResult {
    ...
    fn local_plugins_source() -> ccode_hooks_types::MarketplaceScanResult {
        ccode_hooks_types::MarketplaceScanResult {
            source_name: "local-plugins".into(),
            source_kind: "local".into(),
            source_url_or_path: "/home/user/.ccode/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    struct TestPlugin {
        ...
        components: Option<ccode_hooks_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> ccode_hooks_types::MarketplacePluginEntry {
            ...
            ccode_hooks_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                ccode_hooks_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                ccode_hooks_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(ccode_hooks_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_has_zero_plugins() {
        let error_source = ccode_hooks_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> ccode_hooks_types::ComponentItem {
        ccode_hooks_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> ccode_hooks_types::PluginComponents {
        ccode_hooks_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(ccode_hooks_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = ccode_hooks_types::PluginComponents {
            skills: (0..12)
    ...
    fn plugins_modal_state(
        plugins: Vec<ccode_hooks_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(ccode_hooks_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_maps_each_origin_variant() {
        use ccode_hooks_types::PluginOrigin;
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectCcode, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserCcode, 2, "origin:user", "User"),
            (
            ...
            (
                PluginOrigin::MarketplaceInstall {
                    source_name: Some("ccode Official".into()),
                    git_url: Some("https://example.com/r.git".into()),
                },
                5,
                "ccode-mp:ccode Official",
                "ccode Official",
            ),
            (
                PluginOrigin::MarketplaceInstall {
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = ccode_hooks_types::PluginScope::Project;
        ...
        cli.scope = ccode_hooks_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = ccode_hooks_types::PluginScope::Config;
        assert_eq!(plugin_group(&config).key, "origin:config");

        let mut mp = make_plugin("mp-tool");
        mp.marketplace_source = Some("ccode Official".into());
        let group = plugin_group(&mp);
        assert_eq!(group.key, "ccode-mp:ccode Official");
        assert_eq!(group.label, "ccode Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            ccode_hooks_types::PluginOrigin::Unknown,
        );
        assert_eq!(plugin_group(&unknown).key, "origin:user");

        unknown.marketplace_source = Some("ccode Official".into());
        assert_eq!(plugin_group(&unknown).key, "ccode-mp:ccode Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserCcode),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use ccode_hooks_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserCcode),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use ccode_hooks_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserCcode);
    ...
    fn plugins_fallback_grouping_without_origin() {
        ...
        assert_eq!(buffer_count(&buf, "User (1 plugin)"), 1);
        assert_eq!(buffer_count(&buf, "ccode Official (1 plugin)"), 1);
    ...
    fn marketplace_placeholders_render_only_when_expanded() {
        ...
        source.plugins[0].components = Some(ccode_hooks_types::PluginComponents::default());
