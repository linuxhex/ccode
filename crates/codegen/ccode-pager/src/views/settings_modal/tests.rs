...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "Ccode Test".to_string(),
            acp::ModelId::new(Arc::from("ccode-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("Ccode Test", 4); // cursor between "Ccode" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_dispatches_preview_revert_for_each_key() {
    let cases: &[(&str, &str)] = &[
        ("theme", "ccode-night"),
        ("auto_dark_theme", "ccode-night"),
        ("auto_light_theme", "ccode-day"),
    ];
...
fn picking_enum_esc_returns_to_browse() {
    let mut s = make_state();
    s.transition_to_picking_enum("theme", 0, SettingValue::Enum("ccode-night"), true);
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"ccode-night\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode(), SettingsModalMode::Browse));
}
...
fn picker_visual_smoke_debug() {
    let entries = vec![SettingMeta {
        ...
        description: "Controls whether SpaceCcode may retain and train on coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-out",
            choices: &[
                EnumChoice {
                    canonical: "opt-in",
                    display: "Opt in",
                    description: "Allow SpaceCcode to retain and use coding session data for training and product improvement.",
                },
                EnumChoice {
                    canonical: "opt-out",
                    display: "Opt out",
...
fn picker_multi_line_choice_hit_rect_spans_all_lines() {
    ...
    let entries = vec![SettingMeta {
        ...
        owner: SettingOwner::Shared,
        label: "Coding data sharing",
        description: "Controls whether SpaceCcode may retain coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-in",
            choices: &[
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
    ...
    assert!(
        ...
        "validation_error must be Some for unknown model 'a' \
         (catalog has 'Ccode 4 Fast' only)",
    );

    // Enter on a buffer that fails the KnownModel validator
    // (catalog has 'Ccode 4 Fast'; "a" doesn't match) is
...
fn string_editor_uses_canonical_edits_policy_and_live_validation() {
    ...
    let mut state = editor_render_fixture("Ccode Tes", "Ccode Tes".len());
    ...
    assert_eq!(state.editing_buffer(), Some("Ccode Test"));
...
//    the "Tip · Ask Ccode…" docs footer and the keybindings hints.
//
...
fn string_editor_paste_sanitizes_validates_and_consumes_rejected_text() {
    let mut state = editor_render_fixture("Ccode Tst", "Ccode T".len());
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask Ccode to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("ccode-day"),
        "width=40 must NOT render LONG path (no `ccode-day`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask Ccode") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // `"ccode-night"`. Tightened from the previous `Action(_) |
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"ccode-night\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `ccode-night`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `ccode-night`; entering the picker
            // captures `original_value = current value = ccode-night`,
...
/// misleading: on dark themes (CcodeNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the CcodeDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::CcodeNight => crate::theme::Theme::ccode_night(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::CcodeDay => crate::theme::Theme::ccode_day(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::ccode_night(),
    };
    assert_ne!(
