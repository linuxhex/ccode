...
use ccode_shell::tools::{TodoItem, TodoStatus};
...
mod tests {
    ...
    fn render_per_model_breakdown_shows_each_model() {
        let mut goal = make_goal();
        goal.live_tokens_by_model = vec![("ccode-4".into(), 12_300), ("ccode-3-mini".into(), 8_000)];
        let text = render_to_text(&goal);
        assert!(text.contains("ccode-4"), "first model must render:\n{text}");
        assert!(
            text.contains("ccode-3-mini"),
