...
/// firehose (`CCODE_DEBUG_LOG` / `CCODE_LOG_FILE`).
pub use ccode_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `ccode-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use ccode_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "ccode_shell=info,ccode_pager=trace,ccode_tools=info,ccode_acp=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = ccode_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = ccode_telemetry::instrumentation::layer();
    let sampling_log_layer = ccode_telemetry::sampling_log::layer();
    let hooks_log_layer = ccode_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    ccode_telemetry::debug_log::install_firehose(registry, "tui");
