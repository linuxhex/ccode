...
/// Shared CCODE_HOME boundary fixture for the resume-by-title startup and
/// pre-sandbox tests.
///
/// `ccode_home()` is OnceLock-cached process-wide, so summaries land under the
/// *resolved* home (possibly the real `~/.ccode` when another test pinned the
...
impl Default for CcodeHomeFixture {
    fn default() -> Self {
        Self::new()
    }
}
