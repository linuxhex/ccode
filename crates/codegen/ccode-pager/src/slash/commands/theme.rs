...
mod tests {
    ...
    fn with_test_env(f: impl FnOnce()) {
        ...
        theme_cache::set(ThemeKind::CcodeNight);
        f();
        system_appearance::clear_mock();
        theme_cache::reset_for_test();
    }
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "ccode-night");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "ccode-night");
                }
                other => panic!("expected Action::SetTheme(\"ccode-night\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] = CcodeNight; next is available[1].
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for CcodeNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "ccode-night", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"ccode-night\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // Default auto config maps Light -> CcodeDay.
            assert_eq!(Theme::current_kind(), ThemeKind::CcodeDay);
        });
    }
    ...
    fn cancel_preview_restores_previous_kind() {
        with_test_env(|| {
            ...
            cmd.cancel_preview("ccode-night");
            assert_eq!(
                Theme::current_kind(),
                ThemeKind::CcodeNight,
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::CcodeDay);
            let cmd = ThemeCommand;
