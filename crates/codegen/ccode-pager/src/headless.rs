//! Headless single-turn mode (`ccode -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_ccode_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use ccode_acp::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use ccode_shell::agent::auth_method::AuthMethodKind;
use ccode_shell::agent::config::Config as AgentConfig;
use ccode_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use ccode_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_ccode_shell;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<ccode_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use ccode_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<ccode_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = ccode_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut ccode_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    ccode_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `ccode login` to authenticate \
         (or `ccode login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         ccode login --device-code\n\n\
         Alternatively, set the CCODE_API_KEY environment variable \
         or run `ccode login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !ccode_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !ccode_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &ccode_tools::types::compat::CompatConfig::default());
            ...
            acp::LoadSessionRequest::new(acp::SessionId::new(sid.to_string()), cwd.to_path_buf())
                ...
                .meta({
                    ...
                    if let Some(true) = restore_code {
                        m.insert("ccode.dev/restore_code".into(), serde_json::Value::Bool(true));
                    }
                    Some(m)
                }),
            acp_tx,
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'ccode models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    ccode_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = ccode_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `ccode workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = ccode_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        ccode_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_ccode_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("CCODE_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = ccode_shell::active_sessions::register(
            ccode_shell::active_sessions::ActiveSession {
    ...
    // Pending background work: bash/monitor via ccode.dev/task_backgrounded +
    // task_completed; background subagents via SubagentSpawned + SubagentFinished
    // on ccode.dev/session_notification (prefixed `subagent:{id}` in pending_bg).
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.ccode can't hang exit.
        let _ = ccode_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = ccode_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    ...
    if method == "ccode.dev/task_completed" {
        #[derive(serde::Deserialize)]
    ...
    if method == "ccode.dev/monitor_event" {
        return ExtEvent::MonitorEvent;
    }

    match method {
        "ccode.dev/session_notification" | "ccode.dev/session/update" => {}
    ...
    let Ok(ccode_notif) = serde_json::from_str::<CcodeNotif>(notif.request.params.get()) else {
        return ExtEvent::None;
    };

    match ccode_notif.update {
        CcodeUpdate::AutoCompactStarted { percentage } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        CcodeUpdate::AutoCompactCompleted {} => match format {
            OutputFormat::StreamingJson => {
        ...
        CcodeUpdate::AutoCompactFailed { error } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        CcodeUpdate::AutoCompactCancelled {} => match format {
            OutputFormat::StreamingJson => {
        ...
        CcodeUpdate::AutoContinueCompleted { total_tokens } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        CcodeUpdate::ImageCompressed { message } => match format {
            OutputFormat::StreamingJson => {
                println!(
                    "{}",
        ...
        CcodeUpdate::SubagentSpawned { subagent_id } => {
            return ExtEvent::SubagentSpawned { subagent_id };
        }
        CcodeUpdate::SubagentFinished { subagent_id, .. } => {
...
mod tests {
    ...
    fn reap_request_for_task_kills_with_session_scope() {
        ...
        assert_eq!(request.method.as_ref(), "ccode.dev/task/kill");
    ...
    fn reap_request_for_subagent_cancels_with_stripped_id() {
        ...
        assert_eq!(request.method.as_ref(), "ccode.dev/subagent/cancel");
        let params: serde_json::Value = serde_json::from_str(request.params.get()).unwrap();
        assert_eq!(params["subagentId"], "sub-7");
    }

    use super::*;
    use ccode_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            ccode_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> ccode_acp::AcpArgsBox<acp::ExtNotification> {
    ...
    fn headless_task_backgrounded_parses_task_id() {
        ...
        // the inner update object (matching the real `ccode.dev/task_backgrounded`
        // wire shape: `{ "update": { "sessionUpdate": ..., "task_id": ... } }`).
        let notif = make_ext_notif(
            "ccode.dev/task_backgrounded",
            serde_json::json!({
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let raw = serde_json::value::to_raw_value(&payload).unwrap();
        let (tx, _rx) = tokio::sync::oneshot::channel();
        let notif = ccode_acp::AcpArgs {
            request: acp::ExtNotification::new("ccode.dev/other", raw.into()),
            response_tx: tx,
        }
        .boxed();
        assert!(matches!(
