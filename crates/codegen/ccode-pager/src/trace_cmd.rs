...
use ccode_shell::session::repo_changes::UploadMethod;
...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $CCODE_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            ccode_version: env!("VERSION_WITH_COMMIT").to_owned(),
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    ccode_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
            "Session '{session_id}' not found under {}",
            crate::util::display_user_ccode_path("sessions")
        )
    })
}

pub fn trace_exports_dir() -> PathBuf {
    ccode_home().join("trace-exports")
}
...
async fn run_upload(
    ...
    let upload_method = match upload_method {
        ...
        None => {
            ...
            anyhow::bail!(
                "No upload credentials. Run `ccode login` or set a deployment key. \
                 See {} for upload overrides.",
                crate::util::display_user_ccode_path("docs/user-guide")
            );
        }
    };
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            ccode_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
            "No trace upload bucket configured. Set `CCODE_TELEMETRY_GCS_BUCKET`, \
             `CCODE_TRACE_UPLOAD_BUCKET`, or `endpoints.trace_upload_bucket` in \
    ...
    let upload_config = ccode_shell::session::repo_changes::TraceExportConfig {
...
impl UploadAttempt<'_> {
    ...
    pub fn handle_failure(&self, error: &anyhow::Error) -> anyhow::Error {
        ...
        if self.json {
            ...
            eprintln!("  Retry:  ccode trace {}", self.session_id);
    ...
    fn write_debug_log(&self, error: &anyhow::Error, output_dir: &Path) -> PathBuf {
        ...
        let _ = writeln!(log, "Ccode version: {}", env!("VERSION_WITH_COMMIT"));
        let _ = writeln!(
            log,
            "OS:           {} {}",
            std::env::consts::OS,
...
async fn upload_with_retries(
    config: &ccode_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
    ...
    (|| async {
        tokio::time::timeout(
            UPLOAD_TIMEOUT,
            ccode_fs::gcs::upload_bytes(config, object_path, archive, "application/gzip"),
        )
        .await
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = ccode_shell::auth::ensure_authenticated_or_noninteractive(
