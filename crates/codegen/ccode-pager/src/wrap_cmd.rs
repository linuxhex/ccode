//! `ccode wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `ccode ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`ccode wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("ccode wrap: no command given"))?;
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = ccode_config::shell::is_command_available(program);
        (
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("ccode wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `ccode wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`ccode wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`ccode wrap "$PROG" ...` with `$PROG` unset)
...
/// deliberately not `ccode_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `ccode ssh`, `ccode wrap` does not gate on the terminal brand: the user
