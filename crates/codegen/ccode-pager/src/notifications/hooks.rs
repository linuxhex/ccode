...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("CCODE_EVENT", event_str)
        .env("CCODE_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("CCODE_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'CCODE_EVENT=%s\\nCCODE_MESSAGE=%s\\nCCODE_SESSION_ID=%s\\n' \
             \"$CCODE_EVENT\" \"$CCODE_MESSAGE\" \"$CCODE_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("CCODE_EVENT=Turn complete"),
            "missing CCODE_EVENT: {content}"
        );
        assert!(
            content.contains("CCODE_MESSAGE=hello world"),
            "missing CCODE_MESSAGE: {content}"
        );
        assert!(
            content.contains("CCODE_SESSION_ID=sess-42"),
            "missing CCODE_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("CCODE_SESSION_ID"),
            "CCODE_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("CCODE_EVENT=Turn complete"));
        assert!(content.contains("CCODE_MESSAGE=test body payload"));
        assert!(content.contains("CCODE_SESSION_ID=test-session-123"));
    }
}
