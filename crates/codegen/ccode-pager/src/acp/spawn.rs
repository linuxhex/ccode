...
//!
//! Simplified to only support CcodeShell (in-process) mode.
...
/// Spawn a CcodeShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_ccode_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<ccode_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    let (agent_config, models_manager) =
        ccode_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
            ...
            local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        ccode_agent::prompt::workspace_user::optional_workspace_user_dir();
                    ccode_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    ccode_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    ccode_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
