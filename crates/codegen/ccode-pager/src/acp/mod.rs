...
use ccode_acp::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a ccode-shell instance.
    pub is_ccode_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<ccode_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `ccode.dev`).
    ...
    /// disabled feature produces zero `ccode.dev/recap` traffic. Defaults to `false`
    ...
    pub auth_manager: std::sync::Arc<ccode_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<ccode_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_ccode_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(ccode_shell::auth::AuthManager::new(
        ...
        agent_config.ccode_com_config.clone(),
    ));

    // Leader has no in-process agent; init this process's product telemetry client.
    ccode_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = ccode_shell::util::ccode_home::ccode_home().join("config.toml");
...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    serde_json::json!({
        "ccode.dev/incrementalBashOutput": true,
        "ccode.dev/hunkTracker": { "mode": hunk_mode },
        "ccode.dev/bashOutputNoColor": true,
        "ccode.dev/gitHeadChanged": true,
    })
}
...
async fn initialize(
    ...
    // Check if this is a ccode-shell agent
    let is_ccode_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("ccodeShell"))
...
///
/// Matches TUI startup behavior: if the first method is `ccode.dev`, defer auth
...
/// for a `ccode.dev` or `oidc` method — these are the ones that can trigger
...
/// When `ccode.api_key` was advertised, non-interactive credentials were
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "ccodeShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "ccodeShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_ccode_com_no_provider_needs_login_pending() {
        let methods = vec![make_auth_method("ccode.dev", "ccode.dev", None)];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("ccode.dev"));
        assert_eq!(method_id.as_ref().unwrap().0.as_ref(), "ccode.dev");
        assert_eq!(mode, AuthStartMode::Pending);
    }

    #[test]
    fn startup_auth_ccode_com_with_external_provider_command() {
        let meta = serde_json::json!({ "external_provider": true });
        let methods = vec![make_auth_method("ccode.dev", "Acme Corp", Some(meta))];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
    ...
    #[test]
    fn startup_auth_non_ccode_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.ccode/config.toml` with a
    ...
    /// change re-orders `build_auth_methods()` to put `ccode.api_key` anywhere
    ...
    /// (`agent::auth_method::tests::enterprise_byok_first_method_is_ccode_api_key`
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use ccode_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};

        let built = build_auth_methods(AuthMethodsBuildInputs {
            // enterprise-style: model has `env_key` set and the env var resolves,
            // so the shell-side predicate returns true.
            has_external_api_key: true,
            // Realistic enterprise user: no cached session token, default `ccode.dev`
        ...
        assert!(
            ...
            "shell built auth_methods for a BYOK user, but the pager still \
             ...
             ccode.api_key first or the pager stopped treating ccode.api_key as \
             a no-login method.",
        );
        assert!(label.is_none());
        assert!(method_id.is_none());
    ...
    /// Inverse direction: when `ccode.api_key` is NOT in the list, the pager
    /// MUST show the login screen. We assert this with `ccode.api_key` present
    ...
    /// `startup_auth_ccode_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_ccode_api_key_not_first_still_requires_login() {
        use ccode_shell::agent::auth_method::{CCODE_COM_METHOD_ID, CCODE_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(CCODE_COM_METHOD_ID, "Ccode", None),
            make_auth_method(CCODE_API_KEY_METHOD_ID, "ccode.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with ccode.dev first, the pager must require login -- pinning \
             the BAD-ordering failure mode (ccode.api_key not first)",
        );
    }

    #[test]
    fn startup_auth_method_id_is_copied_not_synthesized() {
        let methods = vec![make_auth_method("ccode.dev", "My Login", None)];
    ...
    fn startup_auth_external_provider_false_is_pending() {
        ...
        let methods = vec![make_auth_method("ccode.dev", "ccode.dev", Some(meta))];
    ...
    fn client_capabilities_meta_defaults_absent_or_blank_mode_to_agent_only() {
        ...
        assert_eq!(absent["ccode.dev/hunkTracker"]["mode"], "agent_only");
        let blank = client_capabilities_meta(&ConnectFlags {
            hunk_tracker_mode: Some("   ".into()),
            ..Default::default()
        });
        assert_eq!(blank["ccode.dev/hunkTracker"]["mode"], "agent_only");
    }
    ...
    fn client_capabilities_meta_canonicalizes_off_and_mixed_case() {
        ...
        for raw in ["off", "OFF", "Disabled"] {
            ...
            assert_eq!(meta["ccode.dev/hunkTracker"]["mode"], "off", "raw={raw}");
        }
    }
}
