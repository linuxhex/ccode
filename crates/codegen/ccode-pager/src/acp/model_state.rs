...
impl ModelState {
    ...
    /// Machine-readable model ID string for the current model (e.g. "ccode-4.5").
    ...
    /// correct today (all current Ccode models accept images, so nothing is
    ...
    /// not offer (e.g. `none` on ccode-4.5) are rejected so we fail in the TUI
...
mod tests {
    ...
    fn update_catalog_preserves_user_effort_when_model_unchanged() {
        let id = acp::ModelId::new(Arc::from("ccode-build"));
        let mut state = ModelState::default();
        state.available.insert(
            id.clone(),
            model_with_effort("ccode-build", "Ccode Build", "high"),
        );
