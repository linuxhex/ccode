...
use ccode_shell::auth::{AuthManager, try_ensure_fresh_auth};
use ccode_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // deployments that only configure a deployment_key + custom ccode_api_base_url.
    // If the user has previously run the interactive `ccode` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.ccode_com_config).await;
    ...
    let client = ccode_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = ccode_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                None,
                limit,
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use ccode_shell::session::merge::REMOTE_TIMEOUT;
            use ccode_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = ccode_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
