...
/// Inclusive bounds for motion cadence env knobs (`CCODE_MIN_DRAW_MS`,
/// `CCODE_SCROLL_CADENCE_MS`).
...
pub fn start(
    ...
    let (min_draw_env_set, min_draw_env_ms) =
        cadence_ms_from_env("CCODE_MIN_DRAW_MS", default_cadence_ms);
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("CCODE_SCROLL_CADENCE_MS", default_cadence_ms);
...
fn spawn_terminal_and_display_refresh_telemetry(tel: StartupTel) {
    tokio::task::spawn_blocking(move || {
        ...
        ccode_telemetry::session_ctx::log_event(t.clone());
        ...
        ccode_telemetry::session_ctx::log_event(
            ccode_telemetry::events::DisplayRefreshProbe {
                terminal: t,
