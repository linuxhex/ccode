...
struct AgentLoadOutcome {
    ...
    success: bool,
    /// `ccode.dev/runningPromptId` from the reload response: the turn another
...
/// [`decide`](ccode_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("CCODE_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = ccode_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(ccode_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = ccode_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match ccode_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.privacy_notice_rollout = ccode_config::env_bool("CCODE_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
        ...
        .unwrap_or(false);
    app.privacy_banner_reshow_days = std::env::var("CCODE_PRIVACY_BANNER_RESHOW_DAYS")
        .ok()
    ...
    app.privacy_banner_acked = ccode_shell::config::load_from_disk()
        .ok()
        .and_then(|root| {
            ccode_shell::util::config::load_config_from_toml(&root)
                .privacy
                .privacy_banner_acked
        });
    app.plugin_cta_enabled = ccode_config::env_bool("CCODE_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("CCODE_SESSION_PICKER_GROUPED")
        .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the ccode.dev method from the advertised list
            let ccode_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "ccode.dev");
            if let Some(method) = ccode_com {
                ...
                // No ccode.dev method available, use the first method as fallback
        ...
        // fail-closed: do not invent ccode.dev / auto-start OIDC.
        tracing::info!(
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    ccode_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<ccode_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == ccode_shell::agent::auth_method::CCODE_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = ccode_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = ccode_shell::config::load_merged_requirements();
    let user_config = ccode_shell::config::load_from_disk().ok();
    let managed_config = ccode_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match ccode_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = ccode_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > CCODE_CCODE_API_BASE_URL env > default).
            let endpoints_base =
                ccode_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .ccode_api_base_url;
            app.voice_config =
                ccode_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to ccode-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = ccode_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        ccode_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        ccode_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        ccode_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
...

    {
        ...
        app.active_announcements = ccode_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let ccode_home = ccode_tools::util::ccode_home::ccode_home();
            app.tip = ccode_shell::util::tips::pick_and_advance(&app.tips, &ccode_home);
        }
    ...
    let hints = ccode_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.ccode/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        ccode_shell::util::config::clamped_display_permission_mode(
            ccode_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        ccode_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = ccode_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = ccode_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // CCODE_SCROLL_SPEED/_MODE/_LINES + CCODE_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<ccode_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`ccode "fix the bug"`). When
    ...
    // `ccode dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `CCODE_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("CCODE_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(ccode_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice could not start. Restart Ccode.");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.ccode/config.toml [ui]`
                // rather than `~/.ccode/pager.toml`. The watcher only knows
...
pub(crate) fn load_initial_ui_config() -> ccode_shell::agent::config::UiConfig {
    use ccode_shell::agent::config::UiConfig;
    let Ok(root) = ccode_shell::config::load_effective_config() else {
