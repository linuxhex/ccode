...
use ccode_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, ccode_home: &Path) -> [PathBuf; 2] {
        ...
        [
            ccode_home
                ...
                .join(skill)
                .join("SKILL.md"),
            ccode_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(ccode_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, ccode_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, ccode_home, metadata_exists).await;
...
pub(crate) fn scan_effect(
    ...
    (compat.claude || compat.codex || compat.cursor).then(|| Effect::ScanForeignSessions {
        cwd: cwd.to_path_buf(),
        compat,
        ccode_home: ccode_home.to_path_buf(),
        coordinator,
        seq,
    })
}
...
mod tests {
    ...
    use ccode_workspace::foreign_sessions::ForeignSessionSource;
    ...
    async fn async_gate_supports_bundled_and_user_skill_locations() {
        let enabled = gated_sources_async_with(compat_all(), Path::new("/ccode"), |path| {
