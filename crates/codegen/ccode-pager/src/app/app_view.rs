...
/// [`ccode_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<ccode_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: ccode_shell::util::config::WorktreeHintMode) -> Self {
        use ccode_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        ccode_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            ccode_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`ccode_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `CCODE_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `CCODE_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("CCODE_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`ccode_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => ccode_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: ccode_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`ccode_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.ccode/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`CCODE_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `CCODE_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<ccode_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<ccode_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `CCODE_PLUGIN_CTA`
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `ccode dashboard`, the
    ...
    /// `ccode.dev/sessions/list` polls and `ccode.dev/sessions/changed` broadcasts.
    ...
    /// poll outside leader mode, so we fetch the same `ccode.dev/session/list` the
    ...
    /// Reconciled from `ccode.dev/queue/changed` broadcasts so
    ...
    /// confirming `ccode.dev/queue/changed` broadcast has not yet arrived. Keyed by
    ...
    /// non-selectable headers. Gated by `CCODE_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `CCODE_CANCEL_REWIND` env /
    ...
    /// When false, the pager must not request recaps (zero `ccode.dev/recap` traffic).
    ...
    /// empty and inert outside `--minimal`; the `ccode-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<ccode_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        ccode_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `ccode_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Stamps `_meta["ccode.dev/session"].kind = "chat"` and omits Build agent
    ...
    /// at startup and on settings toggles from `CCODE_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: ccode_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<ccode_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `ccode wrap` tip: set after the first
    ...
    /// Login button label from `AuthMethod.name` (e.g., "ccode.dev", "Acme Corp").
    ...
    /// `ccode_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, ccode_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `ccode_build_access_gate`. `Some` = blocked.
    pub gate: Option<ccode_shell::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "Premium", "Free").
    ...
    pub pending_gate_verification: Option<ccode_shell::auth::GateInfo>,
    ...
    /// `~/.ccode/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `CCODE_VOICE_MODE=0` — the STT pipeline is not started and
    ...
    pub voice_config: ccode_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `CCODE_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<ccode_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture — toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<ccode_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &ccode_shell::util::config::RemoteSettings,
    ) -> Option<ccode_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(ccode_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &ccode_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            ccode_telemetry::session_ctx::log_event(
                ccode_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: ccode_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: ccode_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: ccode_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<ccode_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && ccode_voice::AUDIO_SUPPORTED
    }
    ...
    /// `ccode.dev/settings/update` handler when the subscription tier changes, so
    ...
    fn voice_send(&self, cmd: ccode_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(ccode_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(ccode_voice::VoiceCommand::PttRelease);
    ...
    /// `ccode.dev/queue/changed` broadcast. The broadcast is
    ...
    /// `ccode.dev/queue/changed` broadcast replaces it (matched by `id`) rather than
    ...
    /// Rows the dev `CCODE_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<ccode_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    ccode_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = ctx.gate_url_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::OpenPremiumUrl);
                }
                if let Some(rect) = ctx.upgrade_cta_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        ccode_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
fn dispatch_access_gate_menu_action(index: usize) -> InputOutcome {
    match index {
        0 => InputOutcome::Action(Action::OpenPremiumUrl),
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                ccode_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            ccode_telemetry::session_ctx::log_event(
                                ccode_telemetry::events::PremiumUpsellShown {
                                    source:
                                        ccode_telemetry::events::PremiumUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`ccode_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use ccode_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = ccode_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                ccode_telemetry::session_ctx::log_event(
                    ccode_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: ccode_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `ccode wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::ContextualTip {
                tip: ccode_telemetry::events::ContextualTipKind::ImageInput,
                action: ccode_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = ccode_ratatui::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    /// including after the fast foreign scan lands rows the default Ccode
    ...
    fn tick_demand_fast_while_modal_session_picker_loads() {
        ...
        assert_eq!(
            app.tick_demand(),
            TickDemand::Fast,
            "foreign rows hidden by the Ccode filter must not end the loading spinner"
        );
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![ccode_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![ccode_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: ccode_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_disables_billing_surface_for_team_users() {
        ...
        let meta = ccode_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_enables_billing_surface_for_personal_users() {
        ...
        let meta = ccode_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_restores_billing_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&ccode_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&ccode_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_lifts_restrictions_for_paid_tiers_and_teams() {
        ...
        app.apply_auth_meta(&ccode_shell::auth::AuthMeta::default());
        assert!(!app.tier_restricted_commands.is_empty());
        app.subscription_tier = Some("Premium".into());
    ...
    fn is_restricted_tier_classification() {
        ...
        assert!(!is_restricted_tier(Some("Premium")));
        assert!(!is_restricted_tier(Some("Premium Heavy")));
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(ccode_shell::auth::GateInfo {
            message: "Subscribe to use Ccode Build".into(),
            url: Some("https://ccode.dev/premium?referrer=ccode-build".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = ccode_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(ccode_workspace::foreign_sessions::RecentForeignSession {
                tool: ccode_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn esc_owned_before_agent_covers_app_level_owners() {
        ...
        app.import_claude_modal = Some(
            crate::views::import_claude_modal::ImportClaudeModalState::new(
                ccode_shell::claude_import::ImportPlan::default(),
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![ccode_workspace::file_system::FuzzyMatchResult {
    ...
    fn welcome_picker_f_cycle_disabled_under_chat_mode() {
        ...
        assert_eq!(
            ...
            crate::views::session_picker::SourceFilter::Ccode,
