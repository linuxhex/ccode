...
fn format_cost(m: &ccode_shell::extensions::notification::PromptUsageModel) -> String {
    use ccode_shell::extensions::notification::ticks_to_usd;
...
mod tests {
    use super::*;
    use ccode_shell::extensions::notification::{PromptUsage, PromptUsageModel};
    ...
    fn session_usage_block_lists_models_when_multiple() {
        ...
        usage
            .model_usage
            .insert("ccode-build".into(), model_row(100, 10, None));
        usage
            .model_usage
            .insert("ccode-4".into(), model_row(50, 5, None));
        let text = session_usage_block_text(&usage);
        assert!(text.contains("By model:"), "{text}");
        assert!(text.contains("ccode-build — 100 in / 10 out"), "{text}");
        assert!(text.contains("ccode-4 — 50 in / 5 out"), "{text}");
    }
