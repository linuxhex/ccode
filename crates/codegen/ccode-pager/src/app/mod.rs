...
pub use ccode_prompt as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `CCODE_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    ccode_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn voice_mode_config_value() -> Option<bool> {
    ccode_shell::config::load_effective_config()
        .ok()
        .and_then(|cfg| {
            cfg.get("features")
                .and_then(|f| f.get("voice_mode"))
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use ccode_shell::agent::config::{BoolFlag, ConfigSource};
    let resolved = BoolFlag::env("CCODE_VOICE_MODE")
...
///
/// Uses [`ccode_ratatui::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `ccode-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`ccode-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&ccode_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<ccode_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<ccode_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
/// should print a message telling the user to relaunch `ccode`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<ccode_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    ccode_tty::redirect_native_stderr();
    ...
    let ccode_com_config = match ccode_shell::agent::config::Config::new_from_toml_cfg(
        &raw_config,
    ) {
        Ok(c) => c.ccode_com_config,
        Err(e) => {
            tracing::warn!(error = %e, "failed to parse config for auth refresh, using defaults");
            ccode_shell::auth::CcodeComConfig::default()
        }
    };
    let refreshed_auth = ccode_shell::auth::try_ensure_fresh_auth(&ccode_com_config).await;
    let early_prefetch =
        ccode_shell::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    ccode_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(ccode_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = ccode_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("CCODE_HUNK_TRACKER").ok();
    ...
    let launch_auto = ccode_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    cancel.cancel();
    ccode_tty::global_process_scope().kill_all();
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  ccode --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  ccode --resume {}", info.session_id);
    }
}
...
fn disable_mouse_paste_raw() {
    ccode_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(ccode_crash::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    ccode_crash::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        ccode_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(ccode_crash::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                ccode_ratatui::Terminal::new(backend)?,
            ...
            if let Ok(term) = ccode_ratatui::Terminal::with_options(
                probe_backend,
            ...
            let term = ccode_ratatui::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        ccode_crash::disable_terminal_escape_restore();
    })
}
...
/// BEL/ESC (titles can arrive from ccode.dev conversation metadata) would
...
mod tests {
    ...
    fn restore_runs_teardown_even_when_writer_failed() {
        ...
        let terminal = ccode_ratatui::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - ccode"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "ccode");
        assert_eq!(terminal_title_string(""), "ccode");
        assert_eq!(terminal_title_string("My chat"), "My chat - ccode");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["ccode-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["ccode-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["ccode-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["ccode-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["ccode-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["ccode-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = ccode_shell::util::config::RemoteSettings {
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["ccode-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["ccode-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["ccode-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["ccode-pager", "--chat"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["ccode-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["ccode-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["ccode-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["ccode-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["ccode-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["ccode-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["ccode-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["ccode-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["ccode-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["ccode-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args =
            try_parse_pager(&["ccode-pager", "-s", "a", "--resume", "b", "--fork-session"]).unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["ccode-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_ccode() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "ccode");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "Ccode Build TUI",
                "",
                "Usage: ccode [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["ccode-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["ccode-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  ccode --resume sess-abc\n"));
    }
    #[test]
