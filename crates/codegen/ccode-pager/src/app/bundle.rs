...
//!
//! Pager-side cache of what `ccode-shell` reports from
//! `ccode.dev/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `ccode.dev/bundle/status` ACP responses.
...
/// Deserialized response from `ccode.dev/bundle/status`.
...
/// Deserialized response from `ccode.dev/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
