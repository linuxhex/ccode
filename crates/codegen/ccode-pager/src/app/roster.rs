...
//!
//! - Request/response `ccode.dev/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `ccode.dev/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `ccode.dev/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `ccode.dev/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `ccode.dev/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `ccode-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `ccode.dev/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `ccode.dev/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> ccode_shell::agent::roster::RosterEntry {
        use ccode_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `ccode.dev/sessions/list`.
        ...
        assert_eq!(e.model_id.as_deref(), Some("ccode-4"));
        assert!(e.yolo);
    ...
    /// `ccode.dev/sessions/changed` round-trip: serialize the agent's `RosterChanged`
