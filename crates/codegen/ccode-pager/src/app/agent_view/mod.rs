...
impl McpInitProgress {
    ...
    ///   `ccode.dev/mcp_initialized` clears the progress.
...
pub struct BannerSlotParams<'a> {
    ...
    pub(crate) announcements: &'a [ccode_announcements::RemoteAnnouncement],
...
pub(crate) struct SessionReload {
    ...
    /// Live dedup highwaters (ACP + ccode) as of window open (same restore
    /// rationale).
    last_applied_event_seq: Option<u64>,
    last_applied_ccode_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<ccode_hooks_types::MarketplacePluginEntry>,
...
/// (`ccode.dev/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    ///
    /// ACP stream only — the ccode stream keeps its own highwater
    /// ([`Self::last_applied_ccode_event_seq`]) because the two streams are not
    /// delivered in one id order: ACP lines ride the agent's FIFO event
    /// pipeline while ccode lines are emitted direct-to-gateway, so a fresh ccode
    ...
    /// ccode-stream sibling of [`Self::last_applied_event_seq`] (see there for
    /// why the highwaters are split). Same drop rule, replay-exempt.
    pub last_applied_ccode_event_seq: Option<u64>,
    /// Raw `eventId` of the most recent update APPLIED to this root session —
    /// replay or live, on both the ACP and ccode paths; dropped updates (dedup,
    ...
    pub context_state: Option<ccode_shell::session::ContextInfo>,
    ...
    /// (optimistic echo). The shell broadcasts `ccode.dev/session/interjection` to
    ...
    pub(crate) deferred_session_mode: Option<ccode_tools::types::SessionMode>,
    ...
    /// MCP servers, cleared when `ccode.dev/mcp_initialized` arrives.
    ...
    /// Running agent definition for this session (`ccode.dev/session/info` `agentName`).
    ...
    /// Armed when an `ccode.dev/session/prompt_complete` broadcast arrives for the
    ...
    /// `ccode.dev/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `ccode.dev/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `ccode.dev/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`ccode.dev/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `ccode.dev/follow_ups` that arrived for a turn that is NOT yet the
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(ccode_telemetry::events::CreditLimitChoice::PayAsYouGo);
            ccode_telemetry::session_ctx::log_event(
                ccode_telemetry::events::CreditLimitUpsellClicked {
                    surface: ccode_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
                    choice,
                },
            );
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            ccode_telemetry::session_ctx::log_event(
                ccode_telemetry::events::PremiumUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use ccode_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        ccode_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = ccode_tools::util::ccode_home::ccode_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &ccode_tools::implementations::ccode_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(crate) mod test_fixtures {
    ...
    /// FIX 4 (b): after adopting a NEW turn, a buffer-replayed `ccode.dev/follow_ups`
    ...
    /// FINDING B (stamped path): a LATE FIRST-TIME (never-seen) `ccode.dev/follow_ups`
    ...
    /// upstream by the `_meta["ccode.dev/replayed"]` gate); it is pinned here so the
    ...
    /// FIX (buffer-before-adoption): a stamped `ccode.dev/follow_ups` for a turn that
