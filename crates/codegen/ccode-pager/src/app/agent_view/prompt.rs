...
impl AgentView {
    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    ccode_telemetry::session_ctx::log_event(
                        ccode_telemetry::events::PromptSuggestion {
                            action: ccode_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                ccode_telemetry::session_ctx::log_event(
                    ccode_telemetry::events::PromptSuggestion {
                        action: ccode_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
    ...
    /// The pty-only `CCODE_ESC_DOUBLE_PRESS_MS` override can exceed this; no
...
mod prompt_suggestion_key_tests {
    ...
    /// right after a turn ends with `ccode.dev/suggestPrompt` resolved. Pins the
