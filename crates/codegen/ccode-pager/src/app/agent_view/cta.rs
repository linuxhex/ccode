...
impl AgentView {
    ...
    pub(crate) fn log_prompt_suggestion_shown_if_visible(&mut self) {
        ...
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::PromptSuggestion {
            action: ccode_telemetry::events::PromptSuggestionAction::Shown,
            chars,
            words,
        });
    }
    ...
    /// Apply an `ccode.dev/follow_ups` notification, keyed by `response_id`
    ...
    /// on each `ccode.dev/follow_ups` notification (the same `promptId` it stamps on
    ...
    /// - A buffer-replayed `ccode.dev/follow_ups` for a PRIOR turn's `response_id`
    ...
    /// Buffer a stamped `ccode.dev/follow_ups` for a turn that is not yet current,
    ...
    /// Flush a buffered `ccode.dev/follow_ups` for `prompt_id` (a turn that has just
    ...
    /// the running turn's `ccode.dev/follow_ups` arrive on the ext channel DURING
    ...
    pub(in crate::app) fn connect_matched_plugin(&mut self) {
        ...
        ccode_telemetry::session_ctx::log_event(
            ccode_telemetry::events::PluginCtaConnectClicked {
        ...
        self.pending_effects
            .push(super::actions::Effect::InstallPluginFromCta {
                agent_id: self.session.id,
                session_id,
                source_url_or_path: ccode_marketplace::OFFICIAL_SOURCE_GIT_URL
                    .to_string(),
                plugin_relative_path,
            });
    }
...
mod plugin_cta_notify_tests {
    ...
    fn cta_entry(name: &str) -> ccode_hooks_types::MarketplacePluginEntry {
