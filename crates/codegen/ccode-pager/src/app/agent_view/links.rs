...
impl AgentView {
    ...
    pub(super) fn push_promo_cta_link_span(
        &self,
        link_spans_out: &mut Vec<ccode_ratatui::LinkSpan>,
        banner_announcements: &[ccode_announcements::RemoteAnnouncement],
    ...
    fn push_cta_link_span(
        ...
        {
            link_spans_out.push(ccode_ratatui::LinkSpan {
                row: rect.y,
                col_start: rect.x,
...
mod link_click_tests {
    ...
    fn open_prompt_dropdown_suppresses_announcement_hide_click_target() {
        ...
        let critical = [ccode_announcements::RemoteAnnouncement {
    ...
    fn privacy_banner_owns_slot_and_clicks_dispatch() {
        ...
        let text: String = (0..buf.area.height)
            ...
            .collect();
        assert!(text.contains("Help improve Ccode"), "banner copy painted");
        assert!(
    ...
    fn header_upgrade_cta_rect_and_ctrl_o_override() {
        ...
        let reg = ActionRegistry::defaults();
        let cta = || {
            Some(ccode_announcements::AnnouncementCta {
                label: Some("Upgrade Account".into()),
                url: Some("https://ccode.dev/promo".into()),
                caption: None,
            })
        ...
        let mut agent = make_agent();
        agent.last_terminal_size = (120, 30);
        let pinned = [ccode_announcements::RemoteAnnouncement {
            id: Some("promo-pin".into()),
            severity: Some("promo".into()),
            message: Some("ZZPROMO".into()),
            dismissible: Some(false),
        ...
        let mut agent = make_agent();
        agent.last_terminal_size = (120, 30);
        let dismissible = [ccode_announcements::RemoteAnnouncement {
    ...
    fn frame_occluder_over_banner_swallows_clicks_and_drops_cta_link_span() {
        ...
        assert_eq!(&*spans[0].url, "https://ccode.dev/promo");
        let outcome = agent.handle_input(&Event::Mouse(mouse_down(cta.x + 1, cta.y)), &reg);
        assert!(
            matches!(
                outcome,
