...
impl AgentView {
    ...
    pub(in crate::app) fn force_interject_queue_row(&mut self, id: u64) -> InputOutcome {
        ...
        if is_server {
            // Server row: the agent promotes it to run next (`ccode.dev/queue/interject`); any kind may send now.
                ...
                && let Some(server_id) = row.server_id.clone()
            {
                ...
                // the intent; the confirming `ccode.dev/queue/changed` broadcast
    ...
    /// `ccode.dev/queue/changed` broadcast (pre-merge entries — the mirrored
    ...
    /// QUEUED — the caller fires `ccode.dev/queue/interject` with that
    ...
    pub(crate) fn flush_pending_adoption_updates(&mut self, prompt_id: &str) {
        ...
        for (pid, update, mut meta) in std::mem::take(&mut self.pending_adoption_updates) {
            if pid == prompt_id {
                // Forward-only: the ccode rail shares this cursor and may have
    ...
    pub(in crate::app) fn handle_queue_key(
        ...
        if let Some(event) = self.queue.handle_key(key, registry) {
            ...
            // Server-origin rows go to the agent as `ccode.dev/queue/*`
    ...
    /// Reorder payload for `ccode.dev/queue/reorder`. Omit only running; include
