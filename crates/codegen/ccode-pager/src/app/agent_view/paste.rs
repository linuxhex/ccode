                ...
                if let Err(msg) = self.prompt.insert_image(pasted) {
                    ...
                    if ctx.source.tip_showing() {
                        ccode_telemetry::session_ctx::log_event(
                            ccode_telemetry::events::ContextualTip {
...
pub(super) mod paste_key_tests {
    ...
    -> crate::views::question_view::QuestionViewState {
        let question = ccode_tools::implementations::ccode_build::ask_user_question::Question {
            question: "Pick one?".to_string(),
            options: vec![
                ccode_tools::implementations::ccode_build::ask_user_question::QuestionOption {
                    label: "A".to_string(),
    ...
    fn agent_completion_inserts_unreadable_file_url_as_path_text() {
        ...
        assert_eq!(
            agent.prompt.text(),
            "/definitely/missing/ccode-primary-paste.png "
        );
        assert!(agent.prompt.images.is_empty());
    }
    #[test]
