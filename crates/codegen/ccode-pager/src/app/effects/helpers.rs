...
/// Run the post-CTA-install `ccode.dev/mcp/list` read (uncached, which also nudges
...
/// rewrite when the body pushes personal Premium), else auth-aware fallback
...
) -> (bool, Option<String>, Option<ccode_workspace::session::git::RestoreDegree>) {
...
/// CANONICAL wire parser for `LoadSessionResponse._meta["ccode.dev/runningPromptId"]`.
///
...
pub(crate) fn parse_session_load_running_prompt_id(
    ...
    resp_meta
        .and_then(|m| m.get("ccode.dev/runningPromptId"))
        .and_then(|v| v.as_str())
        .map(String::from)
}
/// Whether `raw` is (or wraps) a disk-full / ENOSPC failure.
fn is_disk_full_error(raw: &str) -> bool {
    raw.contains(ccode_worktree::OUT_OF_DISK_CONTEXT)
        || raw.contains(ccode_worktree::ENOSPC_OS_MESSAGE)
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    if is_disk_full_error(raw) {
        return ccode_worktree::ENOSPC_OS_MESSAGE.to_string();
    }
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `ccode-build` (default)         | `false`            |
/// | false | true      | false    | `ccode-build` (default)         | `false`            |
/// | false | false     | true     | `ccode-build-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `ccode-build-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `ccode-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `ccode-build-plan`              | `false`            |
/// | true  | false     | true     | `ccode-build-plan-no-subagents` | omitted (shell gate) |
/// | true  | true      | true     | `ccode-build-plan`              | omitted (shell gate) |
///
/// When [`Self::chat_mode`] is set (gateway light-frontend / `--chat`), Build
/// `agentProfile` injection is omitted (K12) and `_meta["ccode.dev/session"].kind`
...
pub(crate) struct SessionFlags {
    ...
    /// Injected as `ccode.dev/restore_code` into `LoadSession` meta, or passed
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `ccode-build` profile (no `_meta`
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("ccode-build-plan"),
            (true, false, _) => Some("ccode-build-plan-no-subagents"),
            (false, _, true) => Some("ccode-build-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    /// `CCODE_AGENT` — the env var chooses the *agent*, but the tool-strip is
    /// independent. Chat mode additionally stamps `ccode.dev/session.kind`.
    pub(super) fn to_meta(&self) -> Option<acp::Meta> {
        let mut meta = serde_json::Map::new();
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("CCODE_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("CCODE_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
        {} else if let Some(profile) = self.agent_profile() {
            meta.insert("agentProfile".into(), serde_json::json!(profile));
        }
        if self.chat_mode {
            meta.insert("ccode.dev/session".into(), serde_json::json!({ "kind": "chat" }));
        }
        if !self.ask_user {
...
pub(super) const CHAT_FORBIDDEN_WORKSPACE_BIND_KEYS: &[&str] = &[
    "envId",
    "ccode.dev/cloud_server_id",
    "ccode.dev/cloud_existing_workspace",
];
/// Stamp `_meta["ccode.dev/session"].kind = "chat"` and strip Build `agentProfile` (K12).
pub(super) fn apply_chat_kind_meta(meta: &mut Option<acp::Meta>) {
    let obj = meta.get_or_insert_with(acp::Meta::new);
    obj.insert("ccode.dev/session".into(), serde_json::json!({ "kind": "chat" }));
...
pub(super) fn extract_first_user_prompt(
    info: &ccode_shell::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = ccode_shell::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use ccode_shell::sampling::{AssistantItem, ConversationItem, UserItem};
...
/// Degraded conversations lane on `ccode.dev/session/list`, parsed from the
/// response's `_meta["ccode.dev/partial"]` envelope.
...
/// Read `_meta["ccode.dev/partial"]` from a session-list payload. `None` when the
...
pub(super) fn parse_session_list_partial(
    ...
    let partial = payload.get("_meta")?.get("ccode.dev/partial")?;
...
/// Reads `_meta["ccode.dev/listScope"]` from a session-list payload.
pub(super) fn parse_session_list_scope(payload: &serde_json::Value) -> ListScope {
    match payload
        .get("_meta")
        .and_then(|m| m.get("ccode.dev/listScope"))
        .and_then(|v| v.as_str())
    {
...
/// Parse the `ccode.dev/session/list` response payload (the unwrapped
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            let is_conversation = v
                .get("_meta")
                .and_then(|m| m.get("ccode.dev/session"))
            ...
            use ccode_tools::implementations::skills::skill::extract_skill_display_text;
            let display = if let Some(ref fp) = first_prompt {
                ...
                let info_cwd = v
                    ...
                    .unwrap_or_default()
                    .to_string();
                let info = ccode_shell::session::info::Info {
        ...
        .filter_map(|mut e| {
            ...
            if e.source == "remote"
                && ccode_shell::session::resolve_local_session_any_cwd(&e.id)
                    .is_some()
            {
...
/// Best-effort `ccode.dev/auth/cancel`: stops the shell's device/loopback wait so a
...
pub(super) async fn send_check_subscription(
    ...
    let req = acp::ExtRequest::new(
        "ccode.dev/auth/check_subscription",
        serde_json::value::to_raw_value(&serde_json::json!({}))
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            ccode_shell::util::config::set_compact_mode(b)
                .await
        ...
        "show_timestamps" => {
            ...
            ccode_shell::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "page_flip_on_send" => {
            ...
            ccode_shell::util::config::set_page_flip_on_send(b)
                .await
                .map_err(|e| e.to_string())
        }
        "combine_queued_prompts" => {
            ...
            ccode_shell::util::config::set_combine_queued_prompts(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            ccode_shell::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            ccode_shell::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            ccode_shell::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            ccode_shell::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            ccode_shell::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            ccode_shell::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            ccode_shell::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            ccode_shell::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.ssh_wrap" => {
            ...
            ccode_shell::util::config::set_contextual_hint_ssh_wrap(b)
                .await
                .map_err(|e| e.to_string())
        }
        "theme" => {
            ...
            ccode_shell::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            ...
            ccode_shell::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            ccode_shell::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            ccode_shell::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            ccode_shell::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            ccode_shell::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            ccode_shell::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            ccode_shell::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            ccode_shell::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            ccode_shell::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            ccode_shell::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            ccode_shell::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            ccode_shell::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            ccode_shell::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            ccode_shell::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            ccode_shell::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            ccode_shell::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            ccode_shell::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            ccode_shell::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            ccode_shell::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            ccode_shell::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            ccode_shell::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_keybind_enabled" => {
            ...
            ccode_shell::util::config::set_voice_keybind_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "voice_capture_mode" => {
            ...
            ccode_shell::util::config::set_voice_capture_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_stt_language" => {
            ...
            ccode_shell::util::config::set_voice_stt_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            ccode_shell::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            ccode_shell::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            ccode_shell::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            ccode_shell::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
/// 2. Fire ACP `ccode.dev/yolo_mode_changed` (gated on disk success for
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = ccode_shell::util::config::update_config(|cfg| {
...
/// Whether to fire the ACP `ccode.dev/yolo_mode_changed` notification.
...
pub(super) fn marketplace_outcome_succeeded(
    outcome: &ccode_hooks_types::ActionOutcome,
) -> bool {
    outcome.status == ccode_hooks_types::OutcomeStatus::Success
}
/// Extract the typed kill outcome from an `ccode.dev/task/kill` ext response.
///
...
/// serializes (`ccode_shell::extensions::task::KillTaskResponse` +
/// `ccode_shell::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<ccode_tools::types::KillOutcome> {
    use ccode_shell::extensions::task::KillTaskResponse;
    use ccode_shell::session::result::ExtMethodResult;
    serde_json::from_str::<ExtMethodResult<KillTaskResponse>>(resp)
        .ok()
        .and_then(|envelope| envelope.result)
        .map(|payload| payload.outcome)
}
/// Map an `ccode.dev/subagent/cancel` response (payload under `result`) to a kill
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use ccode_shell::extensions::task::{
...
pub(super) fn credit_balance_from_config(
    c: ccode_shell::extensions::billing::BillingConfig,
...
/// Fetch the user's auto top-up rule via the `ccode.dev/auto-topup-rule` extension.
...
/// Map an `ccode.dev/auto-topup-rule` payload to an [`AutoTopupFetch`]. A body that
...
pub(super) fn parse_auto_topup_response(
    ...
    use ccode_shell::extensions::billing::GetAutoTopupRuleResponse;
...
/// A blocking flock on the shared, possibly-network `~/.ccode` lock must never
...
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match ccode_shell::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
