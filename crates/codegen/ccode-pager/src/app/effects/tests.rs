...
use super::*;
use ccode_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    ...
    let rpm_body = "You are sending requests too quickly. Please slow down, or upgrade to a Ccode subscription for higher limits: https://ccode.dev/premium";
    let rpm = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited")
        .data(format!("API error (status 429 Too Many Requests): {rpm_body}"));
    assert!(format_acp_error(&rpm, false).contains("ccode.dev/premium"));
...
/// legacy `ccode.dev/interject` wire shape stays byte-identical.
#[test]
...
fn picker_keeps_conversation_with_empty_cwd_and_missing_updated_at() {
    let payload = serde_json::json!({
            "sessions": [{
                ...
                "source": "conversation",
                "_meta": { "ccode.dev/session": { "kind": "chat" } }
            }]
        });
...
/// Untitled ccode.dev chats must stay listed, rendered as "Untitled".
#[test]
...
fn session_list_partial_parses_reasons() {
    let payload = |reason: &str| {
        serde_json::json!({
                "sessions": [],
                "_meta": { "ccode.dev/partial": { "conversations": true, "reason": reason } }
            })
    };
    assert_eq!(
...
fn session_list_partial_absent_for_healthy_or_meta_less_responses() {
    let healthy = serde_json::json!({
            "sessions": [],
            "_meta": { "ccode.dev/partial": { "conversations": false } }
        });
...
fn parse_kill_outcome_reads_result_envelope() {
    use ccode_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use ccode_workspace::session::git::RestoreDegree;
...
/// Spawn a fake ACP agent that counts `ccode.dev/yolo_mode_changed`
/// notifications. Exits when the channel closes.
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<ccode_acp::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let ccode_acp::AcpAgentMessage::ExtNotification(args) = msg {
                if args.request.method.as_ref() == "ccode.dev/yolo_mode_changed" {
...
/// Redirect `CCODE_HOME` to a tempdir for test isolation.
fn setup_ccode_home_in_tempdir() -> tempfile::TempDir {
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use ccode_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            ccode_shell::active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_ccode_home_in_tempdir();
    ...
    assert_eq!(
            ...
            1,
            "ACP `ccode.dev/yolo_mode_changed` notification must fire exactly once \
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use ccode_acp::AcpAgentMessage;
    use ccode_hooks_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "ccode.dev/marketplace/action" => {
                        ...
                        let req: ccode_hooks_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: ccode_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            ccode_home: PathBuf::from("/path/that/must/not/be-read"),
...
/// browse opts into `allowRelax` and parses `ccode.dev/listScope`; all
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "ccode.dev/session/list");
                ...
                let body = if fail {
                    ...
                    serde_json::json!({
                            "result": {
                                "sessions": [],
                                "_meta": { "ccode.dev/listScope": "repo" },
                            }
                        })
                } else {
    ...
    match tasks.join_next().await.expect("task").expect("no panic") {
        TaskResult::SessionListLoaded { scope, seq, query, .. } => {
            ...
            assert!(
                    scope.is_relaxed(),
                    "_meta[\"ccode.dev/listScope\"] must parse into the task result"
                );
        }
        other => panic!("expected SessionListLoaded, got {other:?}"),
    }
...
async fn fetch_workflows_list_sends_session_id() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "ccode.dev/workflows/list");
                let params: serde_json::Value = serde_json::from_str(
                        args.request.params.get(),
                    )
                    .expect("params JSON");
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use ccode_agent::config::BuiltinAgentName;
...
/// Default flags produce no agent profile (uses ccode-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (ccode-build already has TaskTool).
#[test]
...
/// Neutralize `CCODE_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(CCODE_AGENT)]` (process-global env).
fn without_ccode_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("CCODE_AGENT", "")
}
...
#[serial_test::serial(CCODE_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_ccode_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "ccode-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "ccode-build-plan-no-subagents");
...
/// --ask-user alone selects the ccode-build-ask-user profile.
#[serial_test::serial(CCODE_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "ccode-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `ccode-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce ccode-build-plan
...
fn to_meta_chat_mode_stamps_kind_and_omits_agent_profile() {
    ...
    assert_eq!(meta["ccode.dev/session"]["kind"], "chat");
    assert!(
...
fn chat_load_meta_never_includes_workspace_bind_keys() {
    ...
    apply_chat_kind_meta(&mut meta);
    {
        ...
        obj.insert("ccode.dev/cloud_server_id".into(), serde_json::json!("srv-poison"));
        obj.insert(
            "ccode.dev/cloud_existing_workspace".into(),
            serde_json::json!({
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> ccode_shell::session::SessionInfoResponse {
    use ccode_shell::session::acp_types::{ContextInfo, SessionInfoData};
    ccode_shell::session::SessionInfoResponse {
...
fn format_session_info_session_auth_ignores_api_key_env() {
    ...
    assert!(
            text.contains("Manage account and credits: https://ccode.dev/?_s=billing"),
            "{text}"
        );
    assert!(!text.contains("Also present: CCODE_API_KEY"), "{text}");
    assert!(!text.contains("console.ccode.dev"), "{text}");
    assert!(!text.contains("ccode login"), "{text}");
}
#[test]
fn format_session_info_api_key_without_env() {
    ...
    assert!(!text.contains("CCODE_API_KEY"), "{text}");
    assert!(
            text.contains("Manage account and credits: console.ccode.dev"),
            "{text}"
        );
    assert!(
            text.contains("Run `ccode login` to use your Premium subscription instead."),
            "{text}"
        );
    assert!(!text.contains("ccode.dev"), "{text}");
}
#[test]
fn format_session_info_api_key_auth_notes_console_billing() {
    let info = make_session_info("auto", None, 1000, 10000);
    let text = format_session_info(&info, None, false, true, true);
    assert!(text.contains("Auth method: API key (CCODE_API_KEY)"), "{text}");
    assert!(
            text.contains("Manage account and credits: console.ccode.dev"),
            "{text}"
        );
...
#[test]
fn format_session_info_session_only_manage_at_ccode_com() {
    let info = make_session_info("auto", None, 1000, 10000);
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("ccode-4.5", Some("ccode-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: ccode-4.5 (ccode-4.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("ccode-4.5", Some("ccode-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model: ccode-4.5"));
    assert!(!text.contains("ccode-4.3"));
}
#[test]
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("ccode-4.5", Some("ccode-4.5"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: ccode-4.5"));
    assert!(!text.contains("(ccode-4.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("ccode-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    let entry = SessionPickerEntry {
        ...
        model_id: Some("ccode-4".to_string()),
        num_messages: 3,
    ...
    assert_eq!(roster.model_id.as_deref(), Some("ccode-4"));
