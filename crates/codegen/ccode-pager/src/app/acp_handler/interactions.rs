...
/// Handle `ccode.dev/ask_user_question` ext-method.
///
...
pub(crate) fn handle_ask_user_question(
    ext: ccode_acp::AcpArgs<acp::ExtRequest>,
    app: &mut AppView,
) -> bool {
    ...
    if let Some(mut old_qv) = agent.question_view.take() {
        ...
        if let Some(ref kind) = old_qv.local_kind {
            ...
            let cmd = match kind {
                ...
                LocalQuestionKind::FreeUsageUpsell { .. } => "Premium upsell",
...
/// Handle an `ccode.dev/exit_plan_mode` ext_method request.
///
