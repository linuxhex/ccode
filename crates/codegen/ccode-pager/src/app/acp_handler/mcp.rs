...
/// `CCODE_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        ccode_shell::util::config::resolve_mcp_push_server_status(
...
/// Handle `ccode.dev/mcp/tools_changed` and `ccode.dev/mcp_initialized`.
///
/// Routing rules (verified against the four shell emit sites in
/// `ccode-shell/src/session/acp_session.rs` — toggle-tool ~L6661,
...
///    [`ccode_shell::extensions::mcp::McpToolsChanged`] struct), and
...
pub(super) fn handle_mcp_tools_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    // Both `ccode.dev/mcp_initialized` and (newer shell)
    // `ccode.dev/mcp/tools_changed` carry `sessionId`. Route by it so a
    ...
    if method == "ccode.dev/mcp_initialized"
...
/// Handle `ccode.dev/mcp/server_status`.
///
...
/// ([`ccode_shell::extensions::mcp::McpServerStatusPayload`] +
/// [`ccode_shell::extensions::mcp::McpServerStatus`]) instead of
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use ccode_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};

    let Ok(payload) = serde_json::from_str::<McpServerStatusPayload>(notif.params.get()) else {
        tracing::warn!(
            "Failed to parse ccode.dev/mcp/server_status: {}",
            &notif.params.get()
        ...
        match serde_json::from_value::<Vec<McpToolEntry>>(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    server = %payload.name,
                    error = %e,
                    "ccode.dev/mcp/server_status: tools field present but not Vec<McpToolEntry>; status still applied"
                );
                None
            }
        }
...
/// Handle `ccode.dev/mcp/servers_updated`.
///
/// Emitted by the shell from `MvpAgent` on managed-config resolve and
/// on config reload (`crates/codegen/ccode-shell/src/agent/mvp_agent.rs`
