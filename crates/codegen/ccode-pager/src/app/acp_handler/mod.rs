...
//! and ccode session extension notifications (`ccode.dev/session_notification` and
//! replay-path `ccode.dev/session/update`).
...
use ccode_acp::AcpClientMessage;

use super::actions::Effect;
use ccode_shell::extensions::notification::{
    SessionNotification, SessionUpdate as CcodeSessionUpdate, is_reauthable_failure,
};
use ccode_shell::tools::todo::todo_item_from_plan_entry;
use ccode_workspace::permission::bash_command_splitting::BashCommandHighlights;
        ...
        AcpClientMessage::SessionNotification(notif) => {
            ...
            let affected = match find_session_match(app, &notif.request.session_id) {
                Some(SessionMatch::Root(id)) => {
                    ...
                    // time and drained FIFO. The ccode stream is direct-emitted
                    // and keeps a SEPARATE highwater (see the ccode dedup in
...
/// Handle an ccode extension notification.
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    match notif.method.as_ref() {
        ...
        "ccode.dev/follow_ups" => handle_follow_ups(notif, app),
        "ccode.dev/task_backgrounded" => handle_task_backgrounded(notif, app),
        "ccode.dev/task_completed" => handle_task_completed(notif, app),
        "ccode.dev/models/update" => handle_models_update(notif, app),
        "ccode.dev/settings/update" => handle_settings_update(notif, app),
        "ccode.dev/sessions/changed" => handle_sessions_changed(notif, app),
        "ccode.dev/queue/changed" => handle_queue_changed(notif, app),
        ...
        "ccode.dev/session/prompt_complete" => handle_prompt_complete(notif, app),
        "ccode.dev/session/interjection" => handle_interjection(notif, app),
        "ccode.dev/monitor_event" => handle_monitor_event(notif, app),
        "ccode.dev/scheduled_task_created" => handle_scheduled_task_created(notif, app),
        "ccode.dev/scheduled_task_fired" => handle_scheduled_task_fired(notif, app),
        "ccode.dev/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        "ccode.dev/scheduled_task_inject_prompt" => handle_scheduled_task_inject_prompt(notif, app),
        "ccode.dev/announcements/update" => handle_announcements_update(notif, app),
        "ccode.dev/git_head_changed" => handle_git_head_changed(notif, app),
        "ccode.dev/mcp/init_progress" => handle_mcp_init_progress(notif, app),
        "ccode.dev/mcp/tools_changed" | "ccode.dev/mcp_initialized" => handle_mcp_tools_changed(notif, app),
        "ccode.dev/mcp/server_status" if push_server_status_enabled() => {
            handle_mcp_server_status(notif, app)
        }
        "ccode.dev/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}

/// Handle `ccode.dev/session/interjection` — the leader broadcasts this
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse ccode.dev/session/interjection");
        return false;
    };
    ...
    // emits the queue-emptying `ccode.dev/queue/changed` right after it).
...
fn handle_ext_method(ext: ccode_acp::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
    match ext.request.method.as_ref() {
        "ccode.dev/ask_user_question" => handle_ask_user_question(ext, app),
        "ccode.dev/exit_plan_mode" => handle_exit_plan_mode(ext, app),
        unknown => {
