...
pub(crate) fn is_server_initiated_prompt(prompt_id: &str) -> bool {
    ccode_shell::session::PromptOrigin::from_prompt_id(prompt_id).is_synthetic()
}
...
/// `ccode.dev/session/prompt_complete` turn-end signal. A viewer can thus safely
...
pub(crate) fn is_scheduler_fired_prompt(prompt_id: &str) -> bool {
    matches!(
        ccode_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        ccode_shell::session::PromptOrigin::SchedulerFired
    )
}
...
pub(crate) fn is_wake_prompt(prompt_id: &str) -> bool {
    matches!(
        ccode_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        ccode_shell::session::PromptOrigin::TaskCompleted { .. }
            | ccode_shell::session::PromptOrigin::SubagentCompleted { .. }
            | ccode_shell::session::PromptOrigin::WorkflowCompleted { .. }
            | ccode_shell::session::PromptOrigin::NotificationDrain
    )
}
...
/// adoptable iff the turn emits a terminal `ccode.dev/session/prompt_complete`, the
