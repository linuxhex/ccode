use super::*;
use ccode_shell::sampling::error::format_rate_limited_user_message;
...
/// Replay gate shared by the ACP and ccode session-update paths. Returns `true`
...
/// Handle `ccode.dev/session_notification` and replay-path `ccode.dev/session/update`.
///
...
pub(super) fn handle_session_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    match &session_notif.update {
        CcodeSessionUpdate::TaskBackgrounded { .. } => {
            return handle_task_backgrounded(notif, app);
        }
        CcodeSessionUpdate::TaskCompleted { .. } => {
            return handle_task_completed(notif, app);
        }
        CcodeSessionUpdate::ScheduledTaskCreated { .. } => {
            return handle_scheduled_task_created(notif, app);
        }
        CcodeSessionUpdate::ScheduledTaskDeleted { .. } => {
    ...
    let matched = match find_session_match(app, &session_notif.session_id) {
        ...
        None => {
            tracing::debug!(
                ...
                "load-race: ccode.dev/session_notification DROPPED — no agent matches session_id"
            );
            return false;
        }
    };
    ...
    let is_workflow_update = matches!(
        session_notif.update,
        CcodeSessionUpdate::WorkflowUpdated { .. }
    );
    if !is_workflow_update
        && !meta.is_replay
        && meta.event_seq.is_some_and(|seq| {
            agent
                .last_applied_ccode_event_seq
        ...
        })
    {
        tracing::debug!(
            ...
            last_applied = agent.last_applied_ccode_event_seq,
            "ccode.dev/session update DROPPED by dedup highwater (event_seq <= last_applied)"
        );
        return false;
    }
    ...
    let changed = match session_notif.update {
        ref update @ (CcodeSessionUpdate::AutoCompactStarted { .. }
        | CcodeSessionUpdate::AutoCompactCompleted { .. }
        | CcodeSessionUpdate::AutoCompactFailed { .. }
        | CcodeSessionUpdate::AutoCompactCancelled { .. }
        | CcodeSessionUpdate::RetryState(_)
        | CcodeSessionUpdate::ImageDropped { .. }
        | CcodeSessionUpdate::MemoryFlushCompleted { .. }
        | CcodeSessionUpdate::MemoryDreamCompleted { .. }
        | CcodeSessionUpdate::MemorySessionSaved { .. }) => {
            ...
            if let CcodeSessionUpdate::AutoCompactCompleted { tokens_after, .. } = update {
                refresh_context_used(agent, *tokens_after);
                agent.todo.update_todos(Vec::new());
            }
            changed
        }
        CcodeSessionUpdate::ImageCompressed {
            ref images,
            ref message,
        } => apply_image_compressed(agent, images, message),
        CcodeSessionUpdate::TurnCompleted {
            prompt_id,
            stop_reason,
            agent_result,
            ..
        ...
        CcodeSessionUpdate::HookAnnotation { message } => {
        ...
        CcodeSessionUpdate::HookExecution {
            event_name,
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        ccode_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        ccode_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        ccode_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
                            blocked: true,
        ...
        CcodeSessionUpdate::HooksChanged {
            hooks,
            project_trusted,
            load_errors,
        } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(ccode_hooks_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        CcodeSessionUpdate::PluginsChanged { plugins } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                use crate::views::extensions_modal::TabDataState;
        ...
        CcodeSessionUpdate::SessionSummaryGenerated { session_summary } => {
            agent.generated_session_title =
                Some(crate::util::decode_html_entities(&session_summary).into_owned());
            true
        }
        CcodeSessionUpdate::SessionRecap { summary, auto } => {
        ...
        CcodeSessionUpdate::SessionRecapUnavailable => {
            if meta.is_replay {
                false
        ...
        CcodeSessionUpdate::ModelAutoSwitched {
            previous_model_id,
            new_model_id,
            reason,
        } => {
        ...
        CcodeSessionUpdate::ModelChanged {
            model_id,
            reasoning_effort,
        } => {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if ccode_shell::agent::chat_modes::process_chat_mode_enabled() {
        ...
        CcodeSessionUpdate::MemoryFiles { files } => {
        ...
        update @ CcodeSessionUpdate::WorkflowUpdated { .. } => ingest_workflow_update(agent, update),
        CcodeSessionUpdate::GoalUpdated {
            goal_id,
            objective,
            status,
            phase,
        ...
        CcodeSessionUpdate::InteractionResolved { tool_call_id } => {
    ...
    if let Some(agent) = app.agents.get_mut(&parent_id) {
        if let Some(seq) = meta.event_seq
            ...
            && !is_workflow_update
        {
            agent.last_applied_ccode_event_seq = Some(seq);
        }
...
/// Handle an ccode session notification that targets a child (subagent) session.
///
...
pub(super) fn handle_child_session_notification(
    ...
    match update {
        CcodeSessionUpdate::AutoCompactStarted { .. }
        | CcodeSessionUpdate::AutoCompactCompleted { .. }
        | CcodeSessionUpdate::AutoCompactFailed { .. }
        | CcodeSessionUpdate::AutoCompactCancelled { .. }
        | CcodeSessionUpdate::RetryState(_) => {
            let compact_tokens = match &update {
                CcodeSessionUpdate::AutoCompactCompleted { tokens_after, .. } => Some(*tokens_after),
                _ => None,
            };
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(ccode_tokens::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
        ref update @ (CcodeSessionUpdate::MemoryFlushCompleted { .. }
        | CcodeSessionUpdate::MemoryDreamCompleted { .. }
        | CcodeSessionUpdate::MemorySessionSaved { .. }) => {
            if let Some(child_view) = agent.subagent_views.get_mut(child_sid) {
                apply_session_event(
...
pub(super) fn apply_session_event(
    ...
    match update {
        CcodeSessionUpdate::AutoCompactStarted { percentage, .. } => {
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[ccode_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &ccode_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                ccode_telemetry::session_ctx::log_event(
                    ccode_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
            ...
            let is_free_usage = *rate_limited
                && ccode_shell::sampling::error::is_free_usage_exhausted_error(reason);
            if is_credit_limit {
    ...
    if is_credit_limit {
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use ccode_tools::types::SessionMode;
