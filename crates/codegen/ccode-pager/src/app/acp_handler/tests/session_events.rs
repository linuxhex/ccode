#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn apply_compaction_started_sets_activity() {
        ...
        let update = CcodeSessionUpdate::AutoCompactStarted {
            tokens_used: 90000,
    ...
    fn apply_compaction_failed_keeps_held_prompt() {
        ...
        for error in [
            ...
            let update = CcodeSessionUpdate::AutoCompactFailed {
                error: error.into(),
            };
            assert!(apply_session_event(&update, &mut session, &mut scrollback, false));
            assert_eq!(
    ...
    fn apply_image_dropped_pushes_scrollback_block() {
        ...
        let update = CcodeSessionUpdate::ImageDropped {
            notes: notes.clone(),
        };
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use ccode_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use ccode_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_API_KEY;
        ...
        match last_session_event(&scrollback) {
            Some(SessionEvent::RetryFailed { error, .. }) => {
                ...
                assert!(!error.contains("ccode.dev/premium"));
            }
    ...
    fn apply_retry_state_auth_failure_pushes_reauth_prompt() {
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) from https://cli-chat-proxy.ccode.dev/v1/messages: \
                          no auth context"
                    .into(),
            },
            &mut session,
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `ccode logout` /
    /// `ccode login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `ccode logout` then `ccode login`"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
    ...
    fn apply_compaction_completed_defers_message_until_turn_end() {
        ...
        let update = CcodeSessionUpdate::AutoCompactCompleted {
    ...
    fn apply_unhandled_event_returns_false() {
        let mut session = make_session(Some("s1"));
        let mut scrollback = ScrollbackState::new();
        let update = CcodeSessionUpdate::MemoryFlushStarted;
        assert!(!apply_session_event(&update, &mut session, &mut scrollback, false));
    }
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(ccode_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
