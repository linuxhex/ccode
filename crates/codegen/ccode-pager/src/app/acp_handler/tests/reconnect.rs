#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn reconnect_reload_failure_restores_pre_outage_transcript() {
        ...
        let id = AgentId(0);
        {
            ...
            agent.last_applied_ccode_event_seq = Some(5);
            agent.begin_session_reload(1);
        }
        ...
        let _ = handle_ext_notification(&ccode_model_switch_notif("sess-rc", "sess-rc-30"), &mut app);
        assert_eq!(
        ...
        assert_eq!(
            app.agents[&id].last_applied_ccode_event_seq,
            Some(30),
            "the live ccode line advanced its highwater in-window"
        );
        ...
        assert_eq!(
            agent.last_applied_ccode_event_seq,
            Some(5),
            "the ccode highwater reverts with the transcript — left at 30 it would \
    ...
    /// ccode updates dedup on their OWN per-session `eventId` highwater: a
    ...
    /// ccode arms have no other dedup. Replay stays exempt.
    #[test]
    fn ccode_session_update_dedup_drops_already_applied_event() {
        let mut app = make_app_with_agent("sess-xdup");
        let id = AgentId(0);

        assert!(handle_ext_notification(
            &ccode_model_switch_notif("sess-xdup", "sess-xdup-10"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 1);
        assert_eq!(app.agents[&id].last_applied_ccode_event_seq, Some(10));
        ...
        assert_eq!(
            ...
            1,
            "a duplicate ccode event must not push a second block"
        );
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
            Some("sess-xdup-10")
        ...
        assert!(handle_ext_notification(
            &ccode_model_switch_notif("sess-xdup", "sess-xdup-11"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 2);
        assert_eq!(app.agents[&id].last_applied_ccode_event_seq, Some(11));

        // Lower-stale re-delivery (an already-applied lower id re-sent by
        // the cursor tail, e.g. goal mode) is dropped too — `<=`, not just
        // equality.
        assert!(!handle_ext_notification(
            &ccode_model_switch_notif("sess-xdup", "sess-xdup-9"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].scrollback.len(),
            2,
            "a stale lower-id ccode event must not push a block"
        );
        assert_eq!(app.agents[&id].last_applied_ccode_event_seq, Some(11));
    }

    /// An unhandled ccode kind (the default `_` arm) leaves no trace, so it must
    ...
    #[test]
    fn unhandled_ccode_update_does_not_advance_cursor_or_highwater() {
        let mut app = make_app_with_agent("sess-ig");
        let id = AgentId(0);

        assert!(!handle_ext_notification(
            &ccode_unhandled_notif("sess-ig", "sess-ig-7"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id, None,
            "an unhandled ccode update must not advance the reconnect cursor"
        );
        assert_eq!(
            app.agents[&id].last_applied_ccode_event_seq, None,
            "an unhandled ccode update must not advance the dedup highwater"
        );

        // An applied kind (ModelAutoSwitched) advances both.
        assert!(handle_ext_notification(
            &ccode_model_switch_notif("sess-ig", "sess-ig-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
        ...
        assert_eq!(app.agents[&id].last_applied_ccode_event_seq, Some(8));
    }

    /// Split-highwater regression: a fresh direct-emitted ccode id must NOT
    /// make a queued lower-id ACP chunk look stale. ccode lines bypass the
    ...
    #[test]
    fn direct_ccode_event_does_not_shoot_down_delayed_acp_chunk() {
        let mut app = make_app_with_agent("sess-split");
        let id = AgentId(0);

        // Direct ccode emission stamped N+1 arrives first.
        assert!(handle_ext_notification(
            &ccode_model_switch_notif("sess-split", "sess-split-21"),
            &mut app
        ));
        assert_eq!(app.agents[&id].last_applied_ccode_event_seq, Some(21));
        ...
        assert_eq!(
            app.agents[&id].last_applied_ccode_event_seq,
            Some(21),
            "…and the ACP apply must not clobber the ccode highwater either"
        );
    }
    ...
    /// ccode extension session updates: replay-stamped ones are gated like ACP
    /// updates, and applied ones advance the reconnect cursor.
    #[test]
    fn ccode_session_update_replay_gating_and_cursor() {
        fn model_switch_notif(meta: Option<serde_json::Value>) -> acp::ExtNotification {
            let payload = SessionNotification {
                session_id: acp::SessionId::new("sess-ccode"),
        ...
        let mut app = make_app_with_agent("sess-ccode");
        let id = AgentId(0);

        // Replay-stamped with no load in flight → dropped, nothing pushed.
        let replay_meta = serde_json::json!({ "isReplay": true, "eventId": "sess-ccode-7" });
        assert!(!handle_ext_notification(
        ...
        {
            let agent = app.agents.get_mut(&id).unwrap();
            assert!(agent.scrollback.is_empty(), "unexpected ccode replay dropped");
        ...
        assert_eq!(
            agent.last_seen_event_id.as_deref(),
            Some("sess-ccode-7"),
            "applied ccode updates advance the reconnect cursor"
        );
        assert!(agent.finish_session_reload(1, true));
        assert!(
            !scrollback_has_system_text(agent, "pre-outage content"),
            "an ccode replay line counts as replay for the swap decision"
        );
        assert_eq!(
            agent.scrollback.len(),
            1,
            "the staged ccode block is the new transcript"
        );
    }
    ...
    fn reconnect_finalize_reload_skips_adoption_when_terminal_in_replay() {
        ...
        let _ = handle_ext_notification(
            &ccode_turn_completed_notif("sess-1", "p-run", "end_turn", true),
            &mut app,
        );
    ...
    /// Apply-only cursor rule (ccode path): a `ModelChanged` the catalog can't
    ...
    fn ignored_model_changed_does_not_advance_cursor_applied_one_does() {
        ...
        {
            let agent = app.agents.get_mut(&id).unwrap();
            seed_models(agent, "ccode-3", &["ccode-3", "ccode-4"]);
        }

        // Unknown model → ignored → both markers untouched.
        assert!(!handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "ccode-99-unknown", "sess-1-7"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id, None,
        ...
        assert!(handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "ccode-4", "sess-1-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
