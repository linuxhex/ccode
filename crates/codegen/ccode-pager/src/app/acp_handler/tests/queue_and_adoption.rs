#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// `ccode.dev/queue/changed` broadcast, and an empty broadcast clears it.
    #[test]
    ...
    fn queue_changed_adopts_running_prompt_id_only_when_unset() {
        // Shell and pager share the ccode-prompt type, so serializing the payload as the
        // shell emits it pins the wire shape the handler consumes, not cross-crate compat.
        let shell_payload = ccode_shell::session::prompt_queue::QueueChanged {
        ...
        let notif = acp::ExtNotification::new("ccode.dev/queue/changed", raw.into());
    ...
    /// broadcasts `ccode.dev/queue/changed` with `runningPromptId` = that synthetic
    ...
    fn queue_changed_still_adopts_running_prompt_not_terminal_in_replay() {
        ...
        app.agents.get_mut(&id).unwrap().session.loading_replay = true;
        let _ = handle_ext_notification(
            &ccode_turn_completed_notif("sess-1", "p-old", "end_turn", true),
            &mut app,
        );
        app.agents.get_mut(&id).unwrap().session.loading_replay = false;
    ...
    fn adopted_running_prompt_lets_subsequent_chunk_through_gate() {
        ...
        let msg = AcpClientMessage::SessionNotification(ccode_acp::AcpArgs {
            request,
            response_tx: tx,
        });
        handle(msg, &mut app);
    ...
    fn bash_kind_round_trips_and_adoption_sets_bash_turn() {
        // Shell and pager share the ccode-prompt type; pin kind through a serde cycle.
        let shell = ccode_shell::session::prompt_queue::QueueChanged {
            session_id: "sess-1".to_string(),
            entries: vec![ccode_shell::session::prompt_queue::QueueEntryWire {
                id: "b1".to_string(),
                version: 0,
                owner: None,
                last_editor: None,
    ...
    fn bind_session_id_resets_cursor_only_on_id_change() {
        ...
        agent.last_applied_ccode_event_seq = Some(8);
        ...
        assert_eq!(agent.last_applied_ccode_event_seq, Some(8));
        ...
        assert!(agent.last_applied_ccode_event_seq.is_none());
    }
    ...
    fn viewer_does_not_enter_turn_running_for_server_initiated_turn() {
        ...
        // the actor and emits NO `ccode.dev/session/prompt_complete`. If a viewer
    ...
    fn viewer_enters_turn_running_for_scheduler_fired_cron_turn() {
        ...
        // `MvpAgent::prompt()` and DOES emit `ccode.dev/session/prompt_complete`. So a
    ...
    fn viewer_prompt_complete_finishes_turn() {
        // A viewer in TurnRunning receives ccode.dev/session/prompt_complete for its
    ...
    fn viewer_stop_hooks_after_marker_attach_to_it() {
        ...
        let _ = handle_ext_notification(
            &ccode_turn_completed_notif("sess-view-hooks", "pid-v", "end_turn", false),
            &mut app,
        );
        assert_eq!(
        ...
        let _ = handle_ext_notification(
            &ccode_hook_execution_notif("sess-view-hooks", "stop", false),
            &mut app,
        );
        ...
        let _ = handle_ext_notification(
            &ccode_hook_execution_notif("sess-view-hooks", "stop_failure", false),
            &mut app,
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert_eq!(
