...
use ccode_shell::extensions::notification::RetryState;
use ccode_shell::extensions::notification::SessionUpdate as CcodeSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> ccode_shell::extensions::notification::ImageCompressedEntry {
    ccode_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn interjection_broadcast(
    ...
    acp::ExtNotification::new(
        "ccode.dev/session/interjection",
        std::sync::Arc::from(
...
pub(super) fn make_exit_plan_ext(
    ...
    ccode_acp::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<ccode_acp::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn make_exit_plan_ext_with_tool_call_id(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    let request = acp::ExtRequest::new("ccode.dev/exit_plan_mode", raw.into());
...
pub(super) fn queue_changed_ext(session_id: &str, ids: &[&str]) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "ccode.dev/queue/changed",
        std::sync::Arc::from(serde_json::value::to_raw_value(&params).unwrap()),
    )
}
/// Build a `ccode.dev/queue/changed` notification carrying `runningPromptId`.
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(ccode_acp::AcpArgs {
...
pub(super) fn make_inject_notif(payload: &serde_json::Value) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "ccode.dev/scheduled_task_inject_prompt",
...
pub(super) fn make_fired_notif(
    ...
    let notif = SessionNotification {
        ...
        update: CcodeSessionUpdate::ScheduledTaskFired {
            task_id: task_id.into(),
            prompt: prompt.into(),
    ...
    acp::ExtNotification::new("ccode.dev/scheduled_task_fired", std::sync::Arc::from(raw))
}
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[ccode_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
...
pub(super) fn make_created_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::ScheduledTaskCreated {
            task_id: task_id.into(),
            prompt: prompt.into(),
            human_schedule: human_schedule.into(),
            next_fire_at: next_fire_at.map(str::to_string),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("ccode.dev/scheduled_task_created", std::sync::Arc::from(raw))
}
pub(super) fn make_deleted_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::ScheduledTaskDeleted {
            task_id: task_id.into(),
        },
        meta: None,
    };
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("ccode.dev/scheduled_task_deleted", std::sync::Arc::from(raw))
}
...
pub(super) fn ccode_model_switch_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::ModelAutoSwitched {
...
pub(super) fn ccode_unhandled_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::MemoryFlushStarted,
        meta: Some(serde_json::json!({ "eventId": event_id })),
    };
    acp::ExtNotification::new(
        "ccode.dev/session/update",
...
/// Build an `ccode.dev/session/prompt_complete` ext-notification for `session_id`.
pub(super) fn prompt_complete_ext(session_id: &str) -> acp::ExtNotification {
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("ccode.dev/session/prompt_complete", std::sync::Arc::from(raw))
}
...
/// Build an `ccode.dev/session/prompt_complete` ext-notification with an explicit
...
/// Build an `ccode.dev/session/prompt_complete` ext-notification carrying a
...
/// Build a durable `TurnCompleted` update on the `ccode.dev/session/update` rail,
...
pub(super) fn ccode_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    is_replay: bool,
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::TurnCompleted {
...
pub(super) fn ccode_wake_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
...
/// `ccode.dev/session/update` rail, optionally stamped `isReplay`.
/// `prompt_id == None` models pre-attribution shells.
pub(super) fn ccode_hook_execution_notif_for_prompt(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
) -> acp::ExtNotification {
    use ccode_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    ccode_hook_execution_notif_with_runs(
        session_id,
        event_name,
        prompt_id,
        is_replay,
...
pub(super) fn ccode_hook_execution_notif_with_runs(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
    runs: Vec<ccode_shell::extensions::notification::HookRunEntryDto>,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::HookExecution {
...
pub(super) fn ccode_hook_execution_notif(
    session_id: &str,
    event_name: &str,
    is_replay: bool,
) -> acp::ExtNotification {
    ccode_hook_execution_notif_for_prompt(session_id, event_name, None, is_replay)
}
...
/// Build an `ccode.dev/session/interjection` ext-notification (no id).
pub(super) fn interjection_ext(session_id: &str, text: &str) -> acp::ExtNotification {
    interjection_ext_with_id(session_id, text, None)
}
/// Build an `ccode.dev/session/interjection` ext-notification with an optional
...
pub(super) fn interjection_ext_with_id(
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("ccode.dev/session/interjection", std::sync::Arc::from(raw))
}
...
/// Build an `ExtNotification` envelope for `ccode.dev/session_notification`.
pub(super) fn make_ext_session_notification(
    session_id: &str,
    update: CcodeSessionUpdate,
) -> AcpClientMessage {
...
/// Build an `ExtNotification` envelope with an explicit ccode session method.
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(ccode_acp::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
pub(super) fn test_subagent_spawned(
    ...
    CcodeSessionUpdate::SubagentSpawned {
...
pub(super) fn test_subagent_finished(child_sid: &str) -> CcodeSessionUpdate {
    CcodeSessionUpdate::SubagentFinished {
        subagent_id: child_sid.into(),
        child_session_id: child_sid.into(),
...
pub(super) fn test_subagent_progress(
    ...
    CcodeSessionUpdate::SubagentProgress {
        subagent_id: child_sid.into(),
        parent_session_id: parent_sid.into(),
        child_session_id: child_sid.into(),
        duration_ms: 100,
...
/// Shared temp `CCODE_HOME` for disk-replay tests. `ccode_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `CCODE_HOME`
...
pub(super) fn replay_disk_test_home() -> &'static std::path::Path {
    ...
    HOME.get_or_init(|| {
            ...
            unsafe {
                std::env::set_var("CCODE_HOME", tmp.path());
            }
            tmp
        })
        .path()
}
/// Runs `f` with a thread-local ccode home override so disk replay tests do not
/// depend on process-wide `ccode_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_ccode_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_ccode_home_for_tests(None);
    out
}
pub(super) fn write_child_updates_jsonl(
    ccode_home: &std::path::Path,
    child_sid: &str,
    content: &str,
) {
    let sessions_dir = ccode_home
        .join("sessions")
...
pub(super) fn dispatch_goal_update(
    ...
    handle(
        AcpClientMessage::ExtNotification(ccode_acp::AcpArgs {
            request: acp::ExtNotification::new("ccode.dev/session_notification", raw.into()),
            response_tx: tx,
        }),
        app,
    )
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(ccode_acp::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
}
/// Build an `ccode.dev/session_notification` carrying
...
pub(super) fn interaction_resolved_ext(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::InteractionResolved {
            tool_call_id: tool_call_id.into(),
        },
        meta: None,
    };
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("ccode.dev/session_notification", std::sync::Arc::from(raw))
}
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = ccode_workspace::session::git::GitHeadChanged {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("ccode.dev/git_head_changed", std::sync::Arc::from(raw))
}
pub(super) fn make_task_backgrounded_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::TaskBackgrounded {
            tool_call_id: tool_call_id.into(),
            task_id: task_id.into(),
            command: command.into(),
            cwd: "/tmp".into(),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("ccode.dev/task_backgrounded", std::sync::Arc::from(raw))
}
...
pub(super) fn make_replayed_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("ccode.dev/session/update", std::sync::Arc::from(raw))
}
...
pub(super) fn send_late_bg_detection(app: &mut AppView, tc_id: &str) {
    use serde_json::json;
    use ccode_tools::types::output::{BashOutput, ToolOutput};
    let agent = app.agents.get_mut(&AgentId(0)).unwrap();
    let meta = crate::acp::meta::NotificationMeta::default();
    let bash = BashOutput {
...
pub(super) fn task_completed_notif(
    ...
    use ccode_tools::types::TaskSnapshot;
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::TaskCompleted {
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("ccode.dev/task_completed", std::sync::Arc::from(raw))
}
pub(super) fn make_monitor_event_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::MonitorEvent {
            task_id: task_id.into(),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("ccode.dev/monitor_event", std::sync::Arc::from(raw))
}
...
pub(super) fn make_models_update_notif(
    ...
    acp::ExtNotification::new("ccode.dev/models/update", std::sync::Arc::from(raw))
}
/// `ccode.dev/models/update` carrying a single reasoning-capable model whose
...
pub(super) fn model_changed_ext(
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: CcodeSessionUpdate::ModelChanged {
...
/// Helper: build an `ccode.dev/mcp/init_progress` notification.
pub(super) fn make_mcp_init_progress_notif(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("ccode.dev/mcp/init_progress", std::sync::Arc::from(raw))
}
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: ccode_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use ccode_shell::extensions::mcp::{
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("ccode.dev/mcp/server_status", std::sync::Arc::from(raw))
}
...
pub(super) fn make_servers_updated_notif() -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("ccode.dev/mcp/servers_updated", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = ccode_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "ccode_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("ccode.dev/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName": "ccode_com_linear", "tools": [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("ccode.dev/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_mcp_initialized_notif(session_id: &str) -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("ccode.dev/mcp_initialized", std::sync::Arc::from(raw))
}
