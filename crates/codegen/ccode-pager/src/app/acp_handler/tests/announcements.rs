#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.ccode` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.ccode` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.ccode` reads).
    #[test]
