#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression: replay from `updates.jsonl` emits `ccode.dev/session/update` (not
    ...
    fn ext_session_notification_and_update_equivalent_for_subagent_spawned() {
        ...
        let (spawn_notif, finish_notif) =
            run_subagent_lifecycle_via_method("ccode.dev/session_notification", child_sid);
        let (spawn_update, finish_update) =
            run_subagent_lifecycle_via_method("ccode.dev/session/update", child_sid);
    ...
    fn ext_session_update_malformed_params_returns_false() {
        ...
        let request = acp::ExtNotification::new("ccode.dev/session/update", raw.into());
        let msg = AcpClientMessage::ExtNotification(ccode_acp::AcpArgs {
            request,
            response_tx: tx,
        });
        ...
        assert!(
            !affected,
            "malformed ccode.dev/session/update params must not redraw"
        );
        assert!(
    ...
    fn ext_session_notification_for_inactive_agent_updates_its_context_used() {
        // AutoCompactCompleted on the ccode ext path resets the context bar
