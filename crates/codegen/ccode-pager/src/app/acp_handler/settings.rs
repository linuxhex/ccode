...
/// Handle `ccode.dev/models/update` — model list changed (etag-triggered refresh).
pub(super) fn handle_models_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    if let Ok(model_state) = serde_json::from_str::<acp::SessionModelState>(notif.params.get()) {
        ...
        tracing::info!(
            ...
            "models updated via ccode.dev/models/update"
        );
        ...
        tracing::warn!("Failed to parse ccode.dev/models/update");
        false
    }
}

/// Handle `ccode.dev/settings/update` — remote settings refreshed on `/new`.
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(update) = serde_json::from_str::<PagerSettingsUpdate>(notif.params.get()) else {
        tracing::warn!("Failed to parse ccode.dev/settings/update");
        return false;
    };
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        ccode_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = ccode_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = ccode_shell::config::load_effective_config().ok();
    ...
    if let Some(v) = update.privacy_notice_rollout {
        app.privacy_notice_rollout =
            ccode_config::env_bool("CCODE_PRIVACY_NOTICE_ROLLOUT").unwrap_or(v);
    }
    if let Some(v) = update.privacy_banner_reshow_days {
        app.privacy_banner_reshow_days = Some(
            std::env::var("CCODE_PRIVACY_BANNER_RESHOW_DAYS")
                .ok()
    ...
    // Always recompute is_api_key_auth from the tier so a later Free/Premium
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(ccode_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("CCODE_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(ccode_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        ccode_shell::config::load_merged_requirements(),
        ccode_shell::config::load_from_disk().ok(),
        ccode_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = ccode_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = ccode_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = ccode_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use ccode_shell::util::config::resolve_tips;
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use ccode_shell::util::config::resolve_slash_command_tags;
        let effective_config = ccode_shell::config::load_effective_config().ok();
    ...
    tracing::info!("settings updated via ccode.dev/settings/update");
    true
}
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = ccode_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        ccode_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}

/// Tell live sessions to leave Auto on the mid-session kill-switch: fire the
/// `ccode.dev/yolo_mode_changed` notification the agent maps to
...
/// Handle `ccode.dev/sessions/changed` — the leader broadcasts roster
...
pub(super) fn handle_sessions_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(changed) = serde_json::from_str::<crate::app::roster::RosterChanged>(notif.params.get())
        ...
        tracing::warn!("Failed to parse ccode.dev/sessions/changed");
        return false;
    };
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<ccode_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `CCODE_ANNOUNCEMENTS_OVERRIDE` first, so a
/// backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[ccode_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = ccode_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = ccode_announcements::filter_expired(merged);
    ...
    if ccode_announcements::prune_hidden_announcement_ids(
...
) -> Option<ccode_announcements::RemoteAnnouncement> {
...
/// Deserialization type for the `ccode.dev/settings/update` notification payload.
///
/// This is intentionally a separate struct from `SettingsUpdateNotification` in
/// `ccode-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `ccode-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
...
pub(super) struct PagerSettingsUpdate {
    ...
    // remote_settings also emits gen-ordered `ccode.dev/announcements/update`
