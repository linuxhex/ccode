...
use ccode_shell::sampling::types::ReasoningEffort;
...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: ccode_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open ccode.dev in the browser for Premium subscription upsell.
    OpenPremiumUrl,
    /// Re-check subscription status via the shell's `ccode.dev/auth/check_subscription`.
    CheckSubscription,
    ...
    /// Open ccode.dev managed connectors, appending session teamId when set.
    ...
    /// `prompt_id`. Routed to the agent as `ccode.dev/queue/remove`;
    /// the resulting `ccode.dev/queue/changed` rebroadcast is the source of truth.
    QueueRemoveShared {
        id: String,
        expected_version: u64,
    },
    /// Reorder the server-authoritative (shared) queued prompts to match
    /// `ordered_ids`. Routed as `ccode.dev/queue/reorder`.
    QueueReorderShared {
        ordered_ids: Vec<String>,
    },
    /// Clear the caller's server-authoritative (shared) queued prompts.
    /// Routed as `ccode.dev/queue/clear`.
    QueueClearShared,
    /// Replace the text of a server-authoritative (shared) queued prompt.
    /// Routed to the agent as `ccode.dev/queue/edit`; the rebroadcast of
    /// `ccode.dev/queue/changed` is the source of truth. Last write wins via the
    ...
    /// merges its text into the in-flight turn. Routed as `ccode.dev/queue/interject`;
    /// the `ccode.dev/session/interjection` + `ccode.dev/queue/changed` rebroadcasts are
    ...
    OpenTranscriptPager,
    /// Minimal mode (`ccode --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: ccode_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(ccode_hooks_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(ccode_hooks_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(ccode_hooks_types::MarketplaceAction),
    /// Add or update an MCP server via ccode.dev/mcp/upsert.
    UpsertMcpServer {
        name: String,
        config: Box<ccode_shell::util::config::McpServerConfig>,
    },
    /// Delete an MCP server via ccode.dev/mcp/delete.
    DeleteMcpServer {
        server_name: String,
    },
    ...
    /// Toggle a skill enable/disable via ccode.dev/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    DemoteToBackground,
    /// Request current bundle cache status via `ccode.dev/bundle/status`.
    RequestBundleStatus,
    ...
    AnnouncementsOpenCta(ccode_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"ccode-night"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `ccode dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.ccode/config.toml` and route ACP
/// `ccode.dev/yolo_mode_changed` notifications. The ACP notification is
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: ccode_workspace::foreign_sessions::EnabledForeignSessionSources,
        ccode_home: std::path::PathBuf,
    ...
    FetchSessionList {
        /// Text search pushed down to `ccode.dev/session/list` as `query` (chat
    ...
    /// `ccode.dev/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `ccode.dev/session/list` — the non-leader fallback for the
    ...
    /// Cancel a subagent via `ccode.dev/subagent/cancel`.
    KillSubagent {
        session_id: acp::SessionId,
        subagent_id: String,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.ccode/config.toml`.
    ...
    /// Persist a typed setting to `~/.ccode/config.toml`. On failure,
    ...
    /// `ccode.dev/queue/remove`. The agent re-broadcasts the authoritative queue.
    QueueRemove {
        session_id: acp::SessionId,
        id: String,
        expected_version: u64,
    },
    /// Reorder server-owned queued prompts: fire-and-forget `ccode.dev/queue/reorder`.
    QueueReorder {
        session_id: acp::SessionId,
        ordered_ids: Vec<String>,
    },
    ...
    /// `ccode.dev/queue/edit`. The session actor's serialized mailbox makes this
    /// last-writer-wins for concurrent edits; the rebroadcast of
    /// `ccode.dev/queue/changed` is the truth signal.
    QueueEdit {
        session_id: acp::SessionId,
        id: String,
        new_text: String,
    ...
    /// edits it: fire-and-forget `ccode.dev/queue/hold_edit`.
    QueueHoldEdit {
        session_id: acp::SessionId,
        id: String,
    },
    /// Release a previous [`Self::QueueHoldEdit`]: `ccode.dev/queue/release_edit`.
    QueueReleaseEdit {
        session_id: acp::SessionId,
        id: String,
    },
    /// Interject a server-owned queued prompt into the running turn:
    /// fire-and-forget `ccode.dev/queue/interject`. The session actor atomically
    ...
    /// Resolve the running agent name for a session (`ccode.dev/session/info`).
    ...
    /// Fetch MCP server list from the shell (ccode.dev/mcp/list).
    FetchMcpsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
        cache: bool,
    },
    /// Trigger MCP OAuth for a server (ccode.dev/mcp/auth_trigger).
    McpAuthTrigger {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch hooks list from the shell (ccode.dev/hooks/list).
    FetchHooksList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch plugins list from the shell (ccode.dev/plugins/list).
    FetchPluginsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Fetch skills list from the shell (ccode.dev/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Toggle a skill via ccode.dev/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    MarketplaceAction {
        agent_id: AgentId,
        session_id: acp::SessionId,
        action: ccode_hooks_types::MarketplaceAction,
    },
    /// Install a plugin from the inline CTA via `ccode.dev/marketplace/action`,
    ...
    /// Reload plugins after a CTA install via `ccode.dev/plugins/action`
    ...
    /// Read the MCP server list after a CTA install via `ccode.dev/mcp/list`,
    ...
    /// the same `ccode.dev/mcp/list` fetch as `FetchPluginCtaMcps`, reported back via
    ...
    /// Upsert an MCP server via ccode.dev/mcp/upsert.
    UpsertMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        name: String,
    ...
    /// Live-toggle an MCP server via ccode.dev/mcp/toggle (no restart needed).
    ToggleMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Toggle a single MCP tool via ccode.dev/mcp/toggle_tool.
    ToggleMcpTool {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch and display session info via ccode.dev/session/info.
    ...
    /// Fetch and display detailed context usage via ccode.dev/session/info.
    ShowContextInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch current bundle cache status via `ccode.dev/bundle/status`.
    FetchBundleStatus,
    /// Fetch a bundled entry's raw content via `ccode.dev/bundle/entry/get`.
    ...
    /// Send raw note to ccode.dev/memory/rewrite for LLM-powered reformatting.
    ...
    /// Fire a /btw side question via ccode.dev/btw ext method.
    SendBtw {
        agent_id: AgentId,
        session_id: acp::SessionId,
        question: String,
    ...
    /// Request a session recap via the ccode.dev/recap ext method. Fire-and-forget:
    /// the recap arrives later as a `SessionRecap` notification.
    SendRecap {
        session_id: acp::SessionId,
        auto: bool,
    },
    /// Send a mid-turn interjection via ccode.dev/interject ext method.
    SendInterject {
        agent_id: AgentId,
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `ccode.dev/session/interjection`
    ...
    /// Log out via `ccode.dev/auth/logout` (shell clears auth.json + in-memory state).
    Logout,
    /// Cancel an in-flight interactive auth on the shell (`ccode.dev/auth/cancel`).
    ...
    /// Re-check subscription status via `ccode.dev/auth/check_subscription`.
    ...
    /// registry (`~/.ccode/active_sessions.json`).
    ...
    /// Call `ccode.dev/session/fork` to create a peer session that resumes
    ...
    /// Fetch billing/credit usage from the agent's `ccode.dev/billing` extension.
    ...
    FetchAppBilling,
    /// Fetch per-session token/cost via `ccode.dev/session/usage` (auth-agnostic).
    FetchSessionUsage {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Send an ACP `ccode.dev/suggest` request to the shell. `agent_id` is echoed
    ...
    /// Send an ACP `ccode.dev/suggestPrompt` request to the shell — predict the
    /// user's likely next prompt after a completed turn (tab autocomplete
    /// ghost text).
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`ccode-build-0.1` when the
...
/// Outcome of an `ccode.dev/subagent/cancel` request, telling dispatch whether the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<ccode_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionLoaded {
        ...
        /// `_meta["ccode.dev/runningPromptId"]`), present only when the session was
    ...
    SessionListLoaded {
        ...
        /// Degraded conversations lane (`_meta["ccode.dev/partial"]`), surfaced
        /// as an actionable picker notice instead of a silent empty list.
        partial: Option<crate::app::effects::ConversationsPartial>,
        /// Directory scope `sessions` were drawn from (`ccode.dev/listScope`).
        scope: ccode_shell::session::unified_list::ListScope,
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<ccode_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    /// Leader session roster loaded via `ccode.dev/sessions/list`.
    RosterLoaded {
    ...
    CancelComplete,
    /// Response to `ccode.dev/subagent/cancel`; see [`SubagentKillOutcome`].
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<ccode_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<ccode_shell::util::changelog::ChangelogEntry>,
    },
    ...
    AuthUrlReady {
        ...
        external: bool,
        /// Presentation mode from `ccode.dev/auth/get_url`; `None` on older agents.
        mode: Option<String>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<ccode_hooks_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<ccode_hooks_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<ccode_hooks_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<ccode_hooks_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<ccode_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<ccode_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<ccode_shell::extensions::notification::PromptUsage>,
    },
    ...
    /// `ccode.dev/recap` request acknowledged (fire-and-forget). The recap itself
    ...
    LogoutComplete,
    /// Best-effort `ccode.dev/auth/cancel` finished (no UI update; state already left Authenticating).
    AuthCancelComplete,
    /// Shell responded to `ccode.dev/auth/check_subscription`. `verify` echoes
    ...
    DeepSearchResults {
        results: Vec<ccode_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    /// `ccode.dev/session/fork` completed (no-worktree path). The pager adopts
    ...
    /// `ccode.dev/session/fork` failed. The placeholder agent stays in
    ...
    GateRefreshed {
        settings: Option<ccode_shell::util::config::RemoteSettings>,
    },
    ...
    /// Shell suggestions loaded from ACP `ccode.dev/suggest`. `request_text` /
    ...
    /// Predicted next prompt loaded from ACP `ccode.dev/suggestPrompt`.
