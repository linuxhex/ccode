        ...
        _ => {
            ...
            anyhow::bail!(
                "Multiple sessions match title {:?}:\n{listing}\n\
                 Resume by session id instead: ccode --resume <session-id>",
                arg.trim()
            );
        }
    }
...
pub(crate) fn presandbox_resume_target(
    ...
    if let Some(local_id) = ccode_shell::session::resolve_local_session(arg, cwd) {
        return Ok(PinnedResumeTarget::Id(local_id));
    }
    if ccode_shell::session::resolve_local_session_any_cwd(arg).is_some() {
        return Ok(PinnedResumeTarget::Id(arg.to_string()));
    }
    let summaries = ccode_shell::session::persistence::local_summaries_for_cwd_sync(cwd)
        .map_err(|e| {
