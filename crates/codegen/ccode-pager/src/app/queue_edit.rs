...
//! pattern as ccode-shell's `compaction.rs`): entry from the queue pane,
...
pub enum PromptMode {
    ...
    EditingQueued {
        ...
        /// `ccode.dev/queue/changed` rebroadcast for the visual result) instead
...
impl AgentView {
    ...
    pub(super) fn enter_queue_edit(&mut self, id: u64, is_server: bool, row: Option<QueueRowRef>) {
        ...
        // it. Ignore until the confirming `ccode.dev/queue/changed` lands (mirrors
    ...
    fn interject_edited_queued(
        ...
        match server_id {
            Some(server_id) => {
                // Server rows: the queue wire (`ccode.dev/queue/interject` newText)
