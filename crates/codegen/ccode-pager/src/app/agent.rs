...
use ccode_shell::extensions::notification::GoalClassifierVerdict;
...
pub enum AgentCommand {
    ...
    /// `ccode.dev/session/fork` request is in flight.
    ForkSession,
}
impl AgentCommand {
...
pub struct AgentSession {
    ...
    /// (`ccode.dev/prompt_history` scoped via `filter_session_id`). Most-recent-first.
    /// Fetched on session create/load; prompts sent in this session are
    /// additionally front-inserted locally on send.
    pub prompt_history: Vec<String>,
    /// True until the session's startup/load `ccode.dev/prompt_history` fetch completes.
...
pub struct ChipElement {
    ...
    pub kind: ccode_textarea::ElementKind,
...
impl AgentSession {
    ...
    /// [`ccode_prompt::combine_prefix_len`]. `editing_id` is held out of the
    ...
    pub fn dequeue_combined_prompt(&mut self, editing_id: Option<u64>) -> Option<QueuedPrompt> {
        use ccode_prompt::{CombineGate, TEXT_SEPARATOR, combine_prefix_len, join_texts};
