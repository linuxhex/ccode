...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain Ccode session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: ccode_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
/// Build `ccode.dev/session/fork` params shared by TUI effects and headless.
///
...
pub fn fork_session_params(
    ...
    let source_cwd = ccode_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = ccode_shell::util::ccode_home::ccode_home().join("sessions");
    let encoded = ccode_shell::util::ccode_home::encode_cwd_dirname(&cwd_str);
...
/// Parse `newSessionId` from an `ccode.dev/session/fork` ACP response body.
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    ccode_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub struct MaterializeCtx {
    ...
    /// Process-wide flag: resume targets are ccode.dev conversations, not
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = ccode_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'ccode' to start a new session."
        )
    })?;
    Ok((first.info.id.to_string(), first.display_title_opt()))
}
/// `AuthManager` for direct ccode.dev calls made outside the agent (pre-ACP
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &ccode_shell::agent::config::Config,
) -> std::sync::Arc<ccode_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(ccode_shell::auth::AuthManager::new(
        &ccode_shell::util::ccode_home::ccode_home(),
        agent_config.ccode_com_config.clone(),
    ));
    auth.configure_refresher(
        agent_config.ccode_com_config.auth_provider_command.clone(),
        None,
    );
    auth
}
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if ccode_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = ccode_shell::session::resolve_local_session(session_id, cwd) {
    ...
    if let Some(original_cwd) = ccode_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
...
async fn restore_session_from_remote(
    ...
    if let Some((false, source)) =
        ccode_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    let agent_config = ccode_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use ccode_shell::agent::session_registry_client::SessionRegistryClient;
    use ccode_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    ...
    let storage_client = ccode_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: ccode_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["ccode"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["ccode", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["ccode", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["ccode", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["ccode", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["ccode", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["ccode", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["ccode"])).chat_mode);
    }
    /// hardcoded `false` here once disabled it everywhere.
    #[test]
    fn remote_restore_follows_compiled_restore_stack() {
        assert_eq!(
            MaterializeCtx::from_pager_args(&parse(&["ccode"])).allow_remote_restore,
            false
        );
    }
    ...
    mod resume_by_title {
        use super::*;
        use crate::test_util::CcodeHomeFixture;
        ...
        async fn title_fallback_resumes_single_match_case_insensitively() {
            let mut fx = CcodeHomeFixture::new();
