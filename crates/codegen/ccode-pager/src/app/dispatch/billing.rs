...
use ccode_telemetry::events::{PremiumUpsell, PremiumUpsellClicked};
...
/// Whether the user is at the highest subscription tier (Premium Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("premium_heavy") and CCP display names ("Premium Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "premium_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://ccode.dev/premium?referrer=ccode-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://ccode.dev?_s=usage";
...
///
/// **`max_tier = true`** (positively identified as Premium Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        ccode_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Ccode Build",
            CreditLimitCardAction::PurchaseCredits,
            ccode_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            ccode_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(ccode_telemetry::events::CreditLimitUpsellShown {
            surface: ccode_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            ccode_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `PremiumUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_premium_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the Premium upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_premium_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the Premium upsell modal. Controls the heading
...
fn open_premium_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            PremiumUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with Premium.",
            PremiumUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(ccode_telemetry::events::PremiumUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        ...
        QuestionOption {
            label: "Upgrade to Premium Heavy".into(),
            description: "Get the most out of Ccode Build. Highest usage limits.".into(),
            preview: None,
            // No Heavy-specific URL exists; the /premium page lists
...
/// `ccode.dev/auth/check_subscription` completed. Meta is authoritative
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<ccode_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<ccode_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_premium_url(app: &mut AppView) -> Vec<Effect> {
    log_event(PremiumUpsellClicked {
        source: PremiumUpsell::WelcomeScreen,
    ...
    // Funnel attribution: tag CLI-originated Premium upsell clicks
    // with `referrer=ccode-build`, matching the OAuth consent flow and
    // ccode.dev/cli marketing links. Applied even when the URL came from
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "ccode-build");
