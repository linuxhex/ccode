...
fn effective_cancel_subagents_preference(
    ...
    ui: &ccode_shell::agent::config::UiConfig,
) -> Option<bool> {
...
pub(super) fn do_cancel_turn(app: &mut AppView, cancel_subagents: bool) -> Vec<Effect> {
    ...
    // FRONT queued prompt as the next turn — its `ccode.dev/queue/changed`
...
/// Grace window between a driver-side `ccode.dev/session/prompt_complete`
...
/// Finish turns whose end was announced by `ccode.dev/session/prompt_complete`
...
// TODO: Add dispatch_cancel_command() once ccode-shell supports proper
