...
pub(super) fn dispatch_send_feedback(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    agent.scrollback.push_block(RenderBlock::system(
        "Thanks for the feedback! The Ccode Build team is on it.".to_string(),
    ));
...
/// Send a raw remember note for LLM-powered rewriting via `ccode.dev/memory/rewrite`.
...
/// agent is mid-turn. Fires the `ccode.dev/recap` ext method; the recap arrives
...
pub(super) fn dispatch_send_recap(app: &mut AppView, auto: bool) -> Vec<Effect> {
    ...
    // entirely when the feature is off so we never hit `ccode.dev/recap`.
            ...
            Ok(()) => {
                agent
                    ...
                    .push_block(crate::scrollback::block::RenderBlock::system(format!(
                        "Memory saved to {}",
                        crate::util::display_user_ccode_path("memory/MEMORY.md")
                    )));
            }
            Err(error) => {
                agent
