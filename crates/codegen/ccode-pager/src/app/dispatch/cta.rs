...
pub(super) fn plugin_cta_candidates(
    response: ccode_hooks_types::MarketplaceListResponse,
) -> (Vec<ccode_hooks_types::MarketplacePluginEntry>, bool) {
    let mut candidates = Vec::new();
    let mut official_source_present = false;
    for source in response.sources {
        let is_official = source.source_name == ccode_marketplace::OFFICIAL_SOURCE_NAME
            || ccode_marketplace::is_official_source_url(&source.source_url_or_path);
        if !is_official {
            continue;
        }
...
pub(super) fn cta_install_relative_path(
    candidates: &[ccode_hooks_types::MarketplacePluginEntry],
    name: &str,
) -> String {
    candidates
        .iter()
...
pub(super) fn cta_install_error_category(
    ...
    use ccode_hooks_types::OutcomeStatus;
    match result {
...
pub(super) fn plugin_cta_phase_for(
    ...
    use ccode_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(ccode_telemetry::events::PluginCtaInstalled {
    ...
    let install_result = match result {
        Ok(outcome) if outcome.status == ccode_hooks_types::OutcomeStatus::Success => {
...
pub(super) fn handle_cta_plugin_reload_done(
    ...
    let reload_result = match result {
        Ok(outcome) if outcome.status == ccode_hooks_types::OutcomeStatus::Success => Ok(()),
        Ok(outcome) => Err(crate::app::effects::sanitize_user_error(&outcome.message)),
        Err(e) => Err(e),
    };
    match reload_result {
    ...
    match result {
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(ccode_telemetry::events::ExtensionsModalOpened {
                    trigger: ccode_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = ccode_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(ccode_telemetry::events::PluginCtaImpression {
