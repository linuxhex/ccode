...
/// Start (if needed) and submit the initial prompt from `ccode "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ccode_telemetry::events::ContextualTip {
            tip: ccode_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ccode_telemetry::events::ContextualTip {
            tip: ccode_telemetry::events::ContextualTipKind::SmallScreen,
            action: ccode_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(in crate::app) fn show_ssh_wrap_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ccode_telemetry::events::ContextualTip {
            tip: ccode_telemetry::events::ContextualTipKind::SshWrap,
            action: ccode_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ccode_telemetry::events::ContextualTip {
            tip: ccode_telemetry::events::ContextualTipKind::PlanMode,
            action: ccode_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ccode_telemetry::events::ContextualTip {
            tip: ccode_telemetry::events::ContextualTipKind::WordSelect,
            action: ccode_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(ccode_telemetry::events::ContextualTip {
        tip: ccode_telemetry::events::ContextualTipKind::WordSelect,
        action: ccode_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(ccode_telemetry::events::ContextualTip {
            tip: ccode_telemetry::events::ContextualTipKind::SendNow,
            action: ccode_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_send_prompt_inner(
            ...
            .is_restricted(invocation.token)
    {
        ...
        // another question modal already up, `open_premium_upsell` would
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use ccode_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use ccode_telemetry::session_ctx::log_event;
        ...
        // `ccode.dev/queue/changed`. We render an optimistic echo into the shared
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
            ...
            && agent.session.current_prompt_id.as_deref() != Some(response_pid)
        {
                ...
                && crate::app::acp_handler::is_server_initiated_prompt(response_pid)
            {
                ...
                // later `ccode.dev/queue/changed` broadcast can't re-pin a
        ...
        let free_usage_blocked = agent.session.free_usage_blocked
            || result
                .as_ref()
                .err()
                .is_some_and(|e| ccode_shell::sampling::error::is_free_usage_exhausted_error(e));
