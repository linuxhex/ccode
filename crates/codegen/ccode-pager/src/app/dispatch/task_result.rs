...
/// Whether a completed clipboard probe should fall through to the `ccode wrap`
/// host-image request. A clean `FullMiss` always qualifies; a remote read
/// *error* (`AttachmentRead`) also qualifies because inside `ccode wrap` the
...
/// `osc52_sink_active()`, so this is inert outside `ccode wrap`.
    ...
    match result {
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    ccode_home: ccode_tools::util::ccode_home::ccode_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                ccode_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
