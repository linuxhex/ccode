...
//! `ccode.dev/interject` effect, and prompt-history recording. Split out of
...
/// prompt, and fires the `ccode.dev/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `ccode.dev/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject(
    ...
    // record its id so the broadcast echo (`ccode.dev/session/interjection`) is
