...
/// from `~/.ccode/sessions/<urlencoded_cwd>/<session_id>/plan.md`.
...
pub(super) fn set_plan_mode(
    ...
    let mode_id = acp::SessionModeId::new(if new {
        ccode_tools::types::SessionMode::Plan.as_id()
    } else {
...
pub(super) fn set_yolo_mode_inner(app: &mut AppView, new: bool) {
    ...
    if previous_state != new {
        ccode_telemetry::session_ctx::log_event(ccode_telemetry::events::YoloToggled {
            enabled: new,
            previous_state,
            trigger: ccode_telemetry::events::YoloTrigger::Pager,
        });
...
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    match (in_plan, in_auto, in_yolo) {
        ...
        (false, false, false) => {
            ...
            vec![Effect::SetSessionMode {
                session_id,
                mode_id: acp::SessionModeId::new(ccode_tools::types::SessionMode::Plan.as_id()),
            }]
        }
