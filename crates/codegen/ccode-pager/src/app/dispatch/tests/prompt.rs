...
/// Turn end with prompt suggestions enabled fires the `ccode.dev/suggestPrompt`
...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `ccode-build-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `ccode-build-0.1`
...
/// `ccode.dev/queue/changed` rebroadcast — not client-side prediction — updates
...
/// The always-on pipeline end to end, with `CCODE_SUGGESTIONS` semantics
