...
fn e2e_compact_auth_failure_holds_prompt_and_resubmits_after_login() {
    ...
    use ccode_shell::extensions::notification::{RetryState, SessionUpdate as CcodeSessionUpdate};
    ...
    let id = AgentId(0);
    {
        ...
        apply_session_event_for_test(
            &CcodeSessionUpdate::RetryState(RetryState::Failed {
...
/// `ccode.dev` or start an OIDC flow the agent did not advertise.
#[test]
...
fn auth_complete_preserves_show_resolved_model_when_absent() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::AuthComplete {
            request_seq: 1,
            meta: Some(serde_json::to_value(ccode_shell::auth::AuthMeta::default()).unwrap()),
        }),
        &mut app,
    );
