...
fn doctor_planning_opens_refuses_remote_and_rejects_stale_identity() {
    ...
    dispatch_task_result(
        TaskResult::DoctorFixPlanned {
            ...
            result: Ok(crate::app::actions::DoctorPlanningOutcome::RunLocally(
                "ccode doctor fix ssh-wrap".to_owned(),
            )),
        },
        &mut app,
    );
    assert!(
        last_system_text(&app, id)
            .contains("On your local computer, run: ccode doctor fix ssh-wrap")
    );
...
fn stale_auth_copy_timeout_does_not_clear_newer_feedback() {
    ...
    app.auth_state = AuthState::Authenticating {
        request_seq: 1,
        handle: None,
        auth_url: Some("https://ccode.dev/auth".to_owned()),
...
) -> ccode_workspace::foreign_sessions::RecentForeignSession {
    ccode_workspace::foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                ccode_workspace::foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(ccode_workspace::foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                ccode_workspace::foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn wrap_host_image_request_eligible_covers_full_miss_and_attachment_error_only() {
    ...
    // host-image request — the headless-SSH `ccode wrap` image-paste fix.
...
fn marketplace_list_loaded_sanitizes_components_at_ingestion() {
    ...
    entry.components = Some(ccode_hooks_types::PluginComponents {
        skills: vec![ccode_hooks_types::ComponentItem {
...
fn plugins_action_success_sets_result_notice_and_autoreload_preserves_it() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(ccode_hooks_types::ActionOutcome {
...
fn uninstall_result_notice_is_footer_only_not_row_anchored() {
    ...
    let id = AgentId(0);
    {
        ...
        modal.last_plugins_action = Some(ccode_hooks_types::PluginsAction::Uninstall {
...
fn confirmation_required_builds_plugins_confirmation_with_confirmed_true() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(ccode_hooks_types::ActionOutcome {
                status: ccode_hooks_types::OutcomeStatus::ConfirmationRequired,
    ...
    match &modal.modal_message {
        Some(ModalMessage::Confirmation {
            ...
            assert_eq!(
                action,
                &ConfirmationAction::Plugins(ccode_hooks_types::PluginsAction::Uninstall {
                    plugin_id: "user/ab12/gone".into(),
                    confirmed: true,
                })
            );
...
/// Regression (Bugbot): a failed `ccode.dev/subagent/cancel` RPC must NOT
...
fn switch_to_non_reasoning_model_clears_persisted_effort() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Ccode Build".to_string()),
        );
    app.agents
        .get_mut(&id)
        .unwrap()
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = ccode_shell::agent::config::ModelSwitchIncompatibleAgentError {
        code: "MODEL_SWITCH_INCOMPATIBLE_AGENT".into(),
        active_agent_type: "ccode-build".into(),
...
fn same_agent_type_switch_no_modal() {
    ...
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_a = acp::ModelId::new(std::sync::Arc::from("ccode-build-a"));
    let model_b = acp::ModelId::new(std::sync::Arc::from("ccode-build-b"));

    // Add both models to the catalog (no agentType → both use ccode-build).
    let agent = app.agents.get_mut(&id).unwrap();
    agent.session.models.available.insert(
        model_a.clone(),
        acp::ModelInfo::new(model_a.clone(), "Ccode Build A".to_string()),
    );
    agent.session.models.set_current(model_a, None);
    agent.session.models.available.insert(
        model_b.clone(),
        acp::ModelInfo::new(model_b.clone(), "Ccode Build B".to_string()),
    );
    agent.session.model_switch_pending = true;
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = ccode_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    app.gate = Some(ccode_shell::auth::GateInfo {
        message: "Premium subscription required".into(),
        url: Some("https://ccode.dev/premium".into()),
        label: Some("Subscribe".into()),
    });
    assert!(!app.has_access());

    // Server-side settings now show no gate (user purchased subscription).
    let settings = ccode_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = ccode_shell::util::config::RemoteSettings {
...
fn test_gate() -> ccode_shell::auth::GateInfo {
    ccode_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(ccode_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(ccode_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
