...
///    `ccode.dev/yolo_mode_changed` ACP notification back to the shell.
...
fn enable_always_approve_sends_response_and_flips_yolo_and_persists() {
    ...
    //     ~/.ccode/config.toml AND fires ccode.dev/yolo_mode_changed.
    let persist = effects
        .iter()
        .find_map(|e| match e {
...
/// guard MUST precede the `ccode_telemetry::log_event` call
