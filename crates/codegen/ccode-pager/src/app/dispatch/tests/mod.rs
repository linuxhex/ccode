...
fn test_app() -> AppView {
    ...
    AppView {
        ...
        auth_methods: vec![acp::AuthMethod::Agent(acp::AuthMethodAgent::new(
            acp::AuthMethodId::new("ccode.dev"),
            "Ccode".to_string(),
        ))],
...
fn cta_entry(name: &str, status: &str) -> ccode_hooks_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: ccode_hooks_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `ccode_home()` (OnceLock-cached;
/// do not rely on setting `CCODE_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = ccode_shell::util::ccode_home::ccode_home();
    let encoded = ccode_shell::util::ccode_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
/// `ccode.dev/ask_user_question` ext-method request. Returns the args
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: ccode_tools::implementations::ccode_build::ask_user_question::AskUserQuestionMode::Default,
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    ccode_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    ccode_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
