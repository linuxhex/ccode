...
fn show_privacy_info_zdr() {
    ...
    assert!(
        text.contains("CCODE_TELEMETRY_ENABLED") && text.contains("CCODE_EXTERNAL_OTEL"),
...
fn show_privacy_info_opted_out() {
    ...
    assert!(
        text.contains("Other settings (not changed by /privacy)")
            && text.contains("CCODE_TELEMETRY_ENABLED")
            && text.contains("trace_upload")
            && text.contains("CCODE_EXTERNAL_OTEL"),
...
fn dispatch_confirm_reset_setting_reset_dispatches_typed_setter_for_shared_enum() {
    ...
    with_theme_test_env(|| {
        ...
        // Reset → SetTheme("ccode-night") (the registered default).
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting { key, value, .. } => {
                assert_eq!(*key, "theme");
        ...
        assert_eq!(app.current_ui.theme.as_deref(), Some("ccode-night"));
    });
}
