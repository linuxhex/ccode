...
use super::*;
use ccode_shell::sampling::error::is_free_usage_exhausted_error;
...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("premium_heavy")));
    assert!(is_max_tier(Some("Premium Heavy")));
    assert!(is_max_tier(Some("SUPERCCODE_HEAVY")));
}
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("premium_heav")));
    assert!(!is_max_tier(Some("premium_heavy_plus")));
...
#[test]
fn upsell_non_max_upgrade_url_is_premium() {
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
    ...
    let url = agent_qv(&app).questions[0].options[0]
        ...
        .as_deref()
        .unwrap();
    assert!(url.contains("premium"), "got: {url}");
    assert!(url.contains("referrer=ccode-build"), "got: {url}");
}
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): Ccode Build usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: ccode_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
#[serial_test::serial(CCODE_TEST_OPEN_URL_FILE)]
#[test]
fn manage_billing_gates_on_consumer_billing_surface() {
    let out = std::env::temp_dir().join(format!("ccode-manage-billing-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&out);
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("CCODE_TEST_OPEN_URL_FILE", &out) };
    let mut app = test_app_with_agent();
    dispatch(Action::ManageBilling, &mut app);
    let opened = std::fs::read_to_string(&out).unwrap_or_default();
    assert!(opened.contains("ccode.dev/?_s=usage"), "got: {opened}");
    let _ = std::fs::remove_file(&out);
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    let mut app = test_app_with_agent();
    let before = agent_scrollback_len(&app);
    let usage = ccode_shell::extensions::notification::PromptUsage {
        totals: ccode_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        ccode_shell::extensions::notification::PromptUsage {
            totals: ccode_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("premium_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("premium_heavy"));
}
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: ccode_telemetry::events::PremiumUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        ...
        (
            "Upgrade to Premium Heavy",
            "Get the most out of Ccode Build. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use ccode_shell::extensions::notification::{RetryState, SessionUpdate};
...
/// Submitting a tier-restricted command opens the two-option Premium
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: ccode_telemetry::events::PremiumUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
    ...
    assert_eq!(q.options[0].id.as_deref(), Some(UPSELL_URL_UPGRADE));
    assert_eq!(q.options[1].label, "Upgrade to Premium Heavy");
...
/// `CCODE_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "ccode-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("CCODE_TEST_OPEN_URL_FILE", &bad) };
    ...
    unsafe { std::env::remove_var("CCODE_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("ccode-open-url-ok-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("CCODE_TEST_OPEN_URL_FILE", &url_file) };
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "ccode-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("CCODE_TEST_OPEN_URL_FILE", &bad) };
