...
fn plugin_cta_catalog_reload_empty_candidates_resets_matched_phase() {
    ...
    let response = ccode_hooks_types::MarketplaceListResponse {
        sources: vec![ccode_hooks_types::MarketplaceScanResult {
            source_name: ccode_marketplace::OFFICIAL_SOURCE_NAME.into(),
            source_kind: "git".into(),
            source_url_or_path: ccode_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
...
fn slash_model_valid_dispatches_set_default_model_with_switch_and_persist() {
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_id = acp::ModelId::new(std::sync::Arc::from("ccode-4.5"));
    app.agents
        .get_mut(&id)
        .unwrap()
        .session
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Ccode 4.5".to_string()),
        );
    let effects = dispatch(Action::SendPrompt("/model Ccode 4.5".into()), &mut app);
    assert_eq!(
        effects.len(),
        2,
...
fn clear_default_model_persists_but_keeps_live_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("ccode-test"));
    let info = acp::ModelInfo::new(id.clone(), "Ccode Test".to_string());
...
fn set_default_model_resolves_known_name() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("ccode-4.5"));
    let info = acp::ModelInfo::new(id.clone(), "Ccode 4.5".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
fn set_default_model_idempotent_when_already_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("ccode-already"));
    let info = acp::ModelInfo::new(id.clone(), "Ccode Already".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
/// `clamp_to_terminal` might fold to CcodeNight in non-truecolor
...
fn set_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        ...
        crate::theme::cache::set(crate::theme::ThemeKind::CcodeNight);
        let effects = dispatch(Action::SetTheme("ccode-day".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*key, "theme");
                assert_eq!(*value, SettingValue::Enum("ccode-day"));
                assert_eq!(*rollback_value, SettingValue::Enum("ccode-night"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("ccode-day"));
        assert!(
...
/// for the auto-dark sibling. Uses `ccode-day` to avoid the
...
fn set_auto_dark_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoDarkTheme("ccode-day".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
        ...
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("ccode-day"));
    });
}
#[test]
fn set_auto_light_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoLightTheme("ccode-night".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*value, SettingValue::Enum("ccode-night"));
                assert_eq!(*rollback_value, SettingValue::Enum("ccode-day"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(
...
///
/// Scenario: `theme="ccode-night"` (concrete) + system=Dark. User
/// commits `auto_dark_theme="ccode-day"`. The setting is dormant
/// (parent theme is concrete, not auto), so the live display
/// must stay on CcodeNight even though we're committing a
/// different theme. Uses `ccode-day` to avoid `clamp_to_terminal`
...
fn set_auto_dark_theme_does_not_apply_when_theme_is_not_auto() {
    with_theme_test_env(|| {
        ...
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("ccode-night".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::CcodeNight,
        );
        let _ = dispatch(Action::SetAutoDarkTheme("ccode-day".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::CcodeNight,
...
///
/// Uses `CcodeDay` (non-truecolor-requiring) for the dark-mode
...
/// CcodeNight. Using a non-truecolor theme avoids the clamp
...
fn set_auto_dark_theme_applies_when_theme_is_auto_and_system_is_dark() {
    with_theme_test_env(|| {
        ...
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::CcodeDay,
...
/// Uses `ccode-night` for the auto_dark_theme
/// value to avoid `clamp_to_terminal` ambiguity (we want a
/// concrete kind that's clearly different from CcodeDay, the
...
fn set_auto_dark_theme_does_not_apply_when_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoDarkTheme("ccode-night".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::CcodeDay,
            "auto_dark_theme commit must NOT change live display when system=Light",
        );
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("ccode-night"),);
    });
}
/// Symmetric to the dark test: `set_auto_light_theme` applies only
/// when theme=auto + system=Light. Uses a non-truecolor theme
/// (`ccode-night`) for the same clamp reason as the dark variant.
#[test]
fn set_auto_light_theme_applies_when_theme_is_auto_and_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoLightTheme("ccode-night".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::CcodeNight,
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("ccode-day".into()), &mut app);
        let toast = read_toast(&app);
        assert!(
        ...
        assert!(
            toast.contains("Ccode Day"),
            "toast must use display name `Ccode Day`, not canonical `ccode-day`, got: {toast:?}",
        );
...
fn set_auto_dark_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Ccode Day"));
        assert!(toast.contains('\u{2713}'));
    });
}
#[test]
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Ccode Night"));
    });
}
...
///
/// Uses non-truecolor themes (`ccode-day` ↔ `ccode-night`) to avoid
...
fn rollback_theme_reverts_current_ui_and_cache() {
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                key: "theme",
                rollback_value: SettingValue::Enum("ccode-night"),
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                ...
                rollback_value: SettingValue::Enum("ccode-day"),
                error: "disk full".into(),
            }),
            &mut app,
        );
        assert_eq!(app.current_ui.auto_light_theme.as_deref(), Some("ccode-day"));
    });
}
