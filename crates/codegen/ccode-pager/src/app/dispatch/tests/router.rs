...
fn auth_copy_dispatch_preserves_all_delivery_states() {
    for delivery in [
        ...
        let effects = crate::app::dispatch::router::dispatch_copy_auth_url(&mut app, |url| {
            assert_eq!(url, "https://ccode.dev/auth");
            delivery
        });
...
fn external_prompt_editor_refuses_elements_with_visible_message() {
    ...
    let image = crate::prompt_images::PastedImage {
        element_id: ccode_textarea::ElementId::from_raw(0),
        display_number: 0,
...
fn critical_announcement(id: &str) -> ccode_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> ccode_announcements::RemoteAnnouncement {
    ccode_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(ccode_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: Some(format!("https://ccode.dev/{id}")),
            caption: None,
        }),
        ..Default::default()
    }
...
/// the promo url (observed via the `CCODE_TEST_OPEN_URL_FILE` seam, from every
...
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    let url_file = std::env::temp_dir().join(format!("ccode-cta-open-{}.txt", std::process::id()));
    ...
    for surface in [
        ...
        assert!(
            opened().lines().any(|l| l == "https://ccode.dev/promo-open"),
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!({
        "scope": "user",
        "path": "/home/user/.ccode/skills/pick-best/SKILL.md",
    });
