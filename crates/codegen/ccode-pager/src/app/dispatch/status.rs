...
/// Show session info: fetch via ccode.dev/session/info and display in scrollback.
///
...
pub(super) fn dispatch_show_privacy_info(app: &mut AppView) -> Vec<Effect> {
    ...
    if app.is_zdr {
        ...
        lines.push("  Usage and code data may be used by SpaceCcode to improve the product.");
        lines.push("");
    ...
    lines.push("  - [features] telemetry / CCODE_TELEMETRY_ENABLED");
    lines.push("  - [telemetry] trace_upload / CCODE_TELEMETRY_TRACE_UPLOAD");
    lines.push("  - CCODE_EXTERNAL_OTEL / OTEL_*");
    lines.push("");
    lines.push("  Learn more: https://ccode.dev/legal");
    let text = lines.join("\n");
...
/// (persists via ACP ext-request, NOT `~/.ccode/config.toml`).
...
/// Show context info: fetch via ccode.dev/session/info and display rich breakdown.
///
...
pub(super) fn append_consumer_billing_surface(app: &mut AppView, agent_id: AgentId) -> Vec<Effect> {
    ...
    // Remote-settings kill switch (`ccode_build_usage_redirect_url`): link out
...
pub(super) fn dispatch_manage_billing(app: &mut AppView) -> Vec<Effect> {
    ...
    super::router::dispatch(
        crate::app::actions::Action::OpenUrl("https://ccode.dev/?_s=usage".to_string()),
        app,
    )
}
