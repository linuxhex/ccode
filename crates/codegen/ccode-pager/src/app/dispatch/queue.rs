...
/// immediately, before the confirming `ccode.dev/queue/changed` broadcast.
...
/// that bridges the round-trip before the confirming `ccode.dev/queue/changed`
...
///
/// Delegates to the shared implementation in `ccode_tools::reminders`.
...
fn format_cron_prompt(prompt: &str, task_id: &str, human_schedule: &str) -> String {
    ccode_tools::reminders::format_scheduled_task_prompt(prompt, task_id, human_schedule)
}
...
pub(super) fn maybe_drain_queue(agent: &mut AgentView) -> QueueDrain {
    ...
    match queued.kind {
        QueueEntryKind::Prompt => {
            ...
            let multi = ccode_prompt::is_combined(&queued.combined_texts);
            ...
            let effects = if let Some(mut blocks) = queued.wire_blocks {
                ...
                if let Some(acp::ContentBlock::Text(tb)) = blocks.first_mut() {
                    ...
                    ccode_prompt::stamp_combined_display_texts(map, &combined_segs);
                } else {
                    tracing::debug!(
...
fn paint_or_reuse_combined_user_bubbles(
    ...
    let joined = ccode_prompt::join_texts(segments.iter().map(String::as_str));
...
pub(crate) fn apply_turn_start_shim(
    ...
    // seen ring, so a buffer-replayed `ccode.dev/follow_ups` for an older response
    ...
    let page_flip_entry = if let Some(segments) = multi_segments {
        ...
        if rewindable {
            let restore = text.clone().unwrap_or_else(|| {
                ccode_prompt::join_texts(segments.iter().map(String::as_str))
            });
...
mod tests {
    ...
    fn drain_scroll_honors_page_flip_setting() {
        ...
        crate::appearance::cache::set_page_flip_on_send(
            ccode_shell::agent::config::UiConfig::PAGE_FLIP_ON_SEND_DEFAULT,
        );
    }
    ...
    /// `ccode.dev/follow_ups` for a PRIOR turn's response stays rejected (its
