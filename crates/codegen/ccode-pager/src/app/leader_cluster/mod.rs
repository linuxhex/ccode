...
//!
//! Env sandboxing follows this crate's `serial(CCODE_HOME)` idiom; note
//! `ccode_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_ccode_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `CCODE_API_KEY`,
//! `CCODE_LEADER_SOCKET`, `CCODE_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `ccode_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p ccode-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The ccode home the agent actually persisted under: `ccode_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_ccode_home() -> PathBuf {
    ccode_config::ccode_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same CCODE_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(CCODE_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = ccode_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("CCODE_HOME", ccode_home.path()),
            crate::test_util::EnvVarGuard::set("CCODE_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("CCODE_CCODE_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("CCODE_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("CCODE_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("CCODE_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("CCODE_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (ccode_version), not
        ...
        // ccode-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: ccode-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same CCODE_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `CCODE_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    ccode_ws_url: String::new(),
                    ccode_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
        ...
        if !self.authenticated {
            let _: acp::AuthenticateResponse = bounded(
                "authenticate",
                acp_send(
                    acp::AuthenticateRequest::new(acp::AuthMethodId::new("ccode.api_key"))
