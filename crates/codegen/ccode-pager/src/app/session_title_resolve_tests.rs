...
fn title_miss_hint_escapes_arg_and_suggests_search() {
    ...
    assert!(
        hint.contains("ccode sessions search"),
        "missing hint: {hint}"
    );
}
...
fn worktree_failure_message_hint_follows_threaded_provenance() {
    ...
    assert!(msg.contains("ccode sessions search"), "{msg}");
...
fn pin_prefers_restored_child_over_same_id_in_other_cwd() {
    ...
    let mut args =
        crate::app::cli::PagerArgs::try_parse_from(["ccode", "-r", "legacy-remote-7"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
...
async fn materialization_consumes_pinned_id_after_concurrent_rename() {
    ...
    let mut args = crate::app::cli::PagerArgs::try_parse_from(["ccode", "-r", "alpha"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
...
fn pin_ambiguous_title_errors_before_sandbox() {
    ...
    let mut args = crate::app::cli::PagerArgs::try_parse_from(["ccode", "-r", "Dup"]).unwrap();
    let msg = args
...
async fn pinned_no_match_does_not_retry_title_after_sandbox() {
    ...
    let mut args = crate::app::cli::PagerArgs::try_parse_from(["ccode", "-r", "ghost"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
...
async fn duplicate_legacy_id_is_not_title_addressable() {
    ...
    let mut args =
        crate::app::cli::PagerArgs::try_parse_from(["ccode", "-r", "locked down"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
    assert!(args.resume_target_pinned);
