...
//! `scroll_mode` setting (or `CCODE_SCROLL_MODE`) pins [`ScrollInputMode`], and `invert_scroll` /
...
// `ccode-pager-pty/src/scroll_matrix/gestures.rs` (the harness
...
// (`CCODE_SCROLL_CADENCE_MS` from the event loop); the harness still models
// the 16ms default.
...
pub struct ScrollDebugSnapshot {
    ...
    pub flush_cap: i32,
    /// Effective scroll flush cadence in ms (`CCODE_SCROLL_CADENCE_MS` / default 16).
    pub cadence_ms: u64,
}
...
///   redraw cadence (default 16ms / ~60Hz; `CCODE_SCROLL_CADENCE_MS`) to avoid floods.
...
pub struct MouseScrollState {
    ...
    /// `CCODE_SCROLL_LOG` flight recorder ([`crate::input::scroll_log`]).
...
impl MouseScrollState {
    ...
    /// Inject flush cadence (`CCODE_SCROLL_CADENCE_MS`); Default stays 16ms.
    ...
    /// Emit a flight-recorder line; a single branch when `CCODE_SCROLL_LOG`
