...
use ccode_shell::session::worktree::META_KEY_LABEL;
...
mod tests {
    ...
    fn test_abbreviate_home() {
        if let Ok(home) = std::env::var("HOME") {
            let path = std::path::PathBuf::from(format!("{home}/work/ccode"));
            assert_eq!(abbreviate_home(&path), "~/work/ccode");
        }
    }
    ...
    fn make_test_record(metadata: Option<serde_json::Value>) -> ccode_worktree::WorktreeRecord {
        use ccode_worktree::{WorktreeKind, WorktreeRecord, WorktreeStatus};
        WorktreeRecord {
            id: "test".into(),
            path: "/tmp/wt".into(),
