...
use ccode_worktree::WorktreeRecord;
...
pub async fn run(args: WorktreeArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    let spawned = crate::acp::spawn::spawn_ccode_shell(agent_config.clone(), &cancel, None).await?;
...
async fn dispatch(command: WorktreeCommand, tx: &ccode_acp::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &ccode_acp::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &ccode_acp::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
        WorktreeDbCommand::Stats => {
            let stats: DbStats = ext_call(tx, "ccode.dev/git/worktree/db/stats", &()).await?;
        ...
        WorktreeDbCommand::Path => {
            ...
            let resp: PathResp = ext_call(tx, "ccode.dev/git/worktree/db/path", &()).await?;
            println!("{}", resp.path);
            Ok(())
        }
        WorktreeDbCommand::Rebuild => {
            let report: RebuildReport = ext_call(tx, "ccode.dev/git/worktree/db/rebuild", &()).await?;
...
mod tests {
    ...
    fn ext_request_builds_list_with_filters() {
        ...
        .unwrap();
        assert_eq!(req.method.as_ref(), "ccode.dev/git/worktree/list");
        let params: serde_json::Value = serde_json::from_str(req.params.get()).unwrap();
        assert_eq!(params["repo"], "ccode");
    ...
    fn ext_request_builds_db_stats_empty_params() {
        let req = ext_request("ccode.dev/git/worktree/db/stats", &()).unwrap();
        assert_eq!(req.method.as_ref(), "ccode.dev/git/worktree/db/stats");
    }
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.ccode/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.ccode/worktrees.db");
    }
