//! Local PTY wrapper: the engine behind `ccode wrap` (see [`crate::wrap_cmd`]).
//!
...
///
/// This is the engine behind `ccode wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // `ccode` reached over SSH — that its OSC 52 clipboard writes are being
    // intercepted here and copied to the real local clipboard. The inner ccode
    // reads this (see `ccode_pager_render::clipboard::osc52_sink_active`) to
    // *trust* OSC 52 even when it can't detect an OSC-52-capable terminal,
    // which is the usual SSH case (only `TERM` propagates, so the inner ccode
    ...
    // these two without clearing it. The canonical `CCODE_OSC52_SINK` is
    ...
    cmd.env("CCODE_OSC52_SINK", "1");
    cmd.env("LC_CCODE_OSC52_SINK", "1");
    ...
    // detached — `ccode wrap` is short-lived and exits with the child.
