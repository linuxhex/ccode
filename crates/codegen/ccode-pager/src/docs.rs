...
/// Extract user-guide docs to `<ccode_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(ccode_home: &std::path::Path) {
    let docs_dir = ccode_home.join("docs").join("user-guide");
