...
//!
//! voice-api accepts both API keys and OAuth2 tokens directly at `api.ccode.dev`
...
use ccode_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
///
/// Works for every auth method: OAuth / ccode.dev / OIDC session tokens and
/// `CCODE_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<ccode_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        ccode_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
