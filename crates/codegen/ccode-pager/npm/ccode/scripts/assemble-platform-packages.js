...
//   1. Brotli-compresses the built binary into `../ccode-<platform>/bin/<bin>.br`
...
// (`@ccode-official/ccode`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw ccode binary is
...
const ccodeRoot = process.env.CCODE_ROOT || path.resolve(__dirname, '..', '..', '..', '..', '..');
const npmRoot = path.resolve(__dirname, '..', '..');

const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'ccode-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `ccode-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] ccode-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'ccode',
            envVar: 'CCODE_DARWIN_ARM64',
            defaultSource: path.join(ccodeRoot, 'target', 'release', 'ccode-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'ccode',
            envVar: 'CCODE_DARWIN_X64',
            defaultSource: path.join(ccodeRoot, 'target', 'x86_64-apple-darwin', 'release', 'ccode-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'ccode',
            envVar: 'CCODE_LINUX_X64',
            defaultSource: path.join(ccodeRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'ccode-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'ccode',
            envVar: 'CCODE_LINUX_ARM64',
            defaultSource: path.join(ccodeRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'ccode-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'ccode.exe',
            envVar: 'CCODE_WIN32_X64',
            defaultSource: path.join(ccodeRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'ccode-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'ccode.exe',
            envVar: 'CCODE_WIN32_ARM64',
            defaultSource: path.join(ccodeRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'ccode-pager.exe'),
        },
    ];
