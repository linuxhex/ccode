...
Ccode Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.ccode/hooks/`
- **Project hooks** — from `.ccode/hooks/` in your repo
...
Plugin hooks automatically receive `CCODE_PLUGIN_ROOT` and `CCODE_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.ccode/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `ccode plugin marketplace add <source>`)
...
Sources land in `~/.ccode/config.toml`:
...
Or in `~/.ccode/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../ccode-hooks/examples/README.md) — ready-to-use sample hooks
