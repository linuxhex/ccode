...
Ccode asks before doing anything risky — and can plan before it codes.

## Permissions

When Ccode wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: Ccode explores the
...
`s` requests changes — Ccode iterates until you're happy, then implements.
...
background — Ccode keeps working and you're notified when it finishes
