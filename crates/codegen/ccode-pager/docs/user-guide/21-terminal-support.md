...
Ccode Build runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in Ccode to check the current session and see available fixes. If
Ccode cannot start, run `ccode doctor` in your shell. Use `ccode doctor --json`
...
`ccode doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `ccode doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. Ccode preserves the file's
...
Ccode deliberately does **not** run `tmux source-file` or change the live tmux
...
Ccode detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach Ccode.
...
Ccode writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when Ccode runs inside tmux.
...
terminal focused. Older compositors may require Ccode to remain focused until
the copy message appears. Ccode shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`CCODE_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

Ccode emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `CCODE_CLIPBOARD_NO_OSC52=1` before starting Ccode to
...
A remote Ccode process normally cannot read the local terminal's selection. Use
...
When Ccode cannot identify the outer terminal over SSH, it predicts that OSC 52
...
the local clipboard. Ccode also saves each copy to the backup file named in the
copy message (`~/.ccode/last-copy.txt` by default; override with
`CCODE_COPY_FILE`). You can also use `/copy <file>` or `/minimal`.

For direct clipboard forwarding, run the SSH command from the local computer
through `ccode wrap`, for example `ccode wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `ccode wrap`, Ccode shows the one-time tip
...
`[ui.contextual_hints]` in `$CCODE_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `ccode doctor fix ssh-wrap`. It also shows
...
> **Warning**: `ccode wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. Ccode normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.ccode/pager.toml`, or run
`ccode --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with Ccode

Zellij can intercept Ctrl/Alt keys before they reach Ccode. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach Ccode, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in Ccode.
...
keys. Ccode therefore does not negotiate the protocol there, and Shift+Enter can
...
If Ccode stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, Ccode stops capture and shows
...
to the terminal hosting Ccode. Open **System Settings → Privacy & Security →
...
Run `ccode doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone Ccode would use. If no input device is available, Doctor
...
problem, set `CCODE_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
