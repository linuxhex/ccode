...
Hooks let you run a script or send an HTTP request at key moments in a Ccode session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Ccode calls when a specific lifecycle event occurs. Hooks can:
...
2. Create a hook file, e.g. `~/.ccode/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'Ccode session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a Ccode session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.ccode/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.ccode/hooks/*.json` | Requires trust | Per-repo automation |
...
The Claude and Cursor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.ccode/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.ccode/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.ccode/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`CCODE_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
Ccode accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Ccode skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Ccode maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
- **camelCase input**: ccode's stdin envelope uses camelCase keys throughout where Claude uses snake_case. A script reading `.stop_hook_active`, `.hook_event_name`, or `.background_tasks[].agent_type` must switch to `.stopHookActive`, `.hookEventName`, and `.backgroundTasks[].agentType` (the event value is `"stop"`). Hooks registered through the ccode-agent-sdk convert both the top-level keys and the `backgroundTasks`/`sessionCrons` entry keys to snake_case, so the wire's `.backgroundTasks[].agentType` reads as `.background_tasks[].agent_type` in the SDK.
...
- **StopFailure classes**: the emitted set is Claude Code's vocabulary — `rate_limit`, `authentication_failed`, `invalid_request`, `server_error`, `max_output_tokens`, `unknown`. ccode emits a subset: capacity errors (503/529) fold into `rate_limit` as in Claude, and `billing_error` is never emitted (no signal), so a `billing_error` matcher will not fire.
- **permission_mode values**: ccode emits `default`, `auto`, `plan`, or `bypassPermissions`. Claude's `acceptEdits`/`dontAsk` have no ccode equivalent (ccode's `auto` is the nearest), so a check like `permission_mode === "acceptEdits"` never matches.
...
- **`/goal`**: ccode's goal loop is a separate feature that runs before the stop gate; it is not a prompt-type Stop hook.
...
Ccode sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `CCODE_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `CCODE_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `CCODE_SESSION_ID`     | The unique identifier of the current Ccode session. |
| `CCODE_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `CCODE_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, Ccode additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `CCODE_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `CCODE_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`CCODE_PLUGIN_ROOT`, `CCODE_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
```json
{ "type": "http", "url": "https://hooks.example.com/ccode-event", "timeout": 15 }
```
...
Press `r` in the Hooks tab to reload all hooks from disk. Ccode re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.ccode/hooks/`) run with your user permissions -- treat them like shell scripts.
...
5. **Version control project hooks** -- commit `.ccode/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug CCODE_LOG_FILE=/tmp/ccode.log ccode`, then check `/tmp/ccode.log`.
