...
Ccode reads settings from config files, environment variables, and CLI flags. This page covers the common options.
...
2. **Environment variables** (e.g. `CCODE_API_KEY`, `CCODE_MEMORY`)
3. **config.toml** (`~/.ccode/config.toml`)
...
Location: `~/.ccode/config.toml`. If the file is missing, Ccode uses its built-in defaults, so you only need to set the values you want to override.
...
You can also flip it from the settings pane (`/settings` → **Disable vim input mode**); Ccode writes your choice to `[ui] simple_mode`. `simple_mode` and `vim_mode` are independent — one governs the prompt editor, the other governs scrollback navigation. See [Keyboard Shortcuts](03-keyboard-shortcuts.md) for the full binding reference.
...
You can also override this with `CCODE_DEFAULT_SELECTED_PERMISSION`, which is handy for headless or agent test runs that shouldn't mutate `config.toml`. Precedence: env var → `config.toml` → `always_allow_all_sessions`.
...
Toggle it at runtime with `/vim-mode`, or from `/settings` → **Vim scrollback navigation**. Ccode writes the change to `[ui] vim_mode` immediately and applies it to every future pager session, including new agents and subagents in the same process. There's no per-session override — `config.toml` is the source of truth on next launch. `vim_mode` is independent of `simple_mode`.

#### Screen mode

`[ui] screen_mode` is the **default render mode** for plain `ccode` launches. Set it from `/settings` → **Default screen mode** (restart required) or edit `config.toml` by hand — both write the file. CLI flags (`--minimal` / `--fullscreen`) and slash commands (`/minimal` / `/fullscreen`) are session-scoped and do **not** write this key; after a slash switch, the reverse command returns you for that session only.
...
Each setting also has an environment-variable override, applied on first load only (again, handy for headless / test runs): `CCODE_SCROLL_SPEED`, `CCODE_SCROLL_MODE`, `CCODE_INVERT_SCROLL` (`1`/`true`/`0`/`false`), and `CCODE_SCROLL_LINES`. Precedence: env var → `config.toml` → default. Unrecognized values fall back to the default, and out-of-range numbers clamp.
...
allowed_domains = ["docs.rs", "ccode.dev"]          # override the built-in allowlist
allow_local = false                            # true = allow localhost / 127.0.0.0/8 / ::1 only
```

`allow_local` is off by default (SSRF fail-closed). Turn it on (or set `CCODE_WEB_FETCH_ALLOW_LOCAL=1`) and `web_fetch` may reach **explicit** loopback hosts only — private, link-local, and cloud-metadata ranges stay blocked. Resolution: TOML > env > default off.

`[toolset.ask_user_question]` is honored across **requirements.toml**, **managed config**, and your user **`config.toml`**. Precedence: requirements → env (`CCODE_ASK_USER_QUESTION_TIMEOUT_ENABLED` / `CCODE_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed → defaults. Set `timeout_enabled = false` in your user config to disable the automatic questionnaire timeout for yourself; `timeout_secs` must be a positive integer. You can also toggle `timeout_enabled` from `/settings` → **Ask-Question timeout** (under Agent & Approval); changes apply to newly started sessions.
...
env_key = "CCODE_API_KEY"               # env var(s) holding the API key; string or array (first set, non-empty wins)
...
MCP servers can also be set per-project in `.ccode/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.ccode/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.ccode/config.toml` (current dir) > `<repo-root>/.ccode/config.toml` > `~/.ccode/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `CCODE_MEMORY=1`).
...
[subagents.models]
explore = "ccode-build"               # route to different models
```
...
Background workflows — the `workflow` tool, named `.ccode/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default**. Disable with config, env, or remote settings.

```toml
[workflows]
enabled = false                       # disable background workflows (or CCODE_WORKFLOWS=0)
```

Project workflows are discovered from `<repo-root>/.ccode/workflows/`; user workflows from `~/.ccode/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`ccode inspect` reports cells that still need session-start resolution as `?` until a value is available; cells with an explicit env or TOML value use that value. Affected discovery entries report `compatibilityStatus: "unresolved"` in JSON and `[compat unresolved]` in human output.
...
`[hints]` holds small persisted UI preferences — mostly "stop asking me" opt-outs. Ccode writes these for you when you pick a "don't ask again" option in the TUI, but you can edit or delete them by hand; removing a key restores the default.

`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** opt-outs to your user `~/.ccode/config.toml`.
...
|-----|------|---------|-------------|
| `project_picker_disabled` | bool | `false` | When `true`, skips the picker that asks you to choose a project directory on the first prompt when Ccode launches from a non-project directory (home, Desktop, Downloads, `/tmp`). Set automatically when you choose **"Don't ask me again"** in that picker. Teams can pin it in `managed_config.toml` or `requirements.toml`. |
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "ccode"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `ccode`. |
...
| Ccode Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

With `method = "auto"`, Ccode detects the terminal brand and picks the best protocol. Set `method` explicitly to override that.

#### Notification hooks

Run your own commands when events fire. Hooks receive `$CCODE_EVENT`, `$CCODE_MESSAGE`, and `$CCODE_SESSION_ID` in the environment.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Ccode' -message '$CCODE_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$CCODE_MESSAGE' ntfy.sh/my-ccode-alerts"
...
- **`[features] telemetry`** / `CCODE_TELEMETRY_ENABLED` — the product-analytics master switch. `/privacy` doesn't change it.
- **`/privacy`** / Settings — coding-data sharing, separate from telemetry.
- **`[telemetry] trace_upload`** / `CCODE_TELEMETRY_TRACE_UPLOAD` — session traces; follows telemetry when unset.
- **`[telemetry] otel_*`** / `CCODE_EXTERNAL_OTEL` — external OTEL to your own collector (below).
...
Set these only to point telemetry at your own infrastructure or to switch parts off. The built-in endpoint and credentials are managed by Ccode — leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= CCODE_EXTERNAL_OTEL)
...
- `minimum_version` (`CCODE_MINIMUM_VERSION`) is a soft anti-downgrade floor. The
  updater skips a target below it and keeps the current version. It never blocks
  startup.
- `maximum_version` (`CCODE_MAXIMUM_VERSION`) is a soft ceiling. The updater caps
  its target at it and never installs above it.
- `required_minimum_version` (`CCODE_REQUIRED_MINIMUM_VERSION`) and
  `required_maximum_version` (`CCODE_REQUIRED_MAXIMUM_VERSION`) are hard bounds. If
  the running version is outside the range, the CLI exits at startup and instructs
  the user to install an approved version. `ccode update` and `ccode --version` keep
...
- An explicit `ccode update --version X` is allowed above the ceiling, to recover
...
Location: `~/.ccode/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
|----------|-------------|
| `CCODE_API_KEY` | API key from console.ccode.dev |
| `CCODE_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `CCODE_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `CCODE_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `CCODE_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `CCODE_OIDC_ISSUER` | OIDC issuer URL |
| `CCODE_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `CCODE_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `CCODE_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `CCODE_WORKFLOWS` | Enable (`1`) or disable (`0`) background workflows and select the `/goal` driver (default on: host-owned workflow driver; off: legacy `update_goal`) |
| `CCODE_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `CCODE_WEB_FETCH_ALLOW_LOCAL` | Allow `web_fetch` to explicit loopback hosts only (`localhost` / `127.0.0.0/8` / `::1`). Same as `[toolset.web_fetch] allow_local`. Default off; private/metadata stay blocked. |
| `CCODE_AGENT` | Custom agent definition path or name |
| `CCODE_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `CCODE_LOG_FILE` | Write logs to this file path (used verbatim as the path) |
| `RUST_LOG` | Log level filter (e.g. `debug`); controls the `CCODE_LOG_FILE` log and headless stderr output |
...
| `CCODE_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `CCODE_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `CCODE_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload |
| `CCODE_TELEMETRY_MIXPANEL_ENABLED` | Enable/disable Mixpanel specifically |
| `CCODE_EXTERNAL_OTEL` | External OTEL to your collector (see [24-monitoring-usage.md](24-monitoring-usage.md)) |
| `CCODE_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `CCODE_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.ccode/config.toml` | Main configuration file |
| `~/.ccode/pager.toml` | TUI appearance configuration |
...
| `~/.ccode/lsp.json` | LSP server configuration (user-scoped) |
| `~/.ccode/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
| `.ccode/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some settings can be set per-project by placing files in `.ccode/` inside your repository:

| File | What it configures |
|------|--------------------|
| `.ccode/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.ccode/config.toml`) |
| `.ccode/skills/` | Project-specific skills |
| `.ccode/hooks/` | Project-specific lifecycle hooks |
| `.ccode/agents/` | Project-specific agent definitions |
| `.ccode/lsp.json` | LSP server configuration |
| `.ccode/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.ccode/lsp.json` | All projects |
| Project | `.ccode/lsp.json` | Current repository |
...
1. **Project** — `.ccode/lsp.json`
2. **User** — `~/.ccode/lsp.json`
