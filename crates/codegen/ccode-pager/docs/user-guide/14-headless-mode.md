...
Headless mode runs Ccode non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
Ccode processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, Ccode emits an error object (process exit non-zero). Prompt-level
...
Ccode may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `ccode -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
ccode -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
ccode -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
ccode -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
ccode -p "What's the secret number?" --resume abc123
```
...
ccode -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
ccode --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  ccode -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
Ccode's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(ccode -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
ccode -p "Format all files" --yolo

# Run tests and fix failures
ccode -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| ------------------------------- | ------------------------------------------------------------- |
| `CCODE_API_KEY`        | API key for authentication (required when no browser login)   |
| `CCODE_HOME`                    | Override config directory (default: `~/.ccode`)                |
| `CCODE_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
| `RUST_LOG`                     | Log level filter (e.g. `debug`). Headless logs to stderr.     |

For CI environments without browser access, set `CCODE_API_KEY` with an API key from [console.ccode.dev](https://console.ccode.dev):

```bash
export CCODE_API_KEY="ccode-..."
ccode -p "Run the test suite" --yolo
```
...
- **`CCODE_API_KEY`** — simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`ccode login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`ccode login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `ccode -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug ccode -p "..." 2> debug.log`.

---

## Project Root Discovery

When Ccode starts, it discovers the project root by walking upward from `--cwd`
...
Ccode discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
Ccode stores data in `~/.ccode` (override with `CCODE_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.ccode` read-only:

- Pre-populate `auth.json` or use `CCODE_API_KEY`
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export CCODE_API_KEY="ccode-..."
export CCODE_DISABLE_AUTOUPDATER=1
ccode -p "..." --no-auto-update
```
...
| `CCODE_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`CCODE_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `CCODE_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$CCODE_HOME/bin/ccode`).
...
- Resume: `ccode -p "continue" --resume "<id>"` or `ccode -p "continue" --continue`
