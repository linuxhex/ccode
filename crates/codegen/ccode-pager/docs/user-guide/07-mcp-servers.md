...
MCP (Model Context Protocol) servers extend Ccode with external tool integrations. They let Ccode interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Ccode over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Ccode's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.ccode/config.toml` under `[mcp_servers.<name>]` sections.

### stdio Transport (Local Process)

Ccode spawns a local process and communicates over stdin/stdout:
...
> `CCODE_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `CCODE_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Ccode-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.ccode/config.toml`) **or repo-level**
>   (`.ccode/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.ccode/config.toml` >
...
ccode mcp list
ccode mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by ccode.
ccode mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
ccode mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
ccode mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
ccode mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
ccode mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
ccode mcp remove github

# Diagnose a server's configuration and connectivity
ccode mcp doctor               # Check every configured server
ccode mcp doctor github        # Check one server
ccode mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `ccode mcp add` writes to `~/.ccode/config.toml` (`--scope user`). Use `--scope project` to write to `.ccode/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `ccode mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`ccode mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
MCP servers can be configured per-project by placing a `.ccode/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Ccode handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Ccode's own OAuth client with the provider.

Ccode walks from the current directory up to the git repo root, loading `.ccode/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.ccode/config.toml` | All projects | Lowest |
| `<repo-root>/.ccode/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Ccode reads most other config sections only from `~/.ccode/config.toml`.
...
You can enable or disable MCP servers during a session without restarting Ccode.
...
Ccode loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Ccode config | `~/.ccode/config.toml`, `.ccode/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.ccode/config.toml` or the corresponding environment variable (`CCODE_CURSOR_MCPS_ENABLED`, `CCODE_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `ccode inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Ccode handles the credential flow automatically. When an MCP server requests OAuth credentials, Ccode opens a browser-based authorization flow and stores the resulting tokens for future use.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Ccode expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
For stdio servers, Ccode captures the process's standard error to `~/.ccode/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.ccode/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `ccode inspect` to see all loaded MCP servers and their sources:

```bash
ccode inspect          # Human-readable
ccode inspect --json   # Machine-readable
```
