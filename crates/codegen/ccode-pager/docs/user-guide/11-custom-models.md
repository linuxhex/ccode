...
Ccode connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.

---

## Default Models

By default, Ccode uses models hosted by SpaceCcode, and new sessions start with `ccode-build`. Default models require no configuration. Authenticate with `ccode login` or an API key, then start a session.
...
Set a persistent default in `~/.ccode/config.toml`:
...
Ccode supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, Ccode uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. Ccode sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.ccode/config.toml` under `[model.<name>]` sections:
...
env_key = "CCODE_API_KEY"                   # Env var holding the API key (optional; string or array)
...
Ccode resolves the API key in this order:
...
3. Your signed-in session token (from `ccode login`), for a model with no `api_key`/`env_key` of its own
4. The `CCODE_API_KEY` environment variable (global fallback; Ccode also accepts `CCODE_CODE_CCODE_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells Ccode when to trigger auto-compaction. When you override a known model, Ccode inherits that model's context window. When you define a new model and omit `context_window`, Ccode defaults to 200,000 tokens, so set it explicitly to match your provider.
...
Some gateways route or version on the query string. `query_params` appends percent-encoded query parameters to every request Ccode makes for a model. For example, a gateway that selects an API version this way:
...
When you override a built-in model, Ccode starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which Ccode sends verbatim.
...
Point Ccode at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `CCODE_MODELS_BASE_URL` | Yes | Base URL for inference. Ccode fetches the model list from `{base_url}/models`. |
| `CCODE_API_KEY` | Yes | API key sent as `Authorization: Bearer`. Ccode also accepts `CCODE_CODE_CCODE_API_KEY`. |
| `CCODE_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, Ccode inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, Ccode uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `ccode login` -- the API key is enough.
...
```toml
[models]
web_search = "ccode-4.20-multi-agent"
```

Or via environment variable:

```bash
export CCODE_WEB_SEARCH_MODEL="ccode-4.20-multi-agent"
```

If you point web search at a custom model, you also need a `[model.*]` entry so Ccode can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
...
curl -s https://api.example.com/v1/models \
  -H "Authorization: Bearer $CCODE_API_KEY"
```
