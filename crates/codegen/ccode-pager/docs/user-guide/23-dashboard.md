...
- **`ccode dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.ccode/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Ccode Build · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Ccode Build's sibling views (
`tasks_pane`):
...
Set `CCODE_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `ccode --bg`") would list
