...
Memory lets Ccode recall facts, decisions, and patterns from earlier sessions. Ccode indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each Ccode session starts fresh: the model knows nothing about previous sessions. When you enable memory, Ccode can:
...
3. `CCODE_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.ccode/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.ccode/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.ccode/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

Ccode suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, Ccode saves a structured metadata summary to that session's daily log. The summary contains:
...
Ccode builds the summary from conversation metadata without an LLM call, without added latency. Ccode skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask Ccode to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.ccode/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

Ccode records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, Ccode opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, Ccode shows `Memory saved to ~/.ccode/memory/MEMORY.md`.

### Forget

Ask Ccode to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.ccode/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what Ccode remembers:

```
> what do you remember?
```

Ccode searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.ccode/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, Ccode confirms in the scrollback:

```
Memory saved to ~/.ccode/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what Ccode has stored.
...
Dream also runs automatically. By default, Ccode checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, Ccode automatically searches memory for content relevant to the current project and injects it as context. This means Ccode starts with knowledge from previous sessions without a reminder.
...
Ccode searches memory automatically, but you can also trigger searches manually in the chat:
...
The `ccode memory` command manages memory from the shell. It has one subcommand, `clear`:
...
ccode memory clear --workspace

# Clear the global MEMORY.md
ccode memory clear --global

# Clear both workspace and global memory
ccode memory clear --all

# Skip the confirmation prompt (-y is the short form)
ccode memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.ccode/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.ccode/memory/` for external edits and reindex |
...
| `min_score` | unset | Score threshold for first-turn results. When unset, Ccode applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, Ccode uses the session's primary model. |
...
When a session memory is old, Ccode attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, Ccode watches `~/.ccode/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `ccode inspect` output.
2. Check the flag: `ccode --experimental-memory` or `CCODE_MEMORY=1`.
3. Check for `--no-memory` or `CCODE_MEMORY=0` overriding your config.
...
```bash
ls ~/.ccode/memory/
cat ~/.ccode/memory/MEMORY.md
$EDITOR ~/.ccode/memory/MEMORY.md
```

### Debug Logging

```bash
RUST_LOG=debug CCODE_LOG_FILE=/tmp/ccode.log ccode
grep "memory" /tmp/ccode.log
```
