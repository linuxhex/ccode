...
Agent mode runs Ccode as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`ccode -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `ccode agent` command and apply to every mode. Pass them before the mode name, for example `ccode agent --model ccode-build stdio`. The `stdio` subcommand itself takes no options.

| Flag                       | Description                                                       |
| -------------------------- | ---------------------------------------------------------------- |
| `-m, --model <MODEL>`      | Set the model ID (for example, `ccode-build`).                    |
...
```bash
ccode agent serve --bind 127.0.0.1:2419 --secret <token>
```

Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent generates a token and prints it at startup; you can also supply one through the `CCODE_AGENT_SECRET` environment variable. The agent persists across reconnections, so a client can disconnect and later resume in-flight work.
...
Beyond the base ACP protocol, Ccode defines extension methods under the `ccode.dev/` prefix for SpaceCcode-specific functionality. These cover:

| Category                   | Prefix               | Examples                                         |
| -------------------------- | -------------------- | ------------------------------------------------ |
| **Filesystem**             | `ccode.dev/fs/*`          | `list`, `exists`, `read_file`, `write_file`      |
| **Git**                    | `ccode.dev/git/*`         | `status`, `stage`, `commit`, `diffs`, `discard`  |
| **Git Worktree**           | `ccode.dev/git/worktree/*`| `create`, `remove`, `apply`, `list`, `gc`        |
| **Search**                 | `ccode.dev/search/*`      | `fuzzy/open`, `fuzzy/change`, `content`          |
| **Terminal**               | `ccode.dev/terminal/*`    | `create`, `kill`, `output`, `wait_for_exit`      |
| **Session Management**     | `ccode.dev/session/*`     | `fork`, `resolve_local_for_worktree_resume`      |
| **Conversation & History** | `ccode.dev/*`             | `prompt_history`, `rewind/*`, `compact_conversation` |
| **Authentication**         | `ccode.dev/auth/*`        | `get_url`, `submit_code`                         |
| **Feedback & Telemetry**   | `ccode.dev/*`             | `feedback`, `telemetry/*`                        |

The tables here show representative methods in each category. The `ccode.dev/*` set is SpaceCcode-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
...
| -------------------------- | ------------------------------------ |
| `ccode.dev/search/fuzzy/status` | Fuzzy search results update          |
| `ccode.dev/git/worktree/status` | Worktree creation progress           |
| `ccode.dev/fs_notify`           | Filesystem change notification       |
| `ccode.dev/fs/index`            | Full file index update               |
| `ccode.dev/fs/index/delta`      | Incremental file index update        |
| `ccode.dev/session_notification`| Session-specific updates (diff review, retry state, auto-compact) |
| `ccode.dev/session/update`      | Session update (tool calls, content) |
