...
- **Hooks** -- a `hooks/hooks.json` file of lifecycle hooks. Plugin hooks also receive `CCODE_PLUGIN_ROOT` and `CCODE_PLUGIN_DATA` (see the [Hooks guide](10-hooks.md) for every environment variable passed to hooks).
- **MCP servers** -- a `.mcp.json` file of server configurations
- **LSP servers** -- a `.lsp.json` file of language server configurations

If a plugin includes a `plugin.json` manifest, the manifest can override paths or add metadata; otherwise components load from the convention directories. The manifest is optional: without one, Ccode discovers the components above from their standard directories.
...
| `CCODE_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory, for plugin state, caches, and logs. |

Ccode sets these values and overrides any value you declare for the same key in the hook JSON's `env` map. (Ccode also sets the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases for compatibility.) See the [Hooks guide](10-hooks.md) for every environment variable passed to hooks.

---

## Plugin locations

Ccode discovers plugins from these locations, in priority order:

| Location | Scope | Trust |
|----------|-------|-------|
| `_meta.pluginDirs` (`session/new` / `session/load`) | Session -- loaded for that session only | Trusted automatically |
| `--plugin-dir` (CLI flag, `ccode agent`) | Process -- loaded for that agent process only | Trusted automatically |
| `.ccode/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.ccode/plugins/` | User -- personal plugins for every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add in `config.toml` | Depends on location |

Ccode also reads the `.claude/plugins/` equivalents for compatibility. When two plugins share a name, the higher-priority location wins.

The Agent SDKs load per-session plugins through `CcodeOptions.plugins`, which arrives as `_meta.pluginDirs` on `session/new` and `session/load`; because the caller controls the directory, these plugins are always trusted -- their hooks and MCP servers activate without a prompt, and they never persist beyond the session. The `--plugin-dir` flag is the process-wide equivalent for direct CLI use (repeatable: `ccode agent --no-leader --plugin-dir A --plugin-dir B stdio`); it applies to dedicated agent processes only and is ignored in leader mode (the shared leader discovers its own plugins).
...
```bash
ccode plugin list [--json] [--available]   # List installed plugins (--available requires --json)
ccode plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
ccode plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
ccode plugin update [<name>]               # Omit the name to update all plugins
ccode plugin enable <name>
ccode plugin disable <name>
ccode plugin details <name>                # Show the plugin's component inventory
ccode plugin validate [<path>]             # Validate plugin.json (default: current directory)
ccode plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `ccode plugin install <source>` without `--trust` and Ccode prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
or `CCODE_MARKETPLACE_REQUIRE_SHA=1`. Both are tighten-only: either one enables
...
```bash
ccode plugin marketplace list [--json]
ccode plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
ccode plugin marketplace remove <url>      # Git URL or local path of a configured source
ccode plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
ccode plugin marketplace add my-org/team-plugins
ccode plugin marketplace list
ccode plugin install my-org/team-plugins --trust
ccode plugin list
ccode plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `ccode plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.ccode/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `ccode plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.ccode/pager.toml`:
...
Place this file at `~/.ccode/settings.json` or `~/.claude/settings.json`.
...
Ccode trusts plugins from `~/.ccode/plugins/` automatically. Project plugins in `.ccode/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
ccode plugin install <source> --trust
```
...
Run `ccode inspect` to see every discovered plugin and what it provides:

```bash
ccode inspect          # Show plugins with their skills, agents, hooks, and MCP servers
ccode inspect --json   # Emit machine-readable JSON
```
