...
> **Status: alpha.** The schema below is versioned (`ccode_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

Ccode CLI can export usage **metrics** and **events** to your organization's
...
any data flowing through SpaceCcode.
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `CCODE_TELEMETRY_ENABLED` |
| `/privacy` | `/privacy opt-in` / `/privacy opt-out`, or Settings |
| Trace upload | `[telemetry] trace_upload` / `CCODE_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `CCODE_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
- **Structurally separate** from SpaceCcode-internal telemetry: its exporters carry
  only the headers you configure, never SpaceCcode credentials.
- **Independent of SpaceCcode data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams. Those
  settings govern SpaceCcode-side retention; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export CCODE_EXTERNAL_OTEL=1                  # master switch
...
`CCODE_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `CCODE_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `CCODE_TELEMETRY_ENABLED`, which controls SpaceCcode-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `ccode_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `CCODE_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`CCODE_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `ccode setup` managed config /
...
## Metrics (meter scope `ai.ccode.ccode_code`)

| Metric | Unit | Attributes |
|---|---|---|
| `ccode_code.session.count` | `{session}` | base attrs only |
| `ccode_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `ccode_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `ccode_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `ccode_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `ccode_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `ccode_code.token.usage` with your own
...
|---|---|
| `ccode_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `ccode_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `ccode_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `ccode_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `ccode_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `ccode_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `ccode_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `ccode_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `ccode_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `ccode_code.permission_mode_changed` | `to_mode`, `trigger` |
| `ccode_code.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `ccode_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `ccode_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `ccode_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `ccode_code.auth` | `auth_method` |
| `ccode_code.internal_error` | `error_type` (class only — no message, no location) |
| `ccode_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(ccode_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(ccode_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(ccode_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(ccode_code_tool_decision_total[1h]))
```
