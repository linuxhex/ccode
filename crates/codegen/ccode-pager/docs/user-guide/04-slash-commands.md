...
Commands come from two places: **shell builtins**, handled by the agent backend (ccode-shell), and **pager builtins**, handled by the TUI frontend (ccode-pager). Both show up in the same menu, and any enabled skill with `user-invocable: true` appears there too.
...
Compress conversation history to reclaim context-window space. Pass a note to tell Ccode what to keep:
...
Ccode also auto-compacts once the context window hits 85% (tune it with `[session] auto_compact_threshold_percent`).
...
In minimal mode, open an external editor for an empty composer. Ccode resolves `$VISUAL`, then `$EDITOR`, then `vi`; command values may include quoted arguments. Saving replaces the draft without sending it, and saving an empty file clears it. The command is hidden outside minimal mode.
...
Every copy is also written to a backup file — `~/.ccode/last-copy.txt` by default, or `CCODE_COPY_FILE` if set — and the toast tells you exactly where the text landed, so you can retrieve it even when the clipboard couldn't be reached or the copy went out as an OSC 52 escape this terminal couldn't confirm.
...
`/flush`, `/dream`, and `/memory` require memory to be enabled (`--experimental-memory` or `CCODE_MEMORY=1`); `/memory` also needs a configured memory backend. `/remember` is always available.
...
Run a prompt on a recurring interval. Give the interval as `30m`, `1 hour`, or `every 2 days`; leave it out and Ccode will ask.
...
Set, manage, or check an autonomous goal. Ccode works across rounds and only marks the goal complete after an independent evidence review confirms the claim; if that review can't reproduce the result or has no usable evidence, the goal stays active or pauses with concrete gaps.
...
Project workflows live in `.ccode/workflows/*.rhai`; user workflows live in `~/.ccode/workflows/*.rhai`. A same-process pause/resume continues the original immutable script, args, and `agent_budget` cap from committed host-call results — to iterate, edit the returned script copy and launch it as a new run.
...
- `/docs web` opens https://docs.ccode.dev/build/overview in your browser.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter shows up as a slash command. (Turn a skill off via `/skills` and it stops being advertised.) So a skill at `~/.ccode/skills/commit/SKILL.md` runs as:
