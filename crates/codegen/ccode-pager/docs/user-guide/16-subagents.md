...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.ccode/agents/` or `~/.ccode/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.ccode/personas/`; applied during subagent resolution |
...
| **Examples** | `ccode-build`, `explore`, `plan` | `researcher`, `concise` |
...
Ccode Build discovers file-based personas from these locations, in priority order:

- `.ccode/personas/*.toml` (project)
- `~/.ccode/personas/*.toml` (user)
...
> **Note:** Ccode Build applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Ccode Build resolves the effective model and reasoning effort in this order, highest priority first:
...
Ccode Build manages worktrees through the `ccode.dev/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "ccode-build"               # route explore to a specific model
```
...
# instructions_file = ".ccode/personas/concise.md"  # or load from a file
```

Ccode Build also discovers roles from `.ccode/roles/*.toml` and personas from `.ccode/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Ccode Build shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · ccode-3) — Thinking`
