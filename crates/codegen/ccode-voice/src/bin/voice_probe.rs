...
//! cargo run -p ccode-voice --bin voice-probe -- --seconds 5
//! ```
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(code) = ccode_voice::maybe_run_capture_subprocess() {
...
async fn run() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,ccode_voice=debug".into()),
        )
        .init();

    let args = parse_args(std::env::args().skip(1).collect());

    let auth = std::env::var("CCODE_API_KEY")
        .ok()
        .and_then(StaticVoiceAuth::shared)
        .ok_or_else(|| {
            anyhow::anyhow!("set CCODE_API_KEY (standalone probe has no login session)")
        })?;
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                ccode_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("CCODE_CCODE_API_BASE_URL").ok();
    ...
    if let Ok(home) = std::env::var("CCODE_HOME")
...
Environment:
  CCODE_API_KEY     required
  RUST_LOG        optional (default info,ccode_voice=debug)

Reads [voice] from ~/.ccode/config.toml unless --config PATH is set.
"#
    );
}
