...
/// Streaming STT over `wss://api.ccode.dev/v1/stt`.
...
mod tests {
    ...
    fn optional_header_inserted_when_present_skipped_when_empty() {
        let mut req = "wss://api.ccode.dev/v1/stt".into_client_request().unwrap();
        insert_optional_header(&mut req, "x-ccode-client-identifier", "ccode-shell");
        insert_optional_header(&mut req, "User-Agent", "");
        assert_eq!(
            req.headers().get("x-ccode-client-identifier").unwrap(),
            "ccode-shell"
        );
        assert!(
