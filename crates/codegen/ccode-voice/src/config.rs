...
/// `[endpoints].ccode_api_base_url` so enterprise proxies need no second knob.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(default)]
pub struct VoiceConfig {
    /// HTTPS API root (or bare host). Bases may end in `/v1` or `/ccode/v1`; the
...
impl Default for VoiceConfig {
    fn default() -> Self {
        Self {
            api_base: "https://api.ccode.dev".into(),
...
impl VoiceConfig {
    ...
    /// `api_base`: non-empty `[voice].api_base`, else `[endpoints].ccode_api_base_url`
    /// from `root`, else `resolved_endpoints_base`, else `https://api.ccode.dev`.
    ///
    ...
    pub fn from_config_table(root: &toml::Table, resolved_endpoints_base: Option<&str>) -> Self {
        ...
        // makes "unset" and an explicit `https://api.ccode.dev` indistinguishable.
        ...
        .or_else(|| {
            non_empty_str(
                root.get("endpoints")
                    .and_then(|e| e.get("ccode_api_base_url"))
                    .and_then(|v| v.as_str()),
            )
        })
...
fn ws_url(api_base: &str, path: &str) -> Result<String, VoiceError> {
    ...
    let rest = strip_scheme(base, "https://")
        ...
        .unwrap_or(base);
    // Default path `/v1/stt`; bases often end in `/v1` or `/ccode/v1`.
...
mod tests {
    ...
    fn default_stt_ws_uses_wss() {
        assert_eq!(
            ...
            "wss://api.ccode.dev/v1/stt"
        );
    }

    #[test]
    fn scheme_less_and_wss_bases() {
        for base in ["api.ccode.dev", "wss://api.ccode.dev", "HTTPS://api.ccode.dev"] {
            let cfg = VoiceConfig {
                api_base: base.into(),
                ..VoiceConfig::default()
            };
            assert_eq!(cfg.stt_ws_url().unwrap(), "wss://api.ccode.dev/v1/stt");
        }
    }
    ...
    #[test]
    fn ccode_v1_base_preserves_prefix() {
        let cfg = VoiceConfig {
            api_base: "https://proxy.example.com/ccode/v1".into(),
            ..VoiceConfig::default()
        };
        assert_eq!(
            cfg.stt_ws_url().unwrap(),
            "wss://proxy.example.com/ccode/v1/stt"
        );
    }
    ...
    fn inherits_endpoints_when_voice_api_base_unset() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
ccode_api_base_url = "https://proxy.example.com/ccode/v1"
"#,
        )
        .unwrap();
        let cfg = VoiceConfig::from_config_table(&table, None);
        assert_eq!(cfg.api_base, "https://proxy.example.com/ccode/v1");
        assert_eq!(
            cfg.stt_ws_url().unwrap(),
            "wss://proxy.example.com/ccode/v1/stt"
        );
...
"#,
    ...
    fn table_endpoints_beat_resolved_endpoints_base() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
ccode_api_base_url = "https://config.example.com"
"#,
        )
        .unwrap();
...
ccode_api_base_url = "https://proxy.example.com/ccode/v1"
[voice]
api_base = "https://api.ccode.dev"
language = "es"
"#,
        )
        .unwrap();
        let cfg = VoiceConfig::from_config_table(&table, None);
        assert_eq!(cfg.api_base, "https://api.ccode.dev");
