//! Voice input for Ccode Build CLI: an ccode streaming STT client and the
...
mod intercept_tests {
    ...
    fn capture_subcommand_matches_only_argv1() {
        assert!(is_capture_subcommand(&argv(&["ccode", "__mic-capture"])));
        ...
        assert!(!is_capture_subcommand(&argv(&["ccode"])));
        assert!(!is_capture_subcommand(&argv(&["ccode", "chat"])));
        assert!(!is_capture_subcommand(&argv(&[
            "ccode",
            "chat",
            "__mic-capture"
