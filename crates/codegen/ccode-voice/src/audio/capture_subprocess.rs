...
//! run) — and can be forced with `CCODE_VOICE_CAPTURE=inprocess`.
...
/// Env escape hatch: `CCODE_VOICE_CAPTURE=inprocess` forces the legacy
/// in-process cpal backend (accepting its permanent footprint cost).
const CAPTURE_BACKEND_ENV: &str = "CCODE_VOICE_CAPTURE";
