//! `paste-latency` — clipboard paste latency benchmark for `ccode-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p ccode-pager-pty --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p ccode-pager-pty --bench paste_latency -- \
//!   --binary ~/Downloads/ccode-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for ccode-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
