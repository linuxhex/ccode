//! `pty-bench` — PTY benchmark CLI for `ccode-pager`.
//!
...
//! ```bash
//! PAGER_BINARY=./artifacts/ccode-${VERSION}-linux-x86_64 \
//!   cargo bench -p ccode-pager-pty \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for ccode-pager",
    long_about = None,
)]
struct Cli {
