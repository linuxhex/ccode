...
//!
//! ```bash
//! cargo test -p ccode-pager-pty --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use ccode_pager_pty::{ContentController, PtyExitPoll, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.ccode/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".ccode").join("sessions");
