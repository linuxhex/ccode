...
//! as the active auth (`CCODE_API_KEY` removed) and the rollout forced on via
//! `CCODE_PRIVACY_NOTICE_ROLLOUT=1`.
//!
//! ```bash
//! cargo test -p ccode-pager-pty --test privacy_banner_e2e \
//!   -- --ignored --nocapture
//! ```
...
const ROWS: u16 = 50;
const COLS: u16 = 120;
const BANNER_TITLE: &str = "Help improve Ccode";
...
/// sandbox's fake `CCODE_API_KEY` removed so the seeded opted-out OAuth entry
/// is the active auth — the banner's two preconditions.
fn banner_env_ops() -> [EnvOp<'static>; 2] {
    [
        EnvOp::set("CCODE_PRIVACY_NOTICE_ROLLOUT", "1"),
        EnvOp::remove("CCODE_API_KEY"),
    ]
}
...
/// Poll `<home>/.ccode/config.toml` for the async `privacy_banner_acked`
...
fn wait_for_ack_on_disk(pager: &mut PtyHarness, home: &Path, timeout: Duration) -> Result<()> {
    let path = home.join(".ccode").join("config.toml");
