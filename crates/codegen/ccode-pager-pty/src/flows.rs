...
/// Seed a fake ccode OAuth entry into the isolated home's `auth.json` so the
/// shell has session auth (the harness's `CCODE_API_KEY` is ApiKey/BYOK mode
...
fn seed_fake_oauth_with_opt_out(content: &ContentController, user: &str, opted_out: bool) {
    ...
    std::fs::write(
        ...
        format!(
            r#"{{
  "https://auth.ccode.dev::b1a00492-073a-47ea-816f-4c329264a828": {{
...
pub fn oauth_credential_ops() -> [crate::EnvOp<'static>; 1] {
    [crate::EnvOp::remove("CCODE_API_KEY")]
}
