//! Unified PTY harness for ccode-pager.
//!
...
//!    `ccode-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `CCODE_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// (XTVERSION probes and ccode wrap). Content-backed launches must use
    /// [`Self::new_in_sandbox`].
    ...
    /// Spawn from a canonical [`ccode_test_support::TestSandbox`] baseline
    ...
    pub fn new_in_sandbox(
        ...
        cols: u16,
        args: &[&str],
        sandbox: &ccode_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use ccode_pager_pty::{PtyHarness, ContentController, pager_binary};
    ...
    /// through ccode's stdout. Used to reproduce the doubled-line class of bugs
    /// where ccode's diff renderer never re-asserts a region it didn't write
