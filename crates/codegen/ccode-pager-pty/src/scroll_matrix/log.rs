//! `CCODE_SCROLL_LOG` JSONL parsing, per-stream grouping, and finalize
//! synchronization.
//!
//! Wire schema source of truth: the pager's `ScrollLogRecord` in
//! `ccode-pager/src/input/scroll_log.rs`. [`ScrollLogLine`] mirrors it
...
/// Parse a `CCODE_SCROLL_LOG` JSONL file into records.
///
...
/// session (`CCODE_SCROLL_LOG` set at spawn): `stream_start` → `flush`* →
...
mod tests {
    ...
    // in `ccode-pager/src/input/mouse/tests.rs`.
