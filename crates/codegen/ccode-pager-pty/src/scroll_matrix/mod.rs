...
//! `CCODE_SCROLL_LOG` pointed at a tempfile, then validates the pager's
//! flight-recorder JSONL (producer:
//! `ccode-pager/src/input/scroll_log.rs`) against gesture invariants.
//!
...
//! producer-side twin lives in `ccode-pager/src/input/mouse/tests.rs`
