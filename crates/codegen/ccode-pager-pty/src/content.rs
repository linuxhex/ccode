...
//! [`MockInferenceServer`] from `ccode-test-support` and provides the
...
use ccode_test_support::{MockInferenceServer, TestSandbox};

pub use ccode_test_support::mock_server::LogEntry;
pub use ccode_test_support::mock_server::MockModelEntry as MockModel;
pub use ccode_test_support::mock_server::StorageUpload;
pub use ccode_test_support::sse;
pub use ccode_test_support::{
...
impl ContentController {
    ...
    pub async fn start_with_models(models: Vec<MockModel>) -> Result<Self> {
        ...
        sandbox.set_env("CCODE_PROMPT_SUGGESTIONS", "false");
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.ccode
...
mod tests {
    ...
    async fn read_foreground(url: String, endpoint: InferenceEndpoint) -> String {
        ...
        reqwest::Client::new()
            ...
            .header("x-ccode-turn-idx", "1")
            .header("x-ccode-req-id", format!("direct-{endpoint:?}"))
            .json(&body)
            .send()
            .await
    ...
    /// on the Premium upsell screen and breaks every PTY test.
    #[tokio::test]
