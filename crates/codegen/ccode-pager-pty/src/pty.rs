...
use ccode_test_support::{TestProcessTree, TestSandbox, process_has_exited_without_reap};
...
impl PtyController {
    /// Inherit the parent environment for terminal-brand probes, ccode-wrap
    ...
    fn spawn_inner(
        ...
        let process_tree = process_pid.map(|pid| TestProcessTree::attach(pid, "ccode PTY child"));
...
const CLIPBOARD_SINK_ENV_VARS: &[&str] = &["CCODE_OSC52_SINK", "LC_CCODE_OSC52_SINK"];

/// Host terminal identity markers stripped from the child environment.
///
/// The pager's terminal detection
/// (`ccode-pager-render/src/terminal/mod.rs`:
...
/// [`ccode_test_support::TestSandbox`], which always clears inheritance.
/// The explicitly named inherited-env path is reserved for terminal probing and
/// ccode-wrap fixtures. Caller overrides are always applied last.
fn apply_child_env(cmd: &mut CommandBuilder, sandbox: Option<&TestSandbox>, env: &[EnvOp<'_>]) {
    ...
    // A harness launched under `ccode wrap` must not silently confirm clipboard
...
mod tests {
    ...
    fn apply_child_env_strips_all_host_terminal_markers() {
        ...
        cmd.env("CCODE_SCROLL_LOG", "/tmp/scroll.jsonl");
        let sandbox = TestSandbox::new();
        ...
        assert_eq!(
            cmd.get_env("CCODE_SCROLL_LOG").and_then(|v| v.to_str()),
            None,
            "hermetic baseline must remove unrelated inherited vars"
        );
        assert_eq!(
            cmd.get_env("CCODE_HOME").and_then(|v| v.to_str()),
            sandbox.ccode_home().to_str()
        );
    }
    ...
    fn apply_child_env_uses_sandbox_baseline() {
        ...
        assert_eq!(cmd.get_env("CCODE_LEADER_SOCKET"), None);
    }
    ...
    fn apply_child_env_remove_deletes_sandbox_credential() {
        ...
        apply_child_env(&mut cmd, Some(&sandbox), &[EnvOp::remove("CCODE_API_KEY")]);

        assert_eq!(cmd.get_env("CCODE_API_KEY"), None);
        assert_eq!(
            cmd.get_env("CCODE_CCODE_API_BASE_URL")
                .and_then(|v| v.to_str()),
    ...
    fn apply_child_env_caller_env_overrides_survive_strips() {
        ...
        assert_eq!(
            cmd.get_env("CCODE_OSC52_SINK").and_then(|v| v.to_str()),
            Some("1"),
