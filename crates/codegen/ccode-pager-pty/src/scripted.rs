//! Scripted TUI scenario runner for ccode-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$CCODE_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&ccode_home)
                .with_context(|| format!("create scenario CCODE_HOME {}", ccode_home.display()))?;
            fs::write(ccode_home.join("config.toml"), config_toml)
...
mod tests {
    ...
    fn workspace_rejects_paths_outside_the_tempdir() {
        ...
        let sandbox = ccode_test_support::TestSandbox::new();
