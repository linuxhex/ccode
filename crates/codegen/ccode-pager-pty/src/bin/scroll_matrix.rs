//! `scroll-matrix` — scroll validation matrix sweep for ccode-pager.
//!
...
use ccode_pager_pty::pager_binary;
use ccode_pager_pty::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against ccode-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_ccode-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves CCODE_SCROLL_LOG against ITS cwd (the
