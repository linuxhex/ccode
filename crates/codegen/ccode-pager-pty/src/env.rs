...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build ccode-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build ccode-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_ccode-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p ccode-pager-bin` (the composition-
///    root package that owns the `ccode-pager` binary)
