...
//! elected leader) plus a fixed leader socket beneath its `CCODE_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `CCODE_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared CCODE_HOME => one leader; the socket lives beneath it so
        // every client (sharing the same env) elects/attaches to the same one.
        let ccode_home = content.home().join(".ccode");
        std::fs::create_dir_all(&ccode_home).context("create ccode home")?;
        let socket = ccode_home.join("leader-e2e.sock");
    ...
    /// The cluster's sessions root: `CCODE_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".ccode").join("sessions")
    }
...
mod tests {
    ...
    fn envelope(update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_ccode.dev/session/update","params":{{"sessionId":"s","update":{update_json}}}}}"#
        )
    }
    ...
    fn parse_update_payloads_unwraps_and_tolerates_torn_trailing_line() {
        let body = format!(
            "{}\n{}\n{{\"timestamp\":2,\"method\":\"_ccode.dev/sess",
            envelope(
