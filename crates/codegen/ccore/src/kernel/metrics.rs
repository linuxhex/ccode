...
//! ## 功能
//!
//! - 性能指标：吞吐量、延迟分布（RwLock 读并发）、错误率
//! - 资源使用：内存池使用率、连接数
//! - 健康检查：Node 心跳超时检测、队列水位
//! - 背压流控：令牌桶 + 水位线，超阈值自动降速
//!
//! ## 并发模型
//!
//! - `latencies`: RwLock<HashMap>，读并发零竞争，写短持锁
//! - `node_heartbeats`: RwLock<HashMap>，同上
//! - `global()`: OnceLock 懒初始化，减少启动时构造
