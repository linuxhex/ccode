...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        session_handler_resolver: Option<ccode_hub_sdk::SessionHandlerResolver>,
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(ccode_version::VERSION)
...
struct CallCompletedGuard {
    ...
    outcome: ccode_fs::events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: ccode_fs::events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: ccode_fs::events::ToolOutcome) {
...
impl ToolServerHandler for SessionRoutedToolHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let hub_session = ctx
            .extensions
            .get::<ccode_tool_runtime::SessionContext>()
            .map(|s| s.0.clone());
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        tool = %self.name(),
                        session = %session_id,
                        "CCODE_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    ToolStreamItem::Terminal(Ok(run_result)) => {
                        ...
                        _guard.set_outcome(ccode_fs::events::ToolOutcome::Success);
                    ...
                    ToolStreamItem::Terminal(Err(e)) => {
                        ...
                        _guard.set_outcome(ccode_fs::events::ToolOutcome::Error);
...
fn parse_server_id(id: &str) -> Result<ccode_tool_protocol::ServerId, ClientError> {
    ccode_tool_protocol::ServerId::new(id)
...
mod tests {
    ...
    use futures::StreamExt;
    use ccode_tool_runtime::{SessionContext, ToolCallId};
    ...
    async fn handle_call_terminal_matches_non_streaming_call() {
        ...
        match reference {
            Ok(run_result) => {
                ...
                assert_eq!(
                    typed.model_output,
                    vec![ccode_tool_runtime::ContentBlock::Text {
    ...
    use ccode_tools::types::tool_metadata::ToolMetadata as CcodeToolMetadata;
    #[derive(Debug)]
    struct GateStreamingStub;
    impl CcodeToolMetadata for GateStreamingStub {
    ...
    impl ccode_tool_runtime::Tool for GateStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> ccode_tool_protocol::ToolId {
            ccode_tool_protocol::ToolId::new("gate_streaming_stub").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::ccode_tool_runtime::ListToolsContext,
        ) -> ccode_tool_types::ToolDescription {
            ccode_tool_types::ToolDescription::new("gate_streaming_stub", "gate streaming stub")
        }
        async fn run(
            &self,
        ...
        async fn execute(
            ...
            use ccode_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    async fn drain_counts<T>(mut stream: ccode_tool_runtime::ToolStream<T>) -> (usize, usize, bool) {
        let mut progress = 0;
        let mut terminal = 0;
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("CcodeBuild:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    async fn wait_until(
        ...
        pred: impl Fn(&ccode_tool_protocol::ToolServerStatusPayload) -> bool,
        timeout: Duration,
    ) -> ccode_tool_protocol::ToolServerStatusPayload {
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use ccode_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use ccode_tools::computer::types::{TaskKind, TaskSnapshot};
