...
//! [`crate::trust::TrustStore`] (`~/.ccode/trusted_folders.toml`). The
//! consume/gating half (the `DECISIONS` cache, `resolve_and_record`,
//! `project_scope_allowed`, the loader filters) lives in `ccode-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `ccode-shell`
...
use ccode_config_types::{BoolFlag, RemoteSettings};
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.ccode`).
...
/// this binary — true on a local/dev build (no `CCODE_VERSION` release stamp).
///
/// THE single security short-circuit: every explicit trust auto-grant site calls
/// this (greppable via `folder_trust_inert`). When true a self-built ccode never
...
/// (`CCODE_VERSION` unset at compile time) — i.e. a local/dev build.
///
/// Kept local (not in `ccode-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned CCODE_TEST_VERSION simulates a release build,
    // so tests/CI (which run unstamped, i.e. local-looking) can exercise the gate.
    if std::env::var(ccode_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
    option_env!("CCODE_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `CCODE_VERSION` release stamp) the feature is OFF
/// regardless of env/config/remote — a self-built ccode auto-trusts (never
...
/// env `CCODE_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    // shipped, release-stamped binaries. Even an explicit CCODE_FOLDER_TRUST/config
    // opt-in is ignored here so a self-built ccode never prompts.
    if is_local_build {
        return false;
    }
    ...
    let user = ccode_config::load_from_disk().ok();
    let managed = ccode_config::load_managed_config().ok();
    BoolFlag::env("CCODE_FOLDER_TRUST")
...
/// same `~/.ccode/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `[permission]` / `.claude` / `.ccode/agents` / subdir-launch gaps. NOT itself
/// the trust gate.
pub fn repo_config_kinds(cwd: &Path) -> Vec<&'static str> {
    collect_repo_config_kinds(cwd, false)
}

/// Whether a project `.ccode/config.toml` `[permission]` value would contribute
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = ccode_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.ccode/config.toml` declaring repo-controlled code-exec or
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = ccode_config::load_config_file(&path) else {
            continue;
        };
    ...
    // Project `.ccode/lsp.json`.
    if cwd.join(".ccode").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.ccode/hooks/evil.json`) would
    ...
    if path_present_or_uncertain(&hook_root.join(".ccode").join("hooks"))
    ...
    // that must be gated — else a plugin clone (e.g. `.ccode/plugins/evil/`, even
    // one in a subdir launched via `cd sub && ccode`) would resolve trusted and
    ...
    if !ccode_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.ccode/agents` / `.claude/agents`): a project agent
    ...
    if !ccode_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
    // Presence matches exact-cwd discovery without parsing repository content.
    let ccode = cwd.join(".ccode");
    if directory_present_or_uncertain(&ccode.join("roles")) {
        hit!("roles");
    }
    if directory_present_or_uncertain(&ccode.join("personas")) {
        hit!("personas");
    }
    if directory_present_or_uncertain(&hook_root.join(".ccode").join("workflows")) {
        hit!("workflows");
    }
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .ccode/lsp.json / hooks) \
...
mod tests {
    ...
    #[test]
    fn repo_configs_present_detects_ccode_config_mcp_servers() {
        let tmp = repo_tmp();
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(ccode.join("config.toml"), "[mcp_servers.x]\ncommand=\"y\"\n").unwrap();
        assert!(repo_configs_present(tmp.path()));
    }

    #[test]
    fn repo_configs_present_detects_ccode_lsp_json() {
        let tmp = repo_tmp();
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(ccode.join("lsp.json"), "{}").unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.ccode/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_roles() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode").join("roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_personas() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode").join("personas")).unwrap();
    ...
    fn project_subagent_marker_regular_file_is_absent() {
        ...
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(ccode.join("roles"), "not a directory").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }
    ...
    fn project_subagent_marker_at_repo_root_is_absent_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode/roles")).unwrap();
    ...
    fn project_subagent_marker_symlink_to_directory_is_present() {
        ...
        std::fs::create_dir_all(&ccode).unwrap();
        std::os::unix::fs::symlink(&target, ccode.join("roles")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn dangling_project_subagent_marker_is_absent() {
        ...
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::os::unix::fs::symlink("missing", ccode.join("personas")).unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_workflows_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode").join("workflows")).unwrap();
        let subdir = tmp.path().join("crates").join("inner");
        std::fs::create_dir_all(&subdir).unwrap();
        assert!(repo_configs_present(&subdir));
    ...
    fn repo_configs_present_detects_project_hooks_file() {
        ...
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(ccode.join("hooks"), "{}").unwrap();
    ...
    fn repo_configs_present_detects_dangling_project_hooks_symlink() {
        ...
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::os::unix::fs::symlink("missing-hooks", ccode.join("hooks")).unwrap();
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ccode").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".ccode").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_configs_present_false_for_empty_mcp_servers_table() {
        ...
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(&ccode).unwrap();
        std::fs::write(ccode.join("config.toml"), "[mcp_servers]\n").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }

    #[test]
    fn repo_configs_present_detects_ccode_config_plugins_paths() {
    ...
    #[test]
    fn repo_configs_present_detects_ccode_config_permission() {
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.ccode/agents` — even when launched from a SUBDIR (the cwd→git-root walk
        // that `first_only` shares). Guards against silent drift between the two.
        let tmp = repo_tmp();
        let ccode = tmp.path().join(".ccode");
        std::fs::create_dir_all(ccode.join("agents")).unwrap();
        std::fs::write(ccode.join("config.toml"), "[plugins]\npaths = [\"./x\"]\n").unwrap();
    ...
    // CCODE_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    // tests (the workspace crate has no `serial_test`/`ccode-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(ccode_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("CCODE_HOME", home.path());
        let _flag = EnvVarGuard::unset("CCODE_FOLDER_TRUST");
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty CCODE_HOME (no
        // config.toml/managed_config.toml) + CCODE_FOLDER_TRUST unset. nextest's
        // process-per-test makes ccode_home()'s OnceLock pick up the temp dir.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("CCODE_HOME", home.path());
        let _flag = EnvVarGuard::unset("CCODE_FOLDER_TRUST");
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // CCODE_FOLDER_TRUST=1 does NOT enable the feature (so a self-built ccode
        // never prompts). CCODE_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("CCODE_HOME", home.path());
        let _flag = EnvVarGuard::set("CCODE_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty CCODE_HOME (no config.toml/managed config) +
        // CCODE_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("CCODE_HOME", home.path());
        let _flag = EnvVarGuard::unset("CCODE_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned CCODE_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(ccode_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no CCODE_VERSION) is a local build.
        // Guard to the unstamped case so a release-stamped test binary (CI release)
        // doesn't spuriously fail this arm.
        let _unset = EnvVarGuard::unset(ccode_version::TEST_VERSION_ENV);
        if option_env!("CCODE_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); CCODE_HOME-isolated
        // and ENV_LOCK-serialized so toggling CCODE_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("CCODE_HOME", home.path());
        let _unset = EnvVarGuard::unset(ccode_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // CCODE_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("CCODE_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // CCODE_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("CCODE_HOME", home.path());
        let _sim = simulate_release_build();
