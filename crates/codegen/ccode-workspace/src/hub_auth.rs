//! Hub [`AuthProvider`] from `~/.ccode/auth.json` for the standalone
...
//!
//! The in-leader `ccode workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let ccode = ccode_config::user_ccode_home()
        .ok_or_else(|| anyhow::anyhow!("no user ccode home (set $CCODE_HOME or $HOME)"))?;
    Ok(ccode.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `ccode login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `ccode login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.ccode/auth.json`).
pub fn provider(
    hub_url: &Url,
...
mod tests {
    ...
    fn read_auth_entry_picks_oidc_entry() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "legacy": { "key": "ccode-plainkey", "user_id": "u1" },
            "oidc": {
    ...
    fn read_auth_entry_rejects_non_oidc() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "api_key": { "key": "ccode-plainkey", "user_id": "u1" }
        }"#,
        );
    ...
    fn read_auth_entry_tolerates_extra_fields() {
        ...
        let path = write_auth_json(
            ...
            "scope": {
                ...
                "email": "test@ccode.dev",
                "first_name": "Test",
                "refresh_token": "rt1",
                "oidc_issuer": "https://auth.ccode.dev",
                "oidc_client_id": "c1",
    ...
    fn build_oidc_provider_requires_refresh_token() {
        let entry = AuthEntry {
            ...
            user_id: "u1".into(),
            refresh_token: None,
            oidc_issuer: Some("https://auth.ccode.dev".into()),
    ...
    fn write_refreshed_token_updates_jwt_entry() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "legacy": { "key": "ccode-old", "user_id": "u1" },
            "oidc": { "key": "eyJ.old", "user_id": "u2", "refresh_token": "rt-old", "oidc_issuer": "https://auth.ccode.dev" }
        }"#,
        );
        ...
        assert_eq!(updated["legacy"]["key"], "ccode-old"); // untouched
    }
    ...
    fn write_refreshed_token_targets_exact_scope_key() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "zzz": { "key": "eyJ.z", "refresh_token": "rt-z", "oidc_issuer": "https://auth.ccode.dev" },
            "aaa": { "key": "eyJ.a", "refresh_token": "rt-a", "oidc_issuer": "https://auth.ccode.dev" }
        }"#,
        );
    ...
    fn write_refreshed_token_preserves_existing_rt_when_not_rotated() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "oidc": { "key": "eyJ.old", "user_id": "u1", "refresh_token": "rt-keep", "oidc_issuer": "https://auth.ccode.dev" }
        }"#,
        );
    ...
    fn provider_loopback_uses_bearer() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{ "oidc": { "key": "eyJ.tok", "user_id": "u1", "refresh_token": "rt", "oidc_issuer": "https://auth.ccode.dev", "oidc_client_id": "c1" } }"#,
        );
