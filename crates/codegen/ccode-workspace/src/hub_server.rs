...
use ccode_hub_sdk::ToolServerHandler;
use ccode_tools::computer::types::TaskKind;
use ccode_tools::implementations::ccode_build::scheduler::interval::interval_to_human;
use ccode_tools::implementations::ccode_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use ccode_tools::registry::types::FinalizedToolset;
use ccode_tools::types::resources::Terminal;
use ccode_workspace_types::rpc::workspace::{
    BackgroundTaskSnapshotWire, ScheduledTaskSnapshotWire, TasksSnapshotResponse,
};
use ccode_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ccode_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ccode_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_REQUESTS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ccode_workspace_rpc_requests_total",
...
static WORKSPACE_RPC_ERRORS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ccode_workspace_rpc_errors_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "ccode_workspace_rpc_duration_seconds",
...
) -> Vec<ccode_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use ccode_tools::computer::types::TaskKind;
    use ccode_tools::types::resources::Terminal;
    use ccode_tools::types::tool::ToolKind;
    use ccode_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &ccode_tools::registry::types::FinalizedToolset,
) -> Vec<ccode_workspace_types::rpc::workspace::TodoSummaryWire> {
    use ccode_tools::implementations::ccode_build::todo::{TodoState, TodoStatus};
    use ccode_tools::types::resources::State;
    use ccode_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use ccode_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use ccode_workspace_types::rpc::search::FuzzyStatusReq;
        use ccode_workspace_types::rpc::skills::DiscoverPluginsReq;
        use ccode_workspace_types::rpc::workspace::{
        ...
        use ccode_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<ccode_tool_runtime::SessionContext>();
    ...
    async fn handle_hook(&self, session_id: SessionId, frame: HookFrame) {
        match frame.event {
            ...
            HookEvent::Custom { kind, payload } => {
                use ccode_tool_protocol::turn_hook::{
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use ccode_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    ) -> Option<ccode_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use ccode_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use ccode_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                ...
                Some(ccode_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(ccode_tool_runtime::SessionContext("main".to_owned()));
    ...
    async fn handle_call_records_rpc_metrics_and_collapses_unknown_method() {
        ...
        let has_bogus_series = prometheus::gather()
            .iter()
            .filter(|mf| mf.name() == "ccode_workspace_rpc_requests_total")
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use ccode_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();
