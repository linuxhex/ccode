...
use ccode_hub_sdk::ToolHarness;
use ccode_tools::types::output::ToolRunResult;
use ccode_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use ccode_workspace_types::rpc::WorkspaceRpc;
pub use ccode_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use ccode_workspace_types::rpc::code_nav::{
...
pub use ccode_workspace_types::rpc::fs::{
...
pub use ccode_workspace_types::rpc::git::{
...
pub use ccode_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use ccode_workspace_types::rpc::hunks::{
...
pub use ccode_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use ccode_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use ccode_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use ccode_workspace_types::rpc::workspace::WorkspaceInfoReq;
...
fn hunk_line_info_to_wire(info: &ccode_hunk::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: ccode_hunk::types::HunkSource) -> HunkSourceWire {
    use ccode_hunk::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &ccode_hunk::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: ccode_hunk::types::FileContentStatus,
) -> FileContentStatusWire {
    use ccode_hunk::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: ccode_hunk::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: ccode_hunk::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &ccode_hunk::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: ccode_hunk::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: ccode_hunk::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> ccode_hunk::types::HunkAction {
    match kind {
        HunkActionKind::Accept => ccode_hunk::types::HunkAction::Accept,
        HunkActionKind::Reject => ccode_hunk::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<ccode_hunk::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = ccode_hunk::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                ccode_hunk::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &ccode_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<ccode_hooks::discovery::HookRegistry> {
    let value = serde_json::to_value(wire).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
    let mut registry: ccode_hooks::discovery::HookRegistry =
        serde_json::from_value(value).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<ccode_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<ccode_graph::QueryResult, ccode_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<ccode_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](ccode_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: ccode_hunk::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<ccode_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](ccode_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(ccode_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = ccode_tool_protocol::ToolId::new(name).map_err(|e| {
                    ccode_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                        format!("invalid tool name: {e}"),
                    )
                })?;
                let mut ctx = ccode_tool_runtime::ToolCallContext::default();
                ctx.call_id =
                    ccode_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
...
mod tests {
    ...
    /// request types are defined in `ccode-workspace-types` (and re-exported
    ...
    fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/ccode-main");
        let window_b = std::path::PathBuf::from("/repos/ccode-main-2");
        assert_eq!(
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            ccode_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    fn hunk_to_wire_serializes_identically() {
        use ccode_hunk::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use ccode_hunk::FileContentEntry;
        use ccode_hunk::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use std::sync::Arc;
        use ccode_hunk::SessionSummary;
        use ccode_hunk::types::{Hunk, HunkSource, TurnSummary};
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = ccode_hooks::config::HookSpec {
            ...
            event: ccode_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.ccode/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = ccode_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use ccode_hooks::event::HookEventName as E;
    ...
    /// can't depend on `ccode-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use ccode_hooks::config::HookSpec;
