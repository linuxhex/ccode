...
use dashmap::DashMap;
use ccode_fs::events::{Event, EventWriter, ToolOutcome};
use ccode_fs::queue::UploadQueueStats;
use ccode_tool_protocol::{ToolServerLifecycleStatus, ToolServerStatusPayload};
...
/// `CCODE_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub struct ActivityTracker {
    ...
    /// `CCODE_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    /// idle-hold cap comes from `CCODE_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`
    ...
    pub fn snapshot_session(&self, session_id: &str) -> ToolServerStatusPayload {
        ...
        ToolServerStatusPayload {
            status,
            session_id: ccode_tool_protocol::SessionId::new(session_id).ok(),
            connection_id: None,
...
/// The durability idle-hold cap from `CCODE_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
fn durability_idle_hold_max_from_env() -> u64 {
    durability_idle_hold_from_raw(std::env::var("CCODE_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS").ok())
}
