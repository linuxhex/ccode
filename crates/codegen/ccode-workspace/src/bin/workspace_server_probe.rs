...
//! access token from `~/.ccode/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use ccode_hub_sdk::pool::HubConnectionPool;
use ccode_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &ccode_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<ccode_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "CcodeBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
