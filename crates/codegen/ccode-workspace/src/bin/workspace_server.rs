...
//!
//! Reads OIDC credentials from `~/.ccode/auth.json`, connects to a
...
use url::Url;
use ccode_workspace::config::WorkspaceServerMetadata;
use ccode_workspace::daemonize;
use ccode_workspace::diag_server::{self, DiagHandle, ErrorClass};
use ccode_workspace::error::WorkspaceError;
use ccode_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_ccode_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<ccode_tool_protocol::ServerId>()
        .err()
...
#[derive(Parser)]
#[command(name = "ccode-workspace-server")]
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.ccode.dev/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `CCODE_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "CCODE_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.ccode/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "CCODE_WORKSPACE_PROJECT_LSP_TRUSTED",
        default_value_t = false,
        action = clap::ArgAction::Set,
    )]
    project_lsp_trusted: bool,
    /// Confine `ccode.dev/fs/*` resolution to the workspace root (reject `..`,
    ...
    /// Override with `CCODE_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "CCODE_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/ccode-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = ccode_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("CCODE_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match ccode_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use ccode_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("CCODE_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(value = %val,
                        "Unrecognized CCODE_SANDBOX_PROFILE, defaulting to workspace");
                    ProfileName::Workspace
                } else {
                    parsed
                }
            }
            None if ccode_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let profile_name = profile.to_string();
        if profile == ProfileName::Off {
            tracing::info!(profile = %profile_name, "Sandbox explicitly disabled via CCODE_SANDBOX_PROFILE=off");
        } else {
            ...
            sandbox.install();
            let active = ccode_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = ccode_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let auth_provider = ccode_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(
        hub_url = %url,
        cwd = %cwd.display(),
    ...
    let session_id = std::env::var("CCODE_SESSION_ID").ok();
    ...
    ccode_workspace::init_metrics();
    let ws_handle = match ccode_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = ccode_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            ccode_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    fn classify_from_client_error_display_round_trip() {
        let handshake = WorkspaceError::HubError(
            ccode_hub_sdk::ClientError::HandshakeAuthFailed { status: 401 }.to_string(),
        );
        ...
        let auth = WorkspaceError::HubError(
            ccode_hub_sdk::ClientError::AuthError("token rejected".into()).to_string(),
        );
        assert_eq!(
        ...
        let network = WorkspaceError::HubError(
            ccode_hub_sdk::ClientError::NetworkError("connection refused".into())
                .to_string(),
        );
        assert_eq!(
    ...
    fn capabilities_flag_parses_and_defaults_off() {
        let args = Args::try_parse_from(["ccode-workspace-server"]).unwrap();
        assert!(!args.capabilities);
        let args = Args::try_parse_from(["ccode-workspace-server", "--capabilities"]).unwrap();
        assert!(args.capabilities);
    }
    #[test]
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("CCODE_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["ccode-workspace-server"]).unwrap();
        assert!(!args.project_lsp_trusted);
        let args = Args::try_parse_from(["ccode-workspace-server", "--project-lsp-trusted", "true"])
            .unwrap();
        assert!(args.project_lsp_trusted);
    }
    #[test]
    ...
    fn capabilities_probe_of_legacy_binary_exits_nonzero() {
        ...
        let err = LegacyArgs::try_parse_from(["ccode-workspace-server", "--capabilities"])
    ...
    fn ready_file_is_accepted_as_a_deprecated_no_op() {
        let args =
            Args::try_parse_from(["ccode-workspace-server", "--ready-file", "/tmp/x.ready"]).unwrap();
    ...
    fn argv_rejection_exit_code_is_distinct_from_server_id_exit_code() {
        let err = Args::try_parse_from(["ccode-workspace-server", "--flag-from-the-future"])
            .err()
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        ...
        assert_eq!(
            ...
            Some("https://ccode.dev/preview-auth")
        );
