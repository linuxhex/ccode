...
use std::sync::Arc;
use ccode_hub_mcp::McpBridgeHandle;
use ccode_mcp::servers::McpState;
use ccode_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use ccode_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
use ccode_hunk::HunkTrackerHandle;
use ccode_tool_protocol::ToolId;
use ccode_tool_runtime::WorkspaceViewerContext;
...
pub struct WorkspaceSession {
    ...
    ///
    /// [`HunkTrackerActor`]: ccode_hunk::HunkTrackerActor
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, ccode_hunk::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<ccode_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<ccode_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<ccode_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<ccode_hooks::error::HookError>,
    ...
    /// Auth provider for ccode service calls.
    pub(crate) auth_provider: Option<ccode_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<ccode_tools::notification::types::ToolNotificationHandle>>,
    /// Sink for workspace-originated ext-notifications to the client (e.g.
    /// `ccode.dev/search/fuzzy/status`). Mode-agnostic: the shell wires it to the
    ...
    pub(crate) local_registry: ccode_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn ccode_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `CCODE_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$CCODE_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<ccode_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<ccode_fs::queue::UploadQueue>>,
    ...
    /// (`CCODE_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](ccode_fs::events::EventWriter::noop) and
    ...
    /// enabled (`CCODE_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, ccode_fs::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<ccode_fs::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<ccode_fs::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](ccode_fs::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<ccode_fs::events::EventWriter> {
    ...
    /// Auth provider used for ccode service calls.
    pub fn auth_provider(&self) -> Option<&ccode_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](ccode_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<ccode_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, ccode_fs::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> ccode_fs::events::EventWriter {
...
mod tests {
    ...
    use dashmap::DashMap;
    use ccode_fs::events::{Event, EventWriter};
