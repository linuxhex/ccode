...
/// Directory (under `<cwd>/.ccode`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.ccode/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
