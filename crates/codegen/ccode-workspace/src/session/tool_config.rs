...
use std::sync::Arc;
use ccode_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<ccode_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<ccode_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<ccode_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<ccode_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn ccode_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            ccode_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`CCODE_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("CCODE_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = ccode_fs::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// `CCODE_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: ccode_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<ccode_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$CCODE_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    pub fn with_auth(auth: ccode_hub_sdk::SharedAuthProvider, api_base_url: String) -> Self {
        Self {
            auth: Some(auth),
    ...
    /// (`$CCODE_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(ccode_tools::computer::local::LocalFs)
            as Arc<dyn ccode_tools::computer::types::AsyncFileSystem>;
        let notification_handle = ccode_tools::notification::ToolNotificationHandle::noop();
        let (image_gen_config, video_gen_config, web_search_config, app_builder_deployer_config) =
            if let (Some(auth), Some(url)) = (&self.auth, &self.api_base_url) {
                ...
                match cred {
                    ccode_hub_sdk::AuthCredential::Bearer { token, .. } => {
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            ccode_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = ccode_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("ccode-workspace/{version}"),
    );
    headers.insert("x-ccode-client-version".to_string(), version.to_string());
    headers.insert(
        "x-ccode-client-identifier".to_string(),
        std::env::var("CCODE_CLIENT_NAME").unwrap_or_else(|_| "ccode-shell".to_string()),
    );
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-Ccode-Token-Auth".to_string(), "ccode-cli".to_string());
        headers.insert(
...
/// `CCODE_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> ccode_tools::implementations::ccode_build::web_fetch::WebFetchConfig
{
    use ccode_tools::implementations::ccode_build::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("CCODE_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("CCODE_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    if ccode_config::env_bool("CCODE_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
        params.allow_local = Some(true);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("CCODE_WEB_SEARCH_MODEL").unwrap_or_else(|_| "ccode-4.20-multi-agent".to_string())
}
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("CcodeBuild:search_replace", Some(ToolKind::Edit)),
                tc("CcodeBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("CcodeBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("CcodeBuild:search_replace".to_owned(), ToolKind::Edit),
            ("CcodeBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("CcodeBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                // Pre-set kinds must never be overwritten by the registry.
                test_support::tc("CcodeBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("CcodeBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("CcodeBuild:read_file", None),
                test_support::tc("CcodeBuild:grep", None),
                test_support::tc("CcodeBuild:list_dir", None),
                test_support::tc("CcodeBuild:search_replace", None),
                test_support::tc("CcodeBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("CcodeBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"CcodeBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "CCODE_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use ccode_tools::types::resources::{State, WebCitationCounter};
