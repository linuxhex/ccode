...
pub async fn git_cli(cwd: &Path, args: &[&str]) -> Result<String> {
    ...
    for &(key, val) in ccode_tty::GIT_AUTH_SUPPRESSION_ENVS.iter() {
        cmd.env(key, val);
    }
...
///
/// ```
/// use ccode_workspace::session::git::normalize_repo_url;
///
/// assert_eq!(
...
/// Payload for the `ccode.dev/git_head_changed` ACP extension notification.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub const RESTORE_CODE_LOG: &str = "ccode_restore_code";
...
///
/// 1. `supplied_cwd` is a ccode-managed worktree (`~/.ccode/worktrees/...`).
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = ccode_tools::util::ccode_home::ccode_home().join("worktrees");
...
const REWIND_GIT_ENV: &str = "CCODE_WORKSPACE_REWIND_GIT";
...
pub fn git_rewind_enabled() -> bool {
    ccode_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
...
mod tests {
    ...
    fn strip_url_credentials_removes_token() {
        let url_with_token = "https://x-access-token:secret-token@github.com/ccode-org/example.git";
        assert_eq!(
            strip_url_credentials(url_with_token),
            "https://github.com/ccode-org/example.git"
        );
    }
    #[test]
    fn strip_url_credentials_preserves_clean_https_url() {
        let clean_url = "https://github.com/ccode-org/example.git";
        assert_eq!(strip_url_credentials(clean_url), clean_url);
    }
    #[test]
    fn strip_url_credentials_preserves_ssh_url() {
        let ssh_url = "git@github.com:ccode-org/example.git";
        assert_eq!(strip_url_credentials(ssh_url), ssh_url);
    }
    #[test]
    fn strip_url_credentials_removes_username_password() {
        let url_with_creds = "https://alice:secret@github.com/ccode-org/example.git";
        assert_eq!(
    ...
    fn test_resolve_persisted_session_git_metadata_collects_sorted_unique_remotes() {
        ...
        repo.remote(
            "origin",
            "https://x-access-token:secret-token@github.com/ccode-org/example.git",
        )
        .unwrap();
        repo.remote("backup", "https://gitlab.com/ccode-org/example.git")
            .unwrap();
        repo.remote("duplicate", "https://github.com/ccode-org/example.git")
            .unwrap();
        ...
        assert_eq!(
            ...
            vec![
                "https://github.com/ccode-org/example.git".to_string(),
                "https://gitlab.com/ccode-org/example.git".to_string(),
            ]
        );
    }
    #[test]
    fn test_resolve_persisted_session_git_metadata_captures_head() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = git2::Repository::init(tmp.path()).unwrap();
        repo.remote("origin", "https://github.com/ccode-org/example.git")
            .unwrap();
        let metadata = resolve_persisted_session_git_metadata_sync(tmp.path());
    ...
    fn test_resolve_persisted_session_git_metadata_worktree_resolves_remotes() {
        ...
        assert_eq!(
            metadata.git_remotes,
            vec!["https://github.com/ccode-org/example.git".to_string()],
        );
    }
    #[test]
    ...
    fn test_effective_worktree_cwd_empty_offset() {
        let result =
            effective_worktree_cwd("/home/user/.ccode/worktrees/repo/ab-123-a", Path::new(""));
        assert_eq!(result, "/home/user/.ccode/worktrees/repo/ab-123-a");
    }
    #[test]
    fn test_effective_worktree_cwd_single_level_offset() {
        let result =
            effective_worktree_cwd("/home/user/.ccode/worktrees/repo/ab-123-a", Path::new("src"));
        assert_eq!(result, "/home/user/.ccode/worktrees/repo/ab-123-a/src");
    }
    #[test]
    fn test_effective_worktree_cwd_nested_offset() {
        let result = effective_worktree_cwd(
            "/home/user/.ccode/worktrees/repo/ab-123-b",
            Path::new("packages/frontend/src"),
        );
        assert_eq!(
            result,
            "/home/user/.ccode/worktrees/repo/ab-123-b/packages/frontend/src"
        );
    }
    #[test]
    ...
    fn test_effective_cwd_roundtrip_with_compute_offset() {
        ...
        let worktree_root = "/home/user/.ccode/worktrees/myrepo/ab-test-a";
    ...
    fn normalize_ssh_scp_url() {
        assert_eq!(
            normalize_repo_url("git@github.com:ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    fn normalize_https_url() {
        assert_eq!(
            normalize_repo_url("https://github.com/ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_and_https_produce_same_result() {
        let ssh = normalize_repo_url("git@github.com:ccode-org/example.git");
        let https = normalize_repo_url("https://github.com/ccode-org/example.git");
        assert_eq!(ssh, https);
    }
    #[test]
    fn normalize_https_without_git_suffix() {
        assert_eq!(
            normalize_repo_url("https://github.com/ccode-org/example"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    fn normalize_https_with_credentials() {
        assert_eq!(
            normalize_repo_url("https://x-access-token:secret@github.com/ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_scheme_url() {
        assert_eq!(
            normalize_repo_url("ssh://git@github.com/ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_scheme_with_port() {
        assert_eq!(
            normalize_repo_url("ssh://git@github.com:22/ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    ...
    fn normalize_http_url() {
        assert_eq!(
            normalize_repo_url("http://github.com/ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    ...
    fn normalize_git_plus_ssh_scheme() {
        assert_eq!(
            normalize_repo_url("git+ssh://git@github.com/ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    fn normalize_git_plus_https_scheme() {
        assert_eq!(
            normalize_repo_url("git+https://github.com/ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    fn normalize_scp_no_user() {
        assert_eq!(
            normalize_repo_url("github.com:ccode-org/example.git"),
            Some("github.com/ccode-org/example".into()),
        );
    }
    #[test]
    ...
    fn resolve_normalized_remote_urls_deduplicates_across_transports() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = git2::Repository::init(tmp.path()).unwrap();
        repo.remote("origin", "git@github.com:ccode-org/example.git")
            .unwrap();
        repo.remote("https-mirror", "https://github.com/ccode-org/example.git")
            .unwrap();
        let urls = resolve_normalized_remote_urls(tmp.path());
        assert_eq!(urls, vec!["github.com/ccode-org/example"]);
    }
}
#[cfg(test)]
...
mod restore_code_tests {
    ...
    async fn stash_before_destructive_op_dirty_tracked_returns_ref() {
        ...
        assert!(
            list.contains("ccode: pre-test sess-2"),
    ...
    fn restore_code_checkout_allowed_worktree_cwd_is_allowed() {
        let worktrees = Path::new("/home/u/.ccode/worktrees");
        assert!(restore_code_checkout_allowed_in(
            Path::new("/home/u/.ccode/worktrees/home-u-repo/2026-05-22-9f2e51ce"),
            Some("/home/u/repo"),
            worktrees,
        ));
    }
    ...
    fn restore_code_checkout_allowed_source_repo_with_worktree_session_is_refused() {
        ...
        assert!(!restore_code_checkout_allowed_in(
            Path::new("/home/u/repo"),
            Some("/home/u/.ccode/worktrees/home-u-repo/2026-05-22-9f2e51ce"),
            worktrees,
        ));
    }
    #[test]
