...
//!
//! Moved from `ccode-shell/src/session/worktree.rs` into the workspace
...
use ccode_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
// Canonical in ccode-workspace-types; re-exported for existing paths.
pub use ccode_workspace_types::rpc::worktree::{
...
const WORKTREE_LOG: &str = "ccode_worktree";

/// Map a [`WorktreeType`] to the fast-worktree crate's `CreationMode`.
pub(crate) fn to_creation_mode(t: WorktreeType) -> ccode_worktree::CreationMode {
    match t {
        WorktreeType::Linked => ccode_worktree::CreationMode::Linked,
        WorktreeType::Standalone => ccode_worktree::CreationMode::Standalone,
        WorktreeType::Git => ccode_worktree::CreationMode::GitCheckout,
    }
}
...
/// Resolve the ccode home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`ccode_worktree::resolve_ccode_home`), so checkout dirs and
/// the metadata DB always live under the same `.ccode` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `ccode_config::ccode_home()`,
/// so worktree paths also agree with trust/hooks and other ccode-home paths.
fn ccode_home() -> std::path::PathBuf {
    ccode_worktree::resolve_ccode_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.ccode/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    ccode_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.ccode/worktrees/<repo_name>`)
/// for a given source path, correctly handling ccode-managed worktrees.
///
/// When `source_path` is already under `~/.ccode/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the ccode worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = ccode_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.ccode/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the ccode-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.ccode/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to ccode_worktree::WorkingTreeMode
    ...
    // Use ccode-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(ccode_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        ccode_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        ccode_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        ccode_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        ccode_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        ccode_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.ccode/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        ccode_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use ccode-worktree -- it handles copying from any worktree path
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(ccode_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
...
///
/// ACP equivalent of `ccode -w -r <session_id>` (optionally with `--ref`).
...
/// Response from `ccode.dev/git/worktree/resume_session`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response from `ccode.dev/session/rehydrate`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
use ccode_worktree::{
    ...
    rebuild_worktree_db, resolve_ccode_home, resolve_worktree_auto_gc_from_layers,
};
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &ccode_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use ccode_config_types::WorktreeKindMaxAge;
...
/// Env + `$CCODE_HOME/config.toml` only — **`remote=None` is intentional**.
///
...
/// processes; use `CCODE_WORKTREE_AUTO_GC=0` / `CCODE_WORKTREE_AUTO_GC_DRY_RUN=1`
/// or local TOML until remote is plumbed into `make_workspace_handle`.
fn resolve_worktree_auto_gc_local() -> ccode_worktree::ResolvedWorktreeAutoGc {
    use ccode_config_types::WorktreeAutoGcSettings;

    let local = if let Ok(home) = resolve_ccode_home() {
...
pub fn worktree_db_rebuild() -> Result<ccode_worktree::RebuildReport> {
    let home = resolve_ccode_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = ccode_worktree::ListFilter {
...
/// Scan `~/.ccode/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[ccode_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/ccode/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/ccode/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/ccode/subagents/standalone-1";
    ...
    async fn snapshot_failure_preserves_worktree() {
        ...
        let result = snapshot_and_remove_subagent_worktree(
            &not_a_worktree,
            &not_a_worktree,
            "refs/ccode/subagents/dispose-3",
        )
        .await;
    ...
    /// Point `CCODE_HOME` at an isolated tempdir (`resolve_ccode_home` re-reads
    ...
    /// and restores `CCODE_HOME` on drop (before releasing the lock), so the
    ...
    fn worktree_db_fixture(
        ...
        let env = LockedTestEnv::lock().set("CCODE_HOME", &home);
        ...
        let record = WorktreeRecord {
            ...
            status: ccode_worktree::WorktreeStatus::Alive,
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (CcodeHomeFixture is not visible across crates).
