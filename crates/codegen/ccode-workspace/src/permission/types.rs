...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "ccode-shell", alias = "ccode_shell")]
    Generic,
    /// Ccode TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "ccode-tui", alias = "ccode_tui")]
    CcodeTUI,
    /// Ccode Web client - identified by clientIdentifier "ccode-web"
    #[serde(rename = "ccode_web")]
    CcodeWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "ccode-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Ccode Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to CcodeTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"ccode-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"ccode_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "ccode-pager", alias = "ccode_pager")]
    CcodePager,
    /// Ccode Desktop (Electron) client - identified by clientIdentifier "ccode-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "ccode_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `ccode-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "ccode-shell",
            Self::CcodeTUI => "ccode-tui",
            Self::CcodeWeb => "ccode-web",
            Self::Nebula => "nebula",
            Self::Extension => "ccode-code-extension",
            Self::CcodePager => "ccode-pager",
            Self::Desktop => "ccode-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"ccode-web"`, `"ccode-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("ccode-web") => Self::CcodeWeb,
            Some("nebula") => Self::Nebula,
            Some("ccode-code-extension") => Self::Extension,
            Some("ccode-desktop") => Self::Desktop,
            Some("ccode-pager") => Self::CcodePager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::CcodeTUI | Self::CcodePager => "tui",
            Self::CcodeWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&ccode_tools::types::ToolInput> for AccessKind {
    fn from(input: &ccode_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.ccode/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use ccode_tools::implementations::ccode_build_hashline::edit::types::HashlineEditInput;
        use ccode_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use ccode_tools::implementations::ccode_build::bash::BashToolInput;
        use ccode_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use ccode_tools::implementations::use_tool::UseToolInput;
        use ccode_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use ccode_tools::implementations::ccode_build::monitor::types::MonitorInput;
        use ccode_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use ccode_tools::implementations::ccode_build::web_fetch::WebFetchInput;
        use ccode_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use ccode_tools::implementations::ccode_build::web_search::WebSearchInput;
        use ccode_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_ccode_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("ccode-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("ccode_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
