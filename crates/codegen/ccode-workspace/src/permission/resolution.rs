//! Permission resolution engine: merges native `.ccode/config.toml`,
...
/// (fail-safe) with a warn + skip record for `ccode inspect`.
...
/// user `~/.ccode` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    ccode_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Load `[permission]` rules from native Ccode TOML config files:
///
///   * `~/.ccode/config.toml` (lowest priority)
///   * Each `.ccode/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.ccode/config.toml` first (lowest priority within this layer).
    // Gated on user_ccode_home() so a project's .ccode/config.toml is never read as
    // global permissions when neither CCODE_HOME nor a home dir resolves.
    if let Some(global_path) = ccode_config::user_ccode_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match ccode_config::load_config_file(&global_path) {
    ...
    // `.ccode/config.toml` (same gate as project `.claude/settings.json`).
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match ccode_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[ccode_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Ccode and Claude sources.
...
/// `.ccode/config.toml` permission rules (mirrors [`load_claude_env_with_project`]).
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `ccode inspect`.
...
struct ResolveInputs<'a> {
    ...
    /// `.claude/settings.json` / `.ccode/config.toml` permission rules are dropped
    ...
    fn live(project_trusted: bool) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &ccode_config::managed_config_layers(),
            ),
            project_trusted,
        }
    }
...
/// bypass is pinned off via ccode `requirements.toml`
...
///
/// `project_trusted` gates project-tier Claude settings and `.ccode/config.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `ccode inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in claude_settings_paths_for_trust(cwd, project_trusted) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `ccode inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `ccode inspect` via the outer resolver.
...
/// [`ccode_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = ccode_config::claude_managed_settings_path() else {
...
/// not consulted: ccode must not inherit a host-wide always-approve lockdown from
/// that file. ccode still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in ccode use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = ccode_config::requirements_layers();
...
/// without `~/.ccode`); `path` only names the layer in a non-bool warning.
...
/// Namespace prefix for managed (ccode.dev-injected) MCP server names. Defined
/// here (shell depends on workspace) and re-exported by shell's `to_managed_name`
/// so the prefix and policy matching never drift.
pub const MANAGED_MCP_PREFIX: &str = "ccode_com_";

/// Max `char` length of a managed runtime name (`ccode_com_` + normalized display
...
///
/// Both sides reduce to one key (strip `ccode_com_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `CCODE_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // CCODE_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("CCODE_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_CCODE_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate CCODE_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate CCODE_HOME so the claude-import marker reads clean (an
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "ccode_com_foo"));
        assert!(mcp_name_matches("ccode_com_foo", "foo"));
        assert!(mcp_name_matches("ccode_com_foo", "ccode_com_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "ccode_com_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "ccode_com_slack"));
        assert!(mcp_name_matches("My Server", "ccode_com_my_server"));
        assert!(mcp_name_matches("ccode_com_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("ccode_com_", "ccode_com_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        let managed = http_named("ccode_com_foo", "https://foo.example.com/mcp");
        assert!(al.is_server_denied(&managed));
        assert!(!al.is_server_allowed(&managed));

        // Name match is transport-agnostic.
        let stdio = stdio_named("ccode_com_foo", "npx");
        ...
        for unrelated in ["foobar", "ccode_com_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&http_named("ccode_com_foo", "https://evil.example.com/x")));
        assert!(al.is_server_allowed(&stdio_named("ccode_com_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("ccode_com_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "ccode_com_foo" } ]
        }));

        assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
        assert!(al.is_server_denied(&http_named("ccode_com_foo", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("foobar", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("ccode_com_foobar", "https://x.example.com/mcp")));
    }
    ...
    fn untrusted_project_claude_permissions_are_not_honored() {
        ...
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("HOME", home.path());
        let _ccode_guard = EnvVarGuard::set("CCODE_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_CCODE_CLAUDE_MARKER_OVERRIDE");
    ...
    /// Untrusted clone must not contribute project `.ccode/config.toml` [permission].
    ///
    /// Sync + `block_on` so `ENV_LOCK` is not held across `.await` (clippy
    /// `await_holding_lock`). Does not assert exact global rule counts:
    /// `ccode_config::ccode_home()` is a process-wide `OnceLock`, so under
    /// single-process `cargo test` an earlier test may have already pinned
    /// `CCODE_HOME`. Project-rule filtering is independent of that; global
    /// survival is checked only when our temp home is the live `user_ccode_home()`.
    #[test]
    fn untrusted_project_config_toml_permissions_are_not_honored() {
        ...
        // Global allow (survives untrusted project when CCODE_HOME resolves here).
        std::fs::write(
...
"#,
        ...
        let rt = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .expect("test runtime");
        // Untrusted may be None when no global rules load (CCODE_HOME OnceLock
        ...
        let global_live = ccode_config::user_ccode_home()
            .is_some_and(|g| g == home.path() || g.starts_with(home.path()));
        if global_live {
            let untrusted = untrusted.expect("global rules present when CCODE_HOME is live");
            assert!(
                untrusted
                    .config
                    .rules
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/ccode/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/ccode/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.ccode/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/ccode/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.ccode/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/ccode/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/ccode.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for ccode inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            ccode_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            ccode_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
