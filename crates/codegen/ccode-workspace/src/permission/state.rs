...
pub struct PermissionState {
    ...
    /// Exact MCP tool names (e.g. `"ccode_com_notion__notion-fetch"`)
    /// the user has granted "always allow" for. Lookup is exact.
    pub allowed_mcp_tools: HashSet<String>,
    /// Server components of valid qualified MCP IDs (e.g. `"ccode_com_notion"`)
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    ccode_config::sessions_cwd_dir(cwd.as_str())
}
...
async fn try_load_state(path: &std::path::Path) -> Option<PermissionState> {
    try_load_state_with_writer(path, |path, contents| {
        ccode_config::fs_atomic::write_atomically(path, contents, None)
    })
    .await
}
...
pub async fn cleanup_stale_permission_state(max_age: std::time::Duration) {
    let sessions_dir = ccode_home().join("sessions");
...
mod tests {
    ...
    fn roundtrip_with_allowed_mcp_tools() {
        ...
        state
            .allowed_mcp_tools
            .insert("ccode_com_notion__notion-fetch".to_string());
        state
            .allowed_mcp_tools
        ...
        assert!(
            restored
                .allowed_mcp_tools
                .contains("ccode_com_notion__notion-fetch")
        );
    ...
    fn roundtrip_with_allowed_mcp_servers() {
        ...
        state
            .allowed_mcp_servers
            .insert("ccode_com_slack".to_string());
        ...
        assert!(restored.allowed_mcp_servers.contains("ccode_com_slack"));
