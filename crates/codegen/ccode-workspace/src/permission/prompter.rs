...
use ccode_fs::events::{Event, EventWriter, PermissionDecision};
use ccode_mcp::servers::parse_mcp_qualified_name;
use ccode_tools::implementations::ccode_build::web_fetch::domain_from_url;

const REJECT_ONCE_LABEL: &str = "No, and tell Ccode what to do differently";
...
///        `~/.ccode/config.toml` via the `Effect::PersistPermissionMode` effect
///     4. Sends the existing `ccode.dev/yolo_mode_changed` ACP notification so
...
/// (global yolo) option that is prepended for CcodeTUI / CcodePager / Desktop.
///
...
/// (`CcodeTUI`, `CcodePager`, `Desktop`) that wire the option id through
...
fn client_supports_enable_always_approve(client_type: ClientType) -> bool {
    matches!(
        client_type,
        ClientType::CcodeTUI | ClientType::CcodePager | ClientType::Desktop
    )
}
...
/// Canonical definition lives in `ccode_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `ccode-mcp` can
...
/// reached `ccode_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
/// `"list_issues"` → `"List Issues"`, `"ccode_com_notion"` →
/// `"Ccode Com Notion"`, and `"getMyTaskList"` → `"GetMyTaskList"`.
...
pub struct McpToolPermission {
    ...
    /// (e.g. `"ccode_com_notion__notion-fetch"`).
    pub tool_name: String,
    /// Server component of a valid qualified MCP ID (e.g. `"ccode_com_notion"`).
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for CcodeTUI - interactive selection with expandable/contractable terms
    ...
    fn build_options_inner(
        ...
        match access {
            ...
            AccessKind::Bash(bash_command) => {
                // For CcodeTUI clients, use the fancy interactive options with term selection
...
mod tests {
    ...
    fn gate_off_strips_bash_always_allow_keeps_yes_no() {
        let p = prompter_with_gate(ClientType::CcodePager, false);
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::CcodePager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn gate_off_strips_generic_bash_always_and_reject_always() {
        let p = prompter_with_gate(ClientType::CcodeWeb, false);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::CcodePager, true);
    ...
    fn bash_reject_always_command_maps_selected_words() {
        let p = prompter(ClientType::CcodePager);
    ...
    fn mcp_prompt_includes_allow_always_with_meta() {
        let p = prompter(ClientType::CcodeTUI);
        for (name, server) in [
            ("linear__list", "linear"),
            ("123__lookup", "123"),
    ...
    fn mcp_fallback_client_returns_plain_allow_always() {
        // non-TUI clients (Generic / CcodeWeb / Extension / …) see `fallback_options`. The
    ...
    fn mcp_titleize_segment_handles_snake_camel_kebab() {
        ...
        assert_eq!(mcp_titleize_segment("ccode_com_notion"), "Ccode Com Notion");
    ...
    /// Same pin as above for `CcodeTUI` and `Desktop` — both client types
    ...
    fn enable_always_approve_is_first_for_tui_and_desktop() {
        for ct in [ClientType::CcodeTUI, ClientType::Desktop] {
            let p = prompter(ct);
