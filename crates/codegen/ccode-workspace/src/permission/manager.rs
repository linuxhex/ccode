...
use ccode_tools::implementations::ccode_build::web_fetch::{
    DomainMatcher, config::DEFAULT_ALLOWED_DOMAINS, domain::normalize_domain,
};
use ccode_tools::types::resources::resolve_model_path;
...
fn permission_mode_artifact_str(mode: ccode_telemetry::enums::PermissionMode) -> &'static str {
    use ccode_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.ccode/sessions/<cwd>/permission.toml. This caused
        // the allow to survive full restarts (new ccode process, new agent
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            ccode_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        ...
                        _ => {
                            ...
                            let (decision, outcome_str) = match &prompt_outcome {
                                ...
                                PromptOutcome::AllowAlways => {
                                    // Fallback clients (Generic / CcodeWeb /
...
mod tests {
    ...
    /// always-approve option is only offered for `CcodeTUI | CcodePager | Desktop`).
    ...
    fn manager_with_recording_client_remember(
        ...
        let (gateway, receiver) = ccode_acp::acp_gateway::<acp::AgentSide, _>(client);
    ...
    async fn auto_classifier_block_denies_then_escalates_to_prompt() {
        ...
        local
            .run_until(async {
                ...
                let client = RecordingClient::default();
                let prompts = client.prompts.clone();
                // CcodePager wires the always-approve option through to its YOLO
                // toggle; it is the option set the auto path prompts under.
                let (mgr, _e) =
                    manager_with_recording_client(&cwd, None, client, ClientType::CcodePager);
                mgr.set_auto_mode(true);
                mgr.set_classifier(Some(LlmPermissionClassifier::with_fixed_model_text(
