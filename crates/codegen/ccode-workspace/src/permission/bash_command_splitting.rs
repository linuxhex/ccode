...
///
/// ```bash
/// CCODE_API_KEY='ccode-some-key' cargo run --bin ccode-pager
/// ```
///
...
///   prefix: ["CCODE_API_KEY=ccode-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "ccode-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "CCODE_API_KEY='ccode-some-key' cargo run --bin ccode-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["CCODE_API_KEY=ccode-some-key".to_owned()],
                highlighted_words: vec![
                    "cargo".to_owned(),
                    "run".to_owned(),
                    "--bin".to_owned(),
                    "ccode-pager".to_owned()
                ],
                suffix: vec![],
            })
        );
        ...
        let redirection_command = "cargo build --bin ccode-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package ccode-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
