...
use std::sync::Arc;
use ccode_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    pub fn local(backend: ccode_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn ccode_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub viewer_ctx: Option<ccode_tool_runtime::WorkspaceViewerContext>,
    ...
    pub tools: Option<Vec<ccode_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(ccode_tools::registry::proto_convert::ToolConfigEntryError),
}
...
impl WorkspaceBindConfig {
    ...
    /// [`ccode_tool_runtime::WorkspaceBindMetadata`] (same type the emitter
    /// serializes); `tool_config` is a consumer-only raw escape hatch read
    /// separately.
    pub fn from_metadata(metadata: &serde_json::Value) -> Self {
        let wire: ccode_tool_runtime::WorkspaceBindMetadata =
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = ccode_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match ccode_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    running_version = ccode_version::VERSION,
...
mod bind_config_tests {
    ...
    fn presets_are_never_resolved() {
        for preset in ["explore", "ccode-computer", "bogus"] {
    ...
    fn tools_entries_resolve_to_tool_server_config() {
        ...
        assert_eq!(grep.id, "CcodeBuild:grep");
        assert_eq!(
            grep.params,
        ...
        assert_eq!(toolset.tools[1].id, "CcodeBuild:read_file");
    }
    #[test]
...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.ccode/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.ccode/hooks/`).
    ...
    /// Auth provider for ccode service calls made from workspace-scoped code.
    ...
    /// `<cwd>/.ccode/lsp.json`: `false` drops them at load, `true` keeps them. The
    ...
    /// Confine `ccode.dev/fs/*` / `workspace.fs_*` resolution to the workspace root
...
pub struct WorkspaceServerMetadata {
    ...
    /// Logical sandbox-service session UUID, from the `CCODE_SESSION_ID` env
...
impl WorkspaceServerMetadata {
    ...
    ///
    /// `env_session_id` is the raw `CCODE_SESSION_ID`; empty is normalized to
...
impl WorkspaceConfig {
    ...
    pub fn new_for_proxy(
        ...
        auth_provider: ccode_hub_sdk::SharedAuthProvider,
...
/// `*.json` hook files. Maps 1:1 to [`ccode_hooks::discovery::HookSource`]
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.ccode/hooks/`).
    Directory(PathBuf),
}
