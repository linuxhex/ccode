...
use ccode_mcp::rmcp;
use ccode_mcp::servers::{McpClient, parse_mcp_qualified_name};
use ccode_tool_protocol::ToolId;
use ccode_tool_runtime::{ToolCallContext, ToolStream, TypedToolOutput};
...
impl McpTransport for McpClientTransportAdapter {
    async fn initialize(&self) -> Result<McpServerInfo, ccode_hub_mcp::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
            .map_err(|e| ccode_hub_mcp::McpError::Transport(e.to_string()))?;
        let info = service.peer_info().ok_or_else(|| {
            ccode_hub_mcp::McpError::Transport("no peer info after init".into())
        })?;
        Ok(McpServerInfo {
    ...
    async fn list_tools(
        &self,
    ) -> Result<Vec<McpToolDefinition>, ccode_hub_mcp::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn call_tool(
        ...
        name: &str,
        arguments: Value,
    ) -> Result<McpCallResult, ccode_hub_mcp::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn close(&self) -> Result<(), ccode_hub_mcp::McpError> {
...
/// Extract a server name from an [`McpError`](ccode_mcp::servers::McpError),
/// falling back to `"unknown"`.
pub(crate) fn server_name_from_mcp_error(e: &ccode_mcp::servers::McpError) -> &str {
    e.server_name().unwrap_or("unknown")
}

/// Bridge config factory for MCP bridge connections.
pub(crate) fn make_bridge_config(
    session_id: ccode_tool_protocol::SessionId,
    server_name: &str,
) -> McpBridgeConfig {
    McpBridgeConfig {
        session_id,
...
mod tests {
    use super::*;
    use ccode_hub_mcp::{McpBridge, McpError};
