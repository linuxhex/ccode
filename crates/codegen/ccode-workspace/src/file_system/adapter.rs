//! AcpFsAdapter: implements `ccode-tools::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates ccode-tools' `AsyncFileSystem` trait into ACP protocol calls:
...
use ccode_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps ccode-shell's ACP gateway to satisfy ccode-tools' AsyncFileSystem.
///
