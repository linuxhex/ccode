...
use ccode_graph::{IndexManager, IndexManagerConfig, IndexManagerHandle};

use ccode_tools::util::ccode_home::ccode_home;

/// Get the cache path for a cwd's index.
///
/// Cache is stored in: `~/.ccode/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `ccode.dev/code/status` reporting `reason: notStarted`.
    ...
    /// In `ccode.dev/code/status`, `resolve_index_handle()` calls
