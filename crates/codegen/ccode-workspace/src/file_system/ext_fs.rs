...
//! for the shell's `ccode.dev/fs/*` ACP extension methods.
//!
//! These mirror the pure functions that previously lived only in the
//! shell (`ccode-shell/src/session/file_system.rs`) so that, in proxy
//! mode, a `ccode.dev/fs/*` request executes on the *remote* workspace server
...
use ccode_workspace_types::rpc::fs::FsReadEncoding;
