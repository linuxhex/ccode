...
pub use ccode_workspace_client::consume_stream_terminal;

/// Extract a `WorkspaceEvent` from a custom `ToolNotificationFrame`.
pub fn extract_workspace_event(
    frame: &ccode_tool_protocol::ToolNotificationFrame,
) -> Option<WorkspaceEvent> {
    use ccode_tool_protocol::WireToolNotification;
...
mod tests {
    use super::*;
    use ccode_tool_protocol::{ToolId, ToolNotificationFrame, WireToolNotification};
    ...
    // consume_stream_terminal tests live in ccode-workspace-client.
}
