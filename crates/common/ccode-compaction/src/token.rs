...
//!
//! - Ccode chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `CcodeTurn::get_num_tokens`.
//! - ccode-build estimates with `bytes / 4`.
//!
...
//! (Ccode chat: `ItemTokenCounter<Arc<CcodeTurn>>`), so exactly one mechanism
//! is in play.
