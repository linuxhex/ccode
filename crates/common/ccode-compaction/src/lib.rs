...
//! stay in each product host (for example `ccode-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `ccode-sampling-types`. It is decoupled from both Ccode chat and
//! ccode-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — ccode-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Ccode chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Ccode chat's chunked, between-turn pass.
//!
...
// ccode-build's full-replace subsystem now lives under `code_compaction`;
