//! Compacted-history assembly (ccode-build's rebuild structure, generic).
//!
//! Moved from `ccode-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! ccode-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // ccode-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn ccode_build_order_recent_before_summary() {
