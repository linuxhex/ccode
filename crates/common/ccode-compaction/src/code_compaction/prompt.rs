//! ccode-build's session-level summarization prompt.
//!
//! Split out of the crate-root `prompt` module so ccode-build's full-replace
//! prompt lives alongside the rest of its [`code_compaction`](crate::code_compaction)
//! subsystem. The Ccode chat's step-level intra prompt
//! ([`format_compaction_prompt`](crate::prompt::format_compaction_prompt))
//! stays at the crate root.

/// Build ccode-build's session-level summarization prompt (no chat history).
///
...
/// `ccode-shell::session::helpers::session_compact::build_compaction_prompt`
...
/// (mirrors `ccode-shell`'s `SELF_SUMMARIZATION_PROMPT`). Framed
...
/// so the structured (ccode-build) and short self-summary prompts stay
/// shared instead of duplicated per harness.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SummaryPromptKind {
    /// ccode-build's detailed, numbered-section summary prompt.
    #[default]
    Structured,
...
mod tests {
    ...
    fn kind_structured_matches_build_summary_prompt() {
        ...
        // so routing through the selector never changes ccode-build's prompt.
        assert_eq!(
