//! Observability seam for the full-replace (ccode-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (ccode-build:
...
//!
//! Emission points are part of the behavior contract: the ccode-build observer
