...
//!
//! The canonical `sample → classify → retry` loop, used by **both** ccode-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Ccode chat's intra `Shared` summarizer
...
//! the winning summary (ccode-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the ccode-build host uses to step down its input size).
