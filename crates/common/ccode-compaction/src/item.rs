...
//! harness implements [`CompactionItem`] for its `CcodeTurn`;
//! ccode-build implements it for `ccode_sampling_types::ConversationItem`.
//!
...
//! `<ccode_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `CcodeRole` (Ccode chat) and the
/// `ConversationItem` variants (ccode-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Ccode chat) — maps to System on harnesses without a
...
/// extraction (`<ccode_file id=".." name=".." />` lines in the
/// `<ccode_user_queries>` preamble).
...
///
/// Implementors:
/// - Ccode chat: `CcodeTurn`
/// - ccode-build: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Ccode chat's
    /// `CcodeTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Ccode chat: a
    ...
    /// strips their `<ccode_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<ccode_file>` lines in the
    /// `<ccode_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Ccode chat: a `Developer` turn with category
    ...
    /// requests/results (Ccode chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up ccode-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Ccode chat's
/// `CcodeTurn`; adding these constructors to it as required methods would break
/// that impl. They are also ccode-build-specific (Ccode chat's tail-keep path
...
///
/// The ccode-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Ccode chat stores turns as `Arc<CcodeTurn>`).
