...
// The `Shared` summarizer reuses ccode-build's full-replace summarization core
...
/// `FullReplace` strategy (default): ccode-build's full-replace — summarize the
...
/// `<ccode_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching ccode-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through ccode-build's shared core.
    ...
    //     Shared with Ccode chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<ccode_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<ccode_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<ccode_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): ccode-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<ccode_user_queries>` preamble so
...
/// — ccode-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and ccode-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // ccode-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // ccode-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Ccode chat's
...
/// the literal error messages produced by the Ccode chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Ccode chat sampler when the
    ...
    /// — guards the forwarding that Ccode chat (`Arc<CcodeTurn>`) relies on.
    #[test]
