...
///
/// - `FullReplace` (default): ccode-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from ccode-shell's
...
/// time (token threshold only, matching ccode-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as ccode-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<ccode_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "ccode-4.20";
