...
pub enum CompactionTarget {
    ...
    /// with a single summary — ccode-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by Ccode chat's
/// `StreamProcessor` (`Item = Arc<CcodeTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<CcodeTurn>`) already satisfies it.
