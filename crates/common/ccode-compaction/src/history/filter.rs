...
//!
//! Behavior is byte-for-byte identical for Ccode chat (`T = Arc<CcodeTurn>`).
...
///   summaries so their `<ccode_user_queries>` sections can be split out.
...
/// Extracts **all** `<ccode_user_queries>...</ccode_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<ccode_user_queries>";
    let end_tag = "</ccode_user_queries>";
...
/// Extract a `<ccode_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<ccode_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<ccode_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<ccode_user_queries>\n");
    ...
    result.push_str("</ccode_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<ccode_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<ccode_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<ccode_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<ccode_user_queries>` block.
...
/// `<ccode_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<ccode_user_queries>` from earlier rounds.
...
///   would see the prior `<ccode_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<ccode_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<ccode_user_queries>"));
        assert!(out.ends_with("</ccode_user_queries>"));
        assert!(out.contains("<ccode_query>hello world</ccode_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<ccode_user_queries>\n<ccode_query>first</ccode_query>\n</ccode_user_queries>";
        let inner2 = "<ccode_user_queries>\n<ccode_query>second</ccode_query>\n</ccode_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<ccode_user_queries>\n<ccode_query>Q1</ccode_query>\n</ccode_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<ccode_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<ccode_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries = "<ccode_user_queries>\n<ccode_query>Q</ccode_query>\n</ccode_user_queries>";
